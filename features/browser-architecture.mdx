...
Browser code is split across three tiers so the core SDK stays lightweight.

```mermaid
graph LR
    Protocol[📋 praisonaiagents<br/>protocol types] --> Browser[⚙️ praisonai-browser<br/>implementation]
    Browser --> Wrapper[🤖 praisonai<br/>thin wrapper / shim]
...
|------|---------|----------------|
| Protocol | `praisonaiagents.tools.protocols.browser` | Type definitions only (`BrowserAction`, `BrowserObservation`, …) |
| Implementation | `praisonai-browser` | CLI, server, CDP/Playwright agents, sessions |
| Wrapper | `praisonai.browser` | Compatibility shim re-exporting `praisonai_browser` |

## Component Overview

| Component | Location | Purpose |
|-----------|----------|---------|
| **CLI** | `praisonai_browser/cli/commands/browser.py` | Entry point, command routing |
| **Server** | `praisonai_browser/server.py` | WebSocket server, message routing |
| **Agent** | `praisonai_browser/agent.py` | LLM calls, action decisions |
| **Protocol bridge** | `praisonai_browser/_protocol_bridge.py` | dict ↔ typed protocol conversion |
