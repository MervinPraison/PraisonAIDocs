...
**Symptom:** `ImportError: No module named 'praisonai.browser'` (or `praisonai_browser`).

**Root Cause:** Browser automation now ships in the standalone `praisonai-browser` package, which is not present in your environment.
...
`pip install "praisonai[all]"` also pulls it in. In monorepo dev layouts the wrapper's `ensure_praisonai_browser()` bootstrap helper adds the sibling `praisonai-browser` source to `sys.path` automatically.
...
### 5. "Extension did not connect within 15s"

**Symptom:** A run aborts with "Extension did not connect to bridge server within 15s".

**Root Cause:** No Chrome extension connected to the bridge before the readiness poll timed out. The poll caps at `min(15.0, timeout)` seconds.

**Fix:** Load the extension and confirm it connects:
```bash
praisonai browser doctor extension
```

For **unit tests or mocked bridges** (no live extension), skip the poll:
```bash
export PRAISONAI_BROWSER_SKIP_EXTENSION_WAIT=1
```

Accepted truthy values are `1`, `true`, `yes`, `on` (case-insensitive).

> [!WARNING]
> Set `PRAISONAI_BROWSER_SKIP_EXTENSION_WAIT` only for tests or mocked bridges — never for live runs. It bypasses the readiness check, so a run without a connected extension fails at `start_session` instead of erroring early.

### 6. Model Doesn't Support Vision
...
### 7. "selector matched no element: …"

**Symptom:** A CDP step returns
`{"success": false, "error": "selector matched no element: <selector>"}`
and the LLM either retries with a new selector or gives up.

**Root cause:** The CSS selector chosen by the LLM did not match any element on the page. This is the correct behaviour introduced in PraisonAI #4312 — before the fix, this failure was reported as success, so `--max-retries` and every alternative-selector strategy silently did nothing.

**When it self-heals:** `--engine cdp` (or `hybrid`) with `--max-retries >= 1` will try text-match, fallback-selector, and click→navigate strategies before propagating the failure. Bump `--max-retries` if the page is slow to hydrate.

**When to intervene:** If the same missing selector repeats step after step, the page structure has changed since the LLM's prompt was built. Options: rerun with `--debug` to see the observation the LLM saw, tighten the goal to name a stable element (`aria-label`, `data-testid`), or precede the goal with an explicit `wait_for_selector`.

**Related:** for Playwright mode, the equivalent surface is `Selector not found` (already documented in the Deep Dive).
