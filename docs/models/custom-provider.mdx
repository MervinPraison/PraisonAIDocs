...
Following [PR #4800](https://github.com/MervinPraison/PraisonAI/pull/4800), the local routes `ollama`, `ollama_chat`, `lm_studio`, `vllm`, `hosted_vllm` are also auto-registered — `create_llm_provider('ollama/llama3')` now works with no setup.
</Note>
...
| `ollama` | — | LiteLLM (local Ollama server) |
| `ollama_chat` | — | LiteLLM (Ollama with tool-calling prefix) |
| `lm_studio` | — | LiteLLM (LM Studio local server) |
| `vllm` | — | LiteLLM (self-hosted vLLM) |
| `hosted_vllm` | — | LiteLLM (OpenAI-compatible vLLM server) |

<Note>
The adapter layer (`Agent(llm="…")`) also recognises `llamacpp/…` and `llama_cpp/…` and routes them to a `LocalOpenAIAdapter` with tool-call repair enabled — see [OpenAI-Compatible Endpoints → Local prefix route](/models/openai-compatible#local-prefix-route-tool-call-repair). These are adapter-only aliases; they are not registered in `praisonai.llm_providers`, so `create_llm_provider("llamacpp/…")` will raise. Use `Agent(llm="llamacpp/…")` instead.

Since PR [#4870](https://github.com/MervinPraison/PraisonAI/pull/4870), `llama_cpp/<model>` (underscore variant) is transparently rewritten to `openai/<model>` at request time — the `LocalOpenAIAdapter` is still selected because adapter selection reads the original prefix.
</Note>

```python
from praisonaiagents import Agent

# No setup needed - the ollama/ prefix is built-in.
agent = Agent(
    name="Local Assistant",
    instructions="You run locally via Ollama",
    llm="ollama/llama3.2",
)
agent.start("Explain what a local LLM is in one sentence.")
```
...
The five local routes added in [PR #4800](https://github.com/MervinPraison/PraisonAI/pull/4800) also build directly — no `register_llm_provider()` needed:

```python
from praisonai.llm import create_llm_provider

# All five now work out of the box - no register_llm_provider() needed.
create_llm_provider("ollama/llama3")
create_llm_provider("ollama_chat/llama3")           # tool-calling prefix
create_llm_provider("lm_studio/qwen2.5-7b-instruct")
create_llm_provider("vllm/meta-llama/Llama-3.1-8B-Instruct")
create_llm_provider("hosted_vllm/Qwen2.5-7B")       # OpenAI-compatible vLLM
```
...
```mermaid
graph TB
    Start["🔤 model = 'prefix/name'"] --> Q{Prefix built-in?<br/>openai · groq · anthropic ·<br/>google · cohere · ollama · openrouter}
    Q -->|Yes| Fast["⚡ Fast direct-SDK path"]
    Q -->|No| Reg{Registered<br/>provider?}
    Reg -->|Yes| Prov["🔌 Your provider"]
    Reg -->|No| Ladder["🪜 LiteLLM → OpenAI fallback"]
...
class BedrockProvider:
    ...
    async def generate_structured(self, response_model, messages, **kwargs):
        # Return an instance of `response_model` (a Pydantic model).
        ...

register_llm_provider("bedrock", BedrockProvider)
# AutoGenerator now delegates structured completion for "bedrock/..." models
# to your provider, before falling back to LiteLLM/OpenAI.
```
...
<Note>
Registering a provider under a built-in prefix (`openai/`, `groq/`, `cohere/`, `ollama/`, `ollama_chat/`, `lm_studio/`, `vllm/`, `hosted_vllm/`, `anthropic/`, `google/`, `openrouter/`) does **not** hijack the built-in path. Both hot paths gate against a shared `_BUILTIN_MODEL_PREFIXES` set so the built-in ladder always wins for those prefixes. Use a unique namespace (e.g. `mycompany-openai/`) if you need custom routing under an OpenAI-like protocol.

`_BUILTIN_MODEL_PREFIXES` is now imported directly from `praisonai.inc.models` by the auto-generator's provider gate — the previous drift-prone local fallback set was removed, so newly added built-in providers are recognised consistently. If `praisonai.inc.models` is not importable, that surfaces as a hard install error rather than silent behavioural divergence.
</Note>
...
<Tip>
Skip `generate_structured` if you only need chat completion — `PraisonAIModel.get_client()` is enough for framework adapters. Add `generate_structured` when you also need YAML auto-generation (`praisonai --auto ...`) or another generator that produces pydantic models. The emitted YAML now matches your **configured** model — the job-workflow generator no longer hard-codes `gpt-4o-mini` (see [Auto Mode Providers → Job-workflow generation honours your configured model](/features/auto-generator-providers#job-workflow-generation-honours-your-configured-model)).
</Tip>
...
- `ollama` - Auto-registered LiteLLM provider (local Ollama server) — [PR #4800](https://github.com/MervinPraison/PraisonAI/pull/4800)
- `ollama_chat` - Auto-registered LiteLLM provider (Ollama tool-calling prefix) — [PR #4800](https://github.com/MervinPraison/PraisonAI/pull/4800)
- `lm_studio` - Auto-registered LiteLLM provider (LM Studio) — [PR #4800](https://github.com/MervinPraison/PraisonAI/pull/4800)
- `vllm` - Auto-registered LiteLLM provider (self-hosted vLLM) — [PR #4800](https://github.com/MervinPraison/PraisonAI/pull/4800)
- `hosted_vllm` - Auto-registered LiteLLM provider (OpenAI-compatible vLLM) — [PR #4800](https://github.com/MervinPraison/PraisonAI/pull/4800)
- Their aliases: `oai`, `claude`, `gemini`, `google_genai`

<Note>
Re-registering any of them without `override=True` will raise `ValueError: Provider 'X' is already registered`. This now includes the five local routes (`ollama`, `ollama_chat`, `lm_studio`, `vllm`, `hosted_vllm`). Use `override=True` only if you're intentionally replacing the built-in provider.
</Note>
...
### Built-in Names Win Over Entry-Point Plugins

`LLMProviderRegistry` subclasses the base `PluginRegistry`, so since [PraisonAI PR #4176](https://github.com/MervinPraison/PraisonAI/pull/4176) a package cannot ship a `praisonai.llm_providers` entry point named `openai`, `anthropic`, or `google` and expect it to take over. During entry-point discovery, any name whose lowercase form matches a built-in loader is skipped, the built-in is kept, and a `DEBUG` line notes the collision. Matching is case-insensitive (`OpenAI` and `openai` collide).

Runtime registration is the supported override path — it is deliberate dependency injection and still overrides a built-in:
...
- [Models Overview](/models) - Built-in provider configuration
- [LLM Gateways](/features/llm-gateways) - Pre-registered gateway providers
- [Advanced Multi-Provider Patterns](/features/multi-provider-advanced) - Routing across providers
