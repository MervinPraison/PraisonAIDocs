...
| `vertex_ai/gemini-1.5-pro` | Gemini | ✅ supported |
| `openrouter/anthropic/claude-3.5-sonnet` | Anthropic | ✅ supported |
| `openrouter/google/gemini-1.5-pro` | Gemini | ✅ supported |

See [Routed model provider detection](/models#routed-model-provider-detection) for the full rules.
...
```python
# Simplest: pass base_url and api_key at the top level.
# Works with any OpenAI-compatible /v1 host — a bare catalog model id is fine.
from praisonaiagents import Agent
...
See [OpenAI-Compatible Endpoints](/models/openai-compatible) for a full walkthrough.
