...
Some local models answer a tool call as JSON in the response body instead of using the tool-call field. `OllamaAdapter` recovers that JSON and dispatches the tool; other providers (OpenAI, Anthropic, Gemini) do not — they treat a JSON-looking answer as plain text.
