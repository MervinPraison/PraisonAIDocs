...
Some local models answer a tool call as text in the response body instead of using the tool-call field. `OllamaAdapter` recovers that text and dispatches the tool; other providers (OpenAI, Anthropic, Gemini) do not — they treat a JSON-looking answer as plain text.

Since [PraisonAI PR #5215](https://github.com/MervinPraison/PraisonAI/pull/5215), three text dialects are recovered — whole-content JSON, `<tool_call>{...}</tool_call>` blocks, and `<function=name>{...}</function>` blocks — each guarded by an advertised-name allowlist (a hallucinated name never runs) and a fenced-code mask (a call shown as documentation never runs).

Recovery fires on both the sync path (`agent.start()` / `agent.chat()`) and the async path (`agent.astart()` / `agent.achat()`). [PraisonAI PR #4940](https://github.com/MervinPraison/PraisonAI/pull/4940) closed the async-path gap — before that release, the same agent silently stopped calling the tool the moment the caller awaited instead of called. The streaming path does not (yet) recover text-emitted tool calls on any adapter.

See [OpenAI-Compatible → Local prefix route](/models/openai-compatible#local-prefix-route-tool-call-repair) for how this compares across adapters, and [Tool Call Self-Repair → Text-based tool-call recovery](/features/tool-call-self-repair#text-based-tool-call-recovery-local-models) for the full behaviour spec.
...
| Small models emit tool calls as JSON text | Parses the JSON and dispatches the tool call anyway (up to 2 repair attempts) on sync and async response paths |
