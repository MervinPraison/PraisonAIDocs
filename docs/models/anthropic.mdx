...
If you just want the Anthropic default, setting `ANTHROPIC_API_KEY` alone is enough — no `MODEL_NAME`, no `llm=`. `Agent(...)` picks `anthropic/claude-3-5-sonnet-latest` and routes it through litellm. See [Provider Auto-Detection](/models#provider-auto-detection-no-config-first-run).
...
See [Routed model provider detection](/models#routed-model-provider-detection) for the full detection rules.
