...
Text-emitted JSON tool-call recovery — where the model answers with the tool call as JSON in the response body instead of using the tool-call field — is **only** performed for `ollama/…` and `ollama_chat/…`. `LocalOpenAIAdapter` (LM Studio, vLLM, llama.cpp) and `DefaultAdapter` (everyone else) do not attempt this parse; they treat JSON-looking assistant messages as text.
