...
| Prefix | Server | Adapter | `max_tool_repairs` | Text-based tool-call recovery | Applies on |
|---|---|---|---|---|---|
| `lm_studio/…`, `lmstudio/…` | LM Studio | `LocalOpenAIAdapter` | 2 | ✅ | sync, async |
| `vllm/…`, `hosted_vllm/…` | vLLM | `LocalOpenAIAdapter` | 2 | ✅ | sync, async |
| `llamacpp/…`, `llama_cpp/…` | llama.cpp `llama-server` | `LocalOpenAIAdapter` | 2 | ✅ | sync, async |
| `ollama_chat/…` | Ollama (tool-calling) | `OllamaAdapter` | 2 | ✅ | sync, async |
| `ollama/…` | Ollama | `OllamaAdapter` | 2 | ✅ | sync, async |
...
Local-server prefixes (`lm_studio/`, `vllm/`, `hosted_vllm/`, `llamacpp/`, `llama_cpp/`) also inherit **text-based tool-call recovery**: when the model emits a tool call as JSON text — common after a repair prompt on a small local model — the adapter parses it and dispatches the tool call anyway. A hosted OpenAI-compatible endpoint (a `base_url` pointing at a hosted API) deliberately does **not** get this behaviour: a hosted model returning JSON prose must never have it silently interpreted as a tool call. Recovery fires on both sync and async response paths as of [PraisonAI PR #4940](https://github.com/MervinPraison/PraisonAI/pull/4940). Before that release, only `agent.start()` / `agent.chat()` recovered a text-emitted tool call — `agent.astart()` / `agent.achat()` would silently ignore it. The streaming path does not yet have this recovery on any adapter. See [Tool Call Self-Repair → Text-based tool-call recovery](/features/tool-call-self-repair#text-based-tool-call-recovery-local-models).
