...
If you just want the Gemini default, setting `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) alone is enough — no `MODEL_NAME`, no `llm=`. `Agent(...)` picks `google/gemini-1.5-flash` (or `gemini/gemini-1.5-flash`) and routes it through litellm. See [Provider Auto-Detection](/models#provider-auto-detection-no-config-first-run).
