...
<Note>
OpenRouter also supports **video generation** via `VideoAgent` — see [Video → OpenRouter](/video/openrouter).
</Note>
...
<Note>
For LiteLLM Proxy, Custom Gateway, auto-registration, and multi-gateway patterns, see [LLM Gateways](/features/llm-gateways).
</Note>