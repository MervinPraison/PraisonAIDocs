...
<Note>
This page covers **server-side** auth for `praisonai-mcp`. To **connect** an agent to an OAuth-protected remote MCP server, use the zero-config client flow in [OAuth Authentication](./mcp-oauth) — no `client_id` required.
</Note>
...
- [MCP Elicitation](/mcp/mcp-elicitation) - URL mode for OAuth flows
- [PraisonAI MCP Server](/mcp/praisonai-mcp-server) - Full documentation
- [MCP Tasks API](/mcp/mcp-tasks-api) - Protected operations
