...
<Tip>
If Ollama is already running, `praisonai run "..."` works with no API key and no setup — PraisonAI detects the local endpoint automatically. See [Keyless Local-First Run](/features/keyless-local-first-run).
</Tip>
