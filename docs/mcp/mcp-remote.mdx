...
<Note>
`type: remote` servers are served through the Python SDK's MCP client and are not passed via the `praisonai run` command-string path. Local (stdio) servers use the `command` key instead. See [Single-Source Config](/features/single-source-config) for the full schema.
</Note>

### With OAuth

OAuth-protected servers are now zero-config — just declare `type: remote` and `url`. On the first request, PraisonAI discovers the endpoints, registers a client, and opens your browser once to approve. See [OAuth Authentication](./mcp-oauth) for the full flow.
...
Supply an explicit `oauth:` block only as a fallback for servers that don't advertise standard metadata or dynamic client registration:
...
Runs the full OAuth 2.1 flow (PKCE + code exchange) and caches a real bearer token at `~/.praisonai/mcp-auth.json`. See [OAuth Authentication](./mcp-oauth#authenticate) for the full flow.
