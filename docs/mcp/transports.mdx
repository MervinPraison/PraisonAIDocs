...
The `timeout` parameter on `MCP()` (default `60s`) bounds every call issued via the MCP session — tool calls, resource reads (`read_resource`), and prompt fetches (`get_prompt`). It is enforced at two layers:
