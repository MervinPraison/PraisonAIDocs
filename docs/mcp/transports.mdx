...
- **Caller-side:** the response queue also uses `get(timeout=...)`, so a caller that never receives a reply sees `"Error: MCP tool call timed out after {N} seconds"`.

`timeout=` also bounds the initial MCP handshake — `initialize()` waits at most `timeout` seconds before failing with `"Error: MCP initialization timed out after {N} seconds"`.
