...
<Note>
As of C12, the MCP server host ships as the standalone `praisonai-mcp` Tier-2 package. This page documents the heavy MCP host, whether you run it via `praisonai-mcp` (standalone) or `praisonai mcp` (umbrella). Both invocations are equivalent for host commands. See the [praisonai-mcp Package](/features/praisonai-mcp-package) guide and [The Three MCP Layers](/features/mcp-three-layers) for how it fits.

To serve a single recipe as a scoped MCP server with `praisonai mcp serve-recipe`, see the [Recipe → MCP Bridge](/features/mcp-recipe-server) guide.
</Note>
...
<Note>
**Layer positioning:** this is the *heavy host*. For the lighter alternatives (client and light server), see [The Three MCP Layers](/features/mcp-three-layers).
</Note>
...
See the full [`doctor` reference](/docs/cli/mcp-server#doctor) for the JSON payload shape, exit codes, and error output.
...
- `praisonai.knowledge.clear` — clear the knowledge base. Returns `"Knowledge cleared"` only when the configured backend actually reset. When the backend does not support `reset()` (e.g. `ChromaKnowledgeAdapter`), the tool returns `"Error: knowledge was NOT cleared - the configured backend (<AdapterName>) does not support reset()"` — a truthful failure on a destructive path.
- `praisonai.knowledge.stats` - Corpus statistics

<Note>
**Since PraisonAI [PR #4991](https://github.com/MervinPraison/PraisonAI/pull/4991):** `praisonai.knowledge.clear` no longer returns `"Knowledge cleared"` when the backend silently no-opped. It now returns an error string naming the backend that refused, so a user is never told a destructive operation succeeded when nothing was erased. Because the string starts with `"Error: "`, `_handle_tools_call` sets `isError: true` — see [Error semantics](#error-semantics).
</Note>
...
<Note>
You don't configure any of this — it's the default behaviour of the STDIO transport. See [MCP STDIO Server Transport](/features/mcp-stdio-server-transport) for the full picture.
</Note>
...
Clients can also raise the log level at runtime via the JSON-RPC `logging/setLevel` method. This scopes to the same per-server `praisonai.mcp_server.<name>` logger, and requires the `admin` scope when [scoped API keys](/features/mcp-scoped-api-keys) are configured.
