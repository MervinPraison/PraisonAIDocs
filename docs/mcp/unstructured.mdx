...
<Steps>
    ...
    <Step title="Run the Agent">
        ...
        ```
        Transform is asynchronous: the agent starts a job with `start_transform_job`, polls `check_job_status` until it is complete, then retrieves the output with `get_job_results`. Large or scanned documents can take a few minutes.
    </Step>
</Steps>
