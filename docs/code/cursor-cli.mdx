...
cursor = CursorCLIIntegration(workspace="/project", force=True)

agent = Agent(
    name="Code Assistant",
    role="Software Developer",
    goal="Help with coding tasks",
    tools=[cursor.as_async_tool()],   # registers as `cursor-agent_atool`
)

await agent.astart("Add error handling to all functions")
```
