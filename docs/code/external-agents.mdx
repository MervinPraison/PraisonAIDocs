...
These ship with PraisonAI. `--external-agent` and `ExternalAgentsHandler.list_integrations()` return these built-ins **plus** any registered plugins — the set is registry-driven, not fixed. See [Register a custom external agent](/features/external-cli-integrations#register-a-custom-external-agent).
...
<Tip>
Instead of writing Python code, you can enable external agents directly from any PraisonAI UI by flipping toggles in the interface. See [External Agents in UI](/features/external-agents-ui) for complete documentation on UI-based external agent management.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Gemini CLI" icon="google" href="/code/gemini-cli">
    Detailed Gemini CLI integration guide
  </Card>
  <Card title="Codex CLI" icon="openai" href="/code/codex-cli">
    Detailed Codex CLI integration guide
  </Card>
  <Card title="Cursor CLI" icon="cursor" href="/code/cursor-cli">
