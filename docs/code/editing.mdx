...
<Warning>
Not a sandbox. Do not pass untrusted code from end users without isolating the host process (subprocess with `setrlimit`, container, or VM). See [`python_tools`](/tools/python_tools) for the safer, already-timeout-boxed alternative for user-supplied code.
</Warning>
