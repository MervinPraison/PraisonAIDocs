...
<Note>
`cli_backend=` is deprecated (removal in 2.0.0). Prefer `runtime="claude-code"`. See [Subscription Auth](/docs/features/subscription-auth) for login details and the [CLI Backend Protocol](/docs/features/cli-backend-protocol) for the full config surface.
</Note>
