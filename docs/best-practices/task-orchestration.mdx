...
<Note>
**Async parity ([PR #3326](https://github.com/MervinPraison/PraisonAI/pull/3326)):** A `task_type="loop"` start task now pre-expands one subtask per input-file row on the **async** path — `await process.aworkflow()` and `process.workflow()` produce the same subtask set. Before this fix, the async engine skipped pre-expansion and ran the loop task once as an ordinary task, so a 500-row input file processed only the first row.
</Note>
...
analyzer = Agent(name="Analyzer", instructions="Analyze one row at a time")

loop_task = Task(
    name="analyze_row",
    description="Analyze row {row}",
    agent=analyzer,
    task_type="loop",
    input_file="rows.csv",   # one Task per row, on the async path too
    is_start=True,
)

process = Process(tasks={"analyze_row": loop_task}, agents=[analyzer])
...
```mermaid
graph LR
    subgraph "Loop-task expansion"
        Csv[📄 rows.csv] --> PreExp[🧩 pre-expansion]
        PreExp --> S1[loop_start_1]
        PreExp --> S2[loop_start_2]
        PreExp --> S3[loop_start_3]
    end

    subgraph "Engines (same output)"
        Sync[⚙️ workflow]
        Async[⚡ aworkflow]
    end
    ...
    classDef sub fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef engine fill:#10B981,stroke:#7C90A0,color:#fff

    class Csv input
    class PreExp process
    class S1,S2,S3 sub
