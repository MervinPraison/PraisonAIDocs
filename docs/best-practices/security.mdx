...
<Warning>
**PR #2062 (2026-06-22) breaking change:** `enable_injection_defense()` now returns `Dict[str, str]` instead of `str`. If you capture the return value to remove hooks later, update your code:
...
If you don't capture the return value (just call `enable_injection_defense()`), no change needed.
</Warning>
...
<Tabs>
  ...
  <Tab title="Selective enable">
    ...
    # Returns {"before_tool": "<id1>", "before_agent": "<id2>"} since PR #2062
