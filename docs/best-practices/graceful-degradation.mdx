...
## Falling Back on Empty Responses

When `chat` or `achat` returns an empty string, fall back to a secondary prompt or a canned reply instead of surfacing nothing to the user.
...
def answer_with_fallback(question: str) -> str:
    ...
    # Empty reply — content filter, refusal, or length limit.
    # Try a safer rephrasing before giving up.
    reply = agent.chat(f"Give a brief, general answer to: {question}")
    if reply:
        return reply

    return "I couldn't answer that. Try rephrasing your question."

print(answer_with_fallback("Tell me about the weather"))
```

<Note>
The empty string returned here is the same signal documented in [Handling Empty Agent Responses](/best-practices/error-handling#handling-empty-agent-responses). Check your logs for the `WARNING` that names the exact cause.
</Note>
