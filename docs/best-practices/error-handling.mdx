...
graph TB
    subgraph "Empty Response Causes"
        ...
        Empty --> Stop[🔖 Sets agent.last_stop_reason]
    end
    ...
    classDef signal fill:#6366F1,stroke:#7C90A0,color:#fff
...
### Getting the specific reason

The empty string is only half the story — the agent also records a distinct `last_stop_reason` alongside it, so you can branch on *why* nothing came back.

```python
from praisonaiagents import Agent

agent = Agent(name="Assistant", instructions="Answer clearly.", llm="gpt-4o-mini")
reply = agent.chat("Tell me a joke about kittens")

if not reply:
    reason = agent.last_stop_reason
    if reason == "content_filtered":
        print("The provider blocked that request.")
    elif reason == "refused":
        print("The model declined to answer.")
    elif reason == "length_truncated":
        print("Response was cut off — try a higher max_tokens.")
    else:
        print("The assistant couldn't produce a reply.")
```

### Structured outcome

For a typed result instead of an empty-string check, opt into `return_outcome=True` — a blocked run returns a `RunOutcome` whose `reason` is the specific provider block.

```python
from praisonaiagents import Agent

agent = Agent(name="Assistant", instructions="Answer clearly.", llm="gpt-4o-mini")
outcome = agent.run("Tell me a joke about kittens", return_outcome=True)

if outcome.reason in ("content_filtered", "refused", "length_truncated"):
    print(f"Blocked: {outcome.reason}")
elif outcome.succeeded:
    print(outcome.output)
```

See [Run Outcome](/features/run-outcome#provider-block-outcomes) for the full terminal-reason vocabulary and precedence.
