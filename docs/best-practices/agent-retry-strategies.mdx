...
<Tip>
PraisonAI has built-in LLM retry support — **enabled by default** on every `Agent()`. See [Agent Retry](/features/agent-retry) for the API reference and for the `retry=False` opt-out before implementing custom retry logic.
</Tip>
