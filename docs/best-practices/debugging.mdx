...
## Understanding Agent WARNING Logs

An agent logs a `WARNING` when the model returns no content — a content filter, a refusal, or a non-standard `finish_reason`.

The warning follows this format:

```
Agent <name>: model returned no content (finish_reason='<reason>', refused=<bool>)
```

The refusal text itself is never logged, so safety-sensitive strings do not leak into your logs.

Match the `finish_reason` you see to its cause and the recommended action:

| `finish_reason` | Meaning | `agent.last_stop_reason` | User action |
|---|---|---|---|
| `content_filter` | Provider's safety filter blocked the response | `"content_filtered"` | Rephrase the prompt or adjust provider safety settings |
| `length` / `max_tokens` | Model hit the output length limit | `"length_truncated"` | Increase `max_tokens`, or shorten the prompt |
| `stop` + `refused=True` | Model refused (safety refusal) | `"refused"` | Rephrase or try a different model; refusal text is intentionally not logged |
| (any other non-standard reason) | Provider-specific | *(unchanged, `"completed"`)* | Check provider docs |

The agent returns an empty string for every blocked case above **and** records the specific `agent.last_stop_reason`. See [Handling Empty Agent Responses](/best-practices/error-handling#handling-empty-agent-responses) for the detection pattern and [Run Outcome](/features/run-outcome#provider-block-outcomes) for the terminal-reason contract.
