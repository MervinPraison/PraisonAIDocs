...
| `finish_reason` | Meaning | User action |
|---|---|---|
| `content_filter` | Provider's safety filter blocked the response | Rephrase the prompt or move sensitive content out of the request |
| `length` | Model hit `max_tokens` before producing any content | Increase `max_tokens`, or shorten the prompt |
| `stop` + `refused=True` | Model refused (safety refusal) | Rephrase the request; refusal text is intentionally not logged |
| (any other non-standard reason) | Provider-specific | Check provider docs |

The agent returns an empty string for every case above. See [Handling Empty Agent Responses](/best-practices/error-handling#handling-empty-agent-responses) for the code pattern to detect and handle these turns.
