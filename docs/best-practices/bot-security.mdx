...
<Warning>
`unknown_user_policy` defaults to `"deny"` — an empty `allowed_users` **drops** inbound DMs from unknown users unless you also set `unknown_user_policy: "pair"` (with `owner_user_id`) or `"allow"`. Since [PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112) the drop is no longer silent — every drop logs a `WARNING` with the sender's `user_id`, `platform`, and `conversation`.

When `allowed_users` is empty, the startup banner names the resolved policy explicitly:

| Resolved policy | Level | Banner message |
|----------------|-------|----------------|
| `deny` (default) | `WARNING` | "Channel has no allowed_users and unknown_user_policy=deny. DMs from users not in allowed_users (and not already paired) are dropped and the bot responds to NOBODY new. Set unknown_user_policy=allow or add allowed_users to let users through." |
| `allow` | `WARNING` | "Channel has no user restrictions (allowed_users/allowlist/blocklist) and unknown_user_policy=allow. Bot will respond to EVERYONE. Consider adding allowed_users for security." |
| `pair` | `INFO` | "Channel has no allowed_users and unknown_user_policy=pair. Unknown users must complete pairing before the bot replies." |

Every `deny` drop also logs this WARNING so you can grep for the exact sender being rejected:

```text
Dropped message from unknown user_id=<id> platform=<telegram|discord|slack|...>
conversation=<chat_id>: unknown_user_policy=deny and user is not in
allowed_users. Set unknown_user_policy=allow (or add the user to
allowed_users) to let this user through.
```

If you see the `deny` banner and expected messages to flow through, either populate `allowed_users` or change the policy.

On `praisonai bot start` (single-bot CLI), the startup banner also prints the resolved policy so you can confirm it directly — grep your logs for:

```text
unknown_user_policy: <value>
```

`<value>` is the YAML value or `deny` when omitted ([PR #5111](https://github.com/MervinPraison/PraisonAI/pull/5111)). A typo (e.g. `alow`) now fails closed at load time — treat that as intended, not a bug.
</Warning>
...
<Note>
`observe` (Telegram only) is safe from a "who does the bot answer" perspective — it responds to no one extra, exactly like `mention_only`. It does, however, **retain** every unmentioned group message in session storage, so account for the higher volume of stored conversation text (retention and PII implications) before enabling it. See [`observe`: passive group context](/docs/features/bot-gateway#observe-passive-group-context).
</Note>

<Note>
The `mention_only` default also admits a **direct reply to the bot** as an implicit mention (so a natural follow-up works without re-typing `@bot`). This is safe by design — the sender is still gated by `allowed_users` / `allowlist` first; the mention gate is not an auth gate. Operators who want the strictest interpretation can set `implicit_mentions: []` to require a literal `@bot` every time. See [Implicit Mentions](/features/bot-implicit-mentions).
</Note>
...
### 1. Callback Signing (default is safe)

<Note>
Callback and pairing-code signing share one HMAC secret stored at
`~/.praisonai/gateway/.gateway_secret` (mode `0600`), auto-created on first
use. Buttons and pairing codes survive gateway restarts by default — no
operator action required.

Set `PRAISONAI_CALLBACK_SECRET` only when running the gateway on **multiple
hosts or replicas** that need to verify each other's callbacks. Set it to the
...
The signing key resolves in this order (pairing codes and callback buttons alike):

- **Single host, default install:** nothing to do — inline buttons and pairing codes just work across restarts.
- **Multiple hosts / replicas:** set `PRAISONAI_CALLBACK_SECRET` to the same value on every host.
- **Explicit control:** setting `PRAISONAI_CALLBACK_SECRET` overrides the persisted secret — useful for rotation or shared-vault-managed environments.
...
Pick the policy that matches how open your bot should be:

| Scenario | Policy | Rationale |
|----------|--------|-----------|
| Public bot with self-service onboarding | `pair` + `owner_user_id` | Owner approves each new user; audit trail; safe default for growing communities. |
| Internal-only bot on trusted network | `allow` + empty `allowed_users` | Everyone on the network is trusted; skips friction. **Never use on a public token.** |
| Fixed team / private bot | `deny` + explicit `allowed_users` | Whitelist is the security boundary; policy is defense-in-depth. |
| Default / uncertain | `deny` (leave unset) | Secure by default — nothing surprising leaks through. |

All three values are read per-channel from `gateway.yaml` — see [Bot Gateway → Channel Security](/docs/features/bot-gateway#channel-security).
...
<Tip>
For detailed pairing documentation, see the [Bot Pairing](/features/bot-pairing) guide.
</Tip>
...
<AccordionGroup>
  <Accordion title="✅ DM Policy Configured" icon="message">
    ...
    - [ ] Group policies set to `mention_only`, `command_only`, or `observe` (Telegram)
  ...
  <Accordion title="✅ Gateway Pairing Active" icon="link">
    - [ ] Callback/pairing signing works by default via `~/.praisonai/gateway/.gateway_secret` (`0600`) — no action needed on a single host
    - [ ] `PRAISONAI_CALLBACK_SECRET` set to a shared value **only** across multiple hosts/replicas
    - [ ] `unknown_user_policy` configured (`deny`/`pair`/`allow`)
    - [ ] If using `praisonai bot start` with a `bot.yaml`, confirm the `unknown_user_policy` line in the startup banner matches what you configured. A typo will now fail closed at load time — treat that as intended, not a bug.
    - [ ] A typo in `unknown_user_policy` now fails closed at load time in **both** `bot.yaml` ([PR #5111](https://github.com/MervinPraison/PraisonAI/pull/5111)) and multi-channel `gateway.yaml` ([PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112)). Fix the typo — do not disable the validator.
    - [ ] `owner_user_id` set for inline approvals
    - [ ] Pairing workflow tested and verified
    - [ ] Revocation process documented
    - [ ] At-rest hashing is on by default from PraisonAI [#3974](https://github.com/MervinPraison/PraisonAI/pull/3974) — `pairing.json` never contains a plaintext code
    - [ ] A per-channel-type brute-force lockout defends every approval surface (HTTP, in-chat button, CLI). Tunable via `PairingStore(max_failures=, lockout_window=, lockout_cooldown=)`
  </Accordion>
  ...
  <Accordion title="✅ Monitoring & Alerts" icon="chart-line">
    ...
    - [ ] Audit logging enabled (`praisonai.security.enable_audit_log`) — sensitive keys (api keys, tokens, passwords) in tool inputs are redacted by default; see [Audit Logging](/features/audit-logging#secret-redaction) to customise
...
**Problem:** My bot silently ignores every DM from users I haven't listed in `allowed_users` and I don't know why.
**Solution:** Since [PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112), every deny-policy drop leaves a `WARNING` in the bot log:

`Dropped message from unknown user_id=... platform=... conversation=...: unknown_user_policy=deny and user is not in allowed_users.`

Grep your bot log for `unknown_user_policy=deny`. If you see the WARNING, either add the printed `user_id` to `allowed_users`, or set `unknown_user_policy=allow` (or `pair` with an `owner_user_id`). If you don't see the WARNING, the message isn't reaching the admission pipeline — check the token, network, and platform adapter first.
...
**Solution:** PR #1885 fixed a Telegram bug where empty `allowed_users` bypassed `unknown_user_policy`. Either add your user ID to `allowed_users`, or set `unknown_user_policy: "allow"` if you actually want the bot to respond to anyone. Default is `"deny"`. Since [PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112), a dropped DM now leaves a `WARNING` with the sender's `user_id` — the fastest way to confirm this is what's happening.
...
<CardGroup cols={2}>
  ...
  <Card title="Gateway Setup" icon="server" href="/features/gateway">
