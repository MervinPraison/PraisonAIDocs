...
For more details on profiling configuration and best practices, see [Profiling](/docs/features/profiling).

## Resetting long-term memory

`memory.reset_long_term()` scopes to this instance's own Chroma collection — sibling agents that share the same persistent path are unaffected.
...
agent = Agent(
    name="Session Worker",
    instructions="Answer, then forget between sessions.",
    memory=True,
)

agent.start("Remember: the pilot's name is Alex.")

# Only THIS agent's long-term memory is cleared.
# Other agents sharing ./chroma_db keep their collections intact.
try:
    agent.memory.reset_long_term()
except Exception as e:
    print(f"Reset failed: {e}")   # now surfaced, was previously silent
```

A failed reset now raises instead of being silently swallowed — wrap the call in `try/except` if you want to continue on error. Give each agent a distinct `collection_name` (see [Memory Configuration](/docs/configuration/memory-config#chroma-collection-name)) when several share one `rag_db_path`.
