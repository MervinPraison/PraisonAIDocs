...
# TrainingStorage is a context manager — closes its backend on exit
with TrainingStorage(session_id="train-123", backend=backend) as storage:
    storage.save_iteration(...)
```
...
with TrainingStorage(session_id="train-123") as storage:
    storage.save_iteration(iteration)

# With SQLite backend for better performance
with TrainingStorage(
    session_id="train-123",
    backend=SQLiteBackend(db_path="training.db"),
) as storage:
    storage.save_iteration(iteration)
    iterations = storage.load_iterations()
# Connection closed automatically — safe on Windows, no ResourceWarning
```

<Note>
Using `with` (or calling `.close()` explicitly) releases the underlying SQLite or Redis
...
`FileBackend`) `.close()` is a safe no-op.
</Note>

When you use a `SQLiteBackend` (or `RedisBackend`) directly — for example via
...
backend = SQLiteBackend(db_path="training.db")
try:
    sessions = list_sessions_from_backend(backend)
finally:
    backend.close()
```
...
<CardGroup cols={2}>
  ...
  <Card title="Memory Storage" icon="brain" href="/memory/storage">
    Agent memory persistence
  </Card>
  <Card title="Training" icon="graduation-cap" href="/concepts/agent-train">
