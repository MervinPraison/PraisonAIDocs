...
<AccordionGroup>
  <Accordion title="Wrap tools with clear type hints">
    Add clear type hints to LangChain tool wrappers for better agent understanding.
  </Accordion>
  <Accordion title="Return strings from all tools">
    Ensure tool functions return strings - LLMs process text responses most reliably.
  </Accordion>
  <Accordion title="Use BaseTool for complex tools">
    Use LangChain's BaseTool class when your tool needs validation or complex parameter handling.
  </Accordion>
  <Accordion title="Test tools independently">
    Test each tool function directly before adding it to an agent to ensure it returns expected output.
  </Accordion>
</AccordionGroup>
