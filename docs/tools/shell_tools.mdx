...
### execute_command(command, cwd=None, timeout=30, capture_output=True, shell=False, env=None, allow_dangerous=False)
...
<Note>
The `shell` parameter is **deprecated** for security reasons and has no effect. Do not pass `shell=True` in new code.
</Note>
