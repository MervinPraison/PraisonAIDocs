---
title: "Browser Automation via CDP Bridge"
sidebarTitle: "CDP Bridge Extension"
description: "praisonai-browser developer extension that connects to the bridge server for browser automation with multi-layer persistence"
icon: "chrome"
---
...
<Note>
If you launched with `--no-server`, start the bridge first (`praisonai browser start`), then re-run `praisonai browser doctor extension`. See the full flow in [Manual Extension Load](/features/browser-agent-deep-dive#manual-extension-load-chrome-137-windows).
</Note>
...
The **developer extension** (which connects to the bridge server) requires:
...
<Warning>
The published **PraisonAI Browser Agent** on the Chrome Web Store is a different build with a different, narrower permission set: `sidePanel`, `tabs`, `debugger`, `storage`, `contextMenus`, `notifications` — with no `host_permissions`, no `offscreen`, no `activeTab`, and no `<all_urls>`. See [chrome.praison.ai](https://chrome.praison.ai).
</Warning>
