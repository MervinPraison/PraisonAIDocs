...
SurrealDB tool provides native support for SurrealDB, a powerful multi-model database that handles document, graph, and relational data in a single platform. This tool enables AI agents to interact with SurrealDB through its native query language, SurrealQL.
...
# Query data
results = surrealdb_tool.query("SELECT * FROM users")
print(results)
```
...
agent = Agent(
    name="DatabaseAgent",
    instructions="You are a database analyst. Use SurrealDB to manage multi-model data.",
    tools=[surrealdb_tool]
)

response = agent.chat("Create a user record and show all users")
print(response)
```
...
docker run -d --name surrealdb \
    ...
    -v $(pwd)/data:/data \
    surrealdb/surrealdb:latest \
    start --log trace --user root --pass root rocksdb:/data
```
