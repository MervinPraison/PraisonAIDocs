...
| Octal IPv4 | `http://0177.0.0.1:8765/` | `0177` = `127` in octal — `inet_aton` reads this as loopback |
| Octal that only the resolver reads as private | `http://010.0.0.1:8765/` | Some libcs read `010.0.0.1` as `10.0.0.1` (RFC1918) via `getaddrinfo`, though `inet_aton` reads it as public `8.0.0.1`. Both readings are consulted; either being internal blocks. |
...
The alternative loopback encodings (trailing-dot localhost, short-form/octal/hex IPv4, etc.) are validated by the unified `_host_is_blocked` helper, which also powers URL safety for [`@url:` mentions](/cli/mentions).

`_host_is_blocked` consults two independent readings of the host — `socket.inet_aton` (what libc-based HTTP clients dial) and `socket.getaddrinfo` (what a resolver returns) — and blocks if **either** yields an internal address. `ipaddress.ip_address` refuses ambiguous forms like `0177.0.0.1` (leading zeros) and `127.1` (short form) outright, so those would otherwise fall through to only the resolver — which some libcs read as the *public* `177.0.0.1`, letting them dial loopback. Consulting both readings closes that gap in both directions ([GHSA-5c6w-wwfq-7qqm](https://github.com/advisories/GHSA-5c6w-wwfq-7qqm), fixed in [PR #4966](https://github.com/MervinPraison/PraisonAI/pull/4966)).

The mentions handler (`praisonaiagents.tools.mentions`) uses this validator on **every** redirect hop, not just the first. Prior versions only validated the initial URL, so a redirect to `169.254.169.254` could exfiltrate cloud metadata. See [Crawl4AI — URL Safety](/tools/crawl4ai#url-safety).
</Note>
...
# Octal-encoded loopback — 0177 = 127 in octal, dials 127.0.0.1
scrape_page("http://0177.0.0.1:8765/")
# {'error': 'Invalid or potentially dangerous URL: http://0177.0.0.1:8765/'}
