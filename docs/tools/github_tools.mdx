...
```python
from praisonaiagents import Agent
from praisonaiagents.tools import github_create_branch, github_commit_and_push, github_create_pull_request

agent = Agent(
    name="GitHub Agent",
    instructions="Create a branch, commit changes, and open a pull request.",
    tools=[github_create_branch, github_commit_and_push, github_create_pull_request],
)

agent.start("Create branch 'feature/update', commit all changes, then open a PR to main.")
```
</Step>
