...
sidebarTitle: "Google Calendar Tools"
...
agent = Agent(
    ...
    instructions="Create and list Google Calendar events.",
    tools=["google_calendar"],
)
agent.start("Schedule a 30-minute sync tomorrow at 10:00 with the team")
```
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Google Calendar Tools
...
<Step title="Install">
```bash
pip install praisonaiagents google-api-python-client google-auth
```
</Step>
<Step title="Create a calendar agent">
```python
from praisonaiagents import Agent

agent = Agent(
    name="CalendarAgent",
    instructions="Manage Google Calendar events. My timezone is UTC.",
)

agent.start("List my events for tomorrow")
```
</Step>
</Steps>
