...
Create a file in `~/.praison/plugins/` or `./.praison/plugins/`:
...
2. **User-level**: `~/.praison/plugins/` - Plugins available to all your projects
...
praisonai plugin init my_plugin

# With options
praisonai plugin init weather_tools --author "John Doe" --with-hook
...
plugins = discover_plugins(["./plugins", "~/.praison/plugins"])
for p in plugins:
...
<AccordionGroup>
  <Accordion title="Plugin not discovered">
    - Check the file is in `.praison/plugins/` or `~/.praison/plugins/`
