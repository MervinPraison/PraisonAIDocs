...
sidebarTitle: "Single-File Plugins"
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Single-File Plugins
...
<Note>
Bundling tools with hooks, config, or session lifecycle in a class-based plugin? Return callables from `Plugin.get_tools()` instead — see [Tool Plugins](/features/plugins#tool-plugins-subclass-plugin-with-get-tools).
</Note>
...
praisonai plugins create my_plugin

# With options
praisonai plugins create weather_tools --author "John Doe"

# In a specific directory
praisonai plugins create custom --output ./my_plugins/
```
  </Tab>
  <Tab title="List & Scan">
```bash
