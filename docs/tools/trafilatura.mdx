...
sidebarTitle: "Trafilatura Web Extraction"
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Trafilatura Web Extraction
...
<Step title="Install">
```bash
pip install praisonaiagents trafilatura
```
</Step>
<Step title="Extract article content with agent">
```python
import trafilatura
from praisonaiagents import Agent
...
agent = Agent(
    name="ExtractAgent",
    instructions="Extract and analyze article content from URLs.",
    tools=[extract_article],
)

agent.start("Extract and summarize https://example.com/article")
```
</Step>
</Steps>
...
For more information about web scraping and content extraction patterns, see the [Web Automation documentation](/docs/tools/custom).
