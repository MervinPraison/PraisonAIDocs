...
<Note>
  **Prerequisites**
  ...
  - A language server binary on `PATH` (see [Language Server Installation](#language-server-installation))
</Note>
...
When you pass a `file_path`, the client initialises the language server against the nearest **root marker** (`pyproject.toml`, `go.mod`, `Cargo.toml`, `tsconfig.json`, `.git`, …) instead of the current working directory. A nested-project search resolves symbols against the file's real project root — see [LSP Auto-Detect](/features/lsp-auto-detect) for the full marker table.
...
Beyond these five, you can register any additional language server via `LSPClient(..., servers=...)` — see [LSP Auto-Detect › Adding or Overriding a Language Server](/features/lsp-auto-detect#adding-or-overriding-a-language-server).
...
<Info>
When the server is missing, the tool returns `Error: python language server \`pylsp\` not installed (install with \`pip install python-lsp-server\`); install it to use lsp navigation (falling back to grep is advised)` — the error names the missing binary and its install command, and the agent keeps running so it can fall back to other search tools.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="LSP Navigation (Feature)" icon="magnifying-glass-code" href="/features/lsp-navigation-tools">
