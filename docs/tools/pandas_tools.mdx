...
from praisonaiagents import read_csv, get_summary

agent = Agent(
    name="Data Analyst",
    instructions="Analyse tabular data with pandas tools.",
    tools=[read_csv, get_summary],
)
agent.start("Describe the sales.csv dataset and highlight outliers")
```
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Pandas Agent
