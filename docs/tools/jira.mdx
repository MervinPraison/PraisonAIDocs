...
from praisonaiagents.tools import jira_search, jira_create_task

agent = Agent(
    name="Jira Assistant",
    instructions="Search and create Jira issues from natural language.",
    tools=[jira_search, jira_create_task],
)
agent.start("Find open bugs in PROJ and create a task for the login timeout")
```
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub Tools" icon="github" href="/tools/github_tools">
