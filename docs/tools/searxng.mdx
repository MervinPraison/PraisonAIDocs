...
agent = Agent(
    name="Private Search",
    instructions="Search the web via the local SearxNG instance.",
    tools=[search_web],
)
agent.start("Search for privacy-focused search engines and summarise")
```

The user asks for web results; the agent queries SearxNG instead of a commercial search API.
```mermaid
graph LR
    U[Input] --> A[Agent]
    A --> T[Tool]
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as SearxNG Search
...
<Steps>
<Step title="Start SearxNG with Docker">
```bash
...
agent = Agent(
    name="PrivateSearchAgent",
    instructions="Search the web privately using SearxNG.",
    tools=[search_web],
)
...
# Allow SearxNG on a custom (non-loopback-named) host, in addition to localhost
os.environ['SEARXNG_URL_ALLOWLIST'] = 'searxng.internal,my-searxng'
...
<Note>
SearxNG on `localhost` / `127.0.0.1` / `::1` works out of the box — loopback is permitted for SearxNG by design. A SearxNG instance reachable only by a **custom hostname that resolves to loopback** needs that host in `SEARXNG_URL_ALLOWLIST` (comma-separated, trimmed, lowercased). A SearxNG instance on a private or internal IP stays blocked by the SSRF policy — this env var cannot exempt private ranges, and that is intentional.
</Note>
