...
description: "Agent-centric scheduling — let agents add, list, pause, resume, update, and remove scheduled jobs"
icon: "clock"
---
...
| `max_runs` | `int` | No | Default `0` (unbounded). Fire at most this many times, then auto-remove. Generalises `once` — `max_runs=1` is equivalent to `once=True`. Combine with `until` to cap both count and end date. |
| `until` | `str` | No | Default `""` (no end date). ISO 8601 wall-clock instant after which a recurring schedule stops firing and is retired. Timezone-resolved exactly like an `at:` timestamp using `tz`. |

<Warning>
An unparseable `until` value **fails safe**: the job is not fired and is retired by the claim path rather than running forever. Prefer a full ISO 8601 timestamp with an offset, or pair with `tz=` for a naive wall-clock reading.
</Warning>
...
The CLI exposes the same surface — see [`praisonai schedule pause / resume / update`](/cli/schedule#managing-store-backed-schedules).
...
See [Scheduler Delivery → Continuable Delivery](/docs/features/scheduler-delivery#continuable-delivery) for the full seed contract.
...
Set `max_runs` to fire a fixed number of times, then auto-remove.

```python
from praisonaiagents.tools import schedule_add

# "Remind me every hour, three times total."
schedule_add(
    name="deploy-recheck",
    schedule="*/1h",
    message="Check deploy status",
    deliver="slack:C0123456",
    max_runs=3,     # retires after the third fire
)
```

### Bounded by Date

Set `until` to stop a recurring schedule after a wall-clock instant.

```python
from praisonaiagents.tools import schedule_add

# "Every weekday at 8am, until Friday."
schedule_add(
    name="week-standup",
    ...
    tz="Europe/London",
    until="2026-09-25T17:00:00",   # timezone-resolved via tz
)
```

### Combined: count and date

Combine both — whichever condition is met first retires the job.

```python
from praisonaiagents.tools import schedule_add

# "Every hour, up to 10 times, but stop by end of day either way."
schedule_add(
    name="rollout-watch",
    schedule="*/1h",
    message="Check rollout health",
    deliver="slack:C0123456",
    tz="Europe/London",
    max_runs=10,                   # cap the count
    until="2026-09-25T23:59:00",   # and the end date
)
```

An unparseable `until` fails safe: the job is not fired and is retired by the claim path rather than running forever.

### How Should It Stop?

Pick a stop condition to bound a recurring `every` / `cron` job.

```mermaid
graph TD
    Q[How should it stop?]
    Q -->|Never| N["Default<br/>unbounded"]
    Q -->|After N fires| M["max_runs=N"]
    Q -->|After a date| U["until=..."]
    Q -->|After the first fire| O["once=True<br/>(same as max_runs=1)"]
...
See [Scheduler Providers](/features/scheduler-providers) for full patterns.
...
Schedule lifecycle events are available via the hook system. Both `SCHEDULE_ADD` and `SCHEDULE_REMOVE` are emitted by the schedule store, so they fire on a real persist/delete no matter who triggered it.

| Event | When | Input Type | Key Fields |
|-------|------|------------|------------|
| `SCHEDULE_ADD` | A schedule is persisted | `ScheduleAddInput` | `job_name`, `job_id`, `schedule`, `message`, `agent_id`, `principal`, `enabled` |
| `SCHEDULE_REMOVE` | A schedule is deleted | `ScheduleRemoveInput` | `job_name`, `job_id`, `schedule`, `principal` |
| `SCHEDULE_TRIGGER` | A scheduled job fires | `ScheduleTriggerInput` | `job_name`, `job_id`, `message` |

Both input types are importable from `praisonaiagents.hooks`.

<Note>
The same hook fires whether the schedule was added via the **tool** (`schedule_add`), the **`praisonai schedule` CLI**, or the gateway **`config.yaml` reconciler** — the store is the single chokepoint every path funnels through.
</Note>

### Observing tool-driven schedules

Register hooks, then call the tools — the store emits the payload for you.

```python
from praisonaiagents.hooks import HookEvent, HookResult, get_default_registry
from praisonaiagents.tools import schedule_add, schedule_remove

registry = get_default_registry()

@registry.on(HookEvent.SCHEDULE_ADD)
def on_add(event_data):
    print(f"ADD: {event_data.job_name} → {event_data.schedule} (enabled={event_data.enabled})")
    return HookResult.allow()

@registry.on(HookEvent.SCHEDULE_REMOVE)
def on_remove(event_data):
    print(f"REMOVE: {event_data.job_name} ({event_data.job_id})")
    return HookResult.allow()

schedule_add(name="daily-report", schedule="daily", message="Send the daily report")
schedule_remove("daily-report")
```
...
<Tip>
`ScheduleLoop` is the default provider. For event-driven firing (cloud webhook, systemd timer, K8s CronJob) see [Scheduler Providers](/features/scheduler-providers).
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Bound recurring jobs when you can">
    An unbounded `every` / `cron` job runs until manually removed. Prefer `max_runs` or `until` whenever the ask has an implicit stop ("three times", "until Friday", "for the next hour"). Both may be combined; whichever fires first retires the job.
  </Accordion>
</AccordionGroup>
