...
sequenceDiagram
    ...
    alt Adapter supports it
        ...
        Knowledge-->>User: reset returns True / Result
...
<Note>
**Since PraisonAI PR [#4991](https://github.com/MervinPraison/PraisonAI/pull/4991)**, `Knowledge.reset()` returns `bool` — `True` when the backend actually reset, `False` when the adapter has no `reset()` and the call was a silent no-op. On destructive paths, check the return value before telling a user their data was erased: a `False` means nothing was cleared. Existing callers that ignore the return value are unaffected.
</Note>
...
# reset() never raises — it warns and no-ops on adapters without native reset
agent.knowledge.reset()
