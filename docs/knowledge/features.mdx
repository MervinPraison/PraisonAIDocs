...
<Warning>
**Remote URLs are not yet ingested.** Both `Agent(knowledge=[url])` and
`Knowledge.add(url)` skip URLs — `Knowledge.add` raises
`NotImplementedError("URL processing not yet implemented")`. Download the file
first, then add the local path:
