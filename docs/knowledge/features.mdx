...
<Note>
All supported UTF-8 text extensions — `.txt`, `.md`, `.csv`, `.json`, `.xml`, `.html`, `.htm` — are chunked with the configured `chunker` (respecting `chunk_size` and `chunk_overlap`), and the original case is preserved. Behaviour matches every other file type. Since PraisonAI PR [#4831](https://github.com/MervinPraison/PraisonAI/pull/4831).
</Note>
