...
<Warning>
**Remote URLs are not yet ingested.** Both `Agent(knowledge=["https://..."])` and
`Knowledge.add(url)` skip web sources — `add()` raises
`NotImplementedError("URL processing not yet implemented")`, and the Agent path logs:
...
Fetch the page yourself and pass the text or a local file path instead:
...
See [Knowledge Source Types](/features/knowledge-source-types).
</Warning>
