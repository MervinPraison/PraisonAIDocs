...
|----------|----------|------|
| `Agent(knowledge=[...])` | Most use cases | [Quick Start →](/knowledge/quickstart) |
| `Knowledge()` class | Custom indexing | [Knowledge API →](/sdk/praisonaiagents/knowledge/knowledge) |
| `RAG()` class | Custom pipelines | [RAG Module →](/rag/module) |
...
<Note>
Each `SearchResultItem` can be passed directly into RAG context utilities like `build_context` and `deduplicate_chunks`. See [Search Results](/features/knowledge-search-results) for the full field reference.
</Note>
