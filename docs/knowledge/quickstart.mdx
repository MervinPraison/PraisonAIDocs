...
Search returning results does not prove your agent used them. Run a unique-marker check — both flags must be `True`:
...
chat_ok = "ZEBRA-QUOTA-9917" in answer
print(f"SEARCH_OK={search_ok}  CHAT_OK={chat_ok}")
```

If `SEARCH_OK=True` but `CHAT_OK=False`, see [Knowledge Troubleshooting](/features/knowledge-troubleshooting).

<Note>
**Where is my data stored?** By default, the Chroma store persists at `<project>/.praisonai/knowledge/chroma` (absolute, per project). Override it with `knowledge={"vector_store": {"config": {"path": "..."}}}`. See [Knowledge Storage](/features/knowledge-storage).
</Note>
