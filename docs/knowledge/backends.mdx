...
<Warning>
`pgvector` is a mem0 inner backend, not a top-level Knowledge provider. Setting `vector_store.provider` to `pgvector` at the Knowledge top level raises `ValueError`. The `config` blocks below apply inside mem0's nested `vector_store` — see [Supported Providers](/features/knowledge-backends#supported-providers).
</Warning>
...
<Note>
Top-level `vector_store.provider` accepts only registered Knowledge adapters (`mem0`, `chroma`, `chromadb`, `rag`, `mongodb`, `sqlite`, plus anything registered). Any other name raises `ValueError`. See the [Supported Providers table](/features/knowledge-backends#supported-providers) for the single source of truth.
</Note>

### mem0 Backend (Default)

Stores like `qdrant` and `pgvector` are mem0's inner backends — set the top-level `provider` to `mem0` and nest them under mem0's own `config`.
...
<Warning>
A corrupt or cross-project Chroma store raises `RuntimeError: Chroma persist failed at '<path>' (PanicException: ...)` instead of crashing the process. Delete `<path>` and re-index, or point `path` at a fresh directory. See [Knowledge Storage](/features/knowledge-storage).
</Warning>
