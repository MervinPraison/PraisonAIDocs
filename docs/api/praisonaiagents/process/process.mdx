...
loop_task = Task(
    ...
    agent=processor,
    task_type="loop",
    loop_data="customers.csv"  # Process each row
)
...
item_task = Task(
    ...
    agent=analyst,
    task_type="loop",
    loop_data="items.txt"  # One item per line
)
...
analysis_task = Task(
    ...
    agent=analyst,
    retain_context=True  # Keep in context for future tasks
)
...
summary_task = Task(
    ...
    context_window=2000  # Limit context size
)
```
...
- **max_retries** (int): Global retry limit for failed tasks
- **retry_delay** (float): Delay between retry attempts
- **context_window** (int): Maximum context size between tasks
...
3. **Optimize Context Sharing**:
   ...
   task = Task(
       ...
       context_fields=["key_findings"]  # Specific fields only
   )
   ```
