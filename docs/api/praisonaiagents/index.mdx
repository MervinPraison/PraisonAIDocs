---
title: "praisonaiagents API"
description: "HTTP API endpoints for praisonaiagents"
icon: "server"
---

# praisonaiagents API

HTTP endpoints exposed by the praisonaiagents package.

<CardGroup cols={2}>
  <Card title="A2A API" icon="robot" href="/api/praisonaiagents/a2a/endpoints">
    Agent-to-Agent communication protocol endpoints
  </Card>
  <Card title="AG-UI API" icon="window" href="/api/praisonaiagents/agui/endpoints">
    AG-UI protocol for frontend integration
  </Card>
</CardGroup>

## Available Endpoints

| Protocol | Endpoints | Description |
|----------|-----------|-------------|
| A2A | `GET /.well-known/agent.json`, `GET /status`, `POST /a2a` | Agent-to-Agent communication |
| AG-UI | `POST /agui`, `GET /status` | Frontend integration with CopilotKit |

## SDK Reference

For SDK documentation (classes, modules, functions), see the [SDK Reference](/sdk/praisonaiagents/index).