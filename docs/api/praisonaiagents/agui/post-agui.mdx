...
`CUSTOM` events carry a `praisonai.*` name and must be ignored by clients that do not recognise them. See the [AGUI API reference](/deploy/api/agui-api#praisonai-custom-events) for payload shapes.
...
- [AG-UI API Overview](/api/praisonaiagents/agui/endpoints)
- [GET /status](/api/praisonaiagents/agui/get-status)
