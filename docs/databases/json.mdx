...
# .close() is a safe no-op for FileBackend — the with block keeps usage consistent
```
...
<Note>
`TrainingStorage` is a context manager. `FileBackend` holds no open connection, so
`.close()` is a safe no-op here — but using `with` keeps the pattern identical to the
SQLite and Redis backends, where closing releases a file or network handle (essential
on Windows and for long-running processes).
</Note>
