...
<Tip>
`praisonai persistence doctor` auto-detects Weaviate URLs as the `weaviate` backend (previously silently defaulted to `qdrant`) — no `--*-backend` flag needed. See [Backend Auto-Detection](/cli/persistence#backend-auto-detection).
</Tip>
