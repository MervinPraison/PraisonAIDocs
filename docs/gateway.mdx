...
<Note>
The heavy gateway implementation (`WebSocketGateway`) lives in the **`praisonai_bot.gateway`** package. `praisonaiagents.gateway` holds only protocols and lightweight utilities. `praisonai serve gateway` and `from praisonai.gateway import WebSocketGateway` still work exactly as documented here via re-export shims; for a standalone install see [praisonai-bot Migration](/guides/praisonai-bot-migration).
</Note>
...
<Tip>
For bot gateways, use the one-line **`reliability="production"`** preset on `BotOS`, gateway YAML, or `praisonai gateway start --reliability production` instead of tuning drain and admission separately. See [Gateway Reliability](/features/gateway-reliability).
</Tip>

## Hot Reload

Edit `gateway.yaml` while the gateway is running — file watch (with optional `watchdog`) and `SIGHUP` share the same diff-driven reload path, with drain-coordinated channel restarts. Three timing/logging keys (`gateway.logging.level`, `gateway.drain_timeout`, `gateway.reload_drain_timeout`) are **hot-applied in place** — no channel restart at all — see [Hot-Apply (no restart)](/docs/features/gateway-hot-reload#hot-apply-no-restart). Rotating `gateway.auth_token` in the same reload also revokes any live sessions on the old secret — see [Gateway Credential Rotation](/features/gateway-credential-rotation). See [Gateway Config Reload](/features/gateway-config-reload).

<Note>
**Event-loop watchdog (separate from filesystem watching).** The `watchdog` above is the Python package that watches `gateway.yaml` on disk. A different, opt-in **event-loop** watchdog catches a wedged asyncio loop and restarts the process — enable it with `gateway.watchdog.enabled: true` (or `--watchdog`). See [Event-Loop Watchdog](/docs/features/gateway-loop-watchdog#using-it-from-the-gateway).
</Note>
...
To declare recurring agent → channel deliveries directly in `gateway.yaml` (with matching `praisonai gateway schedule add | list | remove` CLI), see [Gateway Schedules](/features/gateway-schedules).
...
<Note>
The gateway's scheduled delivery now routes **threaded** sends through the shared resilient `DeliveryRouter` (durable dedup, rate limiter, dead-target registry) via the same `platform:channel:thread` token — the old `_ThreadBindingBot` workaround is gone. The gateway's dead-target registry is backed by the shared SQLite `dead_targets` table in `~/.praisonai/state/delivery_control.sqlite` (falling back to the node-local JSON sidecar only if the SQLite store can't be constructed). See [Scheduler Delivery → Thread semantics](/docs/features/scheduler-delivery#thread-semantics-per-platform) and [Dead-Target Registry](/features/dead-target-registry).

The gateway outbox handles ordered, retryable *queuing*. **Dedup** itself is now the shared `SqliteIdempotencyStore` at `~/.praisonai/state/delivery.db` — the same store the lightweight `AgentScheduler` and out-of-process tick use — so effectively-once delivery no longer depends on the gateway being up. See [Restart-safe delivery](/features/scheduler-delivery#restart-safe-delivery).

The same durable store now also backs the **inbound operator-request path** (the client → gateway `message` frame): a resend carrying the same `request_id` runs the turn exactly once. See [Gateway Client → Exactly-Once Send](/features/gateway-client#exactly-once-send-queue-and-flush).
</Note>
...
[Voice Notes (Speech-to-Text)](/features/gateway-stt).

The bot can also **speak back**. Configure an opt-in `voice:` block under any
channel to have the agent's reply delivered as a native voice note — the
outbound mirror of `stt:`. See
[Voice Reply (Text-to-Speech)](/features/gateway-voice-reply).
...
<Card title="Bind-Aware Authentication" icon="shield" href="/features/gateway-bind-aware-auth">
  Complete authentication security guide
</Card>

### Rotating the shared secret

Change `gateway.auth_token` in the running `gateway.yaml` and hot-reload — every already-connected session that authenticated under the old secret is force-closed with WebSocket close code `4001` and reason `credentials_rotated`. New connections use the new secret. Defaults on; opt out with `gateway.revoke_on_secret_rotation: false`. See [Gateway Credential Rotation](/features/gateway-credential-rotation) for the full behaviour matrix.

The auto-generated per-install HMAC secret (`~/.praisonai/gateway/.gateway_secret`, mode `0600`) is stored and permission-remediated separately from `gateway.auth_token`. Both the pairing codes and the inline Approve/Deny callback buttons sign with this **same** secret, so both survive gateway restarts by default — no operator action required. Set `PRAISONAI_CALLBACK_SECRET` to a shared value only when running the gateway on multiple hosts/replicas. See [Bot Pairing → Secret Management](/docs/features/bot-pairing#secret-management) for pairing codes and [Interactive Bot Actions](/features/interactive-bot-actions) for the callback-button flow.
...
<Card icon="shield-exclamation" href="/features/gateway-bind-aware-auth#weak-placeholder-secret-guard">
...
- Gateway commands are gated by content provenance — see [Control-Trust Provenance](/features/bot-control-trust).
...
<Card title="Gateway Client — Terminal vs Transient Errors" icon="plug" href="/features/gateway-client#terminal-vs-transient-connect-errors">
...
<Card title="Gateway Configuration" icon="code" href="/sdk/reference/typescript/classes/GatewayConfig">
...
| `revoke_on_secret_rotation` | `bool` | `true` | Force-close live sessions when `auth_token` is rotated via config reload. `false` adopts the new secret for new connections only. See [Gateway Credential Rotation](/features/gateway-credential-rotation). |
| `api.openai` | `bool` | `false` | When on, mounts `/v1/chat/completions`, `/v1/responses`, `/v1/models` on the same port and auth. See [Gateway API Endpoints](/features/gateway-api-endpoints). |
| `api.mcp` | `bool` | `false` | When on, mounts an MCP JSON-RPC endpoint at `/mcp` on the same port and auth. See [Gateway API Endpoints](/features/gateway-api-endpoints). |
...
channels:
  local: {}                  # chat from your own terminal — no token
  telegram:
    platform: telegram
...
Inbound voice notes on Telegram, Slack, and WhatsApp are transcribed automatically — see [Voice Notes](/features/gateway-voice-notes).

### Local terminal channel

Add `local: {}` alongside your remote channels to chat with the same live agent from your own terminal — no token, no webhook, one shared session across both surfaces. See [Local (Terminal) Channel](/features/local-channel).

<Tip>
Beyond `${ENV_VAR}`, `token` (and `app_token` / `verify_token`) accept a `{ source, id }` reference to load credentials from a mounted file or secret manager — never plaintext. See [Secret References](/features/gateway-secret-references).
</Tip>

<Tip>
Discord and Slack channels are auto-discovered on every refresh — your bot can address a channel before anyone messages it. See [Channel Directory](/features/gateway-channel-directory).
</Tip>
...
| `max_concurrent_runs_per_scope` | `int` | `None` | ≥ 0 — per-tenant sub-limit within `max_concurrent_runs`. See [Per-tenant fairness](/features/gateway-admission-control#per-tenant-fairness). |
...
| `reliability` | `str` | `None` | See [Gateway Reliability](/features/gateway-reliability). |
| `durable_runs` | `bool` | auto (true when session store is durable) | Auto-enabled when a durable session store is present (the default). Explicit `false` or `gateway.reliability: "off"` opts back out. Accepts env-substituted string forms. Per-agent `durable` overrides. See [Gateway Durable Runs](/features/gateway-durable-runs). |
...
| `notify_on_undelivered` | `bool` | `false` | Opt in to a last-resort user notice + `MESSAGE_UNDELIVERED` hook when a reply fails permanently. See [Undelivered Notice](/features/gateway-undelivered-notice). |
...
Both surfaces are opt-in and off by default. See [Gateway API Endpoints](/features/gateway-api-endpoints) for the endpoint list, auth, and client examples.
...
| `max_restarts_per_hour` | `int` | `10` | `≥ 0` (per channel) |
| `fleet_restarts_per_hour` | `int` | `40` | `≥ 1` — [fleet-level breaker](/features/gateway-crash-loop-guard#fleet-level-breaker) |
| `failing_channel_fraction` | `float` | `0.5` | `0.0 < x ≤ 1.0` |
| `breaker_cooldown_s` | `float` | `120.0` | `≥ 0` |
...
See [Gateway Inbound Hooks](/features/gateway-hooks) for signature-verification, event-filter, and deliver-only examples.

### `scheduler:` block

Tune the default `RunPolicy` the gateway applies to every unattended scheduled run. All fields are optional; omitted fields keep the fail-closed defaults.
...
| `scan_prompt` | `bool` | `true` | Run the injection scan over the assembled prompt before the turn. |
| `denied_toolsets` | `list[str]` | `DEFAULT_DENIED_TOOLSETS` | Tool categories removed for the unattended turn. Provide a list to override; omit to keep defaults. |
| `audit_dir` | `str` | `null` | Directory for the durable full-output audit trail. |

Overrides apply on the next tick without a process restart. See [Scheduled Run Policy](/features/scheduled-run-policy) for the full policy semantics.
...
Omitting `agent_id` on `POST`/`DELETE` targets a legacy any-agent grant; supplying it scopes to that agent. See [Gateway Scoped Approvals](/features/gateway-scoped-approvals) for full curl examples.
...
|--------|---------|---------------|
| `ScaleToZeroPolicy` | Quiesce when idle; wake on next inbound | [Scale to Zero](/features/gateway-scale-to-zero) |
| `DrainTimeoutPolicy` | Bounded wait for in-flight turns on shutdown | [Session Continuity](/features/gateway-session-continuity) |
| `DrainMarkerPolicy` + `current_epoch()` | Port-less, restart-safe external drain signal | [Drain Trigger](/features/gateway-drain-trigger) |
| `classify_exit_reason` + `FatalConfigError` | Supervisor-friendly sysexits exit codes (0 / 75 / 78) | [Gateway Exit Codes](/features/gateway-exit-codes) |
...
| `ThreadResult` + `ThreadStatus` | Typed outcome of `create_thread` — open a native thread/topic, never raises | [Gateway Threads](/features/gateway-threads) |
...
### Python API — repair and provisioning

The same detect → repair → re-validate lifecycle behind `praisonai gateway doctor --fix` and the interactive `praisonai onboard` wizard is available as a typed Python API — call it from a management script or notebook instead of shelling out to the CLI.
...
| Function | One-liner |
|----------|-----------|
| `repair_gateway_config(config_path, *, fix=False, dry_run=False)` | Detect → repair → re-validate a config; returns a `GatewayRepairResult`. |
| `provision_gateway_config(*, platform, token, agents=None, config_path=None)` | Write `bot.yaml` + persist the channel token to `~/.praisonai/.env`; returns the config `Path`. |

**Repair from a management script**
...
print("token minted:", result.auth_token_minted)
print("version migrated:", result.config_version_migrated)
for line in result.changes:
    print("changed:", line)

# JSON-safe copy for structured logging
import json
print(json.dumps(result.to_dict()))
```

**Provision on first run**
...
`provision_gateway_config` is fail-closed: an empty `token` or unsupported `platform` raises `ValueError` and writes nothing.

<Note>
**Python vs CLI.** Use the CLI (`praisonai gateway doctor --fix`, `praisonai onboard`) interactively; use these functions from CI, installers, and tests where you want a typed result to assert on. Both surfaces run the same helpers. Full reference: [Gateway Admin API](/features/gateway-admin-api).
</Note>
...
- Every git command the dispatcher runs is now bounded by `PRAISONAI_KANBAN_GIT_TIMEOUT` (default 300 s). A hung git operation returns `returncode=124` with a diagnosis in stderr instead of stalling the event loop. See [Gateway Subprocess Guard](/features/gateway-subprocess-guard).
...
Open a new thread under a chat so a delegated subtask has its own scoped conversation instead of flooding the shared channel.

```mermaid
graph LR
    subgraph "Thread Creation"
        A[👥 Parent Chat] --> B[🧵 create_thread]
        B --> C{Supports threads?}
        C -->|Yes| D[🆕 New Thread]
        C -->|No| E[📢 Parent Fallback]
        D --> F[✉️ send subtask]
        E --> F
    end
...
The running gateway registers an `OutboundMessengerProtocol` into the per-turn context. Resolve it with `get_outbound_messenger()`, open a thread, then route the subtask into it — falling back to the parent target when the platform can't thread.

```python
from praisonaiagents.session.context import get_outbound_messenger

async def handoff():
    messenger = get_outbound_messenger()
    res = await messenger.create_thread("telegram:12345", name="research")
    target = f"telegram:12345:{res.thread_id}" if res.ok else "telegram:12345"
    await messenger.send(target, "starting research...")
```
...
sequenceDiagram
    ...
    User->>Agent: delegate subtask
    Agent->>Messenger: create_thread(target, name)
    Messenger->>Adapter: open native thread/topic
    Adapter-->>Messenger: thread_id
    Messenger-->>Agent: ThreadResult(status="ok", thread_id=…)
    Agent->>Messenger: send("<target>:<thread_id>", subtask)
```

### `ThreadResult`

`create_thread(target, name) -> ThreadResult` **never raises** — a channel that cannot thread returns a typed `unsupported` outcome and the caller falls back to the parent target. Exported from `praisonaiagents.gateway` as `ThreadResult` and `ThreadStatus`.

| Field | Type | Description |
|-------|------|-------------|
| `status` | `Literal["ok", "unsupported", "failed", "no_route"]` | Outcome; drives fallback logic |
...
| `detail` | `Optional[str]` | Model-readable explanation |
| `.ok` | `bool` (property) | `status == "ok"` |
| `.as_dict()` | `dict` | Serializable form for tool return values |

`ThreadStatus` is a closed set of four values:

- `ok` — a new thread/topic was opened; `thread_id` carries its id.
- `unsupported` — the channel has no `supports_threads` capability.
- `failed` — the transport rejected the attempt (topics mode off, or missing permission).
- `no_route` — the target could not be resolved to a reachable channel.

### Fallback Pattern

Compose the routed target from the result — never wrap the call in `try/except`, it always resolves to a typed outcome:

```python
res = await messenger.create_thread(parent, name="subtask")
target = f"{parent}:{res.thread_id}" if res.ok else parent
await messenger.send(target, subtask_output)
```

### Capabilities: `supports_threads`

Threading is gated on the channel's `PlatformCapabilities.supports_threads`, which defaults to `False` — so unaware adapters degrade to `unsupported`, safe by default. Adapters that can open native threads must set `supports_threads=True`.

<Note>
Native thread implementations for Telegram / Discord / Slack live in the `praisonai-bot` wrapper. This page documents the protocol surface agents call; per-adapter conformance is tracked separately. See [Gateway Threads](/features/gateway-threads) for the adapter-authoring guide and per-platform support table.
</Note>
...
<Card title="Voice Reply" icon="waveform-lines" href="/features/gateway-voice-reply">
  Speak the agent's reply back as a native voice note
</Card>
<Card title="Run Status Controller" icon="gauge-simple" href="/features/bot-run-status-controller">
  Transport-agnostic run-progress state machine with a stall watchdog
</Card>
<Card title="Subprocess Guard" icon="shield-halved" href="/features/gateway-subprocess-guard">
  Bounded, non-interactive git/shell-outs so a hung command cannot stall the loop
</Card>
</CardGroup>