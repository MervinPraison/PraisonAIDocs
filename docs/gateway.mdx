...
New clients should use the `hello` message to open a connection. The server responds with `hello_ok` (negotiated protocol version, supported features, policy limits, and session ID) or `hello_error` (structured error code with a suggested next step).

```json
// Client sends:
{"type": "hello", "agent_id": "my-agent", "protocol_min": 1, "protocol_max": 1, "capabilities": ["streaming"]}

// Server responds:
{"type": "hello_ok", "protocol": 1, "features": {"methods": ["message", "leave"], "events": ["message", "error", "token_stream", ...]}, "session_id": "...", "cursor": 0}
```

<Card title="Handshake Protocol" icon="handshake" href="/docs/features/gateway-handshake-protocol">
  Full reference: version negotiation, capability matrix, policy limits, structured errors, and session resume
</Card>
