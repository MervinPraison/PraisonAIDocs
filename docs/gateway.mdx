...
| `durable_runs` | `bool` | `None` | Opt every gateway agent into durable runs (`ExecutionConfig(durable=True)`) so a restart resumes without re-firing tools or re-billing LLM calls. Accepts env-substituted string forms. Per-agent `durable` overrides. See [Gateway Durable Runs](/features/gateway-durable-runs). |
