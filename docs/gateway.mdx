...
### Programmatic repair and provisioning

The same detect → repair → re-validate lifecycle behind `praisonai gateway doctor --fix` and the interactive `praisonai onboard` wizard is now available as a typed Python API — see [Gateway Admin API](/features/gateway-admin-api).
