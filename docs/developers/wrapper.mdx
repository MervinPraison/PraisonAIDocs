...
| `framework` | `str \| None` | `None` (auto-detect) | One of `"crewai"`, `"praisonai"`, `"autogen"`, `"ag2"`, `"langgraph"`, `"openai_agents"`. LangGraph and OpenAI Agents must be selected explicitly — they are not in the auto-detect chain (`crewai` → `praisonaiagents` → `autogen` → `ag2`). |
