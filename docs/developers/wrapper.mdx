...
# Same as: praisonai run agents.yaml --llm gpt-4o
result = praisonai.run("agents.yaml", model="gpt-4o")

# Same as: praisonai run agents.yaml --resume abc123
result = praisonai.run("agents.yaml", session="abc123")

# Any other CLI option — forwarded through cli_config
result = praisonai.run("agents.yaml", approval="plan", verbose=True)
```

| Loose kwarg | Maps to | Notes |
|-------------|---------|-------|
| `model=` | `config_list[0]["model"]` (like `--llm`) | Also accepts the alias `llm=` |
| `session=` | `resume_session` (like `--resume`) | Renamed from the CLI-side `--session` for Python readability |
| Any other kwarg | `cli_config[key]` | Explicit `cli_config` values still win over loose kwargs |

```mermaid
graph LR
    Run[📞 praisonai.run] --> Kwargs{Loose kwarg?}
    Kwargs -->|model= / llm=| Model[Set config_list model]
    Kwargs -->|session=| Resume[Map to resume_session]
    Kwargs -->|anything else| Bridge[Forward via cli_config]
    Model --> Resolve[🔑 Resolve LLM endpoint]
    Resume --> Resolve
    Bridge --> Gen[🤖 AgentsGenerator]
    Resolve --> Gen
    ...
    class Run call
    class Kwargs check
    class Model,Resume,Bridge,Resolve config
    class Gen out
```
...
```mermaid
graph TB
    Inner[Shared framework tool]:::agent
    ProxyA[Generator A<br/>_TimeoutBoundTool proxy]:::process
    ProxyB[Generator B<br/>_TimeoutBoundTool proxy]:::process
    ExecA[Executor A]:::config
    ExecB[Executor B]:::config

    ProxyA -->|delegates schema| Inner
    ProxyB -->|delegates schema| Inner
    ProxyA -->|_run / run| ExecA
    ProxyB -->|_run / run| ExecB
...
Sharing an `AgentsGenerator` across tenants is still discouraged (per-instance timeout executor lifecycles are per-instance), but sharing the underlying **tool objects** across generators is now safe — the per-generator proxy guarantees that A's timeout wrapper cannot leak into B's calls, and closing A's pool cannot break B's calls on the same tool.
...
<CardGroup cols={2}>
<Card title="praisonai-bot Migration Guide" icon="package" href="/guides/praisonai-bot-migration">
  Install options, CLI reference, backward-compat guarantees, and channel extension guide
</Card>
<Card title="Built-in Channels" icon="robot" href="/features/messaging-bots">
...
The `praisonai.bots` / `praisonai.gateway` / `praisonai.daemon` `alias_package` shims default to an **eager** subtree walk at shim-install time. This preserves `old_name.sub is new_name.sub` — the module-identity guarantee that keeps `unittest.mock.patch("old.sub.attr")` and `isinstance` checks working after the C9 migration, even when the running code imports the moved package under its **new** name.

Set `PRAISONAI_SHIM_LAZY=1` to opt **out** of the eager walk (pure `_AliasFinder` lazy resolution). The trade-off: faster shim install and no eager pull of optional heavy deps — but `mock.patch` against the old dotted path can miss when code imports via the new name.

As of [PR #4261](https://github.com/MervinPraison/PraisonAI/pull/4261), a failed eager import is no longer silently swallowed — it logs a warning with traceback (`shim: eager import of %s failed; will be resolved lazily`) and is left for `_AliasFinder` to resolve lazily. `None` placeholders in `sys.modules` are skipped instead of poisoning the old dotted alias. The warning is a useful debugging aid when a downstream `AttributeError` on the alias would otherwise have no traceback pointing back at the shim.

<AccordionGroup>
<Accordion title="When to keep the default (eager)">
...
<Accordion title="When to set PRAISONAI_SHIM_LAZY=1">
Use it only when no `mock.patch` targets the old dotted path and minimal-container startup speed matters more than mock-patch parity. See [Security Environment Variables → `PRAISONAI_SHIM_LAZY`](/features/security-environment-variables#praisonai_shim_lazy).
</Accordion>
</AccordionGroup>
...
|--------|------|-----------|---------|
| `_ensure_session(self) -> str` | sync | ✅ | Return a session id; called on the producer thread before `_iter_events` |
| `_iter_events(self, session_id, prompt) -> Iterator[str]` | sync generator | ✅ | Yield text chunks for `prompt` on `session_id`; runs on the producer thread so blocking SDK calls are fine |
| `_after_stream(self, full: str) -> None` | sync | ⬜ | Post-stream bookkeeping (persist the assembled assistant turn, sync usage). Default: no-op |
...
```python
from praisonai.integrations._managed_base import ManagedBackendBase

class MyManagedAgent(ManagedBackendBase):
    def _ensure_session(self) -> str:
        # Create/lookup your session with your provider; return its id.
        return self._session_id or self._provision_session()

    def _iter_events(self, session_id, prompt):
        # Runs on the producer thread — blocking SDK calls are fine here.
        for chunk in self._provider_client.stream(session_id, prompt):
            if chunk.text:
                yield chunk.text

    def _after_stream(self, full: str) -> None:
        # Optional: persist the assembled assistant turn + usage.
