...
The `praisonai.adapters` module lazily re-exports existing adapter classes (`AutoReader`, `ChromaVectorStore`, `BasicRetriever`, `FusionRetriever`, `LLMReranker`, and registration helpers) to preserve backward compatibility. Registration into the core-SDK singletons is now explicit — call `praisonai.adapters.register_default_adapters()` to wire the built-in readers/retrievers/rerankers in a single idempotent step, replacing the earlier import-time auto-registration (PraisonAI 4.6.156+).
