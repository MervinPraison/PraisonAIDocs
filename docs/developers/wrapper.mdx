...
<Steps>
<Step>
  Auto-detects the first installed framework in this order: `crewai` → `praisonaiagents` → `autogen` → `ag2`. Pass `framework="openai_agents"` (or any other registered framework name) to override.
</Step>
<Step>
...
| `framework` | `str \| None` | `None` (auto-detect) | One of `"crewai"`, `"praisonai"`, `"autogen"`, `"ag2"`, `"openai_agents"`. `openai_agents` is not in the auto-detect order — pass it explicitly. |
