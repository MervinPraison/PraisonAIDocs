...
### Fully-Mocked Test Invisible to CI

**Symptom:** A fully-mocked file mentions a provider name (e.g. `openai/mistral`, `ollama/llama2`), so the gating plugin infers `network` and the CI marker expression deselects it.

**One-liner check:**
```bash
pytest path/to/test_file.py -m "not network and not provider_ollama" --collect-only -q
```

**Likely root cause:** Whole-file `network` inference on a mocked file that references a provider keyword

**Common failure signature:**
```
N deselected / 0 selected
```

**Fix approach:**
- Add `pytestmark = pytest.mark.offline` at module scope to opt out of provider *and* network auto-marking
- Confirm the file is truly mocked (patches `litellm.completion` or similar) before marking it offline

<Note>
This was the exact defect PR #4801 fixed for `test_base_url_api_base_fix.py` — 7 tests, 100% invisible to CI before the fix.
</Note>
