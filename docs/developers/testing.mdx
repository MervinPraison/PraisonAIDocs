...
- `offline` - Fully mocked; opts the test out of provider *and* network auto-marking. Use on a test or as `pytestmark = pytest.mark.offline` at the top of a fully-mocked module that mentions provider names in its source.
...
@pytest.mark.offline
def test_fully_mocked():
    """Fully mocked — opt out of provider/network auto-marking."""
    pass
...
Provider markers are inferred per test function. The gating plugin parses each test's own source segment plus its decorators; module-level context (imports, `pytestmark`, fixtures) is shared. A provider name inside one test body does **not** leak to siblings.

If your test file is fully mocked but references a provider name — for example a `test_openai_*.py` that patches `litellm.completion` — declare it offline so CI actually selects it:
...
`offline` disables both provider and `network` auto-marking for that test.

The `network` marker itself is still inferred whole-file from provider keywords, so a live-network test cannot silently un-gate just because a neighbour narrowed its provider surface. Only `offline` (or an explicit `real`/`network` marker) changes that.

```mermaid
graph TB
    T[🧪 Test collected] --> Off{Marked offline?}
    Off -->|Yes| Skip[⏭️ No provider/network auto-marking]
    Off -->|No| Per[🔎 Per-test provider scan<br/>body + decorators + module scope]
    Per --> File[📄 Whole-file network scan]
    File --> Add[✅ Add markers]
    ...
    class T start
    class Off decision
    class Per,File action
    class Skip,Add success
```
