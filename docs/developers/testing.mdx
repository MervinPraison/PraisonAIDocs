...
- `allow_sleep` - Opt out of the autouse `fast_sleep` fixture so `time.sleep` runs for real (use for timing-sensitive tests that assert wall-clock duration)
...
The unit-test suite installs an autouse `fast_sleep` fixture (`tests/conftest.py`) that monkeypatches `time.sleep` to clamp every call to **1ms**. This keeps the suite fast — but it breaks any test that asserts a real wall-clock duration (e.g. profiler `duration_ms`, streaming TTFT).

Opt out with `@pytest.mark.allow_sleep`:
...
def test_api_call_duration():
    """Real `time.sleep` runs because of the marker."""
    ...
    with Profiler.api_call("https://api.example.com/test", method="GET") as call:
        time.sleep(0.05)  # 50ms — actually sleeps because of @allow_sleep

    calls = Profiler.get_api_calls()
    # Loose lower bound: time.sleep precision varies by ~5ms on busy CI runners.
    assert calls[-1].duration_ms >= 5
```

**Guidance:**

- Use a **loose** lower bound (≈ 5ms below the requested sleep). `time.sleep` can under-deliver by a few ms on busy CI runners, so tight bounds like `>= 50ms` for a 50ms sleep will be flaky.
- Sleep at least 50ms when you need a measurable duration. Anything shorter is dominated by scheduling jitter.
- Forgetting `@allow_sleep` is the failure mode: the test passes locally (where the marker would be honored anyway in some environments) but fails on CI with `assert <small-number> >= 5`. See PR #1581 for a real example.
...
### Timing assertion fails with a tiny duration

If a duration/TTFT assertion fails like `assert 1.13... >= 5`, the test is hitting the `fast_sleep` clamp. Add `@pytest.mark.allow_sleep` to the test.
