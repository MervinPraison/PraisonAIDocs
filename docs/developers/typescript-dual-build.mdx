...
### When a lazy require is a footgun

A `require()` inside a function body earns the file the banner — even if the file's only importer already re-exports the same symbols statically, so the "lazy" call deferred nothing. This bit the mobile graph before [PR #4720](https://github.com/MervinPraison/PraisonAI/pull/4720): three files (`llm/backend-resolver.ts`, `llm/providers/ai-sdk/index.ts`, `llm/providers/registry.ts`) had lazy provider loaders whose `require()` calls deferred nothing (the upstream files already loaded the modules statically), yet each earned the banner and each broke `praisonai/mobile` at the `chrome58` floor. Rule of thumb: **before you add a `require()` to a `praisonai-ts` source, check whether the file's importers already load the module statically. If they do, use a static `import`. If they don't, and you truly need the deferral, use `await import(…)` inside a function — never at module scope.**

See [Mobile & Webview Entry → Bundle-Size and Parse Floor](/js/mobile-entry) for the reader-facing side of this floor.
