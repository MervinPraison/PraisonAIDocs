...
<Note>
`framework: google_adk` does not support auto-generation mode (`--auto`). Use an explicit `agents.yaml` with the `roles:` format and pass `--framework google_adk` directly. See the [Google ADK](/framework/google-adk) page for the runnable YAML shape.
</Note>
...
Auto uses LiteLLM if installed (100+ providers), with OpenAI as a fallback. See [Auto Mode Providers](/features/auto-generator-providers).
