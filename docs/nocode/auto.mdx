...
# OpenAI Agents (requires explicit --framework flag; auto-detect does not select openai_agents)
python -m praisonai --framework openai_agents --auto "research AI trends"
```

<Note>
Auto-detect (`--auto` without `--framework`) tries frameworks in the order: `crewai` → `praisonaiagents` → `autogen` → `ag2`. To use OpenAI Agents in auto mode, pass `--framework openai_agents` explicitly.
</Note>
