...
# OpenAI Agents (explicit flag — not in auto-detect order)
python -m praisonai --framework openai_agents --auto "research AI trends"
...
<Note>
`framework: google_adk` does not support auto-generation mode (`--auto`). Use an explicit `agents.yaml` with the `roles:` format and pass `--framework google_adk` directly. See the [Google ADK](/docs/framework/google_adk) page for the runnable YAML shape.
</Note>
