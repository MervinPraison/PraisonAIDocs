...
<Warning>
The `[flow]` extra is opt-in and heavy (~500MB) because Langflow pulls many dependencies (Langflow, `a2a-sdk[http-server]`, `fastapi-pagination`, and the FastAPI/Starlette stack). Install only when you need the visual builder: `pip install "praisonai[flow]"`.

**`[flow]` and `[claw]` cannot coexist.** They pin conflicting versions of `a2a-sdk` (`>=1.0.0` for Langflow, `<1.0.0` for the A2UI SDK). Install them in **separate virtual environments**.
</Warning>

<Note>
**Python 3.13 supported** as of PraisonAI (post-[#5103](https://github.com/MervinPraison/PraisonAI/pull/5103)). First-boot on Windows takes ~5 minutes because Langflow builds its migration/cache database — this is Langflow behaviour, not a hang. Watch for `Uvicorn running on http://127.0.0.1:7860` in the log tail.
</Note>
...
See [Flow CLI](/cli/flow) for the full flag reference.
...
<CardGroup cols={2}>
<Card title="Flow CLI" icon="diagram-project" href="/cli/flow">
  CLI reference for all subcommands and flags
</Card>
<Card title="Langflow Integration" icon="plug" href="/integrations/langflow">
  Component reference and model formats
</Card>
<Card title="Agent Workflow" icon="route" href="/features/workflows">
