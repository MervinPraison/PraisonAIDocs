...
On later launches the app also checks the venv against the shipped requirement and re-runs the install once if it drifted (the system tray reads `Engine: updating…` while it runs). This is why a fix in `praisonaiagents` reaches you on the next launch instead of requiring you to delete the venv by hand.

To remove the app completely, uninstall it and delete that folder.

## Video generation

The **Video** tab has a model picker. MiniMax video-01 (Replicate) is a preset, not the only option — SDK-capable providers (OpenAI Sora 2, Google Veo 3.1) become selectable once their key is set. Add the relevant key to the `.env` file inside the data folder above:
...
Models without a configured key appear disabled in the dropdown. See [Video generation](/features/desktop/video) for the full walkthrough.
...
```mermaid
graph LR
  M[🧩 Model id] --> S{Has a slash?}
  S -->|No, bare id| OK[✅ Accepted]
  S -->|Yes, provider/model| NO[❌ Rejected at save]
  OK --> B[🌐 Optional Base URL → any endpoint]
  classDef q fill:#F59E0B,stroke:#7C90A0,color:#fff
...
| ✅ Works | ❌ Rejected |
|---|---|
| `gpt-4o` | `ollama/llama3.2` |
| `gpt-4o-mini` | `anthropic/claude-3-sonnet` |
| `gpt-5` | `gemini/gemini-2.0-flash` |
| `llama3.2` (with a Base URL pointing at your Ollama server) | any `provider/model` id |
...
<Steps>
  <Step title="Enter the bare model name">
...
To reach Anthropic, Gemini, or others, use the same pattern: a bare model id plus a Base URL for any provider that exposes an OpenAI-compatible endpoint (or a proxy such as LiteLLM Proxy). See [OpenAI-compatible endpoints](/models/openai-compatible) and [LiteLLM Proxy](/models/litellm-proxy).
