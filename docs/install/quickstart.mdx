...
|-----------|-------------|
| **praisonaiagents** | Core SDK for building AI agents (pip extras must be added separately if needed) |
...
<AccordionGroup>
  ...
  <Accordion title="Pip extras">
    Install only the features you need, or grab everything at once:
    ...
    pip install "praisonaiagents[memory]"      # ChromaDB + LiteLLM
    pip install "praisonaiagents[knowledge]"   # Mem0 + Markitdown + Chonkie
    pip install "praisonaiagents[graph]"       # Mem0 graph memory
    pip install "praisonaiagents[llm]"         # LiteLLM provider
    pip install "praisonaiagents[mcp]"         # MCP client/server
    pip install "praisonaiagents[api]"         # FastAPI + Uvicorn
    pip install "praisonaiagents[os]"          # AgentOS server (api + auth)
    pip install "praisonaiagents[auth]"        # JWT + bcrypt + python-jose
    pip install "praisonaiagents[autonomy]"    # ast-grep-cli for code intelligence
    pip install "praisonaiagents[search]"      # DuckDuckGo web search
    pip install "praisonaiagents[crawl]"       # Crawl4AI + Playwright
    pip install "praisonaiagents[mongodb]"     # MongoDB driver
    pip install "praisonaiagents[telemetry]"   # PostHog telemetry
    ```

    <Note>
      `[all]` includes every extra above (memory, knowledge, graph, llm, mcp,
      api, os, auth, autonomy, search, crawl, mongodb, telemetry). If you
      install `[all]`, you do **not** need any of the others.
    </Note>
  </Accordion>
