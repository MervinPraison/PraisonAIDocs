...
The Windows PowerShell installer bootstraps Python, creates an isolated venv, and installs the full `praisonai[all]` wrapper so `praisonai setup` runs at the end.

<Info>
As of PraisonAI **4.6.145+** ([PR #3029](https://github.com/MervinPraison/PraisonAI/pull/3029)), `install.ps1` ships at `src/praisonai/scripts/install.ps1` (published as `https://praison.ai/install.ps1`) and installs `praisonai[all]` by default, matching `install.sh`.
</Info>

```mermaid
flowchart LR
    A[iwr install.ps1 | iex]:::tool --> B[Detect / install Python 3.10+]:::process
    B --> C[Create venv at .praisonai/venv]:::process
    C --> D[pip install praisonai all]:::process
    D --> E[Add Scripts dir to user PATH]:::process
    E --> F[python -m praisonai setup]:::success
...
# With options (must use scriptblock)
& ([scriptblock]::Create((iwr -useb https://praison.ai/install.ps1))) -Extras "ui,chat"
```

<Check>
The Windows installer automatically:
- Detects an existing Python 3.10+ (`python`, `python3`, `py -3.12/-3.11/-3.10`); installs Python 3.12 via `winget` → Chocolatey → Scoop if none found.
- Creates a venv at `%USERPROFILE%\.praisonai\venv` (skip with `-NoVenv`).
- Installs `praisonai[all]` (or `praisonai[$Extras]` if `-Extras` set) into that venv.
- Adds `%USERPROFILE%\.praisonai\venv\Scripts` to your **user** `PATH`.
- Runs `python -m praisonai setup` at the end unless `-NoOnboard` is set.
</Check>
...
|-----------|------|---------|-----------|
| `-Version` | `string` | `"latest"` | Pin a specific version (appended as `==VERSION`) |
| `-Extras` | `string` | `""` | Comma-separated extras (e.g. `"ui,chat,code"`), applied as `praisonai[EXTRAS]` |
| `-NoVenv` | `switch` | off | Skip the `%USERPROFILE%\.praisonai\venv` virtualenv |
| `-NoOnboard` | `switch` | off | Skip the interactive `praisonai setup` wizard after install |
| `-Python` | `string` | `""` | Path to a specific Python executable |
| `-DryRun` | `switch` | off | Print what would happen without making changes |
| `-Help` | `switch` | off | Show help |
...
| `PRAISONAI_EXTRAS` | `-Extras` |
| `PRAISONAI_SKIP_VENV=1` | `-NoVenv` |
| `PRAISONAI_NO_ONBOARD=1` | `-NoOnboard` |
| `PRAISONAI_PYTHON` | `-Python` |
| `PRAISONAI_DRY_RUN=1` | `-DryRun` |
| `PRAISONAI_NO_PROMPT` | Suppress interactive onboarding (any truthy value) |

<Tip>
WSL and manual routes still work: `pipx install praisonai`, `pip install "praisonai[all]"`, or the Linux/macOS one-liner inside WSL.
</Tip>
