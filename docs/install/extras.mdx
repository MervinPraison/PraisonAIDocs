---
title: "Installation Extras"
sidebarTitle: "Extras"
description: "Optional dependency groups for praisonaiagents"
icon: "puzzle-piece"
---

Pick the features you need, or install `[all]` to get everything in one go.

```mermaid
graph LR
    subgraph "Pip Extras"
        Core[📦 praisonaiagents] --> All[🎁 all]
        All --> Memory[🧠 memory]
        All --> Knowledge[📚 knowledge]
        All --> AgentOS[🚀 os]
        All --> Autonomy[🤖 autonomy]
        All --> MCP[🔌 mcp]
        All --> Etc[…+ 7 more]
    end

    classDef core fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef all fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef extra fill:#189AB4,stroke:#7C90A0,color:#fff

    class Core core
    class All all
    class Memory,Knowledge,AgentOS,Autonomy,MCP,Etc extra
```

## Quick Start

<Steps>
<Step title="Everything">
```bash
pip install "praisonaiagents[all]"
```
</Step>

<Step title="Pick specific features">
```bash
pip install "praisonaiagents[memory,mcp]"
```
</Step>
</Steps>

## Available Extras

| Extra | What it adds | Use when… |
|-------|--------------|-----------|
| `all` | Every extra below in one go | You want zero friction |
| `memory` | ChromaDB + LiteLLM | Storing agent memory |
| `knowledge` | Mem0 + Markitdown + Chonkie | RAG over documents |
| `graph` | Mem0 with graph capabilities | Graph-based memory |
| `llm` | LiteLLM provider | Using non-OpenAI LLMs |
| `mcp` | MCP protocol (`mcp`, `fastapi`, `uvicorn`, `websockets`) | Connecting MCP tool servers |
| `api` | FastAPI + Uvicorn | Exposing agents over HTTP |
| `os` | `api` + `PyJWT` | Running AgentOS / a2a / agui servers |
| `auth` | PyJWT + bcrypt + python-jose + python-multipart | Securing endpoints |
| `autonomy` | `ast-grep-cli` | Autonomy mode + code-intelligence tools |
| `search` | `ddgs` | Zero-config DuckDuckGo web search |
| `crawl` | `crawl4ai` + Playwright (run `playwright install chromium`) | Web crawling |
| `mongodb` | `pymongo` + `motor` | MongoDB persistence |
| `telemetry` | PostHog | Anonymous usage telemetry |

<Note>
As of praisonaiagents `v1.6.37` (PraisonAI PR #1632), `[all]` includes
both `[autonomy]` and `[os]`. Earlier versions did not, and
`from praisonaiagents.config import AutonomyConfig` raised `ImportError`
after `pip install "praisonaiagents[all]"`. Upgrade to ≥ `1.6.37` to fix.
</Note>

## Common Patterns

<AccordionGroup>
  <Accordion title="Autonomous Agent">
    For agents that think and plan:
    ```bash
    pip install "praisonaiagents[autonomy,memory]"
    ```
    Gives you `AutonomyConfig` and memory storage.
  </Accordion>
  
  <Accordion title="AgentOS Server">
    For deploying agent servers:
    ```bash
    pip install "praisonaiagents[os]"
    ```
    Includes FastAPI, auth, and all server dependencies.
  </Accordion>
  
  <Accordion title="Web Crawler">
    For web scraping and crawling:
    ```bash
    pip install "praisonaiagents[crawl,search]"
    ```
    Then run `playwright install chromium` for browser support.
  </Accordion>
  
  <Accordion title="Everything">
    Zero configuration, all features:
    ```bash
    pip install "praisonaiagents[all]"
    ```
    Perfect for development and getting started.
  </Accordion>
</AccordionGroup>

## Related

<CardGroup cols={2}>
  <Card icon="bolt" href="/install/quickstart">
    The one-line installer
  </Card>
  <Card icon="robot" href="/features/autonomy-loop">
    Uses the `[autonomy]` extra
  </Card>
</CardGroup>