...
Analyse stock prices, company fundamentals, and historical trends with a single Agent — no financial-data API glue code required.
...
agent = Agent(
    name="Finance",
    instructions="Analyse stocks and provide insights.",
    tools=[get_stock_price, get_stock_info, get_historical_data],
)
...
agent = Agent(
    name="Finance",
    instructions="Analyse stocks and remember prior comparisons.",
    tools=[get_stock_price, get_stock_info, get_historical_data],
    memory=True,
)

agent.start("Compare AAPL and GOOGL and rank them for a long-term hold.")
```
...
sequenceDiagram
    ...
    User->>Agent: Analyze AAPL vs MSFT
    Agent->>FinanceTools: get_stock_price / get_historical_data
    FinanceTools-->>Agent: Live price + trend data
    Agent-->>User: Investor-friendly summary
```
...
<AccordionGroup>
<Accordion title="Use a plain Agent with finance tools, not a custom class">
Financial analysis is a plain `Agent` plus the yfinance tools — there is no dedicated `FinanceAgent` class. Pass `get_stock_price`, `get_stock_info`, and `get_historical_data` and let the LLM decide which to call.
</Accordion>

<Accordion title="Enable memory for portfolio tracking">
Set `memory=True` when the same user asks follow-up questions across sessions, so prior comparisons inform new recommendations.

```python
agent = Agent(
    name="Finance",
    instructions="Track this portfolio over time.",
    tools=[get_stock_price, get_stock_info, get_historical_data],
    memory=True,
)
```
...
<Accordion title="Return structured output for downstream use">
Attach an `output_pydantic` model to a `Task` when the result feeds a dashboard or database rather than a human reader.
</Accordion>

<Accordion title="Instruct the agent to cite its data">
LLMs can hallucinate prices. Tell the agent to only report figures returned by the tools, so numbers stay grounded in live data.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Data Analyst" icon="chart-line" href="/agents/data-analyst">
    Analyse CSV and Excel data with a single Agent.
  </Card>
  <Card title="Research Agent" icon="magnifying-glass" href="/agents/research">
    Gather market research from the web.
  </Card>
</CardGroup>
