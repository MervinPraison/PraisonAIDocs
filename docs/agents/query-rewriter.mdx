...
```mermaid
graph LR
    subgraph "Query Rewriter Agent"
        User[📋 User Query] --> Agent[🤖 QueryRewriterAgent]
        Agent --> Strategy[🧠 Rewrite Strategy]
        Strategy --> Result[✅ Optimized Queries]
    end
...
result = agent.rewrite(
    "How do I set up a RAG pipeline and which embedding models should I use?",
...
sequenceDiagram
    participant User
    participant QueryRewriterAgent
    participant Strategy

    User->>QueryRewriterAgent: rewrite("AI trends")
    QueryRewriterAgent->>Strategy: Apply BASIC/HyDE/STEP_BACK/SUB_QUERIES
    Strategy-->>QueryRewriterAgent: Optimized query variants
    QueryRewriterAgent-->>User: RewriteResult with primary_query
```
...
<AccordionGroup>
<Accordion title="Pick the strategy for the query shape">
...
<Accordion title="Retrieve with all queries, then merge">
...
<Accordion title="Register domain abbreviations">
Call `add_abbreviations({"K8s": "Kubernetes"})` so the rewriter expands your jargon correctly instead of guessing.
</Accordion>

<Accordion title="Use Prompt Expander for task prompts, not search">
...
<CardGroup cols={2}>
  <Card icon="expand" href="/docs/agents/prompt-expander">
    Expand prompts for task execution instead of search.
  </Card>
  <Card icon="vector-square" href="/docs/agents/embedding">
