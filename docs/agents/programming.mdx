...
<Note>
This page uses a plain `Agent` with code tools — ideal as a task-focused recipe. For the dedicated `CodeAgent` class with sandboxed `generate`/`execute`/`review`/`refactor` methods, see the [Code Agent](/agents/code) page.
</Note>
