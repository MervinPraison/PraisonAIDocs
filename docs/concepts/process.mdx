...
<Card>
  ...
  Hierarchical processes include automatic error recovery mechanisms:
  
  1. **Manager LLM parse failures retry up to 3 times** with exponential backoff (2s, 4s, 8s). After 3 failures, the workflow stops and logs the final exception.
  2. **Invalid task IDs are re-prompted** — if the manager picks a `task_id` not in the workflow, the manager is asked again with the list of valid IDs (up to 3 attempts before aborting).
  3. **Loop subtask file errors are now visible** — failures to read a loop input file mark the task `status="failed"` and attach a `TaskOutput` with the error message instead of failing silently.
  
  ```mermaid
  graph TD
      Start([Manager Request]) --> Parse{Parse Response}
      Parse -->|Success| Execute[Execute Task]
      Parse -->|Parse Error| Retry{Retry < 3?}
      Retry -->|Yes| Wait[Exponential Backoff]
      Wait --> Start
      Retry -->|No| Stop[❌ Stop Workflow]
      
      Execute --> ValidID{Valid Task ID?}
      ValidID -->|Yes| Success[✅ Task Executed]
      ValidID -->|No| Reprompt{Reprompt < 3?}
      Reprompt -->|Yes| Manager[Re-ask Manager]
      Manager --> Parse
      Reprompt -->|No| Stop
      ...
      class Stop error
      class Retry,Reprompt,Wait retry
  ```
  
  <Note>
  The retry constants (`MAX_MANAGER_RETRIES = 3`, `MAX_INVALID_SELECTIONS = 3`) are not user-configurable today; they are SDK defaults for reliability.
  </Note>
</Card>
