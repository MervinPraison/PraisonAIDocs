...
Advanced workflow orchestration using the `Workflow` class with pattern helpers.
...
<Card>
  ...
  def classifier(ctx): return StepResult(output="success")
  def handler_a(ctx): return StepResult(output="Handled A")
  def handler_b(ctx): return StepResult(output="Handled B")
