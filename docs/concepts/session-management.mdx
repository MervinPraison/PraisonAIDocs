...
sequenceDiagram
    ...
    User->>Session: close()
    Session->>Memory: close_connections()
    Memory->>Adapter: close()
    Adapter-->>Memory: cleanup complete
    Memory-->>Session: cleanup complete
    Session-->>User: session closed
    
    classDef user fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef session fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef memory fill:#10B981,stroke:#7C90A0,color:#fff
...
# Session cleanup no longer requires manual teardown
try:
    agent = Agent(
        name="Assistant",
        memory={"session_id": "my-session"}
    ...
    # Session.close() automatically handles memory cleanup
    if hasattr(agent, '_memory_instance'):
...
<CardGroup cols={2}>
  ...
  <Card title="Memory System" icon="brain" href="/docs/concepts/memory">
    Memory integration
  </Card>
  <Card title="CLI Commands" icon="terminal" href="/docs/cli/session">
