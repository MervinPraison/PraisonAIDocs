...
| `memory="in_memory"` | None | In-memory with FIFO eviction |
...
| `memory="mongodb"` | pymongo | MongoDB document store |
...
# Create in-memory adapter with size limit
adapter = InMemoryAdapter(max_size=500)  # Cap at 500 entries
agent = Agent(
    name="Assistant",
...
# Only the most recent 500 entries remain
print(len(agent._memory_instance._data))  # 500
```
...
|--------|------|---------|-------------|
| `max_size` | `int` | `10_000` | Maximum entries before FIFO eviction |
...
# ChromaDB memory will auto-cleanup on close
agent = Agent(
    name="Assistant",
    memory={"provider": "chromadb", "rag_db_path": "./chroma_db"}
)

# MongoDB memory requires pymongo installation
agent_mongo = Agent(
    name="Assistant",
    memory={
        "provider": "mongodb",
...
# Memory.close_connections() now calls adapter.close() automatically
agent._memory_instance.close_connections()
```
