...
PraisonAI provides two distinct execution approaches: **Autonomy** (structured result with optional multi-turn loops) and **Interactive** (single-turn, human-in-the-loop). Understanding the `mode` field — `"caller"` vs `"iterative"` — helps you choose the right approach.
...
<Note>
**Key difference**: In **caller mode**, the agent performs one `chat()` call and wraps the result. In **iterative mode**, the agent drives a loop internally with safety checks. In **interactive mode**, the human drives the loop externally. All three use the same `chat()` → tool executor underneath.
</Note>
...
|-----------|-------------------------------|------------------------------|---------------------------|
| **Mode** | `"caller"` | `"iterative"` | N/A |
| **Turns** | 1 (single `chat()`) | Up to `max_iterations` | Single turn per call |
| **Return type** | `AutonomyResult` | `AutonomyResult` | `str` |
| **Self-correction** | ❌ | ✅ Doom loop + recovery | ❌ |
| **Completion signal** | `"caller_mode"` | Promise/keyword detection | N/A |
| **Stage escalation** | ❌ | ✅ Auto-escalate | N/A |
