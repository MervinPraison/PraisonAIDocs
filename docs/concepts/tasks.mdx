...
```python
# 'action' is a user-friendly alias for 'description'
task = Task(
...
| `action` | `str` | Alias for description (simpler) |
...
Use `context` or `depends_on` (alias) to chain tasks:
...
| `action` | `str` | - | Alias for description |
