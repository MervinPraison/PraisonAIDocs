...
## Task - The Unified Unit of Work

`Task` is the unified class for defining work units in PraisonAI. It consolidates features from the legacy `Task` class.

<CodeGroup>
```python Action Alias
from praisonaiagents import Task, Agent

# 'action' is an alias for 'description' (simpler, more intuitive)
task = Task(
...
# Task with custom handler (no agent needed)
...
```python Loop Support
# Task with loop support
task = Task(
    action="Process item: {{item}}",
    loop_over="items",      # Variable containing list to iterate
    loop_var="item"         # Variable name for each item
)
```
...
```python Feature Configs
# Task with feature configs
task = Task(
    action="Advanced task",
    execution={"max_retries": 5, "timeout": 30},
    routing={"next": ["task_b", "task_c"]},
    autonomy=True,
    knowledge=["docs/"],
    web=True,
    reflection=True,
...
```python Robustness
# Task with robustness features for graceful degradation
task = Task(
    action="Optional enrichment step",
    skip_on_failure=True,   # Workflow continues if this task fails
...
<Warning>
`Task` is deprecated. Use `Task` instead. Existing code will show a deprecation warning but continue to work.
</Warning>

```python
# OLD (deprecated, shows warning)
from praisonaiagents import Task
step = Task(name="writer", action="Write content")

# NEW (recommended)
from praisonaiagents import Task
task = Task(name="writer", action="Write content")
```

| Task Param | Task Equivalent | Notes |
|:-------------------|:----------------|:------|
| `action` | `action` or `description` | Both work, `action` is alias |
| `handler` | `handler` | Custom function execution |
| `should_run` | `should_run` | Conditional execution |
| `loop_over` | `loop_over` | Iteration support |
| `loop_var` | `loop_var` | Loop variable name |
| `execution` | `execution` | Execution config dict |
| `routing` | `routing` | Next step routing |
...
<CodeGroup>
  ...
  # Note: 'Agents' is deprecated, use 'AgentManager' instead
  ...
  # Define a task (using 'action' alias)
  task = Task(
      name="research_task",
      action="Analyze 2024's AI advancements",  # 'action' = 'description'
      expected_output="A detailed report",
      agent=researcher
  )

  # Run with AgentManager
  ...
  task1 = Task(
      name="research_task",
      action="Analyze 2024's AI advancements",
      expected_output="A detailed report",
      agent=researcher
  )
  ...
  task2 = Task(
      name="writing_task",
      action="Create a blog post about AI advancements",
