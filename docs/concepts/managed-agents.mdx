...
> **See also:** [Five-Layer Agent Stack](/docs/features/five-layer-agent-stack) — Managed Agents sit **outside the stack** (where the agent runs).
...
<CardGroup cols={2}>
<Card title="Local Provider" icon="desktop" href="/concepts/managed-agents-local">
...
<Card title="Modal Compute" icon="server" href="/concepts/managed-agents-modal">
  Serverless compute for scalable workloads
</Card>
<Card title="Daytona Workspaces" icon="code" href="/concepts/managed-agents-daytona">
  Development environments with persistent storage
</Card>
<Card title="Tenki Cloud" icon="cloud" href="/features/managed-agents-tenki">
  Disposable Linux microVMs
</Card>
</CardGroup>
...
<Card title="ManagedConfig API Reference" icon="code" href="/sdk/reference/typescript/classes/ManagerConfig">
...
- **Tenki**: Disposable Linux microVMs, provisioned in seconds
</Accordion>
...
Track token usage with `retrieve_session()` — returns a unified schema (`id`, `status`, `title`, `usage`) on all backends. See the [SessionInfo Schema page](/features/managed-agents-session-info) for details.
</Accordion>
</AccordionGroup>
