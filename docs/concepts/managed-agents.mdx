...
<Warning>
`ManagedAgent` is the legacy factory. New code should use [`HostedAgent`](/docs/features/hosted-agent) for cloud-managed runtimes or [`LocalAgent`](/docs/features/local-agent) for local execution. The `provider=` parameter on `ManagedAgent` now emits `DeprecationWarning` for LLM and compute hints.
</Warning>
...
<Tab title="Recommended (HostedAgent)">
```python
from praisonaiagents import Agent
...
<Tip>
`HostedAgentConfig is ManagedConfig` and `LocalAgentConfig is LocalManagedConfig` — same fields, new names — so users do not need to relearn anything.
</Tip>
