...
```mermaid
graph LR
    subgraph "LocalManagedAgent Tool Resolution"
        A[tools=...] --> Alias[TOOL_ALIAS_MAP + translate]
        Alias --> Resolver{ToolResolver}
        Resolver -->|1| L[local tools.py]
        Resolver -->|2| R[wrapper ToolRegistry]
        Resolver -->|3| C[praisonaiagents.tools]
        Resolver -->|4| P[praisonai-tools]
        Resolver -->|5| E[entry-point plugins]
        L --> Bridge{shell tool + compute?}
        R --> Bridge
        C --> Bridge
        P --> Bridge
        E --> Bridge
        Bridge -->|yes| Bridged[compute-bridged tool]
        Bridge -->|no| Direct[direct callable]
    end

    classDef input fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef process fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef resolve fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef output fill:#10B981,stroke:#7C90A0,color:#fff

    class A input
    class Alias,Bridge process
    class Resolver,L,R,C,P,E resolve
    class Bridged,Direct output
```
...
When `praisonai-code` is not installed, the backend falls back to the historical `praisonaiagents.tools` lookup, so existing configs keep working unchanged.
...
| `tools` | `List[str]` | Default tools | Enabled tool names. Resolved through the canonical `ToolResolver` chain (see [Tool Name Resolution](#tool-name-resolution)) |
...
<Card title="Docker Compute" icon="docker" href="/concepts/managed-agents-docker">
  Containerized execution environments
</Card>
<Card title="Tenki Cloud" icon="cloud" href="/features/managed-agents-tenki">
  Disposable Linux microVMs (`compute="tenki"`)
</Card>
<Card title="SessionInfo Schema" icon="circle-info" href="/features/managed-agents-session-info">
