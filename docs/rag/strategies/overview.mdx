...
<Warning>
If you skip the `[knowledge]` extra and select a semantic-family chunker (`semantic`, `sdpm`, or `late` — `semantic` is the default), the first `Knowledge.add(...)` or `agent.start(...)` call raises:

```
ImportError: chonkie package not found. Please install it using: pip install 'praisonaiagents[knowledge]'
```

Fix by installing the extra, or by choosing a chunker that does not need chonkie's embeddings (`sentence`, `paragraph`, `fixed`, `token`). Since PraisonAI PR [#5105](https://github.com/MervinPraison/PraisonAI/pull/5105).
</Warning>
