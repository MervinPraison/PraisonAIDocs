...
The zero-chunk outcome is the CLI surface of the SDK's `result["errors"]` behaviour. See [Knowledge Indexing Errors](/features/knowledge-indexing-errors) for the underlying failure modes (wrong embedding model name, revoked API key, exhausted quota).
