...
<Note>
Want Gemini, Cohere, or a local Ollama model instead of OpenAI? See [Knowledge → Use a non-OpenAI embedder](/features/knowledge#use-a-non-openai-embedder).
</Note>
...
agent = Agent(
    ...
    knowledge=KnowledgeConfig(
        ...
        # Embedder configuration ("openai" is the default and means "no override").
        # Use a different provider — see /features/knowledge#use-a-non-openai-embedder
        embedder="openai",
...
A failed vector-store call returns empty `chunks` with `retrieval_failed=True` and an `error` message on the `RetrievalResult` — it does not raise. Check the flag before trusting an empty result so a broken store never looks like a genuine no-match. See [Detect Retrieval Failures](/features/smart-retrieval#detect-retrieval-failures) for the full pattern.

## Next Steps

- [CLI Retrieval Commands](/cli/retrieval) - Use retrieval from the command line
- [Knowledge Concepts](/concepts/knowledge) - Learn about knowledge bases
- [Memory](/concepts/memory) - Combine with agent memory
