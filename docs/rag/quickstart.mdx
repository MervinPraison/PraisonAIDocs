...
<Tip>
**Verify it grounded.** A search hit does not prove the agent used it. Drop a unique marker into a doc, then assert it round-trips through `chat()`:

```python
chat_ok = "ZEBRA-QUOTA-9917" in agent.chat("What is policy ZEBRA-QUOTA-9917?")
```

If this is `False` while search finds the chunk, see [Knowledge Troubleshooting](/features/knowledge-troubleshooting).
</Tip>
...
Check the exit code — `rag index` now exits `1` if any source stored zero chunks, so a broken embedding backend or an empty glob no longer looks like a successful import. See [Knowledge Indexing Errors](/features/knowledge-indexing-errors) for the SDK-side failure modes.
...
- [RAG Module Reference](/rag/module) - Full API documentation
- [Citations](/rag/citations) - Working with source attribution
- [CLI Commands](/rag/cli) - Command-line reference
- [Evaluation](/js/evaluation) - Testing retrieval quality
