...
At the retriever layer, failures are non-raising by default: a broken vector store returns empty `chunks` instead of throwing. Check `RetrievalResult.retrieval_failed` and `.error` to tell a real failure from a genuine no-match — see [Detect Retrieval Failures](/features/smart-retrieval#detect-retrieval-failures).
