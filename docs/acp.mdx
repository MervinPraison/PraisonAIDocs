...
Writes enabled by `--allow-write` are validated by the [Safe Edit Pipeline](/features/acp-safe-edit).
...
serve(
    ...
    allow_write=True,
    allow_shell=False,
    allow_network=False,     # search / crawl tools — off by default
...
<Card title="Safe Edit Pipeline" icon="shield-check" href="/features/acp-safe-edit">
  How `--allow-write` requests are validated: workspace confinement, symlink guards, atomic apply, and per-workspace isolation.
</Card>
