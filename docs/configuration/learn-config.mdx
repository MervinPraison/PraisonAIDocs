...
<Note>
`Agent(learn=True)` is a coordinated shortcut: it enables `mode=AGENTIC` + `nudge_interval=10` + `nudge_min_tool_iters=0` + `auto_memory=True` all at once. See [Bot Session Learning](/features/bot-session-learning) for the default-on gateway/bot behaviour.
</Note>
...
<CardGroup cols={2}>
<Card title="Bounded Learning" icon="database" href="/features/learning-retention">
  User-flow guide: retention caps, archival, and the prune API
</Card>
<Card title="Agent Learn" icon="lightbulb" href="/concepts/agent-learn">
