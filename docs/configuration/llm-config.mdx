...
PraisonAI resolves these once at startup; all internal components use the same values. For complete environment variable configuration guide, see [LLM Endpoint Configuration](/docs/features/llm-endpoint-config).
...
# Simple: pass model name as string
agent = Agent(
    name="Assistant",
    llm="gpt-4o",
)

# Advanced: pass a config dict
agent = Agent(
    name="Assistant",
    llm={
        "model": "gpt-4o",
...
Catch LLM errors and retry with a different model:

```python
from praisonaiagents import Agent

FALLBACK_CHAIN = ["gpt-4o", "anthropic/claude-3-5-sonnet-20241022", "gpt-4o-mini"]
...
On transient errors (503, timeout, overloaded), the agent surfaces an `LLMError` — catch it and retry with the next model in your chain.
...
<Note>
For model fallback, catch `LLMError` and retry with a different model. See [Model Fallback](/features/model-fallback) for patterns.
</Note>
