...
PraisonAI resolves these once at startup; all internal components use the same values. For complete environment variable configuration guide, see [LLM Endpoint Configuration](/features/llm-endpoint-config).
...
| Default | Value | Overridable via |
|---------|-------|-----------------|
| `timeout` | `60.0` seconds | Per-call `timeout=` kwarg (wins) or env var `PRAISONAI_LLM_TIMEOUT` (float seconds) |
| `num_retries` | `2` | Per-call `num_retries=` kwarg |

```bash
# Tighten the default for the whole process
export PRAISONAI_LLM_TIMEOUT=10
```

```python
# Explicit per-call override always wins over the default
from praisonaiagents import Agent

agent = Agent(name="Assistant", llm={"model": "gpt-4o", "timeout": 30})
```
...
```mermaid
graph LR
    Call[LLM Call] --> Check{timeout set?}
    Check -->|Explicit kwarg| Use[Use caller value]
    Check -->|No kwarg| Env{PRAISONAI_LLM_TIMEOUT?}
    Env -->|Set| UseEnv[Use env value]
    Env -->|Unset| Default[Use 60s default]
    ...
    class Call call
    class Check,Env check
    class Use,UseEnv,Default ok
```
...
<Warning>
**Behaviour change (PraisonAI PR #4245).** Before this release, `temperature` set inside `Agent(llm={...})` was silently overridden by a hardcoded `1.0` at request time — configured values never reached the provider. It now reaches the request as written. If you set `temperature` via the `llm` dict, your output will change; if you did not set it, the provider default now applies (previously an explicit `1.0` was sent). Setting `temperature` per call still wins over the instance/config value.
</Warning>

**Temperature precedence:**

`per-call temperature=` kwarg > `Agent(llm={'temperature': ...})` / `Agent(temperature=...)` > provider default (temperature omitted from the request).

When `temperature` is not configured anywhere, the parameter is **omitted from the request** rather than sent as an explicit `None` or `1.0`. This matters for providers whose default differs from `1.0` — for example reasoning models that reject non-default temperatures (see the note in [JS output](/js/output)).

<Tip>
To confirm the value reached the request, enable LiteLLM debug logging or check your provider dashboard for the temperature recorded on the call.
</Tip>
...
See [Model Fallback](/features/model-fallback) for the full guide.
...
<Warning>
Model fallback is configured via `LLMConfig(fallback_models=[...])`, not keys inside the `llm` dict above. See [Model Fallback](/features/model-fallback).
</Warning>
...
# Default LLM call timeout (seconds) — applied to every LiteLLM call
export PRAISONAI_LLM_TIMEOUT="60"
```

`PRAISONAI_LLM_TIMEOUT` is the only `PRAISONAI_LLM_*` variable wired into the SDK today — see [Default LLM Timeouts](#default-llm-timeouts).
...
<Note>
For a per-call retry override today, pass `num_retries=` on the call — the default is `2`. For per-call header, timeout, or streaming control, set them inside the `llm={...}` dict on the `Agent` (see [Core LLM Configuration](#core-llm-configuration)).
</Note>
