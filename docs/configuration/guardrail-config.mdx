...
description: Complete reference for custom validation rules and guardrail settings
icon: shield
---
...
### Policy Strings (removed)

<Warning>
**Since PraisonAI PR #3790**, policy-string guardrails such as `guardrails=["policy:strict", "pii:redact"]` raise `ValueError` at `Agent(...)` construction time. For tool allow/deny enforcement, use `Agent(policy=PolicyEngine(...))` instead — see [Policy Engine](/features/policy-engine).
</Warning>
