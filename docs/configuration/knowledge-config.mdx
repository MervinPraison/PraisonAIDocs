...
# qdrant is a plugin adapter — register it before use
...
<Note>
Only `chroma` ships as a top-level Knowledge adapter. `qdrant`, `pinecone`, `weaviate`, and similar stores need `register_knowledge_adapter(name, adapter_cls)` first — see [Custom Knowledge Adapters](/features/knowledge-custom-adapters). For a copy-paste-runs example, use `"provider": "chroma"`.
</Note>
