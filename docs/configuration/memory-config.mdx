...
<Note>
Memory sizes its vector store from `get_dimensions(embedding_model)`. Before [PraisonAI PR #3027](https://github.com/MervinPraison/PraisonAI/pull/3027) (releases after 2026-07-14), `all-MiniLM-L6-v2` fell through to the `1536` default because the lookup was case-sensitive — a memory collection created with that model on an older release will be **1536-dim while the model emits 384-dim vectors**, causing "dimension mismatch" errors on upsert. Upgrade `praisonaiagents` and re-create the collection to fix it. `sentence-transformers/all-MiniLM-L6-v2` was affected the same way.
</Note>
