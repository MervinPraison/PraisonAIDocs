...
<Note>
In YAML the field name is still `tool_timeout:`; in Python pass tool execution settings through `tool_config=ToolConfig(timeout=…)`. The standalone `tool_timeout` / `tool_retry_policy` / `parallel_tool_calls` kwargs were removed and raise `TypeError`.
</Note>
...
## Consolidated ToolConfig

PraisonAI provides a unified `ToolConfig` dataclass that consolidates tool-related parameters into a single configuration object.
...
YAML and the CLI are unaffected — `tool_timeout:` in YAML and `--tool-timeout` on the CLI are still valid; the wrapper translates them into a `ToolConfig` before constructing the `Agent`.
