...
description: Complete reference for tool timeout settings and performance tuning
icon: wrench
---
...
<Note>
`ToolConfig.parallel` is a **deprecated alias** for `ExecutionConfig.parallel_tool_calls`, the single source of truth for running batched LLM tool calls in parallel. Prefer `execution=ExecutionConfig(parallel_tool_calls=True)`. Do not set both spellings to conflicting values — that raises `TypeError`.
</Note>
...
| `parallel` | `bool` | `False` | **Deprecated alias** for `ExecutionConfig.parallel_tool_calls`. Prefer `execution=ExecutionConfig(parallel_tool_calls=True)`. Conflicting values across both spellings raise `TypeError`. |
| `retry_policy` | `RetryPolicy` | `None` | Authoritative tool-retry budget. When set, overrides the `ExecutionConfig` retry spelling (`max_retry_limit` / `retry_*`). See [Tool Retry Policy](/features/tool-retry-policy). |
...
| `enable_artifacts` | `bool` | `False` | Master switch — enables artifact spillover for large tool outputs |
| `artifact_retention_days` | `int` | `7` | Days to keep artifacts before garbage collection on agent destruction |
| `artifact_store` | `Any` | `None` | Custom store instance implementing `ArtifactStoreProtocol` |
| `redact_secrets` | `bool` | `True` | Redact common secret patterns before writing artifacts to disk |
| `allow_global_tools` | `bool` | `False` | Allow the agent to resolve tools from the process-global registry. When enabled, a global tool's `idempotent` flag is also consulted for retry-on-timeout decisions. |

<Note>
**Timeout behaviour depends on tool idempotency.** A wall-clock `timeout` is not auto-retried unless the tool is idempotent — mutating or unknown tools surface the timeout once instead of duplicating side effects. See [Tool Timeouts → Retry-on-timeout is opt-in](/features/tool-timeout#retry-on-timeout-is-opt-in-for-mutating-tools).
</Note>
...
agent = Agent(tool_config=ToolConfig(enable_artifacts=True))
```

<Card title="Artifact Storage" icon="database" href="/features/artifact-storage">
  Full documentation: how overflow is stored, the six retrieval tools the agent gains, storage layout, and best practices.
</Card>

### Configuration Shortcuts

ToolConfig supports convenient shorthand notation:

```python
from praisonaiagents import Agent, ToolConfig

# Boolean shorthand
agent = Agent(tool_config=True)   # Uses default ToolConfig()
agent = Agent(tool_config=False)  # Disables tool config
...
<Warning>
Dict `{"parallel": true}` (and `{"parallel": false}`) is read as an **explicit** value for the parallel-tool-calls setting. Combining it with `execution=ExecutionConfig(parallel_tool_calls=...)` set to a conflicting value raises `TypeError`. Prefer the `ExecutionConfig` spelling.
</Warning>

### Removed legacy kwargs

The standalone `tool_timeout`, `parallel_tool_calls`, and `tool_retry_policy` keyword arguments on `Agent(...)` have been removed. Passing them raises `TypeError`. The error now includes an inline migration hint pointing at the `ToolConfig` field:

```text
TypeError: Agent.__init__() got an unexpected keyword argument 'tool_timeout'
  tool_timeout: tool timeout moved into tool_config=; use
    tool_config=ToolConfig(timeout=...).
```

Pass these settings through `tool_config=ToolConfig(...)` instead.

The `TypeError` now carries a "Did you mean?" hint naming the `tool_config=ToolConfig(...)` replacement, so callers upgrading from an older release see the fix inline rather than having to search the 41-parameter signature.

| Removed kwarg | New location |
|---|---|
| `tool_timeout=30` | `tool_config=ToolConfig(timeout=30)` |
| `parallel_tool_calls=True` | `execution=ExecutionConfig(parallel_tool_calls=True)` (canonical) or `tool_config=ToolConfig(parallel=True)` (deprecated alias) |
| `tool_retry_policy=RetryPolicy(...)` | `tool_config=ToolConfig(retry_policy=RetryPolicy(...))` |

Passing `tool_retry_policy` and/or `tool_timeout` now names the replacement inline in the `TypeError`, so the exact error string is searchable:

```text
Agent.__init__() got unexpected keyword argument(s): tool_retry_policy, tool_timeout
  tool_retry_policy: tool retry moved into tool_config=; use tool_config=ToolConfig(retry_policy=RetryPolicy(...)).
  tool_timeout: tool timeout moved into tool_config=; use tool_config=ToolConfig(timeout=...).
```
...
agent = Agent(
    name="Assistant", 
    tools=["web_search"],
    tool_config=ToolConfig(timeout=30)  # seconds
)
...
<Note>
**Reliability after a timeout.** A tool that hangs past `tool_timeout` no longer holds its worker slot — the pool is recycled and the next call starts on a fresh worker. The pool has **2** workers by default; before this fix, two consecutive hangs would deadlock the pool because `future.cancel()` cannot stop a thread that has already started. On timeout the executor is now `shutdown(wait=False)` and dropped, so the next tool call spins up a fresh executor. Recycling is bounded (at most 4 orphaned executors) so repeated timeouts cannot leak an unbounded number of stuck threads. See [PR #3960](https://github.com/MervinPraison/PraisonAI/pull/3960).
</Note>
...
<Note>
For complete concurrency control documentation including per-agent limits and sync/async patterns, see [Concurrency](/features/concurrency).
</Note>

### Wrapper-Level Tool Timeout (YAML / CLI — Defense-in-Depth)

<Warning>
There are **two** `ToolTimeoutError` classes in PraisonAI — they live in different modules, use different units, and are raised at different layers:

- **`praisonai.agents_generator.ToolTimeoutError`** — raised by the **wrapper** (YAML/CLI `tool_timeout: N` in seconds). Carries `tool_name`, `timeout_seconds`, `background_work_may_continue`. Covered in this section.
- **`praisonaiagents.tools.ToolTimeoutError`** — raised by the SDK **tool-call executor** (`llm={"tool_timeout_ms": N}` in milliseconds). Never actually thrown to user code — surfaced as a `ToolResult.error` with `error_kind="timeout"` and a structured envelope for the LLM. See [Tool Call Executor Timeout](/features/tool-call-executor-timeout).

Import the right one for the layer you're catching at.
</Warning>
  ...
  fetcher:
    ...
    tasks:
      fetch:
        ...
        expected_output: Summarised payload, or a translated timeout message if the upstream is slow.
```

When `fetch_url` blocks for more than 15 s, the wrapper raises `ToolTimeoutError` instead of hanging indefinitely; the framework adapter catches it and translates it into a value its framework understands, so the crew keeps moving.

**Precedence note:** Three timeout layers can apply, in different units:

| Layer | Configure with | Unit |
|---|---|---|
| Wrapper boundary | YAML `tool_timeout: N` / CLI `--tool-timeout N` | seconds |
| SDK Agent executor | `Agent(tool_config=ToolConfig(timeout=N))` | seconds |
| SDK tool-call executor | `Agent(llm={"tool_timeout_ms": N})` | **milliseconds** |

When more than one applies, the **shortest effective timeout wins** in practice — whichever fires first short-circuits the tool call. The three layers are independent, so mind the units: `tool_timeout_ms: 5000` equals a 5-second wrapper `tool_timeout: 5`. See [Tool Call Executor Timeout](/features/tool-call-executor-timeout) for the millisecond layer.

As of PR #3176, the wrapper's shared-tool-dict resolver picks the **smallest** declared `tool_timeout` across roles/agents (previously the largest). Combined with the executor's own short-circuit, the effective timeout is genuinely the tightest across all three layers.
...
| Source | Precedence |
|---|---|
| `--tool-timeout N` on the CLI | Wins outright |
| `tool_timeout: N` inside any `roles:` or `agents:` entry in YAML | `min(N)` (tightest) across all entries, used when the CLI does not specify one; every overridden agent is warned in the logs |
| Nothing declared | No wrapper-layer wrapping (SDK-level timeout still applies if `tool_config` is set in Python) |
...
# Tool Security & Filtering
export ALLOWED_TOOLS="search,send_message"      # Whitelist specific tools (prevents collisions)
export HERMES_ONLY_TOOLS="search,send_message"  # Deprecated alias for ALLOWED_TOOLS
```

<Note>
For comprehensive tool whitelisting and collision prevention, see the [Allowed Tools](/features/allowed-tools) feature documentation.
</Note>
...
3. **Circuit breakers for tools are automatic** - see [Tool Circuit Breaker](/features/tool-circuit-breaker)
