...
- **Type**: `float | None`
- **Default**: `None`
- **Description**: Hard USD cap on the agent run. Set via `execution=ExecutionConfig(max_budget=...)`. In `stop` mode (default), the guard fires **before** each LLM call — the over-budget call is never dispatched. In `warn` and callable modes, the check stays post-call (reactive).
...
agent = Agent(
    name="BudgetedAgent",
    instructions="Complete task within budget",
...
agent = Agent(
    ...
    execution=ExecutionConfig(
        ...
        # Enable tool bridge so model-generated code can call registered tools
        code_tools=True,
        code_tools_allow=["fetch", "parse"],  # Explicit per-run allow-list
    ),
