...
TypeScript also honours `maxContextTokens` — unlike Python, which declares the field but never reads it. See [TypeScript Handoffs](/docs/js/handoffs).
</Tab>
...
<Note>
A handoff's seeded history is **invocation-scoped** — it does not leak into subsequent chats or later handoffs that reuse the same target, and sequential handoffs do not accumulate context. See [Handoff Filters](/features/handoff-filters).
</Note>
...
See [Handoff Tool Policy](/features/handoff-tool-policy) for modes, patterns, and migration guidance.
