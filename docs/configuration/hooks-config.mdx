...
<Note>
**See also — don't confuse the three systems.** `HooksConfig` on this page is the live path, consumed by `Agent(hooks=...)`. It is not the same as `.praisonai/hooks.json` (the standalone [Memory HooksManager](/features/memory-hooks-manager) utility you call yourself). For the full live-hooks package see [Hooks](/features/hooks).
</Note>

<CardGroup cols={2}>
<Card title="Hooks System" icon="webhook" href="/features/hooks">
  Learn about the hooks system
</Card>
<Card title="Memory HooksManager" icon="webhook" href="/features/memory-hooks-manager">
