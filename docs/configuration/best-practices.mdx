...
The SDK resolves the real window for any litellm-supported model automatically — see [How the context window is resolved](/features/context-budgeter#how-the-context-window-is-resolved). The static map below is only an illustrative fallback.
