...
config = ExecutionConfig(
    ...
    # Rate limiting: positive int auto-builds a live RateLimiter
    max_rpm=None,
...
| `max_retry_limit` | `int` | `2` | Max retries for retryable tool failures and guardrail validation failures. Total attempts = `1 + max_retry_limit`. For tools, this feeds the **same effective `RetryPolicy`** as `ToolConfig(retry_policy=...)`; an explicit `retry_policy` takes precedence. Retries use exponential backoff with jitter (see `retry_initial_delay`, `retry_backoff_factor`, `retry_jitter`). |
...
| `compaction_strategy` | `CompactionStrategy \| None` | `None` | Which strategy the compactor uses when `context_compaction` is on. `None` defaults to `CompactionStrategy.TRUNCATE`. Set to `CompactionStrategy.LLM_SUMMARIZE` for summary-based compaction. Import: `from praisonaiagents import CompactionStrategy`. |
...
<Note>
For budget limits, use `execution=ExecutionConfig(max_budget=...)` on your Agent. See [Agent max_budget](/features/agent-max-budget) for details.

These retry settings apply to both tool execution and guardrail validation retries.
</Note>

<Warning>
Invalid values raise `ValueError`: `retry_initial_delay` must be `> 0`, `retry_backoff_factor` must be `>= 1.0`, `retry_jitter` must be `>= 0`. `max_rpm <= 0` also raises `ValueError` — validated before the `rate_limiter` precedence check, so `max_rpm=0` raises even when a valid `rate_limiter` is passed.
</Warning>
...
For tools, `max_retry_limit` / `retry_*` are translated into the same effective `RetryPolicy` as `ToolConfig(retry_policy=...)`. An explicit `retry_policy` (or per-tool `@tool(retry_policy=...)`) **wins over** these fields. Users who never set `retry_policy=` see no behaviour change. The guardrail-retry backoff continues to use these fields regardless.

See [Tool Retry & Backoff](/features/tool-retry-backoff) for retry classification and patterns.
...
Save an `ExecutionConfig` to a dict for YAML/JSON storage and restore it later — every field round-trips.

```python
from praisonaiagents import Agent, ExecutionConfig, CompactionStrategy
...
# Restore
restored = ExecutionConfig.from_dict(saved)

agent = Agent(name="Persisted Agent", instructions="...", execution=restored)
```

<Note>
`retry_initial_delay`, `retry_backoff_factor`, `retry_jitter`, `compaction_strategy`, and `code_sandbox_mode` all survive `to_dict()` → `from_dict()` as of [PraisonAI #3486](https://github.com/MervinPraison/PraisonAI/pull/3486). Unknown `compaction_strategy` values decode to `None` instead of raising, so a config written by a newer release can still be loaded by an older one. Round-trip preserves the value, not behaviour — `code_sandbox_mode` has no execution consumer today; see the [Execution](/features/execution) page for the actual isolation path.
</Note>
...
agent = Agent(
    ...
    execution=ExecutionConfig(
        max_rpm=60,  # auto-builds a live RateLimiter(requests_per_minute=60)
        max_retry_limit=3
    )
)
```
...
<Note>
`max_rpm=60` alone is enough — the Agent auto-creates a live `RateLimiter(requests_per_minute=60)`. Only pass a full `rate_limiter=` object when you need `burst` or `tokens_per_minute` control.
</Note>
...
Set `max_rpm` when calling external APIs to avoid rate limit errors. Setting `max_rpm` now enforces the limit — the Agent auto-builds a live `RateLimiter`, so no separate `rate_limiter` object is required for simple RPM throttling.
</Accordion>
...
<Card title="Async Execution" icon="clock" href="/features/async">
  Async agent execution
</Card>
<Card title="Background Tasks" icon="list-check" href="/features/background-tasks">
...
<Card title="Tool Retry & Backoff" icon="rotate" href="/features/tool-retry-backoff">
  Exponential backoff for tool and guardrail retries
</Card>
<Card title="Rate Limiter" icon="gauge" href="/features/rate-limiter">
  Auto-wire, precedence, and burst/TPM control for `max_rpm`
</Card>
</CardGroup>
