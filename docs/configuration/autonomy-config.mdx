...
<Note>
**`sandbox=` on AutonomyConfig is not exposed.** `AutonomyConfig` carries a `sandbox: Optional[SandboxConfig]` field for forward compatibility; **nothing currently reads it and no sandbox is auto-enabled at any autonomy level.** Pass `Agent(sandbox=…)` to enable the explicit `agent.execute_code()` API; to isolate a whole workflow in a real container, use `AgentFlow(run_on=…)`.
</Note>
