...
`run` connects to the local bridge server. If that server is not up, it prints an actionable message instead of a cryptic connection error:
...
Fix it by starting the server (see [Chrome Management → Start Server](#start-server)), then run again in a second terminal:
...
sequenceDiagram
    ...
    User->>TermA: praisonai browser start --port 8765
    TermA->>Bridge: listen
    Bridge-->>TermA: PraisonAI Browser Server (blocks)
    User->>TermB: praisonai browser run "…"
    TermB->>Bridge: WebSocket connect
    Bridge-->>TermB: session accepted
    TermB-->>User: Step 0, Step 1, Task completed
```
