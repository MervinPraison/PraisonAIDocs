...
| `run` | Run a recipe |
| `runs` | List recipe run history (feeds `recipe judge <run-id>`) |
...
Every run is recorded automatically. List past runs to find a Run ID for `recipe judge`.

```bash
praisonai recipe runs                                  # last 20 runs
praisonai recipe runs my-recipe --status failed        # filter
praisonai recipe runs --session session-abc123 --json  # JSON output
```

See [Run History](/cli/recipe-runs) for full options.
...
```bash
praisonai recipe judge trace.jsonl --goal "Analyze the image and create a blog post"
praisonai recipe judge run-abc123 -g "Explain the error and suggest a fix"
```

<Note>
**Fixed in PraisonAI PR #4946.** `-g/--goal` was declared (and shown in this command's own example) but dropped. It now threads through both `judge_trace(recipe_goal=...)` and the plan generator (`generate_plan_from_report(..., goal=...)`). **Precedence:** an explicit `--goal` overrides any `recipe_goal` extracted from the recipe YAML.
</Note>
...
- [Recipes](/cli/recipes) - Recipe details
- [Recipe Registry](/cli/recipe-registry) - Recipe registry
