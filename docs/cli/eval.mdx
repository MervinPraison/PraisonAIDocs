...
<Info>
**Install requirement:** the wrapper-delegating `praisonai eval` subcommands (`accuracy`, `performance`, `judge`) require `pip install praisonai` (the full wrapper). On a standalone `pip install praisonai-code` install they exit `1` with `eval requires the full wrapper. Install the full wrapper: pip install praisonai` — a single-line hint, no Rich traceback ([PR #2854](https://github.com/MervinPraison/PraisonAI/pull/2854)). The native `eval list-judges` and its `eval list` alias run standalone in-process against `praisonaiagents.eval`.
</Info>
...
def calculate(expression: str) -> str:
    ...
    # Use an AST allow-list for user math — see /features/custom-tool-safe-eval
    return str(float(expression))  # replace with safe_eval in production
...
<Note>
**Fixed in PraisonAI PR #4946.** Pass the agent as the **named** option `--agent <path>` (aliased `-a`) — a bare positional `agent.yaml` used to fail with `unrecognized arguments: agent.yaml`. `--iterations`/`-n` is now forwarded to the accuracy runner (it was previously declared but dropped, so every run used the wrapper's default).
</Note>
...
Use this after registering a custom judge (see [Custom Judges](/docs/features/llm-as-judge#custom-judges)) to confirm the registration took effect.
