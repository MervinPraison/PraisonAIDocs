...
<Tip>
**`run` is optional for one-shot prompts.** `praisonai "What is the capital of France?"` is equivalent to `praisonai run "What is the capital of France?"` — a bare positional that isn't a file or a `.yaml`/`.yml` path is routed to the same one-shot path as `praisonai run "..."`.
</Tip>
