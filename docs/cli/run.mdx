...
`--thinking` sets the reasoning budget applied across direct-prompt, actions-mode, and `--agent` paths.
...
```bash
praisonai run "Refactor utils.py" --thinking high
praisonai run "Build a CSV report" --agent data-analyst --thinking medium
```
