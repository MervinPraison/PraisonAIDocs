...
On a standalone `pip install praisonai-code` install, default `run "…"` and the human-readable text modes (`--output plain/verbose/silent`) now require the `praisonai` wrapper and surface an install hint. Structured modes (`--output actions|json|stream|stream-json`) run in-process and stay standalone-safe.
...
`run --output actions|json|stream|stream-json` all run in-process with no wrapper — the standalone-safe paths. See the [PraisonAI Code CLI standalone-limits table](/docs/features/praisonai-code-cli#standalone-limits) for the full command matrix.
...
In `run` actions mode, `--tools` and `--toolset` are now wired end-to-end (previously silently dropped).
