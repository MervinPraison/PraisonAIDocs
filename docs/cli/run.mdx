...
| `--attach` | | Tag this run with a session id so other terminals can observe its events via `praisonai attach <session-id>`. Direct-prompt runs only. | `None` |

<Warning>
`--attach` is only supported for **direct prompt runs**. Combining it with `--agent`, `--command`, `--profile`, `--profile-deep`, or a YAML file target exits with `--attach is only supported for direct prompt runs`.
</Warning>
...
### Run with a session id for live observation

Start a run in Terminal A, then observe it from Terminal B using [`praisonai attach`](/docs/features/attach):

```bash
# Terminal A
praisonai daemon start --background
praisonai run "investigate the failing test" --attach my-session
...
- [Attach](/docs/features/attach) - Stream live events from a running session in a second terminal
- [Daemon](/docs/cli/daemon) - Start and manage the warm local runtime required for `--attach`
