...
`TARGET` may be a YAML/agent file path or a direct prompt. When you type the verb explicitly (`praisonai run TARGET`), the router hands `TARGET` to the modern agent runner whenever it ends in `.yaml` / `.yml` **(even if the file does not exist yet — the runner is what emits the clear file-not-found error)**, or it is an existing file on disk. Only a `TARGET` that is neither of those **and** is not a reserved async-jobs verb (`submit / status / result / cancel / list / stream`) falls through to the jobs argparse parser and errors out. For free-text prompts, prefer the bare-prompt shape (`praisonai "TARGET"`), which reaches the modern engine unconditionally — see [CLI Dispatcher](/features/cli-dispatcher).
...
```bash
# Typo? Wrong directory? You still get a clear error from the runner.
praisonai run typo.yaml
# Error: [Errno 2] No such file or directory: 'typo.yaml'
```
...
<Note>
A misspelled or missing-on-disk `.yaml` target (e.g. wrong directory, typo like `agent.yaml` instead of `agents.yaml`) still routes to the modern agent runner and surfaces a clear file-not-found error — it does **not** fall through to the async-jobs `invalid choice` / missing-`job_id` failure that the [original onboarding report](https://github.com/MervinPraison/PraisonAI/issues/5095) described. This routing is locked by a regression test ([PR #5107](https://github.com/MervinPraison/PraisonAI/pull/5107)).
</Note>
...
| `--pick` / `--interactive-session` | | Interactively pick a recent session to resume before running (TTY only; ignored with `--session` / `--continue` or `--json`). Aborts with exit 1 if the user cancels or no sessions exist. See [Pick a session to resume](#pick-a-session-to-resume). | `false` |
...
| `--append-system-prompt` | | Append text (or `@file`) to the system prompt for this invocation only. Env fallback: `PRAISONAI_APPEND_SYSTEM_PROMPT`. Runs in-process (bypasses the warm runtime). See [Append System Prompt](/features/append-system-prompt). | |

<Note>
Setting `--append-system-prompt` keeps `run` in-process; the warm-runtime fast path is bypassed for this invocation so the suffix is always applied. See [Append System Prompt](/features/append-system-prompt).
</Note>
...
<Card title="Multimodal Agents (Python / YAML)" icon="images" href="/features/multimodal#one-shot-cli">
...
| `1` | `--pick` used, but the user cancelled or no sessions exist | Direct prompt / YAML |
...
See [Step Budget](/features/max-steps) for how `max_steps` truncation works, and [Agent Run Outcomes → CLI Mapping](/docs/features/agent-run-outcomes#cli-mapping-praisonai-run) for the SDK-to-CLI status mapping.
...
JSON / stream-JSON consumers do **not** receive the stderr warning — the `status` field carries the machine signal. A provider block wins over a generic empty-result failure, so an actionable reason is never masked as a plain `run_failed`. See [Provider block outcomes](/features/run-outcome#provider-block-outcomes) for the SDK-level classifier and precedence.
...
See [Step Budget](/features/max-steps) for the truncation contract, and [Agent Run Outcomes → CLI Mapping](/docs/features/agent-run-outcomes#cli-mapping-praisonai-run) for how the SDK-level statuses map to these exit signals.
...
No id handy? Use `--pick` (alias `--interactive-session`) to choose a recent session from a picker, then run a new prompt against it:

```bash
# Pick a recent session interactively, then run a new prompt against it
praisonai run --pick "now add tests"

# Alias
praisonai run --interactive-session "now add tests"
```

`--pick` opens the same picker as [`session resume`](/cli/session-resume#interactive-picker). It is a **no-op** when `--session` or `--continue` already names a session, and silently ignored under `--json` or a non-TTY so scripts and CI keep today's behaviour. On a TTY with no `--session` / `--continue`, cancelling or an empty store **aborts the run with exit code 1** rather than falling through to a fresh run.
...
"Supported env var" now means any of the 14 credential keys in the [Credential → default-model map](/cli/setup#credential--default-model-map), so `MISTRAL_API_KEY` (or any catalogue key) also silences the credential gate:
...
<Note>
**Project rules and local tools apply whether or not the daemon is running.** As of PR [#4943](https://github.com/MervinPraison/PraisonAI/pull/4943), the warm runtime honors `AGENTS.md` / `CLAUDE.md` subtree instructions and loads project-local `.praisonai/tools/*.py` on every run, so a daemon-forwarded `praisonai run` produces the same result as the cold in-process path. Previously the warm runtime dropped both implicit sources silently. See [Daemon → Project rules and local tools](/cli/daemon#project-rules-and-local-tools).
</Note>
