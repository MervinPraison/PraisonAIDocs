...
| `--restore` | | Restore workspace to a checkpoint id (or `last`/`latest`) and exit | |
| `--no-checkpoint` | | Disable the automatic pre-run checkpoint | `false` |

<Tip>
`praisonai run` saves an automatic checkpoint tagged `run:` before every run. Use `--restore last` to undo and `--no-checkpoint` to opt out. See [Checkpoint CLI](/docs/cli/checkpoint) for the full subcommand reference.
</Tip>
...
### Undo a run with checkpoint restore

```bash
# Restore workspace to whatever it looked like before the last run
praisonai run --restore last

# Or rewind to a specific checkpoint
praisonai run --restore abc12345

# Skip auto-checkpoint for a quick read-only task
praisonai run --no-checkpoint "Summarise the README"
```
