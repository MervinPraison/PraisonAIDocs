...
On a standalone `pip install praisonai-code` install, default `run "…"` and the human-readable text modes (`--output plain/verbose/silent`) route through the in-process `Agent` (PR #2853) — the same path used by the structured modes (`--output actions|json|stream|stream-json`). `chat` and `code` now run **resident** on the built-in split-pane TUI too (PR #3818), so `pip install praisonai-code` alone delivers `run`, `chat`, and `code`. Installing the `praisonai` wrapper is optional — it upgrades `chat` / `code` to a richer dispatch mode.
