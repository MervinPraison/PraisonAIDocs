...
graph LR
    ...
    class Start input
    class Check,Reason gate
    class Ok success
    class Trunc truncated
    class Fail failure
```
...
<Note>
Exit `2` covers two independent cases: **truncation** (a new terminal outcome introduced by PR [#4100](https://github.com/MervinPraison/PraisonAI/pull/4100)) and the pre-existing **reserved-command guard**. They share the code but are unrelated — a truncated run always emits `status: "truncated"` under `--output json`, while the guard fails fast before any run.
</Note>
...
```
Run hit the step/iteration limit; the answer above is a summary of partial progress, not a completed task. Re-run with a higher --max-iter/max_steps to finish.
```
...
case "$status" in
  success)   echo "done" ;;
  truncated) echo "hit the step limit; consider re-running with a higher --max-iter"; exit 2 ;;
...
case $? in
  0) echo "done" ;;
  1) echo "run failed" ;;
  2) echo "truncated — partial answer above" ;;
esac
```
