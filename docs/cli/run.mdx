...
```mermaid
graph TB
    A[praisonai ARGV] --> B{First token is<br/>Typer command<br/>or .yaml?}
    B -->|Yes| C[Typer command / legacy YAML]
    B -->|No| D{All flags<br/>run-supported?}
    D -->|No — legacy-only flag| E[Legacy argparse<br/>+ stderr notice]
    D -->|Yes / no flags| F[Modern Typer run]
    F --> G[Same as: praisonai run PROMPT flags]
...
<Note>
A bare prompt whose flags are all accepted by `run` (`--model`, `--continue`, `--session`, `--output`, `--stream`, …) reaches the modern engine; the value of a value-taking option stays attached to its flag. Only a positional ending in `.yaml`/`.yml` or a genuinely legacy-only flag (`--framework`, `--auto`, `--serve`, …) keeps the legacy dispatcher — and when a legacy-only flag forces that, a one-line stderr notice points at `praisonai run "…"`. Multi-token unquoted prompts are joined into a single `run` positional so `praisonai build a weather agent` reaches the model as one prompt.
</Note>
