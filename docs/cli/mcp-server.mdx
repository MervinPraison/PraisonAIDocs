...
<Note>
The MCP server host ships as the standalone [praisonai-mcp](/features/praisonai-mcp-package) package. Every `praisonai mcp serve …` example below also runs as `praisonai-mcp serve …` — both forms are equivalent for host commands. See [The Three MCP Layers](/features/mcp-three-layers) for how the host fits.
</Note>
...
- [praisonai doctor](/cli/doctor) - Umbrella doctor with `--json` and status-symbol handling
