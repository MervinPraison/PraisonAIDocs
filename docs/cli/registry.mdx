...
<Info>
**Install requirement:** `praisonai registry` requires `pip install praisonai` (the full wrapper). On a standalone `pip install praisonai-code` install these subcommands (`list`, `show`, `add`, `remove`) exit `1` with `registry requires the full wrapper. Install the full wrapper: pip install praisonai` — a single-line hint, no Rich traceback ([PR #2854](https://github.com/MervinPraison/PraisonAI/pull/2854)).
</Info>
...
<Warning>
`praisonai registry serve` is **deprecated** and will be removed in a future version. Use [`praisonai serve registry`](/deploy/recipe-registry-server) instead. The deprecated form prints a yellow warning to `stderr` on every invocation:
...
<CardGroup cols={2}>
  <Card title="Recipe Registry CLI" icon="box-archive" href="/cli/recipe-registry">
    Publish, pull, and list recipes from local or remote registries
  </Card>
  <Card title="Recipe Registry Server" icon="server" href="/deploy/recipe-registry-server">
    Deploy the canonical `praisonai serve registry` HTTP server
  </Card>
  <Card title="Recipe Registry API" icon="code" href="/deploy/api/recipe-registry-api">
    HTTP API endpoints exposed by the registry server
  </Card>
  <Card title="Recipe Registry (Python)" icon="box-archive" href="/features/recipe-registry">
