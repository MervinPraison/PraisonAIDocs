...
<Note>
The doctor does **not** include MCP tools — its counts reflect only the four resolver buckets (`builtin` / `local` / `external` / `registered`). For MCP tools, run [`praisonai tools list --source mcp`](/cli/tools#list-mcp-tools).
</Note>
...
See [`ToolResolver.list_available_sources()`](/docs/features/tool-resolver#configuration-options) and the [source label table](/cli/tools) for full details.
</Note>
...
<CardGroup cols={2}>
<Card title="Tools Doctor CLI" icon="terminal" href="/cli/tools-doctor-cli">
  Run diagnostics from the command line
</Card>
<Card title="Tool Resolver" icon="wrench" href="/features/tool-resolver#configuration-options">
  list_available_sources() and source buckets
</Card>
<Card title="Tools" icon="wrench" href="/cli/tools">
  Source label table and CLI reference
</Card>
<Card title="Strict Tools Mode" icon="shield-check" href="/cli/strict-tools">
