...
| `create` | Create a new single-file plugin scaffold in `.praisonai/plugins/` |
...
| Field | Type | Meaning |
|-------|------|---------|
| `requests_conversation` | `bool` or `null` | `true` if the plugin declares a conversation-content hook (`before_llm`, `after_llm`, `before_agent`, `after_agent`, `message_*`). `null` for an entry-point plugin not yet loaded — its hooks are unknown without executing it, which the runtime deliberately avoids. |
| `conversation_granted` | `bool` | `true` if the plugin is first-party (bundled) or has `allow_conversation: true` in its config block. |

### Audit ungranted conversation plugins

Find third-party plugins that want conversation content but haven't been granted it — their middleware silently no-ops until you add `allow_conversation: true`:

```bash
praisonai plugins list --json | jq '.[] | select(.requests_conversation == true and .conversation_granted == false)'
```

See [Plugins → Conversation-Content Capability Gate](/features/plugins#security-conversation-content-capability-gate) for how to grant access.
...
```bash
praisonai plugins create <name>
```

<Note>
`create` is the command `praisonai plugins discover`'s empty-state hint prints — so the docs and the CLI now agree.
</Note>
...
<Note>
A separate boundary controls whether a loaded plugin may read or rewrite prompts and responses — third-party plugins need `allow_conversation: true`. Audit ungranted plugins with the [`--json` filter above](#audit-ungranted-conversation-plugins), and see [Plugins → Conversation-Content Capability Gate](/features/plugins#security-conversation-content-capability-gate).
</Note>
