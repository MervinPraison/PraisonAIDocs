...
<Tip>
If the requested backend isn't installed, the CLI exits with code 2 and prints an install hint. Install optional backends with `pip install "praisonai[docker]"` (or `modal`, `ssh`, `e2b`, `daytona`, `novita`). See [Installing Optional Backends](/features/sandbox-backends#installing-optional-backends) for the full matrix.
</Tip>
