...
<Note>
For the `praisonai code` coding agent, a separate **OS-level** sandbox path confines shell commands with Seatbelt (macOS) or `bwrap` (Linux) — the one that actually runs on macOS today. It is driven by [`PRAISON_SHELL=sandboxed`](/features/praison-shell), distinct from `SandboxConfig.native()` (Linux-only `sandlock`). See [OS Sandbox](/features/os-sandbox).
</Note>
...
```bash
praisonai sandbox run --code "print('hello')" --type docker --image python:3.12-slim
```

```bash
praisonai sandbox run --code "print('hello')" --type novita
```

<Tip>
If the requested backend isn't installed, the CLI exits with code 2 and prints an install hint. Wrapper-delegated backends install with `pip install 'praisonai[docker|e2b|modal|daytona]'`; sandbox-only backends install with `pip install 'praisonai-sandbox[ssh|sandlock|novita]'`. See [Installing Optional Backends](/features/sandbox-backends#installing-optional-backends) for the full matrix.
</Tip>
...
| `--type`, `-t` | `subprocess` | Sandbox backend name: `subprocess`, `docker`, `e2b`, `modal`, `ssh`, `sandlock`, `daytona`, or `novita` |
| `--image` | `python:3.12-slim` | Docker image (docker backend only) |
...
ssh          unavailable    pip install 'praisonai-sandbox[ssh]'
modal        unavailable    pip install 'praisonai[modal]'
e2b          unavailable    pip install 'praisonai[e2b]'
daytona      unavailable    pip install 'praisonai[daytona]'
novita       unavailable    pip install 'praisonai-sandbox[novita]'
```

Unavailable backends show the exact install command. See [Sandbox Backends](/docs/features/sandbox-backends#installing-optional-backends) for the full matrix and [praisonai-sandbox Package](/features/praisonai-sandbox-package) for the slim standalone install.
...
<CardGroup cols={2}>
  ...
  <Card title="Sandbox Backends" icon="shield" href="/features/sandbox-backends">
    All eight built-in backends, install commands, and backend selection guide
  </Card>
