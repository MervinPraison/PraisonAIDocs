...
<Note>
As of PR #3818, the Rich / Textual interactive TUI ships inside `praisonai-code` — the wrapper is no longer required to get the interactive frontend. The identity-preserving shims at `praisonai.cli.interactive.{async_tui, core, config, events}` keep the old import paths working, so `from praisonai.cli.interactive import InteractiveCore` and the `from praisonai.cli.features import InteractiveRuntime, RuntimeConfig` examples below continue to resolve unchanged.

As of PR #4264, the primary Textual TUI (`praisonai chat` / `praisonai code`) streams intermediate LLM narrative live for tool-using turns — the model's reasoning between tool calls appears immediately instead of after the whole turn finishes. See [Interactive TUI → Multi-step (tool-using) turns](/docs/cli/interactive-tui#multi-step-tool-using-turns).
</Note>

```bash
# The wrapper still works and adds gateway/bot channels
...
The runtime boots on the **shared `_async_bridge` background loop** (the same loop already used by ~77 wrapper-side `run_sync` callers), not on a per-run scoped loop. This is what makes ACP/LSP agent-centric tools safe — every async primitive owned by the runtime stays bound to a loop that is **still running** when tools re-enter. Tool calls made through the ACP/LSP `create_agent_centric_tools` set use the **sibling** `praisonai_code.cli.features.agent_tools._run_sync` bridge (Tier-2, no wrapper import) — it does **not** run on the wrapper's background loop, but it does read the same `PRAISONAI_RUN_SYNC_TIMEOUT` env var so hung tool calls are bounded consistently (PR [#3361](https://github.com/MervinPraison/PraisonAI/pull/3361)).
...
The shared default bridge's `atexit` teardown hook is registered **lazily, on that bridge's first real use** — a bare `import praisonai` no longer installs a process-wide `atexit` hook, so Django/Airflow/Streamlit embedders are unaffected until they actually call `run_sync`.
...
<Info>
For more details on agent-centric tools and InteractiveRuntime integration, see [PraisonAI Agents Framework](/framework/praisonaiagents).
...
**PR #4477 (Aug 2026)** — injected ACP/LSP agent-centric tools now carry the per-run `tool_timeout` guard. Previously they bypassed the wrapper's timeout. A hung LSP server or blocked ACP call now raises `ToolTimeoutError` at the CLI/YAML deadline instead of hanging.
</Info>
...
<Card icon="arrows-left-right" href="/features/async-bridge">
...
<Note>
In interactive `praisonai code`, the `/continue` command **visually replays** the restored user / assistant turns (bounded to the last N, with a `… N earlier turns` note) through the live renderer instead of a one-line summary. See [Session Resume › Visual Replay on `/continue`](/persistence/session-resume#visual-replay-on-continue).
</Note>
...
In `prompt` mode, the Rich and Textual frontends render an approval dialog where "Always allow" persists the narrowest reusable pattern and "Always Allow All" is a separate, explicit blanket grant. See [Interactive Approval › Rich / Textual TUI](/docs/features/interactive-approval#rich-%2F-textual-tui-praisonai-code-praisonai-chat).

## Visual replay on `/continue`

Resuming with `/continue` redraws your prior conversation through the same renderer live turns use, so scrollback shows what was actually said — bounded to the last 20 turns.
...
Repeated `/continue` clears the display first, so turns never duplicate. Assistant messages that only ran tools show as `[tool call] <name>`, and `session resume` (non-interactive) renders the same bounded replay.

See [Session Visual Replay](/features/session-visual-replay) for the full behaviour table and fallback-renderer notes.

## Related

- [Session Visual Replay](/features/session-visual-replay) - What appears on screen when you resume
