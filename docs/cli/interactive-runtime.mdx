...
### Lifecycle in YAML/CLI runs

Runtime is started before agent creation, kept alive throughout `PraisonAIAgents.start()`, and only stopped in the `finally` block. This is what makes `create_agent_centric_tools(interactive_runtime)` (loaded via `praisonai chat`/YAML runs) usable from inside agent tool calls.
