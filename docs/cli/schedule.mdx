...
<Note>
See also: [Gateway Schedules](/features/gateway-schedules) — when running the gateway, a `schedules:` block in `gateway.yaml` loads at boot, so you don't need `praisonai schedule` to run recurring agent → channel deliveries.
</Note>
...
<Note>
The runtime supports two additional stop conditions — `max_runs` (bounded count) and `until` (bounded date) — but they are not yet exposed as CLI flags. Set them via the `schedule_add` agent tool or by constructing a `ScheduleJob` directly. See [Schedule Tools → schedule_add](/tools/schedule-tools#schedule_add).
</Note>
...
A bare clock time (`at 5pm`) is a one-shot at the next matching local time; any recurring form (`every …`, `daily …`, or a day-of-week name) becomes a cron schedule. See [Async Agent Scheduler → Natural language](/features/async-agent-scheduler#natural-language) for the full grammar.
...
See [Schedule Tools → Timezones](/tools/schedule-tools#timezones-for-at-and-cron) for the full precedence ladder.
...
Set the platform's home channel first — either by exporting `TELEGRAM_HOME_CHANNEL=<chat_id>` before you run this command, or by adding an explicit `--deliver telegram:<chat_id>` argument. See [Scheduler Delivery](/features/scheduler-delivery) for the full precedence ladder.
...
The CLI maps `--poll` to [`ScheduleLoop.run_forever(poll_seconds=…)`](/features/scheduler-providers), the foreground counterpart to `ScheduleLoop.start()`.
...
`--monitor-url` probes the URL on every tick, hashes the response, and only runs the agent when the hash changed. An unchanged source records `no_change` — zero tokens, no delivery. See [Scheduler Monitor](/features/scheduler-monitor) for the full feature page and reliability guarantees.
...
- [Scheduler Monitor](/features/scheduler-monitor) - Wake the agent only when a URL or command output changes (`--monitor-url` / `--monitor-command`), with the full URL-monitor reliability guarantees
