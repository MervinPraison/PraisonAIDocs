...
| `--continuable / --no-continuable` | | Whether a reply to the delivered brief resumes the job's session. Default `--continuable`. See [Continuable Delivery](/docs/features/scheduler-delivery#continuable-delivery). |
