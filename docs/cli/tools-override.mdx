...
`create_tool_registry_with_overrides()` used to re-implement `praisonai-tools` package discovery and a `praisonaiagents` built-ins hook of its own. As of **PraisonAI#3122** those are handled only by the canonical [`ToolResolver`](/docs/features/tool-resolver), so:
