...
| `praisonai validate schema` | Print the YAML config schema (fields, types, required markers), or emit the machine-readable JSON Schema with `--agents` / `--output` |
...
Prints all main config sections with field names, types, and required markers. Add `--agents` (or `--output`) to emit the machine-readable JSON Schema for `agents.yaml` instead of the human-readable summary.
...
| `--output` / `-o` | `str` | `None` | Write the JSON Schema to a file instead of stdout (implies `--agents`; useful for offline / pinned editor setups) |
