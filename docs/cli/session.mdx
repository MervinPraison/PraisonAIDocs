...
On a TTY, `praisonai session resume` (no id) and `praisonai run --pick` open an [interactive picker](/cli/session-resume#interactive-picker) of recent sessions instead of requiring you to paste an id.
...
<Note>
Unnamed sessions get an auto-title after their first exchange, so the `Name` column is no longer just ids for fresh runs. See [Session Auto-Title](/features/session-auto-title).
</Note>
...
Most sessions are already auto-titled after their first exchange (see [Session Auto-Title](/features/session-auto-title)). Use `session rename` to override that name or to set one before the first turn lands.
...
1. The explicit stored `metadata["title"]` — set manually via `session rename` / `--title` or auto-generated after the first exchange
2. The agent name
3. A message snippet
4. The session id
...
`session search` runs a ranked full-text search over every stored transcript — the same FTS5/bm25 engine as the [`session_search` agent tool](/features/cross-session-recall), reachable from the terminal.
...
It iterates the same canonical stores as `resume` / `show` / `delete` / `export`, dedupes hits by `session_id` across stores, and prints a table ranked by score (or JSON with `--json`). See [Session Search](/cli/session-search) for the full page.
...
Defaults to the most recent session when no id is given. See [Session Handoff](/features/session-handoff) for the full feature, options, and Python API.
...
<CardGroup cols={2}>
  ...
  <Card title="Gateway Session Portability" icon="download" href="/features/gateway-session-portability">
