...
See [Cost Tracking](/docs/cli/cost-tracking) for how per-session totals accumulate across runs.
...
<Note>
**Identity invariant (praisonai #3133):** any id shown by `praisonai session list` — or resumable via `praisonai run --continue` — can be inspected with `session show`, exported with `session export`, and deleted with `session delete` using the **same id**. There is no hidden second store: `show` / `delete` / `export` now route through the same `DefaultSessionStore` (project-scoped + global) that `list` / `resume` use, so `id → session` is unambiguous.
</Note>
