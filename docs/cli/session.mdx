...
`session rename` gives a session a human-readable title so `session list` reads like a menu of tasks. The title is display-only metadata — the session id never changes. See [Session Rename](/features/session-rename) for the full feature page.
