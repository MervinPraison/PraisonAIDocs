...
Pass `--json` for machine-readable output with the full `ResolvedSession` shape (`session_id`, `agent_name`, `model`, `chat_history`, `metadata`, `message_count`, `created_at`, `updated_at`, `found`).
