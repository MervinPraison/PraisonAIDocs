...
<Note>
Tools registered via the wrapper `ToolRegistry`, shipped by `praisonai-tools`, or provided by core SDK plugins are now reachable from recipe `tools:` lists (PR #2059). Previously these worked only in the main agent build path. See [Tool Resolver](/features/tool-resolver).
</Note>
