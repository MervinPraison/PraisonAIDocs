...
This matches the concurrency posture of `recipe.arun_background()` (default `max_concurrent=5`) and of multiple `praisonai run` invocations sharing `~/.praison/runs/`.
...
# Same instance, same in-process lock — safe under threads on any platform.
h1 = get_history("/opt/agents/runs")
h2 = get_history("/opt/agents/runs")
assert h1 is h2
```

## Next Steps

- [Recipe Registry](/cli/recipe-registry) - Publish and pull recipes
