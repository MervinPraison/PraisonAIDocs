...
<Warning>
Binding to a non-localhost host (`--host 0.0.0.0`) **requires** an API key. Without one, `praisonai serve agents` exits at startup with a `SystemExit` because `POST /agents` can execute YAML-defined tools. Set `PRAISONAI_SERVE_API_KEY` or pass `--api-key`. See [Serve Agents Authentication](/docs/features/serve-agents-auth).
</Warning>
...
See [Serve Agents Authentication](/docs/features/serve-agents-auth) for the full localhost allowlist and error reference.
...
- [Host Integration](/docs/features/host-integration) - UI-Gateway configuration options
- [Integration Patterns](/docs/features/integration-patterns) - Pattern C and deployment patterns
