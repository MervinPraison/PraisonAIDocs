...
<Note>
You do not need `praisonai setup` at all if a local Ollama endpoint is reachable — PraisonAI adopts it automatically. See [Keyless Local-First Run](/features/keyless-local-first-run).
</Note>
...
graph TB
    ...
    A -. or skip setup entirely .-> L[🖥️ Local endpoint reachable?]
    L -->|Yes| G
...
Providers with multiple env-var spellings round-trip back under the same alias you set:

| Provider | Alias env vars (honoured in order set) | Fallback when neither set |
|----------|----------------------------------------|---------------------------|
| `together` | `TOGETHER_API_KEY` / `TOGETHERAI_API_KEY` | `TOGETHER_API_KEY` |
| `fireworks` | `FIREWORKS_API_KEY` / `FIREWORKS_AI_API_KEY` | `FIREWORKS_API_KEY` |
| `google` / `gemini` | `GEMINI_API_KEY` / `GOOGLE_API_KEY` | `GEMINI_API_KEY` |

Setting `TOGETHERAI_API_KEY` (or `FIREWORKS_AI_API_KEY`) writes back to that same spelling — the run no longer ends without a resolved key. Existing `google` / `gemini` alias behaviour is unchanged.
...
If verification fails, your config is already saved — run [`praisonai doctor`](/cli/doctor) to diagnose the key or model.
...
<Note>
**CI / zero-disk override.** `PRAISONAI_AUTH_CONTENT` takes precedence over all three files `setup` writes. When it's set, credentials load from that JSON blob in memory and nothing is read from or written to disk — ideal for containers. See [Zero-disk credentials](/cli/auth#zero-disk-credentials-ci-ephemeral).
</Note>

`setup` writes to the same canonical home that every other CLI subsystem — sessions, traces, logs, cache, `config set` — reads. `PRAISONAI_HOME` re-roots all of them together (see [CLI Home Root](/docs/features/cli-configuration#cli-home-root)).
...
| 2 | Most-recently-used model (`~/.praisonai/state/model.json`) | Only *user-chosen* values are remembered |
...
This map is now derived from `PROVIDER_ENV_CATALOGUE` in `praisonai_code/llm/catalogue.py` — one source of truth for first-run auto-detection and default-model inference. `_PROVIDER_DEFAULTS` in `llm/env.py` derives from the same catalogue (with a literal fallback), so run-time inference and setup detection stay in sync. Adding a provider is now a data row, not spread-out code.
...
| `TOGETHER_API_KEY` or `TOGETHERAI_API_KEY` | `together_ai/meta-llama/Llama-3.3-70B-Instruct-Turbo` |
| `PERPLEXITYAI_API_KEY` | `perplexity/sonar` |
| `FIREWORKS_API_KEY` or `FIREWORKS_AI_API_KEY` | `fireworks_ai/accounts/fireworks/models/llama-v3p3-70b-instruct` |
| Local endpoint reachable at `OPENAI_BASE_URL` / `OLLAMA_HOST` / `127.0.0.1:11434` (no cloud key) | `ollama/<detected name>` (or `openai/<id>` for non-Ollama local servers) |

The historical eight stay at the head of the catalogue, so ordered-preference behaviour is unchanged — `OPENAI_API_KEY` still wins when both it and `MISTRAL_API_KEY` are present. The additions are purely additive and backward compatible. When a provider declares multiple env-var spellings (Together, Fireworks), whichever you actually set is honoured for both detection and the credential written back.

The last row is the **detection** case — when no cloud key is set, PraisonAI probes for a running local endpoint and adopts what `detect_local_model()` returns. See [Keyless Local-First Run](/features/keyless-local-first-run).
...
If you skip setup at install time (or run `--no-prompt`), the first `praisonai`, `praisonai run`, `praisonai code`, or `praisonai chat` command detects the missing credentials and offers to launch this wizard for you. See [First-run Onboarding](/features/first-run-onboarding) for the full flow.

<Note>
`setup` is now a **suggestion**, not a hard requirement, when a local endpoint (e.g. Ollama) is running — PraisonAI auto-detects the local model and continues. See [Keyless local-first](/docs/features/first-run-onboarding#keyless-local-first-ollama-openai-compatible).
</Note>
...
With just one credential env var set (e.g. `ANTHROPIC_API_KEY`), running `praisonai chat` without `--model` now picks the matching default automatically. You only need `--model` to override or to mix providers in one session. On first run, the same resolved model is validated by the [onboarding gate](/features/first-run-onboarding#shared-onboarding-helper).
</Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Quick Start (--init)" icon="bolt" href="/quickstart">
  ...
  <Card title="Installer Internals" icon="gear" href="/install/installer">
