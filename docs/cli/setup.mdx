...
Shown only when no `*_API_KEY` was detected or you declined the detected one. The menu is now **catalogue-driven** — every provider `ModelCatalogue` knows about is offered (curated providers first, `custom` last):
...
<Note>
Non-interactive `setup` accepts **any** catalogue-known provider beyond the curated rows above — e.g. `--provider groq` or `--provider deepseek`. For a non-curated provider the wizard derives its env var as `<PROVIDER>_API_KEY` automatically (uppercased provider id + `_API_KEY`).
</Note>
