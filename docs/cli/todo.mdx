...
<Info>
**Install requirement:** `praisonai todo` requires `pip install praisonai` (the full wrapper). On a standalone `pip install praisonai-code` install every `todo` subcommand exits `1` with `todo requires the full wrapper. Install the full wrapper: pip install praisonai` — a single-line hint, no Rich traceback ([PR #2854](https://github.com/MervinPraison/PraisonAI/pull/2854)).
</Info>
...
# Set a priority (as of PraisonAI PR #4946, -p/--priority is honoured)
praisonai todo add "Ship the release" --priority high
praisonai todo add "Tidy up docs" -p low
```
...
|------|-------|--------|---------|-------------|
| `--priority` | `-p` | `low`, `medium`, `high` | `medium` | Priority for the new todo. `high` → 🔴, `medium` → 🟡, `low` → 🟢. Unknown values are rejected. |
...
<Note>
The todo workspace (`~/.praison/`) is a **home-level** location and was **not** changed by PraisonAI PR #4946 — it does not trigger the project-marker path drift. However, **project-level** CLI state — thinking, compaction, output-style, file-history, the queue DB, and knowledge vector stores — now lives under `./.praisonai/` (canonical). See [Project data directory](/features/cli-configuration#project-data-directory-praisonai).
</Note>
...
See [MCP Todo Tools](/features/mcp-todo-tools) for the tool table, record shape, and store resolution.

## Related

- [MCP Todo Tools](/features/mcp-todo-tools)
