...
<Tip>
For a **multi-agent team** project (agents.yaml + tasks.yaml + a Python entry point), use [`praisonai init team <name>`](/cli/init-team) instead. `praisonai init` (this page) scaffolds the single-agent `.praisonai/` convention.
</Tip>
...
<Tip>
For a single agent without commands or tools, use [`praisonai agent create`](/docs/cli/agent#create) instead. For a multi-agent team, use [`praisonai init team`](/cli/init-team).
</Tip>
...
The header is a plain YAML comment, so runtime parsing is unaffected. See [Editor Support](/features/editor-support) for full setup across VS Code, Neovim, and JetBrains IDEs.
...
`praisonai init --generate` runs a short analysis agent that reads a cheap snapshot of your repo and writes a tailored [`AGENTS.md`](/features/project-agents-md) at the repo root — so your next `praisonai run` starts with real project context instead of boilerplate.
...
<Warning>
`--global` changes only where the **static scaffold** lands (`~/.praisonai/`). The generated `AGENTS.md` always writes to the repo root, because the [rules loader](/cli/rules) discovers it there on the next run.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Init Team" icon="users-gear" href="/cli/init-team">
    Scaffold a multi-agent AgentTeam project (agents.yaml + tasks.yaml)
  </Card>
  <Card title="Custom Agents, Commands & Tools" icon="file-code" href="/features/custom-agents-commands">
  ...
  <Card title="Project AGENTS.md" icon="file-lines" href="/features/project-agents-md">
    How the generated AGENTS.md is auto-loaded as project context
  </Card>
  <Card title="Rules CLI" icon="scroll" href="/cli/rules">
