...
agestatt <na:e> "ask"` | Start scheduler as daemn | `paisonai schedule start m-bot "Tsk"` |
| `istLi all schedulers | `pasoi schedule lis`|
 `logs<nme> [-f]` | View log |`aisonai schedule lgs y-bot -f` |
| `sto <name>` | Sopschedule |`raisoni scedulesop my-bt`|
| `rstart <ame>` | Resart cheduler | `praisonai schedule restart m-bot` |
| `delete <ne>` | Remove from ist`praisonai schedule delete mybot`
|dsribe<am>` | Sho detail | praisonai scheduledescibemy-bot |

### Options

| Option | Type | Dscriptio | Defaul | Expe|
|--------|------|-------------|---------|---------
  - name: "AI News Monitor"
    role: "Technology News Analyst"
...
task: s"Search for latest AI news and provide top 3 stories"
