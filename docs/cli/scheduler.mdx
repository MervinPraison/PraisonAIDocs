...
See [Scheduler Delivery → Delivery outcomes](/docs/features/scheduler-delivery#delivery-outcomes) for the full flow, stats fields, and the `MESSAGE_UNDELIVERED` hook.
...
See [Out-of-process delivery](/features/scheduler-delivery#out-of-process-delivery) for the env-var table, chat-id resolution order, and per-platform failure modes.
...
See [Scheduler Delivery → Delivery outcomes](/features/scheduler-delivery#delivery-outcomes) for the full flow, stats, and `MESSAGE_UNDELIVERED` hook.
...
- **Time-of-day accuracy** — `--interval "cron:0 9 * * *"` fires at 09:00 UTC every day, not at whatever o'clock the daemon started.
...
Requires `pip install croniter`. Without it, a `cron:` schedule falls back to a coarse interval anchored to process start and the daemon logs a one-time warning.
...
If a scheduled run misses its slot after a crash, grep the logs for those two messages first. The async scheduler shares the same behaviour — see [Async Scheduler → Daemon state persistence](/features/async-scheduler#daemon-state-persistence).
...
| `cron:0 9 * * *` | wall-clock | Fires at real time-of-day. Requires `pip install croniter`. |
...
See [Async Agent Scheduler → Natural language](/features/async-agent-scheduler#natural-language) for the full grammar.
...
**Budget & timeout (PR #1771):** `AsyncAgentScheduler` now accepts `timeout` (per-run seconds) and `max_cost` (total USD cap) in its constructor — see [Async Agent Scheduler](/features/async-agent-scheduler) for the full pattern.
...
`SUPPRESSED` and `NOT_CONFIGURED` runs count as `successful_executions` but do **not** count as either `delivered_deliveries` or `undelivered_deliveries` — each counter is bumped only by its own outcome. See the [per-outcome counter table](/features/scheduler-delivery#delivery-outcomes) in Scheduler Delivery.
...
- [Scheduled Run Policy](/docs/features/scheduled-run-policy) - Tool scoping, prompt scanning, and output auditing for unattended runs (configure via the `scheduler:` block in `gateway.yaml`, or via `RunPolicy` in Python for non-gateway embeddings)
