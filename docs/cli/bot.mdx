...
If you installed only `praisonai-code`, this command will not appear in `--help` and will fail at dispatch time. See [Package Structure](/docs/installation#package-structure) for the six-package install model.
</Warning>
...
Stop the other instance first, or use a different token for each environment (dev / staging / prod). Webhook mode is unaffected — the single-instance rule applies to long polling (the default) only. See [409 Conflict troubleshooting](#409-conflict-bot-stops-replying).
</Warning>
...
This `playwright install chromium` step is required — no browsers are downloaded automatically by `pip install` in any tier.

Without those, `--browser` falls back to the cloud `BrowserBaseTool` (which needs BrowserBase credentials), or — if neither is installed — becomes a no-op with a startup warning. See [Bot Browser Automation](/features/bot-browser-automation) for details.
</Note>
...
```mermaid
graph LR
    subgraph "Single-instance rule"
        Token[🔑 Bot Token] --> Poller{One active poller?}
        Poller -->|Yes| OK[✅ Bot replies]
        Poller -->|No — 2+| Fail[❌ 409 Conflict / RuntimeError]
    end

    classDef input fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef check fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef good fill:#189AB4,stroke:#7C90A0,color:#fff
...
<Tabs>
  <Tab title="macOS / Linux">
    ```bash
    # 1. Find every Python process that mentions praisonai
    ps aux | grep -i praisonai | grep -v grep

    # 2. Kill duplicates (replace <PID>)
    kill <PID>

    # 3. Confirm webhook is not set (long-polling mode)
    curl "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/getWebhookInfo"

    # 4. Restart exactly one bot
    praisonai bot telegram --token $TELEGRAM_BOT_TOKEN
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ...
    Get-Process python | Format-Table Id, CPU, StartTime -AutoSize

    # 2. Stop all Python processes (careful — kills every python.exe)
    Get-Process python -ErrorAction SilentlyContinue | Stop-Process -Force
    Start-Sleep -Seconds 3

    # 3. Restart exactly one bot
    $env:TELEGRAM_BOT_TOKEN = "..."
    praisonai bot telegram --token $env:TELEGRAM_BOT_TOKEN
    ```
  </Tab>
</Tabs>
...
<CardGroup cols={2}>
  ...
  <Card title="Bot Browser Automation" icon="globe" href="/features/bot-browser-automation">
