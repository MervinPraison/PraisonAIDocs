...
<Note>
As of [PR #2642](https://github.com/MervinPraison/PraisonAI/pull/2642), `praisonai tools discover` surfaces the `registered` bucket in addition to installed packages — including tools contributed by third-party entry points. A tool that resolves at run time is guaranteed to appear here. Entry-point plugins under the canonical `praisonai.tools` group are what `tools discover` reports here; the legacy `praisonaiagents.tools` and `praisonai.tool_sources` groups still surface but emit a `DeprecationWarning` on first discovery.
</Note>

<Note>
MCP tools are **not** part of the resolver chain. `ToolResolver.list_available_sources()` covers only the four static buckets (`local` / `builtin` / `external` / `registered`). The fifth `mcp` source is a live network source surfaced **only** by `praisonai tools list`, which layers it on top at listing time — see [`praisonai tools list --source mcp`](/cli/tools#list-mcp-tools).
</Note>
...
| `registered` | Tools added via `ToolRegistry.register_function()` or an entry-point plugin under the canonical `praisonai.tools` group (or its deprecated `praisonaiagents.tools` / `praisonai.tool_sources` aliases) |
| `mcp` | Tools from MCP servers configured under `mcp.servers.*`. **CLI listing only** — not returned by `list_available_sources()`; surfaced live by `praisonai tools list` |
...
# MCP tools from configured mcp.servers.* (CLI-only, live)
...
```mermaid
graph LR
    subgraph Resolver["ToolResolver.list_available_sources()"]
    ...
    subgraph CLI["praisonai tools list (CLI only)"]
        M["mcp\nmcp.servers.* (live)"] --> LIST["Merged listing"]
    end
...
<Card title="Tools Doctor" icon="stethoscope" href="/cli/tools-doctor">
