...
<Note>
As of [PR #2642](https://github.com/MervinPraison/PraisonAI/pull/2642), `praisonai tools discover` surfaces the `registered` bucket in addition to installed packages — including tools contributed by third-party `praisonai.tool_sources` entry points. A tool that resolves at run time is guaranteed to appear here.
</Note>
