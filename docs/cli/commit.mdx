...
<Info>
**Install requirement:** `praisonai commit` requires `pip install praisonai` (the full wrapper). On a standalone `pip install praisonai-code` install every `commit` subcommand exits `1` with `commit requires the full wrapper. Install the full wrapper: pip install praisonai` — a single-line hint, no Rich traceback ([PR #2854](https://github.com/MervinPraison/PraisonAI/pull/2854)).
</Info>
...
```bash
# 1. CLI flag — recommended, overrides everything else
praisonai commit --llm claude-3-5-sonnet-latest

# 2. MODEL_NAME env var — inherits your usual PraisonAI model
MODEL_NAME=claude-3-5-sonnet-latest praisonai commit

# 3. OPENAI_MODEL_NAME env var — still works for OpenAI users
OPENAI_MODEL_NAME=gpt-4o praisonai commit

# 4. Fallback — gpt-4o-mini when nothing is set
praisonai commit
```
