...
If you are migrating from an earlier version and previously saw tools reported as missing in strict mode, verify those tools are registered via `ToolRegistry` or an entry-point plugin under the canonical `praisonai.tools` group. The legacy `praisonaiagents.tools` and `praisonai.tool_sources` groups still resolve tools registered under them (with a one-cycle `DeprecationWarning` on first discovery), and canonical-group tools take precedence on a name collision — they will now appear as `available` with `source: "registered"`.
</Note>
