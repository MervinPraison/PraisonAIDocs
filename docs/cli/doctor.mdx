...
| `packaging` | Check packaging and entry point configuration (Windows daemon compatibility) |
...
```bash
# Check packaging & entry points (helps diagnose Windows daemon issues)
...
### Packaging (`packaging`)
- Package structure (`__main__.py` presence)
- `python -m praisonai` execution test
- `praisonai` console script execution test
- Windows daemon context (venv, `Scripts/praisonai.exe`, Microsoft Store Python, `PYTHONIOENCODING`)
- Pip vs editable install detection
