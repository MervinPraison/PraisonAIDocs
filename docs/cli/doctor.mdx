...
<Note>
See [Optional Dependencies Check](/features/doctor-optional-deps) for the four-bucket table, per-package timeout formula, and CI gating examples.
</Note>
...
Validate multi-agent team YAML for runtime compatibility before `AgentTeam.start()`. No LLM calls or API keys required. See [Runtime Preflight](/features/runtime-preflight) for the capability matrix and check IDs.
...
Detect and migrate legacy `cli_backend` fields to the new `models.default.runtime` format. See [Runtime Config Migration](/features/doctor-runtime-migration) for full details.
...
See also: [Runtime Config Migration](/features/doctor-runtime-migration) — describes the `cli_backend`→`runtime` mapping that `fix` currently automates.
...
<Note>
On a standalone `pip install praisonai-code` install (no `praisonai` wrapper), several check families skip with the reason **"Install full wrapper: pip install praisonai"** instead of failing — so `praisonai-code doctor` returns **exit 0** when core checks pass ([PR #2851](https://github.com/MervinPraison/PraisonAI/pull/2851), fixes [#2838](https://github.com/MervinPraison/PraisonAI/issues/2838)).

Skipped IDs on standalone:

- `performance_praisonai_import` (Performance)
- `acp_module` (Tools / ACP)
- `praisonai_package_structure` · `console_script_execution` (Packaging)
- Existing family-level skips: `runtime`, `bots`, `gateway`, `serve`, `acp`, `config_wrapper`

The `python_module_execution` check does not skip — it retargets to `python -m praisonai_code --version` on standalone (and stays on `python -m praisonai --version` when the wrapper is installed).

Install `praisonai` to run every check, or hide the skipped families with `--skip runtime,bots,gateway,serve,acp,config_wrapper,performance_praisonai_import,acp_module,praisonai_package_structure,console_script_execution`.
</Note>

```mermaid
graph LR
    subgraph "praisonai-code doctor exit code"
        Start[📋 doctor invoked] --> Wrapper{🔍 praisonai wrapper installed?}
        Wrapper -->|Yes| RunAll[🧠 Run every check]
        Wrapper -->|No| SkipFour[⏭ SKIP 4 wrapper checks + retarget python -m]
        RunAll --> Score
        SkipFour --> Score{Any core FAIL?}
        Score -->|No| Zero[✅ exit 0]
        Score -->|Yes| One[⛔ exit 1]
    end
    ...
    class Start entry
    class Wrapper,Score gate
    class RunAll,SkipFour proc
    class Zero ok
    class One bad
```
...
- Project config directory — canonical `.praisonai/` first, with a legacy `.praison/` read-only fallback
- .env file detection

<Note>
As of PraisonAI PR #4946, project **data** checks (skills, memory) scan the canonical `.praisonai/` directory first and only fall back to a legacy `.praison/` directory for reads. The `config` and `permissions` checks still describe the `config.toml`-style `.praison` directory as a documented **read-only legacy fallback**, not a primary write target. The `mcp` check was already canonical-first and is unchanged.
</Note>
...
- `~/.praison` directory (legacy home fallback — intentionally **not** migrated in PR #4946)
- Project `.praison` directory (legacy fallback — intentionally **not** migrated in PR #4946)
- Temp directory
...
### Packaging (`packaging`)
- `praisonai_package_structure` — validates the `praisonai` wrapper's `__main__.py` entry point
- `console_script_execution` — runs `praisonai --version`
- `python_module_execution` — runs `python -m {praisonai|praisonai_code} --version` (retargets on standalone)
...
- Multi-channel token configuration validation (`multi_channel_tokens`) † — **MEDIUM**
...
- Gateway shell readiness (`gateway_shell_readiness`) † — **HIGH**
- Gateway channel probe (`gateway_channel_probe`) † — **HIGH**, deep-only (`--deep`)
- Gateway duplicate services (`gateway_duplicate_services`) † — **HIGH**, deep-only (`--deep`)
- Gateway recent inbound (`gateway_no_inbound_recent`) † — **MEDIUM**, deep-only (`--deep`)

*Skips with reason `"Install full wrapper: pip install praisonai"` when `pip install praisonai-code` is used without the `praisonai` wrapper.*

<Note>
**† Skips when the optional `praisonai-bot` package is missing.** On CLI-only installs (`pip install praisonai` without the bot extra), these bot checks report:

```
○ Bot Config: praisonai-bot not installed (optional for CLI-only users)
    Fix: pip install 'praisonai[bot]'
```

`bot_tokens` and `gateway_env_substitution` still run — they don't need the bot schema. See [Installation Extras](/features/installation-extras) for the full list of optional extras.
</Note>

### ACP
- `acp_module` — ACP module import (`praisonai.acp`)
...
See [Gateway Config Migration](/features/gateway-config-migration) for before/after examples.
...
#### Gateway Shell Readiness (`gateway_shell_readiness`)

**Severity:** HIGH

Offline validation of `allow_shell` / `auto_approve_shell` / approval wiring per channel — no LLM or network calls. Auto-discovers config from `--file`, then `gateway.yaml`, then `bot.yaml`.

| Status | Condition |
|--------|-----------|
| PASS | Every `allow_shell: true` channel has `execute_command` wired, an approval backend, and a synced approval registry. Also PASS when no channel enables shell. |
| WARN | No gateway/bot config found. |
| FAIL | One or more channels have missing shell tools, no approval backend, or an unsynced approval registry. |

The check builds each shell-enabled channel through the same `enable_shell_tools()` path the gateway uses, so an exposed/multi-user channel that would be downgraded surfaces as a WARN — matching the gateway's runtime WARN when `auto_approve_shell: true` is set on a `0.0.0.0` bind or a channel with a `group_policy` without `auto_approve_shell_acknowledge_exposed: true`. See [Bot Shell Execution → Exposure-aware auto-approval](/features/bot-shell-execution#exposure-aware-auto-approval).

```bash
praisonai doctor --only gateway_shell_readiness
praisonai doctor bots --file /path/to/bot.yaml
```

#### Gateway Channel Probe (`gateway_channel_probe`)

**Severity:** HIGH · **Deep-only** (`requires_deep=True`)

Live per-channel credential probe (`auth.test` / `getMe` / `identify`). Runs only under `praisonai doctor --deep` or `praisonai doctor bots --deep`.

| Status | Condition |
|--------|-----------|
| PASS | Every channel's credentials probe successfully (surfaces bot identity). |
| FAIL | One or more channels fail the probe. Remediation points at `praisonai gateway test --config <path>`. |
| SKIP | No config file or no channels configured. |

```bash
praisonai doctor bots --deep --only gateway_channel_probe
praisonai doctor --deep --only gateway_channel_probe
```

#### Gateway Duplicate Services (`gateway_duplicate_services`)

**Severity:** HIGH · **Deep-only** (`requires_deep=True`)

Scans for competing gateway services and shared Slack tokens — the same scan behind `praisonai gateway test --check-duplicates`. Inspects macOS LaunchAgents (`ai.praison.bot`, `ai.hermes.gateway`), both `.env` files, and Hermes runtime state. Runs only under `praisonai doctor --deep`.

| Status | Condition |
|--------|-----------|
| PASS | No duplicate LaunchAgent conflict and no shared token fingerprint. |
| WARN | A competing gateway, shared `*_TOKEN` fingerprint, or connected Hermes Slack channel is detected. |
| SKIP | No gateway/bot config found. |

Remediation: *"Compare SLACK_APP_TOKEN across services; stop duplicate gateways before messaging Slack."* See [Duplicates tier](/docs/features/gateway-readiness#duplicates-tier-hermes-conflict-detection).

```bash
praisonai doctor --deep --only gateway_duplicate_services
```

#### Gateway Recent Inbound (`gateway_no_inbound_recent`)

**Severity:** MEDIUM · **Deep-only** (`requires_deep=True`)

Scans the gateway log for recent `@mention received:` markers (last 10 minutes) — the same check behind `praisonai gateway test --check-inbound`. Runs only under `praisonai doctor --deep`.

| Status | Condition |
|--------|-----------|
| PASS | One or more inbound mentions found in the window. |
| WARN | No inbound delivery in recent logs — hint suggests messaging the bot. |
| SKIP | No gateway/bot config found. |

Remediation: *"Send a Slack @mention to your bot, then run: praisonai gateway test --config &lt;cfg&gt; --check-inbound --since 5m."* See [Inbound tier](/docs/features/gateway-readiness#inbound-tier-check-inbound).

```bash
praisonai doctor --deep --only gateway_no_inbound_recent
```
