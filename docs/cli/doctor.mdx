...
When `praisonai[openai-agents]` is installed, the **runtime** category reports **OpenAI Agents SDK** with capabilities such as `agent_creation`, `tool_execution`, and optional `handoff_support` (`supports_handoff=True`). Handoff-capable runtimes are `praisonai`, `autogen_v4`, and `openai_agents`. See [OpenAI Agents framework](/docs/framework/openai_agents).
