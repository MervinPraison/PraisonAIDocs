...
| `--agent` | `-a` | Use a named custom agent profile from `.praisonai/agents/` | |
| `--thinking` | | Set reasoning effort level: `off`, `minimal`, `low`, `medium`, `high` | |
...
`--agent <name>` loads a named custom agent profile from `.praisonai/agents/<name>.md`. The profile's `tools` and `permission`/`mode` scope are applied for the session.

```bash
praisonai code --agent reviewer
praisonai code --agent planner --thinking medium
```

<Note>
A read-only `plan` or `review` profile is rejected for write/edit/shell operations in a `code` session. Use a profile with appropriate permissions for the task.
</Note>

## Thinking Levels (`--thinking`)

`--thinking` maps to the agent's reasoning budget. When omitted, defaults are unchanged.
...
Level names are case-insensitive. An unknown value exits with code 1 and a clear error.

```bash
praisonai code --thinking high "Refactor the auth module"
praisonai code --agent reviewer --thinking medium "Review the diff"
```
