...
| `--file` | `-f` | Attach file(s) to context. **Fully wired as of PR #4943** — attachments are forwarded to the agent via the shared attachment loader used by [`praisonai chat`](/cli/chat), so the model actually reads the file (previously the flag was accepted and silently dropped). Repeatable. | |
...
| `--autonomy` / `--no-autonomy` | | Enable agent autonomy for complex tasks. Wired as of PraisonAI PR #4943 (and #4946) so it now feeds `AsyncTUIConfig.autonomy_mode` (previously declared but never reached the config), mirroring the same flag on [`praisonai chat`](/cli/chat#options). Pass `--no-autonomy` to disable. | `true` |
| `--dangerously-skip-approval` | | Skip all tool approval prompts. Registers `AutoApproveBackend` on the approval registry (so the core `@require_approval` gate is also bypassed) **and** exports `PRAISON_APPROVAL_MODE=auto` / `PRAISONAI_TOOL_SAFETY=off` for the subprocess tree. Strictly opt-in; mutually exclusive with `--plan`. See [Tool Approval](/cli/tool-approval). | `false` |
...
<Note>
As of PR [#4943](https://github.com/MervinPraison/PraisonAI/pull/4943), an AST test guarantees every declared option on `code_main` is read — no `code` option can silently vanish again. `--file` and `--no-autonomy` were the last two that were accepted and dropped; both are now wired.
</Note>

<Note>
Safe mode (`--safe`) is **on by default** as of PR #2369. Dangerous built-in tools ask for approval in interactive sessions and are denied in non-interactive (CI) sessions. Use `--no-safe` to opt out, or `--dangerously-skip-approval` for a complete bypass — both register `AutoApproveBackend` on the approval registry so the core `@require_approval` gate is also bypassed. A safe-mode run *clears* a bypass a prior `--no-safe` call left in the same process (REPL/worker/test), removing only the backend it installed itself. See [Approval](/features/approval) for full details.
</Note>
...
See [Step Budget](/features/max-steps#praisonai-code-defaults) for the general `ExecutionConfig.max_steps` contract and how to detect truncation with `agent.last_stop_reason`.
...
This matches [`praisonai run` → truncation](/cli/run#exit-codes) and closes a hazard where a wrap-up summary (a perfectly non-empty string) was classified as `status: "ok"`, exit `0`, silently reporting truncation as success to CI and benchmark adapters.
...
The `mcp` tool group is **enabled by default** under `-p` as of [PR #4965](https://github.com/MervinPraison/PraisonAI/pull/4965) — if your project declares `mcp.servers`, those tools reach the headless agent alongside the built-ins. Opt out with `PRAISON_TOOLS_DISABLE=mcp` (the same knob that works interactively). See [MCP tools in coding sessions](#mcp-tools-in-coding-sessions).
...
graph TB
    subgraph "praisonai code -p — headless"
        ...
        MCP[🔌 mcp group<br/>from mcp.servers] --> Run
...
This is not sessions-only: as of PraisonAI PR #4946 the wider set of **project-level** CLI state (thinking, compaction, output-style, file-history, queue DB, knowledge vector stores) is also canonical under `.praisonai/`. See [Project data directory](/features/cli-configuration#project-data-directory-praisonai).
</Note>
...
`--tools` is now first-class under `-p`. The resolved tools are merged onto the default headless coding toolset (ACP + edit + search + LSP + MCP), so custom tools **add to** the defaults rather than replacing them.

```bash
# Add the `github` tool to a scripted coding run
praisonai code -p --tools github "Open a PR that fixes the failing lint" --output json
```

With `mcp.servers` set in your project config, MCP tools are in the default set too — the resolved `--tools` argument stacks on top of `ACP + edit + search + LSP + MCP`. See [MCP tools in coding sessions](#mcp-tools-in-coding-sessions).
...
## Shell operators (`PRAISON_SHELL`)

`PRAISON_SHELL` controls what `execute_command` does with a shell operator (`>`, `|`, `&&`, `$(...)`).

| `PRAISON_SHELL` | Behaviour |
|---|---|
| unset / `off` (default) | Operators are refused with an error naming the operator; plain commands run as before |
| `sandboxed` | Real `/bin/sh -c` inside OS containment (Seatbelt/`bwrap`); refuses if enforcement can't be proven |
| `unsafe` | Real `/bin/sh -c`, no containment — opt-in only |

The real-shell path is gated by the same `critical`-risk `execute_command` approval as the plain executor. See [Shell Modes](/features/praison-shell) and [OS Sandbox](/features/os-sandbox).

## MCP tools in coding sessions

Declaring `mcp.servers` in your project config makes those tools appear inside `praisonai code` (both the interactive TUI and the `-p` headless one-shot) as a standard `mcp` tool group.
...
The coding session reuses the same collector as `praisonai run`, so the server's tools (for example `tiny_add`) reach the agent. Two rules apply:

- **Opt out** with `PRAISON_TOOLS_DISABLE=mcp` to drop the whole group.
- **No shadowing** — an MCP server can never override a built-in tool name; the built-in wins and the duplicate MCP tool is skipped.

See [MCP](/features/mcp) and [Load MCP Tools](/features/load-mcp-tools).
...
- [Step Budget](/features/max-steps#praisonai-code-defaults) - The `max_steps` contract and the coding-sized default
