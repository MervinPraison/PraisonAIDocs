...
| `--autonomy` / `--no-autonomy` | | Enable agent autonomy for complex tasks. As of PraisonAI PR #4946 this now feeds `AsyncTUIConfig.autonomy_mode` (previously declared but never reached the config), mirroring the same flag on [`praisonai chat`](/cli/chat#options). Pass `--no-autonomy` to disable. | `true` |
