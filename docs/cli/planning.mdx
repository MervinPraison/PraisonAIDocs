...
<Warning>
**Don't confuse `--planning` with `--plan`.** `--planning` enables the agent's multi-step plan-then-execute workflow (this page). `--plan` on `praisonai run` / `praisonai code` is a completely different flag: it selects the read-only planning **permission** mode (`PermissionMode.PLAN`) so mutating tools are denied. Inside `praisonai chat` / `praisonai code`, the interactive `/plan` slash command toggles the same read-only permission mode on/off. See [Slash Commands › /plan](/cli/slash-commands#plan), [Permissions](/cli/permissions), and [Permission Modes](/features/permission-modes).
</Warning>
