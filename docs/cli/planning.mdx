...
<Warning>
**Don't confuse `--planning` with `--plan`.** `--planning` enables the agent's multi-step plan-then-execute workflow (this page). `--plan` on `praisonai run` / `praisonai code` is a completely different flag: it selects the read-only planning **permission** mode (`PermissionMode.PLAN`) so mutating tools are denied. See [Permissions](/cli/permissions) and [Permission Modes](/features/permission-modes).
</Warning>
