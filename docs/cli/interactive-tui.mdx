...
- **Slash commands** - `/help`, `/model`, `/stats`, `/compact`, `/undo`, and more
...
praisonai --interactive --llm gpt-4o
```

## Chat Mode (Non-Interactive Testing)

For testing and scripting, use `--chat-mode` to run a single prompt with interactive-style output:
...
praisonai "read the file README.md" --chat-mode
```

<Note>
`--chat-mode` is different from `praisonai chat` which starts a web-based Chainlit UI.
</Note>
