...
For testing and scripting, use `--chat` (or `--chat-mode`) to run a single prompt with interactive-style output:
...
<Note>
`--chat` is different from `praisonai chat` which starts a web-based Chainlit UI. The `--chat-mode` flag is an alias for backward compatibility.
</Note>
