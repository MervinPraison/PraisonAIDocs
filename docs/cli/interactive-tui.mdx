...
When `checkpoints.auto` is on, `/undo` in the TUI now rolls back the workspace using the same engine as the coding REPL. The TUI captures a **session-start baseline** on launch and a **pre-turn checkpoint** before every submission, so `/undo` unwinds one turn at a time:
