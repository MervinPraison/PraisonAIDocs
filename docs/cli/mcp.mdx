...
<Note>
All `praisonai mcp <cmd>` **host** commands (`serve`, `list-tools`, `doctor`, `serve-recipe`) also run as `praisonai-mcp <cmd>` when only the standalone [praisonai-mcp](/features/praisonai-mcp-package) package is installed. **Config** commands (`list`, `add`, `remove`, `test`, `sync`, `tools`, `describe`, `status`, `run`, `auth`, `logout`) require `praisonai-code`; running one without it prints:
