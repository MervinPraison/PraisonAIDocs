...
| `list` | List configuration values (`--scope all\|user\|project`) |
...
```bash
praisonai config list                     # resolved, merged config (default: --scope all)
praisonai config list --scope user        # only what ~/.praisonai/config contributes
praisonai config list --scope project     # only what ./.praisonai/config contributes
```

<Note>
**Fixed in PraisonAI PR #4946.** `--scope` (`-s`) now selects a single layer. Previously `--scope user` and `--scope project` produced **byte-identical** output because the body always printed the fully merged config. Valid scopes are `all` (default), `user`, and `project`; an unknown scope exits `2`.
</Note>

| Scope | Shows |
|-------|-------|
| `all` (default) | The resolved, deep-merged configuration across all layers |
| `user` | Only the global user config file's contribution |
| `project` | Only this project's config file contribution |
...
| `user` (default) | `~/.praisonai/config.toml` | `0644` |
| `project` | `./.praisonai/config.yaml` | `0644` |

`praisonai config set` / `praisonai config reset` write TOML to the canonical `~/.praisonai/config.toml`. Both the canonical home and the `config.yaml` written by `setup` / `init` honour `PRAISONAI_HOME` — see [CLI Home Root](/docs/features/cli-configuration#cli-home-root).
...
<Note>
Model lives under `agent.model`, not at the top level. See [CLI Configuration](/features/cli-configuration) for the full schema.
</Note>
...
Store API keys via env vars or [`praisonai auth`](/cli/auth) — `api_key` is never written to YAML.
...
| Path | Status |
|------|--------|
| `~/.praison/config.toml` | **Read-only** legacy fallback — migrated to `agent.*` / `rag.*`; resolved only if the canonical `~/.praisonai/config.yaml` is absent |
...
- [Single-Source Config](/features/single-source-config) — Model + MCP + permissions in one file
- [CLI Configuration](/features/cli-configuration) — feature guide with diagrams and best practices
- [Declarative Permissions](/features/declarative-permissions) — all permission surfaces
- [Profile](/cli/profile) — Performance profiling
- [Environment](/cli/env) — Environment diagnostics
