...
The `--thinking` flag is applied after the agent is constructed. It takes effect on the very next request whether the LLM instance has been materialised yet or not (fixed in PR #4479).
...
- [Thinking Budgets Feature](/features/thinking-budgets)
- [Reasoning Effort — Session Persistence](/features/reasoning-effort#session-persistence) — why `--thinking` restores automatically on `--continue`
