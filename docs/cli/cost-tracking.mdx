...
Models that are **not** in `DEFAULT_PRICING` are priced at $0 — they still run, but `total_cost_usd` will under-report and the `max_cost` brake won't trip. See [Async Agent Scheduler](/features/async-agent-scheduler) for the full budget pattern.
