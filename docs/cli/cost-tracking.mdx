...
See [Usage](/docs/cli/usage) for cross-session aggregate reporting.
...
To aggregate these persisted totals across many sessions — by day, model, or project — see [Usage](/docs/cli/usage).
...
![Cost Tracking Monitors API Expenses](./cost-tracking-cost-tracking-monitors-api-exp.gif)
...
Cross-link: see [Session](/docs/cli/session) for the full `session list` column reference and [Run](/docs/cli/run) for the per-run usage footer.
...
See [Usage](/docs/cli/usage) for the full command reference, grouping options, and JSON output shape.

## Related Features

- [Usage](/docs/cli/usage) - Aggregate spend/token reporting across sessions
...
- [Session](/docs/cli/session) - Per-session token and cost columns in `session list`
- [Run](/docs/cli/run) - Per-run usage footer after each prompt
- [Usage](/docs/cli/usage) - Cross-session aggregate token and cost reporting
