...
| **Basic** | `read_file`, `write_file`, `list_files`, `execute_command`, `internet_search` | Standard tools. `execute_command` refuses shell operators unless `PRAISON_SHELL` enables a real shell — see [Shell operators](#shell-operators) |
| **MCP** | *(discovered at runtime from `mcp.servers`)* | Tools from configured MCP servers. Opt out with `PRAISON_TOOLS_DISABLE=mcp`; a server can never shadow a built-in tool name (built-in wins). See [MCP tools in coding sessions](/cli/code#mcp-tools-in-coding-sessions) |
...
| `execute_command` | Run shell commands. Shell operators (`>`, `\|`, `&&`, `$(...)`) are refused unless `PRAISON_SHELL` enables a real shell — see below |
| `internet_search` | Search the web |

#### Shell operators

`execute_command` runs `subprocess.Popen(shell=False)`, so a shell operator would become a literal argument and report false success. To prevent that, operators are refused by default and only run when `PRAISON_SHELL` enables a real, approval-gated `/bin/sh`.

| `PRAISON_SHELL` | Behaviour |
|---|---|
| unset / `off` (default) | Operators refused; plain commands run as before |
| `sandboxed` | Real `/bin/sh -c` inside OS containment (Seatbelt/`bwrap`) |
| `unsafe` | Real `/bin/sh -c`, no containment |

See [Shell Modes](/features/praison-shell) and [OS Sandbox](/features/os-sandbox).
