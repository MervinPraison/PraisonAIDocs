...
Repeated calls to `resolve_tools()` over the **same `registry` object** share one underlying [`ToolResolver`](/features/tool-resolver) instead of rebuilding it per call.
...
<Note>
**Canonical resolution chain:** `resolve_tools()` is a thin wrapper around [`ToolResolver`](/features/tool-resolver) with a registry-override layer on top. See [Templates Module → Tool Override](/docs/sdk/praisonai/templates#tool-override-integration) for programmatic usage.
