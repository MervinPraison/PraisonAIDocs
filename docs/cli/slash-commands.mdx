...
File-based commands in `.praisonai/commands/*.md` auto-register in interactive mode with `kind=CommandKind.CUSTOM`. Disable discovery with `SlashCommandHandler(discover_custom=False)`. See [Custom Agents & Commands](/docs/features/custom-agents-commands).
