...
| `/btw` | Ask a side question in a throwaway context (`--keep` to note it in history) — see [/btw side question](/features/btw-side-question) |
...
File-based commands in `.praisonai/commands/*.md` (and `.claude/commands/*.md` / `.agents/commands/*.md` when present) auto-register in interactive mode with `kind=CommandKind.CUSTOM`. Disable discovery with `SlashCommandHandler(discover_custom=False)`. See [Custom Agents & Commands](/features/custom-agents-commands).
...
**Not every installed skill appears in the `/` menu.** A skill whose `SKILL.md` frontmatter sets `user-invocable: false` is treated as model-only: the LLM can still auto-trigger it from the system-prompt listing, but it is absent from the completion menu and typing `/<name>` falls through to "Unknown command". See [Skills → Invocation Policy](/features/skills-invocation#invocation-policy) for the full visibility truth table.
...
<Note>
Browse and run installed skills from the same menu — see [Skill Invocation → Discover skills in the interactive menu](/features/skills-invocation#discover-skills-in-the-interactive-menu).
</Note>
...
<Note>
`/compact` used to be a display toggle. That behaviour now lives under `/compact-display` (alias `/dense`). See [Compaction](/cli/compaction).
</Note>
...
See [Git Integration → Operator slash commands](/cli/git-integration#operator-slash-commands).
...
See [Repository Map](/cli/repo-map).
...
You don't need to `--session` first — the fallback means `/export` always produces something useful. See [`praisonai session export`](/cli/session#export-a-session) for the same renderer on the CLI.
...
<Note>
In **bot chats**, `/tasks` is per-user scoped: a caller only ever sees and cancels tasks whose `metadata["user_id"]` matches theirs (fail-closed). See [Bot Chat Commands → /tasks](/features/bot-commands#tasks).
</Note>
...
- [Background Tasks](/cli/background) - `/tasks` and `praisonai background list`
