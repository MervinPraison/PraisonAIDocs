...
See [Dataset Tooling](/features/praisonai-train-dataset-tooling) for recipes, dedup, snapshots, and the `generate_dataset()` Python API.
...
See [Dataset Tooling](/features/praisonai-train-dataset-tooling) for the full check list and the `score()` / `filter_rows()` Python API.

<Card title="Export trials with QC (from-trials --qc)" icon="filter" href="/features/praisonai-train-dataset-tooling#export-trials-with-qc-from-trials-qc">
...
See [Speed Benchmark](/features/praisonai-train-benchmark) for the `benchmark_deployments()` Python API and cross-endpoint benchmarking.
...
<Card title="Cross-file dedup (Python API)" icon="database" href="/features/praisonai-train-dataset-tooling#cross-file-dedup">
...
<Card title="from-trials (feature guide + Python API)" icon="recycle" href="/features/praisonai-train-dataset-tooling#from-trials">
...
<Note>
The four `--remote-*` flags send this one run to a remote GPU box instead of training locally — the same as a `remote:` block in `config.yaml` or the desktop "Run on" dropdown. Omit `--remote-host` (and set no `remote.host`) to train here as before. See [Remote in one step](/features/praisonai-train-inline-remote).
</Note>
...
<Card title="Full chat_template reference" icon="graduation-cap" href="/train#model--template-keys">
...
<Note>
`method: cpt` with `modules_to_save: [embed_tokens, lm_head]` enables continued pretraining on a raw-text corpus (a dataset with a `text` column). See [Continued Pretraining](/features/praisonai-train-cpt).
</Note>
...
<Note>
**Vision configs no longer publish by default (PraisonAI [#4879](https://github.com/MervinPraison/PraisonAI/pull/4879)).** The CLI writes only the keys you supplied to `config.yaml`, so a vision run (any model whose name contains `vision`, `-vl-`, or `visionmodel`) typically omits `huggingface_save`. The vision trainer used to default that flag to the string `"true"` and push to the Hub anyway — or crash on a missing `hf_model_name` after training. From #4879 an omitted flag skips publishing, matching the LLM path. See [Vision Fine-Tuning](/features/praisonai-train-vision).
</Note>
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `method` | `str` | `"sft"` | Training method — `sft`, `dpo`, `orpo`, or `kto`. Case-insensitive. See [Preference Training](/features/train-preference-tuning). |
| `model` | `str` | — | Alias for `model_name` (parity with `--model`). If both are set, `model_name` wins. |
| `method` | `str` | `"sft"` | Training method. One of `sft`, `cpt`, `dpo`, `orpo`, `kto`, `grpo`. `cpt` is continued pretraining on a raw-text corpus — see [Continued Pretraining](/features/praisonai-train-cpt); `grpo` is scored by reward functions — see [Reward Functions & GRPO](/features/train-reward-functions). |
...
Model access, PEFT selectors, quant, and vLLM rollouts — all optional and additive. See [Unsloth Surface](/features/praisonai-train-unsloth-surface) for the full walkthrough.
...
<Note>
Invalid or unknown YAML keys are reported before training starts. Missing required keys (`model_name`, `max_seq_length`, `dataset`) raise `ValueError` with a minimal example; unknown keys are collected into one `WARNING` block with `did you mean` suggestions and training continues. See [Train → Config validation](/docs/train#config-validation).
</Note>
...
See [Train → Preflight validation](/docs/train#preflight-validation) for the full check table, fix guidance, and the 35 valid quantization values.
...
On a bare `pip install praisonai-train`, `llm` prints one of two messages: `LLM fine-tuning dependencies not installed` with `pip install "praisonai-train[llm]"` when `praisonai-code` is absent, or `Failed to load LLM fine-tuning runner: <ImportError>` when `praisonai-code` is present but a downstream import (torch/unsloth) failed. See [Train](/train) for the full flow.
...
Run the printed `pip install -U ...` command — it upgrades every listed package at once. A `dev` / `rc` suffix on a matching release satisfies the floor (`2.6.0rc1` counts as `2.6.0`). See [Train → Runtime version-floor enforcement](/train#runtime-version-floor-enforcement).
...
The trainer detects the distributed launch, loads the full model per rank, and lets only rank 0 save and publish. See [Multi-GPU](/features/praisonai-train-multigpu) for the full walkthrough.
...
The dispatcher ships the config (stripping the `remote:` block so the far side does not dispatch again), streams the log, and forwards Ctrl-C to the remote run. Credentials in the block (`password`, `token`, `identity_file`, …) are refused up front. See [Remote in one step](/features/praisonai-train-inline-remote).
...
See [Checkpointing](/features/praisonai-train-checkpointing) for `save_total_limit`, best-checkpoint, and early-stopping keys.
...
When `unsloth` isn't installed (the CLI is importable without the training extra), the command falls back to a 7-entry curated list of one current base per family, sized for a single 24 GB GPU in 4-bit. See [Supported models](/features/praisonai-train-package#supported-models).
...
| `--quant` | — | str | — | Quantization method for `gguf`/`ollama`. One of 35 values — 24 standard plus 11 imatrix; see [Export → Valid quant values](/features/praisonai-train-export#valid-quant-values). An invalid value fails fast, listing every valid choice. |
...
<Card title="Export an already-trained model" icon="cloud" href="/features/praisonai-train-package#exporting-an-already-trained-model">
...
<Note>
The server binds to `127.0.0.1` — there is no `--host` flag. Override the `llama-server` binary with the `LLAMA_CPP_BIN` env var (points at the binary or its directory). See [Serve & MTP Fast-Inference](/features/praisonai-train-serve) for the concept-level walkthrough and the honest runtime caveats.
</Note>
...
See [List Checkpoints](/features/praisonai-train-checkpoints) for the full workflow and `jq` patterns.
...
See [Infer (one-shot inference)](/features/praisonai-train-infer) for the full walkthrough and the `infer` vs `serve` vs `benchmark` decision guide.
...
- [Train](/train) - Training overview and fine-tuning setup
- [Export a trained model](/features/praisonai-train-export) - Publish a trained model without re-training
- [Unsloth Surface](/features/praisonai-train-unsloth-surface) - Gated models, PEFT selectors, offline merge, and vLLM rollouts
- [Train → Preflight validation](/docs/train#preflight-validation) - Fast, friendly config checks before any GPU load
- [Serve & MTP Fast-Inference](/features/praisonai-train-serve) - Serve a GGUF over OpenAI HTTP with MTP speculative decoding
- [Dataset Tooling](/features/praisonai-train-dataset-tooling) - Generate and quality-check instruction datasets
- [praisonai-train Package](/features/praisonai-train-package) - Standalone package guide
- [Multi-GPU Training](/features/praisonai-train-multigpu) - Fine-tune across multiple GPUs with torchrun
- [Remote Training](/features/praisonai-train-remote) - Ship a run to a GPU box over SSH with the `remote` group
- [Remote in one step](/features/praisonai-train-inline-remote) - Send a run to a remote GPU box with `--remote-host`, `remote:` YAML, or the desktop dropdown
- [Checkpointing](/features/praisonai-train-checkpointing) - Save, resume, and keep the best checkpoint
- [Eval](/cli/eval) - Evaluation and testing
- [Storage Backends](/storage/backends) - Pluggable storage backends
