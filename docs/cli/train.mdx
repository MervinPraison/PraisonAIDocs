...
<Note>
Training dependencies are lazy-loaded to improve CLI startup time — they're only imported when the `train` subcommand actually runs.
</Note>
...
```
Training dependencies not available. Install with: pip install torch transformers unsloth datasets trl psutil
```
