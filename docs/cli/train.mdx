...
```bash
praisonai train {generate|validate|benchmark|dedup|llm|models|agents|export|serve|list|show|apply|checkpoints|infer|remote}
```
...
| `--dry-run` | - | Print the resolved config as YAML and exit, without training. Does not load the runner. | `false` |
