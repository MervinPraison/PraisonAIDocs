...
```bash
praisonai train {generate|validate|benchmark|dedup|llm|agents|export|serve|list|show|apply}
```
...
<Card title="Export trials with QC (from-trials --qc)" icon="filter" href="/features/praisonai-train-dataset-tooling#export-trials-with-qc-from-trials-qc">
  `data from-trials --qc` reuses this QC filter but auto-disables the Tamil script check for English agent trajectories — pass `qc_cfg={"script_range": [...]}` to opt back in for another language.
</Card>
...
graph LR
    subgraph "Export a trained model"
        Dir[📁 lora_model/] --> Target{🎯 Target}
        Target -->|hf| HF[✅ Hugging Face repo]
        Target -->|gguf| GGUF[✅ Local .gguf + optional push]
        Target -->|ollama| Ol[✅ ollama.com]
    end
    ...
    class HF,GGUF,Ol output
```

```bash
praisonai train export TARGET --model-dir DIR [OPTIONS]
```
...
| Option | Short | Description | Default |
|--------|-------|-------------|---------|
| `target` (positional) | - | Export target — `ollama`, `gguf`, or `hf` | **required** |
| `--model-dir` | `-d` | Directory of the already-trained model (e.g. `lora_model`) | **required** |
| `--config` | `-c` | Optional `config.yaml` for extra export knobs | - |
| `--ollama` | - | Ollama model name, e.g. `myuser/mymodel` | - |
| `--hf` | - | Hugging Face repo id, e.g. `myuser/mymodel` | - |
| `--quant` | - | Quantization method for `gguf`/`ollama`, e.g. `q4_k_m` | - |
| `--base-model` | - | Base model id (drives chat-template selection) | recovered from `config.json` |

### Examples

```bash
# Publish a merged model to Hugging Face
praisonai train export hf     --model-dir lora_model --hf   me/my-model

# Produce a local GGUF; add --hf to also push it to the Hub
...
# Create and push an Ollama model
praisonai train export ollama --model-dir lora_model --ollama me/my-model --quant q4_k_m
```

<Note>
When `--base-model` is omitted, the exporter reads `<model-dir>/config.json` `_name_or_path` (falling back to the directory name) to pick the chat template — so a fresh training run exports with the right template for free.
</Note>
...
| Target | Requires | Error when missing |
|--------|----------|--------------------|
| `hf` | `--hf` | `A Hugging Face repo id is required for this target` |
| `gguf` | *(none)* — writes a local `.gguf`; `--hf` only adds a Hub push | — |
| `ollama` | `--ollama` | `An Ollama model name is required for this target` |

An unknown `target` fails with `Unknown export target: <target>` and exit `1`. A `--quant` value outside the [valid set](/docs/train#preflight-validation) fails before the model loads.
...
<Note>
The `DATASET` positional now actually reaches the trainer. On PraisonAI releases before [PR #4239](https://github.com/MervinPraison/PraisonAI/pull/4239) the argument was silently dropped and the run fell back to `yahma/alpaca-cleaned`; upgrade if `praisonai-train show <session>` reports a model trained on a corpus you didn't provide.
</Note>
...
| `method` | `str` | `"sft"` | Training method. One of `sft`, `cpt`, `dpo`, `orpo`, `kto`. `cpt` is continued pretraining on a raw-text corpus — see [Continued Pretraining](/features/praisonai-train-cpt). |
...
See [Train → Preflight validation](/docs/train#preflight-validation) for the full check table, fix guidance, and the 12 valid quantization values.
...
| `--quant` | — | str | — | Quantization method for `gguf`/`ollama`. One of `q4_k_m, q5_k_m, q8_0, q4_0, q4_1, q5_0, q5_1, q3_k_m, q6_k, f16, bf16, q2_k`. An invalid value fails fast, listing every valid choice. |
...
```bash
# Publish merged FP16 weights to Hugging Face
praisonai-train export hf --model-dir lora_model --hf me/my-model

# Write a local GGUF build (add --hf to also push it to the Hub)
praisonai-train export gguf --model-dir lora_model --quant q4_k_m
...
See [Export a trained model](/features/praisonai-train-export) for the full workflow, failure modes, and Python API.
