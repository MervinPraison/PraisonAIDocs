...
# Non-Llama bases use their own chat template automatically
praisonai train llm --model unsloth/gemma-2-2b-it-bnb-4bit dataset.json
praisonai train llm --model unsloth/Qwen2.5-0.5B-Instruct-bnb-4bit dataset.json
```
...
| `assistant_only_loss` | `bool` | `false` | Compute loss only on assistant turns (model-agnostic). Requires a chat template that emits `{% generation %}` blocks. |
