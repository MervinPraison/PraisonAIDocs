...
| `read:.env` | Reads of `.env` — gated by default[^secret] |
| `read:*.pem` | Reads of PEM private keys — gated by default[^secret] |
| `read:id_rsa` | Reads of SSH private keys — gated by default[^secret] |
...
[^secret]: Reads of secret files default to ask even without a rule — see [Secret Read Gate](/docs/features/secret-read-gate).
