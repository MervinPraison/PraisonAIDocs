...
| Item | Value |
|------|-------|
| Python | 3.11 / 3.12 / 3.13 (3.13 fixed in [PraisonAI#5103](https://github.com/MervinPraison/PraisonAI/pull/5103)) |
| Install size | ~500MB (Langflow + a2a-sdk + fastapi stack) |
| First-boot on Windows | ~5 minutes (Langflow migrations + cache) |
| First-boot on macOS/Linux | Typically < 2 minutes |

<Warning>
`praisonai[flow]` and `praisonai[claw]` are **mutually exclusive** — the two extras pin conflicting `a2a-sdk` versions. Install them in separate virtual environments.
</Warning>
...
<Note>
If you set `LANGFLOW_COMPONENTS_PATH` in your shell before running `praisonai flow`, use **commas** to separate paths — Langflow's pydantic settings only split on `,`. PraisonAI joins its own components dir with your value using a comma (PR #3792); a semicolon-separated pre-existing value would still be treated as a single (broken) path by Langflow itself.
</Note>

| Env var pre-set | Effect on `LANGFLOW_COMPONENTS_PATH` |
|---|---|
| unset | `<PraisonAI COMPONENTS_DIR>` |
| set to `/x` | `<PraisonAI COMPONENTS_DIR>,/x` |
| set to `/x,/y` | `<PraisonAI COMPONENTS_DIR>,/x,/y` (works) |
| set to `/x;/y` | `<PraisonAI COMPONENTS_DIR>,/x;/y` — **`/x;/y` is treated as a single path by Langflow (broken)** |
...
<Accordion title="First boot on Windows is slow — that's expected">
Langflow runs Alembic migrations and warms a component cache on first launch, which takes about 5 minutes on Windows. Watch for `Uvicorn running on http://127.0.0.1:<port>` in the log tail; subsequent starts are under 60s.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="Visual Workflow Builder" icon="sitemap" href="/ui/flow">
  UI guide for Agent and Agent Team nodes
</Card>
<Card title="Langflow Integration" icon="diagram-project" href="/integrations/langflow">
