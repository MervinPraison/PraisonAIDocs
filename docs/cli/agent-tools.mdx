...
<Note>
The Tier-1 wrapper bridge (`praisonai._async_bridge`) now resolves `PRAISONAI_RUN_SYNC_TIMEOUT` **per call** (not at import), so a late-set value takes effect on the next call rather than requiring a Python restart. The Tier-2 `praisonai_code._run_sync` bridge below reads the same env var. See [Async Bridge → Timeout resolution](/features/async-bridge#timeout-resolution-omitted-vs-none-vs-number).
</Note>
...
<Warning>
This env var is **shared** with the wrapper-level `praisonai._async_bridge.run_sync`. Setting it applies to **both** the wrapper bridge (used by gateway, a2u, mcp_server, scheduler) and the `praisonai-code` agent-tool bridge (used by the ACP/LSP tools created above). See [Async Bridge](/features/async-bridge) for the wrapper-level surface.
</Warning>
