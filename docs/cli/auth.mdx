...
graph LR
    subgraph "Auth Flow"
        A[🔑 API Key] --> B[💾 ~/.praison/credentials.json]
        B --> C[🚀 praisonai run]
        C --> D[✅ Injected into env]
    end
...
```
Enter your key at the prompt (hidden while typing).
</Step>
...
```
Credentials are injected automatically — no `export` needed.
</Step>
</Steps>
...
<AccordionGroup>
<Accordion title="Never store keys in environment files committed to git">
Use `praisonai auth login` to store keys in `~/.praison/credentials.json` (outside the project) instead of `.env` files that may be accidentally committed.
</Accordion>
<Accordion title="Validate after storing">
Run `praisonai auth status openai --validate` immediately after login to confirm the key is accepted by the API.
</Accordion>
<Accordion title="Use --skip-validation in CI pipelines">
If your CI environment cannot make live API calls during setup, pass `--skip-validation` when storing credentials.
</Accordion>
<Accordion title="Rotate keys safely">
Run `praisonai auth login openai --key sk-new...` to overwrite an existing entry — no need to logout first.
</Accordion>
</AccordionGroup>
