...
| Command | Description |
|---|---|
| `auth login [provider]` | Store credentials. Provider is optional — omit it for a catalogue-driven picker. `--method auto` (default) picks OAuth if available, else API key |
...
<Note>The `auth list` and `auth status` table columns changed in this release — `Method`, `Source`, and `Expires` were added, and active env-var credentials are now surfaced. Scripts that parse the table output need updating. The `--json` output is additive only (adds `source` / `env_var`; no existing keys removed).</Note>
