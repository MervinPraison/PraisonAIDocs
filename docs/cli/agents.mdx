...
<Info>
**Install requirement:** `praisonai agents` requires `pip install praisonai` (the full wrapper). On a standalone `pip install praisonai-code` install these subcommands (`run`, `list`, `show`, `delete`) exit `1` with `agents requires the full wrapper. Install the full wrapper: pip install praisonai` — a single-line hint, no Rich traceback ([PR #2854](https://github.com/MervinPraison/PraisonAI/pull/2854)).
</Info>
