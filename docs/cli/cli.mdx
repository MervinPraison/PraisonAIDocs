...
graph TB
    ...
    Legacy --> FileOrYaml{File exists on disk OR<br/>ends in .yaml/.yml?}
    FileOrYaml -->|Yes| YAMLFile[📄 Agent file]
    FileOrYaml -->|No| FreeText[💬 One-shot prompt<br/>handle_direct_prompt]
...
```bash
# Simplest one-shot run — no flags, no YAML, no setup
...
<Tip>
A bare positional that is not an existing file and not a `.yaml`/`.yml` path is run as a one-shot prompt — no `-p`, `--prompt`, or `--auto` flag needed. Existing files and YAML paths still load as agent files.
</Tip>
