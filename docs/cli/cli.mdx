...
graph TB
    ...
    Auth -->|Yes| RouteCheck{Flag or<br/>.yaml/.yml?}
    Wizard -->|User accepts| SetupRun[⚙️ praisonai setup]
    Wizard -->|User declines| Exit0[💡 Exit 0 with hint]
    SetupRun --> RouteCheck
    RouteCheck -->|No — bare prompt| Typer[⚡ Typer run<br/>modern engine]
    RouteCheck -->|Yes — YAML / flags| Legacy[🔄 Legacy shim]
...
A **bare prompt** (no flags, not a `.yaml`/`.yml` file) routes to Typer `run` — the same modern engine as `praisonai run "…"`. Only YAML files and flag-bearing invocations reach the legacy shim.
