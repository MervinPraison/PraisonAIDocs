...
graph TB
    ...
    Auth -->|Yes| RouteCheck{Legacy-only flag<br/>or .yaml/.yml?}
    Wizard -->|User accepts| SetupRun[⚙️ praisonai setup]
    Wizard -->|User declines| Exit0[💡 Exit 0 with hint]
    SetupRun --> RouteCheck
    RouteCheck -->|No — bare prompt / run-supported flags| Typer[⚡ Typer run<br/>modern engine]
    RouteCheck -->|Yes — YAML / legacy-only flags| Legacy[🔄 Legacy shim]
...
A **bare prompt** routes to Typer `run` — the same modern engine as `praisonai run "…"`. Flags accepted by `run` itself (`--model`, `--continue`, `--session`, `--output`, `--stream`, `--verbose`, …) are forwarded to the modern engine too. Only YAML files and **legacy-only** flags (`--framework`, `--auto`, `--serve`, …) reach the legacy shim; when a legacy-only flag forces that fallback, a one-line stderr notice points at the modern equivalent.

<Note>
As of PraisonAI 4.x (PR [#3466](https://github.com/MervinPraison/PraisonAI/pull/3466)), a bare prompt combined with a flag `run` accepts — `--model`, `--continue`, `--session`, `--output`, `--stream`, `--verbose`, … — now reaches the modern engine (session continuity, `--output` modes, credential gate, permissions all in effect).
</Note>
...
## Bare interactive launch (no subcommand)

Running `praisonai` with no prompt and no subcommand launches the interactive TUI. As of PR [#4912](https://github.com/MervinPraison/PraisonAI/pull/4912) it accepts three continuity flags — parity with `run` and `chat`:

| Flag | Short | What it does |
|------|-------|--------------|
| `--continue` | `-c` | Resume the most recent session for this project into the TUI. |
| `--session <id>` | `-s <id>` | Resume a specific session id into the TUI. |
| `--fork` | | Fork the resumed session non-destructively (requires `--continue` or `--session`). |

```bash
# Fresh interactive session (unchanged)
praisonai

# Resume the most recent project session into the TUI
praisonai -c

# Resume a specific session id
praisonai --session abc12345

# Branch a resumed session into a new timeline
praisonai --fork --session abc12345
```

The resume path reuses `run` / `chat`'s existing machinery — the recorded session model is restored, the prior transcript is replayed into scrollback, and the primary agent is wired to the resumed session's store so a follow-up prompt has real conversational memory of prior turns (not just display-only history).

`--fork` alone (without `--continue` / `--session`) and `--continue` + `--session` together both exit `1` with an actionable error — parity with `praisonai run`. See [Interactive TUI → Launching with a resumed session](/cli/interactive-tui#launching-with-a-resumed-session) for the full flow.
...
The guard fires on the legacy dispatcher only; a plain `praisonai show` now reaches Typer `run` and is sent to the model as the literal prompt `show`. It only kicks in once an invocation is already on the legacy path — because it carries a flag or points at a `.yaml`/`.yml` file — and a lone token there looks like a mistyped or reserved command.

```mermaid
graph LR
    subgraph "Unknown Command Guard (legacy path)"
        A[💻 legacy invocation<br/>flag or .yaml] --> C{🔍 Classify}
        C -->|Reserved verb| H[💡 Hint + exit 2]
        C -->|Fuzzy match| S[🧠 Did you mean?]
        C -->|Unrelated word| P[✅ LLM prompt]
    end
...
|---------|-------|--------|
| `praisonai show` (bare) | Typer `run` | prompt `show` sent to the model |
| `praisonai show --framework crewai` | Legacy | `Unknown command: 'show'` hint + `exit 2` |
| `praisonai memoyr --framework crewai` | Legacy | `Did you mean: memory?` + `exit 2` |
| `praisonai "write a poem"` (bare) | Typer `run` | prompt sent to the model |

### The Two Hint Shapes

On the legacy path, a **reserved verb** prints its canonical replacements:

```bash
$ praisonai show --framework crewai
...
```bash
$ praisonai memoyr --framework crewai
...
<Note>
On the legacy path the guard is **case-insensitive** — `praisonai SHOW --framework crewai` fires the same reserved-verb hint as its lowercase form.
</Note>
...
|---------|:---------:|--------|
| Guard hit on legacy (reserved verb or typo) | `2` | Hint on `stderr` |
...
A bare prompt is **identical to** `praisonai run "hello world"` — same modern engine, same features (session continuity, `--output` modes, credential gate, permissions/checkpoints).
...
<CardGroup cols={2}>
  <Card title="CLI Help Panels" icon="list-tree" href="/features/cli-help-panels">
    `--help` grouped into 7 stable, categorised panels
  </Card>
  <Card title="Tool Tracking" icon="wrench" href="/cli/tool-tracking">
  ...
  <Card title="Autonomy Modes" icon="robot" href="/cli/autonomy-modes">
  ...
  <Card title="Usage" icon="chart-column" href="/cli/usage">
    Local token/cost spend reporting across sessions
  </Card>
  <Card title="Repository Map" icon="sitemap" href="/cli/repo-map">
  ...
  <Card title="Sandbox Execution" icon="shield-halved" href="/cli/sandbox-execution">
...
<CardGroup cols={2}>
  <Card title="Deep Research" icon="magnifying-glass-chart" href="/cli/deep-research">
    Automated multi-step research with citations
  </Card>
  <Card title="Planning" icon="clipboard-list" href="/cli/planning">
  ...
  <Card title="Workflow" icon="sitemap" href="/cli/workflow">
    Multi-step YAML workflow execution
  </Card>
  <Card title="Hooks" icon="webhook" href="/cli/hooks">
    Event-driven actions and callbacks
  </Card>
  <Card title="Claude Memory" icon="brain" href="/cli/claude-memory">
    Anthropic's memory tool integration
  </Card>
  <Card title="Guardrail" icon="shield-check" href="/cli/guardrail">
    Validate agent outputs with LLM-based guardrails
  </Card>
  <Card title="Metrics" icon="chart-bar" href="/cli/metrics">
    Track token usage and cost metrics
  </Card>
  <Card title="Image Processing" icon="image" href="/cli/image">
    Process images with vision-based AI agents
  </Card>
  <Card title="Telemetry" icon="signal" href="/cli/telemetry">
    Enable usage monitoring and analytics
  </Card>
  <Card title="MCP" icon="plug" href="/cli/mcp">
    Integrate Model Context Protocol servers
  </Card>
  <Card title="Fast Context" icon="bolt" href="/cli/fast-context">
    Search codebase for relevant context
  </Card>
  <Card title="Knowledge" icon="book" href="/cli/knowledge">
    Manage RAG/vector store knowledge bases
  </Card>
  <Card title="Scheduler" icon="clock" href="/cli/scheduler">
    Run agents 24/7 with timeout and cost limits
  </Card>
  <Card title="Session" icon="clock-rotate-left" href="/cli/session">
    Manage conversation sessions
  </Card>
  <Card title="Tools" icon="wrench" href="/cli/tools">
    Discover and manage available tools
  </Card>
  <Card title="Handoff" icon="arrows-turn-right" href="/cli/handoff">
    Enable agent-to-agent task delegation
  </Card>
  <Card title="Tracker" icon="crosshairs" href="/cli/tracker">
    Step-by-step execution tracking with quality judging
  </Card>
  <Card title="Auto Memory" icon="lightbulb" href="/cli/auto-memory">
    Automatic memory extraction and storage
  </Card>
  <Card title="Todo" icon="list-check" href="/cli/todo">
    Manage todo lists from tasks
  </Card>
  <Card title="Router" icon="route" href="/cli/router">
    Smart model selection based on task complexity
  </Card>
  <Card title="Flow Display" icon="diagram-project" href="/cli/flow-display">
    Visual workflow tracking
  </Card>
  <Card title="Query Rewrite" icon="pen-to-square" href="/cli/query-rewrite">
    RAG query optimization
  </Card>
  <Card title="Prompt Expansion" icon="expand" href="/cli/prompt-expansion">
    Expand prompts with detailed context
  </Card>
  <Card title="Prompt Caching" icon="floppy-disk" href="/cli/prompt-caching">
    Cache prompts for cost reduction
  </Card>
  <Card title="Web Search" icon="globe" href="/cli/web-search">
    Real-time web search integration
  </Card>
  <Card title="Web Fetch" icon="download" href="/cli/web-fetch">
    Fetch and process URL content
  </Card>
  <Card title="Docs" icon="file-lines" href="/cli/docs">
  ...
  <Card title="n8n Integration" icon="diagram-project" href="/cli/n8n">
    Import n8n workflows
  </Card>
  <Card title="Agent Skills" icon="brain-circuit" href="/features/skills">
...
| Flag | Description | Example |
|------|-------------|---------|
| `--tools`, `-t` | Comma-separated tool names **or** a path to a `tools.py` file. Names are resolved through the unified [ToolResolver](/features/tool-resolver). | `praisonai "task" --tools tavily_search,github` |
...
<Note>
`--tools` accepts either a comma-separated list of tool names or a `tools.py` file path. Names go through the same [ToolResolver](/features/tool-resolver) chain as YAML and Python, so a name unknown to every source prints `Warning: Unknown tool '<name>'` and is skipped. Loading a local `tools.py` file still requires `PRAISONAI_ALLOW_LOCAL_TOOLS=true`. Run `praisonai tools list` to see resolvable names.
</Note>
...
**Note:** As of PR #1713, `--deploy --schedule` invokes the real `DeployHandler` (previously a stub). Combine with `--provider {gcp,aws,azure}` and `--max-retries N`. See [Scheduler Deployment](/deploy/scheduler-deploy) for details.
...
| `usage` | Local token/cost spend reporting | `praisonai usage --by model` |
