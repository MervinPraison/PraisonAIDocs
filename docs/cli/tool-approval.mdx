...
[1] Yes, once  [2] Always for this session  ...  [5] Reject
> 5
Reason (optional): don't touch production.env — use staging.env instead
```

Leave the prompt blank to reject without a reason — the plain-denial behaviour is unchanged.
...
| Console backend | Already supported "deny **and** steer" | Rich/Textual now match |

The reason travels as `ApprovalResponse.reason` (a new `Optional[str]` field, defaulting to `None`, also serialised in `to_dict()`). Downstream, the denied tool's result is set from `ApprovalDecision.reason`, so the agent sees the correction as the tool's output — not just a log line.
