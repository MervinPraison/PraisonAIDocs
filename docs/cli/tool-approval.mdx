...
When you reject a tool call, PraisonAI can capture a one-line explanation and hand it back to the agent as the tool's result, so the next turn course-corrects instead of retrying blindly. On the terminal `ConsoleBackend`, pick `[d] deny & redirect` to open the `What should the agent do instead?` follow-up; the Rich prompt and Textual TUI ask for an optional reason after **Reject**.

```
Allow execute_command?  [o] once   [s] this session   [a] always (bash:git status *)   [n] no   [d] deny & redirect
Choice [o/s/a/n/d] (n): d
What should the agent do instead?: don't touch production.env — use staging.env instead
```

Leave the follow-up prompt blank to reject without guidance — the plain-denial behaviour is unchanged.

| Frontend | Reject prompt | Notes |
|----------|---------------|-------|
| Rich (default terminal) | Asks for an optional reason after choosing **Reject** | Reason threaded into the reject response |
| Textual (TUI) | Reject dialog takes an optional reason | Reason threaded into the reject response |
| Console backend | First-class `[d] deny & redirect` key that opens a second prompt | Deny **and** steer in one keystroke |

On the console path, the guidance travels as `ApprovalDecision.feedback` (a new `Optional[str]` field on `ApprovalDecision`, defaulting to `None`). When present, `tool_execution.py` rewrites the denied tool's `error_msg` to `"Tool '<name>' was denied by the user. Do not retry it as-is. User guidance: <feedback>"`, so the agent's next turn sees the correction as the tool's result — not just a log line. The Rich/Textual TUI still surfaces the same value through `ApprovalResponse.reason` and the `praisonai_code` wrapper (`InteractiveCLIApprovalBackend`) now sets `feedback` on the returned decision as well, keeping the two paths in sync.
...
This is identical to `--no-safe`. Both do two things:

- **Set env vars** — `PRAISON_APPROVAL_MODE=auto`, `PRAISONAI_TOOL_SAFETY=off` — consumed by the CLI's own tool wiring.
- **Register `AutoApproveBackend`** on the approval registry — consumed by the core `@require_approval` decorator that gates critical tools like `acp_execute_command`. Without this the flag was a no-op for any registry-decorated tool, so a scripted run with no TTY auto-denied every prompt while still exiting `0`.

A safe-mode run *removes* the `AutoApproveBackend` a prior `--no-safe` installed in the same process (REPL/worker/test), and removes **only** that backend — a caller-supplied backend or one installed by `--plan` is preserved. A per-agent backend still wins (the core consults `Agent(approval=…)` first), and `--plan` remains mutually exclusive with `--no-safe` / `--dangerously-skip-approval` (exits `1`).
...
Pre-declare allow/deny rules for CI-safe runs. See [Declarative Permissions](/features/declarative-permissions).
...
<Note>
Under `PRAISON_SHELL=sandboxed` or `PRAISON_SHELL=unsafe`, the real-shell path in `praisonai code` is wrapped in `require_approval(risk_level="critical")` under the same `execute_command` identity as the plain executor — so existing allowlists and `path_overlap` serialisation still apply. Denying raises `PermissionError` and no file is written. See [Shell Modes](/features/praison-shell).
</Note>
...
| `--plan` | OFF | Read-only — every mutating tool is denied. Discoverable alias for `--approval plan` (`PermissionMode.PLAN`). Cannot be combined with `--no-safe` / `--dangerously-skip-approval` (fails closed). |
| `--trust` | OFF | Auto-approve all tool executions. |
| `--approval <backend>` | — | Route approvals to a backend (`console`, `slack`, …). |

`--plan` is the opposite of `--dangerously-skip-approval` — it denies the mutating class of tools outright rather than approving them all. See [Permission Modes](/features/permission-modes).
...
|-------|---------|-------------------|
| `console` | Interactive terminal prompt (default) — see [Interactive Tool Approval](/features/interactive-approval) for the once/session/always scope choices | — |
...
<Note>
**Multi-agent safety for `[s] session` grants (since [MervinPraison/PraisonAI#4533](https://github.com/MervinPraison/PraisonAI/pull/4533))**: session-scoped approvals are keyed per Agent instance, so two same-named workers (including two unnamed `Agent()` objects, which both default to `name="Agent"`) never inherit each other's human-granted `[s] this session` decisions. Name-keyed features — YAML `--allow` / `--deny` rules, per-agent backend, risk level, and durable `[a] always` persistence — still use `agent_name` and are unchanged.
</Note>
