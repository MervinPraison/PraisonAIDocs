...
<Note>
Twelve options are declared by `praisonai chat` but not yet wired to the runtime: `--knowledge`, `--guardrails`, `--web`, `--reflection`, `--planning`, `--context`, `--execution`, `--hooks`, `--caching`, `--ui-backend`, `--no-color`, `--theme`. Passing any of them prints a one-line stderr warning and continues; the command still exits `0`. See PraisonAI PR [#4782](https://github.com/MervinPraison/PraisonAI/pull/4782).
</Note>
...
|---|---|---|
| `--knowledge` | Attach a knowledge store to the chat | Not implemented — see [Knowledge](/features/knowledge) for the SDK path. |
| `--guardrails` | Apply input/output guardrails | Not implemented — see [Guardrails](/features/guardrails) for the SDK path. |
| `--web` | Enable web search inside chat | Not implemented — use `--tools web_search` today. |
| `--reflection` | Enable reflection loop | Not implemented — see [Reflection](/features/reflection) for the SDK path. |
| `--planning` | Enable planning mode | Not implemented — see [Planning](/features/planning) for the SDK path. |
| `--context` | Extra context source | Not implemented — the sibling `--no-context` (env `PRAISON_NO_CONTEXT`) *is* honoured. |
| `--execution` | Execution-mode selector | Not implemented. |
| `--hooks` | Register hooks for the chat run | Not implemented — see [Hooks](/features/hooks) for the SDK path. |
| `--caching` | Enable prompt/response caching | Not implemented — see [Caching](/features/caching) for the SDK path. |
