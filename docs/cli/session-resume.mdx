...
The CLI restores your prior model, agent name, full chat history, and cumulative token usage, then shows:
...
│ Usage: 1,240 in / 3,980 out · $0.0140              │
╰─────────────────────────────────────────────────────╯
...
The `Usage:` line shows cumulative totals for the session. Subsequent runs with `--continue` or `--session <id>` keep accumulating into the same record — totals are never reset on resume.
