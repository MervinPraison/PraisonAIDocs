...
Resume a previous conversation exactly where you left off — [same model](#model-restoration), same agent, same chat history.
...
<Step title="Or skip the id — pick from a list">

On an interactive terminal, omit the id to open a picker of recent sessions:
...
#   Title           Updated               Messages   Tokens    Cost
1   fix-auth-bug    2026-09-22T14:31:07   12         12,440    $0.0710
2   draft-release   2026-09-22T11:02:44   5          3,120     $0.0180

Session number (0 to cancel):
```

Type a number to resume that session, or `0` to cancel. See [Interactive Picker](#interactive-picker).
...
<Step title="Give the session a friendly title (optional)">

Rename the session so it's easy to spot in `session list` — the id you resume with never changes:

```bash
praisonai session rename <session_id> "fix-auth-bug"
```

The title then surfaces in the `Name` column of `session list`. See [Session Rename](/features/session-rename) for details.
...
## Resume into the interactive TUI

The same `resume` primitive is available on the bare `praisonai` interactive launch — no need to type `/continue` inside a cold TUI. As of PR [#4943](https://github.com/MervinPraison/PraisonAI/pull/4943), the interactive TUI auto-persists every conversation, so this primitive now has data to resume against on a **fresh install** — you no longer need to have previously passed `--auto-save`:

```bash
# Most recent session
praisonai -c

# Specific session id (equivalent to praisonai session resume <id> without a follow-up prompt)
praisonai --session <id>

# Fork non-destructively
praisonai --fork --session <id>
```

The recorded session model is restored, the prior transcript is replayed into scrollback, and the primary agent is wired to the resumed session's store so your next prompt continues the conversation with full memory of prior turns.

Added in PR [#4912](https://github.com/MervinPraison/PraisonAI/pull/4912) as parity with `run --continue` / `chat --continue`. See [Interactive TUI → Launching with a resumed session](/cli/interactive-tui#launching-with-a-resumed-session).

---

## Interactive Picker

Omit the session id on an interactive terminal and the CLI opens a picker of recent sessions — no more copy-pasting an opaque id.

The picker fires only when **all** of these hold: a real TTY, no `--json`, and no id passed. It lists the same project-scoped sessions as `session list` (most-recently-updated first, up to 20).
...
Enter a number `1..N` to resume that session, or `0` to cancel. Ctrl-C / EOF also cancels.

```mermaid
graph TB
    Start([praisonai session resume]) --> Has{Session id<br/>passed?}
    Has -->|Yes| Rehydrate[📂 rehydrate_session]
    Has -->|No| Mode{TTY & not --json?}
    Mode -->|No| Err[❌ Missing session id]
    Mode -->|Yes| List[📋 list_project_sessions limit=20]
    List --> Empty{Rows?}
    Empty -->|None| NoS[ℹ️ No sessions found]
    Empty -->|>=1| Pick[🔢 Session number<br/>0 to cancel]
    Pick -->|0 / Ctrl-C| Cancel[ℹ️ Cancelled]
    Pick -->|1..N| Rehydrate
    ...
    class Start input
    class Has,Mode,Empty check
    class List,Pick action
    class Rehydrate ok
    class Err,Cancel,NoS err
```

### When it exits without resuming

The picker never silently starts a fresh run. Three cases exit without resuming:

| Case | Message |
|------|---------|
| Empty store | `No sessions found` |
| Cancel (`0`, Ctrl-C, EOF) | `Cancelled` |
| Listing failure | `Could not list sessions: <error>` + `→ Use 'praisonai session list' to inspect the session store` |

### Non-TTY / `--json`

Scripts and CI are unaffected — the id stays required. With no id under `--json` or a non-TTY, the command errors instead of prompting:

```
Error: Missing session id
  → Pass a session id, or run interactively to pick one
```

<Note>
Added in PR [#5214](https://github.com/MervinPraison/PraisonAI/pull/5214) (fixes [#5164](https://github.com/MervinPraison/PraisonAI/issues/5164)). The same picker backs [`run --pick`](/cli/run#pick-a-session-to-resume).
</Note>
...
Resuming restores the model the session was running with — not the current default. Change your default model between runs and a resumed session still runs on the original model.
...
graph TB
    ...
    R -->|No| D[Fall through to default]
...
When the recorded model is used, the CLI prints:

```
Restored session model: anthropic/claude-3-5-sonnet-latest
```

**Precedence** (first match wins): explicit `--model` → config `agent.model` → **recorded session model** → keyless local-first fallback → default resolution.

<Note>
The recorded-model lookup runs **before** the keyless local-first credential gate. A resumed cloud session on a machine with a reachable local endpoint (Ollama, LM Studio) is no longer silently switched to the local model. See [PraisonAI #3685](https://github.com/MervinPraison/PraisonAI/issues/3685).
</Note>
...
```bash
praisonai session resume abc123 --model gpt-4o "continue with a different model"
```
...
| Flag | Description |
|---|---|
| `<session_id>` | Session ID to restore — optional on a TTY (omit to open the [interactive picker](#interactive-picker)); required in `--json` / non-TTY |
...
<Note>
For the memory-driven resume path (different code path, uses `DefaultSessionStore` memory layers), see [Session Resume in Persistence](/guides/persistence/session-resume).
</Note>
...
<Accordion title="Use the interactive picker for exploratory resume">

Omit the id and let the [picker](#interactive-picker) surface recent sessions when you're browsing rather than resuming a known id. Scripts and CI should still pass an explicit id — the picker never fires under `--json` or a non-TTY.
...
<CardGroup cols={2}>
  <Card title="Session List" icon="list" href="/cli/session">
  ...
  <Card title="Memory-based Session Resume" icon="brain" href="/guides/persistence/session-resume">
