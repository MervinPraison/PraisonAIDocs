...
<Note>
The `praisonai code` coding agent has its own **OS-level** containment for shell commands — Seatbelt (macOS) or `bwrap` (Linux), driven by [`PRAISON_SHELL=sandboxed`](/features/praison-shell). It is the path that actually runs on macOS today, distinct from `SandboxConfig.native()` (Linux-only `sandlock`). See [OS Sandbox](/features/os-sandbox).
</Note>
...
![Sandbox Execution](./sandbox-execution-sandbox-executes-code-in-isola.gif)
...
- [Autonomy Modes](/cli/autonomy-modes) - Control AI autonomy
- [Git Integration](/cli/git-integration) - Safe code changes
