...
graph TB
    ...
    Verb -->|Yes| Jobs
    Verb -->|No| File{🔍 ends in .yaml/.yml?<br/>(existence not required)<br/>or otherwise an existing file?}
...
sequenceDiagram
    ...
    User->>CLI: praisonai run typo.yaml
    CLI->>Router: first token = "typo.yaml"
    Router->>Router: ends in .yaml? ✅
    Router->>Runner: hand off (existence check happens here)
    Runner-->>User: Error: file 'typo.yaml' not found
    Note over User,Runner: NOT the jobs API "invalid choice" / missing-job_id
...
| `praisonai run typo.yaml` (file absent) | Agent runner | Runner emits a clear file-not-found error — not the async-jobs `invalid choice` |
...
<Note>
Free-text prompts belong to the bare-prompt shape `praisonai "…"` (which the [CLI Dispatcher](/features/cli-dispatcher) rewrites to a modern `run` invocation), not to `praisonai run "…"`. Typing `run` explicitly with a non-file, non-verb token still falls through to the jobs argparse parser and errors with `invalid choice`.
</Note>

<Tip>
A `.yaml`/`.yml` target is routed to the agent runner **before** the existence check, so a typo or wrong-directory path (the trap in [PraisonAI issue #5095](https://github.com/MervinPraison/PraisonAI/issues/5095)) still gets a helpful "file not found" from the runner, not a confusing jobs-CLI error. Locked by [PR #5107](https://github.com/MervinPraison/PraisonAI/pull/5107).
</Tip>
...
- [Async Jobs Feature](/features/async-jobs)
- [Background Tasks CLI](/cli/background)
- [Scheduler CLI](/cli/scheduler)
- [Async Jobs SDK](/sdk/praisonai/async-jobs)
- [Jobs API Reference](/deploy/api/async-jobs/index)
