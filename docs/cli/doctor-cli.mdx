...
```bash
# Check packaging and entry point configuration
...
Useful for diagnosing Windows daemon issues where `python -m praisonai` and `praisonai` console script behave differently.
