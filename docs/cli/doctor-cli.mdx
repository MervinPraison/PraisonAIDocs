...
<Tip>
Multi-bot `bot.yaml` files produced by `praisonai onboard` (PR #1757) use custom channel keys like `telegram_cfo` and `telegram_ops` with `platform: telegram`. These are valid since [PR #1772](https://github.com/MervinPraison/PraisonAI/pull/1772) — older releases will reject them with `Unknown channel '<key>'`. Upgrade if you see this error.
</Tip>
