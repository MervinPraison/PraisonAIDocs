...
<Info>
**Install requirement:** the wrapper-delegating `praisonai skills` subcommands (`list`, `validate`, `create`, `add`) require `pip install praisonai` (the full wrapper). On a standalone `pip install praisonai-code` install they exit `1` with `skills requires the full wrapper. Install the full wrapper: pip install praisonai` — a single-line hint, no Rich traceback ([PR #2854](https://github.com/MervinPraison/PraisonAI/pull/2854)). Several subcommands remain standalone-safe (see the Tip below).
</Info>

<Tip>
The `skills bundle`, `skills check`, and `skills eligible` subcommands are **native** and work standalone (`pip install praisonai-code`) — they run in-process against `praisonaiagents.skills`. The wrapper-delegating subcommands (`list`, `validate`, `create`, `add`) require `pip install praisonai`.
</Tip>
...
### Add or Install a Skill

Install a skill from a local path or a git URL. `add` is a familiar alias — it and `install` do the same thing.

A real run: pick a repo (`https://github.com/acme/team-skills`), run `praisonai skills add <url>`, and the CLI shallow-clones it and copies the skill into `~/.praisonai/skills/` (or `--dest`). On success it prints `Installed: <path>`; a bad source exits with code `1`. Every agent that discovers `~/.praisonai/skills/` picks the skill up on its next run — no code change.

```bash
# Add from a git URL (mirrors `npx skills add <url>`)
praisonai skills add https://github.com/acme/team-skills

# Add from a local path
praisonai skills add ./my-skill

# Choose a destination (default: ~/.praisonai/skills)
praisonai skills add https://github.com/acme/team-skills --dest ./team-skills

# `install` is the original spelling — identical behaviour
praisonai skills install ./my-skill
```
...
|------|------|---------|-------------|
| `source` (positional) | `str` | *(required)* | Local path or `https://` git URL of the skill. |
| `--dest`, `-d` | `str` | `~/.praisonai/skills` | Install destination. |

<Note>
`add` and `install` are the same command — pick whichever verb reads more naturally. Use [`praisonai skills sync`](/features/skills-remote-sources) instead when you want a **versioned, refreshable cache** of a shared team repo.
</Note>

```mermaid
graph TB
    Q{How do you want<br/>to consume the skill?} -->|One-off copy, familiar verb| A[praisonai skills add URL]
    Q -->|One-off copy, original verb| B[praisonai skills install URL]
    Q -->|Versioned cache, team repo| C[praisonai skills sync URL]
