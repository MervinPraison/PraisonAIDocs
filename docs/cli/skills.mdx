...
<Note>
`add` and `install` are the same command — pick whichever verb reads more naturally. Use [`praisonai skills sync`](/docs/features/skills-remote-sources) instead when you want a **versioned, refreshable cache** of a shared team repo.
</Note>
