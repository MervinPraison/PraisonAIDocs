...
# Create agent with skills - tools are auto-added
agent = Agent(
    name="Data Analyst",
...
# Run analysis - agent automatically:
# 1. Reads the skill's SKILL.md for instructions
# 2. Uses read_file to read the CSV data
# 3. Can run scripts from the skill's scripts/ directory
print(agent.run("Analyze the data in ./data.csv using the csv-analyzer skill."))
```
...
3. **System Prompt**: Skills are injected into the system prompt with their descriptions and locations
4. **Progressive Disclosure**: Agent reads SKILL.md only when the skill is relevant to the task
5. **Script Execution**: Agent runs scripts from `scripts/` directory using the modular `skill_tools` module
