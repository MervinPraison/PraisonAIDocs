...
praisonai registry serve --port 7777 --read-only
```
...
praisonai registry serve --dir /path/to/registry
...
When running `praisonai registry serve`, the following endpoints are available:
