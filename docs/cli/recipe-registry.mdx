...
List every recipe in the local registry. This is registry-scoped and differs from `praisonai recipe list` — `recipe list` reads per-recipe metadata, while `registry list` enumerates the full registry with tag filtering.

```bash
praisonai registry list [options]
```
...
| `--tags <a,b>` | comma-separated | `None` | Filter recipes by tags (any-match) |
...
<Warning>
`praisonai registry serve` is **deprecated** and will be removed in a future version. Use `praisonai serve registry` instead — both accept the same flags and default to port `7777`. See the [Recipe Registry Server](/deploy/recipe-registry-server) guide for the canonical form. The deprecated form prints this warning to `stderr`:
...
To enumerate recipes in the local registry, use the [`list`](#list) command above.
...
- [Run History](/cli/recipe-runs) - Store and export run history
- [Security Features](/cli/recipe-security) - SBOM, signing, auditing
- [Policy Packs](/cli/recipe-policy) - Manage tool permissions
