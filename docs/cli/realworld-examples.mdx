...
| `-c, --continue` | Continue last session (re-roots tools into the resumed session's directory) |
| `-s, --session` | Resume specific session ID (re-roots tools into the resumed session's directory) |
...
```mermaid
graph LR
    subgraph "praisonai code on Windows"
        A[👤 User Prompt] --> B{🪟 Windows?}
        B -->|Yes| C[➕ add --no-acp<br/>set PYTHONIOENCODING=utf-8]
        B -->|No|  D[Run as normal]
        C --> E[🤖 Agent edits files<br/>runs tests, fixes bugs]
        D --> E
        E --> F[✅ Green tests]
    end
...
```powershell
# Windows PowerShell — run praisonai code non-interactively
$env:PRAISON_APPROVAL_MODE = "auto"
$env:PYTHONIOENCODING       = "utf-8"
$env:OPENAI_API_KEY         = "${OPENAI_API_KEY}"

praisonai code -w . --no-acp "Implement celsius_to_fahrenheit in src/converter.py and run pytest until it passes."
```
...
```cmd
:: Windows cmd.exe
set PRAISON_APPROVAL_MODE=auto
...
```bash
praisonai code -w . --no-acp "your prompt"
```
