...
<Tip>
Prefer `praisonai agent create` for a single, focused agent; use [`praisonai init`](/cli/init) when you also want starter commands and tools next to it. See [Custom Agents & Commands](/features/custom-agents-commands) for the full definition format.
</Tip>
...
See [`praisonai run`](/cli/run) for the full runtime surface.
...
<Tip>
No agents yet? Run [`praisonai agent create`](#create) for the shortest path to a single custom agent, or [`praisonai init`](/cli/init) to scaffold a full starter project in `.praisonai/`.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Presets & Modes" icon="sliders" href="/features/agent-presets-and-modes">
