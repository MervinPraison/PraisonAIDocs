...
Every runtime advertises its protocol version, and the CLI checks major-version compatibility before forwarding any work to it.

**Compatibility rule:** major-version numbers must match. For example, `v2.3.4` ↔ `v2.9.0` ✅ (same major); `v2.x` ↔ `v3.0.0` ❌ (different major). A runtime or client that reports no version is treated as incompatible.

```mermaid
graph TB
    A[Read descriptor / lockfile] --> B{Descriptor present?}
    B -->|No| C[In-process run / no attach]
    B -->|Yes| D{Version compatible?}
    D -->|Yes| E[Forward to warm runtime]
    D -->|No| F{Which command?}
    F -->|daemon start| G[Print incompatibility error
Exit 1]
    F -->|run| H[Fall back to in-process]
    F -->|attach| I[Print error
Exit 1]
...
| Command | Compatible runtime | Incompatible runtime | No runtime |
|---|---|---|---|
| `daemon start` | "Runtime already running" (exit 0) | Prints incompatibility error, exits 1 | Starts new runtime |
| `praisonai run` | Forwards to warm runtime | Falls back to in-process (silent) | Falls back to in-process |
| `praisonai attach` | Subscribes to event stream | "No compatible warm runtime" (exit 1) | "No compatible warm runtime" (exit 1) |

**`daemon start` incompatibility error:**

If a runtime is already running but speaks an incompatible version, `daemon start` prints:

```
A runtime is running at http://127.0.0.1:49201 (pid 12345) but speaks an incompatible version (v1). Stop it first with: praisonai daemon stop
```

and exits with code `1`. Fix it with:
...
```
Warm runtime running at http://127.0.0.1:49201 (pid 12345, v2).
```

When the running runtime is version-incompatible with the current client:

```
Warm runtime running at http://127.0.0.1:49201 (pid 12345, v1) [version mismatch].
```
...
```json
{"running": false, "version": null, "compatible": false}
```
...
<CardGroup cols={2}>
<Card title="Attach" icon="play" href="/docs/features/attach">
  Stream live events from a session running on the warm runtime
</Card>
<Card title="Run" icon="play" href="/docs/cli/run">
  The `--attach` flag registers a session observable via `praisonai attach`
</Card>
