...
## Project rules and local tools

As of PR [#4943](https://github.com/MervinPraison/PraisonAI/pull/4943), the warm runtime honors `AGENTS.md` / `CLAUDE.md` subtree instructions and loads project-local `.praisonai/tools/*.py` on **every** run — so the daemon now behaves identically to the cold path for these two implicit, project-scoped sources.

Previously the warm runtime built a bare agent with no subtree instruction hook and no local tools, so a daemon-forwarded `praisonai run` returned fluent text and exited `0` while touching nothing — indistinguishable from success until you checked the filesystem.

```bash
# Cold path (already worked)
praisonai run "list the todos"     # honored ./AGENTS.md and ./.praisonai/tools/

# Warm path (now equally honored, from PR #4943)
praisonai daemon start
praisonai run "list the todos"     # same rules, same tools, same result
```

<Note>
Explicit runs that pass `--tools` / `--toolset` / `--mcp` / `--instructions` still fall back to in-process execution (see the [fallback table](#when-run-falls-back-to-in-process)) — PR #4943 only closes the two **implicit** sources that were being silently dropped. Wiring is best-effort: it never stops the daemon from serving a turn.
</Note>
...
See [Attach](/cli/attach) for streaming live session events from a second terminal.
...
<Card title="MCP" icon="plug" href="/mcp/mcp-server">
  MCP handshake is the biggest cold-start cost that the daemon eliminates
</Card>
<Card title="Session Resume" icon="rotate" href="/cli/session-resume">
...
<Card title="Attach" icon="play" href="/cli/attach">
