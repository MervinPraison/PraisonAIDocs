...
Detected env vars (any one satisfies the check): `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `GEMINI_API_KEY`, `GROQ_API_KEY`, `COHERE_API_KEY`. Stored credentials from `praisonai setup` are also accepted. See [First-run Onboarding](/features/first-run-onboarding) for details.
...
├── praisonai chat           # TUI mode (wrapper-required)
├── praisonai chat                  # Single prompt chat mode (wrapper-required)
│
...
| `praisonai chat` / `-i` | flag | Interactive TUI mode (wrapper-required) |
| `praisonai chat` | flag | Single prompt chat mode (wrapper-required) |

<Note>
On a standalone `pip install praisonai-code` install, `chat` and `code` require the `praisonai` wrapper and emit an install hint (`Install with: pip install praisonai`). Default `run "…"` and every `run --output …` mode now route through the in-process `Agent` (PR #2853). See the [standalone-limits table](/docs/features/praisonai-code-cli#standalone-limits).
</Note>
