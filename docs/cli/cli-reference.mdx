...
```
praisonai
├── [direct prompt]              # Any text → runs agent (wrapper-required on standalone praisonai-code)
...
<Note>
On a standalone `pip install praisonai-code` install, `chat`, `code`, and default `run "…"` require the `praisonai` wrapper and emit an install hint (`Install with: pip install praisonai`). `run --output actions|json|stream|stream-json "…"` are the standalone-safe paths. See the [standalone-limits table](/docs/features/praisonai-code-cli#standalone-limits).
</Note>
