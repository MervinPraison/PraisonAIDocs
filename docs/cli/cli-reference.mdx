...
Detected env vars (any one satisfies the check): any credential key in `PROVIDER_ENV_CATALOGUE` — `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `GEMINI_API_KEY`, `GROQ_API_KEY`, `COHERE_API_KEY`, `OPENROUTER_API_KEY`, `OLLAMA_HOST`, `MISTRAL_API_KEY`, `DEEPSEEK_API_KEY`, `XAI_API_KEY`, `TOGETHER_API_KEY` / `TOGETHERAI_API_KEY`, `PERPLEXITYAI_API_KEY`, `FIREWORKS_API_KEY` / `FIREWORKS_AI_API_KEY` (full [14-row table](/cli/setup#credential--default-model-map)). Stored credentials from `praisonai setup` are also accepted. See [First-run Onboarding](/features/first-run-onboarding) for details.

---

## Top-level options

Accepted on the bare `praisonai` invocation (no subcommand):

| Option | Short | Description |
|--------|-------|-------------|
| `--version` | `-V` | Print version and exit. |
| `--output-format` | `-o` | `text` / `json` / `actions`. |
| `--json` | | Shortcut for `--output-format json`. |
| `--no-color` | | Disable ANSI colours. |
| `--quiet` | `-q` | Suppress non-essential output. |
| `--verbose` | `-v` | Verbose logging. |
| `--screen-reader` | | Screen-reader-friendly output. |
| `--observe` | `-O` | Enable observability (langfuse, langextract). |
| `--continue` | `-c` | Resume the most recent session into the interactive launch (PR [#4912](https://github.com/MervinPraison/PraisonAI/pull/4912)). |
| `--session` | `-s` | Resume a specific session id into the interactive launch (PR [#4912](https://github.com/MervinPraison/PraisonAI/pull/4912)). |
| `--fork` | | Fork the resumed session non-destructively (requires `--continue` or `--session`) (PR [#4912](https://github.com/MervinPraison/PraisonAI/pull/4912)). |

`--continue` / `--session` / `--fork` on the bare invocation only apply when no subcommand is given; a subcommand's own flags always win. See [Interactive TUI → Launching with a resumed session](/cli/interactive-tui#launching-with-a-resumed-session).

## Command Tree

```
praisonai
├── [direct prompt]              # Any text → runs agent (in-process on standalone as of PR #2853)
├── (bare, no subcommand)        # Interactive TUI; accepts -c/--continue, -s/--session <id>, --fork (PR #4912)
├── [file.yaml]                  # YAML workflow execution
├── praisonai chat           # TUI mode (resident standalone; wrapper adds richer dispatch)
├── praisonai chat                  # Single prompt chat mode (resident standalone; wrapper adds richer dispatch)
│
...
│   ├── show                     # Show current memory (--limit N, --user-id ID)
│   ├── list                     # Alias for show (--limit N, --user-id ID)
...
│
├── usage                        # Local token/cost usage reporting
│   └── --days/-d --by/-b --project/-p --json
│
...
│   ├── create [name]            # Author a new custom agent (interactive or --yes)
...
│
├── backends                     # List registered external coding-CLI backends
│   └── list                     # Explicit form (same as bare `backends`)
│
└── [Capabilities - LiteLLM parity] (27 commands)     # route to legacy handlers (except ⚠ verbs)
    ...
    ├── vector-stores            # Vector store management  ⚠ still billed to LLM pending #4322
    ├── rerank                   # Reranking
    ├── ocr                      # OCR
    ...
    ├── a2a                      # Agent-to-Agent
    ├── containers               # Container management  ⚠ still billed to LLM pending #4322
...
<Note>
All 27 capabilities in the LiteLLM-parity block route to their legacy handlers as of PR [#4329](https://github.com/MervinPraison/PraisonAI/pull/4329), **except** `containers` and `vector-stores`, which are deliberately held back until [#4322](https://github.com/MervinPraison/PraisonAI/issues/4322) lands. Those two currently reach the bare-prompt path and are still forwarded to an LLM.
</Note>

<Note>
`praisonai agent create` authors a new `.praisonai/agents/<name>.md` definition interactively or with `--yes`. See [Create Custom Agents](/features/agent-create) for the full walkthrough.
</Note>
...
```bash
praisonai "summarise this folder"   # bare prompt → Typer run (modern engine)
praisonai show                      # bare token → Typer run, prompt "show"
praisonai memoyr                    # close typo of memory → exit 2, Did you mean: memory?
praisonai agents.yaml               # flagless .yaml → Typer run (modern engine)
praisonai memory show               # known subcommand → routed normally

praisonai show --framework crewai   # flag → legacy; ⚠ reserved-verb hint, exit 2
```

<Note>
Two typo guards apply. The **mistyped-verb guard** on the modern dispatcher blocks a lone, non-prefix close typo of a real command (`praisonai deploi`, `memoyr`) with exit 2 and a `Did you mean …?` suggestion, on any invocation. The **reserved-verb guard** on the legacy path fires only once a legacy-only flag has already forced legacy. A plain `praisonai show` is *not* a close match, so it routes to Typer `run` and is sent to the model as the literal prompt `show`. See [CLI Dispatcher → Two typo guards](/features/cli-dispatcher#two-typo-guards).
</Note>

<Note>
A flagless `.yaml`/`.yml` positional reaches the **modern `run` engine** — `praisonai agents.yaml` is equivalent to `praisonai run agents.yaml`, inheriting session continuity, `--output` modes, the credential gate and permissions. Global flags in front of it are hoisted ahead of `run` (`praisonai --output-format json agents.yaml` → `--output-format json run agents.yaml`). Only a genuinely legacy-only flag (`--auto`, `--serve`, `--n8n`, `-s`, `-f`) keeps it on the legacy roles-file dispatcher. See [CLI Dispatcher](/features/cli-dispatcher).
</Note>
...
|------|-------|------|-------------|
| `--output-format` | `-o` | choice | `text` \| `json` \| … output format; env `PRAISONAI_OUTPUT_FORMAT` |
| `--json` | — | flag | Alias for `--output-format json` |
| `--no-color` | — | flag | Disable colored output; env `NO_COLOR` |
| `--quiet` | `-q` | flag | Minimal output |
| `--screen-reader` | — | flag | Screen-reader friendly output (no spinners/panels) |
| `--observe` | `-O` | choice | Enable observability (`langfuse`, `langextract`); env `PRAISONAI_OBSERVE` |
...
| `--init` | remainder | Initialize `agents.yaml` from a task description. Prints provider-setup guidance and exits if no LLM provider is configured (see [`praisonai setup`](/cli/setup)). |
...
| `--external-agent` | str | External CLI tool — choices are dynamically populated from the registry (built-ins claude/gemini/codex/cursor plus any entry-point plugins), like `--cli-backend`. Uses manager-Agent delegation by default. See [Register a custom external agent](/features/external-cli-integrations#register-a-custom-external-agent) |
...
| `--append-system-prompt` | str | Append text (literal or `@file`) to the system prompt for this invocation only (`code` / `chat` / `run`). Env fallback: `PRAISONAI_APPEND_SYSTEM_PROMPT`. Never persisted. See [Append System Prompt](/features/append-system-prompt). |
...
| `praisonai chat` / `-i` | flag | Interactive TUI mode (resident on `praisonai-code`; wrapper adds richer dispatch) |
| `praisonai chat` | flag | Single prompt chat mode (resident on `praisonai-code`; wrapper adds richer dispatch) |

<Note>
As of [PR #3818](https://github.com/MervinPraison/PraisonAI/pull/3818), `chat` and `code` open their resident interactive TUI on a standalone `pip install praisonai-code` — no wrapper required. Default `run "…"` and every `run --output …` mode also route through the in-process `Agent` (PR #2853). The wrapper (`pip install praisonai`) adds the richer legacy dispatch for `code`, gateway, and channel bots. See the [standalone-limits table](/docs/features/praisonai-code-cli#standalone-limits).
</Note>

<Note>
Fourteen stub command groups — `agents`, `workflow`, `registry`, `memory`, `skills`, `hooks`, `rules`, `eval`, `package`, `templates`, `todo`, `research`, `commit`, and `call` — delegate to the wrapper for their legacy entry points. On a standalone install those entry points fail fast with a single-line install hint (`<group> requires the full wrapper. Install the full wrapper: pip install praisonai`) and `exit 1` as of [PR #2854](https://github.com/MervinPraison/PraisonAI/pull/2854) — no more Rich `ImportError` traceback. Native subcommands stay standalone-safe: `memory learn`, `skills bundle/check/eligible`, `eval list-judges`, `eval list`. See the [mixed-group table](/docs/features/praisonai-code-cli#standalone-limits).
</Note>

**Note on `--tools`:** Comma-separated tool names (e.g., `--tools tavily_search,my_tool`) are now resolved via the unified [ToolResolver](/features/tool-resolver), so any tool reachable from YAML is also reachable from the CLI.
...
| **Integrations** | `integrations/` | Built-in: Claude Code, Gemini CLI, Codex CLI, Cursor CLI — plus any registered `praisonai.external_agents` plugins | `--external-agent` |
...
# Memory management
praisonai memory show                # 10 most recent
praisonai memory list --limit 25     # alias for show
...
`praisonai session resume <id>` restores full conversational state — chat history, model, and agent name — for a prior session. See the dedicated [Session Resume](/cli/session-resume) page for full details.
...
`praisonai usage` reports aggregate token and cost totals from the local session store — no network, no configuration. See the dedicated [Usage CLI](/cli/usage-cli) page for options and JSON output.
...
- [Usage CLI](/cli/usage-cli) - Local token/cost usage reporting

- [CLI Commands](/cli/cli) - Detailed CLI documentation
- [Session Resume](/cli/session-resume) - Deterministic CLI session resume
- [Doctor CLI](/cli/doctor) - Health checks and diagnostics
- [Workflows](/features/workflows) - Workflow management
- [Flow CLI](/cli/flow) - Visual workflow builder (Langflow)
- [Memory](/concepts/memory) - Memory and sessions
