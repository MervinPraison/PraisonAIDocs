...
<Note>
`praisonai agent create` authors a new `.praisonai/agents/<name>.md` definition interactively or with `--yes`. See [Create Custom Agents](/docs/features/agent-create) for the full walkthrough.
</Note>

### Direct Prompt Examples

The `[direct prompt]` entry in the tree above means a bare positional with no flags that isn't a `.yaml`/`.yml` name routes to Typer `run` — the same modern engine as `praisonai run "<prompt>"`:
...
`praisonai usage` reports aggregate token and cost totals from the local session store — no network, no configuration. See the dedicated [Usage CLI](/docs/cli/usage-cli) page for options and JSON output.
...
- [Usage CLI](/docs/cli/usage-cli) - Local token/cost usage reporting
