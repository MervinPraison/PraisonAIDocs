...
praisonai show                      # ⚠ reserved verb → prints hint on stderr, exits 2
praisonai memoyr                    # ⚠ typo of a known command → "Did you mean: memory?"
praisonai run "hello"               # ✅ single-word prompt → use `run` to bypass the guard
```
