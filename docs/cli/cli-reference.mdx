...
<Note>
As of [PR #3818](https://github.com/MervinPraison/PraisonAI/pull/3818), `chat` and `code` open their resident interactive TUI on a standalone `pip install praisonai-code` — no wrapper and no install hint on default `chat` / `code`. Default `run "…"` and every `run --output …` mode also route through the in-process `Agent` (PR #2853). The wrapper adds only the richer legacy dispatch for `code`. See the [standalone-limits table](/docs/features/praisonai-code-cli#standalone-limits).
</Note>
