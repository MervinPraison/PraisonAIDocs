...
| `--append-system-prompt` | str | Append literal text (or `@file`) to the system prompt for this invocation only (shared by `code`, `chat`, `run`). Env fallback: `PRAISONAI_APPEND_SYSTEM_PROMPT`. See [Append System Prompt](/features/append-system-prompt). |
