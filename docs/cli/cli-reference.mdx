...
praisonai memoyr                    # bare token → Typer run, prompt "memoyr"
praisonai agents.yaml               # ends in .yaml → legacy agent file
...
<Note>
The reserved-verb / typo guard (`praisonai show`, `memoyr`) fires on the **legacy path only** — reached when the invocation carries a flag or points at a `.yaml`/`.yml` file. A plain `praisonai show` now routes to Typer `run` and is sent to the model as the literal prompt `show`.
</Note>

<Note>
A `.yaml`/`.yml` positional **stays on the legacy roles-file dispatcher** — that is the whole point of the positional form. `praisonai agents.yaml` runs the roles-file loader; `praisonai run "<prompt>"` does not. See [YAML Reference → Running a Roles-File YAML](/docs/features/yaml-configuration-reference#running-a-roles-file-yaml).
</Note>
