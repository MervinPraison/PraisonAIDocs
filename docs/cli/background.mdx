...
/tasks <id>            # detail view (incl. result / error)
/tasks cancel <id>     # cancel a running task
```

In bot chats `/tasks` is per-user scoped — a caller only sees and cancels tasks whose `metadata["user_id"]` matches. See [Bot Chat Commands → /tasks](/features/bot-commands#tasks).

## See Also

- [Background Tasks Feature](/features/background-tasks)
- [Bot Chat Commands](/features/bot-commands#tasks) - Inspect and cancel tasks from chat with `/tasks`
- [Async Jobs CLI](/cli/async-jobs)
- [Scheduler CLI](/cli/scheduler)
- [Background Tasks SDK](/sdk/praisonai/background-tasks)
