...
<Note>
This page is about `Memory` operation hooks (`beforeStore`/`afterStore`/etc.). It is not the same as the standalone [Hooks Manager](/js/hooks-manager), whose events fire only when you call them yourself.
</Note>

```mermaid
graph LR
    Mem[Memory] --> Hooks[Memory Hooks]
...
```typescript
const hooks = createMemoryHooks({
  beforeStore: async (key, value, metadata) => ({ key, value, metadata }),
  verbose: true,
});
```
...
<CardGroup cols={2}>
  ...
  <Card title="Sessions" icon="lock" href="/js/sessions">
    Session management
  </Card>
