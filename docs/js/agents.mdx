...
// Simple array syntax
const agents = new AgentManager([agent1, agent2]);
await agents.start();
```
...
const pipeline = new AgentManager([researcher, analyst, writer]);
...
const agents = new AgentManager([researcher, analyst]);
await agents.start();
```
...
```typescript
const agents = new AgentManager([agent1, agent2, agent3]);
const results = await agents.start();
...
const a1 = new AgentManager([agent1, agent2]);
const a2 = new AgentManager({ agents: [agent1, agent2] });
```
