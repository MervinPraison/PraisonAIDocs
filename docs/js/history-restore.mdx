...
<Note>
The `praisonai-mobile` app uses `setHistory` **automatically on every turn**, restoring the prior conversation from the mobile session store bounded to `HISTORY_CHAR_BUDGET` (24,000 characters, ~6,000 tokens). See [Mobile → History & Reopen](/features/mobile/history-and-reopen#the-model-actually-gets-the-conversation). Callers of `Agent.setHistory` directly are responsible for their own truncation — the mobile helper (`truncateHistory` in `praisonai-mobile/core`) is not part of the public JS SDK surface.
</Note>
