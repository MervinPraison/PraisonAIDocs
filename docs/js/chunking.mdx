...
```typescript
import { Knowledge, Chunking } from 'praisonai';

const kb = new Knowledge();
const chunker = new Chunking({ strategy: 'size', chunkSize: 512, overlap: 50 });

const document = 'Your long document text...';

for (const chunk of chunker.chunk(document)) {
  await kb.store(chunk.content, {
    userId: 'u1',
...
<Note>`kb.add('doc.pdf')` already chunks internally using the `chunker` block of `KnowledgeStoreConfig`. Hand-chunk and `store()` only when you want to control the chunks yourself.</Note>

## Related

<CardGroup cols={2}>
  <Card title="Chunking CLI" icon="terminal" href="/js/chunking-cli">
    CLI knowledge commands
  </Card>
  <Card title="Knowledge Base" icon="book" href="/js/knowledge-base">
    Store chunked documents
  </Card>
  <Card title="RAG Agent" icon="robot" href="/js/rag-agent">
    Retrieval workflows
  </Card>
</CardGroup>
