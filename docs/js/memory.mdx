...
<Note>
The sync `embed(...)` / `embeddings(...)` functions now throw instead of returning noise — see [Sync vs. Async](/js/embeddings#sync-vs-async) for the async replacement.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Vector Stores" icon="database" href="/js/vector-stores">
