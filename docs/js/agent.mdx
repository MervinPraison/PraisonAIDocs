...
<Note>
Some options on `Agent.__init__` and `Agent.chat` are accepted for Python-SDK parity but not yet acted on in the TypeScript SDK. If you pass one and see a `[praisonai] … not yet honoured` notice, see the [Parity Notices](/docs/features/typescript-parity-notices) page for the full list.
</Note>
