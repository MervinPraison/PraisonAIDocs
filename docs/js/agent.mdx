...
<Note>
As of PR #4841 every `Agent` constructor and `Agent.chat` option is honoured — see [Agent Options](#agent-options) below. Two remain **partial**: `attachments` and `toolConfig.parallel` work on the OpenAI-compatible backend and emit a notice on the AI SDK backend. See [Parity Notices](/docs/features/typescript-parity-notices) for the ledger.
</Note>
...
| `toolsRunOn` | `string \| ToolPlaceLike` | A tool place runs the tools; the loop stays local. Throws on an unknown name. |
