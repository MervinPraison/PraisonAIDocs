...
```text
[praisonai] tool '<toolName>' has no command mapping for place '<placeName>',
so it runs on the HOST, not in the sandbox.
Give it a shell command (place.setCommand('<toolName>', '...')) to run it there.
```
...
The team's place also reaches an agent introduced on a **task** (`TeamTask.agent`) or built lazily from **`agentConfig`** — an agent that never appeared in the `agents` array at construction. It joins the same shared sandbox on first use.

```typescript
import { Agent, AgentTeam } from 'praisonai';

const member = new Agent({ instructions: 'Collect data' });
const team   = new AgentTeam({ agents: [member], toolsRunOn: 'local' });

// A second agent that lives on a task — never in the team's agents array
const taskOnly = new Agent({ instructions: 'Also runs' });
team.addTask({ agent: taskOnly, prompt: 'do it' });

// On first use, taskOnly joins the SAME sandbox as member
taskOnly.getToolPlace() === member.getToolPlace();   // true
```

Under the hood, the team calls `Agent.adoptToolPlace(place)` on each member. It returns `false` when the agent already has its own place (its own `toolsRunOn` or one adopted earlier — a safe no-op), and `true` when adopted. The call is idempotent, so a member that already has a place is never overridden. See [AgentTeam](/js/agent-team) for the full team-level option list.
