...
Redis gives Agents fast caching, sessions, and real-time messaging.

```mermaid
graph LR
    Agent[Agent] --> Redis[Redis]
...
if (!cached) await redis.set('key', await agent.chat('Hello'), 3600);
```
...
<CardGroup cols={2}>
  ...
  <Card title="PostgreSQL" icon="database" href="/js/postgres">
    Persistent storage
  </Card>
</CardGroup>
