...
const auto = new AutoAgentManager();
...
const sequential = new AutoAgentManager({ pattern: 'sequential' });

// Parallel - tasks run simultaneously
const parallel = new AutoAgentManager({ pattern: 'parallel' });

// Routing - tasks routed based on conditions
const routing = new AutoAgentManager({ pattern: 'routing' });

// Orchestrator-workers - one agent coordinates others
const orchestrator = new AutoAgentManager({ pattern: 'orchestrator-workers' });

// Evaluator-optimizer - iterative improvement
const evaluator = new AutoAgentManager({ pattern: 'evaluator-optimizer' });
```
...
const auto = new AutoAgentManager({ singleAgent: true });
...
const auto = createAutoAgentManager({
  pattern: 'sequential',
  verbose: true
});
```
