...
<Warning>
`reflection` is accepted on a `Task` for Python-SDK parity but is **not yet implemented** in the TypeScript SDK — passing it emits a `[praisonai] … not yet honoured` notice and has no effect. This feature is Python-only for now.
</Warning>
...
<Card title="ReflectionConfig" icon="code" href="/sdk/reference/typescript/classes/ReflectionConfig">
