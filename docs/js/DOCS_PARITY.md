# Documentation Parity Tracker (TypeScript/JavaScript)

> **Categories:** 76 | **Documented:** 75 | **Parity:** 98.7%

This report compares **TypeScript/JavaScript SDK feature categories** against **TypeScript/JavaScript documentation** (docs/js/).

## Summary

| Metric | Count |
|--------|-------|
| Feature Categories | 76 |
| **Documented Categories** | **75** |
| **Undocumented Categories** | **1** |
| **Parity** | **98.7%** |

## Documented Categories

| Category | Features | Docs | Lines |
|----------|----------|------|-------|
| ✅ AGUI | 1 | 1 | 195 |
| ✅ AI SDK | 38 | 8 | 1143 |
| ✅ Agent | 69 | 7 | 2338 |
| ✅ Agent-to-Agent (A2A) | 15 | 1 | 342 |
| ✅ Approval | 9 | 1 | 281 |
| ✅ Audio | 8 | 1 | 162 |
| ✅ Auto Generation | 14 | 3 | 620 |
| ✅ Autonomy | 5 | 1 | 164 |
| ✅ Bots | 5 | 1 | 184 |
| ✅ Budget | 2 | 1 | 197 |
| ✅ CLI | 2 | 1 | 159 |
| ✅ Caching | 2 | 1 | 249 |
| ✅ Chunking | 4 | 2 | 299 |
| ✅ Citations | 3 | 1 | 156 |
| ✅ Code Execution | 7 | 4 | 464 |
| ✅ Conditions | 1 | 1 | 372 |
| ✅ Configuration | 3 | 1 | 272 |
| ✅ Context Management | 32 | 2 | 345 |
| ✅ Criteria | 1 | 1 | 162 |
| ✅ Database | 14 | 2 | 469 |
| ✅ Display | 20 | 1 | 342 |
| ✅ Embeddings | 12 | 3 | 813 |
| ✅ Evaluation | 8 | 4 | 866 |
| ✅ Events | 4 | 1 | 161 |
| ✅ Execution | 4 | 1 | 159 |
| ✅ Failover | 2 | 1 | 160 |
| ✅ Files | 11 | 1 | 151 |
| ✅ Flow | 4 | 1 | 164 |
| ✅ Gateway | 6 | 1 | 401 |
| ✅ Guardrails | 16 | 3 | 744 |
| ✅ Handoffs | 12 | 1 | 179 |
| ✅ Hooks | 8 | 1 | 203 |
| ✅ Image | 4 | 2 | 282 |
| ✅ Jobs | 8 | 1 | 156 |
| ✅ Knowledge | 5 | 2 | 312 |
| ✅ LLM | 10 | 2 | 279 |
| ✅ Loops | 11 | 1 | 180 |
| ✅ MCP | 20 | 4 | 870 |
| ✅ Memory | 19 | 4 | 783 |
| ✅ Middleware | 2 | 1 | 162 |
| ✅ OCR | 2 | 1 | 162 |
| ✅ Observability | 6 | 28 | 2318 |
| ✅ Optimizer | 3 | 1 | 162 |
| ✅ Output | 3 | 1 | 220 |
| ✅ Parallel Execution | 3 | 2 | 256 |
| ✅ Planning | 11 | 2 | 525 |
| ✅ Plugins | 9 | 2 | 710 |
| ✅ Prompts | 4 | 2 | 262 |
| ✅ Providers | 25 | 116 | 8145 |
| ✅ PubSub | 1 | 1 | 149 |
| ✅ Query | 7 | 2 | 246 |
| ✅ RAG | 16 | 2 | 386 |
| ✅ Realtime | 2 | 1 | 150 |
| ✅ Reflection | 3 | 1 | 147 |
| ✅ Retrieval | 5 | 1 | 150 |
| ✅ Routing | 1 | 1 | 153 |
| ✅ Sandbox | 8 | 2 | 132 |
| ✅ Scheduler | 2 | 2 | 401 |
| ✅ Security | 2 | 1 | 157 |
| ✅ Sessions | 14 | 2 | 433 |
| ✅ Skills | 7 | 2 | 380 |
| ✅ Streaming | 2 | 4 | 1211 |
| ✅ Tasks | 4 | 1 | 165 |
| ✅ Teams | 1 | 1 | 170 |
| ✅ Telemetry | 6 | 2 | 269 |
| ✅ Templates | 1 | 2 | 598 |
| ✅ Token Management | 1 | 1 | 159 |
| ✅ Tools | 34 | 15 | 3267 |
| ✅ Tracing | 11 | 3 | 477 |
| ✅ Vector Store | 7 | 2 | 530 |
| ✅ Video | 2 | 1 | 150 |
| ✅ Vision | 2 | 1 | 155 |
| ✅ Voice | 1 | 2 | 515 |
| ✅ Web | 4 | 1 | 150 |
| ✅ Workflows | 7 | 3 | 720 |

## Undocumented Categories (Need Documentation)

| Category | Features |
|----------|----------|
| ❌ Process | 2 |

## Documentation Without Features

These docs exist but don't match any implemented feature category:

- ℹ️ Callbacks (1 docs, 211 lines)
- ℹ️ Deep Research (2 docs, 292 lines)

---

## Manual parity notes

<!-- MANUAL: preserved across regeneration by the parity author -->

- **Scheduler `at:` timezone (Python #4732).** Python now stamps a naive `at:` timestamp with the resolved zone at parse time (aware stored value, per-target-date DST-correct) and raises on an unknown `PRAISONAI_SCHEDULE_TIMEZONE`. The TS SDK scheduler does not yet match this. Once the JS behavior aligns, mirror the wording from [Schedule Tools → Timezones](/tools/schedule-tools#timezones-for-at-and-cron) into `docs/js/scheduler.mdx`.

---

*Generated by `praisonai._dev.parity.docs_generator`*
