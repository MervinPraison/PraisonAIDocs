...
<Note>
Nine team-level options are now honoured — `memory`, `context`, `hooks`, `execution`, `planning`, `managerLlm`, `autonomy`, `toolsRunOn`, and `runOn` (which is **refused** with a `TypeError`, matching Python). Six more (`knowledge`, `guardrails`, `web`, `reflection`, `caching`, `learn`) are accepted for Python-SDK parity but emit a `[praisonai] … not yet honoured` notice. Both `autonomy` and `toolsRunOn` propagate to members that declared none — a member's own value always wins. See the [Parity Notices](/docs/js/typescript) page for the full list.
</Note>
...
| `autonomy` | `'suggest' \| 'auto_edit' \| 'full_auto' \| AutonomyConfig` | – | Propagated to every member that declared no autonomy of its own. A member's own `autonomy` wins — autonomy is a permission boundary, and silently widening one an agent set for itself is the direction that causes harm. A prompting level (anything not `full_auto`) also creates the member's approval gate. |
| `toolsRunOn` | `string \| ToolPlaceLike` | – | One shared tool place for the whole team. Every member that declared no `toolsRunOn` of its own gets the **same** `ToolPlace` instance — one sandbox for the whole team, not one per member. A member's own `toolsRunOn` wins. Unknown place names throw at team construction. |
...
Nine options now shape how the whole team runs.
...
<Tab title="autonomy">
One autonomy level for the whole team. Every member that declared no autonomy of its own adopts it; a member's own `autonomy` always wins.

```typescript
// Every member without its own autonomy adopts 'full_auto'
const team = new AgentTeam({
  agents: [researcher, writer],
  autonomy: 'full_auto',
});
// researcher and writer both get _autonomyConfig.level === 'full_auto'

// Precedence — a member's own choice wins
const cautious = new Agent({ instructions: 'Careful edits', autonomy: 'suggest' });
const bold = new Agent({ instructions: 'Ship it' });
new AgentTeam({ agents: [cautious, bold], autonomy: 'full_auto' });
// cautious keeps 'suggest'; bold adopts 'full_auto'

// A prompting level creates the approval gate on adopted members
new AgentTeam({ agents: [worker], autonomy: 'suggest' });
// worker gets an ApprovalManager (CLI prompt for anything not full_auto) and a DoomLoopTracker

// Explicit opt-out — members are left untouched
new AgentTeam({ agents, autonomy: false });
```

<Note>
Autonomy is a permission boundary. The team only reaches members that declared none — silently widening a level an agent set for itself is the direction that causes harm, so a member's own value always wins. Same rule Python uses.
</Note>
</Tab>

<Tab title="toolsRunOn">
One shared tool place for the whole team. Every member that declared no `toolsRunOn` of its own gets the **same** `ToolPlace` instance — one sandbox for the whole team, not one per member.

```typescript
// Both 'local' and 'docker' are built in — no import needed
const team = new AgentTeam({ agents: [a, b], toolsRunOn: 'local' });
a.getToolPlace()?.placeName === 'local';   // true
a.getToolPlace() === b.getToolPlace();      // true — same instance

new AgentTeam({ agents, toolsRunOn: 'docker' });

// Precedence — a member's own place wins
const own = new Agent({ instructions: 'Sandboxed', toolsRunOn: 'local' });
new AgentTeam({ agents: [own], toolsRunOn: 'docker' });
// own keeps its 'local' place

// Explicit opt-out — members are left unplaced
new AgentTeam({ agents, toolsRunOn: false });

// Unknown place — throws at team construction
new AgentTeam({ agents, toolsRunOn: 'nowhere' });
// TypeError: 'nowhere' is not a known place.
```

The team's place reaches any agent the team uses, including one that lives on a task or is built lazily from `agentConfig` — the same shared instance, on first use. A member that already has a place (its own `toolsRunOn` or one adopted earlier) is not overridden.

<Note>
One shared instance matters: separate instances per member would multiply cost and lose state that tools leave for each other in the same sandbox.
</Note>
</Tab>

<Tab title="runOn (refused)">
`runOn` is refused on a team: it hands one agent's whole loop to a managed runtime, and a team orchestrates several agents locally — there is no single loop to hand over. Team-level `toolsRunOn` **is** honoured (see the tab above) — run every member's tools in one shared sandbox.
