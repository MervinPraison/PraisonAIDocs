...
For the common case, register tools on the global registry with the Python-style `register_tool` / `get_tool`:

```typescript
import { register_tool, get_tool, list_tools } from 'praisonai';

register_tool(greeter);
register_tool(calculator);

list_tools();              // ['greeter', 'calculator']
get_tool('calculator');    // look up by name
```
...
Use a scoped `new ToolRegistry()` only when you need an isolated registry:

```typescript
import { ToolRegistry } from 'praisonai';

const registry = new ToolRegistry();

// The class method throws on duplicate names unless { overwrite: true }.
registry.register(greeter);
registry.register(calculator, { trustLevel: 'trusted' });
...
// Definitions advertise the registration key
...
<CardGroup cols={2}>
  ...
  <Card title="Custom Tools" icon="puzzle-piece" href="/js/customtools">
    More examples
  </Card>
</CardGroup>
