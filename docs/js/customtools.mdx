...
<Note>
If you don't pass a `description`, the tool advertises itself to the model as `Tool: <name>` (e.g. `Tool: getWeather`). Give it a clear `description` when you want the model to pick the right tool reliably.
</Note>
