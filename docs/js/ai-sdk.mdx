...
`outputSchema` takes a JSON Schema object (not a Zod schema) and is **OpenAI-only** in v1.7.4 — other providers log a warning and skip it. See [Structured Output](/js/structured-output).
...
<Note>
`outputSchema` and `tools` currently work end-to-end only on OpenAI. On `anthropic/...`, `google/...`, and other providers the agent emits a warning and answers without them — cross-provider support is a future PR. See [Output](/js/output) for details.
</Note>
