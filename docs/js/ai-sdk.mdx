...
`outputSchema` takes a JSON Schema object (not a Zod schema) and constrains the model to matching JSON on **every provider**. OpenAI uses native `response_format: json_schema`; other providers (Anthropic, Google, Groq, Mistral, Ollama, …) route through the AI SDK backend's `generateObject({ schema })`. See [Structured Output](/js/structured-output).
...
<Note>
`outputSchema` works on every provider from PraisonAI PR #4412 onward. Reasoning families (`gpt-5*`, `o1*`, `o3*`, `o4*`) still omit `temperature` automatically. See [Output → Schema-constrained output](/js/output#schema-constrained-output-outputschema).
</Note>
