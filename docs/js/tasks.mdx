...
const team = new Team({
  ...
  tasks: [new Task({ name: 'research', description: 'Research AI trends in 2024' })]
});
...
<Note>
Only `autonomy`, `web`, `reflection`, and `planning` are still accepted for Python-SDK parity but **not yet implemented** in the TypeScript SDK. Passing one emits a `[praisonai] … not yet honoured` notice — see [Parity Notices](/docs/features/typescript-parity-notices). Every other Task option below is fully live through `team.start()`.
</Note>
...
```typescript
new Task({ name: 'alpha', description: 'alpha', asyncExecution: true });
new Task({ name: 'quote', description: 'quote the price', caching: true, rerun: true });
```
