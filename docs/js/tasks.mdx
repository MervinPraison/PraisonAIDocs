...
<Note>
32 options on `Task.__init__` are accepted for Python-SDK parity but not yet acted on in the TypeScript SDK. If you pass one and see a `[praisonai] … not yet honoured` notice, see the [Parity Notices](/docs/features/typescript-parity-notices) page for the full list.
</Note>

<Info>
The behaviour behind 28 of these options is now implemented in the SDK's Task engine and will become active once the team runner wires it in — see PR [#4838](https://github.com/MervinPraison/PraisonAI/pull/4838). Until then these options still emit the parity notice above.
</Info>
