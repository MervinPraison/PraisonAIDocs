...
await knowledge.add('./docs', { userId: 'u1' });
...
async function answer(query: string) {
  if (autoRag.shouldRetrieve(query)) {
    const found = await knowledge.search(query, { userId: 'u1', limit: 5 });
    return generateWithContext(query, found.results);
  }
...
<CardGroup cols={2}>
  <Card title="AutoRagAgent" icon="code" href="/sdk/reference/typescript/classes/AutoRagAgent">
    AutoRagAgent class reference
  </Card>
  <Card title="AutoRagAgentConfig" icon="sliders" href="/sdk/reference/typescript/classes/AutoRagAgentConfig">
    Configuration options
  </Card>
</CardGroup>
