...
// Load every plugin file from a directory. Returns the count loaded.
const n = manager.loadFromDirectory('./my-plugins');
console.log(`loaded ${n} plugins`);

// Opt-in scan of the standard locations. Gated by an env var — see below.
process.env.PRAISONAI_ALLOW_PLUGIN_DISCOVERY = 'true';
const total = manager.autoDiscoverPlugins();
```
...
```javascript
// ~/.praisonai/plugins/greeter.js
const { Plugin } = require('praisonai');

module.exports.createPlugin = () => new Plugin({
  name: 'greeter',
  version: '1.0.0',
  type: 'hook',
});
...
<CardGroup cols={2}>
  <Card title="PluginManager" icon="puzzle-piece" href="/sdk/reference/typescript/classes/PluginManager">
    Plugin manager class
  </Card>
  <Card title="Plugin" icon="plug" href="/sdk/reference/typescript/classes/Plugin">
    Plugin base class
  </Card>
</CardGroup>
