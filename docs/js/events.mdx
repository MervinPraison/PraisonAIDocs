...
<Note>
This page covers the **`AgentEventBus`** — a publish/subscribe hook system for agent lifecycle (`start`, `message`, `complete`, `error`). For the **per-turn structured event stream** used to render tool calls and text deltas in real time, see [Streaming Events](/js/streaming-events).
</Note>
...
<Card title="Events Module" icon="code" href="/sdk/reference/typescript/modules/events">
  Complete events module
</Card>
<Card title="AgentEventBus" icon="robot" href="/sdk/reference/typescript/classes/AgentEventBus">
  Event bus class
</Card>
...
<CardGroup cols={2}>
  ...
  <Card title="PubSub" icon="envelope" href="/js/pubsub">
    Publish-subscribe
  </Card>
</CardGroup>
