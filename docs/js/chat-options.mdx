...
| `errorsAsNull` | `boolean` | `chat()` only (`AgentChatCallOptions`). Resolve with `null` instead of rejecting; widens the return to `Promise<string \| null>`. |

<Note>
`errorsAsNull` lives on `AgentChatCallOptions`, which extends `AgentChatOptions`. Only `chat()` honours it — `start()` and `stream()` accept `AgentChatOptions` and ignore it. Even with `errorsAsNull: true`, these still reject (Python re-raises them too):
...
Swallowed errors are still reported through `Logger.error`, so a failed turn is never silent.
</Note>
