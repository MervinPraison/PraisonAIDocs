...
Custom transport and browser support are configurable on `ProviderConfig`.

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `fetch` | `typeof fetch` | `undefined` | Custom fetch — lets a host route egress through native code (e.g. a Tauri command). Implies `dangerouslyAllowBrowser`. |
| `dangerouslyAllowBrowser` | `boolean` | auto-detected | Force-enable the OpenAI SDK's browser guard. Auto-`true` in browser-like runtimes. |

See [Browser & Webview Runtimes](/features/browser-runtimes) for when to use each option.
