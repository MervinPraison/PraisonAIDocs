...
<Note>
`tools` and `outputSchema` currently work end-to-end only on OpenAI. On non-OpenAI providers (`anthropic/...`, `google/...`, etc.) the agent emits a warning and answers without them — cross-provider support is a future PR.
</Note>
