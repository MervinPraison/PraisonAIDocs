...
<Note>
`tools` and `outputSchema` work end-to-end on every provider — OpenAI, Anthropic (`anthropic/...`), Google (`google/...`), xAI, Groq, and any AI-SDK backend. Non-OpenAI providers route through the AI SDK backend, which streams pre-tool text as tokens when `stream: true`. See [Streaming](/js/streaming) for the tool-loop streaming behaviour.
</Note>
