...
Track latency, throughput, and errors across your Agent deployments. Identify bottlenecks and optimise performance.

```mermaid
graph LR
    Agent[Agent] --> Mon[Monitor]
    Mon --> Metrics([Metrics])
...
const agent = new Agent({ name: 'Monitored Agent', instructions: 'You are a helpful assistant.' });

const timer = monitor.startTimer('agent_response');
await agent.chat('Hello');
timer.stop();
console.log(monitor.getStats('agent_response'));
```
...
<CardGroup cols={2}>
  ...
  <Card title="Observability" icon="eye" href="/docs/js/observability">
