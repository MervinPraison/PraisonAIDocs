...
Long-running tools receive `ctx.signal` on `ToolExecutionContext` — check it to stop early when cancelled. See [Cancellation](/js/cancellation).
...
<CardGroup cols={2}>
  <Card title="Custom Tools" icon="screwdriver-wrench" href="/js/customtools">Build custom tools</Card>
  <Card title="Factory Registry" icon="industry" href="/js/tools/factory-registry">Build instances from config</Card>
  <Card title="Tool Errors" icon="triangle-exclamation" href="/js/tools/errors">Missing vs broken tools</Card>
  <Card title="MCP Tools" icon="plug" href="/js/mcp-tools">MCP integration</Card>
</CardGroup>
