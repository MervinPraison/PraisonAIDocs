...
**Sandbox extras:** the sandbox backends live in the standalone [`praisonai-sandbox`](/features/praisonai-sandbox-package) package. Install a cloud backend with `pip install "praisonai[docker]"`, `"praisonai[ssh]"`, `"praisonai[modal]"`, `"praisonai[e2b]"`, `pip install "praisonai-sandbox[daytona]"` for the Daytona backend (needs `DAYTONA_API_KEY`), or `pip install "praisonai-sandbox[novita]"` for the Novita backend (needs `NOVITA_API_KEY`). See [Sandbox Backends](/docs/features/sandbox-backends#installing-optional-backends) for the full matrix.
</Note>
