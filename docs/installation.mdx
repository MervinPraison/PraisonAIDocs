...
<CardGroup cols={2}>
  ...
  <Card title="Training only" icon="graduation-cap" href="/docs/features/praisonai-train-package">
    `pip install "praisonai-train[llm]"` — LLM fine-tuning + agent training
  </Card>
  <Card title="Browser automation" icon="globe" href="/docs/features/praisonai-browser-package">
...
**Sandbox extras:** the sandbox backends live in the standalone [`praisonai-sandbox`](/docs/features/praisonai-sandbox-package) package. Install a cloud backend with `pip install "praisonai[docker]"`, `"praisonai[ssh]"`, `"praisonai[modal]"`, `"praisonai[e2b]"`, or `pip install "praisonai-sandbox[daytona]"` for the Daytona backend (needs `DAYTONA_API_KEY`). See [Sandbox Backends](/docs/features/sandbox-backends#installing-optional-backends) for the full matrix.
</Note>
...
Running Ollama on the default `127.0.0.1:11434`? Skip `export OLLAMA_HOST` — PraisonAI auto-detects a local endpoint when no cloud key is set. See [Keyless local-first fallback](/docs/models#keyless-local-first-fallback-no-env-vars-set).
  ...
  <Tab title="Train (praisonai-train)">
    ...
    See the [praisonai-train Package page](/docs/features/praisonai-train-package) for fine-tuning and agent-training details.
  </Tab>
  <Tab title="Browser (praisonai-browser)">
    ...
    See the [praisonai-browser Package page](/docs/features/praisonai-browser-package) and the [Browser CLI](/docs/cli/browser) for the full command set.
  </Tab>
