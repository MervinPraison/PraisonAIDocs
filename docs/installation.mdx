...
<Tabs>
  ...
  <Tab title="Windows">
    No PowerShell installer is shipped yet. Use `pipx` or `pip`:
    ```powershell
...
| `pip install praisonai-code` | Terminal-native agent CLI + LLM runtime | ✅ `run --output actions|json|stream|stream-json`, `daemon`, `doctor`, `config`, `version` — `chat`/`code`/default `run`/`run --output plain|verbose|silent` need the wrapper | ❌ No — needs `pip install praisonai` |
