...
<Note>
Prefer a native app over the CLI? Get the **[Desktop App](/install/desktop)** for macOS, Windows, or Linux — no Python setup required.
</Note>

<div align="center">
  <a href="https://github.com/MervinPraison/PraisonAI/releases/latest">
    <img src="https://img.shields.io/badge/Download_for_macOS-000000?style=for-the-badge&logo=apple&logoColor=white" />
  </a>
  <a href="https://github.com/MervinPraison/PraisonAI/releases/latest">
...
<Note>
The one-liner installs a managed, isolated CLI and adds `praisonai` to your PATH. Keep it current with `praisonai upgrade` and remove it with `praisonai uninstall` — see [Upgrade & Uninstall](/features/upgrade-uninstall). Prefer `pip`? That still works exactly as before — pick a package below.
</Note>
...
```
```bash Train (fine-tune)
pip install "praisonai-train[llm]"
```
```bash Browser (Chrome automation)
...
<CardGroup cols={2}>
  <Card title="Terminal-only (smaller)" icon="terminal" href="/features/praisonai-code-cli">
    `pip install praisonai-code` — agentic CLI without gateway or bots
  </Card>
  <Card title="Bots + gateway only" icon="comments" href="/sdk/praisonai-bot/index">
    `pip install "praisonai-bot[gateway,bot]"` — channel runtime without the wrapper
  </Card>
  <Card title="Full (default)" icon="box" href="/installation">
    `pip install praisonai` — wrapper, code, bot, and SDK together
  </Card>
  <Card title="Training only" icon="graduation-cap" href="/features/praisonai-train-package">
    `pip install "praisonai-train[llm]"` — LLM fine-tuning + agent training
  </Card>
  <Card title="Browser automation" icon="globe" href="/features/praisonai-browser-package">
    `pip install "praisonai-browser[all]"` — Chrome extension + Playwright engine. Run `playwright install chromium` once before first browser use.
  </Card>
  <Card title="Sandbox backends only" icon="shield-halved" href="/docs/features/praisonai-sandbox-package">
...
<Info>
**First browser use.** `pip install praisonai` no longer downloads Playwright browsers. Install them on demand the first time you use a browser tool:
...
This keeps the default install fast, small, and **offline / air-gapped / CI-friendly**. `praisonai[browser]` and `praisonai[all]` behave the same way — they include the Python packages but not the browser binaries. See [Offline & CI Install](/features/offline-and-ci-install).
</Info>

**What you get with `pip install praisonai`:**

- Full CLI, gateway, bots, and the `praisonaiagents` SDK.
- Browser binaries: **not included** — run `playwright install chromium` on first use.
...
<Tip>
Keep the managed install current with [`praisonai upgrade`](/cli/upgrade), or remove it cleanly with [`praisonai uninstall`](/cli/uninstall).
</Tip>
...
graph TB
    ...
    Q1 -->|Fine-tune LLMs or\ntrain agents| P5["pip install praisonai-train[llm]"]:::process
    Q1 -->|Automate Chrome /\na browser| P6["pip install praisonai-browser[all]"]:::process
    Q1 -->|Only sandbox backends,\nno wrapper| P7["pip install praisonai-sandbox[docker]"]:::process
...
**`[claw]` extra:** `pip install "praisonai[claw]"` installs the full wrapper plus `praisonai-bot[gateway,bot]` — the WebSocket gateway and Telegram/Discord/Slack channel bots. Use this when you want the full wrapper and all channel runtime features in one command. For gateway + bots **without** the wrapper, use `pip install "praisonai-bot[gateway,bot]"` directly (see [Standalone Bot Gateway](/features/standalone-bot-gateway)).

**Sandbox extras:** the sandbox backends live in the standalone [`praisonai-sandbox`](/features/praisonai-sandbox-package) package. Install a cloud backend with `pip install 'praisonai[docker]'`, `'praisonai[e2b]'`, `'praisonai[modal]'`, `'praisonai[daytona]'` (needs `DAYTONA_API_KEY`), `pip install 'praisonai-sandbox[ssh]'`, or `pip install 'praisonai-sandbox[novita]'` for the Novita backend (needs `NOVITA_API_KEY`). See [Sandbox Backends](/docs/features/sandbox-backends#installing-optional-backends) for the full matrix. The `praisonai[e2b]`, `praisonai[docker]`, and `praisonai[daytona]` extras were fixed in [PR #4073](https://github.com/MervinPraison/PraisonAI/pull/4073) — before that release they either did not exist or were empty. If `pip` warns `does not provide the extra 'e2b'`, upgrade `praisonai`.
</Note>
...
<Tip>
Running Ollama on the default `127.0.0.1:11434`? Skip `export OLLAMA_HOST` — PraisonAI auto-detects a local endpoint when no cloud key is set. See [Keyless local-first fallback](/docs/models#keyless-local-first-fallback-no-env-vars-set).
</Tip>
      </Step>
  ...
  <Tab title="Bot (praisonai-bot)">
    ...
    See the [praisonai-bot SDK page](/sdk/praisonai-bot/index) for Python API details.
  </Tab>
  <Tab title="Train (praisonai-train)">

    Standalone training tier — LLM fine-tuning via Unsloth and iterative agent training, without the full wrapper. Depends on `praisonaiagents` only at PyPI level.

    <Note>
    As of `praisonai-train 2026.7+`, the `[llm]` extra requires CUDA 12.4+ and installs the modern Unsloth / TRL / torch 2.6+ stack. The install command is unchanged.
    </Note>
    ...
    <Steps>
      ...
      <Step title="Install">
        ...
        ```
        <Note>
        LLM fine-tuning needs Python 3.11, CUDA 12.4, and `torch>=2.6` (plus `unsloth>=2025.9.1`, `trl>=0.18.2`, `transformers>=4.51.3`). Agent training (`pip install praisonai-train`) has no such requirement.
        </Note>
      </Step>
    ...
    See the [praisonai-train Package page](/features/praisonai-train-package) for fine-tuning and agent-training details.
  </Tab>
  <Tab title="Browser (praisonai-browser)">

    Standalone browser tier — the bridge server, CDP agent, and Playwright engine for Chrome automation, without the full wrapper. Depends on `praisonaiagents` only at PyPI level.
    ...
    <Steps>
      ...
      <Step title="Install a browser engine (once)">
        ```bash Terminal
        playwright install chromium
        ...
        ```
        <Note>
        No browser binaries are downloaded automatically by `pip install` in any tier — they are provisioned on demand. See [Offline & CI Install](/features/offline-and-ci-install).
        </Note>
      </Step>
    ...
    See the [praisonai-browser Package page](/features/praisonai-browser-package) and the [Browser CLI](/cli/browser) for the full command set.
  </Tab>
...
graph LR
    ...
    A --> S[praisonai-sandbox\nCode-execution backends]:::process
...
Each layer depends on the one to its left. Installing `praisonai` also pulls `praisonai-sandbox` (the code-execution backends) alongside `praisonai-code`; installing `praisonai-code` pulls only `praisonaiagents`; installing `praisonaiagents` has no PraisonAI dependencies.
...
<CardGroup cols={2}>
  <Card
    ...
    icon="terminal"
    href="/features/praisonai-code-cli"
  >
...
<Note>
The one-liner installer uses an isolated tool manager (`uv tool` → `pipx` → bootstrap `uv`) and exposes `praisonai` via a `~/.local/bin/praisonai` shim — your global Python environment is untouched. See [Isolation Backends](/install/isolation-backends) for details.
</Note>
...
<CardGroup cols={2}>
  <Card title="Upgrade" icon="arrow-up" href="/features/praisonai-upgrade">
    `praisonai upgrade` — update the CLI in place, or `--check` for a newer version
  </Card>
  <Card title="Uninstall" icon="trash" href="/features/praisonai-uninstall">
...
| `pip install praisonai-code` | Terminal-native agent CLI + LLM runtime | ✅ Default `run "…"`, `run --output plain|verbose|silent|actions|json|stream|stream-json`, `chat`, `code` (resident TUI, PR #3818), `daemon`, `doctor`, `config`, `version` | ❌ No — needs `pip install praisonai` |
...
```
1. praisonaiagents  →  2. praisonai-code + praisonai-bot + praisonai-train + praisonai-browser  →  3. praisonai
```

If you pin versions, ensure all six packages resolve to the same release cycle.

Since 4.6.104, `praisonai` pins `praisonai-code>=0.0.2`, so `pip install praisonai` always pulls a compatible runtime automatically. See [`praisonaiagents` SDK page](/api/praisonaiagents/index) for the release-cycle notes.