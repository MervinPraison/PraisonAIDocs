...
graph LR
    ...
    classDef tool fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef process fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef bot fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef success fill:#10B981,stroke:#7C90A0,color:#fff
```

Each layer depends on the one to its left. Installing `praisonai` pulls all four; installing `praisonai-code` or `praisonai-bot` pulls only `praisonaiagents`; installing `praisonaiagents` has no PraisonAI dependencies.
...
PraisonAI ships as four separate PyPI packages. Understanding which to install avoids surprises at runtime.
...
graph TB
    ...
    H["pip install praisonai-bot[gateway,bot]"] --> G
...
|-----------------|--------------|----------------------------------------|-------------------------|
| `pip install praisonai` | Wrapper + code + bot + agents | ✅ Full | ✅ Yes |
| `pip install praisonai-code` | Terminal-native agent CLI | ✅ Hot path — `run`, `chat`, `code` | ❌ No (graceful skip) |
| `pip install "praisonai-bot[gateway,bot]"` | Bots + gateway, no wrapper | ❌ No agentic CLI | ✅ Yes |
...
```
1. praisonaiagents  →  2. praisonai-code + praisonai-bot  →  3. praisonai
```

If you pin versions, ensure all four packages resolve to the same release cycle.