...
<Note>
**Resume works for team runs launched from a YAML file too.** `praisonai run agents.yaml --continue` / `--session <id>` restores per-agent chat history and shared team state, so each agent replays its earlier context before the new prompt. See [YAML / Team Session Continuity](/docs/features/yaml-session-continuity).
</Note>
