...
<Note>
**DB-backed sessions preserve tool calls on resume too.** As of PraisonAI PR [#5082](https://github.com/MervinPraison/PraisonAI/pull/5082), the `DbAdapter` persistence path routes assistant `tool_calls` and `role="tool"` results to the optional `on_assistant_message` / `on_tool_message` hooks (see [Custom DB Adapter](/features/database-persistence#custom-db-adapter)). The built-in `praisonai.db()` adapter implements them, so resuming a DB-backed session (`memory=MemoryConfig(db=…, session_id=…)`) now reconstructs the same message list the model saw before — tool calls included. Custom adapters that don't implement the new methods keep the old text-only behaviour, backward-compatible.
</Note>
