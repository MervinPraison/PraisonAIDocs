...
`DefaultSessionStore` mutators that change metadata (`update_session_metadata`, `set_agent_info`, `set_gateway_info`, `clear_session`) and the `LocalManagedAgent` state-persist path now perform a locked, atomic read-modify-write from disk, so concurrent `add_message()` calls from another store instance, worker, or process can no longer be silently overwritten (PR #1709, #1724, #1727).
