...
<Note>
For **gateway** deployments the executor is now built with a default `RunPolicy` automatically — all protections on. You only write Python `RunPolicy` code when you run your own `ScheduledAgentExecutor` outside the gateway. To tune the gateway defaults from ops, add a `scheduler:` block to `gateway.yaml` — see [Scheduled Run Policy → Configuring via `gateway.yaml`](/features/scheduled-run-policy#configuring-via-gatewayyaml).
</Note>

<Note>
**Host-app embedding.** The `praisonai.integration.bridges.schedules_runner` bridge builds its executor with a default `RunPolicy` too. If it cannot build an executor, its poll loop does **not** start, so due jobs stay genuinely due (never silently lost) until a later start finds an executor.
</Note>

For non-gateway deployments, use `RunPolicy` to add run-scoped guardrails to every scheduled execution:
...
See [Scheduled Run Policy](/features/scheduled-run-policy) for the full configuration reference.

## See Also

- [Scheduler CLI](/cli/scheduler)
- [Scheduled Run Policy](/features/scheduled-run-policy)
- [Background Tasks Deployment](/deploy/background-tasks)
- [Async Jobs Deployment](/deploy/async-jobs-deploy)
