...
<Note>
`praisonai serve registry` is the canonical command. The legacy `praisonai registry serve` form is **deprecated** (prints a warning to `stderr`) and its default port is now `7777` — aligned with this form. Update any old scripts or Dockerfiles that relied on the old `8080` default. See the [Registry CLI](/docs/cli/registry#serve) for details.
</Note>
...
CMD ["praisonai", "serve", "registry", "--host", "0.0.0.0", "--port", "7777"]
```
...
- [Recipe Registry](/features/recipe-registry) - Python API reference
- [Recipe Registry API](/deploy/api/recipe-registry-api) - HTTP API endpoints
- [Recipe CLI](/cli/recipe-registry) - CLI commands
