...
- The background API deploy path spawns children via `sys.executable`, so it inherits the parent venv's Flask install on Windows and multi-Python setups (no more `ModuleNotFoundError: flask`).
</Note>
