...
deploy:
  ...
  docker:
    ...
    push: false
    registry: null
  # Optional sibling block — configures the generated in-container server.
  api:
    host: 0.0.0.0
    port: 8005
...
deploy:
  ...
  api:
    host: 0.0.0.0
    port: 8005
    auth_enabled: false   # /chat without a bearer token
```

| Behavior | Effect |
|----------|--------|
| Omit the block | Auth **on**, CORS on, port 8005, bearer token required for `/chat` |
| `auth_enabled: false` | Injects `PRAISONAI_API_AUTH=disabled` into the container; `/chat` is reachable without a token |
| `auth_enabled: true` (or omitted) | No env override; the generated server enforces auth |
...
| `PRAISONAI_API_AUTH` | No | Set to `disabled` by the deploy layer when `api.auth_enabled: false`. Do not set manually. |
