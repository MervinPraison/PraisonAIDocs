...
```bash
# One command, done — with a persistent SQLite store
praisonai serve jobs --port 8005 --db /var/lib/praisonai/jobs.db
```

Advanced / factory mode (direct uvicorn control, multiple workers):
...
# Run server
CMD ["praisonai", "serve", "jobs", "--host", "0.0.0.0", "--port", "8005", "--db", "/data/jobs.db"]
# Advanced / factory mode:
...
praisonai serve jobs --host 0.0.0.0 --port 8005
```
