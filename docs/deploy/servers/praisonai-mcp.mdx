...
<Note>
`praisonai-mcp serve …` (standalone) and `praisonai mcp serve …` (umbrella) are equivalent for host commands. See the [praisonai-mcp Package](/features/praisonai-mcp-package) guide and [The Three MCP Layers](/features/mcp-three-layers).
</Note>
...
See [Serve Agents](/features/serve-agents) for the full guide.
...
<Warning>
The `instructions=` parameter was **removed** from `praisonai.agent.chat` and `praisonai.agent.run` in PR #3531 (fixes #3528). A connecting client can no longer rewrite the assistant's persona — the server owns it. To publish *your* agents with *your* persona, use [`serve_agents([...])`](/features/serve-agents).
</Warning>
...
The todo tools read and write the **same** store as the agent runtime — `<workspace>/todos.json` if a workspace is active, else `~/.praisonai/todos.json`. Rebound to the runtime store in PraisonAI PR [#4962](https://github.com/MervinPraison/PraisonAI/pull/4962); before that they used the wrong path `~/.praison/todo.json` in an incompatible schema.

| Tool | Arguments | Description |
|------|-----------|-------------|
| `praisonai.todo.list` | — | List every todo. Returns `"No todos found"` when the store is empty. |
| `praisonai.todo.add` | `content`, `priority="medium"` | Append a todo. Returns `"Todo added: <id>"` (integer id, starts at `1`). |
| `praisonai.todo.complete` | `todo_id` | Mark the matching todo `completed`. Returns `"Todo not found: <id>"` if no match. |
| `praisonai.todo.delete` | `todo_id` | Delete the matching todo. Returns `"Todo not found: <id>"` if no match. |

See [MCP Todo Tools](/features/mcp-todo-tools) for the record shape and store resolution.
...
- [Serve Agents](/features/serve-agents) - Publish your own agents as `ask_{name}` tools
- [praisonai-mcp Package](/features/praisonai-mcp-package) - Standalone host package guide
- [The Three MCP Layers](/features/mcp-three-layers) - Client vs light server vs heavy host
