...
<Warning>
A **public bind requires a key**. `praisonai serve agents` exits with `SystemExit` if `--host` is not `127.0.0.1` / `localhost` / `::1` and neither `--api-key` nor `PRAISONAI_SERVE_API_KEY` is set — because `POST /agents` can execute YAML-defined tools. See [Serve Agents Authentication](/docs/features/serve-agents-auth).
</Warning>

```bash
# Authorization header
curl -H "Authorization: Bearer $API_KEY" http://localhost:8000/agents
...
| `/__praisonai__/*` | Discovery endpoints |

```mermaid
graph LR
    Req[🔑 Request] --> Check{🛡️ Public path?}
    Check -->|Yes| Allow[✅ 200 Response]
    Check -->|No| Key{🛡️ Valid key?}
    Key -->|Yes| Allow
    Key -->|No| Deny[⛔ 401 Unauthorized]

    classDef request fill:#8B0000,stroke:#7C90A0,color:#fff
