...
<Warning>
A **public bind requires a key**. `praisonai serve agents` exits with `SystemExit` if `--host` is not `127.0.0.1` / `localhost` / `::1` and neither `--api-key` nor `PRAISONAI_SERVE_API_KEY` is set — because `POST /agents` can execute YAML-defined tools. See [Serve Agents Authentication](/features/serve-agents-auth).
</Warning>
