...
<Steps>
  ...
  <Step title="List Available Recipes">
    ...
    **Expected Output:**
    ...
    Filter by source with `--source custom|project|package`, by tags with `--tags video,social`, or add `--json` for CI. See [Recipe → MCP Bridge](/features/mcp-recipe-server#list-recipes).
  </Step>
...
| `list-recipes --source` rejects a value | `--source` accepts only `custom`, `project`, `package` — `local` and `all` were never valid |
