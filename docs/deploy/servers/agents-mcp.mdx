...
<Note>
Two paths publish agents over MCP — pick based on your workload:

- **`Agent.launch(protocol="mcp")`** (this page) — one process per agent, exposes a generic `execute_{name}_task` tool, no built-in session continuity across turns. Best for a single-agent workload behind an existing HTTP host.
- **[`serve_agents([...])`](/docs/features/serve-agents)** — one process for many agents, exposes `ask_{name}` per agent plus `list_agents()`, session continuity built in. Best for publishing a real product to Claude/Cursor.
</Note>
...
- [Serve Agents](/docs/features/serve-agents) - Publish many agents as `ask_{name}` tools in one line
