...
### Emitting events from tool callbacks

Sync emitters (`emit_agent_started`, `emit_agent_thinking`, `emit_agent_tool_call`, `emit_agent_response`, `emit_agent_completed`, `emit_agent_error`) are safe to call from any thread, including a tool callback firing on a background thread. The event bus schedules delivery on the SSE consumer's own loop via `call_soon_threadsafe`, so events published from a different thread reach the stream instead of being silently dropped.

Events published in the small window between `POST /a2u/subscribe` and the client's first `GET /a2u/events/sub/{id}` are buffered in a bounded pre-bind deque and drained into the queue on first consume. The cap is the same bound as the runtime queue (`PRAISONAI_A2U_QUEUE_MAX`), so a subscription that never consumes cannot grow without limit.

If the queue is full (slow or stalled consumer), events are still dropped with a warning log — only cross-loop delivery and the subscribe→consume gap are fixed.
