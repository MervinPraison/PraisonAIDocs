...
<ResponseField name="pressure" type="object">
  Gateway saturation snapshot — present only when the back-pressure machinery is wired. Classifies load as `nominal`, `elevated`, or `saturated`. See [Pressure Telemetry](/features/gateway-pressure-telemetry).
</ResponseField>
...
- [Discovery API](/deploy/api/discovery-api) - Full discovery document
