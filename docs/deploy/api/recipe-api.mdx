...
- Long-running recipes no longer starve `/health`, `/metrics`, or concurrent requests — sync recipe work is offloaded to a worker thread pool and SSE events are queued off the event loop

## Related

- [Recipe CLI](/cli/recipes) - CLI for recipes
- [Agents API](/deploy/api/agents-api) - Agent endpoints
