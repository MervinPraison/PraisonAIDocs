...
| Agents | `praisonai serve agents` | 8000 | Agent HTTP API |
...
<Warning>
`serve unified` and `serve agents` **require** `--api-key` (or `PRAISONAI_SERVE_API_KEY`) when binding to a non-localhost host — the tool-invocation routes are refused without one. The key is enforced by the shared `_api_auth` middleware on every route except `/health`.
</Warning>

**Using API key:** `--api-key` accepts either `Authorization: Bearer` or `X-API-Key`.
```bash
# Authorization header
curl -H "Authorization: Bearer your-secret-key" http://127.0.0.1:8765/agents

# X-API-Key header
curl -H "X-API-Key: your-secret-key" http://127.0.0.1:8765/agents
```

The async Jobs API uses a separate key — see [Jobs API Authentication](/docs/deploy/api/jobs-api#authentication).
...
| `--api-key` | - | API key. **Required when `--host` is not localhost** for `serve unified` / `serve agents`. Enforced on all routes except `/health`. |
...
<CardGroup cols={2}>
  <Card title="Discovery API" icon="compass" href="/deploy/api/discovery-api">
    Unified discovery endpoint
  </Card>
  <Card title="Agents API" icon="robot" href="/deploy/api/agents-api">
    Agent invocation endpoints
  </Card>
  <Card title="Recipe API" icon="book" href="/deploy/api/recipe-api">
    Recipe runner endpoints
  </Card>
  <Card title="A2U API" icon="stream" href="/deploy/api/a2u-api">
