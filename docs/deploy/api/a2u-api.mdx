...
curl http://127.0.0.1:8083/a2u/health \
  -H "Authorization: Bearer $A2U_AUTH_TOKEN"
```
...
| 200 | Success |
| 400 | Invalid request |
| 401 | Missing/invalid bearer token (when `A2U_AUTH_TOKEN` is set) |
| 404 | Subscription not found |
| 429 | Subscription cap reached — `{"error": "A2U subscription limit reached"}` (`PRAISONAI_A2U_MAX_SUBS`, default 1024) |
| 500 | Server error |
| 503 | Non-loopback bind without `A2U_AUTH_TOKEN` — `{"error": "A2U_AUTH_TOKEN not configured; refusing non-loopback traffic"}` |

The `429` applies to `POST /a2u/subscribe` and `GET /a2u/events/{stream_name}` when the subscription cap is reached; the `503` applies to any `/a2u/*` route on a non-loopback bind with no token. See [Server: A2U → Security & Limits](/deploy/servers/a2u#security-limits).
...
<Note>
**Safe from any thread.** The sync emitters (`emit_agent_started`, `emit_agent_thinking`, `emit_agent_tool_call`, `emit_agent_response`, `emit_agent_completed`, `emit_agent_error`) can be called from any thread, including a tool callback firing on a background thread. The event bus schedules delivery on the SSE consumer's own loop via `call_soon_threadsafe`, so cross-thread events reach the stream instead of being silently dropped. Events published between subscribe and first consume are buffered in a bounded pre-bind deque and drained on first read. See [Server: A2U → Emitting events from tool callbacks](/deploy/servers/a2u#emitting-events-from-tool-callbacks).
</Note>
...
- [A2A API](/deploy/api/a2a-api) - Agent-to-Agent protocol
- [Agents API](/deploy/api/agents-api) - Agent invocation
