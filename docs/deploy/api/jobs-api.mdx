...
```bash
# Start the dedicated async jobs server (port 8005)
praisonai serve jobs --port 8005
```

**Base URL:** `http://127.0.0.1:8005`

### Combined unified server

```bash
# The unified server bundles the jobs API alongside other providers (port 8765)
...
<Note>
Use `serve jobs` (port `8005`) when you only need the async jobs API. `serve unified` (port `8765`) includes the jobs API as one of several providers on a single port. Endpoints below are identical on either server.
</Note>
...
curl -H "Authorization: Bearer $JOBS_API_KEY" http://127.0.0.1:8765/api/v1/runs

# X-API-Key header
curl -H "X-API-Key: $JOBS_API_KEY" http://127.0.0.1:8765/api/v1/runs
```

`/health` is the only endpoint reachable without a key. Enable the key with `PRAISONAI_JOBS_API_KEY` — see [Async Jobs Deployment](/docs/deploy/async-jobs-deploy#authentication).
...
<ParamField body="framework" type="string">
  Framework to use: `praisonai`, `crewai`, `autogen`. Omit to use the registry-selected default (`praisonai` on a standard install; a tenant/custom registry's `pick_default()` when one is configured). See [Registry Dependency Injection](/features/registry-dependency-injection) and [`resolve_or_default()`](/features/framework-adapter-plugins#resolve_or_default-for-optional-framework-inputs).
</ParamField>
...
# Omitting "framework" resolves to the registry-selected default (praisonai on a standard install).
```

```python Python
import requests

# The request body below omits "framework", so it resolves via the
# registry-selected default rather than a hardcoded "praisonai".
...
- [A2U API](/deploy/api/a2u-api) - Event streaming
- [Recipe API](/deploy/api/recipe-api) - Sync recipe execution
