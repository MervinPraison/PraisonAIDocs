...
<Note>
**Two Different API Servers**: This page documents the `Agent.launch()` HTTP server. For the auto-generated API server created by `praisonai deploy run --type api`, see [API Server Authentication](/deploy/api/api-server-auth).
</Note>
