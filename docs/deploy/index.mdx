...
# As of 4.6.34, requires bearer token by default
# See API Server Authentication guide
```
  </Tab>
  <Tab title="Docker">
```bash
...
|--------|-------------|-------|
| **API Server** | Local FastAPI server for development | [API Guide](./cli/api) · [Auth Guide](/features/api-server-auth) |
...
Local stacks, cloud providers, Kubernetes charts, and one-command scaffolding.

<CardGroup cols={2}>
  <Card title="Docker Compose Stack" icon="docker" href="/docs/features/compose-stack">
    Local API + Postgres with `praisonai deploy compose up`
  </Card>
  <Card title="Deploy Templates" icon="file-code" href="/docs/features/create-templates">
    Scaffold projects with `praisonai deploy create`
  </Card>
...
<Warning>
`praisonai serve agents` **and** `praisonai serve unified` refuse a non-localhost bind (`--host 0.0.0.0`) unless an API key is set — both drive tool-invocation routes, so they exit with `SystemExit` when neither `--api-key` nor `PRAISONAI_SERVE_API_KEY` is present. Localhost binds need no key. See [Agents Server → Security](./servers/agents#security) and [Unified Server → Security](./servers/unified#security).
</Warning>
...
| I want to... | Use |
|--------------|-----|
| Test locally | `praisonai deploy run --type api` or `agent.launch()` (auto-generated API server prints bearer token on each startup) |
