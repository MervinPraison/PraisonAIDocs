...
# Authentication disabled by default - see API Server Authentication guide to enable
```
  </Tab>
  <Tab title="Docker">
```bash
...
|--------|-------------|-------|
| **API Server** | Local FastAPI server for development | [API Guide](./cli/api) · [Auth Guide](./api/api-server-auth) |
