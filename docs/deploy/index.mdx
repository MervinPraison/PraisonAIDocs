...
|--------|-------------|-------|
| **API Server** | Local FastAPI server for development | [API Guide](./cli/api). See [API Server Authentication](/features/api-server-auth) |
...
| I want to... | Use |
|--------------|-----|
| Test locally | `praisonai deploy run --type api` or `agent.launch()` (local dev server now prints bearer token on first run) |
