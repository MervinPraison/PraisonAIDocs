...
<Warning>
`praisonai serve agents` refuses a non-localhost bind (`--host 0.0.0.0`) unless an API key is set — it drives YAML-defined tools, so it exits with `SystemExit` when neither `--api-key` nor `PRAISONAI_SERVE_API_KEY` is present. Localhost binds need no key. See [Agents Server → Security](./servers/agents#security).
</Warning>
