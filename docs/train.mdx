...
<Note>
**Supported Ollama templates:** Llama, Qwen, Mistral, Phi, DeepSeek, LLaVA, and Gemma (new). Any other model falls back to a Llama-style template. Gemma folds the system prompt into the first user turn (per the Gemma chat template spec) and stops on `<end_of_turn>` / `<start_of_turn>`.
</Note>

<Note>
Tested on Linux with 1–2 GPUs; the current dependency floor requires CUDA 12.4+ (see the conda env installer).
</Note>
...
<Note>
All `lora_*` keys (`lora_r`, `lora_alpha`, `lora_target_modules`, `lora_dropout`, `lora_bias`, `use_gradient_checkpointing`, `random_state`, `use_rslora`, `loftq_config`) now flow through to the LoRA adapter — they were silently ignored before. Set them intentionally, or drop them to keep the defaults shown above.
</Note>
