...
**Unknown framework error**: If you see `Unknown framework 'X'. Available frameworks: crewai, autogen, autogen_v4, ag2, praisonai`, check spelling — frameworks are registered via the adapter registry.

**Missing installation errors**:
- `AG2 is not installed. Please install with: pip install "praisonai[ag2]"` — Install AG2 dependencies
- `AutoGen v0.2 is not installed. Please install with: pip install "praisonai[autogen]"` — Install AutoGen v0.2 dependencies

Note: AG2 and AutoGen adapters no longer show tools warnings since `requires_tools_extra=False` for AG2 and `requires_tools_extra=True` only for CrewAI and AutoGen v0.2.
