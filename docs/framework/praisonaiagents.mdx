...
## Run from YAML with `framework: praisonai`

The `framework: praisonai` now runs end-to-end, building a real `AgentTeam` from YAML configuration.
...
roles:
  researcher:
    role: Senior Research Analyst
    goal: Find latest AI trends about {topic}
    backstory: Expert technology researcher
    tools: [duckduckgo]
    tasks:
      summarise:
        description: Summarise findings about {topic}
...
The adapter understands these YAML keys and maps them to SDK equivalents:

| YAML Key | Maps To | Notes |
|----------|---------|-------|
| `roles.<name>.role/goal/backstory/instructions` | `Agent(...)` | Templated with `{topic}` |
| `roles.<name>.llm` (per-role) | Not currently read — `llm_config[0].model` is used | Document gap honestly |
| `roles.<name>.allow_delegation` | `Agent(allow_delegation=...)` | |
| `roles.<name>.tools` (list of names) | `Agent(tools=[...])` resolved via `tools_dict` | |
| `roles.<name>.tasks.<task>.description/expected_output` | `Task(...)` | Templated with `{topic}` |
| `process: hierarchical` | `AgentTeam(process="hierarchical", manager_llm=...)` | |
| `manager_llm` | `AgentTeam(manager_llm=...)` | Falls back to model from `llm_config` |
| `memory: true` | `AgentTeam(memory=True)` | |

### Current Limitations

**Configuration gaps** not yet implemented:
- Per-role `llm` override — all agents use the model from `llm_config`
- Per-task `agent` selection — tasks are auto-assigned to their role's agent
- Custom `process` values other than `hierarchical` 

These limitations may be addressed in future updates. For advanced agent configurations, use the Python API directly.
