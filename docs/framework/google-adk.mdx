...
|-------|----------|--------------|
| `praisonai[google-adk]` | `praisonai-frameworks[google-adk]>=0.1.8`, `praisonai-tools>=0.1.0` | Probe + doctor recognition + adapter dispatch for Google ADK |
...
<AccordionGroup>
  ...
  <Accordion title="When to use handoffs">
    Handoff support is available since `praisonai-frameworks>=0.1.8` (PraisonAI PR #2507). Use `handoff.to` in your YAML when you need a router agent to delegate to specialists. Multi-task configs fall back to sequential execution. See [Handoffs](/features/handoffs) for the full conceptual model.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Agno" icon="robot" href="/framework/agno">
  ...
  <Card title="Pydantic AI" icon="shield" href="/framework/pydantic-ai">
