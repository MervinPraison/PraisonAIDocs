...
sidebarTitle: "Agno"
description: "Run the Agno agent framework through PraisonAI's YAML and CLI for single-task and sequential-task agents"
icon: "robot"
---
...
<Warning>
**Agno does not support YAML `handoff:` blocks.** The `AgnoAdapter` runtime card declares `supports_handoff=False` (`runtime_checks.py`). If you need multi-agent handoffs in YAML, choose `framework: praisonai`, `autogen_v4`, or `openai_agents` (see [OpenAI Agents page](/docs/framework/openai_agents) for the handoff YAML shape).
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="OpenAI Agents" icon="robot" href="/docs/framework/openai_agents">
    OpenAI Agents — choose this if you need handoffs
  </Card>
