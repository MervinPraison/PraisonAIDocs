...
CrewAI framework integration with PraisonAI provides collaborative multi-agent workflows with advanced configuration options for agents, tasks, and **native async execution**.

<Note>
Need a framework that isn't listed here? See [Framework Adapter Plugins](/features/framework-adapter-plugins) to register your own via Python entry points.
</Note>
...
| `max_rpm` | int | `None` | Requests per minute throttle. A positive int auto-builds a live `RateLimiter(requests_per_minute=max_rpm)` when no explicit `rate_limiter` is set, so CrewAI-style `max_rpm` now enforces throttling. `max_rpm <= 0` raises `ValueError`. | `max_rpm=` |
...
The CrewAI adapter runs natively async via `crew.kickoff_async()` — no worker thread is pinned, `asyncio.CancelledError` propagates, and multiple crews fan out cooperatively under `asyncio.gather`.
...
<Info>
Requires `crewai >= 0.28` (which introduced `Crew.kickoff_async()`). Older installs transparently fall back to the base thread-offload path — behaviour degrades instead of breaking ([PR #4909](https://github.com/MervinPraison/PraisonAI/pull/4909)).
</Info>

See [Async Crew Kickoff](/features/async-crew-kickoff) for the full FastAPI / Jupyter / concurrency patterns — everything on that page now applies to `framework: crewai` too.
...
<CardGroup cols={2}>
  <Card title="AutoGen" icon="robot" href="/framework/autogen">
