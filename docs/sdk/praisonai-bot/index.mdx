...
<Steps>
  <Step title="Install">
    ...
    ```
    <Tip>
    For the six-package overview, see [Installation](/installation).
    </Tip>
  </Step>
...
<Note>
`RunPolicy` (the safety gate) is not shipped in `praisonai-bot` — install the `praisonai` wrapper if you need it. See [Scheduled Run Policy](/features/scheduled-run-policy).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Gateway CLI" icon="server" href="/features/gateway-cli">
    Gateway operations
  </Card>
