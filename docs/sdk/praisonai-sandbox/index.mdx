...
description: "Standalone sandbox backends for PraisonAI — Docker, E2B, Modal, Daytona, Novita, Sandlock, SSH, subprocess"
icon: "shield-halved"
---
...
| `daytona` | `pip install "praisonai-sandbox[daytona]"` | `daytona-sdk>=0.1.0` (set `DAYTONA_API_KEY`) |
| `novita` | `pip install "praisonai-sandbox[novita]"` | `novita-sandbox>=2.0.0` (set `NOVITA_API_KEY`) |
| `all` | `pip install "praisonai-sandbox[all]"` | all of the above, including `daytona` and `novita` |
...
| `DaytonaSandbox` | `daytona` | Daytona cloud sandboxes (`[daytona]` + `DAYTONA_API_KEY`) |
| `NovitaSandbox` | `novita` | Novita cloud sandboxes (`[novita]` + `NOVITA_API_KEY`) |
...
Install extras: pip install praisonai-sandbox[docker,e2b,sandlock,ssh,modal,daytona,novita]
```
