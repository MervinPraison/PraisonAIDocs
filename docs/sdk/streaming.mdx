...
All `StreamEventType` values, in the order they fire during a typical run:

| Event | When it fires | Key fields |
|-------|--------------|------------|
| `REQUEST_START` | Before the API call | `timestamp` |
| `HEADERS_RECEIVED` | HTTP 200 headers received | `timestamp` |
| `FIRST_TOKEN` | First content delta (TTFT marker) | `timestamp` |
| `DELTA_TEXT` | Text content chunk | `content`, `is_reasoning` |
| `DELTA_TOOL_CALL` | Tool call delta (name/args chunk) | `tool_call` |
| `TOOL_CALL_START` | Tool execution starting (parsed args ready) | `tool_call`, `agent_id` |
| `TOOL_PROGRESS` | **Incremental output from a running tool** | `content`, `metadata.stream`, `metadata.progress`, `tool_call.name` |
| `TOOL_CALL_END` | Tool call stream-end marker | `tool_call` |
| `TOOL_CALL_RESULT` | Tool execution completed with result | `tool_call`, `metadata` |
| `LAST_TOKEN` | Final content delta | `timestamp` |
| `STREAM_END` | Stream completed successfully | `timestamp` |
| `ERROR` | Error during streaming | `error` |
| `STREAM_UNAVAILABLE` | Streaming not available in current config | — |
...
sequenceDiagram
    ...
    loop While tool runs
        T-->>C: TOOL_PROGRESS (live output)
    end
    T-->>A: Result
    A->>C: TOOL_CALL_END
    A->>C: TOOL_CALL_RESULT
    L-->>C: STREAM_END
```

### Tool lifecycle in a stream

When an agent calls a tool, four events bracket the execution. Use them to build loading spinners, progress bars, or live output panels:
...
def on_event(event: StreamEvent):
    if event.type == StreamEventType.TOOL_CALL_START:
        print(f"▶ {event.tool_call['name']} started")
    elif event.type == StreamEventType.TOOL_PROGRESS:
        print(f"  {event.content}", end="")
    elif event.type == StreamEventType.TOOL_CALL_END:
        print(f"✅ {event.tool_call['name']} finished")
    elif event.type == StreamEventType.DELTA_TEXT:
        print(event.content, end="")

agent = Agent(name="assistant", instructions="You are a helpful assistant")
agent.stream_emitter.add_callback(on_event)
agent.start("Run a quick test")
```

<Note>
`TOOL_PROGRESS` is emitted by tools that call `emit_tool_progress()` inside their implementation. Built-in tools and custom tools that use this function stream output line-by-line as they run. See [Tool Progress Streaming](/docs/features/tool-progress-streaming) for details.
</Note>
