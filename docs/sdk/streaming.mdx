...
def my_callback(event: StreamEvent):
    if event.type == StreamEventType.DELTA_TEXT:
        tag = "[thinking] " if event.is_reasoning else ""
        print(f"{tag}{event.content}", end="", flush=True)

async def main():
    agent = Agent(name="thinker", llm="gpt-5")
    agent.stream_emitter.add_callback(my_callback)
    await agent.astart("Solve: how many r's in strawberry?")

asyncio.run(main())
```
</CodeGroup>

Reasoning-summary deltas arrive on both sync (`start`/`chat`) and async (`astart`/`achat`) paths — the same callback works for both.

<Note>
`start(..., stream=True)` and `astart(...)` accept `max_tokens` and `top_p` alongside `temperature` — each is forwarded to the provider only when present. See [Streaming → Sampling knobs on the streaming path](/features/streaming#sampling-knobs-on-the-streaming-path).
</Note>
...
| Provider | Streaming | Reasoning | Async parity | Notes |
|----------|-----------|-----------|--------------|-------|
| OpenAI (Responses API) | ✅ | ✅ | ✅ | `gpt-5`, `o3`, `o4-mini`, reasoning-capable `gpt-4.1` tiers |
| OpenAI (Chat Completions) | ✅ | ✅ | ✅ | `o1`, `o1-mini` |
| LiteLLM | ✅ | ✅ | ✅ | Via `provider_specific_fields.reasoning_content` |
| Gemini | ✅ | ❌ | — | Standard streaming only |

<Tip>
`is_reasoning=True` is emitted on both sync and async streaming paths. On async Responses-API runs, reasoning-summary text arrives as `DELTA_TEXT` deltas with `is_reasoning=True`, identical to the sync path.
</Tip>
