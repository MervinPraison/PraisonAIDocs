...
|-----------|------|----------|---------|
| `approved` | `bool` | Yes | `-` |
| `modified_args` | `Optional[Dict]` | No | `None` |
| `reason` | `str` | No | `''` |
...
```python
def console_approval_callback(function_name) -> ApprovalDecision
```
...
```python
async def request_approval(function_name) -> ApprovalDecision
```
...
| Name | Value |
|------|-------|
| `DEFAULT_DANGEROUS_TOOLS` | `\{'execute_command': 'critical', 'kill_process': 'c` |
