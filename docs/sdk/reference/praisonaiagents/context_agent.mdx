...
| `project_path` | `Optional[str]` | No | `None` |
...
- **extract_implementation_patterns**(`project_path`) → `Dict[str, Any]`
...
- **generate_comprehensive_prp**(`feature_request`) → `str`
...
- **build_implementation_blueprint**(`feature_request`) → `Dict[str, Any]`
  Build detailed implementation blueprint following PRD template.
- **create_quality_gates**(`requirements`) → `Dict[str, Any]`
...
- **generate_prp**(`feature_request`) → `str`
  Protocol-compatible alias for generate_comprehensive_prp.
- **create_implementation_blueprint**(`feature_request`) → `Dict[str, Any]`
  Protocol-compatible alias for build_implementation_blueprint.
- **async aanalyze_codebase**(`project_path: str`) → `Dict[str, Any]`
  Async version of analyze_codebase.
- **async agenerate_prp**(`feature_request`) → `str`
  Async version of generate_prp.
- **async acreate_implementation_blueprint**(`feature_request`) → `Dict[str, Any]`
...
```python
def create_context_agent(llm) -> ContextAgent
```
