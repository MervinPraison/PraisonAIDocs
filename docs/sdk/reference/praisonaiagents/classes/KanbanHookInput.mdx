...
<ResponseField name="board" type="str">
  The store's board name, or `"default"`. Always present.
</ResponseField>

<ResponseField name="status" type="str">
  Post-transition status (pre-transition status for `FAILED` when the circuit did not trip).
</ResponseField>

<ResponseField name="assignee" type="Optional[str]">
  `worker_id` for `CLAIMED`/reclaim; current assignee for `MOVED`/`DONE`/`BLOCKED`.
</ResponseField>
