...
When you pass both `backend=` and `config=`, an explicit `provider` or `backend` key inside `config=` wins; otherwise the outer `backend=` fills in. The `config` dict you pass is never mutated — a provider key, if injected, is written to an internal copy, so the same dict is safe to share across agents.

See [Memory → Combining `backend` and `config`](/features/memory#combining-backend-and-config) for the full precedence table.
...
<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-agents/praisonaiagents/config/feature_configs.py#L226">
  `praisonaiagents/config/feature_configs.py` at line 226
</Card>
