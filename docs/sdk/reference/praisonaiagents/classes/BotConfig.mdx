...
<ResponseField name="ack_scope" type="str">
  Which inbound messages get an ack reaction. One of `off`, `direct`, `group-mentions`, `group-all`, or `all`. Default `group-mentions`: ack DMs and group mentions/replies, but stay quiet on ambient group chatter the bot won't answer.
</ResponseField>
