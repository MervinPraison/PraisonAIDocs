---
title: "Create Session • AI Agent SDK"
sidebarTitle: "Create Session"
description: "create_session: Create a new session."
icon: "function"
---

# create_session

<div className="flex items-center gap-2">
  <Badge color="purple">Method</Badge>
</div>

> This is a method of the [**GatewayProtocol**](../classes/GatewayProtocol) class in the [**protocols**](../modules/protocols) module.

Create a new session.

## Signature

```python
def create_session(agent_id: str, client_id: Optional[str], session_id: Optional[str]) -> GatewaySessionProtocol
```

## Parameters

<ParamField query="agent_id" type="str" required={true}>
  The agent to handle this session
</ParamField>

<ParamField query="client_id" type="Optional[str]" required={false}>
  Optional client ID
</ParamField>

<ParamField query="session_id" type="Optional[str]" required={false}>
  Optional custom session ID
</ParamField>

### Returns

<ResponseField name="Returns" type="GatewaySessionProtocol">
  The created session
</ResponseField>


## Source

<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-agents/praisonaiagents/gateway/protocols.py#L1019">
  `praisonaiagents/gateway/protocols.py` at line 1019
</Card>



---

## Related Documentation

<CardGroup cols={2}>
  <Card title="Session Management" icon="clock" href="/docs/concepts/session-management" />
  <Card title="Sessions Feature" icon="folder" href="/docs/features/sessions" />
  <Card title="Session Persistence" icon="database" href="/docs/features/session-persistence" />
</CardGroup>
