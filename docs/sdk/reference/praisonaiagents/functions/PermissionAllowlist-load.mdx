---
title: "load • AI Agent SDK"
sidebarTitle: "load"
description: "load: API reference for PermissionAllowlist.load"
icon: "function"
---

# load

<div className="flex items-center gap-2">
  <Badge color="purple">Method</Badge>
</div>

> This is a method of the [**PermissionAllowlist**](../classes/PermissionAllowlist) class in the [**approval**](../modules/approval) module.



## Signature

```python
def load(filepath: str) -> 'PermissionAllowlist'
```

## Parameters

<ParamField query="filepath" type="str" required={true}>
  No description available.
</ParamField>

### Returns

<ResponseField name="Returns" type="'PermissionAllowlist'">
  The result of the operation.
</ResponseField>


## Uses

- `cls`
- `exists`
- `json.load`
- `allowlist.add_tool`


## Used By

- [`PermissionAllowlist.load`](../functions/PermissionAllowlist-load)
- [`FileEmergencyStop.state`](../functions/FileEmergencyStop-state)
- [`FileTracker.load`](../functions/FileTracker-load)
- [`Knowledge.index`](../functions/Knowledge-index)
- [`ToolRegistry.discover_plugins`](../functions/ToolRegistry-discover_plugins)


## Source

<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-agents/praisonaiagents/approval/__init__.py#L342">
  `praisonaiagents/approval/__init__.py` at line 342
</Card>
