...
<Note>
This module exists purely to satisfy the parity tracker's **exported-name** check — export names here must match the Python SDK exactly so the tracker detects them. Presence of an export on this page means the name is part of the public surface; it is **not** a promise that the capability is reachable, wired up, or behaves like its Python counterpart. See the [Documentation Style Guide](/docs/guides/documentation-style-guide#multi-sdk-reference) for the difference between `PARITY.md` (code-vs-code) and `DOCS_PARITY.md` (docs-vs-code).
</Note>
