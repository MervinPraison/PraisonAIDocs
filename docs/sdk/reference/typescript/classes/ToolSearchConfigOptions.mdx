---
title: "Tool Search Config Options • TypeScript AI Agent SDK"
sidebarTitle: "Tool Search Config Options"
description: "ToolSearchConfigOptions: TypeScript ToolSearchConfigOptions class"
icon: "brackets-curly"
---

# ToolSearchConfigOptions

> Defined in the [**config**](../modules/config) module.

<Badge color="green">TypeScript AI Agent</Badge>

TypeScript ToolSearchConfigOptions class

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#8B0000', 'primaryTextColor': '#fff', 'primaryBorderColor': '#710101', 'lineColor': '#189AB4', 'secondaryColor': '#189AB4', 'tertiaryColor': '#fff' }}}%%

graph LR
    agent["Agent"] -- "uses" --> tool["Tool: ToolSearchConfigOptions"]
    tool -- "returns" --> result["Result"]
    style tool fill:#189AB4,color:#fff
    style agent fill:#8B0000,color:#fff
    style result fill:#8B0000,color:#fff
```


## Source

<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-ts/src/config/index.ts#L1332">
  `src/config/index.ts` at line 1332
</Card>



---

## Related Documentation

<CardGroup cols={2}>
  <Card title="JS Tools" icon="wrench" href="/docs/js/tools" />
  <Card title="JS Tools CLI" icon="terminal" href="/docs/js/tools-cli" />
  <Card title="JS Custom Tools" icon="plus" href="/docs/js/customtools" />
  <Card title="JS MCP Tools" icon="plug" href="/docs/js/mcp-tools" />
  <Card title="JS Deep Research" icon="search" href="/docs/js/deep-research" />
</CardGroup>
