---
title: "aembed • TypeScript AI Agent SDK"
sidebarTitle: "aembed"
description: "aembed: API reference for aembed"
icon: "function"
---

# aembed

<div className="flex items-center gap-2">
  <Badge color="blue">Async</Badge>
  <Badge color="teal">Function</Badge>
</div>

> This function is defined in the [**embeddings**](../modules/embeddings) module.



## Signature

```python
async def aembed(text: string, config?: EmbeddingConfig) -> Promise<EmbeddingResult>
```

### Returns

<ResponseField name="Returns" type="Promise<EmbeddingResult>">
  The result of the operation.
</ResponseField>


## Uses

- `import`
- `embedAsync`


## Used By

- [`aembedding`](../functions/aembedding)


## Source

<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-ts/src/embeddings/index.ts#L152">
  `src/embeddings/index.ts` at line 152
</Card>



---

## Related Documentation

<CardGroup cols={2}>
  <Card title="JS Embeddings" icon="code" href="/docs/js/embeddings" />
  <Card title="JS Embeddings CLI" icon="terminal" href="/docs/js/embeddings-cli" />
</CardGroup>
