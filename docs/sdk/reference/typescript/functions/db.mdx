---
title: "DB • TypeScript AI Agent SDK"
sidebarTitle: "DB"
description: "db: API reference for db"
icon: "function"
---

# db

<div className="flex items-center gap-2">
  <Badge color="teal">Function</Badge>
</div>

> This function is defined in the [**db**](../modules/db) module.



## Signature

```python
def db(configOrUrl: string | DbConfig = { type: 'memory' }) -> DbAdapter
```

### Returns

<ResponseField name="Returns" type="DbAdapter">
  The result of the operation.
</ResponseField>


## Uses

- `parseDbUrl`
- `createDbAdapter`


## Used By

- [`createDbAdapter`](../functions/createDbAdapter)


## Source

<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-ts/src/db/index.ts#L177">
  `src/db/index.ts` at line 177
</Card>



---

## Related Documentation

<CardGroup cols={2}>
  <Card title="JS Database" icon="database" href="/docs/js/database" />
  <Card title="JS NL Postgres" icon="database" href="/docs/js/nl-postgres" />
</CardGroup>
