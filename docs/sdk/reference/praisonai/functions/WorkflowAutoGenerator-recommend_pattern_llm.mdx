---
title: "Recommend Pattern LLM • AI Agents Framework"
sidebarTitle: "Recommend Pattern LLM"
description: "recommend_pattern_llm: Use LLM to recommend the best workflow pattern with reasoning."
icon: "function"
---

# recommend_pattern_llm

<div className="flex items-center gap-2">
  <Badge color="purple">Method</Badge>
</div>

> This is a method of the [**WorkflowAutoGenerator**](../classes/WorkflowAutoGenerator) class in the [**auto**](../modules/auto) module.

Use LLM to recommend the best workflow pattern with reasoning.

## Signature

```python
def recommend_pattern_llm(topic: Optional[str]) -> Any
```

## Parameters

<ParamField query="topic" type="Optional[str]" required={false}>
  The task description (uses self.topic if not provided)
</ParamField>

### Returns

<ResponseField name="Returns" type="Any">
  Pattern with reasoning and confidence score
</ResponseField>


## Source

<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai/praisonai/auto.py#L1463">
  `praisonai/auto.py` at line 1463
</Card>



---

## Related Documentation

<CardGroup cols={2}>
  <Card title="Models Overview" icon="microchip" href="/docs/models" />
  <Card title="LLM Configuration" icon="gear" href="/docs/configuration/llm-config" />
  <Card title="Model Router" icon="route" href="/docs/features/model-router" />
  <Card title="Model Failover" icon="shield" href="/docs/features/model-failover" />
</CardGroup>
