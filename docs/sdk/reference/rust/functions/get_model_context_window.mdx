---
title: "Get Model Context Window • Rust AI Agent SDK"
sidebarTitle: "Get Model Context Window"
description: "get_model_context_window: Get model context window size."
icon: "function"
---

# get_model_context_window

<div className="flex items-center gap-2">
  <Badge color="teal">Function</Badge>
</div>

> This function is defined in the [**rag**](../modules/rag) module.

Get model context window size.

## Signature

```python
def get_model_context_window(model: &str) -> usize
```

## Parameters

<ParamField query="model" type="&str" required={true}>
  No description available.
</ParamField>

### Returns

<ResponseField name="Returns" type="usize">
  The result of the operation.
</ResponseField>


## Uses

- `contains`


## Source

<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-rust/praisonai/src/rag/mod.rs#L408">
  `praisonai/src/rag/mod.rs` at line 408
</Card>



---

## Related Documentation

<CardGroup cols={2}>
  <Card title="Rust LLM" icon="microchip" href="/docs/rust/llm" />
  <Card title="Rust Providers" icon="server" href="/docs/rust/providers" />
  <Card title="Rust Context Management" icon="layer-group" href="/docs/rust/context-management" />
  <Card title="Rust Token Management" icon="coins" href="/docs/rust/token-management" />
</CardGroup>
