---
title: "Estimate Tool Schema Tokens • Rust AI Agent SDK"
sidebarTitle: "Estimate Tool Schema Tokens"
description: "estimate_tool_schema_tokens: Estimate tokens for tool schemas."
icon: "function"
---

# estimate_tool_schema_tokens

<div className="flex items-center gap-2">
  <Badge color="teal">Function</Badge>
</div>

> This function is defined in the [**context**](../modules/context) module.

Estimate tokens for tool schemas.

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#8B0000', 'primaryTextColor': '#fff', 'primaryBorderColor': '#710101', 'lineColor': '#189AB4', 'secondaryColor': '#189AB4', 'tertiaryColor': '#fff' }}}%%

graph LR
    agent["Agent"] -- "uses" --> tool["Tool: estimate_tool_schema_tokens"]
    tool -- "returns" --> result["Result"]
    style tool fill:#189AB4,color:#fff
    style agent fill:#8B0000,color:#fff
    style result fill:#8B0000,color:#fff
```

## Signature

```python
def estimate_tool_schema_tokens(tools: &[serde_json::Value]) -> usize
```

## Parameters

<ParamField query="tools" type="&" required={true}>
  No description available.
</ParamField>

### Returns

<ResponseField name="Returns" type="usize">
  The result of the operation.
</ResponseField>


## Uses

- `unwrap_or_default`
- `estimate_tokens_heuristic`


## Source

<Card title="View on GitHub" icon="github" href="https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-rust/praisonai/src/context/mod.rs#L344">
  `praisonai/src/context/mod.rs` at line 344
</Card>



---

## Related Documentation

<CardGroup cols={2}>
  <Card title="Rust Tools" icon="wrench" href="/rust/tools" />
  <Card title="Rust Code Execution" icon="terminal" href="/rust/code-execution" />
  <Card title="Rust Web Search" icon="search" href="/rust/web-search" />
  <Card title="Rust Token Management" icon="coins" href="/rust/token-management" />
  <Card title="Rust Budget" icon="wallet" href="/rust/budget" />
</CardGroup>
