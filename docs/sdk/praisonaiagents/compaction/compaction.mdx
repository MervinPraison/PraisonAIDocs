...
#### CompactionStrategy.from_policy_value

Classmethod. Translates a `context.protocols.CompactionStrategy` policy value into the compactor's strategy member. This is the single source of truth for the mapping — prefer it over recomputing the dict at each call site.
...
| any other value | `CompactionStrategy.PRUNE` (safe fallback) |
...
CompactionStrategy.from_policy_value("summarise")
...
- [Session](/sdk/praisonaiagents/session) - Session management
- [Memory](/sdk/praisonaiagents/memory/memory) - Long-term memory
