...
| Provider | Name | Type | Install |
|----------|------|------|---------|
| In-Memory | `memory` | Shipped | Built-in |
| ChromaDB | `chroma` | Shipped | `pip install chromadb` |
| Pinecone | `pinecone` | Plugin | `pip install pinecone-client` |
| Qdrant | `qdrant` | Plugin | `pip install qdrant-client` |
| Weaviate | `weaviate` | Plugin | `pip install weaviate-client` |

<Note>
As top-level Knowledge providers, only `chroma` (and the built-in `memory`) ship out of the box. Plugin providers need `register_knowledge_adapter(name, adapter_cls)` before use — see [Custom Knowledge Adapters](/features/knowledge-custom-adapters). They also work as inner backends of mem0 without registration.
</Note>
