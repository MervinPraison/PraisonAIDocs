...
`RerankerRegistry` is a thread-safe singleton — `register`, `get`, `list_rerankers`, and `clear` are all guarded by an internal lock, and concurrent `get_reranker_registry()` calls always return the same instance.
...
- `RerankerRegistry` operations acquire a lightweight lock — negligible overhead in single-threaded use, no `KeyError` or iteration errors under concurrent register/clear
