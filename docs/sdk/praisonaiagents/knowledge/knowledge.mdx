...
config = {
    "vector_store": {
        "provider": "chroma",  # Shipped: chroma. Plugin (register first): qdrant, pinecone, weaviate
        "config": {
...
`path` is optional. Omit it and the store persists under an absolute per-project directory: `<project>/.praisonai/knowledge/chroma`.
