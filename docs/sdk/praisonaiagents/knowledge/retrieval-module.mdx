...
`RetrieverRegistry` is a thread-safe singleton — `register`, `get`, `list_retrievers`, and `clear` are all guarded by an internal lock, and concurrent `get_retriever_registry()` calls always return the same instance.

<Note>
Safe to call from multiple agents at once — no external locking required. Since PraisonAI [#5197](https://github.com/MervinPraison/PraisonAI/pull/5197).
</Note>
...
- `RetrieverRegistry` operations acquire a lightweight lock — negligible overhead in single-threaded use, no `KeyError` or iteration errors under concurrent register/clear
