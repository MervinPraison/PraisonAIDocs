...
agent = Agent(name="Analyst", instructions="Analyse files using tools")

try:
    agent.start("Analyse this file")
except LLMResponseError as e:
    print(f"LLM loop failed: {e}")
...
graph LR
    subgraph "Before"
        A1[LLM.get_response] --> B1{tool loop iter fails}
        B1 -->|Exception| C1[log + break]
        C1 --> D1["return '' (silent)"]
    end

    subgraph "After"
        A2[LLM.get_response] --> B2{tool loop iter fails}
        B2 -->|Exception| C2[raise LLMResponseError from e]
        C2 --> D2[caller catches / surfaces error]
    end
...
- [Agent](/sdk/praisonaiagents/agent/agent) - Using LLM with agents
- [Telemetry](/sdk/praisonaiagents/telemetry) - Token tracking
