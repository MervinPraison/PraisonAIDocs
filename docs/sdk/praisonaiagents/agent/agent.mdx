...
| **URL** | `memory="mongodb://localhost:27017"` | Backend-specific config |
| **Dict** | `memory={"backend": "mongodb"}` | Custom config as dict |
...
# Dict
agent = Agent(instructions="...", memory={"backend": "mongodb", "user_id": "u1"})
...
<Note>
For Redis/Postgres, use `memory=MemoryConfig(db=db(url=...))` — see [Redis persistence](/features/persistence-redis). `MemoryConfig(backend="redis")` now raises a `ValueError`.
</Note>
...
| `"mem0"` | Mem0 service |
| `"mongodb"` | MongoDB |
| `"dakera"` | Dakera memory server |
| `"in_memory"` | In-process store |
...
- [Agents Module](/sdk/praisonaiagents/agents/agents) - Multi-agent orchestration
- [Handoff Module](/sdk/praisonaiagents/handoff/handoff) - Agent-to-agent handoffs
- [Tools Module](/sdk/praisonaiagents/tools/tools) - Tool system
- [Memory Module](/sdk/praisonaiagents/memory/memory) - Memory management
- [Knowledge Module](/sdk/praisonaiagents/knowledge/knowledge) - Knowledge bases