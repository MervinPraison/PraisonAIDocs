...
<Note>
`aexecute()` has full parity with `execute()`. `loop_over` iteration, `branch_condition` / `next_steps` routing, and `checkpoint` / `resume` all work identically on the async path.
</Note>
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `steps` | `List` | — | Steps to execute concurrently |
...
| Strategy | Behavior |
|----------|----------|
| `"partial_ok"` | Continue with partial results. Failed branches return `"Error: <msg>"` |
| `"fail_fast"` | Cancel remaining branches and raise `WorkflowStepError` on first failure |
| `"fail_all"` | Wait for all branches, then raise `WorkflowStepError` if any failed |
...
workflow = AgentFlow(steps=[
    parallel([research_market, research_competitors, research_customers], on_failure="fail_fast"),
    # summarize_results never runs if any parallel step fails
])

# Handle errors
try:
    result = workflow.start("Research request")
