...
agent = Agent(instructions="Write content", guardrails=validate)
```
...
agent = Agent(instructions="...", guardrails=content_length_guardrail)
```

### Config Instance

```python
from praisonaiagents.config import GuardrailConfig
...
# Use in agent
agent = Agent(instructions="...", guardrails=guardrail)
```
...
agent = Agent(instructions="Write content", guardrails=validate_length)
```

### LLM-based Guardrail

```python
from praisonaiagents import Agent, Task, Agents, LLMGuardrail
