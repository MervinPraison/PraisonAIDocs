...
<Note>
For reranker registry lookups, call `register_default_adapters()` once at startup — importing reranker classes no longer auto-registers them (PraisonAI 4.6.156+). See [Registering the built-in adapters](/docs/sdk/praisonai/adapters#registering-the-built-in-adapters).
</Note>
