...
<Note>
Wrapper entry points (`praisonai.run`, `praisonai.arun`, and the `praisonai` CLI) call `register_default_adapters()` for you before the agent starts, so `reranker: llm` (etc.) works from YAML/CLI without any Python setup. Call it explicitly only when using reranker classes directly from your own Python code (PraisonAI 4.7.9+). See [Registering the built-in adapters](/docs/sdk/praisonai/adapters#registering-the-built-in-adapters).
</Note>
...
Async reranking that offloads the blocking body via `asyncio.to_thread(...)`, so calling it from an event loop no longer freezes it. Authors of custom rerankers on a natively-async provider should override `arerank` and `await` the native call directly — see [Native async in custom retrievers / rerankers](/docs/sdk/praisonai/adapters#native-async-in-custom-retrievers-rerankers).
...
- [Retrieval Module](/sdk/praisonai/retrieval) - Retrieve documents
