...
description: "Programmatic access to PraisonAI templates"
icon: "code"
---
...
`resolve_tools()` delegates name resolution to [`ToolResolver`](/features/tool-resolver) (PR #2059). The `registry=` argument is preserved for backward compatibility — it sits on top of the canonical five-source chain (local `tools.py` → wrapper `ToolRegistry` → `praisonaiagents.tools` → `praisonai-tools` → core SDK plugins). See [Tools Resolve](/cli/tools-resolve) for CLI usage.
