...
| `client` | Lazily-constructed shared `httpx.Client` bound to `api_url` — reused by every `get_status()` / `get_result()` / `cancel()` / `wait()` poll. |
| `get_status()` | Fetch current status from API |
| `get_result()` | Fetch job result from API |
| `cancel()` | Cancel the job |
| `wait(poll_interval, timeout)` | Wait for completion (reuses one keep-alive connection across all polls) |
| `close()` | Close the shared HTTP client and release its connection pool |
| `__enter__` / `__exit__` | Context-manager protocol — calls `close()` on scope exit |
...
with recipe.submit_job("my-recipe", input="test") as job:
    result = job.wait(poll_interval=5, timeout=300)
    print(result)
# `job.client` is closed here — pooled sockets are released.
```
...
`recipe_name` and `recipe_config` are honoured by the router as of PR [#4085](https://github.com/MervinPraison/PraisonAI/pull/4085); earlier versions accepted the fields but did not forward them to the executor.

If your code creates its own client to hit the Jobs API directly (rather than using `submit_job`), keep the client alive across calls — the wrapper's own `JobHandle` does exactly this.
...
| `PRAISONAI_JOBS_API_KEY` | *(unset)* | When set, all requests except `/health` must send the key via `Authorization: Bearer <key>` or `X-API-Key: <key>`. |
| `PRAISONAI_JOBS_BIND_HOST` | 127.0.0.1 | Bind host for the auth guard. Any value outside `{127.0.0.1, localhost, ::1}` requires `PRAISONAI_JOBS_API_KEY`. |
...
- [Async Jobs CLI](/sdk/praisonai/cli/async-jobs-cli) - CLI commands for jobs
- [Background Tasks](/sdk/praisonai/background-tasks) - In-process background execution
- [Scheduler](/sdk/praisonai/scheduler) - Periodic job scheduling
