...
`register_default_adapters()` wires the wrapper's default readers, retrievers, and rerankers into the core-SDK registries. The wrapper entry points call it for you; library-mode Python callers call it themselves.
...
<Note>
As of PraisonAI 4.7.9, the wrapper's entry points (`praisonai.run`, `praisonai.arun`, and the `praisonai` CLI) call `register_default_adapters()` for you before the agent starts. A YAML/CLI user who declares `retriever: fusion` or `reranker: llm` in their agent config gets it wired up automatically.

Call `register_default_adapters()` explicitly only when you are importing `praisonai.adapters.retrievers` / `rerankers` from your own Python code (not via the wrapper entry points) and want the registry populated.
</Note>

**Behaviour**

- Idempotent and thread-safe — subsequent calls are no-ops.
- `register_default_*` helpers use **register-only-if-absent** semantics. A multi-tenant host that has already registered a custom retriever/reranker/reader under a built-in name (e.g. its own `fusion`) keeps that custom entry — the wrapper defaults never silently overwrite it.
- Composite readers (`DirectoryReader.load`, `GlobReader.load`, `AutoReader.load`) auto-wire the built-in readers on first `.load()` call, so a reader-only workflow works without the explicit call. Retrievers and rerankers have no equivalent safety net — call `register_default_adapters()` if you look them up by name from the registry from your own Python code.
...
- [Readers Module](/sdk/praisonai/readers) - Document loading
- [Vector Store Module](/sdk/praisonai/vector_store) - Vector storage
- [Retrieval Module](/sdk/praisonai/retrieval) - Document retrieval
- [Reranker Module](/sdk/praisonai/reranker) - Result reranking
