...
<Note>
An `at:` string that already carries an offset (`at:2026-03-01T09:00:00+05:30`, `at:2026-03-01T09:00:00Z`) is unaffected — the offset in the string always wins. See [Schedule Tools → Timezones](/tools/schedule-tools#timezones-for-at-and-cron).
</Note>
...
- `SchedulerProviderProtocol` — trigger backend protocol deciding *when* jobs fire; `ScheduleLoop` (aliased `InProcessScheduleProvider`) is the default. See [Scheduler Providers](/features/scheduler-providers).
...
See [Scheduled Run Policy](/features/scheduled-run-policy) for the full reference.
...
- [Scheduler Pre-Run Gate](/features/scheduler-pre-run-gate) - Full user-facing documentation
- [Scheduled Run Policy](/features/scheduled-run-policy) - Safety gate for unattended runs
- [praisonai-bot SDK](/sdk/praisonai-bot/index) - Bot-tier package reference
- [Deploy Module](/sdk/praisonai/deploy) - Deployment functionality
- [CLI Scheduler](/cli/scheduler) - CLI scheduling commands
