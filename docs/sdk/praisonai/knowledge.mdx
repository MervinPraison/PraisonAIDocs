...
from praisonai.adapters import (
    ...
    FusionRetriever,
    LLMReranker,
    register_default_adapters,   # only needed for library-mode Python; wrapper entry points auto-call it
)
# register_default_adapters()  # call this only when NOT going through praisonai.run / praisonai.arun / the CLI
```

<Note>
Wrapper entry points (`praisonai.run`, `praisonai.arun`, and the `praisonai` CLI) call `register_default_adapters()` for you before the agent starts, so `retriever: fusion` / `reranker: llm` in YAML resolve automatically (PraisonAI 4.7.9+). `AutoReader.load()` auto-wires the built-in readers on first call, so the reader example below works untouched. Call `register_default_adapters()` yourself only when looking up retrievers or rerankers by name from your own Python code.
</Note>
...
- [Retrieval Module](/sdk/praisonai/retrieval) - Retrieval strategies
- [Reranker Module](/sdk/praisonai/reranker) - Result reranking
- [Adapters Module](/sdk/praisonai/adapters) - All adapters overview
