...
- **AutoGen**
- **AutoGen v4**
- **OpenAI Agents** (via `praisonai-frameworks[openai-agents]`)
...
<Note>
Install with `pip install "praisonai[openai-agents]"`. Requires `OPENAI_API_KEY`. See [OpenAI Agents framework](/docs/framework/openai_agents) for handoffs and task chaining.
</Note>
