...
Interactive `chat` and `code` open their resident TUI standalone as of PR #3818. Default `run` and every `--output` text mode also work standalone in-process (PR #2853). The wrapper (`pip install praisonai`) adds only the richer legacy dispatch for `code` and the gateway/bot channels.
