...
`chat`, `code`, and every `run` output mode work on a `praisonai-code`-only install (PR #2853, PR #3818). Default `run` and every `--output` text mode also work standalone in-process. The wrapper (`pip install praisonai`) adds only the richer legacy dispatch for `code` and the gateway/bot channels.
