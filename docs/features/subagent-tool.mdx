...
| `agent_resolver` | `Callable[[str], Agent]` | `None` | Optional callback that returns a pre-configured `Agent` for a name — used to delegate to named `.praisonai/agents` definitions under their own model/tools/permissions. |
| `resolvable_agents` | `Dict[str, str]` | `None` | `{name: description}` map of delegatable agents; each entry appears in the tool description so the model can pick one. |
