...
`resolvable_agents` enriches the tool description so the model knows which named agents it may delegate to. The CLI wires both parameters for you from `.praisonai/agents/*.md` — see [Named Agent Delegation](/docs/features/named-agent-delegation) as the primary entry point.
...
The `deliver` vocabulary mirrors the scheduling tool's `deliver=` parameter. See [JOB_COMPLETED Hook](/docs/features/job-completed-hook) for the hook that fires on completion.
...
See [Named Agent Delegation](/docs/features/named-agent-delegation) for the full CLI-first story.
...
<Note>
When a call is served by an `agent_resolver` (named agent delegation), `agent_name` in the result is the resolved named agent (e.g. `"researcher"`), not `"subagent"`. See [Named Agent Delegation](/docs/features/named-agent-delegation).
</Note>
