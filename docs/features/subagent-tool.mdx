...
The Subagent Tool lets a parent agent spawn specialized subagents for specific tasks — synchronously, or in the **background** so the parent keeps working and collects the result later.
...
result = func(
    ...
    background=False  # Set True for fire-and-forget
)
```
...
| `background` | `bool` | No | When `True`, run on the background job runner and return immediately with a `job_id`. Default `False`. |
...
# handle == {"success": True, "job_id": "...", "status": "running", ...}
job_id = handle["job_id"]
```
</Step>

<Step title="Poll or wait for the result">
```python
...
if not result["success"]:
    ...
    # The job completed but the subagent itself reported failure
...
graph TB
    ...
    Q -->|No| S[Use synchronous<br/>spawn_subagent]
...
**Synchronous (`background=False`, default):**
...
**Background (`background=True`) — handle returned immediately:**

```python
handle = func(task="Run tests", background=True)
# {
#     "success": True,
#     "job_id": "<id>",
#     "status": "running",
...
**`subagent_result(job_id)` — non-blocking poll while still running:**

```python
{"success": True, "job_id": "<id>", "status": "running"}
```

**`subagent_result(job_id, wait=True)` — on completion:**
...
**`subagent_result` on failed job:**

```python
{"success": False, "job_id": "<id>", "status": "failed", "error": "<message>"}
```

**`subagent_result` on unknown `job_id`:**

```python
{"success": False, "job_id": "<id>", "error": "Job '<id>' not found"}
```
...
<Accordion title="Use background mode for slow work">
Kick off slow work (full test runs, broad scans, big refactors) with `background=True` so the parent agent can keep making progress. Collect with `subagent_result(job_id, wait=True)` when you actually need the answer.
</Accordion>
