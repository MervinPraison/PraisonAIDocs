...
The Subagent Tool lets a parent agent spawn specialised subagents for specific tasks — controlled by model, permission mode, and allowed agent types.
...
Calls to `spawn_subagent` return the final result directly:
...
<Accordion title="Break large tasks into focused subtasks">
For complex multi-step work, delegate each concern to a specialised agent type — `explorer` for discovery, `coder` for implementation, `reviewer` for quality checks.
</Accordion>
