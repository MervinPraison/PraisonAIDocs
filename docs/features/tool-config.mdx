...
<Note>
`ToolConfig.parallel` is a **deprecated alias** for `ExecutionConfig.parallel_tool_calls`, the single source of truth for running batched LLM tool calls in parallel. Prefer `execution=ExecutionConfig(parallel_tool_calls=True)`. `ToolConfig(parallel=True)` still works, but do not set both spellings to conflicting values — that raises `TypeError`. As of [PraisonAI PR #4634](https://github.com/MervinPraison/PraisonAI/pull/4634), `parallel_tool_calls` takes effect on `agent.achat()` / `agent.astart()` too — earlier releases silently ran the async tool loop sequentially.
</Note>
...
<Card title="ToolConfig SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/feature_configs">
...
| `parallel` | `bool` | `False` | **Deprecated alias** for `ExecutionConfig.parallel_tool_calls`. Prefer `execution=ExecutionConfig(parallel_tool_calls=True)`. Setting both to conflicting values raises `TypeError`. The flag is respected on both the sync (`chat()` / `start()`) and async (`achat()` / `astart()`) tool loops as of [PR #4634](https://github.com/MervinPraison/PraisonAI/pull/4634). |
...
| `allow_global_tools` | `bool` | `False` | Allow resolving tool calls via the process-global `@tool` registry when a tool name is not in this agent's declared `tools=[...]`. Off by default so an agent is scoped strictly to its own tools (safe by default); opt in for plugin-host style dynamic dispatch. |

<Info>
`ToolConfig.timeout` applies uniformly to async tools regardless of whether they run through the direct executor path or through the tool middleware chain (`@wrap_tool_call` and friends). Before [PR #5080](https://github.com/MervinPraison/PraisonAI/pull/5080), the middleware async path silently ignored the timeout — that gap is fixed and no configuration change is required.
</Info>
...
agent = Agent(
    ...
    tool_config=ToolConfig(timeout=15),          # Fail fast if any tool is slow
    execution=ExecutionConfig(parallel_tool_calls=True),  # Run all three at once
)
...
```python
# Raises TypeError: both name the same setting and they disagree.
Agent(
...
# @tool auto-registers into the process-global registry at import time.
@tool
def get_weather(city: str) -> str:
    """Return the current weather for the given city."""
    return f"Sunny in {city}"

# Opt in — this agent can now discover @tool-decorated callables
# even though it did not list them in tools=[...]
plugin_host = Agent(
    name="PluginHost",
    instructions="Use any registered tool the user asks for.",
    tool_config=ToolConfig(allow_global_tools=True),
)
plugin_host.start("What's the weather in Paris?")
```
...
<Accordion title="Keep allow_global_tools=False unless you are a plugin host">
...
<CardGroup cols={2}>
<Card title="Artifact Storage" icon="box" href="/features/artifact-storage">
...
<Card title="Tool Retry Policy" icon="rotate-cw" href="/features/tool-retry-policy">
