...
| `parallel` | `bool` | `False` | **Deprecated alias** for `ExecutionConfig.parallel_tool_calls`. Prefer `execution=ExecutionConfig(parallel_tool_calls=True)`. Setting both to conflicting values raises `TypeError`. |
