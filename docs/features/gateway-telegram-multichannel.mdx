...
```mermaid
graph LR
    subgraph "Multi-Channel Telegram Gateway"
        Request[📋 User Request] --> Process[⚙️ Multi-Channel Telegram Gateway]
...
<Note>
A typo in a `routes:` target now fails at load time — see [Fail-Fast Validation](/docs/features/gateway-route-bindings#fail-fast-validation).
</Note>

<Note>
**One token per bot — never reuse a token across channels.** Give each channel (`telegram_cfo`, `telegram_ops`, …) its own bot token from @BotFather. Telegram permits only one active `getUpdates` poller per token, so reusing a token across two channels — or across a gateway and a standalone `praisonai bot telegram` / Claw Channels worker — makes the second poller fail fast with `RuntimeError: Another process is already polling this Telegram token`. See [Bot CLI 409 troubleshooting](/cli/bot#409-conflict-bot-stops-replying).
</Note>
