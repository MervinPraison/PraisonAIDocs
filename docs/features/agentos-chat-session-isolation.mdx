...
<Note>
Session isolation is unchanged by [Bot Session Learning](/features/bot-session-learning): `LearnManager` writes are scoped by the same `user_id` the session already carries, so per-user learn stores stay isolated even with the default-on posture.
</Note>
