...
Output presets map as `plain` → `plain`, `silent`/default → `silent`, `verbose` → `verbose`; silent-style presets print the final text. `chat` and `code` now run **resident** on the built-in split-pane TUI standalone (PR #3818) — `pip install praisonai-code` alone delivers full interactive sessions. When the `praisonai` wrapper is installed, `code` prefers its richer legacy dispatch (`PraisonAI._start_interactive_mode`).
...
`chat`, `code`, and every `run` output mode work on a `praisonai-code`-only install (PR #2853, PR #3818). Installing the wrapper (`pip install praisonai`) adds the richer legacy dispatch for `code` and the gateway/bot channels — it is no longer required for the baseline interactive experience.
