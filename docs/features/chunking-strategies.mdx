...
sidebarTitle: "Chunking Strategies"
...
<Note>
Chunker settings (`chunk_size`, `chunk_overlap`, `type`) apply uniformly to every supported file type — PDFs, DOCX, media via MarkItDown, source files, and plain-text files (`.md`, `.txt`, `.csv`, `.json`, `.xml`, `.html`, `.htm`). On pre-[#4831](https://github.com/MervinPraison/PraisonAI/pull/4831) releases these settings were silently ignored for the plain-text extensions.
</Note>
...
<Note>
`semantic`, `sdpm`, and `late` chunkers lazily import `chonkie.embeddings.AutoEmbeddings` on the first lookup. Since PraisonAI PR [#5105](https://github.com/MervinPraison/PraisonAI/pull/5105) a missing `chonkie` install raises `ImportError: chonkie package not found. Please install it using: pip install 'praisonaiagents[knowledge]'` — a broken transitive dependency (e.g. `sentence-transformers`) keeps its original error unchanged.
</Note>
  </Tab>
</Tabs>
...
| Symptom | Cause | Fix |
|---|---|---|
| `ImportError: chonkie package not found. Please install it using: pip install 'praisonaiagents[knowledge]'` on the first `Knowledge.add(...)` / `agent.start(...)` call | `semantic` (the default), `sdpm`, or `late` chunker is selected but the `[knowledge]` extra is not installed. Since PraisonAI PR [#5105](https://github.com/MervinPraison/PraisonAI/pull/5105) both the chunker registry and the embedding backend surface the same actionable hint. | `pip install 'praisonaiagents[knowledge]'`. Or fall back to `chunking_strategy="sentence"` / `"paragraph"` / `"fixed"` / `"token"`, which do not need chonkie's embedding model. |
| `ImportError` that names `chonkie.embeddings` but **omits** the `pip install` hint | `chonkie` is installed but its embeddings submodule cannot import (broken `sentence-transformers`, `torch`, or `transformers`). PR [#5105](https://github.com/MervinPraison/PraisonAI/pull/5105) intentionally preserves the original error so real transitive failures are not masked. | Read the underlying error, then reinstall or upgrade that package (`pip install --upgrade sentence-transformers` is the most common fix). |
