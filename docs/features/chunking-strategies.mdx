...
sequenceDiagram
    ...
    User->>Agent: Index document
    Agent->>Chunker: Split document
    Chunker-->>Agent: Text chunks
    loop For each chunk
        Agent->>VectorStore: Store chunk + embedding
    end
    Agent-->>User: Indexing complete
    User->>Agent: Query
    Agent->>VectorStore: Search relevant chunks
    VectorStore-->>Agent: Top matching chunks
