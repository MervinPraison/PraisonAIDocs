...
<Note>
`AgentTeam` requires at least one agent (or a handler-only task) at construction time. Spawn-and-announce examples add a single **seed agent** to satisfy this; the seed does not receive the spawned work — sub-agents you pass to `spawn_sub_agent` do.
</Note>

```python
from praisonaiagents import Agent, AgentTeam, Task

# AgentTeam requires at least one agent (or a handler-only task). The seed
# agent is not what runs the sub-agents you spawn — it just satisfies the
# constructor. get_spawned_agents() is still empty until spawn_sub_agent.
seed = Agent(name="seed", instructions="Seed agent for the team.")
team = AgentTeam(agents=[seed], name="research_team")
...
seed = Agent(name="seed", instructions="Seed agent for the team.")
team = AgentTeam(agents=[seed], name="my_team")
...
seed = Agent(name="seed", instructions="Seed agent for the team.")
team = AgentTeam(agents=[seed], name="callback_team")
...
seed = Agent(name="seed", instructions="Seed agent for the team.")
team = AgentTeam(agents=[seed], name="parallel_team")
...
seed = Agent(name="seed", instructions="Seed agent for the team.")
team = AgentTeam(agents=[seed], name="event_team")
...
async def main():
    seed = Agent(name="seed", instructions="Seed agent for the team.")
    team = AgentTeam(agents=[seed], name="async_team")
