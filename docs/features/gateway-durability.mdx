...
The `durability:idempotency` owner can carry either of two reasons:

| Reason | Meaning |
|--------|---------|
| `"durable idempotency store unavailable (running in-memory)"` | The durable SQLite store could not initialise; dedup is per-process and does **not** survive restart. The more severe of the two. |
| `"redis idempotency backend not available (running per-replica)"` | The operator asked for cross-replica dedup via `store_backend: redis`, but no Redis backend is implemented; the wrapper is running per-replica on the durable SQLite fallback. Cross-replica dedup only holds if replicas share the SQLite state file. See [Gateway Hooks → Multi-replica gateways](/features/gateway-hooks#multi-replica-gateways-and-store_backend-redis). |

<Note>
The milder `"...per-replica"` reason never masks the more severe `"...in-memory"` one — if the SQLite fallback for `store_backend: redis` *also* fails, the `in-memory` reason wins.
</Note>
