...
<Note>
**See also — idempotency `store_backend: redis`.** Because the turn lock has a real Redis backend, operators often expect the inbound idempotency store to mirror it. It **does not** have a Redis backend yet — setting `store_backend: redis` falls back to durable SQLite and records a degraded fact. See [Gateway Hooks → Multi-replica gateways](/features/gateway-hooks#multi-replica-gateways-and-store_backend-redis) for what happens if you set it.
</Note>
