...
When the agent hits a rate limit or transient error, it emits a `RETRY` event **before** it waits, then retries. The signal fires on both the sync and async retry loops — and emission is guarded so there is zero overhead when nothing is listening.
...
sequenceDiagram
    ...
    Agent->>Emitter: emit RETRY (attempt, max_attempts, delay, reason)
    Emitter-->>Consumer: RETRY event
    Agent->>LLM: retry
    LLM-->>Agent: 200 OK
