...
`start(stream=True)` forwards `temperature`, `max_tokens`, and `top_p` straight to the provider stream.
...
| Double fallback | Prevented — the outer `except` in `_start_stream_impl` re-raises when the inner fallback already ran. |
...
<Note>
Before PraisonAI PR [#4731](https://github.com/MervinPraison/PraisonAI/pull/4731) (issue [#4719](https://github.com/MervinPraison/PraisonAI/issues/4719)) the fallback forwarded `start()`'s raw kwargs unchanged, so `stream=True` reached the sync adapter and `max_tokens` raised `TypeError` before any request was made. Callers on the desktop (which passes `max_tokens` on every turn) saw the `TypeError` with the original streaming error buried two `__context__` levels deep. Since #4731, the fallback filters kwargs, forces `stream=False`, and chains the original cause via `raise ... from streaming_error`.
</Note>
...
| `start(stream=True)` | ✅ Yes | ✅ Auto | Force streaming, interactive chat — accepts `temperature`, `max_tokens`, `top_p` |
...
Context management runs on the streaming path exactly like it does on the non-streaming path.
...
Before PraisonAI PR [#4729](https://github.com/MervinPraison/PraisonAI/pull/4729) (issue [#4714](https://github.com/MervinPraison/PraisonAI/issues/4714)), streaming bypassed compaction and long-running chats hit the provider's context-length limit. Since #4729, `start(stream=True)` / `iter_stream()` and `chat()` / `start()` behave identically for context management. There is no configuration to enable this — it inherits from your existing `context=` / `Agent(tools=[...])` setup.
...
Fixed with single-agent smart-fallback in PR #1734. Since [PR #4731](https://github.com/MervinPraison/PraisonAI/pull/4731) the fallback also filters kwargs (`max_tokens`, `top_p`, and anything else `chat()` doesn't declare) and chains the original streaming exception via `raise ... from streaming_error`. If you were previously catching `TypeError: chat() got an unexpected keyword argument 'max_tokens'` as a proxy for "streaming failed", switch to inspecting `exc.__cause__` instead — the real cause is now preserved. For multi-agent teams that use sync-only providers, explicitly disable streaming with `output=["verbose", {"stream": False}]` or similar. See [Multi-Agent Output](/features/multi-agent-output) for configuration options.
