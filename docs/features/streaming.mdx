...
Since [PraisonAI PR #3632](https://github.com/MervinPraison/PraisonAI/pull/3632), the SDK emits a clear warning as soon as streaming begins on an agent with a guardrail attached:
...
| Method | Guardrail applied? | Use when |
|---|---|---|
| `chat("prompt")` | ✅ Yes — full response validated | You need validated output |
| `start("prompt")` (non-streaming) | ✅ Yes — full response validated | You need validated output |
| `astart("prompt")` (non-streaming) | ✅ Yes — full response validated | You need validated output |
| `start("prompt", stream=True)` | ❌ Bypassed — warning logged | Real-time UI, no validation needed |
| `iter_stream("prompt")` | ❌ Bypassed — warning logged | App integration, custom UI |

<Warning>
If your production path relies on guardrail validation, do not consume streamed output as if it were validated. Grep for `output guardrail is not applied to streamed responses` in your logs after upgrading — every match is a caller that thought streaming was validated when it wasn't.
</Warning>

See [Guardrails → Streaming bypass](/features/guardrails#streaming-bypass) for the guardrail-side view.
...
<AccordionGroup>
  ...
  <Accordion title="Do not stream when a guardrail must run">
    `iter_stream()` and `start(stream=True)` bypass output guardrails — the SDK logs a warning when both are combined. Use `chat()` (or the non-streaming path of `start()` / `astart()`) when you need validated output. See [Streaming and guardrails](#streaming-and-guardrails).
  </Accordion>
</AccordionGroup>
...
Fixed with single-agent smart-fallback in PR #1734. For multi-agent teams that use sync-only providers, explicitly disable streaming with `output=["verbose", {"stream": False}]` or similar. See [Multi-Agent Output](/features/multi-agent-output) for configuration options.
...
<CardGroup cols={2}>
  ...
  <Card title="Guardrails" icon="shield-halved" href="/features/guardrails#streaming-bypass">
