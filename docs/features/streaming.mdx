...
### Stopping a streamed tool batch

A Stop / `interrupt_controller.request()` during a streamed tool phase now short-circuits pending tool calls with typed `cancelled` results and ends the stream — no leftover completion request is issued for the cancelled turn.
...
agent = Agent(
    instructions="Run the requested tools.",
    tools=["shell", "search"],
    interrupt_controller=controller,
)

# From a UI Stop button / signal handler on another thread:
# controller.request("user_stop")

for chunk in agent.start("Search the web and download the result", stream=True):
    print(chunk, end="", flush=True)
```

<Note>
**Fixed in PraisonAI [PR #5081](https://github.com/MervinPraison/PraisonAI/pull/5081) (issue [#5073](https://github.com/MervinPraison/PraisonAI/issues/5073)).** Before #5081 the streaming path silently dropped `cancel_token` at the `execute_batch` boundary — a Stop mid-batch let subprocesses and MCP calls run to completion and still spent one more completion request. Since #5081, the token is forwarded end-to-end and three additional cancellation guards break the streaming tool loop and the Anthropic / Gemini / Ollama fallback loop. See [Cooperative Tool Cancellation → Streaming honours the same Stop](/features/cooperative-tool-cancellation#streaming-honours-the-same-stop).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Anthropic, Gemini and Ollama don't natively stream with tools">
    These providers do not natively stream with tools. `start(stream=True)` still works, but under the hood the tool loop runs non-streamed and each model turn's answer is yielded as one block. Since PR [#4757](https://github.com/MervinPraison/PraisonAI/pull/4757), tool calls on this path actually execute — earlier versions silently produced an empty stream. `max_iterations` (falls back to `Agent(max_iter=...)`) bounds how many round trips the fallback makes, and `max_tool_calls_per_turn` bounds tool calls per model turn. Since PR [#5081](https://github.com/MervinPraison/PraisonAI/pull/5081), this fallback loop also honours `cancel_token` between iterations and between per-tool dispatch — a Stop during the fallback breaks the loop instead of running every batched tool to completion. See [The non-streaming fallback loop](#the-non-streaming-fallback-loop).
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Async Knowledge Retrieval" icon="brain" href="/features/async-knowledge-retrieval">
    Knowledge injection works on streaming and async paths, not just sync
  </Card>
  <Card title="Search Results" icon="magnifying-glass" href="/features/knowledge-search-results">
