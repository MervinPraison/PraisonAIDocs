...
| **Multi-agent** `output=None` (default) | `stream=False` — no streaming | Default — works on every provider |
| **Multi-agent** `output="verbose"` / `"minimal"` | `stream=False` since PR #1911 | Display without streaming |
| **Multi-agent** `output=MultiAgentOutputConfig(stream=True)` | Force streaming for all team agents | Opt-in streaming |
...
Multi-agent teams (`AgentTeam`, `PraisonAIAgents`) now default to `stream=False` for all presets — including `"verbose"` and `"minimal"` — as of PR #1911. Use `output=MultiAgentOutputConfig(stream=True)` or `output=["verbose", {"stream": True}]` to opt in.
</Note>
...
Fixed in two stages. PR #1734 added single-agent smart-fallback. PR #1911 additionally made `stream=False` the default for all multi-agent presets — `AgentTeam`, `PraisonAIAgents`, and the `"verbose"` / `"minimal"` / `"silent"` presets — so multi-agent teams now Just Work on sync-only providers. See [Multi-Agent Output](/docs/features/multi-agent-output) to opt back in to streaming when your provider supports it.
