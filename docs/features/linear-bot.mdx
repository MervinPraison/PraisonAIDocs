...
agent = Agent(name="linear-bot", instructions="Manage Linear issues and projects.")
agent.start("Create a new Linear issue: Fix the login bug in production.")
```
...
<Note>
`praisonai bot linear` now stays running correctly. Previously it exited `0` in about a second due to a non-blocking-start bug, so `systemd Restart=on-failure` never restarted it. The CLI now holds the loop open while the adapter is running, and startup failures exit **1**.
</Note>
</Step>
...
For complete bot configuration options, see the [messaging bots documentation](/features/messaging-bots).
...
The bot implements Linear's recommended security measures via the shared [Webhook Verification](/features/webhook-verification) stack:
...
<Tip>
See the complete [Linear tools documentation](/tools/external/linear) for detailed usage examples.
</Tip>
...
Set `LINEAR_WEBHOOK_SECRET` for HMAC verification. Without it, webhooks are rejected with HTTP 401. Use `PRAISONAI_INSECURE_WEBHOOKS=true` only for local development. See [Webhook Verification](/features/webhook-verification).
</Accordion>
...
<Card title="Bot Gateway" icon="globe" href="/features/bot-gateway">
  Multi-platform bot orchestration
</Card>
<Card title="Linear Tools" icon="wrench" href="/tools/external/linear">
  Complete Linear API tool reference
</Card>
<Card title="CLI Bot Command" icon="terminal" href="/cli/bot">
  Command-line bot management
</Card>
<Card title="GitHub Commit Guardrails" icon="github" href="/tools/github_tools">
  Branch-safety rules for `github_commit_and_push`
</Card>
</CardGroup>