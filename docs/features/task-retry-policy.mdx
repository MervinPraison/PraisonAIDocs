...
Structured tasks (`output_pydantic` / `output_json`) also drive this retry loop when the model returns text that doesn't parse — see [Structured Outputs](/features/structured).

```python
from praisonaiagents import Agent, Task, AgentTeam

agent = Agent(name="Retry Agent", instructions="Process requests with retries")
