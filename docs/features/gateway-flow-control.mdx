...
<Note>
Flow control bounds **outbound** send throughput and per-session inbox depth. For bounding **inbound** concurrent agent runs across all users, see [Admission Control](/docs/features/gateway-admission-control). Admission also enforces a soft/hard RSS threshold when `max_rss_mb` is set — see [Memory-aware admission](/docs/features/gateway-admission-control#memory-aware-admission).
</Note>
