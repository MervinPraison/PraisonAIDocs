...
<Note>
This page documents a point-in-time historical fix. The file paths below are historical monorepo paths — the current code lives at the same relative path in the separate [PraisonAI-Platform](https://github.com/MervinPraison/PraisonAI-Platform) repo. See [Where Platform Lives](/features/platform/where-platform-lives).
</Note>
...
def create_jwt_token(user: User) -> str:
    ...
    payload = {
        ...
        "exp": datetime.now(timezone.utc) + timedelta(seconds=JWT_TTL)
    }
...
<Card title="Platform API Reference" icon="api" href="/features/platform/sdk-client">
