...
All `GET` / `PATCH` / `DELETE` endpoints for issues are scoped to the workspace in the URL. If an issue ID exists but belongs to a different workspace, the API returns `404 Not Found` — your auth token never reveals whether the ID exists in another workspace.
