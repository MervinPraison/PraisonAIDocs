...
<Note>
**Schema (PraisonAI Platform PR #1591):** Activity entries include `entity_type` and `entity_id`. Authenticated workspace actors use `actor_type: "member"` (not `"user"`). See [Platform data model](/docs/features/platform/data-model) for full entity tables.
</Note>
