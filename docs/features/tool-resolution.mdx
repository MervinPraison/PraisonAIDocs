...
One `tools=[...]` list can freely mix callables and tool-name strings.
...
# Callable + built-in name string in the same list — both work.
agent = Agent(
    name="Assistant",
    instructions="Use the local tool and read files",
    tools=[my_tool, "read_file"],
)
agent.start("Read config.json and process it")
```

Previously, adding a single callable to a list of names silently left every name unresolved; each name is now resolved individually.

```mermaid
graph LR
    subgraph "Mixed tools list"
        A["🧰 tools=[my_tool, 'read_file', 'oops']"] --> B{Per element}
        B -->|callable / instance| C[✅ Pass through in place]
        B -->|known name| D[🛠️ Resolve to tool]
        B -->|unknown name| E[⚠️ Warn + suggest / ⛔ strict raise]
        C --> F[📦 Final tools list]
        D --> F
    end
...
Unknown names inside a mixed list behave exactly like an all-strings list — warned and suggested by default, or raised under strict mode.
...
# 'no_such_tool' is unknown → warned + suggested (or raised under strict mode),
# my_tool is kept. Same contract as tools=["no_such_tool"].
Agent(name="Assistant", instructions="x", tools=[my_tool, "no_such_tool"])
# WARNING  Unknown tool 'no_such_tool'. ... Run 'praisonai tools list'.
```

### Resolve a mixed list yourself

Call `resolve_tools_list()` to resolve a possibly-mixed list without building an agent.
...
# Non-string entries keep their position; name strings resolve.
tools = resolve_tools_list([my_tool, "read_file"])
# strict=True raises ToolResolutionError on any unknown name; on_unknown routes suggestions.
```

<Warning>`resolve_tools_list` is not re-exported from `praisonaiagents.tools`. Import it from `praisonaiagents.tools.resolver`.</Warning>

| Argument | Type | Default | Behaviour |
|----------|------|---------|-----------|
| `items` | `List[Any]` | — | Mixed list of tool-name strings and tool objects. Non-strings pass through in place. |
| `strict` | `Optional[bool]` | `None` | `True` raises `ToolResolutionError` on any unknown name; `False` warns and returns resolved; `None` reads `PRAISONAI_STRICT_TOOLS`. |
...
<CardGroup cols={2}>
<Card title="Allowed Tools" icon="shield-check" href="/features/allowed-tools">
...
<Card title="Tool Call Self-Repair" icon="screwdriver-wrench" href="/features/tool-call-self-repair">
  Runtime auto-fix when the LLM emits a drifted tool name during a run.
</Card>
<Card title="Gateway Pre-flight Tool Check" icon="plane-departure" href="/features/gateway-cli#pre-flight-tool-check">
  Wrapper-level `--strict-tools` gate — fail fast at gateway start (exit 78).
</Card>
</CardGroup>
