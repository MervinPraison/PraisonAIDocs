...
<Note>
The helpers below (`load_context_files`, `load_context_files_for_path`, `PathContextAttacher`, `build_subtree_context_hook`, `file_tool_matcher`) are importable from **three** module paths — same API, same behaviour. Pick the one available in your install:

- `praisonai_code.integration.context_files` — resident copy on a standalone `pip install praisonai-code` (PR #4338).
- `praisonai.integration.context_files` — wrapper install.
- `praisonai_bot.integration.context_files` — bot install.

The examples below use the wrapper path; swap in `praisonai_code.…` on a standalone install.
</Note>
...
<Note>
As of PraisonAI PR **#3552**, both `praisonai chat` and `praisonai run` (including `praisonai run --agent …` and `praisonai run --profile …`) automatically register the subtree instruction hook — drop a `packages/foo/AGENTS.md` and it is attached the first time the agent reads or edits a file inside `packages/foo/`. If you're on an older release the hook fires only under `chat`; upgrade to pick up parity under `run`.

(Historical: PR #2922 fixed the `AFTER_TOOL` hook's `additional_context` being dropped. If you tried this recipe before **`praisonaiagents 1.6.147` / PraisonAI `v4.6.144`** and saw no effect on either surface, upgrade first.)
</Note>
...
<Step title="Wire it into an agent (auto-attach as tools run)">
...
hooks = HookRegistry()
hooks.register_function(
    HookEvent.AFTER_TOOL,
    build_subtree_context_hook(already_loaded=up_front),
...
sequenceDiagram
    ...
    participant Agent
    participant Tool
    participant Hook as AFTER_TOOL hook<br/>(build_subtree_context_hook)
    participant Attacher as PathContextAttacher

    User->>Agent: start("edit packages/foo/main.py")
    Agent->>Tool: edit_file(file_path="packages/foo/main.py")
    Tool-->>Agent: result
    Agent->>Hook: AFTER_TOOL(tool_input)
    Hook->>Attacher: attach_for_path("packages/foo/main.py")
    Attacher-->>Hook: packages/foo/AGENTS.md
    Hook-->>Agent: additional_context
    Agent-->>User: reply (now aware of foo's rules)
...
<Note>
Config-declared or `--instructions`-declared sources land in the backstory as `# Project Instructions` and are passed as `already_loaded` to the subtree hook, so a nested `packages/foo/AGENTS.md` mentioned in both places is not re-attached. See [Instruction Sources](/features/instruction-sources).
</Note>
...
```mermaid
graph TB
    Q0{"On a CLI<br/>(chat / run / code / tui)?"}
    Q0 -->|Yes| CLI["✅ Nothing to wire<br/>the CLI registers the hook for you<br/>unless --no-rules / PRAISON_NO_RULES"]
    Q0 -->|No — Python API| Q1{"Are all instruction files inside cwd or above it?"}
    Q1 -->|Yes| UPFRONT["✅ Up-front load_context_files is enough<br/>(no attacher)"]
    Q1 -->|No| Q2{"How does the session touch files?"}
    Q2 -->|"One known path"| ONESHOT["🎯 load_context_files_for_path(path)<br/>(stateless helper)"]
    Q2 -->|"Many paths, I call attach_for_path myself"| CLASS["🔁 PathContextAttacher(...)<br/>(session-scoped cache + dedup)"]
    Q2 -->|"An agent + tools — auto-attach as tools run"| HOOK["🪝 build_subtree_context_hook()<br/>+ file_tool_matcher()"]

    classDef q fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef pick fill:#10B981,stroke:#7C90A0,color:#fff

    class Q0,Q1,Q2 q
    class CLI,UPFRONT,ONESHOT,CLASS,HOOK pick
```
...
As of PraisonAI PR #2358, `praisonai chat`, `praisonai run`, `praisonai code`, and `praisonai tui` automatically discover and inject AGENTS.md-style project context — no flag required. Each command wires **two** things: the up-front `load_context_files()` merge that prepends the walk-up context to the system prompt, **and** the on-demand `build_subtree_context_hook` attach that fires the first time the agent touches a file under a nested `AGENTS.md`. Parity for `run` (including `run --agent …` and `run --profile …`) landed in PR **#3552** — before that, only `chat` registered the subtree hook.
...
sequenceDiagram
    ...
    participant Hook as build_subtree_context_hook
    ...
    CLI->>Hook: register AFTER_TOOL hook (unless --no-rules)
    Note over Hook: fires on first touch under a nested AGENTS.md
...
Zero-config in a monorepo: from the repo root, `praisonai run "refactor packages/foo/bar.py"` picks up `packages/foo/AGENTS.md` the moment it opens a file there — the same behaviour `chat` has always had.

Beyond auto-discovered `AGENTS.md` / `CLAUDE.md`, teams can declare extra sources — files, globs, or URLs — via the top-level `instructions:` config key or the `praisonai run --instructions` flag. Declared sources load up front alongside the walk-up and dedup against the same files. See [Instruction Sources](/features/instruction-sources).

- As of PraisonAI PR **#4338**, `praisonai run` (and the resident TUI's `chat` / `code` in `praisonai-code`) register the subtree hook **standalone** — no `praisonai_bot` / `praisonai` needed. The CLI falls back to the resident `praisonai_code.integration.context_files` copy when the optional packages are absent. Previously the hook was a silent no-op without those packages.
...
Two independent switches gate the two mechanisms: `--no-context` / `PRAISON_NO_CONTEXT` skips the up-front `load_context_files()` merge, while `--no-rules` / `PRAISON_NO_RULES` skips the on-demand `build_subtree_context_hook` attach. Both switches work on a standalone `pip install praisonai-code` — the subtree hook is now registered standalone via the resident `praisonai_code.integration.context_files` copy (PR #4338), so these opt-outs apply even without `praisonai_bot` / `praisonai`.

| Surface | Opt-out for up-front load (`load_context_files`) | Opt-out for subtree hook (`build_subtree_context_hook`) |
|---------|--------------------------------------------------|---------------------------------------------------------|
| CLI flag | `--no-context` | `--no-rules` |
| Env var | `PRAISON_NO_CONTEXT=true` | `PRAISON_NO_RULES=true` (accepts `1` / `true` / `yes`, case-insensitive) |
| `InteractiveConfig` field | `no_context=True` | `no_rules=True` |
| `configure_host()` param | `no_context=True` (or via `agent_kwargs`) | `no_rules=True` (or via `agent_kwargs`) |
...
`PRAISON_CONTEXT_BUDGET` — integer character budget for the **subtree hook** (`build_subtree_context_hook(max_chars=…)`). Applies to both interactive (`chat`) and non-interactive (`run`) CLI. `PRAISON_CONTEXT_BUDGET=0` — or a missing / non-integer value — disables the cap on the CLI subtree hook. Set a positive integer to bound the total subtree text emitted across the session.

<Note>
`PRAISON_CONTEXT_BUDGET` is **distinct** from `InteractiveConfig(context_token_budget=…)` / `configure_host(context_token_budget=…)`. The env var caps the on-demand subtree hook; the config field caps the up-front `load_context_files()` merge (default `8000`). When you call `build_subtree_context_hook()` yourself in Python, its own `max_chars` default is `8000` — the CLI wiring passes `PRAISON_CONTEXT_BUDGET` (defaulting to `0` = uncapped) instead.
</Note>
...
Two helpers wire the attacher into an agent's hooks so subtree rules attach automatically as file tools run.

| Helper | Signature | Purpose |
|--------|-----------|---------|
| `build_subtree_context_hook` | `(already_loaded=None, *, max_chars=8000) -> Callable` | `AFTER_TOOL` hook that auto-attaches subtree instruction files as file tools run. Session-scoped. |
| `file_tool_matcher` | `() -> str` | Regex that limits the hook to file-touching tool names. Matches wrapper defaults (`read_file`, `write_file`, `edit_file`, `list_files`), ACP tools (`acp_create_file`, `acp_edit_file`, `acp_delete_file`), and interactive-core / Claude-Code-style names (`read`, `edit`, `write`, `apply_patch`, `str_replace`, `multi_edit`, `search_replace`). Pass as `matcher=` when registering. |

Options for `build_subtree_context_hook`:

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `already_loaded` | `Optional[str]` | `None` | Up-front instruction text (used only to seed dedup so files already loaded at session start are not re-attached). |
| `max_chars` | `int` | `8000` | Character budget across the whole session; `0` disables it. |
...
<Accordion title="Prefer the hook for interactive/agentic sessions">
...
<CardGroup cols={2}>
<Card title="Host Integration" icon="plug" href="/features/host-integration">
