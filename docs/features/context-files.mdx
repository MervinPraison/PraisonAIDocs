...
Two independent switches gate the two mechanisms: `--no-context` / `PRAISON_NO_CONTEXT` skips the up-front `load_context_files()` merge, while `--no-rules` / `PRAISON_NO_RULES` skips the on-demand `build_subtree_context_hook` attach.
