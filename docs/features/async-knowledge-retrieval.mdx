...
<Step title="Streaming + knowledge">
...
<Accordion title="Async, streaming, and sync now use the same normalization" icon="equals">
Since [PR #4887](https://github.com/MervinPraison/PraisonAI/pull/4887), `.chat()`, `.achat()`, `.start_stream()`, and `.astart(..., stream=True)` all route knowledge retrieval through `_get_knowledge_context()`. The `SearchResult` dataclass, legacy dict, and plain list shapes are handled in one place — no per-path guardrails needed. Switch an agent between sync, async, and streaming freely; knowledge injection behaves identically.
</Accordion>
</AccordionGroup>
