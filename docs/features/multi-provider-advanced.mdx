...
response = router_agent.execute("What's the weather today?")
```
...
response = agent.execute("Analyze this complex financial report...")
```
...
report = router_agent.get_usage_report()
print(f"Total cost: ${report['total_cost_estimate']:.4f}")
```
...
auto_agents = AutoAgents(instructions="Create a research team", router=router)
