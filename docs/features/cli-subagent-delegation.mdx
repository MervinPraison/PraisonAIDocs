...
This is CLI-first delegation of **named user agents** — distinct from the SDK-level [Subagent Tool](/features/subagent-tool) and [Subagent Delegation](/features/subagent-delegation). No Python required: define agents as one-file Markdown definitions and expose them with a single flag.
...
If the model calls `spawn_subagent(agent_name="explorer")` and `explorer` isn't in the allow-list, a generic subagent handles the task — see [Subagent Tool](/features/subagent-tool). Delegation to named agents is fully backward-compatible: with no `--subagents` and no `mode: subagent` markers, runs behave exactly as before.
