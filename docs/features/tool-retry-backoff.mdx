...
| Outcome | Retried? |
|---|---|
| Tool declares `@tool(restart_safe=False)` / `.idempotent = False` — **any** retryable error | ❌ (runs exactly once) |
| Tool listed in `MUTATING_TOOLS` — any retryable error | ❌ (runs exactly once) |
| Wall-clock (outer) timeout — **idempotent** tool | ✅ |
| Wall-clock (outer) timeout — mutating / unknown tool | ❌ |
...
<Note>
A tool that **explicitly** declares itself unsafe (`restart_safe=False`, `.idempotent = False`, or listed in `MUTATING_TOOLS`) is vetoed from **every** retryable error, not just timeouts. Undeclared tools keep the behaviour above.
</Note>
...
graph TB
    ...
    Q -->|restart_safe=False / idempotent=False<br/>or in MUTATING_TOOLS| Once[🛑 Runs once — failure returned as-is]
...
- Membership in the shared `MUTATING_TOOLS` set.
