...
sequenceDiagram
    ...
    Note over Agent: delay = min(min(1.0 × 2^0, 60s) + jitter, 60s)
...
Delay: `min( min(initial_delay × factor^(attempt−1), 60s) + random(0, jitter × base), 60s )`. As of [PR #5050](https://github.com/MervinPraison/PraisonAI/pull/5050), the cap is re-applied **after** jitter, so `max_delay` is a hard ceiling for both the base delay and the jittered result — no attempt ever waits longer than the cap.
...
<Accordion title="Backoff is capped at 60s (hard ceiling, including jitter)">
The 60-second cap applies *after* jitter is added, so no attempt ever waits longer than `max_delay`. Very large backoff factors cannot exceed the cap. The cap only matters when retries actually happen — declared-unsafe tools do not retry.

<Note>
[PR #5050](https://github.com/MervinPraison/PraisonAI/pull/5050) re-clamps the delay after jitter. Earlier, jitter was added *after* the cap, so the effective wait could overshoot the ceiling by ~1.25×–1.5×. It cannot now.
</Note>
</Accordion>
