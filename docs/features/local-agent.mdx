...
<Note>
`LocalAgent` uses `compute=` to place its tools on a provider. To share one sandbox across a whole flow or team, use `AgentFlow(run_on=…)`. See [Placement](/features/placement).
</Note>
</Step>
</Steps>
