...
Use `tools_run_on=` to run tools in a cloud sandbox while thinking stays local:
...
local = LocalAgent(
    tools_run_on="e2b",  # Cloud sandbox for tools
    config=LocalAgentConfig(
        model="gpt-4o-mini",
...
<Note>
Renamed: `compute=` → `tools_run_on=`. `compute=` still works but emits a `FutureWarning`. See [Placement](/features/placement).
</Note>
</Step>
</Steps>
...
Instead of passing `image` / `packages` / `env` as kwargs on every call, commit them to a `.praisonai/environment.yaml`. Every `tools_run_on=` backend loads it automatically; kwargs still win when provided.

<Card icon="file-code" href="/features/environment-yaml">
  Repo-committed `.praisonai/environment.yaml` — image, packages, setup, resources
</Card>

<Tip>
The docker backend also caches the provisioned environment across runs — see [Environment Capture](/features/environment-capture).
</Tip>
...
<Card title="LocalAgent API Reference" icon="code" href="/sdk/reference/typescript/classes/AgentConfig">
  Complete LocalAgent configuration options
</Card>

<Card title="LocalAgentConfig Reference" icon="code" href="/sdk/reference/typescript/classes/AgentConfig">
...
| `ManagedAgent(provider="e2b", config=LocalManagedConfig(...))` | `LocalAgent(tools_run_on="e2b", config=LocalAgentConfig(...))` |
| `ManagedAgent(provider="modal", config=LocalManagedConfig(...))` | `LocalAgent(tools_run_on="modal", config=LocalAgentConfig(...))` |
...
- Use `tools_run_on=` to specify sandboxing backends separately from LLM choice (`compute=` still works but emits a `FutureWarning`)
