...
<Note>
Most sessions are already auto-titled after their first exchange — see [Session Auto-Title](/features/session-auto-title). Use `session rename` to override that name or to title a session you started without any content yet.
</Note>
...
The title lands in `metadata["title"]` via the same locked read-modify-write path as `update_session_metadata`, and surfaces in each `list_sessions()` row under the `"title"` key (`None` when never renamed). The [auto-title](/features/session-auto-title) path writes to this **same** `metadata["title"]` field, so a manual `rename` cleanly overrides an auto-generated title.
...
<CardGroup cols={2}>
  ...
  <Card title="Session Auto-Title" icon="wand-magic-sparkles" href="/features/session-auto-title">
    Titles that appear automatically after the first exchange
  </Card>
</CardGroup>
