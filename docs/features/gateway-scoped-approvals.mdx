...
| `resolver` | `Optional[str]` | `None` | Identity of the principal resolving this request. Recorded on the audit trail and the allow-always grant. `None` keeps today's anonymous `"gateway"` attribution. |

The gateway's `POST /approval/resolve` handler derives `resolver` automatically from the authenticated request — see [Operator identity attribution](/features/gateway-approval-attribution#how-resolver-identity-is-captured).

---

## Per-request Reviewer Custody

Bind a single request to specific reviewers so only they can resolve it — a finer boundary than the coarse [APPROVALS scope](/features/gateway-operator-scopes).

Pass `authorized_reviewers` on the `ApprovalRequest`; unauthorised resolvers are rejected and the request stays pending.
...
request = ApprovalRequest(
    ...
    risk_level="high",
    agent_name="deployer",
    authorized_reviewers=["operator:alice", "operator:oncall"],  # only these may resolve
)
```
...
sequenceDiagram
    ...
    UI->>GW: resolve (resolver="operator:bob")
    GW->>Mgr: resolve(rid, Resolution(resolver="operator:bob"))
    alt resolver not in authorized_reviewers
        Mgr-->>GW: False + re-insert pending
        GW-->>UI: 404 resolver not authorised
...
| Behaviour | Detail |
|---|---|
| Default | `None` — any resolver may resolve (legacy behaviour) |
| Unauthorised resolver | Returns `False`; the request is **re-inserted** into the pending map so an authorised reviewer can resolve it later |
| Rejection response | `404` with `{"error": "Request not found, already resolved, or resolver not authorised"}` — same status as "not found" so IDs cannot be enumerated |
...
<Note>
Custody is enforced by the gateway backend. Other approval backends that do not implement reviewer custody ignore `authorized_reviewers`.
</Note>
...
```json
{
  ...
  "grants": [
    {"agent_id": "shell-bot", "tool_name": "shell_exec", "arg_signature": null, "created_at": 1720000000.0, "approver": "operator:alice"},
...
<Note>
When the resolving request populates `Resolution.resolver` (e.g. `"operator:alice"`), the `approver` on the grant carries that real principal. `"gateway:human"` is only the default when no resolver is supplied.
</Note>
</Tab>
...
<Note>
Mutations (`POST`/`DELETE`) require `OperatorScope.APPROVALS` and pass the per-IP rate limiter. See [Gateway Operator Scopes](/features/gateway-operator-scopes).
</Note>
...
<CardGroup cols={2}>
<Card title="Gateway" icon="server" href="/features/gateway">
...
<Card title="Approval Attribution" icon="user-check" href="/features/gateway-approval-attribution">
  Who approved a request and per-request reviewer custody
</Card>
