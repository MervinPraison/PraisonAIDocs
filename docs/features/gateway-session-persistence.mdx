...
<Note>
As of PraisonAI PR #3409, `session.persist: true` uses `SqliteTranscriptStore` by default (WAL SQLite, one row per session, indexed lookups). Set `session.store: file` to keep the legacy per-session JSON layout. See [SQLite Transcript Store](/features/sqlite-transcript-store) for the full picture.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Bot vs Gateway" icon="code-compare" href="/features/bot-gateway">
