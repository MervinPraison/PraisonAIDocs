...
<Note>
[Gateway Memory-Pressure Eviction](/features/gateway-memory-pressure-eviction) relies on this — an unflushed transcript is never evicted, because only a durably persisted cache can be losslessly rebuilt on the next turn.
</Note>
