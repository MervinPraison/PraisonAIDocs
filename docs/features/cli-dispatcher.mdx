...
- **First positional is not a flag, not a reserved job verb (`submit / status / result / cancel / list / stream`), and either ends with `.yaml`/`.yml` (the existence check happens later, inside the runner) or names an existing file** → forwarded to the modern Typer agent runner (`commands/run.py` `run_app`).
...
<Tip>
A `.yaml`/`.yml` target is routed to the agent runner **before** the on-disk existence check, so a misspelled or wrong-directory path (the trap in [PraisonAI issue #5095](https://github.com/MervinPraison/PraisonAI/issues/5095)) surfaces a clear file-not-found error from the runner, not the async-jobs `invalid choice` / missing-`job_id` failure. Locked by [PR #5107](https://github.com/MervinPraison/PraisonAI/pull/5107). See [Async Jobs → How routing works inside `run`](/cli/async-jobs#how-routing-works-inside-run).
</Tip>
...
| `praisonai --session abc123` | **Bare interactive launch** | Resumes into the TUI (no `run` subcommand). See [Interactive TUI → Launching with a resumed session](/cli/interactive-tui#launching-with-a-resumed-session). |
