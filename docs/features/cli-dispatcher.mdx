...
| 5 | Lone positional that is a close match to a registered command and is not an extension of a registered command | **Exit 2** — `No such command`, `Did you mean: …?`, plus a `praisonai run "<token>"` escape hatch |
