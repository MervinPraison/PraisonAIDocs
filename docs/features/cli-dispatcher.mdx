...
PraisonAI picks one of seven paths based on what you type — and adding a new subcommand means it Just Works.
...
sequenceDiagram
    ...
    main()->>main(): Check routing rules 1-7
    alt --version / -V
...
|-----------|---------|---------------|
| `main()` | Entry router | Applies 7 rules in order |
...
`main()` applies seven rules in order. The first match wins.
...
The reserved-verb / typo guard is a **legacy-path** behaviour. It fires only when an invocation already reached the legacy dispatcher for another reason — a legacy-only flag (`--auto`, `--serve`, `--n8n`, …). A plain `praisonai show` now routes to Typer `run` (rule 5) and is sent to the model as the literal prompt `show`.
...
| `praisonai show --auto` | ✅ | legacy-only flag forces legacy; guard runs, stderr hint, exit 2 |
| `praisonai memoyr` (bare) | ❌ | routes to Typer `run` as the prompt `memoyr` |
| `praisonai memoyr --auto` | ✅ | legacy-only flag forces legacy; `Did you mean: memory?`, exit 2 |
