...
graph TB
    ...
    K -->|No| B{🔍 bare prompt?<br/>flags all run-supported &amp;<br/>not .yaml/.yml}
...
| `_looks_like_bare_prompt()` | Bare-prompt predicate | True when not `.yaml`/`.yml` **and** every flag (if any) is in `run`'s supported option set (derived via `_get_run_option_names()`) |
...
| Legacy | Fallback handler | YAML files and legacy-only flags |
...
| 5 | Bare free-text prompt (optionally with `run`-supported flags like `--model`, `--continue`, `--session`, `--output`, `--stream`) | **Typer `run`** (modern engine) |
| 6 | Everything else | Legacy (`.yaml`, legacy-only flags) — a one-line stderr notice fires when a legacy-only flag is what forced the fallback |

Rule 5 fires when `_looks_like_bare_prompt(argv, first_cmd)` returns `True` — that is, when `first_cmd` is non-empty, doesn't end with `.yaml`/`.yml`, and **either** no dash-prefixed token appears anywhere in argv **or** every flag present is one that Typer `run` itself accepts (`--model`, `--continue`, `--session`, `--output`, `--stream`, …, derived at dispatch time by introspecting `run`'s Click parameters). A single genuinely legacy-only flag (`--auto`, `--serve`, `--framework`, …) still forces legacy. If `run`-option discovery fails, the predicate falls back to the original conservative any-flag→legacy rule.

### Never-silent legacy fallback

When a bare-prompt invocation reaches the legacy engine solely because it contains a flag that `run` doesn't accept, `praisonai` prints a one-line stderr notice pointing at the modern equivalent:
...
The notice fires only in the legacy-flag case: `.yaml`/`.yml` workflows do **not** produce it (they legitimately live on the legacy path).
...
| `praisonai "Create a weather app" --framework crewai` | **Legacy** | `--framework` is legacy-only; the stderr notice fires |
...
Add a **legacy-only** flag (e.g. `--framework`, `--auto`, `--serve`) or use a `.yaml`/`.yml` file. Flags accepted by the modern `run` command (`--model`, `--continue`, `--session`, `--output`, `--stream`, …) now stay on the modern engine instead of forcing legacy. When a legacy-only flag is what triggers the fallback, a one-line notice prints to stderr — the fallback is never silent, so you can spot un-migrated flags. The reserved-verb / typo guard only runs once you are already on legacy.
</Accordion>
