...
<AccordionGroup>
<Accordion title="Let the SDK handle retries">
  Don't wrap `team.start()` in your own retry loop. The built-in exponential backoff and re-prompt logic is already tuned for the manager LLM's failure modes.
</Accordion>
<Accordion title="Use a capable manager LLM">
  Set `manager_llm` to a model that reliably outputs valid JSON (e.g. `gpt-4o`). Weaker models trigger more parse-error retries and may exhaust all three attempts.
</Accordion>
<Accordion title="Keep task descriptions unambiguous">
  Clear, distinct task descriptions reduce the chance the manager selects an invalid or wrong task ID. Ambiguity is the primary cause of invalid-selection retries.
</Accordion>
</AccordionGroup>
