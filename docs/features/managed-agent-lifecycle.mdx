...
```mermaid
graph LR
    subgraph "Managed Agent Lifecycle"
        Request[📋 User Request] --> Process[⚙️ Managed Agent Lifecycle]
...
graph LR
    subgraph "Managed Agent Lifecycle"
        In[📝 CRUD Request] --> Manager[⚙️ Lifecycle Manager]
        Manager --> Agent[🤖 Agent / Session]
        Agent --> Out[✅ Create → Run → Archive]
    end
...
<Card title="Managed Vault" icon="key" href="/features/managed-vault">
