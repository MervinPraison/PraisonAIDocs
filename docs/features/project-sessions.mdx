...
sequenceDiagram
    ...
    CLI->>Agent: build MemoryConfig(session_id, auto_save, history=True)
    CLI->>Store: get_chat_history(session_id)
    Store-->>CLI: prior messages
    CLI->>Agent: restore chat_history, set _auto_save_last_index
    Agent->>Agent: run new prompt with full prior context
...
The two `Store` round-trips (read prior history, append new messages) are what [PR #1963](https://github.com/MervinPraison/PraisonAI/pull/1963) wired through. Earlier builds had the history load missing and saves going to the wrong store.
