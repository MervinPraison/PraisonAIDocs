...
Shows every session `--continue` could resume for this project — the project's own sessions **plus** any globally-stored sessions (from `chat`, gateway, TUI, API, or a bare `Agent(
    memory=MemoryConfig(
        session_id=...,
    ))`). Duplicates are collapsed; the freshest record wins.
</Step>
</Steps>
...
`find_last_session()` merges the **project-scoped store** and the **global default store** so `--continue` resolves the most-recent session regardless of how it was created (`run`, `chat`, gateway, TUI, API, or a bare `Agent(
    memory=MemoryConfig(
        session_id=...,
    ))`). Sub-agent / forked children are skipped — the last **root** session wins. It falls back to a child only if the project has none. Each store is scanned with a `limit` window (default `50`).
...
| 2 | Repository **root commit SHA** (oldest across all refs; pinned to **`.git/praisonai-root-commit`** on first use) | `root-commit` | Repo has commits but no remote |
...
graph TB
    ...
    Commits -->|Yes| RootCommit[🌱 root-commit<br/>Oldest root, pinned in<br/>.git/praisonai-root-commit]
...
| `git checkout --orphan feature-x` + first commit | ✅ Yes | Oldest root wins over the newer one; the SHA is pinned so ties never flip |
| Two orphan branches created in the same second (scripted setup) | ✅ Yes | The pin locks the choice on first resolve — no tie-break to flip |
| Rewritten history / shallow clone where the pinned SHA is no longer a root | ⚠️ Re-chosen and re-pinned | The dead pin is not trusted; the fresh choice is persisted so it stays stable from then on |
...
After upgrading, the first resolve in each git repo also writes a small **`.git/praisonai-root-commit`** file next to the existing `.git/praisonai-project`. Both are internal to PraisonAI, shared across all worktrees of the repo, and safe to commit or leave untracked — deleting either just causes the next run to recompute and re-pin.
...
## Works with YAML / team runs

The same four flags apply to `praisonai run agents.yaml`, so an AgentTeam resumes exactly like a single-agent prompt.
...
| Command | Behavior |
|---|---|
| `praisonai run agents.yaml --continue` | Resumes the most recent root session; prior team turns replay |
| `praisonai run agents.yaml --session <id>` | Resumes a specific session — merges saved `team._state` and re-injects each agent's chat history |
| `praisonai run agents.yaml --fork --session <id>` | Forks via the hierarchical store; history is copied into the fork (deduplicated) |
| `praisonai run agents.yaml --no-save` | One-off team run — nothing is persisted after kickoff |

Per-agent `chat_history` is captured into team state on save (under the reserved key `_cli_session_chat_history`) and re-injected on restore, so a resumed YAML team continues the actual conversation — not just team bookkeeping.

<Note>
`shared_memory` is auto-enabled for active sessions — you do **not** need `memory: true` in `agents.yaml` for continuity to work. See [YAML / Team Session Continuity](/features/yaml-session-continuity) for the full team flow.
</Note>
...
<Accordion title="Orphan branches don't reassign the project id">
Creating an orphan branch (`git checkout --orphan …` + a first commit) adds a new root commit but does not change the project id — the oldest root wins and the choice is pinned to `.git/praisonai-root-commit` on first resolve, so even two roots created in the same second stay stable.
</Accordion>
</AccordionGroup>
