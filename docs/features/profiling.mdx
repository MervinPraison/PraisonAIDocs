...
Each buffer uses a bounded `deque` — when full, oldest records drop. See [Performance Profiling](/features/profiler) for per-agent isolation via `set_profiler()`.
...
<CardGroup cols={2}>
<Card title="Performance Profiling" icon="gauge" href="/features/profiler">
