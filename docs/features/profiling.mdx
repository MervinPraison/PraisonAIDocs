...
PraisonAI's profiler measures agent execution time, API latency, memory usage, and streaming performance — with zero overhead when disabled.

```mermaid
graph LR
    subgraph "Agent Profiling"
        A[🤖 Agent] --> B[📊 Profiler]
        B --> C{Feature?}
        C -->|Timing| D[⏱️ Timing Records]
        C -->|API| E[🌐 API Records]
        C -->|Memory| F[💾 Memory Records]
        D --> G[📤 Report / Export]
        E --> G
        F --> G
    end
    ...
    classDef category fill:#F59E0B,stroke:#7C90A0,color:#fff
...
def run_agent():
    ...
    return agent.start("Summarize quantum computing")
...
<Step title="Profile a block of code">
...
<CardGroup cols={2}>
  <Card title="Agent Tracing" icon="chart-line" href="/docs/features/agent-tracing">
    Trace agent execution with OpenTelemetry
  </Card>
  <Card title="Monitoring" icon="gauge-high" href="/docs/features/monitoring">
