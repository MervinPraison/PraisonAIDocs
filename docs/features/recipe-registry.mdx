...
### `get_registry()` parameters

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `registry` | `str` | `None` (local) | Filesystem path or HTTP(S) URL |
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `bundle_path` | `str` | — | Path to `.praison` bundle file |
| `force` | `bool` | `False` | Overwrite existing version |
| `metadata` | `dict` | `None` | Additional metadata to store |
...
| `version` | `str` | `None` (latest) | Version to pull |
| `output_dir` | `str` | `None` (cwd) | Directory to extract to |
| `verify_checksum` | `bool` | `True` | Verify bundle integrity |

### Remote registry

To set a custom HTTP timeout, instantiate `RemoteRegistry` directly:

```python
from praisonai.recipe.registry import RemoteRegistry

registry = RemoteRegistry("http://localhost:7777", token="...", timeout=60)
```

### Environment variables

Pass tokens explicitly via `os.environ` — `get_registry()` does not auto-read environment variables:
...
registry = get_registry(
    os.environ.get("PRAISONAI_REGISTRY_URL"),
    token=os.environ.get("PRAISONAI_REGISTRY_TOKEN"),
)
```
