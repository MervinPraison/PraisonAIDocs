...
<Note>
`AgentTeam(web=…)` is not wired yet (PraisonAI [#4004](https://github.com/MervinPraison/PraisonAI/pull/4004)) — set `web=…` on each `Agent(...)` in the team instead.
</Note>
...
<Card title="WebConfig API Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/feature_configs">
...
<CardGroup cols={2}>
<Card title="Search Providers" icon="globe" href="/features/search-provider-registry">
