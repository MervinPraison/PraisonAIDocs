...
agent = Agent(
    name="Researcher",
    instructions="Use my custom search provider.",
    web=True,
)
# Set WEB_SEARCH_PROVIDER=my-search in the environment to activate
agent.start("Search for latest research on solar energy")
```
<Note>
The entry-point group is `praisonai.tools.search`. Once installed, set `WEB_SEARCH_PROVIDER=my-search` to route searches through your plugin.
</Note>
</Step>
</Steps>
