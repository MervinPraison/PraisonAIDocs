...
from praisonai.code import write_file, apply_diff

agent = Agent(
    name="SafeCoderAgent",
    instructions="Edit code safely. Never touch system files.",
    tools=[write_file, apply_diff],
)

# Returns error — /etc/passwd is protected
agent.start("Write 'test' to /etc/passwd")
```
...
| Tool | Enforces `is_protected()` | Module |
|------|---------------------------|--------|
| `write_file` | Yes | `praisonai.code` |
| `apply_diff` | Yes | `praisonai.code` |
| `append_to_file` | No | `praisonai.code` |
| `search_replace` | No | `praisonai.code` |
