...
graph TB
    ...
    Q1 -->|Browser / React Native| Deep[Use deep imports<br/>praisonai/agent/simple<br/>Agent + types]
...
| `./agent/simple` | `praisonai/agent/simple` | The `Agent` class plus agent-related **type** exports (`SimpleAgentConfig`, `AgentTeamConfig`, `PraisonAIAgentsConfig`). The team-class **values** (`AgentTeam`, `PraisonAIAgents`, `Agents`) are not re-exported here — import those from `praisonai` or `praisonai/agent`. |

<Note>
`praisonai/agent/simple` does not re-export `AgentTeam`, `PraisonAIAgents`, or `Agents` as runtime values. This keeps mobile and other webview bundles from statically pulling in the `agent/team` subsystem when they only need `Agent`. Import the team classes from `praisonai` or `praisonai/agent`. Type imports still work — `import type { AgentTeamConfig } from 'praisonai/agent/simple'` is erased at compile time and costs nothing at runtime.
</Note>
