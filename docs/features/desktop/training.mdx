...
sequenceDiagram
    ...
    Engine-->>App: SSE start / state / log / progress / metric
...
| **Adopt-in-flight** | A fresh window reattaches to a run from a previous session — but only when the recorded **pid + start-time fingerprint** still matches the live process (mirroring what `src-tauri/src/adopt.rs` already did for the engine). A live pid alone is not enough — pids are recycled. A recycled pid is refused and the run is surfaced as terminal, not adopted, so Stop never SIGTERMs a stranger. A pre-upgrade state file with no `pid_start` is also refused, at the cost of at most one run shown as interrupted across a single restart after upgrade. |
...
<Note>
The full log is persisted to `<PRAISONAI_DESKTOP_HOME>/runs/<run_id>/train.log` (UTF-8, `errors="replace"`, flushed per line so it is readable while the run is live), and the config is written next to it as `config.yaml` (JSON if PyYAML is missing). See [Data & Privacy](/features/desktop/data) for where `PRAISONAI_DESKTOP_HOME` points.
</Note>
...
On restart the engine reads each `runs/<id>/run.json` and only adopts a run it can prove is still its own — a live pid is not enough because pids are recycled.

```mermaid
graph TB
    Restart[🔁 Restart → read run.json] --> HasFP{Has pid_start?}
    HasFP -->|no| Refuse1[⛔ Refuse — interrupted<br/>pre-upgrade state file]
    HasFP -->|yes| Alive{pid alive?}
    Alive -->|no| Refuse2[⛔ Refuse — interrupted]
    Alive -->|yes| Match{pid_start matches<br/>_pid_start_time pid?}
    Match -->|yes| Adopt[✅ Adopt as RUNNING<br/>Stop still reaches it]
    Match -->|no| Refuse3[⛔ Refuse — recycled pid<br/>do not signal a stranger]
    ...
    class Restart start
    class HasFP,Alive,Match q
    class Adopt ok
    class Refuse1,Refuse2,Refuse3 bad
```
...
<Note>
From PraisonAI [#4879](https://github.com/MervinPraison/PraisonAI/pull/4879) the explicit `ollama_save: false` / `huggingface_save: false` lines are optional for **vision** configs too — omitting the keys entirely also skips publishing. Before #4879 that held only for LLM configs; the vision trainer defaulted those flags to ON. See [Vision Fine-Tuning](/features/praisonai-train-vision).
</Note>
</Accordion>
</AccordionGroup>
...
<Tab title="Interrupted run">
Close the lid mid-training. Hours later, reopen the app — the Train tab reattaches, the loss chart repopulates from history, and the run finishes.
</Tab>

<Tab title="Stop a run">
Click **Stop**. The run and all its dataloader workers die together; the status pill turns `cancelled` and the log tail is preserved.
</Tab>

<Tab title="Separate CUDA env">
Point the app at a matched torch/CUDA environment before launching:

```bash
export PRAISONAI_TRAIN_CMD="/opt/cuda-venv/bin/python -m praisonai_train llm"
```

The override chooses the interpreter; `--config` is still appended by the engine.
</Tab>
</Tabs>
...
| State | Meaning |
|-------|---------|
| `running` | The trainer is live |
| `stopping` | `SIGTERM` sent; teardown in progress |
| `done` | Exited cleanly (exit 0) |
| `failed` | Exited non-zero; `error` holds the last meaningful log line |
