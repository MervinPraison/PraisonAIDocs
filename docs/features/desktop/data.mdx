...
The folder is created the first time you open the app — before you set anything up, and whether or not you click **Get started**. Delete it to reset, and the next launch makes it again. Only the primary launch creates it: a second launch is handed off to the running app before it gets that far. Creation is best effort — if it fails the app still starts and provisioning creates the folder later.

```mermaid
graph LR
    Launch[🚀 First launch] --> Dir[(📁 Data folder created)]
    Dir --> Setup[⚙️ Get started]
    Setup --> Venv[📦 venv + engine files]
