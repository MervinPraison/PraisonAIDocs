...
The engine kept the live run and the history in memory only, so a crash or kill left `/train/runs` returning `[]` while the trainer kept the GPU — and `start` was willing to launch a second trainer beside the live one. Fixed in PraisonAI [#4510](https://github.com/MervinPraison/PraisonAI/pull/4510): the run's state and pid are persisted to `runs/<run-id>/run.json` at every transition, and the next engine boot rebuilds history from disk. A run whose pid is still alive is adopted; an interrupted run whose pid is gone reads as `failed`, not missing. If you saw a run disappear across a restart, upgrade.
</Accordion>
