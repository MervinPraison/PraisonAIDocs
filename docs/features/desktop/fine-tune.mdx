...
A training run is a subsystem, not a single request: it survives the window closing, keeps a ring buffer of progress you can reconnect to, and derives its status from the subprocess itself — so a client that missed the ending still learns how it ended.
...
Progress is a ring buffer replayed from a cursor, not a live-only stream. If the window closes, reopening the run replays what you missed — there's no need to restart training.
</Accordion>
