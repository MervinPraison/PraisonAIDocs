...
The MCP settings overlay dims a toast beneath its scrim, so errors from this panel surface through the in-app dialog instead — that dialog paints above the scrim and stays readable. If neither the dialog nor the write happens on an older build, see [Troubleshooting](/features/desktop/troubleshooting).
