...
The combobox suggests only ids that work with **no further configuration** — every one uses the OpenAI API and your `api_key`. It stays free-text, so a user with a Base URL configured can still type any id.
...
Set a **Base URL** to that provider's OpenAI-compatible endpoint and type its **bare** model id in the combobox. The engine refuses slashed ids like `ollama/llama3.2` in the shipped desktop venv (no `litellm`) — see [PraisonAI#4722](https://github.com/MervinPraison/PraisonAI/pull/4722) — so use the endpoint's own bare id, not the litellm-prefixed one.

| Provider | Base URL | Example bare id |
|----------|----------|-----------------|
...
Both fields are marked **Requires restart** — relaunch the app after changing either. See the Setup table below for the exact fields, and [PraisonAI#4756](https://github.com/MervinPraison/PraisonAI/pull/4756) for the picker change.
