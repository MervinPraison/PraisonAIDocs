...
Your `api_key` is stripped before `settings.json` is written and stored in your platform's secret store, since the POSIX keychain binaries only exist on macOS and Linux:
...
<Warning>
Your `api_key` never touches `settings.json` — `save_settings` strips every secret key before the file is written, so it lives only in the keychain. Clearing it removes the credential from the environment on the next turn.
</Warning>
