...
<Note>
Secrets never reach `settings.json`. The `api_key` is stored in the macOS keychain and stripped before the file is written, and `GET /settings` returns it masked as bullets — the UI only needs to know whether a key is set.
</Note>
...
| Row | Effect |
|-----|--------|
| MCP servers | Manage tool servers launched over stdio |

See [MCP Servers](/features/desktop/mcp) for setup.
...
| Chord | Action | Where it shows |
|-------|--------|----------------|
| `⌘,` | Open Settings | macOS only |
| `⌘Q` | Quit PraisonAI | macOS only |

On Windows and Linux the tray still exists, but no accelerators are advertised: Tauri maps `Cmd` to the Super key off macOS, and `Win+Q` is reserved by Windows — so a rendered "Win+Q" would be a lie. The template tray icon is macOS-only, and left-click-to-open-menu is disabled on Linux because Wayland doesn't support it.
