...
| Field | Type | Default | Notes |
|-------|------|---------|-------|
| `model` | combobox | `"gpt-4o-mini"` | Any OpenAI-compatible model id. New chats use this. A provider-prefixed id (`anthropic/…`, `gemini/…`, `ollama/…`) routes through litellm to that provider — set its API key in the environment, or run Ollama locally. For an OpenAI-compatible endpoint instead, set a Base URL and use that endpoint's bare model id. See [Suggested Models](/features/desktop/models#suggested-models). |
| `temperature` | slider `0`–`2` | `0.7` | Higher is more varied |
| `max_tokens` | number `0`–`128000` | `0` | `0` lets the model decide |
| `top_p` | slider `0`–`1` | `1` | |
| `reasoning_effort` | select | `"off"` | `off` leaves the provider default; `minimal` / `low` / `medium` / `high` map to each backend's native knob (OpenAI/xAI `reasoning_effort`, Anthropic/Gemini extended-thinking budget). See [Reasoning Effort](/features/reasoning-effort). |
| `framework` | text | `"praisonai"` | Requires restart. Which engine runs a turn. `praisonai` is built in. Installing `praisonai-frameworks` adds `crewai`, `autogen`, `langgraph`, `agno`, `google-adk`, `openai-agents`, `pydantic-ai`. An uninstalled name is refused with the exact `pip install` command — the setting never silently falls back to the default. See [Agent Framework](/features/desktop/frameworks). |
| `base_url` | text | `""` | Requires restart. Point at Ollama, LM Studio, vLLM, a proxy or Azure. Sent to whichever provider the model names — `<PROVIDER>_API_BASE` for a slashed id, `OPENAI_API_BASE` for a bare id. Blank uses the provider default. |
...
| Field | Type | Default | Notes |
|-------|------|---------|-------|
| `approval_mode` | select | `"ask"` | `ask` prompts on every gated tool call; `smart` waives an allowlist of low-risk reads (`read_file`, `list_directory`, `current_time`, `web_search`) and prompts on everything else; `never` runs all tools without asking (confirmed once) |
...
Since [MervinPraison/PraisonAI#4730](https://github.com/MervinPraison/PraisonAI/pull/4730), `smart` is meaningful — previously it fell through to the same prompt as `ask`. See [Approvals & Safety](/features/desktop/approvals) for behaviour.
...
Fields marked **Requires restart** (`base_url`, `api_key`, `launch_at_login`, `framework`) say so on the row itself.
