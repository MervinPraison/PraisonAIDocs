...
| `OPENAI_API_KEY` (and peers) | User | Provider key used when `api_key` is blank in settings. Also unlocks Sora 2 in the [Video tab](/features/desktop/video). |
| `REPLICATE_API_TOKEN` / `REPLICATE_API_KEY` | User | Unlocks MiniMax video-01 in the [Video tab](/features/desktop/video). |
| `GEMINI_API_KEY` / `GOOGLE_API_KEY` | User | Unlocks Veo 3.1 and Veo 3.1 Lite in the [Video tab](/features/desktop/video). |
| `OPENAI_API_BASE` | Engine | Set from the `base_url` setting; cleared when you clear that setting. |
| `<PROVIDER>_API_BASE` | Engine | Set from the `base_url` setting for a slashed model id; the provider is named by the id's prefix. |
| `<PROVIDER>_API_KEY` | Engine | Set from the `api_key` setting for a slashed model id, following the same naming rule. |
...
| Slashed prefix | `Base URL` exports as | `api_key` exports as |
|----------------|-----------------------|----------------------|
| `ollama/`, `ollama_chat/` | `OLLAMA_API_BASE` | *(none — Ollama has no key)* |
| `lm_studio/` | `LM_STUDIO_API_BASE` | `LM_STUDIO_API_KEY` |
| `hosted_vllm/` | `HOSTED_VLLM_API_BASE` | `HOSTED_VLLM_API_KEY` |
| `openai/`, `text-completion-openai/` | `OPENAI_API_BASE` | `OPENAI_API_KEY` |
| `together_ai/` | `TOGETHERAI_API_BASE` | `TOGETHERAI_API_KEY` |
| anything else (`groq/`, `mistral/`, …) | `<PROVIDER>_API_BASE` (uppercased) | `<PROVIDER>_API_KEY` (uppercased) |
| bare id (`gpt-4o-mini`, …) | `OPENAI_API_BASE` | `OPENAI_API_KEY` |
...
Each video model in the [Video tab](/features/desktop/video) becomes selectable only when one of its keys is present. A model with no configured key shows `(key needed)` and stays disabled.

| Key | Unlocks in Video tab |
|-----|----------------------|
| `REPLICATE_API_TOKEN` / `REPLICATE_API_KEY` | MiniMax video-01 |
| `OPENAI_API_KEY` | Sora 2 |
| `GEMINI_API_KEY` / `GOOGLE_API_KEY` | Veo 3.1, Veo 3.1 Lite |
