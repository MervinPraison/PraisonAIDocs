...
```mermaid
graph LR
    Env[🔧 Environment] --> Data[📁 Data dir]
    Env --> Secrets[🔐 Secret isolation]
    Env --> Keys[🔑 Provider keys]

    classDef env fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef out fill:#10B981,stroke:#7C90A0,color:#fff

    class Env env
    class Data,Secrets,Keys out
```
...
Export the variable in the shell that starts the app, so both the Rust shell and the Python engine inherit it.
</Step>
