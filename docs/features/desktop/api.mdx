...
| `GET /frameworks` | Which agent frameworks this install can run, plus install hints for the ones it cannot. Returns `{available, known, hints, default, error}`. See [Agent Framework](/features/desktop/frameworks). |
...
| `GET /media/capabilities` | What media generation is available; now returns `video_models` and a `video_hint` |
| `GET /media/video/models` | The video model catalog with a per-model `configured` flag |
| `POST /media/video` | Generate a video; optional `model` field defaults to `replicate/minimax/video-01`. See [Video generation](/features/desktop/video) |
...
{
  "available": ["praisonai"],
  "known": ["praisonai", "crewai", "autogen", "langgraph", "agno", "google-adk", "openai-agents", "pydantic-ai"],
  "hints": {
...
The `/media/*` routes back the Video tab. Generation is a single synchronous `POST /media/video` — there is no job store or polling. Full walkthrough: [Video generation](/features/desktop/video).

`GET /media/video/models` lists the catalog with a per-model `configured` flag:
...
`POST /media/video` accepts `prompt` and an optional `model`. The Replicate path returns a hosted `url`; SDK paths (Sora, Veo) return a `data_url` and a null `url`:
...
// SDK response
{ "id": "vid_…", "path": ".../media-output/videos/vid_….mp4",
  "url": null, "data_url": "data:video/mp4;base64,…", "model": "openai/sora-2", "prompt": "…" }
```

`GET /media/capabilities` includes `video_models` and a `video_hint` alongside the image fields:

```json
{ "ok": true, "video": true,
  "video_models": [ { "id": "replicate/minimax/video-01", "configured": true } ],
  "video_hint": "Set a provider key … (REPLICATE_API_TOKEN for MiniMax, OPENAI_API_KEY for Sora, GEMINI_API_KEY for Veo)." }
```
...
<Card title="Video generation" icon="video" href="/features/desktop/video">
The `/media/*` routes behind the Video tab
</Card>
