...
The `<data folder>` is the per-platform data directory — see [Data & Privacy](/features/desktop/data#where-data-lives) for each path. Override it with `PRAISONAI_DESKTOP_HOME` if you want the venv somewhere else.
