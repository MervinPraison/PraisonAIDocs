...
graph LR
    subgraph "Context Replay"
        A[🤖 Agent run] --> B[💾 Trace emitter]
        B --> C[📝 JSONL file]
        C --> D[🔁 praisonai replay]
        D --> E[🔍 Step-through]
        D --> F[📈 Flow viz]
    end
...
<Steps>
<Step title="Run with trace saving">
```bash
...
sequenceDiagram
    ...
    Emitter->>JSONL: append event
    CLI->>JSONL: read all events
    CLI->>CLI: interactive step-through
```
...
Include a timestamp or task identifier in session IDs so `replay list` is easy to navigate.
</Accordion>

<Accordion title="Enable redaction for sensitive data">
Pass `redact=True` to `ContextTraceEmitter` (the default). This masks secrets in monitoring snapshots.
</Accordion>

<Accordion title="Clean up old traces periodically">
```bash
praisonai replay cleanup --max-age 7 --max-size 200
```
</Accordion>

<Accordion title="Use snapshots sparingly">
Full context snapshots (`context_snapshot` events) can be large. Prefer `message_added` events for most debugging.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="File Snapshot" icon="camera-retro" href="/features/file-snapshot">
  Shadow git repo for file-level undo/restore
</Card>
<Card title="Checkpoints" icon="flag-checkered" href="/features/checkpoints">
  Higher-level checkpoint service for agent state
</Card>
</CardGroup>
