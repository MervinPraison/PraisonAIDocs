...
<Step title="Enable Shell Substitution in Commands">
```bash
...
### PRAISONAI_ALLOW_SHELL

Controls live `` !`cmd` `` substitution in custom command templates (`.praisonai/commands/*.md`).

**Security Risk**: Remote Code Execution (RCE) — any shell command the template author writes is executed in the template's `working_dir`

```bash
# Enable live shell substitution in command templates
export PRAISONAI_ALLOW_SHELL=true

# Disable (default - secure)
unset PRAISONAI_ALLOW_SHELL
```

**Default**: unset → disabled. When a template contains `` !`cmd` `` but the flag is not set, the CLI raises `ShellSubstitutionError` and exits 1 rather than silently escaping.

**Accepted truthy values**: `true` (case-insensitive). Quoted-false strings (`"false"`, `"no"`, `"0"`) fail closed.

**Resolution order** — any one gate is sufficient:

| Gate | Example |
|------|---------|
| `PRAISONAI_ALLOW_SHELL=true` | `export PRAISONAI_ALLOW_SHELL=true` |
| `commands.allow_shell: true` in `.praisonai/config.yaml` | Project-wide opt-in |
| `allow_shell: true` frontmatter | Per-command, narrowest scope |
| `interpolate(..., allow_shell=True)` | Python API |

**Safety bounds** (enforced by the SDK regardless of which gate is used):

| Bound | Value |
|-------|-------|
| Timeout | `SHELL_SUBSTITUTION_TIMEOUT = 30` seconds (wall-clock) |
| Max stdout | `SHELL_SUBSTITUTION_MAX_BYTES = 100 000` bytes — applied while reading |
| Working dir | Template's `working_dir` |
| Non-zero exit | Raises `ShellSubstitutionError` |

**Security guards**: Live substitution is resolved against the original template **before** `$ARGUMENTS` / `@file` injection. `` !`cmd` `` carried by user input or file contents is inert text — it is never executed.

<Note>
Enabling `PRAISONAI_ALLOW_SHELL=true` globally affects every command template the CLI discovers. For narrow scope, use per-command frontmatter (`allow_shell: true`) or the project config (`commands.allow_shell: true` in `.praisonai/config.yaml`).
</Note>

**Usage Example**:
```bash
# Whole-session opt-in
export PRAISONAI_ALLOW_SHELL=true
praisonai run --command commit
```
