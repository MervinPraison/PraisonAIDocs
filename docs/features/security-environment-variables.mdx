...
- `praisonai.cli.features.tools.ToolsCLI.action_add` — **all three source branches** (local file, `github:`, and bare package name) fail closed without the opt-in (PR [#5206](https://github.com/MervinPraison/PraisonAI/pull/5206))
- `praisonai.templates.tool_override.ToolOverrideLoader.load_from_module` — raises `PermissionError` when unset (was ungated; PR [#5206](https://github.com/MervinPraison/PraisonAI/pull/5206))
...
| Env var unset, `ToolOverrideLoader.load_from_module(...)` called directly | raised exception | `PermissionError: PRAISONAI_ALLOW_LOCAL_TOOLS=true is required to import user modules (refusing to load '<module>').` |
| Env var unset, recipe YAML's `tools_source: <dotted>` reaches `create_tool_registry_with_overrides` | logger.warning | `skipping tools_source '<source>': PRAISONAI_ALLOW_LOCAL_TOOLS=true is required to import user modules (refusing to load '<source>').` |
| Env var unset, `praisonai tools add <bare_package_name>` | stdout (error status) | `Refusing to add package tools: set PRAISONAI_ALLOW_LOCAL_TOOLS=true to enable (importing a package runs its code; same opt-in the runtime loader requires).` |
...
See [Async Bridge → `run_sync_or_offload`](/features/async-bridge#run-sync-or-offload-strict-inside-a-running-loop) for the full decision diagram.
...
See [Wrapper → alias_package shim: lazy vs eager](/developers/wrapper#alias-package-shim-lazy-vs-eager) for the developer reference.
...
PR [#5206](https://github.com/MervinPraison/PraisonAI/pull/5206) (`fix: fail closed on tool-loader opt-in bypass and security config parse errors`) extended `PRAISONAI_ALLOW_LOCAL_TOOLS` enforcement to the `praisonai tools add <package_name>` bare-package branch and to `ToolOverrideLoader.load_from_module` — both previously ran user code without the opt-in. The same PR made `~/.praison/security.yaml` [fail closed on parse errors](/features/template-security-config#file-parse-behaviour). No new advisory was filed; the threat model is unchanged from GHSA-g985-wjh9-qxxc.
