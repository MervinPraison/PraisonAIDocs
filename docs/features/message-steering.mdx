...
<Info>
When wrapping a steering-enabled agent in a chat bot, set the bot's `busy_mode="steer"` to surface this capability to end users. See [Bot Run Control](/features/bot-run-control).

`busy_mode="steer"` folds the new message into the *same* turn, so it does **not** trigger the supersede path — a pending approval on the running turn stays live. `busy_mode="interrupt"` is the mode that cancels a pending approval when the turn is superseded. See [Approval Turn Liveness](/features/gateway-approval-turn-liveness).
</Info>
...
<CardGroup cols={2}>
  <Card icon="webhook" href="/features/hooks">Pre/post execution hooks (compile-time interception)</Card>
  <Card icon="keyboard" href="/features/bot-run-control">Interactive input vs background steering</Card>
</CardGroup>