...
`PortableSessionStoreProtocol` adds store-agnostic backup / migrate / restore alongside `SessionStoreProtocol`, `SearchableSessionStoreProtocol`, and `SessionMirrorProtocol`. `DefaultSessionStore` and `SqliteSessionStore` implement it.

| Method | Signature | Returns |
|--------|-----------|---------|
| `export_session` | `(session_id, *, include_lineage=True)` | `dict` — `{"version": 1, "sessions": [...]}` |
| `export_all` | `()` | `dict` — `{"version": 1, "sessions": [...]}` |
| `import_sessions` | `(payload, *, max_sessions=10_000, reset_live_fields=True, overwrite=False)` | `ImportReport` |

```python
from praisonaiagents.session import DefaultSessionStore, PortableSessionStoreProtocol, ImportReport

store = DefaultSessionStore()
assert isinstance(store, PortableSessionStoreProtocol)  # runtime_checkable

payload = store.export_all()
report: ImportReport = store.import_sessions(payload)
```
...
`PortableSessionStoreProtocol` is `@runtime_checkable`, so `isinstance(store, PortableSessionStoreProtocol)` verifies conformance. See [Gateway Session Portability](/features/gateway-session-portability) for the operator-facing story.
...
<CardGroup cols={2}>
  ...
  <Card title="Sessions Overview" icon="clock-rotate-left" href="/features/sessions">
