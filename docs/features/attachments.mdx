...
<Note>
**Looking for the gateway attachment contract?** This page covers `agent.start(attachments=[…])` — attaching files to an agent call so the model sees them as content parts. If you are building a custom `/ws` client and want to hand the gateway a file (or receive an agent-generated artefact back over the same connection), see [Gateway Attachments](/features/gateway-attachments) — it is a different, protocol-level feature.
</Note>
...
<Card title="Gateway Attachments" icon="file-arrow-up" href="/features/gateway-attachments">
  Hand a file to the agent over a `/ws` connection — the protocol-level attachment contract.
</Card>
</CardGroup>
