...
graph LR
    subgraph "Token Budgeting"
        A[🤖 Model] --> B[📊 Context Window]
        B --> C[Reserve: system]
        B --> D[Reserve: history]
        B --> E[Reserve: response]
        B --> F[✅ Available tokens]
        F --> G[📄 Chunks enforced]
    end

    classDef input fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef window fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef reserve fill:#6366F1,stroke:#7C90A0,color:#fff
...
<Steps>
<Step title="Create a token budget for your model">
```python
...
sequenceDiagram
    ...
    RAG->>TokenBudget: TokenBudget(model="gpt-4o-mini")
    RAG->>TokenBudget: dynamic_budget(system, history)
    TokenBudget-->>RAG: available tokens
    RAG->>BudgetEnforcer: enforce(budget, chunks)
    BudgetEnforcer-->>RAG: trimmed chunks
    RAG->>LLM: prompt within budget
    LLM-->>RAG: response
```
...
Default `reserved_response=4096` works for most cases. For long-form generation, increase to 8192 to avoid truncated responses.
</Accordion>

<Accordion title="Monitor token usage">
Run with `verbose=True` or `praisonai knowledge stats --json` to observe actual token consumption per request and tune reservations accordingly.
</Accordion>

<Accordion title="Adjust chunk size per model">
Smaller context models (e.g., `gpt-3.5-turbo` at 16k) need tighter enforcement. Increase `reserved_history` for multi-turn conversations.
</Accordion>

<Accordion title="Account for conversation history">
Multi-turn conversations accumulate tokens fast. Track history length and pass `history_tokens` to `dynamic_budget()` on each turn.
</Accordion>
</AccordionGroup>
...
<Card title="Large Context Overview" icon="expand" href="/features/large-context-overview">
  Strategies for handling very large corpora
</Card>
</CardGroup>
