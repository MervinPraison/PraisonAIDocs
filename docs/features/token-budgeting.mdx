...
sequenceDiagram
    ...
    User->>Agent: Request with large context
    Agent->>Budget: Check available tokens
    Budget-->>Agent: 80,000 tokens available
    Agent->>LLM: Optimized prompt (within budget)
    LLM-->>Agent: Response
    Agent->>Budget: Record usage
    Agent-->>User: Response
    Note over Budget: Alerts when 80% used
```
