...
|-------|------|---------|-------------|
| `success` | `bool` | `True` | `False` when any file failed to index (i.e. `errors` is non-empty). A partial failure — even one file out of many — counts as `success=False`, since a knowledge base quietly missing a document is the loss this flag exists to surface. |
...
k = Knowledge()
result = k.index("./docs", memory={"user_id": "alice"})

if not result.success:
    # One or more files failed to embed — do not treat the corpus as ready.
    for err in result.errors:
        print(f"failed: {err}")
    raise SystemExit(1)

print(f"indexed {result.files_indexed} file(s), {result.chunks_created} chunk(s)")
```
