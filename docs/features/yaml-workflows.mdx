...
| `run_on` | ❌ | `None` | Silent alias for `tools_run_on:` — a **shared remote sandbox** used by every step's shell / file tools. Accepts: `docker`, `e2b`, `modal`, `daytona`, `flyio`, `tenki`, `local`. Validated at load time. See [Shared Sandbox](/features/shared-sandbox) and [Placement](/features/placement). |
...
<Note>
YAML `run_on:` is a **silent alias** for `tools_run_on:` — no warning, no breakage. The Python equivalent is `AgentFlow(tools_run_on=…)`. See [Placement](/features/placement).
</Note>
...
Omit `run_on:` and nothing changes — the workflow runs locally as before. A typo is caught at load time, not mid-run. See [Shared Sandbox](/features/shared-sandbox) for providers, validation, and the Python `AgentFlow(tools_run_on=…)` equivalent.
