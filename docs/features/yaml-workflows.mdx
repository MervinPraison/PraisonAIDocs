...
| `run_on` | ❌ | `None` | Provider name for a **shared remote sandbox** used by every step's shell / file tools. Accepts: `docker`, `e2b`, `modal`, `daytona`, `flyio`, `tenki`, `local`. Validated at load time. See [Shared Sandbox](/features/shared-sandbox). |
...
Setting an explicit `tool_timeout` on a `process: workflow` YAML now **raises `ValueError`** and aborts the run.

<Warning>
**Behaviour change — action required if you had `tool_timeout` on a `process: workflow` YAML.**
Previously the value was silently ignored. It now **aborts the run** with `ValueError`. Either:
1. Remove `tool_timeout` from the workflow YAML, or
2. Change `process:` to `sequential` or `hierarchical` (both enforce it).

A CLI `--tool-timeout` set to the default value (60) is still safe.
</Warning>

The native `process: workflow` engine resolves its own tools by name and bypasses the per-tool timeout wrapper. Accepting the field would silently mean "same YAML, different execution semantics depending on `process:`", so the workflow path fails fast instead:

```
tool_timeout=<value> is not supported for 'process: workflow' YAMLs;
per-tool timeouts are only enforced on the sequential/hierarchical path.
Either remove tool_timeout or convert to a sequential/hierarchical process.
```

Only an **explicit** request is rejected — a per-agent/role YAML `tool_timeout:` value, or a CLI `--tool-timeout N` value that differs from the default (60). The bare CLI default is ignored so ordinary workflow runs are unaffected.

Use `process: sequential` or `process: hierarchical` when you need per-tool timeouts enforced.

```mermaid
graph TB
    Start["YAML sets tool_timeout?"] --> Q1{"process: value?"}
    Q1 -->|sequential / hierarchical| Enforce["✅ Enforced (tightest value wins)"]
    Q1 -->|workflow| Q2{"Explicit request?"}
    Q2 -->|"per-agent/role, or --tool-timeout ≠ 60"| Fail["❌ ValueError — abort the run"]
    Q2 -->|"only bare CLI default (60)"| Ignore["⚠️ Ignored — run continues"]
...
Behaviour changed in [PR #3963](https://github.com/MervinPraison/PraisonAI/pull/3963) (fixes #3962). Previously ([PR #3252](https://github.com/MervinPraison/PraisonAI/pull/3252)) this was a warn-only no-op. See [YAML Reference → tool_timeout](/docs/features/yaml-configuration-reference#recognized-fields) for the field's full contract on the sequential/hierarchical path.
