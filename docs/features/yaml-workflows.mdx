...
| Field | Required | Default | Description |
|-------|:--------:|---------|-------------|
| `agent` | ✅* | - | Agent to execute (*not needed for parallel/route) |
| `action` | ❌ | `{{input}}` | What the step does |
...
<Note>
**How it works**: When `output_json` or `output_pydantic` is specified, PraisonAI uses the LLM's native structured output feature (`response_format`) for models that support it (GPT-4o, GPT-4o-mini, Gemini). For other models, it falls back to prompt engineering with schema instructions.
</Note>
