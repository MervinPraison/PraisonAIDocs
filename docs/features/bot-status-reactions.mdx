...
<Note>
This page covers **outbound** status reactions — the bot **adding** an emoji to the user's message to show run state. For the **inbound** counterpart — receiving the user's own reactions (👍/✅), edits, deletions, and other platform events — see [Inbound Platform Events](/features/bot-inbound-events).
</Note>
...
See [Run Status Controller](/features/bot-run-status-controller) for the low-level API.
</Accordion>
</AccordionGroup>
