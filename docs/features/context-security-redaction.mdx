...
# Register once at process start — all agents in this process
# now have their outgoing messages scrubbed before hitting the LLM.
...
agent.chat("My api_key=sk-ABCDEF12345 and password=hunter2 — help me debug?")
# → the literal secret is replaced with [REDACTED] before the request goes out.
```

`enable_pii_redaction()` registers a **sequential** `BEFORE_LLM` hook. Sequential is what actually mutates the outgoing message stream — parallel hooks cannot write `modified_input` back. If you built your own PII-style hook, register it with `sequential=True` or it will silently no-op. (Fixed in PraisonAI [PR #4034](https://github.com/MervinPraison/PraisonAI/pull/4034), released with `praisonaiagents` versions built from `main` on/after 2026-08-19.)

```mermaid
sequenceDiagram
    participant User
    participant Agent
    participant Hook as BEFORE_LLM (sequential)
    participant LLM

    User->>Agent: chat("api_key=sk-... password=hunter2")
    Agent->>Hook: scrub outgoing messages
    Hook-->>Agent: modified_input (secrets → [REDACTED])
    Agent->>LLM: scrubbed payload
    LLM-->>User: response without leaked secrets
```

<Note>
The scrubbing runs as a `sequential=True` `BEFORE_LLM` hook. See [Sequential vs Parallel Hooks](/features/hooks#sequential-vs-parallel-hooks) for why only sequential hooks can rewrite the payload.
</Note>
...
<Note>
Sharing a saved session? Redact it at export time with `praisonai session export <id> --sanitise` — the CLI writes stable `[redacted:<category>:<n>]` placeholders in place of secrets and absolute paths, and never mutates the on-disk session. See [Sanitise before sharing](/docs/cli/session#sanitise-before-sharing).
</Note>
...
<Card title="Sanitise Session Export" icon="broom" href="/docs/cli/session#sanitise-before-sharing">
