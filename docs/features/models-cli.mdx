...
<Steps>
  ...
  <Step title="Inspect a model">
    ...
    On a non-UTF-8 terminal (Windows cp1252, redirected pipes), the same command prints ASCII labels instead of emoji:
    ```text
    Model: gpt-4o
    Provider: openai
  ...
  <Step title="Validate a model ID">
    ...
    On a non-UTF-8 terminal the same checks drop the emoji markers:
    ```text
    'gpt-4o' is a valid model
    'gpt-4o-mni' is not a valid model
    Did you mean one of these?
...
sequenceDiagram
    ...
    ModelCatalogue->>Cache: load (~/.praisonai/cache/models.json)
...
|--------|-----------|-----------------|
| `~/.praisonai/cache/models.json` | Cache < 1 h old | Full list saved from previous live call |
...
## Encoding-aware Output

As of [PraisonAI PR #3661](https://github.com/MervinPraison/PraisonAI/pull/3661), the `models` subcommands render **ASCII labels** on any terminal whose stdout cannot encode emoji (Windows cp1252/cp850, redirected pipes, minimal SSH sessions). UTF-8 terminals still get the emoji output shown above.

```mermaid
graph LR
    subgraph "Encoding-aware Rendering"
        S[💻 stdout] --> P{"stdout_supports_unicode()?"}
        P -->|UTF-8| E[🔧 emoji labels]
        P -->|cp1252 / pipe / SSH| A[ASCII labels]
        E --> O[✅ safe output]
        A --> O
    end
...
| Subcommand | UTF-8 stdout | Non-UTF-8 stdout (cp1252, redirected pipes, SSH) |
|---|---|---|
| `models list` capability column | `🔧 tools 👁️ vision 🧠 reasoning` | `tools vision reasoning` |
| `models describe` capabilities section | `- Tool calling: ✅ / ❌` | `- Tool calling: yes / no` |
| `models validate` success marker | `✅ '<id>' is a valid model` | `'<id>' is a valid model` |
| `models validate` failure marker | `❌ '<id>' is not a valid model` | `'<id>' is not a valid model` |
| `--json` output | unchanged | unchanged |

The probe encodes `"\U0001f527"` in `sys.stdout.encoding`; a `UnicodeEncodeError` or `LookupError` triggers the ASCII fallback. The probe is automatic — there is no flag, environment variable, or config toggle. On legacy Windows consoles, opt back into emoji with `chcp 65001`.

Extend downstream commands with the same probe:

```python
from praisonai_code.cli.output.console import stdout_supports_unicode

yes, no = ("✅", "❌") if stdout_supports_unicode() else ("yes", "no")
print(f"Tool calling: {yes}")
```

Pick the output that fits where you run:

```mermaid
graph TB
    Q{Where are you running the CLI?}
    Q -->|Windows PowerShell default| ASCII["ASCII labels (auto)"]
    Q -->|Windows Terminal / macOS / Linux| EMOJI["Emoji labels (auto)"]
    Q -->|Piping to a file, jq, CI| JSON["praisonai models list --json"]

    classDef q fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef opt fill:#6366F1,stroke:#7C90A0,color:#fff
    class Q q
    class ASCII,EMOJI,JSON opt
```

```mermaid
sequenceDiagram
    participant User
    participant CLI as praisonai models list
    participant Probe as stdout_supports_unicode()
    participant Table as Rich / print_table

    User->>CLI: praisonai models list
    CLI->>Probe: check sys.stdout.encoding
    alt UTF-8
        Probe-->>CLI: True
        CLI->>Table: render with 🔧 / 👁️ / 🧠
    else cp1252 / pipe / SSH
        Probe-->>CLI: False
        CLI->>Table: render with "tools" / "vision" / "reasoning"
    end
    Table-->>User: capabilities column (never crashes)
```

When `rich` is not installed, `models list` renders a plain-text table via the same encoding-aware helper, so ASCII labels apply there too.
...
|--------|------|---------|-------------|
| `cache_dir` | `Path \| None` | `~/.praisonai/cache` | Directory for the model cache file (honours `PRAISONAI_HOME`) |
...
**Cache file location:** `~/.praisonai/cache/models.json` (honours `PRAISONAI_HOME`)

**Force refresh:** delete the cache file, then run any `praisonai models` command:

```bash
rm ~/.praisonai/cache/models.json
praisonai models list
```
...
<CardGroup cols={2}>
  <Card title="Standalone LLM Modules" icon="plug" href="/features/standalone-llm-modules">
    All four LLM modules available without the wrapper package
  </Card>
  <Card title="AutoGen Config List" icon="list-tree" href="/features/llm-autogen-config-list">
...
*C7 note: As of PR #2550, the tool-discovery pipeline (`tool_resolver`), safe loader (`_safe_loader`), framework probes (`_framework_availability`), and plugin registry (`tool_registry`) all live in `praisonai_code` — the same C7 arc that moved LLM config here. See [Tool Discovery Order](/features/tool-discovery-order) and [Local Tools Loading](/features/local-tools-loading).*
