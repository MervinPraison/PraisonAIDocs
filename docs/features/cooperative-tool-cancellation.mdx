...
sequenceDiagram
    participant User
    participant Agent
    participant Stream as get_response_stream
    participant Exec as execute_batch
    participant Tool as Pending tools
    participant Model

    User->>Agent: start(prompt, stream=True)
    Agent->>Stream: forward cancel_token
    Stream->>Model: request (tools=...)
    Model-->>Stream: tool_calls
    User->>Agent: interrupt_controller.request("user_stop")
    Stream->>Exec: execute_batch(..., cancel_token=token)
    Exec-->>Stream: cancelled ToolResult (pending calls skipped)
    Note over Stream,Model: follow-up completion request is NOT issued
    Stream-->>Agent: stream ends
    Agent-->>User: streamed output stops promptly
    ...
    class User user
    class Agent agent
    class Stream,Exec stream
    class Tool tool
    class Model model
```

| Coverage | Non-streaming (`chat()`) | Streaming (`start(stream=True)` / `iter_stream()`) |
|---|---|---|
| Cancel before model returns tool_calls | ✅ | ✅ |
| Pending tool batch short-circuits on Stop | ✅ | ✅ (since PR #5081) |
| Follow-up completion request skipped on Stop | ✅ | ✅ (since PR #5081) |
| Fallback loop (Anthropic / Gemini / Ollama with tools) honours Stop | ✅ | ✅ (since PR #5081) |
