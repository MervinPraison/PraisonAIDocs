...
| **⬡ Managed** | *Where does it actually run?* | `backend=LocalAgent(compute="e2b")` for one agent, or `AgentFlow(run_on="docker")` / `AgentTeam(run_on="docker")` to share one sandbox across every agent in the run — or `run_on: docker` at the top of an `agents.yaml` / `workflow.yaml` for the no-code path |
