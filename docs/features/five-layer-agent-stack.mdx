...
| **⬡ Managed** | *Where does it actually run?* | `tools_run_on=` on any of `Agent` / `AgentFlow` / `AgentTeam` / `LocalAgent` to move tools to a sandbox — or `run_on: docker` at the top of an `agents.yaml` / `workflow.yaml` for the no-code path. See [Placement](/features/placement). |
...
sandboxed = ManagedAgent(
    provider="local", tools_run_on="e2b",   # or modal | daytona | flyio | docker | tenki | sandlock | novita | ssh
