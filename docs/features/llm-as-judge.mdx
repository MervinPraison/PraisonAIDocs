...
judge = Agent(name="judge", instructions="Evaluate agent outputs for quality and accuracy.")
agent = Agent(name="writer", instructions="Write a short essay.")
output = agent.start("Write about climate change.")
...
```mermaid
graph LR
    Output[Agent Output] --> Judge[LLM Evaluator]
    Judge --> Score[Score + Pass/Fail]
...
sequenceDiagram
    ...
    User->>Agent: Run a task
    Agent-->>User: Output
    User->>Judge: run(output, expected/criteria)
    Judge->>LLM: Score against the rubric
    LLM-->>Judge: Score + reasoning
    Judge-->>User: JudgeResult (score, passed, suggestions)
```

| Phase | What happens |
|---|---|
| 1. Produce | The task agent generates an output |
| 2. Submit | You pass the output plus `expected` or `criteria` to `Judge.run()` |
| 3. Score | The judge LLM rates the output on a 1–10 scale |
| 4. Return | `JudgeResult` reports the score, pass/fail, and improvement suggestions |
...
```mermaid
graph TB
    Q[How do you score?] --> A[One rubric, one model]
    Q --> B[Reduce single-model bias]
    Q --> C[Compare two candidates]
    A --> R1[Single Judge]
    B --> R2[Panel: average several judges]
    C --> R3[Pairwise: A vs B]
...
| `passed` | boolean | `True` only when a numeric score was parsed AND `score >= threshold`. An unparseable verdict is always `passed=False`. |
...
Empty responses, JSON `{}`, and prose-only replies like `"the answer is wrong"` all fail closed.

Markdown-bolded labels like `**SCORE:** 1` (the default output style of most models) and bullet-prefixed labels are tolerated — the parser strips `**`, `*`, `` ` ``, and leading `-`/`>` before reading the value. Prompts no longer need to warn the model against bolding.

```mermaid
graph LR
    Input[LLM Response] --> Parse{Numeric<br/>SCORE?}
    Parse -->|Yes| Score[Parsed Score]
    Parse -->|No| Fail[Score 1.0]
    Score --> Compare[Compare to threshold]
    Compare --> Result[Pass / Fail]
    Fail --> Blocked[passed=False]
    ...
    class Score,Compare process
    class Result output
    class Fail,Blocked warn
```

The safety grader (`BaseLLMGrader`) already followed this pattern — `Judge` now matches it. See [Safety Evaluation](/features/safety-eval).
