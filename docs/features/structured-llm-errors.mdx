...
<Tip>
Every `Agent` retries retryable `LLMError`s by default using jittered exponential backoff — pass `retry=False` to opt out. See [Agent Retry](/docs/features/agent-retry) for the full API.
</Tip>
