...
<Tip>
Every `Agent` retries retryable `LLMError`s by default using jittered exponential backoff — pass `retry=False` to opt out. See [Agent Retry](/features/agent-retry) for the full API.
</Tip>
...
| `error_category` | `AgentErrorKind` | Typed classification — see [LLM Error Classification](/features/llm-error-classification) |
...
- **Default / `retry=` set**: the loop honours `RetryBackoffConfig.max_retries` (e.g. default and `retry=True` → 3, `max_retries=0` → recursion disabled).
- **With `retry=False`**: the loop falls back to 2, preserving prior behaviour.
- **Context compression**: triggered on the `context_overflow` category.

<Note>
This bound is the **recursion depth** default, read from `Agent._max_retry_depth()`, which returns `RetryBackoffConfig.max_retries` when a retry config is present and `2` when retry is disabled. It is a **different loop** from the transient-LLM retry loop: [transient LLM retries](/features/agent-retry) default to **3** and are on by default on both the native and LiteLLM paths, whereas this recursion budget bounds the recursive recovery after compression/backoff. See [Agent Retry](/features/agent-retry) for how to configure both.
</Note>
...
<Card title="Model Failover" icon="arrows-rotate" href="/features/failover">
