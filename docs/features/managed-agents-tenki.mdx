...
| `networking` | `dict` | `{"type": "unrestricted"}` | Only `"unrestricted"` allows outbound; anything else disables it |
| `image` | `str` | *(SDK default)* | Ignored unless explicitly changed; Tenki has no Docker-pull |
| `metadata["tenki_image"]` | `str` | — | Boot a prebaked Tenki snapshot (takes precedence over `image`) |
