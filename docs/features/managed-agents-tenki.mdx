...
managed = ManagedAgent(
    ...
    compute="tenki",  # requires TENKI_API_KEY
)
...
flow = AgentFlow(run_on="tenki", steps=[...])
```

<Note>
`run_on="tenki"` is not Tenki-specific. As of [PR #4071](https://github.com/MervinPraison/PraisonAI/pull/4071) the same pattern hosts the whole loop on any of the eight places — `anthropic`, `docker`, `e2b`, `modal`, `daytona`, `flyio`, `tenki`, `novita`. See [Placement](/features/placement#where-run-on-and-compute-can-point).
</Note>

See [Shared Sandbox](/features/shared-sandbox) and [Placement](/features/placement) for the full `run_on=` behaviour.
