...
Add `allow_shell: true` to any channel. On a **loopback-bound gateway with a single-operator DM**, auto-approve every command:
...
channels:
  slack:
    ...
    allow_shell: true
    auto_approve_shell: true   # kept: loopback bind + no group_policy
```
...
<Warning>
On an **externally-bound gateway** (`0.0.0.0` or a real interface) or **any channel with a `group_policy`**, `auto_approve_shell: true` is automatically downgraded to require approval. Add `auto_approve_shell_acknowledge_exposed: true` to keep blanket auto-approval — see [Exposure-aware auto-approval](#exposure-aware-auto-approval).
</Warning>
</Step>
...
## Exposure-aware auto-approval

`auto_approve_shell: true` runs every shell call immediately **only** when the gateway is loopback-bound *and* the channel is a single-operator DM. On any exposed or multi-user surface it is silently downgraded to an approval backend — the exact wiring `auto_approve_shell: false` produces.

```mermaid
graph TB
    Start[🛡️ auto_approve_shell: true] --> BindQ{🌐 Gateway bound<br/>to loopback?}
    BindQ -->|No| AckQ{🔐 acknowledge_exposed<br/>= true?}
    BindQ -->|Yes| GroupQ{👥 group_policy<br/>set?}
    GroupQ -->|Yes| AckQ
    GroupQ -->|No| Auto[✅ AutoApproveBackend]
    AckQ -->|Yes| Auto
    AckQ -->|No| Downgrade[⚠️ Downgrade →<br/>Approval backend<br/>WARN logged]
    ...
    class Start start
    class BindQ,GroupQ,AckQ decision
    class Auto safe
    class Downgrade warn
```

This mirrors the gateway auth token's "calibrated by exposure" posture (`assert_external_bind_safe`): blanket auto-approve is only trusted where a single operator reaches the bot over loopback.

### Behaviour matrix

| Gateway bind | Channel `group_policy` | `auto_approve_shell_acknowledge_exposed` | Effective backend |
|---|---|---|---|
| loopback (`127.0.0.1` / `::1`) | unset (DM / non-group) | — | Blanket auto-approve (`AutoApproveBackend`) — unchanged behaviour |
| externally-bound (`0.0.0.0` or a real interface) | any | `false` (default) | **Downgraded to approval backend** (Slack / Telegram / Discord / HTTP / gateway approval — same routing as `auto_approve_shell: false`) |
| loopback | any group policy (`mention_only`, `command_only`, `all`, …) | `false` (default) | **Downgraded to approval backend** |
| externally-bound **or** group channel | any | `true` | Blanket auto-approve (explicit opt-in — WARN not emitted) |
| bind host unknown / unresolvable | — | — | **Fail closed** — treat as exposed, downgrade to approval |

<Note>
`command_only` counts as multi-user: a group member's `/cmd` still reaches `execute_command`, so it downgrades too. The gateway passes its resolved bind host to `enable_shell_tools()` — the per-channel config does **not** carry a `bind_host:`; set it under the top-level `gateway:` block.
</Note>

The downgrade routes through the **same** approval-backend precedence documented under [Choosing an approval_mode](#choosing-an-approval-mode) (Slack / Telegram / Discord / HTTP / gateway → fail-closed `CallbackBackend`). No new backend, no new precedence — only the trigger changes. Watch for this WARN line in gateway logs:

```
Channel 'slack' enables shell on an exposed/multi-user surface;
downgrading auto_approve_shell to require approval. Set
auto_approve_shell_acknowledge_exposed: true to keep blanket auto-approval.
```

### Configurations

<Tabs>
<Tab title="Safe default (loopback DM)">
Unchanged behaviour — loopback bind, no group policy, blanket auto-approve kept.
...
<Tab title="Exposed — downgraded automatically">
Externally-bound gateway on a group channel — `auto_approve_shell: true` silently becomes "approval required".
...
channels:
  slack:
    ...
    app_token: "${SLACK_APP_TOKEN}"
    allow_shell: true
    auto_approve_shell: true   # silently becomes "approval required"
    # WARN in gateway log:
    #   Channel 'slack' enables shell on an exposed/multi-user surface;
    #   downgrading auto_approve_shell to require approval. Set
    #   auto_approve_shell_acknowledge_exposed: true to keep blanket auto-approval.
...
<Tab title="Exposed — acknowledged">
Explicit acknowledgement keeps blanket auto-approve on an exposed surface. No WARN is emitted.
...
channels:
  slack:
    ...
    auto_approve_shell_acknowledge_exposed: true   # I accept the RCE surface
```
</Tab>
</Tabs>
...
Nine per-channel fields control shell execution and approval routing.
...
| `auto_approve_shell` | `bool` | `True` | Runs every shell call immediately (`AutoApproveBackend`) **only on a loopback-bound gateway + non-group channel**. On externally-bound gateways or any channel with a `group_policy`, the gateway downgrades this to the approval backend (see [Exposure-aware auto-approval](#exposure-aware-auto-approval) and `auto_approve_shell_acknowledge_exposed`). If `False`, always wires a human-in-the-loop backend via the routing ladder below. |
| `auto_approve_shell_acknowledge_exposed` | `bool` | `False` | Explicit acknowledgement that blanket auto-approval is safe on this externally-exposed or multi-user channel. Set to `true` (with `auto_approve_shell: true`) to keep blanket auto-approve on `0.0.0.0` binds or group channels. Off by default — the gateway fails closed to approval. |
...
If the gateway cannot determine its bind interface (host unresolvable), the [exposure check](#exposure-aware-auto-approval) also fails closed and blanket auto-approve is downgraded to require approval.
...
Fastest to demo — every shell call runs immediately on a loopback-bound gateway with a DM channel.
...
On `0.0.0.0` or a channel with a `group_policy`, add `auto_approve_shell_acknowledge_exposed: true` — otherwise this downgrades to approval. See [Exposure-aware auto-approval](#exposure-aware-auto-approval).
</Tab>
...
```yaml
gateway:
  bind_host: "127.0.0.1"           # loopback: Slack blanket auto-approve stays
...
channels:
  slack:
    ...
    app_token: "${SLACK_APP_TOKEN}"
    allow_shell: true
    auto_approve_shell: true          # kept only on loopback + non-group DM
...
On a `0.0.0.0` bind — or if the Slack channel carries a `group_policy` — the Slack line downgrades to approval unless you add `auto_approve_shell_acknowledge_exposed: true`. See [Exposure-aware auto-approval](#exposure-aware-auto-approval).
</Tab>
</Tabs>
...
Set `auto_approve_shell: true` only on a loopback-bound gateway with a single-operator DM. On externally-bound gateways or group channels the gateway downgrades it to approval automatically; overriding with `auto_approve_shell_acknowledge_exposed: true` re-opens a remote-code-execution surface — only do so on a channel you fully trust. See [Exposure-aware auto-approval](#exposure-aware-auto-approval).
</Accordion>
...
<Note>
This page's channel-level `allow_shell` enables the agent's `execute_command` **tool**. A different `allow_shell` governs live `` !`cmd` `` **template substitution** inside file-based custom commands, which follows a stricter **double opt-in** rule: both the deployment (`commands.allow_shell: true`) and the command's own `allow_shell: true` frontmatter must agree, or the shell segment renders inert. See [File-based & Entry-point Commands → Safety posture](/features/bot-commands#safety-posture-double-opt-in-for-shell).
</Note>

## Related

<CardGroup cols={2}>
<Card title="Chat Commands" icon="terminal" href="/features/bot-commands">
  Built-in, Python-registered, file-based, and entry-point commands — with the double-opt-in shell rule.
</Card>
...
<Card title="Bot Default Tools" icon="wrench" href="/features/bot-default-tools">
  How tools get injected on bot agents.
</Card>
<Card title="Durable Approvals" icon="database" href="/features/durable-approvals">
