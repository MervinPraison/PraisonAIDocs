...
| `agent_name` | `str` | Always `"background"` for jobs the manager runs |
| `session_id` | `str` | Origin session captured at spawn time (empty when unset) |
...
Origin fields (`session_id`, `deliver`, `platform`, `chat_id`, `thread_id`) are always strings — an empty string when not set.
