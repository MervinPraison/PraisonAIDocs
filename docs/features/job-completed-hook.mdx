...
`HookEvent.JOB_COMPLETED` fires from `BackgroundJobManager.start_job` whenever any managed background job reaches a terminal state — whether it succeeded or failed. Register a hook to log completions, send alerts, or trigger follow-up work.
...
sequenceDiagram
    ...
    User->>Agent: "Analyse this repo and email me when done"
    Agent->>Job: spawn_subagent(background=True)
    Agent-->>User: Job started (job_id)
    Job->>Job: Run work
    Job->>Hook: JobCompletedInput (status, result, error)
    Hook-->>User: Log / alert / follow-up
```
...
`JobCompletedInput` carries everything about the finished job, including the delivery context if one was set. All origin fields are strings — empty (`""`) when not set.
...
| `session_id` | `str` | Origin session id captured at spawn time (empty string when unset) |
...
- **Fires for every managed background job**, not only those launched with `spawn_subagent(background=True)` — the emission lives in `BackgroundJobManager.start_job` itself.
- **Fires on both success and failure.** Check `data.status` to distinguish `"completed"` from `"failed"`.
- **Does not fire for `CANCELLED` or `LOST`.** The event documents "finished ok or error"; PR #4994 deliberately did not widen that contract.
...
<Accordion title="My deliver field is '' — is that a bug?">
No. `deliver` is empty for any background job the caller did not route back to a conversation. `JOB_COMPLETED` fires for every managed job, so an empty `deliver` just means this job had no delivery target — treat it as observe-only.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="fire_hook" icon="megaphone" href="/features/fire-hook">
