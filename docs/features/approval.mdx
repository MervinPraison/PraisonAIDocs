...
<Note>
**Restart safety.** When a `PresentationApprovalHandler` is configured with an `ApprovalStore`, pending approvals are persisted to SQLite — a late Allow/Deny tap after a gateway or bot restart still resolves. The `ApprovalRequest` fields `approval_id`, `agent_name`, and `session_id` are stable across restarts and preserved in the audit trail. See [Durable Approvals](/docs/features/durable-approvals).
</Note>
