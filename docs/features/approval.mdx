...
`mark_approved()` and `is_already_approved()` now take the effective **arguments** alongside the tool name. The `@require_approval` decorator binds the call's arguments via `inspect.signature`, merges any `modified_args` the approval callback returns, and derives the cache key from that effective call — not from the tool name alone.

```python
mark_approved(tool_name: str, arguments: dict | None = None) -> None
is_already_approved(tool_name: str, arguments: dict | None = None) -> bool
```

| Item | Old behaviour (≤ PR#3614) | New behaviour (≥ PR#3614) |
|------|---------------------------|---------------------------|
| Approval cache key | `tool_name` only | `(tool_name, effective_arguments)` |
| `mark_approved("write_file")` | Unlocks **any** future `write_file(...)` call | Only unlocks `write_file()` with **no args** |
| `mark_approved("write_file", arguments={"path": "/tmp/a"})` | Not supported | Unlocks only that exact call |
| `modified_args` from the callback | Ignored for the cache key | Rolled into the cache key, so the effective (sanitised) call is what's remembered |
