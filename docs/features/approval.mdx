...
For persistent approvals across sessions, see [Reusable Approval Scopes](/docs/features/reusable-approval-scopes) — the pattern is derived from a small command-arity table so `git status -s` and `git status --short` share one rule (`bash:git status *`).
