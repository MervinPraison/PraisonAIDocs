...
<Note>
On the console backend, the prompt now offers **five** choices: `[o] once`, `[s] this session`, `[a] always`, `[n] no`, `[d] deny & redirect`. See [Interactive Tool Approval](/docs/features/interactive-approval#scope-choice) for the full scope semantics.
</Note>
...
See [Approval Protocol → Live-Authority Binding](/features/approval-protocol#live-authority-binding).
...
This is the core-SDK console preview — it reaches any backend. The `praisonai-code` Rich/Textual TUI has its own [Change Preview](/features/interactive-approval#change-preview); the two are complementary. See [Approval Backends](/features/approval-backends) for how each backend surfaces the diff.
...
<Note>
**Reviewing task outputs, not tool calls?** See [Human-in-the-Loop Review](/features/human-in-the-loop-review) for `Task(human_input=True)`.
</Note>

## Related

<CardGroup cols={2}>
<Card icon="user-check" href="/features/human-in-the-loop-review">
Require a person to sign off a task's output before the next task consumes it
</Card>
