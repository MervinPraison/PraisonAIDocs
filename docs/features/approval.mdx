...
No agent config needed — the tool carries its own approval requirement everywhere it goes. The decorator declaration **composes** with the agent-level `approval=` config: the decorator says *what* needs approval, the agent config says *how* to ask.
...
<Card title="Tool Requires Approval" icon="shield-check" href="/docs/features/tool-requires-approval">
  Full reference for the `@tool(requires_approval=...)` decorator parameter
</Card>
