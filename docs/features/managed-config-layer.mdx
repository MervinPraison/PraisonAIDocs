...
- **On-disk cache** — the last good copy is cached under `~/.praison/state/`. On any fetch failure the cached copy is used; if there's none, the layer is skipped and resolution is local-only.
...
The last-good copy under `~/.praison/state/` keeps policy in effect offline. Fetch failures never block a run.
</Accordion>
</AccordionGroup>
