...
agent = Agent(name="a2a-worker", instructions="Handle long-running A2A tasks with push updates.")
agent.start("Submit this task and notify my webhook when it completes.")
```

<Note>
**A2A push notifications** deliver HTTP webhooks when long-running A2A tasks change state. For **real-time channel-based pub/sub** between gateway and clients, see [Real-Time Push Notifications](/features/push-notifications).
</Note>
...
```python
from praisonaiagents import Agent, A2A, TaskPushNotificationConfig, AuthenticationInfo
...
<CardGroup cols={2}>
<Card title="A2A Protocol" icon="plug" href="/features/a2a">
  Core A2A server setup and configuration
</Card>
<Card title="A2A Tasks" icon="list-check" href="/features/a2a-tasks">
  Task lifecycle and state management
</Card>
<Card title="A2A Client" icon="terminal" href="/features/a2a-client">
  Client SDK with polling examples
</Card>
<Card title="Async Features" icon="rocket" href="/features/async">
