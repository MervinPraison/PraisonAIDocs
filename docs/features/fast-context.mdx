...
<Tabs>
  <Tab title="Ripgrep Backend">
    Use ripgrep for faster pattern matching on large codebases:
    
    ```python
    from praisonaiagents.context.fast import FastContext
    
    # Use ripgrep if installed, else fallback to Python
    fc = FastContext(
        workspace_path=".",
        search_backend="ripgrep"  # "auto" | "python" | "ripgrep"
    )
    ```
