...
# Routes through litellm — same as Agent(llm="ollama/llama3.2").
agent = Agent(instructions="Answer briefly.")
agent.start("2+2")
```

As of [PR #4795](https://github.com/MervinPraison/PraisonAI/pull/4795), `OPENAI_MODEL_NAME="ollama/x"` (or any `provider/model` value) routes to litellm, matching `Agent(llm="ollama/x")`. Before the fix, those two paths disagreed — an `OPENAI_MODEL_NAME` with a provider prefix ended up on the native OpenAI client with a model name OpenAI never heard of. A bare value like `OPENAI_MODEL_NAME="gpt-4o"` still uses the native OpenAI client.

---

## Configuration Options

<Card title="LLMConfig SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/feature_configs">
...
# The container unwraps LLMConfig to its .model string,
# then fills in members that didn't name a model themselves.
team = AgentTeam(
    agents=[researcher, writer],
    llm=LLMConfig(model="anthropic/claude-3-5-sonnet-20241022"),
)
...
<Accordion title="For team-wide endpoint or fallback config, pin per-agent">
...
<CardGroup cols={2}>
<Card title="LLM Gateways" icon="plug" href="/features/llm-gateways">
  Route requests through a unified LLM gateway
</Card>
<Card title="LLM Endpoint Config" icon="settings" href="/features/llm-endpoint-config">
  Fine-tune endpoint parameters per model
</Card>
<Card title="LLM Error Classification" icon="triangle-alert" href="/features/llm-error-classification">
  How errors trigger fallback in the chain
</Card>
<Card title="Failover" icon="shield" href="/features/failover">
