...
mcp = MCP("npx -y @modelcontextprotocol/server-filesystem /tmp", allowed_tools=["read_file", "list_directory"])
agent = Agent(name="Reader", instructions="Read files only — never write.", tools=mcp)
agent.start("List files in /tmp and read the smallest one.")
```
...
sequenceDiagram
    participant User
    participant Agent
    participant McpToolFiltering

    User->>Agent: Request
    Agent->>McpToolFiltering: Process
    McpToolFiltering-->>Agent: Result
    Agent-->>User: Response
```
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as MCP Tool Filtering
...
<Card title="MCP Client Protocol" icon="code" href="/docs/features/mcp-client-protocol">
