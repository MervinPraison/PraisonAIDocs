...
<CodeGroup>
```python Status Output (Recommended for CLI)
...
```python Trace Output (With Timestamps)
from praisonaiagents import Workflow, Agent
...
```python Verbose Output (Rich Panels)
from praisonaiagents import Workflow, Agent
...
```python
from praisonaiagents import Workflow, WorkflowStep
from praisonaiagents import WorkflowManager, WorkflowStepContextConfig, WorkflowStepOutputConfig
...
workflow = Workflow(
    ...
    steps=[
        WorkflowStep(
            ...
            output=WorkflowStepOutputConfig(variable="research_data")  # Store as custom variable
        ),
        WorkflowStep(
            name="analyze",
...
| `WorkflowStep` | Reference to `agent` field | Work item with agent reference |
...
| **Work Item** | `Task` (contains agent ref) | `WorkflowStep` (contains agent ref) |
| **Executor** | `Agent` | `Agent` (same!) |

> **Key Insight:** WorkflowStep is to Workflow what Task is to Agents — it's a work item that **references** an agent, not an agent itself.
