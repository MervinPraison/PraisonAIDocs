...
<AccordionGroup>
  <Accordion title="Advanced: WorkflowPlanningConfig class">
    <Note>
    `WorkflowPlanningConfig` is not yet exported at the top level, so it uses a deep import. Prefer the `bool` or `dict` form above.
    </Note>
...
<AccordionGroup>
  <Accordion title="Advanced: WorkflowMemoryConfig class">
    <Note>
    `WorkflowMemoryConfig` is not yet exported at the top level, so it uses a deep import. Prefer the `bool` or `dict` form above.
    </Note>
...
<Note>
`AgentFlow` is the primary workflow surface. `WorkflowManager` is a secondary API for running markdown-defined workflows and is not yet exported at the top level, so it uses a deep import.
</Note>
