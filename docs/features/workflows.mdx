...
flow.start("Research and summarise AI agents")
```

The user defines steps once; the workflow passes context between agents automatically.
...
workflow = AgentFlow(steps=[researcher, writer], planning=True)
result = workflow.start("Research and write about AI trends")
```

Turn planning on with `planning=True`. For a specific planning model and chain-of-thought reasoning, pass a `WorkflowPlanningConfig`:
...
<Note>
`WorkflowPlanningConfig` imports from `praisonaiagents.workflows`. For most cases, `planning=True` is all you need.
</Note>

**`WorkflowPlanningConfig` options:**
...
workflow = AgentFlow(steps=[researcher, writer], memory=True)

result1 = workflow.start("Research AI trends")
result2 = workflow.start("Continue the research on LLMs")
```

Turn memory on with `memory=True`. To pick a backend or scope memory to a user, pass a `WorkflowMemoryConfig`:
...
<Note>
`WorkflowMemoryConfig` imports from `praisonaiagents.workflows`. For most cases, `memory=True` is all you need.
</Note>

**`WorkflowMemoryConfig` options:**
...
<Note>
`WorkflowManager` imports from `praisonaiagents.workflows`.
</Note>
