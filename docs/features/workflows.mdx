...
Save progress with `checkpoint=` and continue an interrupted run with `resume=`.
...
`aexecute()` accepts the same `checkpoint` and `resume` parameters and now supports `loop_over` and `branch_condition` routing with full parity to `execute()`.
