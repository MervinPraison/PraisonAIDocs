...
<Note>
`AgentFlow(model="gpt-4o")` is now accepted (canonical) as the default model for steps that did not name one. `llm=` still works as a deprecated alias — see [Model Parameter](/features/model-parameter).
</Note>
...
## Preview a Workflow Before It Runs

Call `flow.to_mermaid()` to render a workflow's shape as a mermaid diagram — no run, no cost.
...
See [Workflow Diagram](/features/workflow-diagram) for how each step type renders.
...
graph TB
    ...
    Q --> H[N agents take turns on a thread]
    ...
    class Q q
    class A,B,C,D,E,F,G,H need
    class A1,B1,C1,D1,E1,F1,G1,H1 ans
```
...
| `Discussion([a, b], rounds=3)` | `from praisonaiagents import Discussion` | N agents take turns on a shared thread |
...
Cache a deterministic step's output by its inputs so it is reused across runs instead of paying the full LLM cost every time.
...
flow = AgentFlow(steps=[formatter], cache=True)                                  # simplest
flow = AgentFlow(steps=[formatter], cache=InMemoryStepCache(max_entries=256))    # tuned
```

Caching is opt-in — only successful steps of a deterministic step are stored, and both linear and pattern executors are covered.

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `cache` | `Any` | `None` | `True` for a process-scoped LRU, an `InMemoryStepCache`, or any object with `get()`/`set()` |

<Card title="Step-Result Cache" icon="database" href="/features/workflow-step-cache">
  Cache key composition, correctness guarantees, and custom cache backends.
</Card>
...
See [Error Handling](/features/workflow-error-handling) for the full guide, including `parallel(on_failure=...)` modes.
