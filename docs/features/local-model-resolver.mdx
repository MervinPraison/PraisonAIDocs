...
<Warning>
On releases before PR [#4900](https://github.com/MervinPraison/PraisonAI/pull/4900), `Agent(llm="local")` handed **every** resolved engine the bare server root. That is correct for Ollama, but LM Studio, vLLM, llama-server and mlx-lm all serve under `/v1` — so chat requests went to `/chat/completions` and those servers answered `404`. Upgrade to a release that includes #4900 to use those engines via `llm="local"`.
</Warning>
...
The `base_url` handed to litellm depends on the engine, because Ollama and the OpenAI-compatible engines expect different roots.

| Engine | `base_url` handed to litellm | Reason |
|---|---|---|
| `ollama` | server root (e.g. `http://127.0.0.1:11434`) | litellm's `ollama` provider builds its own `/api` paths from the root |
| `lm_studio`, `vllm`, `llama_cpp`, `mlx_lm`, `transformers_serve` | server root + `/v1` (e.g. `http://127.0.0.1:1234/v1`) | These engines expose the OpenAI-compatible surface under `/v1` — `POST /chat/completions` on the bare root returns 404 |

The resolver exposes both roots on `LocalTarget` as `base_url` (Ollama-shaped) and `openai_base_url` (OpenAI-shaped). The agent picks the right one per engine.
...
## Local embedder auto-selection

When `Agent(llm="local", ...)` needs embeddings (any of `retrieval_config`, `embedder_config`, or `memory` set) and no embedder is configured, the resolver picks one from the same local server.

**Precedence:**

1. `PRAISONAI_LOCAL_EMBED_MODEL` env var (validated against the server's listing).
2. `PREFERRED_EMBED_MODELS` order: `nomic-embed-text`, `mxbai-embed-large`, `bge-m3`, `snowflake-arctic-embed`, `all-minilm`.
3. Any remaining embedding-capable model.

The server's own capability list wins when available (Ollama exposes `"embedding"`); otherwise names containing `embed` or starting with `bge-` are treated as embedders.
...
Since PR [#4924](https://github.com/MervinPraison/PraisonAI/pull/4924), the measurements the resolver already takes now drive behaviour: the picked embedder carries its **measured `embedding_dims`** to the vector store, and the model's **context window comes from the server** rather than the litellm default. Tool schemas are made server-safe per engine — see [Local Tool Schemas](/features/local-tool-schemas) for the `Optional[...]` collapse and the `tools` + structured-output refusal.

### Advanced: build the block yourself

The same helpers are exported from `praisonaiagents.local`:

```python
from praisonaiagents.local import (
    select_embedding_model, local_embedder_config, discover,
)

discovery = discover()   # finds a running local server
model = select_embedding_model(discovery.models, discovery.model_meta)
embedder_block = local_embedder_config(discovery.engine, discovery.base_url, model)
```

<Note>
A resolved `LocalTarget` exposes both roots: `base_url` (Ollama-shaped, bare root) and `openai_base_url` (OpenAI-shaped, ends in `/v1`). Read `openai_base_url` when wiring an OpenAI-compatible engine (LM Studio, vLLM, llama.cpp, mlx-lm) yourself.
</Note>
...
| `PRAISONAI_LOCAL_EMBED_MODEL` | Pin the embedding model the resolver picks for `knowledge` / `memory`. Accepts a bare name (`all-minilm`) or a tagged id (`all-minilm:latest`). Falls back to `PREFERRED_EMBED_MODELS` order when unset. | *(none)* |
...
<Note>
Engines without a built-in `ProbeSpec` (`llamafile`, `localai`, `ramalama`) are reachable **only** via `PRAISONAI_LOCAL_ENGINE` — since PR [#4870](https://github.com/MervinPraison/PraisonAI/pull/4870) they resolve through the `GET /v1/models` fallback probe.
</Note>
