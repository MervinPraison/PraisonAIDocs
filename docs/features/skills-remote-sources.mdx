...
<Accordion title="skills install vs skills sync">
`install` is a one-shot copy. `sync` keeps a versioned cache and refreshes on demand. Use `sync` for shared team libraries.
</Accordion>
</AccordionGroup>
