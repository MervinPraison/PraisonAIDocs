...
<Note>
Compaction checkpoints can also be written by the [gateway idle-compaction sweep](/features/gateway-idle-compaction), which compacts idle sessions in the background — not only at the end of a live turn. When the next user message arrives on that session, resume replays the same compacted working history — no first-message latency spike.
</Note>
...
<CardGroup cols={3}>
  ...
  <Card title="Hook Events" icon="webhook" href="/features/hook-events#system-events">
    Read the summary from an AFTER_COMPACTION hook
  </Card>
  <Card title="Gateway Idle Compaction" icon="compress" href="/features/gateway-idle-compaction">
    Out-of-band sweep that writes these checkpoints for idle sessions
  </Card>
</CardGroup>
