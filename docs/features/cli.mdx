...
<Tabs>
  <Tab title="Create Config">
    ...
    <Note>
    If no LLM provider is configured, `--init` prints provider-setup guidance (supporting OpenAI, Anthropic, Google/Gemini, Groq, Cohere, Ollama, OpenRouter and 100+ models via LiteLLM) and exits. Run [`praisonai setup`](/cli/setup) first to configure a provider.
    </Note>
  </Tab>
...
See the [Validate CLI reference](/cli/validate) for all flags and output formats.
...
`praisonai workflow validate <file.yaml>` checks a workflow against the schema and cross-references every agent reference. It now guarantees:
