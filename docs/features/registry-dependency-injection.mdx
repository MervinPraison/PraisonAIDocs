...
The registry pattern supports multiple configuration levels across PraisonAI registries:

| Registry | Module | Entry-point group | Docs |
|----------|--------|-------------------|------|
...
| External CLI integrations | `praisonai.integrations.registry` | — | [Integration Registry](/docs/features/integration-registry) |
| Bot platforms | `praisonai.bots._registry` | `praisonai.bots` | [Bot Platform Plugins](/docs/features/bot-platform-plugins) |
| Sandbox backends (7) | `praisonai.sandbox._registry` | `praisonai.sandbox` | [Sandbox Backends](/docs/features/sandbox-backends) |
| Cloud deploy providers | `praisonai.deploy.providers._registry` | `praisonai.deploy.providers` | [Cloud Provider Registry](/docs/features/cloud-provider-registry) |
| Endpoint providers (top-level) | `praisonai.endpoints.registry` | `praisonai.endpoint_providers` | [Endpoint Provider Registry](/docs/features/endpoint-provider-registry) |
| Endpoint CLI dispatch | `praisonai.cli.features._endpoint_registry` | `praisonai.endpoints.providers` | [Endpoint Provider Registry](/docs/features/endpoint-provider-registry) |
| Search providers | `praisonai.cli.features._search_registry` | `praisonai.tools.search` | [Search Providers](/docs/features/search-provider-registry) |
| Generic base (advanced) | `praisonai._registry` | configurable | `PluginRegistry[T]` for plugin authors |

All plugin registries expose `Registry.default()` (or module-level `get_default_registry()`) unless noted. See each feature page for built-in keys and entry-point snippets.
