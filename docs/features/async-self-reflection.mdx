...
<Note>
**Parse-error parity (PR #4253)**: The async path always bounded parse-error retries at `max_reflect`. The sync path did not — a reflection response that persistently failed to parse (a structured-output refusal, a content-filter block, or a truncation) looped forever. PR #4253 closes that gap: sync now terminates at `max_reflect` on parse errors, like async, and returns the un-reflected answer.

This is termination parity, not call-count parity — the sync loop still issues one extra primary completion per reflection round, so sync and async do not cost the same under a persistent-refusal scenario.
</Note>
...
<Card title="Self-Reflection Concepts" icon="rotate" href="/features/selfreflection">
