...
<Note>
Both `NO_REPLY` (this page) and `BotLoopGuard` live in `praisonaiagents/bots/silence.py` alongside `classify_final()`, the classifier every adapter uses to distinguish silence from an empty reply. Where `NO_REPLY` lets an *agent* opt out of a single reply, [`BotLoopGuard`](/features/bot-loop-protection) lets the *gateway* opt out of an entire runaway bot-to-bot exchange.
</Note>
...
Intentional silence is a recorded non-outcome (`SUPPRESSED`), whereas a real delivery *failure* now flips the run to `on_failure` and bumps `undelivered_deliveries` — see [Scheduler Delivery → Delivery outcomes](/features/scheduler-delivery#delivery-outcomes).

An intentional-silence run **also skips the continuable-session seed** — nothing was delivered, so nothing is claimed to have been. The next inbound message lands as a fresh turn, same as before. See [Scheduler Delivery → Continuable Delivery](/docs/features/scheduler-delivery#continuable-delivery).
...
If you want the bot to *look at* every message but only *speak* when addressed, prefer [`group_policy: observe`](/features/bot-observe-mode) (no agent runs on unmentioned messages, transcript still captures them) over `respond_all + NO_REPLY` (agent runs on every message, then chooses silence). Observe is cheaper (no LLM calls) and safer (no risk of accidental replies).
...
<Card title="Bot Loop Protection" icon="rotate" href="/features/bot-loop-protection">
