...
<Note>
Both `NO_REPLY` (this page) and `BotLoopGuard` live in `praisonaiagents/bots/silence.py`. Where `NO_REPLY` lets an *agent* opt out of a single reply, [`BotLoopGuard`](/features/bot-loop-protection) lets the *gateway* opt out of an entire runaway bot-to-bot exchange.
</Note>
