...
graph LR
    subgraph "Model Capabilities"
        Task[📋 Task] --> Detect[🔍 Detect Capabilities]
        Detect --> Score[⚙️ Score Models]
        Score --> Output[✅ Optimal Model]
    end
...
<Note>
The `is_reasoning_model()` helper detects OpenAI `o1`/`o3`/`o4`/`gpt-5.x` models so PraisonAI can normalise their parameters. See [Reasoning Models](/features/reasoning-models) for the full behaviour.
</Note>
...
sequenceDiagram
    ...
    User->>Detector: recommend_model(requirements)
    Detector->>Models: Compare capabilities
    Models-->>Detector: Capability scores
    Detector->>Detector: Rank by match score
    Detector-->>User: Best-fit model
```
...
<CardGroup cols={2}>
  <Card icon="route" href="/features/model-router">
    Route each request to the best model based on its capabilities.
  </Card>
  <Card icon="shuffle" href="/features/model-fallback">
