...
Same shape for `create_knowledge_store(..., registry=)` and `create_state_store(..., registry=)`. See [Registry Dependency Injection](/features/registry-dependency-injection) for the wider pattern.
