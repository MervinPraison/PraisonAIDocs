...
registry.register_store("mybackend", my_store_factory, aliases=("mb",))
...
`register_store()` is the canonical registration method. `register()` is preserved as a backward-compatible alias.
</Note>
