  ...
  <Tab title="Code">
    <Steps>
      ...
      <Step title="Create Your Agents">
        ...
        agents = AgentManager(agents=[summarise_agent])
        agents.start()
        ```
...
<CardGroup cols={2}>
  ...
  <Card title="Agents Manager" icon="users">
    ...
    ```python
    AgentManager(agents=[agent1, agent2])
    ```
  </Card>
...
agents = AgentManager(agents=[researcher, analyst])
```
...
agents = AgentManager(agents=[collector, summarizer, writer])
```
