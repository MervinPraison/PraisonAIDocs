...
<Note>
Credential rotation is now aligned end-to-end: at boot (channel preflight isolates a bad token instead of aborting, since [PR #4863](https://github.com/MervinPraison/PraisonAI/pull/4863)), during operation (runtime rejection parks the channel), and on hot-reload (the fresh token drops the channel out of the degraded set). See [Degraded Channel Isolation → Strict opt-in](/features/gateway-degraded-channels#strict-opt-in).
</Note>
...
- When rotation adopts a new secret, it also updates `GATEWAY_AUTH_TOKEN` in the environment so **all** auth paths (HTTP, magic-link, WS) read the same value — matching [Bind-Aware Auth](/features/gateway-bind-aware-auth) (config wins over env).
...
<CardGroup cols={2}>
<Card title="Gateway Config Reload" icon="arrows-rotate" href="/features/gateway-config-reload">
