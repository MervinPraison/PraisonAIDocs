...
<Note>
Unparseable judge verdicts fail closed — an unreadable LLM response yields `score=1.0, passed=False`, so a low `judge_threshold` doesn't produce a false pass. See [LLM as Judge — Fail-Closed Parsing](/features/llm-as-judge#fail-closed-parsing).
</Note>
...
<Card title="Eval Module Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/eval">
...
<CardGroup cols={2}>
  ...
  <Card title="EvalPort Adapter" icon="arrows-left-right" href="/features/evalport-adapter">
    Export the EvalReport as an EvalPort ResultSet for cross-tool CI diffs
  </Card>
</CardGroup>
