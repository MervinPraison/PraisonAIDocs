...
<Note>
**From eval runs:** [`export_sft`](/features/eval-datasets#export_sft) turns the passing runs of an `EvalReport` into a chat-format JSONL that this trainer consumes unchanged.
</Note>
...
The log streams while training; Ctrl-C stops the remote run. See [Remote in one step](/features/praisonai-train-inline-remote).
...
<Note>
Beyond SFT, `praisonai-train llm` also supports preference tuning — **DPO**, **ORPO**, and **KTO** — via the `method` config key. See [Preference Training](/features/train-preference-tuning).
</Note>
...
<Card title="Preference Training" icon="scale-balanced" href="/features/train-preference-tuning">
