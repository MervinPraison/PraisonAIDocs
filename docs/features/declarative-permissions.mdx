...
<Info>
Since PR #2208, `bash:` / `shell:` rules are matched against every sub-operation in a compound command. See [Command-Aware Permissions](/features/command-aware-permissions).
</Info>
...
<Note>
`read:*` no longer authorises `.env` / private-key reads — the built-in secret gate always defaults those to `ask` unless a *secret-specific* rule (e.g. `read:*.env=allow`) opts in. See [Secret Read Gate](/features/secret-read-gate).
</Note>
...
<Note>
Argument-scoped patterns like `bash:rm *` and `write:/etc/*` are enforced at **both** the deny gate and the ask/allow decision. Before PR [#4234](https://github.com/MervinPraison/PraisonAI/pull/4234) only the ask/allow decision honoured them — the name-based deny gate ignored arguments. See [Command-Aware Permissions → Argument-scoped deny reaches the gate](/features/command-aware-permissions#argument-scoped-deny-reaches-the-gate).
</Note>

| Pattern | Meaning |
|---|---|
| `read:*` | Any read tool call (**does not authorise secret files** — see below) |
| `read:*.env` / `read:*.pem` / `read:id_rsa` | Secret-specific reads — required to opt in to reading credential files |
| `bash:git *` | Git shell commands |
| `write:/etc/*` | Writes under `/etc/` (including truncating redirections) |
| `tool:delete_*` | MCP-namespaced tool names — matched against the bare or `tool:<name>` form |
...
<Warning>
**Reading secret files (`.env`, private keys, `*.pem`, `id_rsa`, etc.) defaults to `ask` — even with a broad `"read:*": "allow"` rule.** A catch-all read allow no longer silently authorises credential files; you need a secret-specific rule (e.g. `"read:*.env": "allow"`) to opt in. See [Permissions → Built-in Secret-File Read Gate](/docs/features/permissions#built-in-secret-file-read-gate).
</Warning>

### external_dir: targets

`external_dir:<parent>/*` is emitted for any path that resolves outside `workspace_root`. Use these patterns to pre-authorise or hard-block external filesystem access without interactive prompts. See [Workspace Boundary](/features/workspace-boundary).
...
<Note>
A config object with **no** `permissions=` (e.g. `ApprovalConfig(timeout=30)` or `{"timeout": 30}`) inherits the env-driven default deny set — so you get a safe baseline "for free" when you only tune `timeout=`, `backend=`, etc. Passing `permissions={}` (an explicit empty policy) opts out. See [Approval → Config objects never weaken the default deny set](/features/approval#config-objects-never-weaken-the-default-deny-set).
</Note>

**Precedence:** agent definition `mode:` / `permission:` → agent-level `approval.permissions` → top-level `agents.yaml` `permissions:` → `.praisonai/config.yaml` `permissions:` → CLI `--allow`/`--deny` (override file) → `--permission-default` → built-in defaults. CLI flags override config-yaml rules per-pattern. See [Agent Presets & Modes](/features/agent-presets-and-modes) for the full per-agent permission reference.
...
**Opt in to reading `.env` in a trusted CI** — add a secret-specific allow rule, not a broad `read:*`:

```yaml
permissions:
  "read:*": allow            # ordinary files auto-read
  "read:*.env": allow        # explicit opt-in for .env — the broad rule alone won't
```
...
<Card title="Tool Approval CLI" icon="terminal" href="/cli/tool-approval">
  CLI flags reference
</Card>
</CardGroup>
