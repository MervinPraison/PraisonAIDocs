...
<Note>
`read:*` no longer authorises `.env` / private-key reads — the built-in secret gate always defaults those to `ask` unless a *secret-specific* rule (e.g. `read:*.env=allow`) opts in. See [Secret Read Gate](/docs/features/secret-read-gate).
</Note>
