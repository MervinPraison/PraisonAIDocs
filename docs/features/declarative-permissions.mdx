...
**Precedence:** agent definition `mode:` / `permission:` → top-level YAML `permissions:` → agent-level `approval.permissions` → CLI `--allow`/`--deny` (override file) → `--permission-default` → built-in defaults.

CLI flags always win. See [Agent Presets & Modes](/docs/features/agent-presets-and-modes) for the full per-agent permission reference.
