...
<Info>
Interactive persistent approvals can now record a reusable command-prefix glob instead of the literal command. See [Reusable Approval Scopes](/docs/features/reusable-approval-scopes) — one approval for `bash:git status *` covers every `git status` variant without re-prompting.
</Info>
