...
When you don't specify a model, the CLI picks the best one automatically based on which API keys you have. A one-line notice tells you what was chosen and why. The same resolver drives the SDK `Agent(llm=None)` path — construct an `Agent` with no `llm=` and it picks from your credentials too.

<Note>
[PR #4795](https://github.com/MervinPraison/PraisonAI/pull/4795) fixed the SDK path so it matches the CLI. Setting only `OLLAMA_HOST`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `GOOGLE_API_KEY`, `GROQ_API_KEY`, or `COHERE_API_KEY` now yields a working `Agent()` without any further config.
</Note>
...
# No llm= argument — the Agent picks ollama/llama3.2 and routes through litellm.
...
<CardGroup cols={2}>
<Card title="CLI OAuth Login" icon="key" href="/features/cli-oauth-login">
  Sign in to providers using browser-based OAuth
</Card>
<Card title="Models CLI" icon="microchip" href="/features/models-cli">
