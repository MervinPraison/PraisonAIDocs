...
<CardGroup cols={2}>
  ...
  <Card title="/undo" icon="rotate-left">
    Undo the last turn's file changes. Requires the agent to track changes (autonomy + `track_changes`).
  </Card>
  ...
  <Card title="/resume" icon="play">
    Resume a saved session by id: `/resume <id>`. Use `/sessions` first to find the id.
  </Card>
  <Card title="/retry" icon="arrow-rotate-right">
    Re-dispatch your last message through `session.chat()` — actually re-runs it, not just an echo.
  </Card>
  <Card title="/reasoning" icon="brain">
    Toggle whether extended-thinking ("reasoning") output is shown for this conversation. Per-user.
  </Card>
</CardGroup>
...
Rewinding the last turn restores files to the state before the most recent autonomous iteration.

```mermaid
flowchart LR
    User[User] -->|/undo| Bot[Bot]
    Bot -->|agent.undo\(\)| Agent[AI Agent]
    Agent -->|reverted| Success[✅ Reverted the last turn.]
    Agent -->|nothing| Info[ℹ️ Nothing to undo.]

    classDef user fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef bot fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef ok fill:#10B981,stroke:#7C90A0,color:#fff
    classDef info fill:#F59E0B,stroke:#7C90A0,color:#fff
...
| Situation | Reply |
|-----------|-------|
| Changes reverted | `✅ Reverted the last turn.` |
| Nothing to undo | `ℹ️ Nothing to undo.` |
| No agent configured | `❌ Undo is not available (no agent configured).` |
| Agent without undo support | `❌ This agent does not support undo (change tracking not enabled).` |

`/undo` surfaces `Agent.undo()`, available only when the agent runs with autonomy and `track_changes` enabled. Without those settings, the bot degrades gracefully with an informational reply instead of an error. See [Autonomy](/docs/features/agent-autonomy) for how to enable change tracking.

### /sessions

Listing sessions shows all checkpoints the session manager has stored for the requesting user.

```mermaid
flowchart LR
    User[User] -->|/sessions| Bot[Bot]
    Bot -->|scoped lookup| Manager[Session Manager]
    Manager -->|keys found| List["📁 Recent sessions:\n• session-abc\nUse /resume &lt;id&gt; to switch."]
    Manager -->|no keys| Empty[No saved sessions yet.]
...
| Situation | Reply |
|-----------|-------|
| No sessions saved | `No saved sessions yet.` |
| Sessions found | Header `📁 Recent sessions:`, one `• <id>` line per session (capped at 50), footer `Use /resume <id> to switch to one.` |

<Note>
The lookup is scoped to the requesting user's storage key — `/sessions` never enumerates another user's session keys.
</Note>

### /resume

Switching to a saved session replaces the active conversation context with the stored one.

```mermaid
flowchart LR
    User[User] -->|"/resume session-abc"| Bot[Bot]
    Bot -->|resume_session\(\)| Manager[Session Manager]
    Manager -->|found| Success["✅ Resumed session session-abc."]
    Manager -->|not found| NotFound["❌ Session session-abc not found."]
    Manager -->|no primitive| Unsupported[ℹ️ Switching not supported here.]
...
| Situation | Reply |
|-----------|-------|
| No id provided | `ℹ️ Usage: /resume <id>  (see /sessions for the list)` |
| Session resumed | `✅ Resumed session <id>.` |
| Session not found (with `resume_session`) | `❌ Session <id> not found.` |
| Session not found (via `_histories`) | `❌ Session <id> not found. Use /sessions to list them.` |
| Session exists but switching unsupported | `ℹ️ This bot can see saved sessions, but switching to a different one isn't supported here (no resume primitive).` |
| No session support at all | `❌ Resuming sessions isn't supported by this bot.` |

<CardGroup cols={1}>
  <Card title="Session Resume (memory-driven)" icon="database" href="/docs/persistence/session-resume">
    The persistence layer's session-resume is a different code path based on memory backends — distinct from the bot `/sessions` and `/resume` commands documented here.
  </Card>
</CardGroup>

### /retry

Re-dispatching your last message re-runs it through the normal `session.chat()` path — not an echo.
...
sequenceDiagram
    ...
    User->>Bot: /retry
    Bot->>SessionManager: get_last_user_message(user_id)
    alt No previous message
        SessionManager-->>Bot: None
        Bot-->>User: ℹ️ Nothing to retry — no previous message found.
    else Previous message found
        SessionManager-->>Bot: last message text
        Bot-->>User: 🔁 Retrying your last message…
        Bot->>Agent: session.chat(last message text)
        Agent-->>Bot: response
        Bot-->>User: response
    end
```
...
| Situation | Reply |
|-----------|-------|
| No previous message | `ℹ️ Nothing to retry — no previous message found.` |
| Retry dispatched | `🔁 Retrying your last message…` (followed by the agent's response) |
| Re-dispatch failed | `❌ Retry failed: <error>` |

<Note>
Adapters call `get_last_user_message()` then re-dispatch through `session.chat()`. The `handle_retry_command()` helper in `_commands.py` returns a formatted acknowledgement and is available for custom adapters that need it.
</Note>

### /reasoning

Toggling reasoning changes whether extended-thinking output is included in bot responses for your conversation.

```mermaid
flowchart LR
    User[User] -->|/reasoning| Bot[Bot]
    Bot -->|read current state| Store[Per-user store]
    Store -->|was hidden| ShowMsg["🧠 Reasoning output will be shown for this conversation."]
    Store -->|was visible| HideMsg["🙈 Reasoning output will be hidden for this conversation."]
...
| Toggle result | Reply |
|---------------|-------|
| Now visible | `🧠 Reasoning output will be shown for this conversation.` |
| Now hidden | `🙈 Reasoning output will be hidden for this conversation.` |

- Default state is **hidden** (`False`).
- Preference is per-user: stored on `session_manager._reasoning_visibility` when available, falling back to a module-level map.
- Response-rendering code calls `is_reasoning_visible(session_manager, user_id)` to decide whether to include extended-thinking blocks. Plugin and adapter authors can use this hook directly.
...
import asyncio
asyncio.run(bot.start())
# /status, /new, /stop, /model, /usage, /compress, /queue, /help,
# /undo, /sessions, /resume, /retry, /reasoning available automatically
```
