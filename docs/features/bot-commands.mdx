...
graph TB
    ...
    Q --> F[Learn a skill from sources]
    A --> A1[/stop]
    B --> B1[/compress]
    C --> C1[/model]
    D --> D1[/usage]
...
<CardGroup cols={2}>
  ...
  <Card title="/learn" icon="graduation-cap">
    Author a grounded SKILL.md from sources you describe (repo, docs, PDFs, or this chat). Admin-only by default when a policy is configured.
  </Card>
</CardGroup>

## Per-Command Access Control

Restrict which commands each user may run with `admin_users` and `user_allowed_commands` — admins get full access, regular users only run commands you list. All built-in commands, including the new runtime controls, are gated by this policy.
