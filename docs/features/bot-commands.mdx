...
description: "Built-in chat commands — status, model switching, usage, context compression, queuing — plus Python-registered, file-based, and entry-point custom commands for Telegram, Discord, Slack, and WhatsApp bots."
icon: "terminal"
---
...
<Note>
On Telegram and Discord, every command below appears in the native `/` autocomplete menu automatically. See [Slash Command Menu](/features/bot-slash-command-menu).
</Note>
...
```mermaid
graph TB
    Q{What do you need?} --> A[Cancel current answer only]
    Q --> A2[Cancel current answer AND my queued messages]
    ...
    class Q agent
    class A,A2,B,C,D,E,F,G,H,I,J,K,L,N,A1,A3,B1,C1,D1,E1,F1,G1,H1,I1,J1,K1,L1,N1 tool
    class M,M1 source
```
...
<CardGroup cols={2}>
  ...
  <Card title="/stop all" icon="octagon-x">
    Cancels the current task AND drains any messages the user queued behind it. Session-scoped stop.
  </Card>
  <Card title="/cancel" icon="octagon-x">
    Alias for `/stop all`. Cancels the current task AND drains queued follow-ups.
  </Card>
...
<Note>
Scheduled deliveries pick up their home channel from `~/.praisonai/state/home_channels.json`, the `{PLATFORM}_HOME_CHANNEL` env var, or an explicit `deliver:` target in YAML — no chat command is involved. See [Proactive Delivery](/features/proactive-delivery) and [Scheduler Delivery](/features/scheduler-delivery).
</Note>
...
Command classification is now gated by two layers: **content-trust** (new — `BotMessage.allow_control`, so an untrusted producer's `/`-prefixed body is never treated as a command) and **identity ACL** (existing — `admin_users` / `user_allowed_commands`). Content-trust fires first. See [Control-Trust Provenance](/features/bot-control-trust).

<Card title="Command Access Control" icon="shield-keyhole" href="/features/bot-command-access-control">
...
<Note>
Voice notes are transcribed automatically — see [Voice Notes](/features/gateway-voice-notes).
</Note>
...
Cancels the current agent task immediately. Pick the scope that matches what you want stopped.

```mermaid
graph TB
    Q{What do you want to stop?} --> A[Just the running answer]
    Q --> B[The running answer AND my queued follow-ups]
    A --> A1[/stop  ·  stop]
    B --> B1[/stop all  ·  /cancel]
...
| Command | What it stops | Reply payload |
|---------|--------------|---------------|
| `/stop` (or `stop`) | Current turn only. Queue preserved. | `scope: "turn"`, `pending_cancelled: 0` |
| `/stop all` | Current turn + drains queued messages. | `scope: "session"`, `pending_cancelled: N` |
| `/cancel` | Alias for `/stop all`. | Same as `/stop all`. |
...
`/stop all` and `/cancel` additionally:

- Drain any pending messages the user queued behind the running turn
- Report the drained count as `pending_cancelled: N` so nothing is silently dropped

<Note>
`/stop` works out of the box on any bot using the default `BotSessionManager` — Telegram, Discord, Slack, and WhatsApp all register it. Enabling [Bot Run Control](/features/bot-run-control) with `busy_mode` adds mid-run interruption plus pending-message handling on top of basic cancellation.
</Note>

<Note>
For the full wire protocol — the `abort` frame, the `StopScope` values, and the `StopResult` reply fields — see [Gateway Abort and Per-Turn Timeout → Stop Scopes](/features/gateway-abort-and-timeout#stop-scopes).
</Note>

<Tip>
When your bot routes through the gateway (Telegram, Discord, Slack, WhatsApp with `praisonai_bot`), `/stop` is handled server-side by the gateway's abort path introduced in [PraisonAI#3472](https://github.com/MervinPraison/PraisonAI/pull/3472) — it cancels the in-flight turn cooperatively without requiring `busy_mode`. The `busy_mode` path is still what enables pending-message handling and mid-run steering; `/stop` alone works with any gateway-routed bot. See [Gateway Abort and Per-Turn Timeout](/features/gateway-abort-and-timeout). `/stop all` and `/cancel` (session-scoped, from [#5130](https://github.com/MervinPraison/PraisonAI/pull/5130)) additionally drain the session's queued messages — no `busy_mode` required.
</Tip>
...
<Note>
The override lives on the session and is cleared by [`/new`](#new) (per-user) and by a whole-gateway `reset_all()` (all users). If a model you set with `/model` turns out to be unavailable and every following turn fails at the provider, `/new` is the honest remedy — it wipes the override alongside the history. The same clearing happens on an idle-triggered or scheduled [session reset](/features/bot-session-reset).
</Note>
...
<Note>
`/queue` requires [Bot Run Control](/features/bot-run-control) to be enabled with `busy_mode`. Without it, the bot replies: `"ℹ️ Message queuing isn't enabled for this bot. Just send your message and it will be processed."`
</Note>
...
- [Background Tasks](/features/background-tasks) — how tasks get submitted in the first place
- [Command Access Control](/features/bot-command-access-control) — `/tasks` respects the same policy, so admins can restrict it
...
<Card title="Learn Skill" icon="graduation-cap" href="/features/agent-learn-skill">
...
<Note>
`/automations` and `/blueprint` are generic names, so if your app registers a custom `@bot.on_command` handler of the same name, that handler wins and the built-in steps aside — unlike every other built-in, which always takes precedence. See [Automation Suggestions](/features/automation-suggestions).
</Note>
...
The `description` you pass here IS the label users see in the native `/` menu on Telegram and Discord. See [Slash Command Menu](/features/bot-slash-command-menu).
...
The same `.praisonai/commands/*.md` files already documented for [Custom Agents & Commands](/features/custom-agents-commands) now reach bot chats too — the resolver reuses that loader through the sanctioned `_code_bridge` seam instead of reimplementing it.
...
<Warning>
A shell-carrying command whose deployment forbids shell renders that segment **inert** rather than erroring. This safe-by-default degradation keeps the rest of the command working. See [Bot Shell Execution](/features/bot-shell-execution) for the wider shell-approval model.
</Warning>
...
<Note>
File and entry-point commands pass through the same [Command Access Control](/features/bot-command-access-control) policy as built-in and Python-registered commands — `admin_users` and `user_allowed_commands` gate them identically, and restricted commands stay hidden from the `/` menu for users who cannot run them.
</Note>
...
For full allowlist setup, see the [Bot Security](/best-practices/bot-security) guide. This security hardening was implemented in [PR #1835](https://github.com/MervinPraison/PraisonAI/pull/1835).
...
# /stop all and /cancel (session-scoped stop) are handled by the gateway when the bot is gateway-routed
```
...
<Warning>
**Policy-filtered.** When a `CommandAccessPolicy` is set, `build_command_menu_entries(user_id)` returns only the commands that user may run, so restricted commands stay hidden from users who cannot run them. See [Command Access Control](/features/bot-command-access-control).
</Warning>
...
<AccordionGroup>
  <Accordion title="Restrict privileged commands">
    Configure `admin_users` and `user_allowed_commands` before enabling `/learn`, `/stop`, or `/queue` in production. See [Command Access Control](/features/bot-command-access-control).
  </Accordion>
  <Accordion title="Enable run control for /stop and /queue">
    `/stop` and `/queue` need [Bot Run Control](/features/bot-run-control) with `busy_mode` for mid-run cancellation and follow-up queuing — without it, users get informational fallbacks only.
  </Accordion>
  <Accordion title="Choose the right stop scope">
    Use `/stop` (turn-scoped) when the user wants to interrupt only the current answer and let their queued follow-ups run. Use `/stop all` or `/cancel` (session-scoped) when the user regrets the whole line of questions — a session stop drains every message they queued behind the running turn, and the reply's `pending_cancelled` count tells them exactly how many were dropped.
  </Accordion>
...
<CardGroup cols={2}>
  <Card icon="robot" href="/features/messaging-bots">
    Connect your agent to Telegram, Discord, Slack, and WhatsApp.
  </Card>
  <Card icon="puzzle-piece" href="/features/bot-platform-capabilities">
  ...
  <Card icon="microphone" href="/features/gateway-voice-notes">
    Transcribe inbound voice messages on Telegram, Slack, and WhatsApp.
  </Card>
  <Card title="Custom Agents & Commands" icon="folder-tree" href="/features/custom-agents-commands">
  ...
  <Card icon="clock" href="/features/background-tasks">
    How tasks get submitted, configured, and made durable — the substrate `/tasks` inspects.
  </Card>
  <Card title="Skill Automation" icon="clock" href="/features/skill-automation">
