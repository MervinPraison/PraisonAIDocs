...
Call `prune()` on the `LearnManager` for one-off cleanup:
...
<Note>
Pruning also runs automatically on every write when `max_entries` is set. Manual calls are only needed for one-off cleanup of a long-running agent.
</Note>
