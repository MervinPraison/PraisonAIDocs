...
sequenceDiagram
    participant User
    participant Agent
    participant BoundedLearning

    User->>Agent: Request
    Agent->>BoundedLearning: Process
    BoundedLearning-->>Agent: Result
    Agent-->>User: Response
```
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Bounded Learning
...
Full reference on `max_entries` and `retention_days` is in the [LearnConfig reference](/configuration/learn-config).
...
```
~/.praisonai/learn/private/default/persona.archive.json
```
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="LearnConfig reference" icon="graduation-cap" href="/configuration/learn-config">
  Full configuration options for continuous learning
</Card>
<Card title="Checkpoints" icon="flag" href="/features/checkpoints">
...
<Card title="Bot Session Learning" icon="user-graduate" href="/features/bot-session-learning">
  Retention policies apply the same to bot-injected learn stores
</Card>
</CardGroup>
