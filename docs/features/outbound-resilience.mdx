...
Separate from this boot-time DLQ drainer, `ChannelSupervisor` also re-drains the durable outbox when a channel recovers from a transient outage — see [Channel supervision → Recovery-triggered outbox re-drain](/features/gateway-channel-supervision#recovery-triggered-outbox-re-drain).
...
<Note>
**AgentMail additionally deduplicates at the provider.** `AgentMailBot` forwards a stable `Idempotency-Key` header on every attempt of a logical send. When retry crosses the AgentMail boundary after a timeout-after-send, the provider dedupes — no duplicate email is delivered. See [Duplicate protection on retries](/features/email-bot#duplicate-protection-on-retries).
</Note>
