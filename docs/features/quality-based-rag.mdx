...
graph LR
    subgraph "Quality-Based RAG"
        Query[🔍 User Query] --> Retrieve[📡 Retrieve Chunks]
        Retrieve --> Quality[📊 Quality Scoring]
        Quality --> Completeness[✅ Completeness]
        Quality --> Relevance[🎯 Relevance]
        Quality --> Clarity[💡 Clarity]
        Quality --> Accuracy[🔬 Accuracy]
        Completeness --> Rank[🏆 Rerank Results]
        Relevance --> Rank
        Clarity --> Rank
        Accuracy --> Rank
        Rank --> Answer[📤 High-Quality Answer]
    end

    classDef query fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef retrieve fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef metric fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef rank fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef result fill:#10B981,stroke:#7C90A0,color:#fff

    class Query query
    class Retrieve,Quality retrieve
    class Completeness,Relevance,Clarity,Accuracy metric
    class Rank rank
    class Answer result
```
...
<AccordionGroup>
  <Accordion title="Set Quality Thresholds Based on Use Case">
    Use `quality_threshold=0.7` for general tasks. Raise it to `0.85+` for high-stakes domains (medical, legal) where accuracy is critical. Lower to `0.5` only for exploratory retrieval.
  </Accordion>
  <Accordion title="Balance Metric Weights">
    Adjust `metrics_weights` to match your content type. For factual Q&A, weight `accuracy` higher. For creative content, weight `clarity` and `completeness` higher.
  </Accordion>
  <Accordion title="Use Reranking Selectively">
    Enable `rerank=True` only when retrieval quality matters more than speed. Reranking adds an LLM call per search — disable it for high-throughput pipelines.
  </Accordion>
  <Accordion title="Implement Confidence-Based Fallback">
    Use a multi-stage fallback: try high-quality retrieval first, then progressively lower the threshold if no results are found, rather than returning empty results.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Memory Advanced Search" icon="magnifying-glass-plus" href="/docs/features/memory-advanced-search">
    Reranking and relevance filtering parameters
  </Card>
