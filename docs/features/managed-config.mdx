...
<Note>
**Not the same as [Managed CLI](/features/managed-cli).** That page covers CLI runtime management. This page is about **managed configuration** — org-distributed defaults and policy for the CLI config resolver.
</Note>

<Note>
**Not the same as `PRAISONAI_CONFIG` / `PRAISONAI_CONFIG_CONTENT`.** Those two env vars supply the *user-config* layer (what a user would put in `~/.praisonai/config.yaml`); `PRAISONAI_MANAGED_CONFIG_URL` / `PRAISONAI_MANAGED_CONFIG_DIR` on this page supply the *managed-policy* layer (what an admin distributes org-wide). Managed policy layers **below** user config, so a user's config can still override managed defaults except for the enforced keys (`permissions`, `model_allowlist`). See [Security Environment Variables → PRAISONAI_CONFIG_CONTENT](/features/security-environment-variables#praisonai_config_content) for the user-config equivalents.
</Note>
...
<Card title="Single Source Config" icon="file-code" href="/features/single-source-config">
...
<Card title="Hierarchical Config" icon="layer-group" href="/features/hierarchical-config">
How global, project, and env layers merge.
</Card>
<Card title="Declarative Permissions" icon="list-check" href="/features/declarative-permissions">
