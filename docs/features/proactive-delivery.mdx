...
<Note>
`origin` and `all` need the gateway's request/session context — the lightweight scheduler path warns and skips them. See [Scheduler Delivery](/docs/features/scheduler-delivery) for the full token grammar and precedence rules.
</Note>
