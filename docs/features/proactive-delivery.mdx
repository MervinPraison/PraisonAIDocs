...
<Note>
When `praisonai schedule tick` runs out of process (OS cron, CI, serverless) with no gateway, a bare `deliver: telegram` target resolves its chat id from the persisted `HomeChannelRegistry` file (`~/.praisonai/state/home_channels.json`). A channel registered with `/sethome` inside a bot chat is read straight from that file — no gateway, no reconfiguration. See [Deliver from cron / CI](/features/scheduler-delivery#deliver-from-cron-ci-no-gateway-required).
</Note>
