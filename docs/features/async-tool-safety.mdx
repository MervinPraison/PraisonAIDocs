...
<Note>
**`async def` tool bodies on the sync path.** The sync tool loop (`agent.chat()` / `agent.start()`) awaits `async def` tool bodies automatically via `praisonaiagents.agent.async_safety.run_async_in_sync_context` — a bare un-awaited coroutine is never handed back to the model as a tool result (silent-data-loss guard). No running loop → `asyncio.run(coro)`; running loop → dedicated background thread with its own event loop, joined with a 300 s cap. Relocated to its stable home in [PR #5005](https://github.com/MervinPraison/PraisonAI/pull/5005); see [Tools → Async def tools work on both sync and async loops](/features/tools).
</Note>
...
<Note>
A `/stop` during an async tool parked on approval now unwinds the approval at the **core registry** boundary too, not only inside the tool loop. The stale approval resolution is dropped fail-closed and no `session`/`always` grant is persisted for the abandoned turn (PraisonAI [#4950](https://github.com/MervinPraison/PraisonAI/pull/4950)). See [Approval Protocol → Live-Authority Binding](/features/approval-protocol#live-authority-binding).
</Note>
...
- Async tool **timeouts** (`asyncio.wait_for` firing after `tool_timeout` seconds) are recorded as failures and count toward the BLOCK/HALT thresholds. Previously they early-returned and bypassed the streak. Middleware-wrapped async tools (see [Middleware](/features/middleware)) also honour `ToolConfig.timeout` as of [PR #5080](https://github.com/MervinPraison/PraisonAI/pull/5080) — before that fix, the middleware path blocked on `future.result()` with no bound and timeouts on the middleware path were silently ignored (they were not recorded as failures and did not count toward BLOCK/HALT thresholds).
...
On a fresh interactive session, the runtime now routes dangerous tools through the CLI approval backend automatically — no `approval=` kwarg needed. Off-TTY (pipes, CI) keeps deny-by-default. See [Tool Approval → Default Behaviour](/docs/cli/tool-approval#default-behaviour) for the precedence ladder and bypass flags, and [Approval Backends → Which class runs the prompt?](/features/approval-backends) for how the Python (`ConsoleBackend`) and CLI (`InteractiveCLIApprovalBackend`) paths differ.
