...
<Steps>
<Step title="Install and set up API key">
```bash
pip install praisonai praisonaiagents
export OPENAI_API_KEY=your_api_key_here
```
</Step>
<Step title="Create an agent with audio tools">
```python
...
<Step title="Or use the bot CLI">
```bash
# Enable TTS and STT on a Telegram bot
...
<AccordionGroup>
<Accordion title="Use agent-centric setup for chat workflows">
Prefer `Agent(tools=[create_tts_tool()])` over calling `tts_tool()` directly — the agent decides when to speak and handles retries automatically.
</Accordion>
<Accordion title="Set output_dir for production">
In production, pass a persistent `output_dir` to `create_tts_tool()` so audio files aren't written to `/tmp` and lost on restart.
</Accordion>
<Accordion title="Clean up audio files">
TTS files are not deleted automatically. Add cleanup after the session or use a temp-dir strategy to avoid disk growth.
</Accordion>
<Accordion title="Match language codes for STT accuracy">
For multilingual applications, pass the `language` parameter explicitly (e.g. `language="es"`) instead of relying on auto-detection.
</Accordion>
</AccordionGroup>
