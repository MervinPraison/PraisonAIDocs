...
The same policy gates [`ask_conversation`](/features/ask-conversation-tool): a denied target returns `{"status": "undelivered", ...}` **before** the requester is invoked — the same guard, applied to the request/reply path.

The same policy also gates [`send_message(action="react")` and `action="unreact"`](/features/send-message-tool#reactions): a denied target returns `{"status": "failed", "detail": "..."}` **before** the reaction is dispatched — a steered or prompt-injected agent cannot add a reaction on a channel the operator never intended.
...
| `action="react"` / `action="unreact"`, target denied | Blocked — returns `{"status": "failed", "detail": "..."}` before dispatch |
...
<Card title="Ask Conversation" icon="messages-question" href="/features/ask-conversation-tool">
  Ask another chat a question mid-turn — gated by this same policy.
</Card>
