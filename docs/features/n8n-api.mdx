...
<Note>
`--auth` (with `CALL_SERVER_TOKEN`) is the legacy `praisonai/jobs/server.py` call-server flag — **not** the `serve agents` auth. For `praisonai serve agents`, use `--api-key` / `PRAISONAI_SERVE_API_KEY` instead. See [Serve Agents Authentication](/docs/features/serve-agents-auth).
</Note>
</Step>
