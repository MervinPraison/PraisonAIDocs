...
sequenceDiagram
    participant Client
    participant Route as /invoke handler
    participant Resolve as resolve_session_agent
    participant Clone as Agent clone
    participant Cleanup as clone teardown

    Client->>Route: POST {message, session_id?}
    Route->>Resolve: (agent_id, session_id)
    Resolve->>Clone: clone_for_channel()
    Clone-->>Route: isolated clone
    Route->>Clone: astart() / start()
    Clone-->>Route: result
    Route->>Cleanup: await aclose() / close()
    Cleanup->>Clone: aclose() / close()
    Cleanup-->>Route: (errors logged, swallowed)
