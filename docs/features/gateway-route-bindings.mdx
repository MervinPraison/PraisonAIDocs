...
graph LR
    subgraph "Route Bindings"
        ...
        Resolve -->|peer match| VIP[👑 VIP Agent<br/>profile: vip]
        Resolve -->|channel match acme| Acme[🛟 Support Agent<br/>profile: acme]
        Resolve -->|channel match globex| Globex[🛟 Support Agent<br/>profile: globex]
        Resolve -->|fallback| Default[🤖 Default Agent<br/>profile: none]
    end
    ...
    class VIP,Acme,Globex,Default agent
...
| `profile` | `Optional[str]` | `None` | Isolated tenant-profile name. Names a per-route isolation scope the wrapper enters for the turn (memory namespace / secret scope / home), so one gateway can safely multiplex tenants. `None` means the route is unscoped — the wrapper never falls back to another tenant's profile. Blank/whitespace strings are normalised to `None`. See [Gateway Tenant Profiles](/features/gateway-tenant-profile). |
...
print(match.profile)  # "globex"  ← wrapper keys memory/secrets off this
```
