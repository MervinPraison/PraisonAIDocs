...
<Note>
For non-interactive / CI provisioning, call `praisonai_bot.provision_gateway_config(platform=..., token=..., agents=[...])` — see [Gateway Admin API](/features/gateway-admin-api).
</Note>
...
<Note>
The auto-installed unit already bounds crash-loops on the gateway's fatal-config exit code (78, `EX_CONFIG`) — no manual `RestartPreventExitStatus`/`KeepAlive` tweaking needed. See [Gateway Exit Codes](/features/gateway-exit-codes).
</Note>
</Step>
...
channels:
  telegram:
    ...
    ack_scope: "group-mentions" # Who gets acked: off | direct | group-mentions | group-all | all
```

<Note>
If you leave `TELEGRAM_ALLOWED_USERS` empty, you must also set `unknown_user_policy: "allow"` for the bot to reply to anyone (since [PR #1885](https://github.com/MervinPraison/PraisonAI/pull/1885)). For production, set `TELEGRAM_ALLOWED_USERS` to your user IDs and leave `unknown_user_policy` at the default `"deny"`. This applies to **both** `praisonai gateway start` and `praisonai bot start` ([PR #5111](https://github.com/MervinPraison/PraisonAI/pull/5111)). For the single-bot YAML shape, see [Standalone Bot YAML → Admission policy](/features/standalone-bot-yaml#admission-policy-unknown-user-policy-owner-user-id).
</Note>

Ack and done emojis are pre-populated so freshly onboarded bots give visible feedback during long agent operations from the start. The scope defaults to `group-mentions`, so freshly onboarded bots stay quiet on ambient group chatter out of the box — set `ack_scope: "all"` to ack every message.

<Tip>
**What runs next.** `praisonai gateway start` now auto-discovers this `~/.praisonai/bot.yaml` — no `--config` needed. It prints `Using gateway config: <path>` and binds **with channels**, so the happy path after onboarding is a two-command start:

```bash
praisonai onboard        # writes ~/.praisonai/bot.yaml + ~/.praisonai/.env
praisonai gateway start  # discovers the onboarded file automatically
# → Using gateway config: /Users/you/.praisonai/bot.yaml
```

See [Gateway Config Discovery](/features/gateway-config-discovery) for the full resolution order.
</Tip>
...
## Step 5c: Verify the agent replies

After writing `bot.yaml`, the wizard runs a single `"ping"` turn against the first channel with a token — the same code path as `gateway start --verify-turn`. This confirms your model credential works at **setup**, not on the first user message.

```
Step 5c: Verify the agent replies
  Running one test turn ('ping')...
  ✓ Agent replied — model credentials work        # success
  ⚠ Agent did not reply: <detail>                  # failure (warn, do not abort)
    Set the model/provider API key (e.g. OPENAI_API_KEY) in
    ~/.praisonai/.env, then re-run `praisonai onboard` or
    `praisonai gateway test --turn ping`.
```

This step is **best-effort** — a failure prints a warning with a next step but never aborts onboarding. It is **skipped** when no channel has a token.

Opt out with an environment variable:

```bash
PRAISONAI_NO_VERIFY_TURN=1 praisonai onboard
```

`PRAISONAI_NO_VERIFY_TURN` accepts `1`, `true`, `yes`, or `on`.

| Variable | Values | Effect |
|----------|--------|--------|
| `PRAISONAI_NO_VERIFY_TURN` | `1` / `true` / `yes` / `on` | Skip the Step 5c real-turn check during onboarding. |

<Note>
The startup equivalent is `praisonai gateway start --verify-turn` (on by default). See [Gateway Readiness → Turn pre-flight](/features/gateway-readiness#turn-pre-flight).
</Note>
...
│   praisonai gateway start      # foreground; discovers bot.yaml │
...
The generated `bot.yaml` will contain multiple channels routing to specialized agents. See the [Multi-Channel Bots guide](/features/multi-channel-bots) for complete documentation.
...
<CardGroup cols={2}>
  ...
  <Card title="Installation Guide" icon="download" href="/install/installer">
    Complete installer documentation including onboarding flow
  </Card>
  <Card title="Dashboard" icon="layout-dashboard" href="/cli/dashboard">
