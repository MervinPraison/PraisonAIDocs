...
<Note>
The auto-installed unit already bounds crash-loops on the gateway's fatal-config exit code (78, `EX_CONFIG`) — no manual `RestartPreventExitStatus`/`KeepAlive` tweaking needed. See [Gateway Exit Codes](/docs/features/gateway-exit-codes).
</Note>
</Step>
