...
<Note>
Interactive bot actions cover button and select-menu clicks. For **passive** inbound events like reactions, edits, and member changes, see [Inbound Platform Events](/features/bot-inbound-events).
</Note>
...
Without a store, overflowing values fall back to the lossy `reply:#<sha1[:16]>` hash and the handler is skipped. See [Interactive Callback Payload Store](/features/interactive-callback-store) for the full flow, configuration, and custom backends.

---

## Handling poll results

A poll block collects a group vote; its aggregated tally comes back through the same registry under the reserved `poll` namespace. Register `make_poll_result_handler` and read the typed `PollResult`:
...
The handler reads `context.platform_data["poll_result"]` (accepting a `PollResult` or its dict form) and forwards it to `on_result`. See [Poll Blocks](/features/poll-blocks) for authoring polls and the full `PollResult` API.
...
<Accordion title="Sharing the callback secret across multiple hosts">
Inline-button callbacks are HMAC-signed. The signing key is the persisted
gateway secret at `~/.praisonai/gateway/.gateway_secret` (mode `0600`,
auto-created on first use) — no configuration needed on a single-host deploy.
Buttons survive gateway restarts by default.

Set `PRAISONAI_CALLBACK_SECRET` to a shared value when running the gateway on
multiple hosts so every replica verifies the same signatures:
...
```mermaid
graph LR
    A[Callback fired] --> B{PRAISONAI_CALLBACK_SECRET set?}
    B -->|Yes| C[Use env value]
    B -->|No| D[Load ~/.praisonai/gateway/.gateway_secret]
    D -->|missing| E[Create 0600, use new value]
    D -->|exists| F[Use file value]
    C --> G[HMAC-verify callback]
    E --> G
    F --> G
...
<CardGroup cols={2}>
  ...
  <Card title="Callback Payload Store" icon="database" href="/features/interactive-callback-store">
