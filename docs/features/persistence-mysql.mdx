...
`Agent(
    memory=MemoryConfig(
        db=db(database_url="mysql://..."),
    ))` handles store wiring — no manual setup needed.
</Accordion>
...
<CardGroup cols={2}>
<Card title="MySQL Conversation Store" icon="database" href="/features/persistence-conversation-mysql">
  Direct store API and registry usage
</Card>
<Card title="PostgreSQL Persistence" icon="elephant" href="/features/persistence-postgres">
