...
```python
from praisonaiagents.tools.registry import get_tool_definitions
from praisonaiagents.permissions import PermissionManager

mgr = PermissionManager()
mgr.load_rules_from_config({"write_file": "deny"})

# Deny hides the tool from the advertised schema
defs = get_tool_definitions(permission_resolver=lambda name: not mgr.is_denied(name))
```

| Arg | Type | Default | Description |
|-----|------|---------|-------------|
| `permission_resolver` | `Callable[[str], bool]` \| `None` | `None` | Returns `False` to drop a tool from the schema (deny = hide). Fail-open on error. |
...
<CardGroup cols={2}>
<Card title="Tool Schema Validation" icon="shield-check" href="/features/tool-schema-validation">
