...
sequenceDiagram
    ...
    loop Each LLM call
        ...
        alt total >= max_budget (stop mode)
...
<CardGroup cols={2}>
  <Card title="Budget Management" icon="wallet" href="/features/token-budgeting">
    Full SDK API for budgets, cost tracking, and `on_budget_exceeded` actions
  </Card>
  <Card title="Execution Configuration" icon="gear" href="/configuration/agent-config">
