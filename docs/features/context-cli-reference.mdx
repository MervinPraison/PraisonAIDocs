...
sidebarTitle: "Context CLI Reference"
...
```mermaid
graph LR
    subgraph "Context CLI"
        A[💻 CLI Flags] --> B[🖥️ praisonai chat]
        C[🔑 Env Vars] --> B
        D[📄 config.yaml] --> B
        B --> E[✅ Agent with Context]
    end
...
<Steps>
<Step title="Start with smart context management">
```bash
praisonai chat --context-strategy smart --context-threshold 0.8
```
</Step>

<Step title="Enable monitoring and redaction">
```bash
praisonai chat --context-monitor --context-redact --context-monitor-path ./context.txt
```
</Step>
</Steps>
...
<AccordionGroup>
<Accordion title="Use smart strategy by default">
The `smart` strategy automatically selects the best compression approach, making it safe to use in most situations.
...
<Accordion title="Set threshold to 0.7 for safety-critical agents">
A lower threshold triggers optimization earlier, leaving more headroom for unexpected context growth.
</Accordion>

<Accordion title="Enable monitoring during development">
Use `--context-monitor --context-monitor-frequency turn` to see token usage after every response.
</Accordion>

<Accordion title="Keep redaction enabled in production">
Default redaction masks API keys, tokens, and PII in monitoring snapshots.
</Accordion>
</AccordionGroup>
...
<Card title="Context API" icon="terminal" href="/features/context-api">
  Python SDK and configuration reference
</Card>
</CardGroup>
