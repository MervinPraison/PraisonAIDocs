...
<Note>
**Stop then immediately Send is safe.** If the user taps Stop and the controller then hands the engine a fresh run whose `AbortSignal` is already down, the engine aborts before subscribing — no deltas stream, and nothing is persisted for the aborted run. Before [#4581](https://github.com/MervinPraison/PraisonAI/pull/4581), the run streamed to completion and persisted an answer that had no consumer. The dispose-then-new-run path is the other route into the same already-aborted signal. See [Mobile Engines](/features/mobile/engines) for the engine-side detail.
</Note>

<Note>
The engine side of this contract — that aborting the signal actually stops the stream — is now pinned by a spawned break mode (`abort_ignored`) in the engines conformance suite. See [Adapter Conformance](/features/mobile/adapter-conformance).
</Note>
