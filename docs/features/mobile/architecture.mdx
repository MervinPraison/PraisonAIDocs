...
The mobile app boots in a fixed order so the engine that records a turn and the chat the screen reads are the same store. Navigation then splits into a pure decision layer and a thin DOM layer so the part worth testing has no browser in it.
