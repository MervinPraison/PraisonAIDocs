...
Inside `enginesFor`, `baseUrl` is a resolver, not a captured string — the next request goes wherever Settings says right now. See [Engines → The address is resolved per request](/features/mobile/engines#the-address-is-resolved-per-request).
...
|------|--------------|----------|
| 0. Parser paints the boot indicator | `index.html` paints the `PraisonAI` wordmark and `Starting…` from markup, styled by the render-blocking stylesheet | The first frame is no longer blank, before any script runs. See [Boot Indicator](/features/mobile/boot-indicator). |
| 0a. `boot-guard.js` installs its `error` listener | A classic script, loaded before the deferred `app.js` module, listens in the capture phase for `<script src="./app.js">` failing or a throw whose target is `window` | A listener installed by the deferred module could miss the fetch's `error` when a cache answers instantly. See [Boot Indicator → The failure notice](/features/mobile/boot-indicator#the-failure-notice). |
| 0b. Install crash handler | `installCrashHandler({...})` in `mount()` | Installed before anything else can throw — including `detectPlatform`, which reads `window.localStorage` and raises `SecurityError` in a WKWebView with site data blocked. Running it second left a blank white page on a device nobody can debug. |
...
<Warning>
Because the crash handler is global, any async work started **after** boot — e.g. a route handler rebuilding the chat list — must catch its own rejections. An uncaught reject escalates to the crash screen even though boot succeeded. Pinned by `"a storage failure while the chat list loads stays LOCAL, not fatal"` in `app/src/main.test.ts`. See [History & Reopen → When the list load itself fails](/features/mobile/history-and-reopen#when-the-list-load-itself-fails).
</Warning>
...
The reopen path **preserves the role**: `historyRows` maps a stored `user` message to a `user` row and an `assistant` message to a `text` row, so a restored conversation paints both speakers rather than flattening them into one voice. The shape of the flow above is unchanged; only the row kind each message becomes is.

<Note>
Full detail — including why history row ids carry a `history:{index}:{role}` prefix and the bug the reconciler-seeded restore replaces — is on [History & Reopen](/features/mobile/history-and-reopen). The role-preserving user row itself is on [Transcript User Row](/features/mobile/transcript-user-row).
</Note>
...
The publish gate is the pipeline's single backpressure authority: it skips a paint while the renderer is still catching up, then releases the stranded text once it can keep up.

`core/src/pacing/publish-gate.ts` is a port of the desktop's stream-pacing, with two constants tightened for mobile.

| Constant | Value | Meaning |
|----------|-------|---------|
| `MAX_HELD_CHARS` | `96` | Characters allowed to accumulate behind a closed gate before it forces a paint. |
| `UNPAINTED_REOPEN_MS` | `200` | How long the gate stays shut before it reopens on its own. |
...
| Path | Wiring | Gated? |
|------|--------|--------|
| Per-event (a delta the coalescer paints) | `if (frames.length > 0 && gate(streamed)) publish()` | Yes |
| Flush tick (the fix) | `if (gate(streamed)) publish()` | Yes |
| Structured event (`start`, `end`, `tool-call`, …) | publishes the whole transcript, sets `paintedChars = streamed` | **No — by design** |
| Final `finally`-publish | always publishes at stream end | **No — by design** |

Before the wiring, the flush tick published unconditionally and the coalescer drained the buffer every `maxDelayMs`, so `push()` returned `[]` and the per-event gate was consulted only above ~3200 tokens/sec — unreachable at real streaming speed (20–150 tokens/sec). After the wiring, the gate is consulted on every tick after the first paint, so both constants are load-bearing.

### How backpressure now works

Under load the gate skips a tick's paint; when it reopens — via `requestFrame` or after `UNPAINTED_REOPEN_MS` — a catch-up tick paints whatever the rejected tick had drained onto the transcript but left unpainted. `MAX_HELD_CHARS` bounds how far the paint can fall behind `streamed`, and the ungated `finally`-publish guarantees the tail always lands, so a closed gate can never swallow the end of an answer.

### Stranded progress and catch-up

A tick drains the coalescer *before* the gate is consulted, so text from a rejected tick is already gone from the coalescer — it lives on the transcript but is not yet painted. If the stream then pauses, no later tick sees pending text and the gate reopening does not itself publish. The controller tracks `paintedChars`; on a tick that finds nothing new to drain, if `streamed > paintedChars` and the gate has reopened, it publishes that stranded progress. Streamed text that arrives just before a pause therefore paints mid-pause, not only when the turn ends.

```mermaid
sequenceDiagram
    participant Tick as Flush tick
    participant Gate as Publish gate
    participant Transcript
    participant Paint

    Tick->>Gate: gate(streamed) — open
    Gate-->>Paint: publish (paintedChars = streamed)
    Tick->>Gate: gate(streamed) — closed (backpressure)
    Gate-->>Transcript: text drained, left unpainted
    Note over Gate: reopens after UNPAINTED_REOPEN_MS
    Tick->>Gate: catch-up tick — streamed > paintedChars
    Gate-->>Paint: publish stranded text
```
...
<Card title="Native Shell" icon="mobile-button" href="/features/mobile/native-shell">
  The Tauri shell — safe-area, keyboard, lifecycle, and back-gesture arbitration. On Android the safe-area and keyboard values come from `WindowInsetsCompat` through a native → JS global, not from `env()` / `visualViewport`.
</Card>
<Card title="Native Secrets" icon="shield-keyhole" href="/features/mobile/native-secrets">
  The `tauri-plugin-secrets` plugin behind `platform.secrets` — iOS / macOS via `SecItem*`, Android via `EncryptedSharedPreferences`.
</Card>
