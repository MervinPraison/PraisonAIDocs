...
<Note>
If a user does reach the error path — clears the key mid-chat and sends — [PR #4873](https://github.com/MervinPraison/PraisonAI/pull/4873) now classifies the resulting message (e.g. `The OPENAI_API_KEY environment variable is missing or empty`) as `auth`, and the row's recovery button lands them on Settings, matching this panel's affordance. See [Errors & Recovery → Common Pitfalls](/features/mobile/errors-and-recovery#common-pitfalls).
</Note>
