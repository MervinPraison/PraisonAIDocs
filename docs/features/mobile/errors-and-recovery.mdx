...
If a user still reaches this error prose from another path — clearing the key mid-chat and then sending a message — the recovery action lands them on Settings via the same `strings.recoveryLabel("settings")` → **"Open settings"** label the empty-chat panel uses, so the route reads the same everywhere.
