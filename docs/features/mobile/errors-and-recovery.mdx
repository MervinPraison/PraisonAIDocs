...
<Info>
A module-load failure inside the in-process engine's `createAgent` — expected in the shipping webview build where praisonai-ts is bundle-absent ([#4437](https://github.com/MervinPraison/PraisonAI/issues/4437)) — surfaces as an ordinary recoverable `error` event, so the standard Retry affordance applies. Pinned by `"the in-process engine, when its module cannot load, fails RECOVERABLY"` in `app/src/main.test.ts`.
</Info>

<Note>
**Some transport failures now surface at boot, not mid-turn.** A remote engine that is unreachable at launch is refused by the boot-time health probe as a `retryable` `BootResult` — the app shows an error immediately after startup rather than mid-answer. If a support ticket says "the app got stuck showing an error right after opening", it is the boot path, not this mid-turn one. See [Boot Failures → Handle a retryable boot failure](/features/mobile/boot-failures#handle-a-retryable-boot-failure).
</Note>
