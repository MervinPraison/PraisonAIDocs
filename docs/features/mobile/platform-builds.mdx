...
<Warning>
`MainActivity.onCreate` calls `enableEdgeToEdge()`, so the WebView paints **behind** the system bars and the IME. The consequence is that no web API can see either: `env(safe-area-inset-*)` in Chromium on Android reports the **display cutout only**, and `visualViewport` / `navigator.virtualKeyboard.boundingRect` report nothing for the keyboard. The app recovers these values through a native → JS bridge — see [Native Shell → The Android window-insets bridge](/features/mobile/native-shell#the-android-window-insets-bridge).
</Warning>
