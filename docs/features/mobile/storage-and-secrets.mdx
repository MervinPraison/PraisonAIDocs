...
The storage contract is pinned by the same fixture — `storage_missing_is_undefined` and `storage_namespaces_collide` are the two storage break modes. See [Adapter Conformance](/features/mobile/adapter-conformance).
