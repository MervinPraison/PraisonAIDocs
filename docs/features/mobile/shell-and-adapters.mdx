...
This was a hard `0` whose only writer was the native event — and `on_window_event` in `lib.rs` is a stub, so `--keyboard-height` stayed `0` on device forever. Because `app.css` pins `#root` to `position: fixed; inset: 0`, the layout viewport did not shrink either, and the composer sat under the keyboard the instant anyone tapped it.
