...
The composer button toggles between Send and Stop, and refuses empty input.
...
| Empty and whitespace input | The composer trims (`input.value.trim()`) and blocks a submit whose remaining text is `""`. | Whitespace-only prompts never reach the engine. |
