...
|-----------|--------------|-----|
| Send / Stop | The button carries `data-action="send"` when idle and `data-action="stop"` while a run streams. | Tapping Stop calls `controller.stop()` — the run ends and no further tokens are billed. |
