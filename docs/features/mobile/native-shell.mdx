...
## Where the empty-chat panel lives in the DOM

The empty-chat panel ([Empty Chat](/features/mobile/empty-chat)) is a **sibling** of `.transcript`, never a child.

| Rule | Source | What breaks if it regresses |
|------|--------|-----------------------------|
| The panel is a sibling of `.transcript`, not a child | `emptyPanel` appended to the screen, not the transcript | Three code paths do `transcript.textContent = ""` (New chat, Open chat, delete-the-open-chat) and `applyOps` places rows by `children[index]`; a child panel would be displaced or silently deleted. |
| The screen carries `data-empty` | `screen.dataset["empty"] = view.kind` in `paintEmptyState` | Written as `"welcome"` or `"needs-key"` matching the current `EmptyStateKind`. `.screen[data-empty] .transcript { flex: 0 0 auto }` stops both children of the flex column claiming `flex: 1` and splitting the screen. |
| The panel toggles via the `hidden` attribute | `.empty-state[hidden] { display: none }` | The rule overrides the `display: flex` layout, exactly as `.screen[hidden]` does. Never toggle via `style.display`. |
| Safe-area insets add to the gutter | `.empty-state` uses `--inset-left` / `--inset-right` | The insets are **added** to the gutter rather than replacing it; the topbar's `--inset-top` handling is untouched. |
| Colours are tokens only | `--ink`, `--soft`, `--accent` | All three have dark-scheme values, so the panel follows the device theme with no rule of its own. A hardcoded colour would be invisible on the dark ground. |

The region id `EMPTY_TITLE_ID` (`"empty-state-title"`) is a single constant shared by the heading's `id` and the panel's `aria-labelledby`, so the two cannot drift — see [i18n & A11y → Empty-chat region](/features/mobile/i18n-and-a11y#empty-chat-region).
...
<Card title="Empty Chat" icon="message-square-dashed" href="/features/mobile/empty-chat">
  The empty-state panel that sits beside the transcript.
</Card>
</CardGroup>
