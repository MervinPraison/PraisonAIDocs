...
| Web → Native | `back_gesture_result` | `CMD_BACK_RESULT` | `{ handled: boolean }` | Answered via `bridge.invoke`; failures are swallowed — see arbitration below |
...
The capability file `src-tauri/capabilities/default.json` grants the two permissions this seam needs: `core:event:default` (so the webview may subscribe) and `allow-back-gesture-result` (so it may answer). Without them Tauri rejects `plugin:event|listen` and the command silently.
