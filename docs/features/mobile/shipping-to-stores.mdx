...
The bundle gate also flags **unresolved bare imports** — packages that are not Node builtins and that esbuild could not resolve. A webview has no module resolver, so `import "openai"` in the shipped file dies at import time with the same blank screen as a static Node builtin.
