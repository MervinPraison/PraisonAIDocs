...
The **OpenAI API key** row is the lead section on the screen, in its own **API key** heading above **Engine**. It shows **Not set** and a masked field. The **Remove** button appears only once a key is actually stored ([`canClear`](/features/mobile/settings-screen#secret-rows)), so a "Not set" row has no button under it. On a platform without a hardware keychain (the web adapter, where `isHardwareBacked === false`), a warning above the rows says secrets are kept in app memory.
</Step>
...
To rotate, paste a new value over the old one — the replacement also survives a restart. To revoke, press **Remove** (shown only while the row is **Configured**) — the row returns to **Not set**, the button disappears in the same pass, and the keychain entry is gone.
</Step>
</Steps>
...
| **Remove** is painted only when a key exists to remove | `SecretRow.canClear` is `state === "configured" && ref !== undefined`; the button sets both `hidden` and `disabled` from `!canClear`, so an `unknown` (in-flight) or `not-set` row carries no live button. See the [`canClear` rule](/features/mobile/settings-screen#secret-rows). |
...
```ts
{
  ...
  section: SECTION_API_KEY, // "API key" — the lead section, above "Engine"
  secret: true,
