...
The time contract's `realClock` branch adds two assertions, so it checks `EXPECTED_ASSERTIONS + (realClock ? REAL_CLOCK_ASSERTIONS : 0)` where `REAL_CLOCK_ASSERTIONS = 2`.
