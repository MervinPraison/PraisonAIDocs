...
Two cases in the shared contract pin the Tauri shell's viewport fallback and the single-writer handover to the native event; a third pin covers the Android insets global taking over from the viewport reading.
...
| Test | Where | Asserts |
|---|---|---|
| `"the TAURI shell reports the keyboard height, without any native event"` | `contracts.test.ts` | The viewport fallback reports the height and fires subscribers exactly once, on open and on close. |
| `"a native keyboard height takes over from the viewport reading"` | `contracts.test.ts` | The first native height becomes authoritative and the viewport listener is dropped, so the two sources never race. |
| the Android insets global takes over from the viewport reading | `adapters/src/tauri/native-insets.test.ts` + `tools/shell-seam.test.mjs` | Driven with a fake `Window` that has no `visualViewport` — the Android reality — the global installs, publishes both the insets **and** keyboard channels, drops non-object payloads, coerces via `toPx`, and retires the viewport listener on first fire. `shell-seam.test.mjs` pins the Kotlin `INSETS_GLOBAL` to the TypeScript `NATIVE_INSETS_GLOBAL`. |

See [Shell & Adapters → The three-source keyboard model](/features/mobile/shell-and-adapters#the-three-source-keyboard-model).
...
The `SecretsPort` contract grew two optional branches, supplied only by an adapter that can back them — a durable native store (`reopen`) and one that can count reads (`readsOf`). The web adapter, a module-scoped `Map`, claims neither, so it is not asked to.

| Branch | Supplied via | Cases | What it catches |
|---|---|---|---|
| **durability** | `reopen(secrets)` — a second port over the same store | 4 | A keychain that forgets when the process ends is not a keychain: the user re-types the key on every launch. |
| **presence** | `readsOf(secrets)` — how often a value was read out | 1 | `has()` implemented as `get(ref) !== null` returns the right booleans and copies the key into the webview heap on every repaint. |

The four durability cases (`adapters/src/conformance/secrets-contract.ts`):
...
A separate guard pins the break-mode table itself: `ADAPTER_BREAKS.length >= 18`, raised as new modes land (`>= 15` before the secrets durability and presence branches, `>= 13` before the two storage durability modes). This counts break-mode rows, not passing cases per contract, so it moves independently of the per-contract floors above.
...
|---|---|---|
| secrets | 16 (+ 6 durable, + 4 presence) | A durable adapter (supplies `reopen`) adds six relaunch assertions; one that can count reads (supplies `readsOf`) adds four presence assertions |
