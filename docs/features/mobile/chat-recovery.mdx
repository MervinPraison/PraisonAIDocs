...
Cross-linked from [Storage & Secrets → How Session Persistence Works](/features/mobile/storage-and-secrets#how-session-persistence-works), which shows the sequence but does not state the boundary.
