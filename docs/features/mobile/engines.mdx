...
graph TB
    ...
    Default --> Today[remote-http #40;every platform, for now#41;]
    Today --> Boot[🖼️ mount uses this as engineId]
...
Today the function returns `remote-http` for both `"tauri"` and `"web"`, pinned by `"every platform's first-launch default is the remote engine, for now"` in `app/src/main.test.ts`.

```ts
import { defaultEngineIdFor } from "praisonai-mobile/app/main";

defaultEngineIdFor("tauri"); // "remote-http"
defaultEngineIdFor("web");   // "remote-http"

// mount() calls it once, at boot, when no engineId is persisted.
// A persisted engineId always wins over this default.
```

The tempting device default — in-process on a phone — is wrong today for two independent reasons.

The in-process engine's module is ABSENT from the shipping webview. `build-webview.mjs` bundles only `dist/app.js`; the engine reaches praisonai-ts through a runtime-computed dynamic import deliberately left outside that bundle ([#4437](https://github.com/MervinPraison/PraisonAI/issues/4437)). Defaulting to it would swap the *"engine not answering"* warning (which names Settings as the fix) for a first prompt that fails *"the in-process engine is unavailable in this build"* with **no** recovery — the exact brick `registry.ts` keeps the engine out of `SETTING_DEFS.engineId.choices` to avoid.

`Platform["kind"]` is only `"tauri" | "web"`, and **desktop** Tauri (`cargo tauri dev`) reports `"tauri"` too. So a `kind === "tauri"` default would flip the desktop/dev flow away from the remote engine it exists for, with no way to tell a phone from a laptop.

<Warning>
**A `kind === "tauri"` default would also strand desktop Tauri.** `Platform["kind"]` cannot tell `cargo tauri dev` on a laptop from a Tauri build on a phone — both report `"tauri"`. A future device-vs-desktop split needs its own signal, not `kind`.
</Warning>

The seam still exists so a test drives it directly rather than asserting an expression appears in `mount` (mirrors `stopNotice` and `appEngines`, the house rule), and so the day a device can be told apart AND the in-process engine ships inside the webview, the change is one function body — not a change to `mount()`.

Once [#4437](https://github.com/MervinPraison/PraisonAI/issues/4437) ships praisonai-ts inside the webview and a device is distinguishable from a desktop Tauri build, this function will return the in-process engine on device. Removing the intermediate hop over `http://127.0.0.1:8765` also removes the iOS ATS / Android `usesCleartextTraffic` exception on the default path (see [Errors & Recovery](/features/mobile/errors-and-recovery)).
...
<Note>
Today only `remote-http` appears in the picker (`SETTING_DEFS.engineId.choices`). The in-process engine is offered by the factory list but is not yet a legal `engineId` setting value. In a build that ships praisonai-ts on disk, `engineId: "praisonai-ts"` can be selected programmatically; the picker gains it once [#4435](https://github.com/MervinPraison/PraisonAI/issues/4435) + [#4437](https://github.com/MervinPraison/PraisonAI/issues/4437) land. The first-launch default (`defaultEngineIdFor`) also returns `remote-http` on every platform for now, for the same reason: a device would boot into an engine whose module is not on disk. See [First-Launch Default](#first-launch-default).
</Note>
