...
The web keeps the remote engine: a browser tab has a server to talk to, and no reason to fetch ~900 kB of engine before first paint. See [Web App (PWA)](/features/mobile/web-app#which-engine-the-web-picks) for how the web target boots, installs, and works offline.
