...
<Note>
**Unhealthy is retryable.** An `unhealthy` verdict carries `retryable: true`. An engine still binding its socket must remain worth polling; declaring it permanently dead strands the user until they restart the app. Pinned by `"an unhealthy engine is worth retrying"`, with `"a healthy engine IS ready - the pair"` as its companion.
</Note>

<Note>
**The probe now actually runs at boot.** These readiness rules are no longer hypothetical: `enginesFor` wires `probe: () => probeHealth(deps.http, baseUrl)` on the remote-http choice, and `selectEngine` calls it before the engine is offered. A `200 {"ok": false}`, a non-200, or an unreachable engine is refused at boot with a named reason — see [Boot Failures → Boot-time health probe](/features/mobile/boot-failures#boot-time-health-probe).
</Note>
...
| Step | Check | Failure |
|------|-------|---------|
| 1 | Is `id` known? | `unknown_engine` |
| 2 | Does `protocolVersion` match? | `protocol_mismatch` |
| 3 | Does the probe (if any) return `ready: true`? | `unhealthy` / `http_status` / `malformed` / `transport` / `version_mismatch` — engine is disposed |

That step 3 does not run when step 2 fails is pinned by `"selectEngine runs the probe only after the protocol check passes"`.
...
Where a failed probe becomes a named, retryable boot failure.
</Card>
</CardGroup>
