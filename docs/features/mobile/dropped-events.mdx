...
<Note>
Dropped rows currently render unconditionally; there is no toggle. A visibility setting will land when a settings screen reads it (see PraisonAI [#4636](https://github.com/MervinPraison/PraisonAI/issues/4636) / [#4646](https://github.com/MervinPraison/PraisonAI/pull/4646) for the reasoning).
</Note>
...
<Warning>
The default `baseUrl` is `127.0.0.1:8765`, which on a phone is the phone itself — so this was the first thing every new user saw. The real reason now shows on the transcript, and the error row carries a **recovery** affordance. The story completes without a restart: an unreachable engine's corrected address takes effect on the next message. See [Errors & Recovery → The default baseUrl is the phone itself](/features/mobile/errors-and-recovery#the-default-baseurl-is-the-phone-itself).
</Warning>
