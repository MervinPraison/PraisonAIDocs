...
Group related skills into named, reusable sets with `praisonai skills bundle list` and `praisonai skills bundle show <name>`. Select a whole bundle from an agent with `skills=["@backend-dev"]`. See [Skill Bundles](/docs/features/skill-bundles) for the full guide.
