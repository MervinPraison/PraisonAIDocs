...
<Tip>
To trigger skill creation **automatically** after every task instead of having the model call `skill_manage` on its own, see [Self-Improving Agents](/features/self-improve).
</Tip>
...
| Path | Format | Purpose |
|------|--------|---------|
| `~/.praisonai/skills/.pending_skills.json` | JSON map `{id: record}` | Staged mutations awaiting approval |
| `~/.praisonai/skills/.skill_audit.log` | JSON-lines | Append-only audit: `proposed` / `approved` / `approval_failed` / `rejected` events |
...
```json
{
  ...
  "payload": {
    "content": "…",
    "category": "reporting",
    "base_dir": "/path/to/project-a/.praisonai/skills"
  }
}
```
...
```
INFO praisonaiagents.skills.manager: Skill created: weekly-summary (SKILL.md at /path/to/.praisonai/skills/weekly-summary/SKILL.md)
```
...
mgr = SkillManager()
mgr.create_skill("weekly-summary", "# Weekly Summary", propose=False)
# INFO praisonaiagents.skills.manager: Skill created: weekly-summary (SKILL.md at …/.praisonai/skills/weekly-summary/SKILL.md)
```
</Note>
...
```mermaid
sequenceDiagram
    participant DevA as 🤖 Dev in project A
    participant Store as 💾 ~/.praisonai/skills/.pending_skills.json
    participant DevB as 👤 Approver (any cwd)
    participant DiskA as ✅ project-A/.praisonai/skills/

    DevA->>Store: create_skill(propose=True)<br/>pins base_dir = project-A/.praisonai/skills
    DevB->>Store: approve(id)
    Store-->>DevB: payload.base_dir = project-A/.praisonai/skills
    DevB->>DiskA: writes SKILL.md at the pinned path

    %% #8B0000 agents, #189AB4 store, #10B981 destination
```
...
Writes deliberately skip steps 2 and 4 so mutations can never land in an ancestor or admin-managed location.
...
| **Path Validation** | Prevent traversal | Syntactic prefilter rejects absolute (`/`), home (`~`), null bytes, and any `..`/empty segment |
| **Real Containment** | Enforce the boundary | Caller's `resolve().relative_to(skill_path.resolve())` |
| **Atomic Writes** | Prevent corruption | Temp file + rename operations |
| **Allowed Subdirs** | Restrict file placement | Nested relative paths (`references/`, `scripts/`, `assets/`) allowed |
| **Max Pending** | Prevent accumulation | `SKILL_MAX_PENDING=100` (101st returns error) |

### Nested file paths

Nested relative paths inside a skill directory now work in `write_file` and `patch`. The syntactic filter is a **prefilter**, not the security boundary — real containment is still enforced by the caller's `resolve().relative_to(skill_path.resolve())` check.

**Still rejected** (unchanged): absolute paths (`/etc/...`), user-home paths (`~/...`), null bytes (`\0`), and any `..` segment — across both `/` and `\\` separators.

<Note>
**Skill tool grants are per-agent-instance.** A skill's `allowed-tools` auto-approval is keyed by a unique per-instance id, so it cannot be transferred to another agent by a naming collision — two unnamed agents that both default to `name="Agent"` never share grants. See [Tool Approval → Scope of a skill grant](/features/tool-requires-approval#scope-of-a-skill-grant).
</Note>

**Now allowed** (new): nested relative paths such as `references/palette.md`, `scripts/helper.py`, and `assets/img/logo.png`.
...
# ✅ Allowed — nested relative path inside the skill
skill_manage(action="write_file", name="my-skill",
             file_path="references/palette.md", file_content="# Palette")

# ✅ Allowed — deeper nesting
skill_manage(action="write_file", name="my-skill",
             file_path="assets/img/logo.png", file_content="...")

# ❌ Rejected — traversal
skill_manage(action="write_file", name="my-skill",
             file_path="../etc/passwd", file_content="...")
...
Group related skills into named, reusable sets with `praisonai skills bundle list` and `praisonai skills bundle show <name>`. Select a whole bundle from an agent with `skills=["@backend-dev"]`. See [Skill Bundles](/features/skill-bundles) for the full guide.
...
<Accordion title="Don't pre-create empty pending directories">
...
The pending-approval store is safe under concurrent proposals and approvals: `_stage_pending()`, `approve()`, and `reject()` serialise the load-mutate-save cycle behind a per-manager `threading.Lock`, so two agents proposing at the same time no longer clobber each other's entries in `.pending_skills.json`. Audit-log writes happen outside the lock, so a slow disk on the audit sink can't stall subsequent approvals.
...
sequenceDiagram
    ...
    participant Store as .pending_skills.json

    par Concurrent proposals
        A1->>Mgr: skill_manage("edit", "planner")
    and
        A2->>Mgr: skill_manage("create", "summariser")
    end
    Note over Mgr: _pending_lock serialises load→mutate→save
    Mgr->>Store: write A's record
    Mgr->>Store: write B's record
    Note over Mgr,Store: both entries preserved
...
<CardGroup cols={2}>
<Card title="Skill Lifecycle" icon="rotate" href="/features/skill-lifecycle">
  Provenance, telemetry, archive/restore, and rollback for agent-created skills
</Card>
<Card icon="rotate" href="/features/skill-reload">
  Pick up newly installed / edited skills without restarting
</Card>
...
<Card title="Skill Capability Gates" icon="shield-check" href="/features/skill-capability-gates">
  Capability requirements and enforcement for skills
</Card>
<Card title="Self-Improving Agents" icon="arrows-spin" href="/features/self-improve">
