...
```bash
grep -E 'allow_delegation|allow_code_execution|code_execution_mode|auto_save|rate_limiter|verification_hooks|cli_backend|tool_timeout|tool_retry_policy'
```
</Step>
...
<Note>
See [Model Parameter](/features/model-parameter) for the full rule and the list of classes it applies to.
</Note>
...
| `Agent(tool_timeout=30)` | `TypeError: ... tool_timeout` (with migration hint) | `Agent(tool_config=ToolConfig(timeout=30))` |
| `Agent(tool_retry_policy=RetryPolicy(...))` | `TypeError: ... tool_retry_policy` (with migration hint) | `Agent(tool_config=ToolConfig(retry_policy=RetryPolicy(...)))` |

### tool_timeout → tool_config
...
<Tabs>
  <Tab title="Rejected">
    ...
    agent = Agent(name="Assistant", tool_timeout=30)
    ```
  </Tab>
  <Tab title="Correct">
    ```python
    ...
    agent = Agent(name="Assistant", tool_config=ToolConfig(timeout=30))
    ```
  </Tab>
</Tabs>

### tool_retry_policy → tool_config
...
<Tabs>
  <Tab title="Rejected">
    ...
    # TypeError: Agent.__init__() got an unexpected keyword argument 'tool_retry_policy'
    #   tool_retry_policy: tool retry moved into tool_config=; use
    #     tool_config=ToolConfig(retry_policy=RetryPolicy(...)).
    agent = Agent(name="Assistant", tool_retry_policy=RetryPolicy())
    ```
  </Tab>
  <Tab title="Correct">
    ```python
