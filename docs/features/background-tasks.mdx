...
### Subagent Background Mode

The subagent tool uses the same `BackgroundJobManager` under the hood. Use `spawn_subagent(background=True)` to fire-and-forget agent subtasks from inside another agent, then collect results with `subagent_result`.
...
<Tip>
See [Subagent Tool — Background Mode](/docs/features/subagent-tool#background-mode) for the full two-tool dance, return shapes, and scoping guarantees.
</Tip>
...
<CardGroup cols={2}>
  <Card icon="robot" href="/docs/features/subagent-tool#background-mode" title="Subagent Background Mode">
    Fire-and-forget agent subtasks with spawn_subagent and subagent_result.
  </Card>
  ...
  <Card icon="book" href="/docs/guides/background-subagents" title="Background Subagents Guide">
