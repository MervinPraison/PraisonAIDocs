...
The quickest way to run Signal — no Python needed:
...
<Note>
Channel-specific extras (`account`, `bridge_url`) declared under a `bot.yaml` channel are now forwarded correctly through `praisonai bot start` as well — previously the `start` path dropped them.
</Note>
...
**Inbound liveness.** A delivered envelope refreshes the Signal channel's `last_activity` for the shared [Channel Supervision](/features/gateway-channel-supervision#proactive-health-monitoring) health monitor. Successfully-empty polls do **not** refresh it, so a silently half-open bridge (returns `[]` forever without raising) trips `stale-socket` after `stale_after` seconds and the supervisor reconnects — instead of the bot going quietly deaf.
...
<Card icon="heart-pulse" href="/features/gateway-channel-supervision">
