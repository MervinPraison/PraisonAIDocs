...
See [Gateway Durable Runs](/features/gateway-durable-runs) for the full operator surface, including the `durable_runs: false` and `reliability: "off"` opt-outs.
...
The same declaration also gates the ordinary tool-retry loops: a `restart_safe=False` tool is executed exactly once even when a transient error would normally be retried. See [Tool Retry & Backoff → Retrying Non-Idempotent Tools](/features/tool-retry-backoff#retrying-non-idempotent-tools).
...
<Card icon="tower-broadcast" href="/features/gateway-durable-runs">
  Enable durable runs for every agent in the WebSocket gateway.
</Card>
<Card title="Async Jobs" icon="rocket" href="/features/async-jobs#persistent-store">
