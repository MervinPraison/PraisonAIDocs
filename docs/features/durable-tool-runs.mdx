...
Gateway operators enable this SDK-wide by setting one flag in `gateway.yaml` — every gateway agent then journals its turns:
...
See [Gateway Durable Runs](/features/gateway-durable-runs) for the full operator surface.
