...
`.launch()` binds to `127.0.0.1` by default and enforces a bearer token the moment you expose it beyond localhost.

```mermaid
graph TB
    Start[.launch called] --> Q1{PRAISONAI_LAUNCH_AUTH_TOKEN set?}
    Q1 -->|Yes| Enforce[Every request needs the token, 401 otherwise]
    Q1 -->|No| Q2{host is loopback?}
    Q2 -->|Yes 127.0.0.1 / ::1 / localhost| Keyless[Keyless dev flow]
    Q2 -->|No 0.0.0.0 / LAN IP| Auto[Auto-generate one-time token, print to stdout, enforce it]
    ...
    class Q1,Q2 q
    class Keyless,Enforce ok
    class Auto warn
```

`.launch()` has three authentication states:

| State | Trigger | Behaviour |
|-------|---------|-----------|
| **Keyless dev** | No token + loopback host | No auth needed. `agent.launch(port=8000)` just works locally. |
| **Enforced** | `PRAISONAI_LAUNCH_AUTH_TOKEN` set | Every request must send the token or gets `401 Unauthorized`. |
| **Fail-closed** | No token + non-loopback host | The SDK auto-generates a one-time bearer token, prints it once to stdout, and enforces it. |

Loopback hosts are `127.0.0.1`, `::1`, and `localhost`. Anything else (`0.0.0.0`, a LAN IP) is treated as non-loopback.

<Tabs>
  <Tab title="Local dev (keyless)">
    Loopback is the default, so no token is needed:
    ```python
    from praisonaiagents import Agent

    agent = Agent(name="dev", instructions="Answer.")
    agent.launch(port=8000)   # binds 127.0.0.1, no token needed
    ```
    ```bash
  ...
  <Tab title="Public bind (preset token)">
    Set a token before binding to a public host:
    ```bash
    ...
    agent = Agent(name="public", instructions="Answer.")
    agent.launch(host="0.0.0.0", port=8000)   # enforces the token above
    ```
    ```bash
    curl -X POST http://SERVER:8000/ \
  ...
  <Tab title="Public bind (auto-token)">
    Bind publicly without a preset token and the SDK prints a one-time token:
    ```python
    from praisonaiagents import Agent

    agent = Agent(name="public", instructions="Answer.")
    agent.launch(host="0.0.0.0", port=8000)
    # Prints once to stdout:
    #   \U0001F510 launch() bound to non-loopback host '0.0.0.0' without
    #   PRAISONAI_LAUNCH_AUTH_TOKEN set. Generated a one-time bearer token
    #   (set PRAISONAI_LAUNCH_AUTH_TOKEN to override): <TOKEN>
    ```
    Capture the printed `<TOKEN>` and send it on every request as the `Authorization: Bearer` header.
  </Tab>
</Tabs>
...
<Steps>
    ...
    <Step title="Launch as API">
        ...
        agent.launch(
            ...
            host="127.0.0.1",  # Loopback default; set "0.0.0.0" to expose (requires a token)
            port=8000,
...
### Mix `Agent.launch()` and `AgentTeam.launch()` on One Port

`Agent.launch()` and `AgentTeam.launch()` (aka `PraisonAIAgents.launch()`) share a single process-wide server registry.

In one process, a standalone agent and a team launched on the same port bind the same FastAPI app and the same uvicorn server — one `/` endpoint that lists both sets of routes, one `/health`, one `/docs`. You no longer get `Address already in use` when both call sites target the same port, and endpoints registered through one launcher are visible from the other.

```python
from praisonaiagents import Agent, AgentTeam

assistant = Agent(name="Assistant", instructions="Answer.")
research = Agent(name="Research", instructions="Research topics.")
team = AgentTeam(name="Team", agents=[research])

# Same port — same FastAPI app / uvicorn server / route table
assistant.launch(path="/assistant", port=8000)
team.launch(path="/team", port=8000)

# curl http://localhost:8000/       # lists /assistant and /team
# curl http://localhost:8000/health
```

<Note>
**Fix reference:** [PraisonAI PR #5204](https://github.com/MervinPraison/PraisonAI/pull/5204) unified the launch registry across both entry points.
</Note>
...
| `host` | str | Host to bind to. Loopback by default; set `"0.0.0.0"` to expose (then a token is required). | "127.0.0.1" |
...
|----------|---------|-------------|
| `PRAISONAI_SERVER_READY_TIMEOUT` | `5.0` | Seconds `.launch()` waits for the shared HTTP server to become ready — used by both `Agent.launch()` and `AgentTeam.launch()`. Readiness is signalled only after uvicorn reports the socket is bound; on timeout, a warning is logged and execution continues. |
| `PRAISONAI_LAUNCH_AUTH_TOKEN` | *(unset)* | Bearer token required by every request when set. Unset + loopback = keyless dev flow; unset + non-loopback = a one-time token is auto-generated, printed to stdout, and required on every request. See [Authentication](#authentication). |
...
3. **Multiple Agents**: You can run multiple agents *and* teams on the same port with different paths (HTTP mode only). All `.launch()` call sites — `Agent.launch()`, `AgentTeam.launch()`, `PraisonAIAgents.launch()` — share one FastAPI app and one uvicorn server per port.
...
<AccordionGroup>
  ...
  <Accordion title="Agent server slow to start">
    ...
    If a bind fails (`Address already in use`, missing dependency), the SDK logs a warning and clears the port's readiness flag so the next `.launch()` call on that port re-attempts the bind. You do not need to restart the process.
</Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Set a token before exposing, and terminate TLS at the edge">
