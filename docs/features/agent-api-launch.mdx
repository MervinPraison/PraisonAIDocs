...
<AccordionGroup>
<Accordion title="Deploy behind a reverse proxy in production">
Always put nginx or a load balancer in front of the agent HTTP server. Set HTTPS, rate limiting, and request size limits at the proxy level.
</Accordion>
<Accordion title="Use multiple paths on one port for multi-agent setups">
Launch multiple agents on the same port with different paths (`/sales`, `/support`) rather than separate ports — simplifies firewall rules and TLS configuration.
</Accordion>
<Accordion title="Monitor server startup with PRAISONAI_SERVER_READY_TIMEOUT">
If your machine is slow to start, raise `PRAISONAI_SERVER_READY_TIMEOUT` (default 5 s) so `.launch()` waits long enough before returning.
</Accordion>
<Accordion title="Use debug=True only in development">
`debug=True` enables auto-reload and verbose logging — never use it in production as it reloads the agent on every file change and exposes stack traces.
</Accordion>
</AccordionGroup>
