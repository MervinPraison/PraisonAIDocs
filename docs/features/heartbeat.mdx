...
```typescript
import { Agent, Heartbeat } from 'praisonai';

const agent = new Agent({ instructions: 'Check server status and summarize' });
const hb = new Heartbeat(agent, { schedule: 'every 30m', prompt: 'Report system health' });
```
</Tab>
</Tabs>

In TypeScript the schedule accepts the same keywords (`hourly`, `daily`, `weekly`) plus intervals with an optional `"every "` prefix (`every 30m`, `30m`, `every 6h`). `onError` is `"retry" | "skip"` or a callback, and `maxRetries` defaults to `3`.
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Heartbeat
...
Heartbeat uses the same schedule parser as [Schedule Tools](/tools/schedule-tools):
