...
<Note>
**See also: `SendResult.status`.** `error_kind` classifies *why a send failed*; the separate [`SendResult.status`](/features/bot-platform-adapter#branching-on-delivery-outcome) (`"sent" | "failed" | "queued" | "duplicate"`) classifies *what the send outcome was*. The two are orthogonal — a `"failed"` status carries an `error_kind`, while `"queued"` and `"duplicate"` describe deferred and at-least-once deliveries that never failed at all.
</Note>
