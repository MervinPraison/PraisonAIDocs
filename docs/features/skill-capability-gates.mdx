...
agent = Agent(
    name="File Assistant",
    instructions="You help with local files",
    skills=["./skills/file-ops"],                 # skill declares requires_servers: [filesystem]
    tools=MCP("npx -y @anthropic/mcp-server-filesystem ."),  # registers server name 'filesystem'
)
agent.start("List README files under ./docs")
```

Server names are registered when an `MCP(...)` client calls `with_tool_prefix()`. Registration records **both** the raw name and its sanitised form (e.g. `filesystem` for `@anthropic/mcp-server-filesystem`), so a skill's `requires_servers` entry matches either spelling. Inspect the live set at any time:

```python
from praisonaiagents.mcp import MCP

print(MCP.list_active_server_names())   # {'filesystem', 'fetch'}
```

The set reflects **live** MCP instances only. A name appears while at least one instance that registered it is alive, and drops out once every such instance has been `shutdown()` (or garbage-collected — `__del__` calls `shutdown()`). A skill's `requires_servers` entry must match a name from this set exactly.

<Warning>
Shrink-on-shutdown landed in [PR #5136](https://github.com/MervinPraison/PraisonAI/pull/5136). `list_active_server_names()` is now backed by a per-name refcount, so a name is released once the last MCP instance using it shuts down. Under `STRICT` enforcement, a skill declaring `requires_servers: [name]` is correctly hidden again after every MCP instance using `name` has shut down. This closes a regression where, in a long-running process (gateway, task worker, test suite), the first connection of any server name left the STRICT gate passing forever.
</Warning>

```mermaid
graph LR
    MCP[🔌 MCP client] -->|with_tool_prefix| Reg[📇 Active server names]
    Reg --> Val{🔍 Capability Validator}
    Skill[📦 requires_servers: filesystem] --> Val
    Val -->|name present| OK[✅ satisfied]
    Val -->|name absent| Miss[❌ missing server]
    ...
    class MCP,Skill input
    class Reg,Val process
    class OK success
    class Miss error
```
...
See [Skill Fallback](/features/skill-fallback) for the full reference.
...
<CardGroup cols={2}>
<Card title="Skill Fallback" icon="life-ring" href="/features/skill-fallback">
...
<Card title="Graceful Skill Fallback" icon="shield-half" href="/features/skill-fallback">
