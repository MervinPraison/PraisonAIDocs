...
Interactive approval is now the **default** for dangerous built-in tools — no configuration needed. When an agent calls a sensitive or external tool, PraisonAI pauses and asks you before it runs. Your choices can be saved as project rules for next time. See [Approval](/features/approval) for the full list of gated tools and bypass options, and [Approval Backends → Which class runs the prompt?](/features/approval-backends) for how the Python (`ConsoleBackend`) and CLI (`InteractiveCLIApprovalBackend`) paths differ.
...
- A repeated tool-call loop is detected — the interactive backend is asked (continue / stop) as a `doom_loop` decision on both sync (`chat()` / `start()`) and async (`achat()` / `astart()`) paths ([PR #4858](https://github.com/MervinPraison/PraisonAI/pull/4858) restored async parity); see [Approval → Doom-loop safety gate](/features/approval#doom-loop-safety-gate)
