...
<Note>
When using `PermissionManager.approve()` programmatically, pass `reusable_scope=True` (with `scope="session"` or `scope="always"`) to store a command-prefix glob instead of the literal command — so `bash:git status *` auto-approves every `git status` variant without prompting again. See [Reusable Approval Scopes](/docs/features/reusable-approval-scopes).
</Note>
