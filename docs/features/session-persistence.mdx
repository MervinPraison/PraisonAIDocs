...
<Note>
The SQLite transcript default introduced in PraisonAI PR #3409 is **gateway-only**. The single-user code paths on this page are unaffected — a plain `Agent` script keeps using the default JSON store. See [SQLite Transcript Store](/features/sqlite-transcript-store) for the gateway change.
</Note>
...
<Note>
**Works for AgentTeam / YAML runs too.** `praisonai run agents.yaml --continue` / `--session <id>` resumes a whole team — per-agent chat history and shared team state replay before the new prompt. See [YAML / Team Session Continuity](/features/yaml-session-continuity).
</Note>
...
<Note>
Assistant tool-call turns and `role="tool"` result turns are round-tripped by the default JSON store — a resumed model sees the same tool history it produced before. File-format compatibility is preserved: old text-only files load unchanged. See [Session Resume](/persistence/session-resume) for the full contract.
</Note>
...
<Note>
With `execution=ExecutionConfig(context_compaction=True)` and a bound `session_id`, resume automatically uses the compacted working history. Sessions without a checkpoint resume from raw messages exactly as before. See [Compacted Session Resume](/features/session-compaction-checkpoint).
</Note>
...
| `agent_histories` | `dict` | Populated by `Session._save_agent_chat_histories()` when `session.Agent(name, role=...)` sub-agents exist — maps `agent_key → [messages]` (PraisonAI PR #4126). See [Sessions → Multi-Agent Sessions](/features/sessions#multi-agent-sessions). |
...
On a durable-write failure the store spills **just that turn** to an atomic fallback file under `~/.praisonai/state/session_spill/`, then fires the observe-only [`SESSION_PERSIST_FAILED`](/features/hook-events#session-persist-failed-a-durable-write-failed) hook so operators can alert on the failure. On the next session **load** the store scans the spill directory for this session's files and re-ingests any un-persisted turns.
...
Wrap the store in `EncryptedSessionStore` to encrypt message content and metadata before they reach disk — keys stay in `PRAISONAI_SESSION_KEY`.

```python
import os
from praisonaiagents.session import get_default_session_store, EncryptedSessionStore
...
See [Encrypted Session Store](/features/encrypted-session-store) for key generation and honest limits.
...
class MyDbAdapter:
    def on_agent_start(self, agent_name, session_id, user_id=None, metadata=None):
        # Return the resumed message list (as praisonaiagents.db.DbMessage instances).
        return []
    ...
    def on_agent_message(self, session_id, content, metadata=None):
        ...  # persist a plain assistant turn

    # Optional — implement these two methods to persist tool-call turns
    # faithfully so a resumed session hands the model the same transcript.
    # Adapters that omit them keep text-only behaviour (unchanged).
    def on_assistant_message(self, session_id, content, tool_calls=None, metadata=None):
        ...  # persist assistant turn WITH tool_calls

    def on_tool_message(self, session_id, content, tool_call_id, metadata=None):
        ...  # persist a role="tool" result linked to tool_call_id
...
<Note>
The optional `on_assistant_message` / `on_tool_message` hooks (PraisonAI PR [#5082](https://github.com/MervinPraison/PraisonAI/pull/5082)) persist assistant `tool_calls` and `role="tool"` results so a resumed DB session replays the same tool transcript. See [Custom DB Adapter](/features/database-persistence#custom-db-adapter).
</Note>
...
Compacted (archived) turns stay **searchable**. Both built-in session stores (`DefaultSessionStore`, `SqliteSessionStore`) and the exported `SqliteTranscriptStore` scan a shared **archived + active** projection, so a token that survives only in `archived_messages` after compaction is still recallable via `session_search` and the `praisonai session search` CLI. Every returned context message carries an `archived: bool` flag so the compaction boundary stays visible. See [Cross-Session Recall → Recall Across Compacted History](/features/cross-session-recall#recall-across-compacted-history).
...
<Card title="Hook Events" icon="webhook" href="/features/hook-events#session-persist-failed-a-durable-write-failed">
