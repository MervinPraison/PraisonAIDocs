...
| `update_session_metadata(session_id, **fields)` | Merge run stats / metadata fields (e.g. `model`, `total_tokens`, `cost`, `agent_id`, `source`) into a persisted session. Safe to call concurrently across processes. Skips `None` values. Returns `True` on success. |
| `set_agent_info(session_id, agent_name=None, user_id=None)` | Set or update agent info on a session. |
