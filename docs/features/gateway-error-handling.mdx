...
The `hello` handshake returns machine-readable `ConnectErrorCode` values in `hello_error` responses, replacing free-text errors with structured codes you can branch on:

| Code | Meaning | `next_action` hint |
|------|---------|-------------------|
| `auth_required` | Authentication missing | — |
| `auth_unauthorized` | Credentials invalid or session mismatch | `start_new_session` |
| `protocol_unsupported` | Client/server version mismatch | `upgrade_client` or `use_older_client` |
| `pairing_required` | Device must be paired first | `pair_device` |
| `agent_not_found` | Requested `agent_id` not found | `check_agent_id` |

Unlike the free-text error path below (which handles runtime bot exceptions), `hello_error` codes are returned before any agent communication starts — during the WebSocket connection setup. See [Handshake Protocol](/docs/features/gateway-handshake-protocol) for full details.
