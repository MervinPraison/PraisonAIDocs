...
<Note>
`Judge` and `BaseLLMGrader` now match this same fail-closed pattern: an unparseable verdict returns `score=1.0, passed=False` instead of a fabricated middle score. See [LLM as Judge — Fail-Closed Parsing](/features/llm-as-judge#fail-closed-parsing).
</Note>
