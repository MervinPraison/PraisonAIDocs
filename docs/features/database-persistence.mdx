...
- The auto ID **cannot** resume a conversation across process restarts — a new instance gets a new suffix, hence a new session. For resumable multi-turn conversations, pass an explicit `session_id`. See [Session Persistence](/docs/features/session-persistence).
</Warning>
