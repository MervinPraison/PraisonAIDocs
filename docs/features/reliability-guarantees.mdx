...
<Note>
**Behaviour changes (PraisonAI PR #5121).** Two additional `chat_mixin.py` execution paths now surface the same retryable `role="tool"` error on a malformed tool-call `arguments` JSON, instead of silently dispatching with `{}` and reporting success:

- Async OpenAI-native tool-completion path (`_achat_completion`)
- Sync OpenAI-native streaming path (`_start_stream_impl`)

`TypeError` on a non-text arguments value is now treated the same as malformed JSON (mirrors `llm.py`'s parser).
</Note>
...
All seven execution paths — five in `llm.py` (sync sequential, async sequential, streaming batch, non-streaming batch, and async Responses-API loop) plus two in `chat_mixin.py` (async OpenAI-native tool-completion, sync OpenAI-native streaming) — now surface a `role="tool"` error message when a tool call's `arguments` JSON cannot be parsed, and skip the dispatch instead of calling the tool with wrong arguments.
...
|---|---|
| Sync sequential (`llm.py`) | Reports `role="tool"` error, skips dispatch |
| Async sequential (`llm.py`) | Reports `role="tool"` error, skips dispatch |
| Streaming batch (`llm.py::get_response_stream`) | Reports `role="tool"` error, skips dispatch |
| Non-streaming batch (`llm.py::get_response`) | Reports `role="tool"` error, skips dispatch |
| Async Responses-API loop (`llm.py::get_response_async`) | Reports `role="tool"` error, skips dispatch |
| **Async OpenAI-native tool-completion (`chat_mixin.py::_achat_completion`)** | **Reports `role="tool"` error, skips dispatch (PR #5121)** |
| **Sync OpenAI-native streaming (`chat_mixin.py::_start_stream_impl`)** | **Reports `role="tool"` error, skips dispatch (PR #5121)** |

The sentinel `_TOOL_ARGUMENTS_PARSE_FAILED` and the helpers `tool_arguments_parse_failed` / `tool_parse_error_message` are defined in `agent/tool_execution.py` and reused by `llm/llm.py` via a shared import, so the sentinel is a single object identity across both dispatch layers (an `is` check only works against the same object).
