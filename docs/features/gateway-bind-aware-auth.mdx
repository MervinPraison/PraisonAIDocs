...
<Warning>
Loopback requests to a loopback-bound gateway also receive **all operator scopes** by default. This is intended for local development — see [Operator Scopes](/features/gateway-operator-scopes).
</Warning>
...
<Step title="Option C — let doctor repair it (fastest if gateway.yaml is open)">
```bash
praisonai gateway doctor --config gateway.yaml --fix --dry-run   # preview
praisonai gateway doctor --config gateway.yaml --fix             # apply
```
`--fix` mints a strong token, persists it to `~/.praisonai/.env`, and rewrites an explicit weak `auth_token` in `gateway.yaml` in place (`${ENV}` references are preserved). See [Gateway CLI › Auto-repair](/docs/features/gateway-cli#auto-repair-with-fix).
</Step>
</Steps>
...
<Card title="Gateway Documentation" icon="gateway" href="/gateway">
  Core gateway functionality and configuration
</Card>
<Card title="Onboarding" icon="rocket" href="/features/onboard">
...
<Card title="Chat Interface" icon="comments" href="/features/chat">
