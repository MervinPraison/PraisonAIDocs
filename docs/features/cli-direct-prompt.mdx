...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Direct Prompt
...
<Note>
When to use `praisonai run` instead: the bare form is a shortcut for simple one-off prompts. Use `praisonai run` when you need flags such as `--output stream-json`, `--model`, `--continue`, `--session`, `--no-rules`, `--allow`, etc. See [praisonai run](/cli/run) for the full flag reference.
</Note>
...
<CardGroup cols={2}>
  <Card title="praisonai run" icon="play" href="/cli/run">
