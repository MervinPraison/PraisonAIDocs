...
sidebarTitle: "Context Window Management"
...
graph LR
    subgraph "Context Window Management"
        Request[📋 User Request] --> Process[⚙️ Context Window Management]
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Context Window Management
...
sequenceDiagram
    ...
    User->>Agent: Large input
    Agent->>ContextManager: Analyze + count tokens
    ContextManager->>ContextManager: Truncate / chunk / summarize
    ContextManager-->>LLM: Optimized context
    LLM-->>User: Response within window
```

## Local models — window comes from the server

Since [PR #4924](https://github.com/MervinPraison/PraisonAI/pull/4924), when PraisonAI has already probed a locally-served model it uses **the window the server reported**, not the litellm default.

```python
from praisonaiagents import Agent

# Ollama's qwen3:0.6b reports a 40960-token window via /api/show
agent = Agent(llm="ollama/qwen3:0.6b", instructions="Summarize")
agent.start("...long document...")
# Compaction now targets 40960, not the generic 128000 default.
```

Before the fix, a 4096-context local model was budgeted at 128000 tokens — the server silently dropped the head of the conversation and returned an answer built from the tail alone.

| Model | Before | After (server reports) |
|---|---|---|
| `ollama/qwen3:0.6b` | 128000 | **40960** |
| `gpt-4o` (hosted) | 128000 | 128000 (unchanged) |
| Any local model whose `/api/show` reports a shorter window | litellm default | **server value** |

Hosted providers are untouched: `gpt-4o` still resolves through litellm.

```mermaid
sequenceDiagram
    participant Agent
    participant Budgeter as get_model_limit
    participant Local as local layer (probed)
    participant LiteLLM as litellm registry

    Agent->>Budgeter: get_model_limit("ollama/qwen3:0.6b")
    Budgeter->>Local: context_length_for(...)
    Local-->>Budgeter: 40960 (from /api/show)
    Budgeter-->>Agent: 40960
    Note over Agent,LiteLLM: litellm is only consulted<br/>when the local cache misses
    ...
    class Agent agent
    class Budgeter,Local process
    class LiteLLM output
```
...
<CardGroup cols={2}>
  <Card icon="layers" href="/features/context-management">
    Control what context is assembled and passed into each agent turn.
  </Card>
  <Card icon="gauge" href="/features/model-capabilities">
