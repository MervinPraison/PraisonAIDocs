...
Windows: the piped-read path is skipped, because Python's `select.select` is socket-only on Windows. Attach files instead with `--file` on `praisonai code` / `praisonai chat`:
