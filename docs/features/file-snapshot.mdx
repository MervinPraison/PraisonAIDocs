...
|--------|------|---------|-------------|
| `project_path` | `str` | `"."` | Path to the workspace to snapshot |
| `track_changes` | `bool` | `False` | Auto-snapshot before each agent tool call |
| `message` | `str` | `""` | Label for the snapshot commit |
| `files` | `list[str]` | `None` | Restrict restore to specific file paths |
