...
### Automatic on gateway boot (new — PR #3885)

If you run `praisonai gateway start`, `recover_orphans()` is now called for you and each `LOST`-marked run's origin channel / thread receives a durable "your run was interrupted" notice — see [Gateway Restart Continuation › Run-level recovery](/features/gateway-restart-continuation#run-level-recovery-new-pr-3885). The manual patterns below remain the way to wire recovery into a custom (non-gateway) process.

<Info>
**Default-safe.** Boot recovery is a no-op when no ledger `.db` exists — gateways that never used the ledger are unaffected. The notice is routed durably through the SQLite `scheduled_outbox` under the stable dedup key `run-recovery:{run_id}`, so a boot-time send to a not-yet-ready channel drains on the next boot instead of being lost.
</Info>
