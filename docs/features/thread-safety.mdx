...
sidebarTitle: "Thread-Safe Agent State"
...
#### What changed in PR #5121 — OpenAI-native path TOCTOU parity

<Note>
**Fix reference:** [PraisonAI PR #5121](https://github.com/MervinPraison/PraisonAI/pull/5121).
</Note>

PR #3769 closed the read-then-append TOCTOU on the **custom-LLM** branches only. The OpenAI-native sync `_chat_impl` and async `_achat_impl` paths were still on the unsafe pattern: a concurrent turn's user message that landed at `chat_history[-1]` between the read and the append was mistaken for this turn's own duplicate, **silently dropping this turn's user message** from the record.

PR #5121 routes both OpenAI-native paths through the same atomic `_add_to_chat_history_if_not_duplicate("user", …)` helper. Concurrent same-prompt turns on the OpenAI-native path no longer drop the user message.

All four dispatch branches now share the atomic helper:

| Dispatch path | Atomic helper |
|---|---|
| Sync custom-LLM | ✅ (PR #3769) |
| Async custom-LLM | ✅ (PR #3769) |
| Sync OpenAI-native (`_chat_impl`) | ✅ (PR #5121) |
| Async OpenAI-native (`_achat_impl`) | ✅ (PR #5121) |

<Note>
**Persistence ordering.** The sync path calls `_persist_message("user", normalized_content)` only when the helper actually appended (not a duplicate); the async native path calls the helper unconditionally, and the persistence line at that point is unchanged in PR #5121.
</Note>
...
Telemetry counting is thread-safe as of PraisonAI [#4235](https://github.com/MervinPraison/PraisonAI/pull/4235): the re-entrancy guard is a per-agent `contextvars.ContextVar`, so concurrent `chat()`/`run()`/`start()` calls on one shared agent are each counted independently. See [Telemetry — Concurrent-execution accuracy](/features/telemetry#concurrent-execution-accuracy).
...
| Handoff history seeding | per-target-agent `DualLock` attached lazily as `agent._handoff_seed_lock` | Serialises seed + chat per target agent so overlapping handoffs to one specialist can't interleave `chat_history` — see [Handoff seeding](#handoff-history-seeding) |
...
| `praisonaiagents.output.status._output_lock` | `threading.Lock` | Guards the status singleton's timing dicts, `_ai_call_count`, and session token/cost accumulators — see [Output singleton lock](#output-singleton-tracestatus-lock-pr-5219) |
| `praisonaiagents.output.trace._output_lock` | `threading.Lock` | Guards the trace singleton's `_llm_start_time` / `_tool_start_times` — see [Output singleton lock](#output-singleton-tracestatus-lock-pr-5219) |
...
<Warning>
**Behaviour change in PR #1709**: `praisonaiagents.session.store.FileLock.__enter__` now raises `IOError` instead of returning silently on lock-acquisition timeout. If you have custom code that uses `FileLock` directly from `praisonaiagents.session.store`, wrap it in a `try/except IOError` or raise the timeout via `DefaultSessionStore(lock_timeout=...)`.
</Warning>

<Note>
There are **two distinct `FileLock` classes** with different histories — do not conflate them:

| Class | Used by | Timeout behaviour |
|---|---|---|
| `praisonaiagents.session.store.FileLock` | `DefaultSessionStore` (above) | Raises `IOError` on acquisition timeout (PR #1709) |
| `praisonaiagents.storage.base.FileLock` | `BaseJSONStore` and downstream learn stores | mtime-based staleness + heartbeat (PR #5136, below) |
</Note>

### storage.base.FileLock — mtime staleness + heartbeat

`praisonaiagents.storage.base.FileLock` backs `BaseJSONStore` and every JSON-backed store built on it (including the [learned-memory store](/features/learn)). It judges staleness by the lock **file's** mtime age, not by how long any individual waiter has queued.

| Rule | Behaviour |
|---|---|
| Staleness measure | `time.time() - lock_path.stat().st_mtime` — the lock file's own age |
| Heartbeat | A daemon thread started in `__enter__` refreshes the lock file's mtime every `max(timeout/3, 0.001)` seconds via `os.utime`; `__exit__` stops it |
| Reclaim | Only a truly abandoned lock (holder crashed without running `__exit__` or heartbeating) ages past `timeout` and is reclaimed |

This closes a corruption bug: previously, staleness was measured as `time.time() - start`, where `start` was when *this waiter* began waiting. A live holder legitimately in a long critical section (large JSON write, slow disk) had its lock file `unlink()`ed out from under it as soon as one waiter's `timeout` elapsed — admitting two writers into the same read-modify-write save concurrently and silently clobbering one another's writes. The mtime + heartbeat scheme keeps a long-held lock exclusive. Added in [PR #5136](https://github.com/MervinPraison/PraisonAI/pull/5136).
...
That closes the multi-gateway lost-append window the process-local lock alone could not: a second gateway process cannot slip a read between this one's read and its write. Gateway deployments with `session.persist: true` use this path by default — see [SQLite Transcript Store](/features/sqlite-transcript-store).
...
PraisonAI's launch server is now fully thread-safe **and** unified across `Agent.launch()`, `AgentTeam.launch()`, and `PraisonAIAgents.launch()`. All three share one process-wide `_server_registry` in `praisonaiagents/agent/server_registry.py`, so mixing them on the same port is safe.
...
- A bind failure (`Address already in use`, missing FastAPI/uvicorn) clears the port's `_started` flag so a later `.launch()` call re-attempts the bind. The launcher returns the true bound state, so callers can distinguish a live server from a poisoned port.
...
<Note>
**Fix reference:** [PraisonAI PR #5204](https://github.com/MervinPraison/PraisonAI/pull/5204) unified the launch registry across `Agent.launch()` and `AgentTeam.launch()` and replaced `Agent.launch()`'s heuristic sleeps with the shared readiness `Event`.
</Note>
...
**`PraisonAIDB` background writes** (PR [#4861](https://github.com/MervinPraison/PraisonAI/pull/4861)) — fire-and-forget write tracking (`_bg_writes` / `_bg_writes_lock`) is now **per-instance**, not a process-global set. Each adapter's `close()` / `aclose()` / `flush_pending_writes()` waits only on its own in-flight writes. Previously a single process-global `_BG_WRITES` was shared across every `PraisonAIDB` instance, so one tenant's stuck backend could consume another tenant's 5-second flush budget — or silently discard the caller's own writes when it timed out. The per-instance lock also removes unnecessary serialisation across concurrent multi-tenant `close()` calls. See [Async DB Hooks](/features/async-db-hooks).

The adapter's async init/close is also **cross-loop safe**: a shared `PraisonAIDB` no longer caches an event-loop-bound `asyncio.Lock` on the instance, so its `_ainit_stores` / `aclose` can be awaited from any per-session loop (serialisation runs off-loop via `asyncio.to_thread` under the sync `threading.Lock`) without `RuntimeError: bound to a different event loop`. See [Per-Session Scoped Bridge](/features/async-bridge#per-session-scoped-bridge).
...
<Warning>
**Breaking change (PR #2248):** The four AutoGen-specific methods that were added in PR #1780 (`register_autogen_adapter`, `get_autogen_adapter`, `list_autogen_adapters`, `register_builtin_autogen_adapters`) have been **removed** from `ToolRegistry`. Migrate to `AutoGenAdapter` from `praisonai.framework_adapters` or use `praisonai.persistence.factory` for persistence. See [ToolRegistry AutoGen Migration](/features/tool-registry-autogen-migration) for replacement patterns.
</Warning>
...
### PluginManager (PR #5204)

`PluginManager`'s "thread-safe for concurrent access" contract is now fully honoured. Every method that iterates or mutates `_plugins` / `_enabled` acquires the same `RLock`:

| Method | Lock behaviour |
|---|---|
| `register()` | already held the lock — unchanged |
| `unregister()` | now holds the lock while mutating; plugin `on_shutdown()` runs outside |
| `enable()` / `disable()` | now hold the lock while mutating |
| `execute_hook()` | snapshots `(name, plugin)` pairs under the lock, then dispatches outside |

Concurrent `register()` / `unregister()` on one thread and `execute_hook()` on another can no longer raise `RuntimeError: dictionary changed size during iteration`. Because `_lock` is an `RLock`, a plugin whose `on_shutdown()` re-enters the manager (e.g. via `shutdown()`) is safe.

```mermaid
graph LR
    T1[register / unregister thread] --> L[🔒 RLock]
    T2[execute_hook thread] --> L
    L --> SNAP[📸 snapshot enabled pairs]
    SNAP --> D[▶️ dispatch outside lock]
    D --> OK[✅ no dictionary-changed-size error]

    classDef caller fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef lock fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef snap fill:#189AB4,stroke:#7C90A0,color:#fff
...
Fix reference: [PraisonAI PR #5204](https://github.com/MervinPraison/PraisonAI/pull/5204).
...
### Handoff history seeding

A handoff seeds the filtered conversation onto the **target** agent's `chat_history` for the duration of the transfer, then restores it. Because that history is shared mutable state on the target, overlapping handoffs to the *same* target agent (e.g. two `parallel_handoffs` entries pointing at one specialist) would interleave their seed-and-restore steps and leak one caller's context into another's.

Seeding is now serialised per target agent by a `DualLock` attached lazily as `agent._handoff_seed_lock`:

- **Sync path** holds the lock's threading (`RLock`) side around apply-seed → `chat()` → restore.
- **Async path** holds the lock's per-loop `asyncio.Lock` side **and** the sync side for the whole seeded `achat()` turn. The async side alone would not exclude a concurrent *cross-thread sync* handoff to the same target, so both sides are held together.

If a target agent cannot hold the attribute (e.g. `__slots__`), a transient lock is used and behaviour falls back to the pre-lock path — the common case stays fully guarded. No public API changed; the guarantee is automatic. See [Agent Handoffs](/features/handoffs#concurrent-handoffs-to-the-same-target) and [Multi-Agent Context Safety](/features/multi-agent-context-safety).
...
Each gathered task rebinds `_handoff_chain_var` to a fresh per-task copy on entry (`_handoff_chain_var.set(list(_handoff_chain_var.get() or []))`), which — on top of the existing copy-on-write push/pop — keeps sibling handoffs fully isolated. See [parallel_handoffs reference](/sdk/reference/praisonaiagents/functions/parallel_handoffs) for parameters.
...
### Output singleton (trace/status) lock (PR #5219)

`output="status"` and `output="trace"` route through a shared per-process singleton in `praisonaiagents.output.status` / `praisonaiagents.output.trace`. Both modules declared a `_output_lock` that, before this fix, was **never acquired**.

<Note>
**Fix reference:** [PraisonAI PR #5219](https://github.com/MervinPraison/PraisonAI/pull/5219), fixes [#5162](https://github.com/MervinPraison/PraisonAI/issues/5162).
</Note>

The lock is now actually acquired around every read-modify-write on the singleton's shared state:

- the timing dicts `_llm_start_time`, `_tool_start_times`, and `_agent_start_times`,
- the call counter `_ai_call_count`,
- the session accumulators `_session_tokens_in`, `_session_tokens_out`, `_session_cost`, and `_session_llm_calls`.

Concrete before/after: an `asyncio.gather` over two `agent.achat(...)`s used to let one agent's `llm_end` compute its duration from the *other* agent's `_llm_start_time` (or crash on `None` once the first `llm_end` reset it), and the session token summary silently under-counted because `+=` on shared counters raced. After the fix, both the per-call timings routed through the singleton and the aggregate token/cost totals stay consistent under concurrency.

```python
import asyncio
from praisonaiagents import Agent

# Distinct names → accurate per-agent status/trace attribution (see the note below).
writer = Agent(name="Writer", instructions="Write a blurb.", output="status")
summarizer = Agent(name="Summarizer", instructions="Summarise notes.", output="status")

async def main():
    # The shared status singleton's metrics no longer corrupt under gather.
    await asyncio.gather(
        writer.achat("Blurb for the new dashboard"),
        summarizer.achat("Summarise v2.0 release notes"),
    )
...
<Note>
The fix removes the data-race corruption on the singleton — it does **not** remove the cross-agent aliasing when two concurrent tool calls share a tool name, or two default-named `Agent()` instances share the name `"Agent"`. `_tool_start_times` and `_agent_start_times` are still keyed by name only, so two concurrent `read_file` calls can still pop each other's timestamps. Give each agent a distinct `name=` for accurate per-agent status/trace attribution.
</Note>

Cross-links: [Output](/features/output#concurrent-execution-pr-5219) and [Output Styles](/features/output-styles#concurrent-execution-pr-5219).
...
<Card title="Output" icon="display" href="/features/output">
  output="status" / "trace" presets and their concurrency guarantees
</Card>
<Card title="Import Safety (Python)" icon="shield-check" href="/features/python-import-safety">
  Fork-safe imports and untouched host state
</Card>
</CardGroup>
