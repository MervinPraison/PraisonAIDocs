...
Executes Python code using AST-validated execution for security. Code is parsed, validated against an allowed node list, and executed in a sandboxed namespace.

| Variable | Type | Description |
|----------|------|-------------|
| `flags` | dict | Parsed CLI flags |
| `vars` | dict | Resolved workflow variables |
| `env` | dict | Filtered subset of `os.environ` — only `PRAISON*`, `HOME`, `USER`, `PATH`, with secret-like keys (`*KEY`, `*SECRET`, `*TOKEN`, `*PASSWORD`) removed. Use `vars` or `flags` to pass other values. |
| `cwd` | str | Workflow's working directory |
| `result` | — | **Set this** to produce step output |

#### What's allowed in `script:`

**Expressions and simple assignments only** — complex logic should move to a separate `python:` script step.

**Import allowlist**: `os`, `datetime`, `time`, `json`, `math`, `re`, `collections`, `itertools`

**Forbidden**: `__import__`, `exec`, `eval`, `open`, `compile`, `setattr`, loops (`for`, `while`), function definitions (`def`), class definitions (`class`), exception handling (`try/except`), f-strings, list comprehensions

**Error types you may see**:
- `Syntax error: ...` — Invalid Python syntax
- `Disallowed code node: ...` — Code uses forbidden constructs
- `Import of 'module' not allowed.` — Import not in allowlist
- `Evaluation error: ...` — Runtime execution error

**Examples**:

```yaml
# ✅ Allowed - Simple expression (auto-returns value)
- name: Compute date
  script: |
    import datetime
    datetime.date.today().isoformat()

# ✅ Allowed - Simple assignment
- name: Set version
  script: |
    import datetime
    result = "v" + datetime.date.today().isoformat()

# ❌ Not allowed - f-strings
- name: Wrong way
  script: |
    import datetime
    result = f"v{datetime.date.today().isoformat()}"
