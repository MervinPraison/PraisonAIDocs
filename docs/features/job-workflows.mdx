...
agent = Agent(name="job-agent", instructions="Define and run job workflows.")
...
sequenceDiagram
    ...
    U->>CLI: praisonai workflow run deploy.yaml
    CLI->>J: load job pipeline
    J->>S: run shell step (uv build)
    S-->>J: stdout / exit code
    J->>A: run agent step (generate changelog)
    A-->>J: AI output
    J->>J: judge / approve gate
    J-->>CLI: all steps complete
    CLI-->>U: ✅ Workflow completed
```

```mermaid
graph LR
    Y[deploy.yaml] --> E[JobWorkflowExecutor]
    E --> S[Shell run]
    E --> A[Agent step]
    E --> J[Judge / Approve]
...
Add agent, judge, and approve steps — see [Agent-Centric Steps](#agent-centric-steps) below.
...
```yaml
# ✅ Works - Using available builtins
- name: Compute sum
  script: |
    result = str(sum(range(1, 11)))  # "55"

# ✅ Works - Using vars/flags data
- name: Process data
  script: |
    result = "Version: " + vars.get('version', 'unknown')

# ❌ Won't work - Imports not available
- name: Wrong way
  script: |
    import datetime  # NameError: name 'datetime' is not defined
    result = datetime.date.today().isoformat()

# ✅ Pass dates via vars instead
# In workflow.yml:
vars:
  today: "2024-01-15"
# In script:
...
Actions use a **3-tier resolution chain**: YAML-defined → file-based → built-in. See [Custom Actions](/features/custom-actions) for full details.
...
| `model` | inherits `default_llm` | LLM model to use. **Optional** — omit it and the step inherits the workspace default (`default_llm`). Auto-generated job workflows no longer pin `gpt-4o-mini`; see [Auto Mode Providers → Job-workflow generation honours your configured model](/features/auto-generator-providers#job-workflow-generation-honours-your-configured-model). |
...
<Warning>
`retry` is **not supported** for judge steps. Using it raises:

```text
Unsupported on_fail value 'retry' for judge step. Supported values: 'stop', 'warn'.
```
</Warning>
...
<Accordion title="Dry-run before deploy">
...
<Accordion title="Use flags for CI vs production">
...
<CardGroup cols={2}>
  ...
  <Card title="All Systems" icon="layer-group" href="/features/execution-systems">
