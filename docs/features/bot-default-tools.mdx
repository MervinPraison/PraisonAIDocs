...
Add a local Chromium via `--browser` — see [Bot Browser Automation](/features/bot-browser-automation).

<Note>
The browser tool is **not** in the auto-inject set (opt-in only) because it requires `praisonai-browser[playwright]` and downloads Chromium.
</Note>
...
|------|--------|---------------|
| `delegate_task` | Spawns a sub-agent for a specific task — medium-risk, requires approval — opt in per bot | Add to `default_tools` manually, e.g. `default_tools=[..., "delegate_task"]`. See [Delegate Task Tool](/features/delegate-task). |
| `session_search` | Cross-session conversation recall — opt in per bot | Add to `default_tools` manually, e.g. `default_tools=[..., "session_search"]`. See [Cross-Session Recall](/features/cross-session-recall). |
...
<Note>
Setting `channels.<platform>.allow_shell: true` on a gateway channel additionally injects `execute_command` into the agent's tool list, lifts the tool from the deny-list, and appends a one-line permission grant to `agent.instructions`. On shell-enabled channels with routing rules, routed agents inherit `execute_command` too — the shell setup is re-applied per routed agent, cloned and cached per `(channel, agent)`. On externally-bound gateways or channels with a `group_policy`, `auto_approve_shell: true` is downgraded to require approval unless `auto_approve_shell_acknowledge_exposed: true` opts in — see [Bot Shell Execution → Exposure-aware auto-approval](/features/bot-shell-execution#exposure-aware-auto-approval).
</Note>
