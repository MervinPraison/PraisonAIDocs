...
On Telegram this is now a **performance / clarity** recommendation, not a correctness one — long values still work, they simply take a store round-trip via the [callback payload store](/docs/features/interactive-callback-store). Telegram's 64-byte callback cap is measured in UTF-8 bytes, so compact IDs (e.g. `"approve:42"`) stay inline and skip the extra hop.
</Accordion>
