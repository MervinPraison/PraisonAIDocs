...
<Note>
`context=` produces a smaller contextual line under the prompt. On channels that don't render `reply` natively (e.g. Telegram), the reply degrades to `callback:reply:<value>` — or `reply:#<sha1[:16]>` when the payload would exceed the 64-byte channel cap — so distinct choices never collide.
</Note>
