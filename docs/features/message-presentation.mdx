...
<Step title="One-line question builder">
`MessagePresentation.question(...)` is the shortest way to ask a multi-choice question — prompt, optional context, and option buttons in a single call:
...
presentation = MessagePresentation.question(
    ...
    options=[("🧪 Staging", "staging"), ("🚀 Production", "production"), "cancel"],
    context="Deployments run immediately.",
)
```

Each option becomes a `reply`-action button — a tap sends the chosen value back as the user's next message, so no callback handler is needed.
</Step>
...
`MessagePresentation.question(prompt, options, context=None)` builds a prompt-plus-options presentation in a single call — the symmetric counterpart to `MessagePresentation.approval(...)` for non-binary "which of these?" clarifications.
...
It composes a text prompt → optional context → a `quick_replies` button row. Every option carries `PresentationAction.reply(value)`, so a tap feeds the chosen `value` straight into the next agent turn — no callback handler required.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `prompt` | `str` | *required* | The question text, rendered as the first text block. |
| `options` | `List[Any]` | *required* | Choices as `(label, value)` tuples or plain strings (used as both label and value). Each becomes a `reply`-action button. |
| `context` | `Optional[str]` | `None` | Optional smaller context line rendered under the prompt via `make_context`. |

<Note>
`context=` produces a smaller contextual line under the prompt. On channels that don't render `reply` natively (e.g. Telegram), the reply degrades to `callback:reply:<value>`. When the payload would exceed the 64-byte channel cap the framework persists the value under a short reference and emits `reply:@<ref>`; the registry resolves it back to the exact value on click. Without a store (custom adapters that opt out) it falls back to `reply:#<sha1[:16]>` — distinct choices stay distinct but the original value cannot be recovered.
</Note>
...
```mermaid
graph TB
    Q{What are you asking the user?}
    Q -->|Allow / deny a tool| A["MessagePresentation.approval(...)"]
    Q -->|Pick one of N options| P["MessagePresentation.question(...)"]
    Q -->|Complex layout with dividers,<br/>mixed action types, etc.| M["Compose blocks manually<br/>(make_text + quick_replies + …)"]
...
sequenceDiagram
    ...
    Agent->>Bot: reply + MessagePresentation.question("Which env?", [...])
    Bot->>Channel: prompt + native option buttons
    Channel->>User: "Which env?  [Staging] [Production] [cancel]"
    User->>Channel: tap Staging
    Channel->>Bot: reply action value = "staging"
    Bot-->>Agent: next turn input = "staging"
```
...
| `table` | `PresentationBlock.make_table(columns, rows)` | Tabular data — native table or markdown degrade ([Table & Chart Blocks](/features/table-chart-blocks)) |
| `chart` | `PresentationBlock.make_chart(chart_kind, series)` | Chart/visualisation — native chart or text-summary degrade ([Table & Chart Blocks](/features/table-chart-blocks)) |
| `poll` | `PresentationBlock.make_poll(question, options)` | Group vote — native poll or question + reply-buttons degrade ([Poll Blocks](/features/poll-blocks)) |

<Note>
`MessagePresentation.question(prompt, options, context=None)` is the one-line shortcut for "prompt + optional context + options" — it composes the `text`, `context`, and `quick_replies` blocks above for you.
</Note>
...
| WhatsApp | ✅ Native interactive (reply buttons + list messages) | Delivered via Cloud API `interactive` payloads (`button` / `list`) — see [Bot Presentations](/features/bot-presentations) |
...
<Accordion title="Use MessagePresentation.question(...) for a one-line clarification">
Reach for `question(...)` when you want a prompt, optional context, and options in a single call. Compose blocks manually (`make_text` + `quick_replies` + …) only when you also need dividers, extra text blocks, or mixed action types.
</Accordion>

<Accordion title="Keep callback values short and ASCII">
On Telegram this is now a **performance / clarity** recommendation, not a correctness one — long values still work, they simply take a store round-trip via the [callback payload store](/features/interactive-callback-store). Telegram's 64-byte callback cap is measured in UTF-8 bytes, so compact IDs (e.g. `"approve:42"`) stay inline and skip the extra hop.
</Accordion>
...
<CardGroup cols={2}>
<Card title="Approval Secure Backend" icon="shield" href="/features/approval-secure-backend">
  Durable approval buttons with actor authorisation
</Card>
<Card title="Bot Presentations" icon="layout-grid" href="/features/bot-presentations">
  Overview of bot presentation capabilities
</Card>
<Card title="Interactive Bot Actions" icon="mouse-pointer" href="/features/interactive-bot-actions">
...
<Card title="Table & Chart Blocks" icon="table" href="/features/table-chart-blocks">
