...
```bash
# Agent training only (lightweight — just needs praisonaiagents)
...
|---------|----------|-------------|
| `pip install praisonai-train` | `praisonai-train agents/list/show/apply`, no ML deps | Fastest way to try LLM-as-Judge / human-feedback agent training |
...
| `praisonai-train` | praisonaiagents | ~5MB | Iterative agent training (no ML deps) |
