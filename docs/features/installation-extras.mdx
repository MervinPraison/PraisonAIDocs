...
<Note>
**Lean install without `litellm`?** Capability detection (structured outputs, function calling, prompt caching, web search) still works via a built-in static heuristic. See [Lean Install (litellm-free)](/features/lean-install-capabilities) for the exact coverage matrix and when to add `pip install litellm`.
</Note>
...
- `a2a-sdk[http-server]>=1.0.0` — Langflow transitive dep pinned for Python 3.13 (older versions ship without `a2a.server.active_task`)
- `fastapi-pagination>=0.12.34` — pinned for FastAPI/Starlette compatibility (older versions fail with `cannot import name 'EventSourceResponse'`)

<Warning>
**`[flow]` and `[claw]` are mutually exclusive.** `[flow]` pins `a2a-sdk[http-server]>=1.0.0` (required by Langflow), while `[claw]` — via `praisonaiagents[a2ui]` — pins `a2a-sdk>=0.3.0,<1.0.0` (required by the Google A2UI SDK). Install these two heavy extras in **separate virtual environments**. `pip install "praisonai[flow,claw]"` or `pip install "praisonai[all]"` on top of `[flow]` will resolve to one version and break the other feature at runtime.
</Warning>

<Note>
**Python 3.13 is supported** as of PraisonAI (post-#5103). First-boot on Windows takes ~5 minutes because Langflow builds its migration/cache database — this is Langflow behaviour, not a hang.
</Note>
...
See [Flow CLI](/cli/flow) and [Visual Workflow Builder](/ui/flow).
...
|---------|----------|-------------|
| `pip install praisonai-train` | `praisonai-train agents/list/show/apply/benchmark`, no CUDA/Unsloth; litellm included for LLM-as-Judge grading | Fastest way to try LLM-as-Judge / human-feedback agent training, or [benchmark deployments](/features/praisonai-train-benchmark) before you fine-tune |
...
The bundled `setup_conda_env.sh` (used by `praisonai-train[llm]`) now probes the host for a GPU **before** picking `pytorch-cuda=12.4`. Nvidia hosts get the CUDA build; AMD hosts get a WARNING pointing at the [official PyTorch ROCm instructions](https://pytorch.org/get-started/locally/); CPU-only Linux hosts get a WARNING that `praisonai-train llm` refuses to run on CPU (the env is still usable for dataset work and exports). See [Train → Conda setup](/train#conda-setup-what-the-script-installs-on-your-machine) for the decision table.

See [Train](/train) and [praisonai-train Package](/features/praisonai-train-package).
...
<Accordion title="ModuleNotFoundError: No module named 'a2a.server.active_task'">

**Cause:** Old `a2a-sdk<1.0` (e.g. 0.3.26) is installed. Langflow ≥ 1.10.x imports `a2a.server.active_task`, which only exists on `a2a-sdk>=1.0`.

**Solution:**
```bash
pip install --upgrade "praisonai[flow]"
# or, manually:
pip install "a2a-sdk[http-server]>=1.0.0"
```

This is fixed automatically for fresh installs on PraisonAI ≥ (post-#5103) because the `[flow]` extra now pins `a2a-sdk[http-server]>=1.0.0`. It resurfaces only if another extra downgrades `a2a-sdk` (e.g. `[claw]` — see the mutual exclusivity warning above).

</Accordion>

<Accordion title="ImportError: cannot import name 'EventSourceResponse' from 'fastapi.responses'">

**Cause:** Old `fastapi-pagination<0.12.34` is installed against a newer FastAPI/Starlette.

**Solution:**
```bash
pip install --upgrade "praisonai[flow]"
# or, manually:
pip install "fastapi-pagination>=0.12.34"
```
...
| `[flow]` | langflow, requests, a2a-sdk[http-server]>=1.0, fastapi-pagination>=0.12.34 | ~500MB | Visual workflow builder (Langflow) |
...
<Card title="Bot Integrations" icon="robot" href="/features/bot-gateway">
