...
graph LR
    subgraph "PraisonAI Extras"
        ...
        subgraph "Features Enabled"
            ...
            F5[Visual Workflow Builder + Langflow]
            F6[Everything Included]
        end
    end
...
Launches the Langflow visual builder with PraisonAI Agent + AgentTeam components pre-loaded:
...
- `requests>=2.31.0` - HTTP client for Langflow REST API

**Use cases:**
- Launch the Langflow UI with PraisonAI Agent + AgentTeam components in the sidebar
- Import PraisonAI YAML workflows into Langflow visually
- Export Langflow flows back to YAML for version control
- Drag-and-drop multi-agent orchestration without writing code

**Example:**
```bash
# Start the visual builder on http://localhost:7860
praisonai flow

# Import a YAML workflow
praisonai flow import my_workflow.yaml --open

# Export a flow back to YAML
praisonai flow export <flow_id> -o my_workflow.yaml
```

<Warning>
The `[flow]` extra installs Langflow (~500MB). It is opt-in only — not included in `[all]` — to keep standard installations lightweight.
</Warning>
...
This happens when running `praisonai flow` without installing the `[flow]` extra.
