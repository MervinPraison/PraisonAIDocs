...
|-------|------|---------|-------------|
| `platform` | `str` | `null` | Platform name (`telegram`, `discord`, `slack`, `whatsapp`, `email`, `agentmail`). **Required when the channel key is not a platform name** (e.g. `telegram_cfo`). When omitted, the channel key itself must be a valid platform name. |
| `token` | `str` | `""` | Bot token. Supports `${ENV_VAR}` interpolation. |
| `mode` | `str` | `"poll"` | Transport mode: `poll`, `ws`, `webhook`, or `hybrid`. |
| `group_policy` | `str` | `"respond_all"` | Group chat behaviour: `respond_all`, `mention_only`, or `command_only`. |
| `allowlist` | `list[str]` | `[]` | User IDs / handles allowed to message this channel. |
| `blocklist` | `list[str]` | `[]` | User IDs / handles blocked from this channel. |
| `allowed_users` | `str` | `null` | Comma-separated allowed users (typically `${ENV_VAR}`). Use this instead of `allowlist` when the list lives in an env var. |
| `routes` | `dict[str, str]` | `{}` | Context-to-agent routing — keys are `dm` / `group` / `channel` / `default`, values are agent IDs from the `agents:` block. |
| `webhook_url` | `str` | `null` | Public webhook URL (when `mode: webhook`). |
| `webhook_port` | `int` | `8080` | Local port for inbound webhooks. |

<Info>
The channel **key** under `channels:` is just an identifier — you can use any string (e.g. `telegram_cfo`, `slack_support`). When the key is not a recognised platform name, you **must** set `platform:` explicitly. The gateway uses `platform` to pick the protocol; the key is only used for logs and routing references.
</Info>
