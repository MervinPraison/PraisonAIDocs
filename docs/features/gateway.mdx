...
<Note>
The heavy `WebSocketGateway` implementation lives in the **`praisonai_bot.gateway`** package. `praisonaiagents.gateway` holds only protocols and lightweight utilities; `from praisonai.gateway import WebSocketGateway` keeps working as a re-export shim for full-stack installs (`pip install praisonai`).
</Note>
...
| `outbound_resilience` | `object` | `None` | Retry/DLQ settings for outbound bot delivery. See [Outbound Resilience](/docs/features/outbound-resilience) for the full reference and per-channel behaviour. |
...
<Tip>
Prefer the one-line `reliability="production"` preset over tuning drain and admission separately — it sets a sensible 15 s drain, CPU-scaled concurrency ceiling, and fair queue in one step. See [Reliability Preset](/docs/features/gateway-reliability).
</Tip>
...
The gateway watches `gateway.yaml` for changes and reloads only what changed — no process restart needed. SIGHUP (`kill -HUP <pid>`) triggers an immediate reload on Linux/macOS. Install `praisonai[gateway]` to enable event-driven watching (inotify/FSEvents); polling fallback is automatic without it.

See [Config Reload](/docs/features/gateway-config-reload) for the full trigger path, drain-coordinated restart sequence, and YAML reference.

### Rate Limiting

Bound inbound request rate per identity/scope with `SlidingWindowRateLimitPolicy` (drop-in default) or a custom `RateLimitPolicyProtocol` implementation. A limited request receives a `retry_after_seconds` hint so clients back off cleanly.

See [Rate Limiting](/docs/features/gateway-rate-limiting) for the policy reference, wire format, and bring-your-own-limiter guide.
...
# Start with reliability preset (production posture)
...
<CardGroup cols={2}>
  ...
  <Card title="Rate Limit" icon="gauge-high" href="/docs/features/gateway-rate-limit">
    Bound inbound turns per identity/scope with a sliding window or custom limiter
  </Card>
  <Card title="Reliability Presets" icon="shield-check" href="/docs/features/gateway-reliability">
    One switch that turns on production-grade graceful drain + admission control
  </Card>
  ...
  <Card title="Config Reload" icon="arrows-rotate" href="/docs/features/gateway-config-reload">
    Hot-reload gateway.yaml via file watch or SIGHUP
  </Card>
  <Card title="Rate Limit Policy" icon="gauge-high" href="/docs/features/gateway-rate-limit-policy">
    Inject a custom per-tenant rate limiter into the gateway
  </Card>
</CardGroup>
