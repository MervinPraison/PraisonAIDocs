...
<Note>
Resource reads and prompt fetches share the same per-call timeout as tool calls — the value passed as `MCP(..., timeout=60)`. If a server hangs, the affected call fails with `"MCP resource call timed out after 60 seconds (server side)"` or `"MCP prompt call timed out after 60 seconds (server side)"`, and the agent's other MCP calls keep working. See [MCP Transports → Timeouts](/docs/mcp/transports#timeouts) for the layered enforcement model.
</Note>
