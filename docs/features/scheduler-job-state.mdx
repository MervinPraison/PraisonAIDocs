...
| Removal | Automatic | State is dropped on `remove(job_id)`, `remove_by_name(name)`, and any claim that retires the job via `should_retire()` (one-shot `delete_after_run`, `max_runs` budget met, or past `until`) |
...
<Note>
The state-skip contract is tied to `delete_after_run` specifically, not to `should_retire()` broadly. A job with `delete_after_run=True` is removed (and its state popped) before the run reaches the executor, so the executor never persists state for one-shots. A bounded **recurring** job — `max_runs=N` with `N > 1`, or a live `until` — *does* accumulate cross-run state between fires: it has a "next run" until the final fire, when the same claim path pops its state as it retires.
</Note>
...
`delete_after_run=True` skips persistence by design — a one-shot job has no "next run" to read the notepad. A bounded **recurring** job is different: a `max_runs=N` (`N > 1`) or live `until` job keeps its notepad between fires and only pops it on the final retiring claim. Use cross-run memory freely for those.
</Accordion>
</AccordionGroup>
