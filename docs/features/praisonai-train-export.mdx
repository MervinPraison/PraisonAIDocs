...
See [Unsloth Surface](/features/praisonai-train-unsloth-surface#expanded-quant-list) for the "which quant should I pick" decision guide.
...
<Note>
**401 / 403 translation now covers every push path.** As of PraisonAI [PR #4357](https://github.com/MervinPraison/PraisonAI/pull/4357), the same actionable 401/403 messages are raised by the LLM path, **vision training**, and `praisonai-train upload-vision` — previously only the LLM path translated them and the vision paths surfaced a raw `HfHubHTTPError` traceback. See [Hub Privacy & Upload Options](/features/praisonai-train-hub-privacy#error-translation).
</Note>

<Note>
**Plain vision runs no longer publish (PraisonAI [#4879](https://github.com/MervinPraison/PraisonAI/pull/4879)).** A local vision fine-tune that omits `huggingface_save` used to push to the Hub — the flag defaulted to the string `"true"` — or crash with `KeyError: 'hf_model_name'` after training completed. From #4879 the vision path skips every export stage unless the flag **and** its target (`hf_model_name` / `ollama_model`) are both set, matching the LLM trainer. See [Vision Fine-Tuning](/features/praisonai-train-vision).
</Note>
...
|---|---|---|---|
| `hf_private` | bool | `true` | Push as a **private** repo. Set `false` to publish publicly. |
| `save_method` | string | `"merged_16bit"` | Passed to Unsloth's `push_to_hub_merged`. |
| `commit_message` | string | — | Optional Hub commit message. Only sent when set. |
| `tags` | list[string] | — | Optional Hub repo tags. Only sent when set. |
| `imatrix_file` | str | — | Path to a precomputed importance matrix; forwarded to `push_to_hub_gguf`. Only sent when set — Unsloth auto-fetches one from `unsloth/<base>-GGUF` otherwise. |

<Warning>
Every push is now **private by default**. Add `hf_private: false` to publish publicly — see [Hub Privacy & Upload Options](/features/praisonai-train-hub-privacy).
</Warning>
...
<Card title="Unsloth Surface" icon="sliders" href="/features/praisonai-train-unsloth-surface">
...
Train on another box over SSH, then fetch and export the adapter.
</Card>
</CardGroup>
