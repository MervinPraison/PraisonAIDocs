...
sequenceDiagram
    ...
    participant Cache as ~/.praisonai/state
    participant PyPI
...
- Is backed by a time-boxed cache at `~/.praisonai/state/update_check.json` (24-hour freshness, honours `PRAISONAI_HOME`)
...
<CardGroup cols={2}>
<Card title="One-line Installer" icon="download" href="/install/installer">
