...
<Warning>
**`cli_backend` is deprecated as of PR #2072.** Use the new `runtime` parameter and [Runtime Capabilities](/docs/features/runtime-capabilities) for capability-validated, model-scoped runtime selection. Migrating is a one-line change.
