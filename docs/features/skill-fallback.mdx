...
agent = Agent(
    ...
    skills=["./skills"],
    tools=[terminal],   # web tool is NOT registered
)

agent.start("Find the latest Anthropic release notes.")
# → 'web-via-terminal' fallback is offered (web absent, terminal present)

# Register `web` instead and the fallback disappears automatically:
# agent = Agent(..., tools=[web])
# → 'web-via-terminal' stays hidden (web present → fallback not needed)
```
</Step>
...
requires_tools: [terminal]      # need terminal to actually run
fallback_for_tools: [web]       # only show when 'web' is absent
---
...
`fallback_for_*` is the **inverse** of `requires_*`. Where `requires_tools: [web]` means "I need web to run", `fallback_for_tools: [web]` means "show me only when web is absent".
...
```python
agent = Agent(tools=[web], skills=["./skills"])
# → web-via-terminal hidden (web is present)

agent = Agent(tools=[terminal], skills=["./skills"])
# → web-via-terminal offered (web absent, terminal present)
```
...
requires_tools: [terminal]              # MUST have terminal to function
fallback_for_servers: ["mcp:postgres"]  # only show when MCP postgres absent
---
```

```python
# terminal present + postgres server absent → offered
# terminal absent                           → hidden (requires_tools fails)
...
`requires_tools: [web]` — "I need web to run."  
`fallback_for_tools: [web]` — "Show me only when web is absent."
...
<Accordion title="is_empty() returns False for fallback-only skills">
A skill with only `fallback_for_*` declared (no `requires_*`) is still considered to have requirements:

```python
SkillRequirements(fallback_for_tools=["web"]).is_empty()  # False
```

This means requirement-aware code paths see it correctly.
</Accordion>

<Accordion title="A fallback that itself needs a missing tool stays hidden">
If your fallback skill declares `requires_tools: [terminal]` but `terminal` is absent, the skill stays hidden — even in non-STRICT modes. You'll never see an unusable skill offered to the agent.
</Accordion>
...
`requires_tools` hides the skill when `web` is absent; `fallback_for_tools` hides it when `web` is present. The result is always hidden.
</Accordion>
