...
`praisonai gateway start` propagates a structured exit code so process supervisors can distinguish transient failures from fatal configuration errors:

| Exit code | Constant | Meaning | Supervisor action |
|---|---|---|---|
| `0` | `GATEWAY_OK_EXIT_CODE` | Clean shutdown (including Ctrl-C) | Stop |
| `75` | `GATEWAY_RESTART_EXIT_CODE` | Transient failure (network, API blip) | Restart |
| `78` | `GATEWAY_FATAL_CONFIG_EXIT_CODE` | Fatal config error — restart will not help | Stop — fix config |

Before PR #2439, fatal config errors (missing `gateway.yaml`, invalid `--agents` file, missing `pip install praisonai[api]`) were swallowed and the CLI exited with `0`. They now correctly exit with `78`.

For systemd, add `RestartPreventExitStatus=78` to your unit file. See [Gateway Exit Codes](/docs/features/gateway-exit-codes) for systemd, Kubernetes, and s6 examples.
</Tab>
