...
| `praisonai gateway doctor` | Validate channel credentials **and route/binding targets** before start, and optionally auto-repair weak auth tokens (`--fix`) | `praisonai gateway doctor --config gateway.yaml --fix` |
...
You no longer need `--config` if you have onboarded — `praisonai gateway start` discovers the config `praisonai onboard` wrote.
...
The CLI resolves the config path once, and every `gateway` subcommand (`start`, `doctor`, `test`, `status`, `send`, `channels`) uses the same resolution — so `onboard`, `start`, and `doctor` cannot drift apart. If no config is discoverable, `start` prints a next-step hint (`praisonai onboard`, or pass `--config`/`--agents`) and exits `78` — it never silently starts with no channels.

| Precedence | Source | Notes |
|------------|--------|-------|
| 1 | `--config <path>` / `--agents <path>` | Explicit path always wins |
| 2 | `./bot.yaml` (cwd) | Local project override |
| 3 | `$PRAISONAI_BOT_CONFIG` | Env var — `PRAISONAI_HOME`-aware |
| 4 | `~/.praisonai/bot.yaml` | Where `praisonai onboard` writes |
| 5 | `./gateway.yaml` (cwd) | Backward-compatible alias — legacy name |
...
```mermaid
graph LR
    A[praisonai gateway start] --> R{--config?}
    R -->|explicit| V[_ensure_config_current]
    R -->|omitted| D[_resolve_gateway_config_path]
    D -->|discovered| V
    D -->|none| X[exit 78 — onboard hint]
    V -->|ok| P[pre-flight tools + creds]
    V -->|newer / malformed| X2[exit 78 — ConfigVersionError]
    P --> B[bind + supervise]
