...
| `praisonai gateway doctor` | Validate channel credentials **and route/binding targets** before start (pre-flight) | `praisonai gateway doctor --config gateway.yaml` |
...
The `--json` output gains a top-level `gateway_auth_token` key (present only when the verdict is weak), alongside `probes` and `secrets`:
