...
At teardown, `MCP.shutdown()` dispatches in this order: it prefers `shutdown()`, falls back to `close()`, then to `aclose()` if awaitable. Custom `MCPClientProtocol` implementations that expose only `close()` — as many `httpx`-style clients do — are now fully supported at teardown. See [MCP Lifecycle → `shutdown()`](/features/mcp-lifecycle#shutdown) and [Agent Lifecycle Cleanup](/features/agent-lifecycle-cleanup).
