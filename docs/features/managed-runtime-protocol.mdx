...
```mermaid
graph LR
    subgraph "Managed Runtime Protocol"
        Request[📋 User Request] --> Process[⚙️ Managed Runtime Protocol]
...
runtime = AnthropicManagedAgent(config=ManagedConfig(model="claude-sonnet-4-6"))
agent = Agent(name="Remote Coder", instructions="Write and run code on managed infrastructure.", llm=runtime)
agent.start("Create a hello-world script and run it.")
```
...
graph LR
    subgraph "Managed Runtime Protocol"
        In[📝 Task] --> Runtime[⚙️ Managed Runtime]
        Runtime --> Agent[🤖 Remote Agent Loop]
        Agent --> Out[✅ Result]
    end
...
<Note>
Since PraisonAI PR [#3908](https://github.com/MervinPraison/PraisonAI/pull/3908), `agent.start(prompt, stream=True)` yields chunks from `backend.stream()` as they arrive, on both async and sync callers. Earlier releases buffered the full response in the sync (no running event loop) path. See [Streaming → Managed-backend streaming](/features/streaming#managed-backend-streaming-backend).
</Note>
...
<Card title="Managed Agent Lifecycle" icon="recycle" href="/features/managed-agent-lifecycle">
  Complete CRUD operations for agents, environments, sessions
</Card>
<Card title="Streaming" icon="bolt" href="/features/streaming#managed-backend-streaming-backend">
  How `backend=` streams each chunk incrementally from sync code
</Card>
</CardGroup>