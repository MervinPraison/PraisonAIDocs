...
<Card title="ThinkingBudget API Reference" icon="code" href="/docs/sdk/praisonaiagents/thinking/budget-module">
  Full API reference for `ThinkingBudget` — all fields and class methods
</Card>

### ThinkingBudget Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_tokens` | `int` | `8000` | Maximum thinking tokens allowed |
| `max_time_seconds` | `float \| None` | `None` | Optional wall-clock time cap |
| `adaptive` | `bool` | `True` | Scale tokens with task complexity |
| `level` | `BudgetLevel \| None` | `None` | Preset level (overrides `max_tokens` if set) |
| `min_tokens` | `int` | `1000` | Minimum tokens when adaptive scaling is active |
| `complexity_multiplier` | `float` | `1.0` | Scales adaptive allocation up or down |
