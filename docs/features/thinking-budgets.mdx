...
<Note>
Prefer [`reasoning_effort`](/features/reasoning-effort) for new code — it is provider-portable and persists across sessions. `thinking_budget` remains supported as a backward-compatible alias that folds into the same control.
</Note>
...
sequenceDiagram
    ...
    User->>Agent: Complex reasoning question
    Agent->>LLM: Prompt + thinking budget (max_tokens)
    LLM->>LLM: Extended thinking (up to budget)
    LLM-->>Agent: Reasoned answer
...
<CardGroup cols={2}>
<Card title="Reasoning Effort" icon="brain" href="/features/reasoning-effort">
