...
agent = Agent(name="flex", instructions="Answer with the best model for each task.", model="gpt-4o-mini")
...
<Card title="Agent Config TypeScript Reference" icon="code" href="/sdk/reference/typescript/classes/AgentConfig">
  TypeScript agent configuration
</Card>
<Card title="Agent Rust Reference" icon="code" href="/sdk/reference/rust/classes/AgentConfig">
...
agent = Agent(name="Router", instructions="Answer questions.", model="gpt-4o-mini")
...
```python
agent = Agent(name="Writer", instructions="Write content.", model="gpt-4o-mini")
...
<CardGroup cols={2}>
<Card title="Model Failover" icon="arrow-rotate-right" href="/features/failover">
