...
Accepts any model string supported by LiteLLM (same format as the `llm=` constructor argument), including `"panel:<name>"` strings and `{"provider": "panel", ...}` dicts — see [Multi-Model Panel](/docs/features/multi-model-panel).
