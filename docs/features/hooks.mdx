...
<AccordionGroup>
  ...
  <Accordion title="before_agent / after_agent cost nothing when unregistered">
    `before_agent` and `after_agent` cost nothing when no hook is registered — the runtime checks `has_hooks()` before building the input (including `os.getcwd()` and the tools list) on both sync (`chat`) and async (`achat`) paths. Register these hooks in production without a per-turn overhead concern.
