...
agent = Agent(name="Reporter", instructions="Summarise the input.")
task = Task(
    description="Summarise 'hello'",
    agent=agent,
    callback=my_callback,
    fail_on_callback_error=True,  # re-raise instead of capturing
)
...
try:
    ...
    print("Callback blocked workflow:", e)
```
</Step>
</Steps>
...
- **Tool execution failures**: Tool failures raise `ToolExecutionError` on the sync path — it is **not** swallowed or relabelled as an LLM error (fix in PR #4252). See [Error Handling → Tool Failure Behavior](/features/error-handling#tool-failure-behavior).
...
<Card title="Structured LLM Errors" icon="triangle-exclamation" href="/features/structured-llm-errors">
