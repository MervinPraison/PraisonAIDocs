...
def alert_on_errors(task):
    if task.non_fatal_errors:
        for error in task.non_fatal_errors:
            print(f"ALERT — Task '{task.description}': {error}")
...
Set `fail_on_callback_error=True` on the `Task` to re-raise callback exceptions and stop the workflow. The default behaviour captures the error in `callback_error` without halting execution.
