...
<Note>
**Lightweight retry vs. full durable outbox**: every shipped adapter (Telegram, Slack, Discord, WhatsApp, Email, Linear, AgentMail) automatically retries transient send failures and can park permanent ones in an outbound DLQ — see [Outbound Resilience](/docs/features/outbound-resilience). That mixin is *in-flight* only (no persistence across restarts). The `DurableDelivery` / `OutboundQueue` / `DurableAdapterMixin` primitives on **this** page are the heavier crash-safe outbox — use them when a reply must survive a process restart, not just a transient HTTP error.
</Note>
...
<Card title="Outbound Resilience" icon="shield-check" href="/docs/features/outbound-resilience">
  Lightweight retry mixin — retry transient failures and park permanent ones in a DLQ
</Card>
</CardGroup>
