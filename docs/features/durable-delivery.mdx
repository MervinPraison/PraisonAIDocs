...
<Note>
**All channels supported.** `OutboundResilienceMixin` is applied to Slack, Discord, WhatsApp, Email, Linear, AgentMail, and Telegram. The `outbound_resilience` and `dlq_path` config knobs in [Delivery Config](/docs/features/delivery-config) are meaningful for all six adapters — not just Telegram.
</Note>
