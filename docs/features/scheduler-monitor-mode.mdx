...
<Note>
**State persistence is now wired** ([PR #4057](https://github.com/MervinPraison/PraisonAI/pull/4057)). `JobStateStoreProtocol` has a concrete `ConfigYamlScheduleStore` implementation (16 KiB per-job cap), and `ScheduledAgentExecutor` loads prior state into a "Notepad" prompt block, threads it into `state=`-aware gates, and persists `GateResult.state_updates` + agent-emitted state — including on `no_change` / skip ticks. See [Cross-Run Memory](/features/scheduler-job-state) for the full walkthrough. The change-detection **probe + hash + diff** wrapper (what actually decides a source changed) still ships in a follow-up.
</Note>
...
job = ScheduleJob(
    ...
    schedule=parse_schedule("weekdays at 9am"),   # natural language also works
...
Configure a monitor from the CLI with `--monitor-url` (public HTTP(S)) or `--monitor-command` (shell stdout) — see [CLI: schedule → Wake only when a page changes](/cli/schedule#wake-only-when-a-page-changes). Both flags produce byte-identical jobs to the `monitor={"url": …}` / `monitor={"command": …}` form here, and the URL path carries the [reliability guarantees](/features/scheduler-monitor#url-monitor-reliability-safety) documented on the core Scheduler Monitor page.
...
<CardGroup cols={2}>
<Card title="Scheduler Cross-Run Memory" icon="database" href="/features/scheduler-job-state">
