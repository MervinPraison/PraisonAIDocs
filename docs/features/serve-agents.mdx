...
# is equivalent to
serve_agents([agent], port=7777)
```

Pick whichever idiom fits your code — see [Agents MCP](/deploy/servers/agents-mcp).
</Note>
...
- **AgentOS `/chat` now uses the same per-request clone model** as `serve_agents`, but drops back to the shared template when the agent has `handoffs`. See [AgentOS Chat Session Isolation](/features/agentos-chat-session-isolation).
...
<CardGroup cols={2}>
<Card title="Agents MCP" icon="robot" href="/deploy/servers/agents-mcp">
`launch(protocol='mcp')` — the same code path, delegating here
</Card>
<Card title="PraisonAI MCP" icon="server" href="/deploy/servers/praisonai-mcp">
Umbrella `praisonai mcp serve`
</Card>
<Card title="MCP" icon="plug" href="/features/mcp">
Connect agents *to* MCP servers (client side)
</Card>
<Card title="praisonai-mcp Package" icon="box" href="/features/praisonai-mcp-package">
