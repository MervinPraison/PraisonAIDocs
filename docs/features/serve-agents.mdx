...
Pick whichever idiom fits your code — see [Agents MCP](/docs/deploy/servers/agents-mcp).
</Note>
...
- **AgentOS `/chat` now uses the same per-request clone model** as `serve_agents`, but drops back to the shared template when the agent has `handoffs`. See [AgentOS Chat Session Isolation](/docs/features/agentos-chat-session-isolation).
