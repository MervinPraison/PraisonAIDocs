...
Two protocols are available depending on how your adapter is used:

**`MemoryProtocol`** — for plugging into the registry as a named backend. Implement at minimum: `store_short_term`, `search_short_term`, `store_long_term`, `search_long_term`, and `get_all_memories`.
...
**`AgentMemoryProtocol`** — for passing a memory instance directly to `Agent`. Implement `get_context(query)` and `save_session(name, conversation_history, metadata)`.

```python
from praisonaiagents import Agent

class MyDirectMemory:
    def get_context(self, query=None, **kwargs):
        return "recalled facts here"

    def save_session(self, name, conversation_history=None, metadata=None, **kwargs):
        pass

agent = Agent(name="assistant", memory=MyDirectMemory())
```
...
<Card title="Memory Lifecycle Hooks" icon="webhook" href="/features/memory-lifecycle-hooks">
  Hook into memory write, session switch, and delegation events.
</Card>
