...
<Note>
**Streaming is included since PraisonAI PR [#4462](https://github.com/MervinPraison/PraisonAI/pull/4462).** `iter_stream()` and `start(stream=True)` now validate the input before the first token is yielded. A blocked prompt yields the single chunk `[Input blocked by guardrail: <reason>]` and stops — the durable-run record is never opened.
</Note>
