...
<Warning>
**Fail-closed guarantee** (PraisonAI PR #3545 and #3574). A missing or unsupported LLM (#3545) **and** an ambiguous validator reply (#3574) both block output on an `LLMGuardrail`. Do not rely on any older behaviour that allowed the output through when the guardrail couldn't clearly validate.
</Warning>
