...
<Tab title="custom instance">
```python
from praisonai.integrations.compute.docker import DockerCompute

flow = AgentFlow(
    compute=DockerCompute(image="python:3.12-slim", cpu=2, memory_gb=4),
    steps=[writer, reader],
)
```
</Tab>
...
| Single instance | Concurrent first-use calls (e.g. from `parallel()`) share one sandbox, not several — guarded by an internal lock. |
...
| Provider instance | Any object implementing `ComputeProviderProtocol` (`provision`, `execute`, `shutdown`). Use this to customise image, CPU, memory, env, or working dir. |
...
Custom image or resources — pass a pre-configured provider instance.
...
flow.run("Build and test in /workspace")
```
