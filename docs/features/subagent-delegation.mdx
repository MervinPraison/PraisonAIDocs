...
For slow subtasks — full test suites, broad refactors, or codebase scans — use the `background=True` flag on `spawn_subagent` so the parent agent keeps working while the job runs on a worker thread.
...
# Kick off a slow task — returns immediately with job_id
...
Scoping (`max_depth`, `allowed_agents`, `tools`, `llm`, `permission_mode`) is **preserved on the worker thread** — a background subagent runs under exactly the same constraints as a synchronous one.

<Tip>
See [Subagent Tool — Background Mode](/docs/features/subagent-tool#background-mode) for the full two-tool dance and all return shapes.
</Tip>
...
- [Subagent Tool](/docs/features/subagent-tool)
