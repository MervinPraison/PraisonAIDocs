...
<Note>
For agent-to-agent routing inside a conversation, prefer [Handoffs](/features/handoffs). Use the subagent tool or delegator when you need programmatic spawn control, background jobs, or parallel fan-out.
</Note>
...
<Note>
**Cancellation actually cancels.** `cancel_task(task_id)` and `cancel_all()` propagate cancellation to the underlying `asyncio.Task` — the running coroutine receives `CancelledError` and stops. A task cancelled before it executes never runs. A late completion cannot overwrite `CANCELLED` back to `COMPLETED`. This behaviour was tightened in PraisonAI PR #3545 (earlier versions only flipped the status enum).
</Note>

| Method | What it does |
|---|---|
| `cancel_task(task_id)` | Propagates cancellation to the running `asyncio.Task`; the coroutine receives `CancelledError`. Terminal `CANCELLED` status is preserved even if the task completes late. |
| `cancel_all()` | Same as `cancel_task` applied to every non-terminal task (PENDING or RUNNING). |
