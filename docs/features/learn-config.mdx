...
<Card title="LearnConfig Python Reference" icon="code" href="/sdk/reference/praisonaiagents/classes/LearnConfig">
...
| `nudge_interval` | `int` | `0` | Nudge agent every N turns. `0` = disabled; `Agent(learn=True)` sets it to `10`. |
| `nudge_min_tool_iters` | `int` | `3` | Tunable override, not a hard floor. `<= 0` fires the cadence on conversational (tool-less) turns too; a positive value gates on tool-work count. |
| `propose_skills` | `bool` | `False` | Enable `skill_manage` tool in propose mode |

<Note>
`Agent(learn=True)` is a coordinated shortcut: it enables `mode=AGENTIC` + `nudge_interval=10` + `nudge_min_tool_iters=0` + `auto_memory=True` all at once, so a chat-only companion accrues learnings with no extra wiring. See [Bot Session Learning](/features/bot-session-learning) for the default-on gateway/bot behaviour.
</Note>
...
<CardGroup cols={2}>
  <Card title="Memory Config" icon="brain" href="/configuration/memory-config">
    Session and persistent memory configuration
  </Card>
  <Card title="Agent Learning" icon="graduation-cap" href="/features/learn">
  ...
  <Card title="Learn Skill" icon="star" href="/features/learn-skill">
    Teach agents new skills dynamically
  </Card>
  <Card title="Bot Session Learning" icon="user-graduate" href="/features/bot-session-learning">
    Default-on cross-session learning for gateway/bot agents
  </Card>
</CardGroup>
