...
| Option | Type | Default | Description |
|---|---|---|---|
| `store_backend` | `str` | `"sqlite"` | `"sqlite"` (default, durable) — `"memory"` (per-process, zero-dep, opt-in for tests / ephemeral runs) or `"sqlite"` (durable, survives restart). Unset means `"sqlite"` ([Issue #4339](https://github.com/MervinPraison/PraisonAI/issues/4339)). Unknown values fall back to `"memory"` with a debug log. |
