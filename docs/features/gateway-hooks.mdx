...
sequenceDiagram
    ...
    Note over Dedup: default: in-memory · sqlite for restart durability
    alt duplicate
...
| Option | Type | Default | Description |
|---|---|---|---|
| `store_backend` | `str` | `"memory"` | `"memory"` (per-process, zero-dep) or `"sqlite"` (durable, survives restart). Unknown values fall back to `"memory"` with a debug log. |
