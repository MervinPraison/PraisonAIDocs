...
`channel_target` is set by channel adapters (Slack/Discord/Telegram) and used by [Restart Continuation](/docs/features/gateway-restart-continuation) to proactively notify channel users after a restart. `None` for direct-client sessions.
