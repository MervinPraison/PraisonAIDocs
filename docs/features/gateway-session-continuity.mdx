...
### Server-initiated resume (channel users)

A channel user (Slack/Discord/Telegram) never "reconnects" over WebSocket, so the client-initiated path above can't reach them. On boot, the gateway instead scans persisted sessions for an interrupted turn carrying a `channel_target`, proactively sends the channel a notice, and re-drives the turn when the agent is still registered. See [Gateway Restart Continuation](/features/gateway-restart-continuation) for details.
...
`channel_target` is set by channel adapters (Slack/Discord/Telegram) and used by [Restart Continuation](/features/gateway-restart-continuation) to proactively notify channel users after a restart. `None` for direct-client sessions.
...
See [Gateway Drain Trigger](/features/gateway-drain-trigger) for the marker-file contract and the `DrainMarkerPolicy` / `current_epoch()` API.
...
<CardGroup cols={2}>
  ...
  <Card title="Session Portability" icon="download" href="/features/gateway-session-portability">
    Continuity survives disconnects on the same host; portability moves state to a different host
  </Card>
  <Card title="Error Handling" icon="triangle-alert" href="/features/gateway-error-handling">
