...
<Accordion title="per_chat sessions and continuity">
When `session_scope: per_chat` is enabled, the shared group session key (`telegram:chat:{chat_id}:{thread_id}`) is durable — it reconnects and resumes just like a per-user session. See [Group Chat Sessions](/docs/features/bot-group-sessions).
</Accordion>
