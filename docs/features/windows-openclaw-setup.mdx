...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Windows OpenClaw Setup
...
<AccordionGroup>
  <Accordion title="Isolate PraisonAI in a dedicated venv">
  ...
  <Accordion title="Point OpenClaw at the local daemon, not the internet">
  ...
  <Accordion title="Health-check and auto-restart the service">
  ...
  <Accordion title="Keep credentials out of PowerShell scripts">
...
<CardGroup cols={2}>
<Card title="Onboard CLI" icon="terminal" href="/features/onboard">
Interactive setup wizard including OpenClaw
</Card>
<Card title="MCP Lifecycle" icon="plug" href="/features/mcp-lifecycle">
