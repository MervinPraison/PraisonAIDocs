...
<Tip>
**See also: [Webhook Channel](/features/webhook-channel)** — the generic `webhook` gateway channel is the primary consumer of `HmacWebhookVerifier`, turning any signed HTTP webhook into an agent trigger through declarative YAML routes.
</Tip>

<Note>
This page covers **outbound bot webhook signature verification** (verifying that requests to your bot are from the real platform) using `HmacWebhookVerifier` + `enforce_webhook_verification`. For **inbound gateway hooks** — pointing external services at your gateway to run an agent — signature verification uses a separate helper, `verify_webhook_signature` + `HookConfig.verify_signature` (set `secret` on the hook). See [Gateway Inbound Hooks → Verifying Provider Signatures](/docs/features/gateway-hooks#verifying-provider-signatures-hmac).
</Note>

## Related

<CardGroup cols={2}>
<Card title="Linear Bot" icon="list" href="/features/linear-bot">
  Linear webhook setup and secrets
</Card>
<Card title="Email Bot" icon="envelope" href="/features/email-bot">
  AgentMail webhook mode
</Card>
<Card title="Gateway Inbound Hooks" icon="webhook" href="/features/gateway-hooks">
