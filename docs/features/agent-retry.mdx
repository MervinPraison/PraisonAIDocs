...
<Steps>
<Step title="Default retry (do nothing)">
```python
from praisonaiagents import Agent
...
Omitting `retry=` (or passing `retry=True`) enables retry with sensible defaults: 3 retries, 5 s → 10 s → 20 s exponential schedule, capped at 120 s, with 50% additive jitter — on both the native and LiteLLM paths.
</Step>

<Step title="Disable retry (fast fail)">
```python
from praisonaiagents import Agent

# retry=False turns off the retry loop on both paths.
agent = Agent(
    name="Fast Checker",
    instructions="Surface transient failures immediately",
    retry=False,
)
agent.start("Quick health check")
```
</Step>
