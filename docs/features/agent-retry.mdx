...
<Note>
**v-after-#3769 (2026-08-13):** `retry=…` on the custom-LLM path (any `provider/model` string, dict LLM config, or `base_url`/`api_key`) previously fell through to LLM's hardcoded default. It now maps to `max_retries` on the underlying LLM init. If you had set `retry=False` on a custom-LLM Agent hoping to disable retries, that opt-out finally takes effect after upgrading — expect 0 retries where you may have been getting 3 before. Users who set an explicit `max_retries` inside a dict LLM config keep their value (dict-level `max_retries` wins over `retry=`).
</Note>
...
<Note>
**Fix in PR [#3769](https://github.com/MervinPraison/PraisonAI/pull/3769):** `retry=` now applies to custom LLM configurations (`llm="provider/model"`, dict configs, `base_url` endpoints). Before this fix, the parameter was silently dropped on those paths and every custom LLM ran with `max_retries=3`.
</Note>
...
| Custom LLMs (any `provider/model` string, dict LLM config, or `base_url`/`api_key` construction) | ✅ — honoured. `retry=False` → `max_retries=0`. An explicit `max_retries` inside a dict LLM config wins. |
...
The TypeScript SDK ships the same `RetryBackoffConfig` with byte-for-byte identical validation messages, so error handling ports across languages unchanged:

<Tabs>
<Tab title="TypeScript">
```typescript
import { RetryBackoffConfig } from 'praisonai';

const cfg = new RetryBackoffConfig({ baseDelay: 2.0, maxDelay: 60.0, jitterRatio: 0.25, maxRetries: 5 });
console.log(cfg.delayFor(1)); // seconds for the 2nd attempt

// Invalid config throws a RangeError with the same message text as Python:
//   'base_delay must be > 0'
//   'max_delay must be >= base_delay'
//   'jitter_ratio must be between 0 and 1'
new RetryBackoffConfig({ baseDelay: 0 }); // RangeError: base_delay must be > 0
```
</Tab>
</Tabs>
...
| `retry=` on a custom LLM (`llm="provider/model"`, dict config, or `base_url`) | Same as the native path — `retry=False` → `max_retries=0`, `retry=RetryBackoffConfig(max_retries=N)`/`retry={"max_retries": N}` → `max_retries=N` (since PR #3769) |
...
`retry=` now applies to every LLM configuration path:

- `Agent(llm="openai/gpt-4o", retry=...)` — provider/model string
- `Agent(llm={"model": ..., "base_url": ..., ...}, retry=...)` — dict config
- `Agent(base_url=..., retry=...)` — custom endpoint

Before PraisonAI PR #3769, `retry=` was silently ignored on these paths and the underlying `LLM` fell back to its hardcoded `max_retries=3`. After the fix, `retry=False` correctly disables retries, and `retry=RetryBackoffConfig(max_retries=N)` / `retry={"max_retries": N}` are forwarded to the custom LLM.

**One precedence rule to know:** if you set `max_retries` inside a dict `llm=` config, that wins over `retry=`. This mirrors how every other field in the `llm=` dict is authoritative.

```python
from praisonaiagents import Agent

# dict-config max_retries wins over retry=
agent = Agent(
    name="A",
    instructions="...",
    llm={"model": "openai/gpt-4o", "max_retries": 5},
    retry={"max_retries": 10},   # ignored — dict wins
)
# → custom LLM runs with max_retries=5
```

| Config | Result |
|--------|--------|
| `retry=False` on custom LLM | Custom LLM initialised with `max_retries=0` |
| `retry=RetryBackoffConfig(max_retries=N)` on custom LLM | Custom LLM initialised with `max_retries=N` |
| `retry={"max_retries": N}` on custom LLM | Custom LLM initialised with `max_retries=N` |
| `retry=` unset on custom LLM | LLM class's own default (currently `max_retries=3`) — unchanged |
| Dict config with explicit `max_retries` plus `retry=` | Dict config `max_retries` wins — user-set dict values are never overridden |
| Dict config without `max_retries` plus `retry=` | `retry`-derived `max_retries` is forwarded into the dict |
...
<Note>
Async agents (`await agent.astart(...)`) fire both the `ON_RETRY` hook and the `RETRY` stream event on the async retry loop. Async stream consumers should register via `agent.stream_emitter.add_async_callback(async_fn)`; sync consumers keep using `add_callback(sync_fn)`. Both are dispatched.
</Note>
...
Over the AG-UI wire, a `RETRY` reaches the client as a `custom` event named `praisonai.retry` with `value` `{ message, metadata }`. A `MODEL_FALLBACK` arrives as `praisonai.model_fallback`. See the [AGUI API reference](/deploy/api/agui-api#praisonai-custom-events).

## Related

<Note>
Agent retry now applies to **both** the LiteLLM-backed agent loop **and** the native OpenAI-client path by default (`max_retries=3`). For the SDK-level `Retry-After` + backoff on the native `OpenAIClient` used by `AutoAgents` and direct `get_openai_client()` callers, see [OpenAI Client Retries](/features/openai-client-retries).
</Note>

<CardGroup cols={2}>
<Card title="OpenAI Client Retries" icon="rotate" href="/features/openai-client-retries">
