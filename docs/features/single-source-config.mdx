...
<Warning>
Local stdio commands declared in config are run through the same allowlist as `--mcp`. The executable basename must be in `ALLOWED_MCP_COMMANDS` (e.g. `npx`, `node`, `python`, `uvx`, `deno`, `bun`), and inline-eval flags (`python -c`, `node -e`, `deno eval`, `bun -e`) are rejected. A rejected server is logged and skipped — the rest of the run continues.
</Warning>
...
- `--mcp` is **additive**: its single ad-hoc server is wired alongside every enabled server in `mcp.servers.*`. To exclude a config server from a specific run, set `enabled: false` in the YAML for that invocation (or use a separate `.praisonai/config.yaml` directory).
...
### Wire several MCP servers at once

```mermaid
graph LR
    CFG[📄 config.yaml] --> RUN[▶️ praisonai run]
    RUN --> S1[🖥️ playwright\nstdio]
    RUN --> S2[🖥️ postgres\nstdio]
    RUN --> S3[🌐 docs\nremote]
    S1 --> AGT[🤖 Agent]
    ...
    classDef config fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef run fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef stdio fill:#189AB4,stroke:#7C90A0,color:#fff
...
```bash
praisonai run "Open the staging URL, query the orders table, and link to the matching docs section"
```

All three servers are wired in one run — Playwright + Postgres locally, and the remote docs MCP over HTTP.

<Warning>
Legacy SSE endpoints (URLs ending in `/sse`) cannot forward custom `headers` — the SDK's `SSEMCPClient` doesn't support them. PraisonAI prints a warning and connects without the headers. If the server needs authentication, switch to the HTTP-stream transport (URL **not** ending in `/sse`) or move the credential into a server-side proxy.
</Warning>
...
Config gives the base; flags add on top:

```bash
# Add an extra MCP server on top of config servers for this run
