...
Values can reference `${VAR}`, `{env:NAME:-default}`, or `{file:./relative/path}` — see [Value Interpolation](/docs/cli/config#value-interpolation) — and `praisonai config provenance` shows which layer/file supplied each key.
...
sequenceDiagram
    ...
    Resolver->>Config: walk-up discovery (all ancestors merged)
...
| 3 | `.praisonai/config.yaml` (project, walk-up — **all ancestors merged**) | |
| 4 | Environment variables | |
| 5 | CLI flags | Highest |

CLI flags always win. Config fills the gaps. Walk-up discovery deep-merges **every** ancestor config from the repo root down to `cwd`, not just the nearest one.
...
<Note>
`read:*` here does **not** authorise `.env` / private-key reads — the built-in secret gate defaults those to `ask` unless a secret-specific rule (e.g. `read:*.env=allow`) opts in. See [Secret Read Gate](/features/secret-read-gate).
</Note>
...
<CardGroup cols={2}>
<Card title="Configuration File" icon="gear" href="/features/config-file">
...
<Card title="MCP Transports" icon="plug" href="/mcp/mcp-remote">
