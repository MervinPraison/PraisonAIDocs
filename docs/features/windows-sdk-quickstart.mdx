...
agent = Agent(
    ...
    sandbox=True,  # subprocess backend on Windows — see limitations below
)
agent.start("Print the current working directory")
```
...
See [`/docs/features/sandbox#resource-limits-in-practice`](/docs/features/sandbox#resource-limits-in-practice) for the full capability matrix.

To harden a Windows sandbox, switch backends — see [Sandbox Backends](/features/sandbox-backends).
</Step>
...
<Step title="Running the MCP host on Windows">
...
Python 3.13 is supported. Earlier `praisonai-mcp` releases crashed on Windows with `OSError: [WinError 6] The handle is invalid`; the STDIO transport now reads stdin on a dedicated thread, so it works under the `ProactorEventLoop`.
...
See [MCP STDIO Server Transport](/features/mcp-stdio-server-transport) for the reliability guarantees.
</Step>
</Steps>
...
dev_agent = Agent(
    ...
    llm_model=os.getenv("DEFAULT_MODEL", "gpt-4o-mini")
)
...
| `OSError: [WinError 1314] A required privilege is not held by the client` in a sandboxed job that creates symlinks | Default non-admin Windows users cannot create symlinks | Enable **Windows Developer Mode** (Settings → System → For developers), or run under an account that holds `SeCreateSymbolicLinkPrivilege`. See [PR #3224](https://github.com/MervinPraison/PraisonAI/pull/3224). |
| Sandbox process still running after a timeout on Windows | `os.killpg` is POSIX-only — only the leader process is killed on Windows | Prefer the `docker` backend for reliable timeout enforcement on Windows. See the [Windows tab of the sandbox docs](/docs/features/sandbox#resource-limits-in-practice). |
| `pytest tests -m "not network"` fails in `src/praisonai-sandbox` with `WinError 1314` / `AttributeError: os.killpg` on Windows | Pre-release before PR #3224 lacked platform skips on those tests | Upgrade to a release that contains [PR #3224](https://github.com/MervinPraison/PraisonAI/pull/3224). |
| `RuntimeError: MCP server produced 0 tools ...` when calling `MCP("npx ...")` | `npx` on Windows is a `.cmd` shim; the SDK spawns the wrong executable and the server lists no tools. Fail-closed since PraisonAI [#4423](https://github.com/MervinPraison/PraisonAI/pull/4423). | Pass the shim explicitly: `MCP(command="npx.cmd", args=["-y", "@pkg"])`. Or switch to a Python MCP server: `MCP(command=sys.executable, args=["server.py"])`. See [MCP → Errors on construction](/features/mcp#errors-on-construction). |
| `TimeoutError: MCP initialization timed out after N seconds` when calling `MCP(...)` | The stdio subprocess did not complete the MCP handshake within `timeout=`. Common causes on Windows: Node not on PATH, offline `-y` install, antivirus blocking npx. | Raise `timeout=` (e.g. `MCP(..., timeout=90)`), verify `node --version` and `npx --version` work in the same shell, or pre-install the MCP server so `-y` skips the download. |
...
<CardGroup cols={2}>
<Card title="Skills Documentation" icon="puzzle-piece" href="/features/skills">
  Learn about Windows-compatible skills and UTF-8 handling
</Card>
<Card title="MCP Integration" icon="plug" href="/features/mcp-lifecycle">
