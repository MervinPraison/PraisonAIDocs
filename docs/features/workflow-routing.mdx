...
sequenceDiagram
    ...
    participant Agent1 as Specialist 1
    participant Agent2 as Specialist 2

    User->>Workflow: Request
    Workflow->>Router: Classify request
    Router-->>Workflow: Route decision
    alt Route to Specialist 1
        Workflow->>Agent1: Process request
        Agent1-->>Workflow: Result
    else Route to Specialist 2
        Workflow->>Agent2: Process request
        Agent2-->>Workflow: Result
    end
    Workflow-->>User: Specialized response
```
