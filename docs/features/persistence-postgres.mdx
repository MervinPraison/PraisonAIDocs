...
`Agent(
    memory=MemoryConfig(
        db=db(database_url="postgresql://..."),
    ))` is the simplest path — store wiring is automatic.
</Accordion>
...
<Accordion title="Cold-start bursts are safe with async_postgres">
`create_conversation_store("async_postgres", ...)` guards pool creation with double-checked locking, so concurrent first-requests under FastAPI share one connection pool instead of building and leaking duplicates.
</Accordion>
</AccordionGroup>
