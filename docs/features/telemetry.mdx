...
<Note>
**Runtime opt-out is now live-checked.** `disable_telemetry()` called at runtime immediately silences the import-time performance monitor as well as the main telemetry client — the disabled flag is consulted on every event, not cached at construction.
</Note>
...
### What each agent run counts as

`agent_executions` is incremented **once per logical run**, no matter which entry point you called. `Agent.run()`, `Agent.start()`, and `Agent.chat()` all delegate to `chat()` internally, and telemetry now records only the outermost call — so a single `agent.run("...")` is one event, not two or three, and a failure inside it is one `errors` increment, not two.

<Note>
Synchronous `Agent.chat()` / `Agent.start()` / `Agent.run()` runs are counted in `agent_executions` starting from the release that contains the sync-path telemetry fix (September 2026). Earlier releases counted only async single-agent runs and multi-agent runs.
</Note>

### Concurrent-execution accuracy

Concurrent `chat()`, `run()`, and `start()` calls on one shared `Agent` instance are each counted independently — you'll see one row per top-level call in `agent_executions`, token totals, and duration histograms.

Nested calls (a `run()` that internally calls `chat()`) still record once — only the outermost call is counted, so a single user-facing turn produces one execution row.

<Note>
The re-entrancy guard is a per-agent `contextvars.ContextVar`, so this works in both thread pools and asyncio tasks without any extra setup. Previously a shared instance attribute undercounted N concurrent calls as 1 — 20 parallel `chat()` calls reported a single execution. See [Thread Safety](/features/thread-safety).
</Note>
...
sequenceDiagram
    ...
    Note over M: agent_executions = 3
```

### Async runs produce the same LLM lifecycle spans

Async agent runs now emit the same `llm_start` / `llm_end` lifecycle events in the JSONL trace as sync runs. The trace / status output (`output/status.py`, `output/trace.py`) consumes these events, so an async `--trace` run records per-iteration LLM spans with cost figures.

<Note>
**Fixed in PraisonAI [PR #4887](https://github.com/MervinPraison/PraisonAI/pull/4887).** Prior to this release, the async tool-loop (`achat_completion_with_tools`) emitted no `llm_start` / `llm_end` events, so the async JSONL trace had **no LLM spans and no cost figures** while the identical sync run recorded them. After this release, async and sync runs produce matching `llm_start` / `llm_end` entries — each with `tokens_in`, `tokens_out`, `cost`, and `latency_ms`. See [Cost Tracking](/features/cost-tracking) and [Display Callbacks](/features/display-callbacks).
</Note>

### Metrics are not lost on a failed flush

`flush()` resets counters only after a successful transport-level capture. If PostHog is temporarily unreachable, the batch stays intact and is retried on the next flush instead of being silently zeroed.
...
<CardGroup cols={2}>
  <Card title="Telemetry Not Working" icon="triangle-exclamation">
    ...
    - If you called `disable_telemetry()` and still see one final event at interpreter exit, that was the pre-#4200 `atexit` handler resurrecting a fresh client. On the fixed version, no phantom session is emitted at shutdown
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Gateway Tracing Hook" icon="route" href="/features/gateway-tracing-hook">
