...
<Note>
This page is about **publishing whole `ToolSource` implementations** — a lower-level extensibility point for the resolver itself. If you just want to publish an **individual tool** so agents can call it by name, use the canonical `praisonai.tools` entry-point group instead (see [Tool Discovery Order](/features/tool-discovery-order#publishing-a-tool-package)).
</Note>
...
<Note>
**Built-in names win.** `ToolSourceRegistry` subclasses the base `PluginRegistry`, so since [PraisonAI PR #4176](https://github.com/MervinPraison/PraisonAI/pull/4176) publishing a `praisonai.tool_sources` entry point whose name (case-insensitive) matches an already-shipped built-in source has no effect — the entry point is skipped, the shipped source resolves, and a `DEBUG` line notes the collision. Use runtime `register(...)` for a deliberate override. See [Plugin Precedence](/features/plugin-precedence).
</Note>
...
To verify from the CLI immediately after `pip install`, run `praisonai tools add <package> --dry-run` — it constructs a fresh resolver and prints exactly which tools now resolve. See [tools add](/features/tools-add).
...
<Card title="Persistence Backend Plugins" icon="puzzle-piece" href="/features/persistence-backend-plugins">
...
<Card title="Registry Dependency Injection" icon="code-branch" href="/features/registry-dependency-injection">
