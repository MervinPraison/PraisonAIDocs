...
gateway:
  preflight:
    verify_turn: true            # default on
    verify_turn_prompt: "ping"
```

```bash
praisonai gateway start                     # runs the real-turn check
praisonai gateway start --no-verify-turn    # skip for offline envs
```

This differs from the channel/tool probes below: those prove **wiring** (tokens resolve, tools load) but never exercise a model round-trip. The turn pre-flight sends a real prompt to the agent and waits for a reply.

```mermaid
sequenceDiagram
    participant Operator
    participant GatewayStart as gateway start
    participant Preflight as verify_turn_preflight
    participant Agent
    participant LLM as Model provider

    Operator->>GatewayStart: praisonai gateway start
    GatewayStart->>Preflight: prompt="ping"
    Preflight->>Agent: chat("ping")
    Agent->>LLM: API call
    alt Credential valid
        LLM-->>Agent: reply
        Agent-->>Preflight: text
        Preflight-->>GatewayStart: (ok, "pong")
        GatewayStart-->>Operator: Turn pre-flight OK — starting
    else Missing / invalid key or timeout
        LLM-->>Agent: 401 / 429 / timeout
        Agent-->>Preflight: error
        Preflight-->>GatewayStart: (False, detail)
        GatewayStart-->>Operator: Fail — set model key or --no-verify-turn
    end
```

The turn pre-flight is governed by its **own** toggle, independent of `--preflight` / `--no-preflight`. Running `--no-preflight --verify-turn` still exercises the model round-trip. It runs against the **first configured channel** by default.
...
gateway:
  preflight:
    verify_turn: true              # default true — run one real agent turn at start
    verify_turn_prompt: "ping"     # default "ping" — cheap, deterministic
    verify_turn_timeout: 60        # default 60.0 seconds — bounds the round-trip
```

| Key | Type | Default | Purpose |
|-----|------|---------|---------|
| `verify_turn` | `bool` | `true` | Whether `gateway start` runs the real-turn pre-flight. |
| `verify_turn_prompt` | `str` | `"ping"` | Prompt sent to the agent for the check. |
| `verify_turn_timeout` | `float` | `60.0` | Bounds the agent round-trip (seconds) so a stalled provider fails fast. |
...
| Automated start gate (turn) | `praisonai gateway start` (default on) |
| Skip turn gate | `praisonai gateway start --no-verify-turn` |
| Tool resolution start gate | `praisonai gateway start --strict-tools` |
...
|-----|--------------|
| `probes` | Always. |
| `secrets` | Any channel uses a [secret reference](/features/gateway-secret-references) or credential field. |
...
gateway:
  ...
  health:
    ...
    fleet_restarts_per_hour: 40     # fleet-wide crash-loop breaker (int >= 1)
    failing_channel_fraction: 0.5   # trip when this fraction is failing (0.0 < x <= 1.0)
    breaker_cooldown_s: 120         # hold restarts this long once tripped (float >= 0)
```
...
| `fleet_restarts_per_hour` | `40` | Aggregate restart-rate window for the [fleet-level breaker](/features/gateway-crash-loop-guard#fleet-level-breaker). |
| `failing_channel_fraction` | `0.5` | Trip the fleet breaker when this fraction of channels is failing/parked. |
| `breaker_cooldown_s` | `120` | Hold channel restarts this long once the fleet breaker trips, then re-arm. |
...
Since PraisonAI ≥ [#4197](https://github.com/MervinPraison/PraisonAI/pull/4197), the lock also carries a start-time fingerprint so a recycled PID is not mistaken for the original gateway — the "same PID, different process" case is now caught by the lock itself, not just by this scan.
...
from praisonai_bot.gateway.preflight import (
    ...
    run_turn_test,                  # (config_path, channel_name, prompt, run_timeout=None) -> (ok, message)
    check_gateway_running,          # (config_path, timeout=5.0) -> (ok, message)
    resolve_verify_turn,            # (config_path) -> (enabled: bool, prompt: str)
    verify_turn_preflight,          # (config_path, channel_name=None, prompt="ping", run_timeout=None) -> (ok, detail)
...
ok, detail = asyncio.run(verify_turn_preflight("bot.yaml", prompt="ping"))
if not ok:
    raise SystemExit(f"Agent turn pre-flight failed: {detail}")
```
...
<Note>
Since [#4197](https://github.com/MervinPraison/PraisonAI/pull/4197), the `pid_lock` dict (from `GatewayPIDLock.get_lock_info()`) includes an optional `create_time: float | None` field, and `is_running` returns `True` only when the PID **and** the fingerprint match. Older 4-line lock files (no fingerprint) fall back to the previous PID-only check.
</Note>
