...
The Slack `app_mention` handler fires `MESSAGE_RECEIVED` **before** allow-list checks, so `@mentions` now flow through the same gate as DMs (previously DM-only).

| Hook decision | Effect on the @mention |
|---|---|
| `drop=True` | Handler short-circuits — the mention is silently dropped. |
| rewritten `content` | The LLM prompt uses the hook's content, not the raw Slack event text. |
| empty `content` | Honored verbatim — an intentional redaction to empty is respected. |

Mentions also now count toward channel `last_activity` and increment [`messages_inbound_total`](/features/gateway-metrics), so an @mention registers as inbound activity for `/health`.

<Warning>
Even if the credential probe passes with `SLACK_BOT_TOKEN`, Socket Mode inbound needs `SLACK_APP_TOKEN` (`xapp-…`). Without it, `praisonai gateway test` appends `SLACK_APP_TOKEN missing — Socket Mode will not start` to `probes.<channel>.details.warnings[]` and mentions never arrive. See [Gateway Readiness](/docs/features/gateway-readiness#credential-probe-warnings).
</Warning>
...
See [Quoted Reply Context](/features/bot-quoted-reply-context).
...
<CardGroup cols={2}>
  ...
  <Card title="Quoted Reply Context" icon="reply-all" href="/features/bot-quoted-reply-context">
