...
Mentions also now count toward channel `last_activity` and increment [`messages_inbound_total`](/docs/features/gateway-metrics), so an @mention registers as inbound activity for `/health`.
