...
<Steps>
  ...
  <Step title="Session Store">
    History is loaded from the session store using `get_working_history(session_id, max_messages=limit)` when the store supports it — this replays a compacted summary + retained tail if a compaction checkpoint exists. Stores that implement only `get_chat_history` fall back to raw history automatically. See [Compacted Session Resume](/features/session-compaction-checkpoint).
  </Step>
</Steps>
