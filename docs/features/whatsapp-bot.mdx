...
<Note>
**Multiple agents on one bot?** Route each inbound business number to a different agent with the [Agent Registry](/docs/features/agent-registry) — one bot, N agents, dispatched by phone number.
</Note>
