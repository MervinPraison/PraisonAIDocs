...
<Note>
WhatsApp **cloud** mode now stays running correctly. Previously its non-blocking start path exited `0` in about a second, so `systemd Restart=on-failure` never restarted it. The CLI now holds the loop open while the adapter is running, and startup failures exit **1**.
</Note>
</Step>
</Steps>
</Tab>
...
<Note>
**Multiple agents on one bot?** Route each inbound business number to a different agent with the [Agent Registry](/features/agent-registry) — one bot, N agents, dispatched by phone number.
</Note>
...
See [Bot Presentations](/features/bot-presentations) for the portable model shared across Telegram, Slack, Discord, and WhatsApp.
...
```mermaid
graph TD
    MSG["📩 Message"] --> CANON["🪪 Canonicalize\nLID → phone"]
    CANON --> STALE{"Older than\nconnect time?"}
    ...
    classDef canon fill:#6366F1,stroke:#7C90A0,color:#fff
...
Identity canonicalization runs **first**, so every later check — self-chat, outgoing guard, and allowlist — sees one stable phone-form identity regardless of whether the message arrived as a `<lid>@lid` or `<phone>@s.whatsapp.net` JID. See [Identity: LID vs phone JID](#identity-lid-vs-phone-jid).
...
<Note>
This uses the reusable `IdentityCanonicalizerProtocol` extension point. To plug canonicalization into another platform with multiple address forms, see [Identity Canonicalization](/features/gateway-identity-canonicalization).
</Note>
...
<Note>
**Cloud-mode webhook verification:** inbound Meta webhooks are HMAC-verified via the shared helper. Set `WHATSAPP_APP_SECRET` (see [Messaging Bots](/features/messaging-bots)). Details: [Webhook Verification](/features/webhook-verification).
</Note>
...
| **Message Filter** | Identity canonicalize (LID → phone) → stale guard → self-chat check → allowlist check |
...
<Note>
The same mechanism applies to every channel adapter, not just WhatsApp. For the full evaluator behaviour, configuration knobs (`stuck_after`, `busy_after`), and the `HealthResult.last_run_progress` field, see [Channel Supervision](/features/gateway-channel-supervision).
</Note>
...
You do not need a running WhatsApp Bot process to deliver a scheduled brief to WhatsApp. From OS cron / CI / a scale-to-zero deployment, `praisonai schedule tick` will POST directly to the WhatsApp Cloud API using the same env the live bot reads (`WHATSAPP_ACCESS_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, and either an explicit `deliver: whatsapp:<recipient>` target or `WHATSAPP_HOME_CHANNEL`). Sends are retried up to 4 times with exponential backoff on transient failures. See [Out-of-process delivery](/features/scheduler-delivery#out-of-process-delivery).
...
<CardGroup cols={2}>
<Card title="Inbound Media" icon="image" href="/features/bot-inbound-media">
