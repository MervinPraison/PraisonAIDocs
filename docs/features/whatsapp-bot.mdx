...
<Note>
**Webhook Verification (Cloud mode):** Inbound Cloud API webhooks are HMAC-verified using `WHATSAPP_APP_SECRET` (your Meta app secret). Without it, all webhooks are rejected with HTTP 401. See [Webhook Verification](/docs/features/webhook-verification) for the fail-closed behaviour and dev override options.
</Note>
</Tab>
