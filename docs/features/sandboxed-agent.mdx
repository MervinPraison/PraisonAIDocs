...
<Note>
E2B/Docker execution now runs through the standalone [praisonai-sandbox](/docs/features/praisonai-sandbox-package) package under the hood — no user-visible change. The `SandboxedAgent`/`SandboxedAgentConfig` API is untouched.
</Note>
...
<Note>
Requires `base64` on the sandbox image's `$PATH` — present on every stock PraisonAI compute image (Docker, E2B, Modal, Daytona, Fly.io). Bring-your-own minimal images must include it.
</Note>
</Accordion>
