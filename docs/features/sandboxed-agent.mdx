...
<Tip>
Not sure where the loop and the tools run? Run `print(agent.where_does_it_run())` — see [Where Your Agent Runs](/features/where-does-it-run).
</Tip>

<Note>
E2B/Docker execution now runs through the standalone [praisonai-sandbox](/docs/features/praisonai-sandbox-package) package under the hood — no user-visible change. The `SandboxedAgent`/`SandboxedAgentConfig` API is untouched. As of [PR #4092](https://github.com/MervinPraison/PraisonAI/pull/4092), the vendor compute providers also live in that package (same names, same behaviour; old `praisonai.integrations.compute.*` imports still work through a shim).
</Note>
...
<Card title="SandboxedAgentConfig Reference" icon="code" href="/sdk/reference/typescript/classes/SandboxProtocol">
...
- **Managed Runtime**: Entire agent loop runs remotely (see [Managed Runtime Protocol](/features/managed-runtime-protocol))
</Accordion>
