...
<Tip>
Use `flow.to_mermaid()` to see the shape of any of these patterns before running it. See [Workflow Diagram](/features/workflow-diagram).
</Tip>
...
| `repeat()` | Repeat until condition | Iterative improvement, or a small fixed list of steps run in order each iteration |
| `Discussion` | N-agent conversation loop | Multiple agents take turns on the same thread |

## Import

```python
from praisonaiagents import AgentFlow, WorkflowContext, StepResult
from praisonaiagents import route, parallel, loop, repeat, Discussion
```
...
### 5. Discussion (N-Agent Conversation Loop)

Have N agents take turns on the same thread:

```python
from praisonaiagents import Discussion
...
📖 [Full Documentation →](/features/workflow-discussion)
...
| `Discussion(...)` | `Discussion(agents=[...], rounds=3, until=None, select=None, name="discussion")` |
...
<CardGroup cols={2}>
  ...
  <Card title="Workflow Input Resolution" icon="arrow-right-to-bracket" href="/features/yaml-workflows#workflow-input-resolution">
