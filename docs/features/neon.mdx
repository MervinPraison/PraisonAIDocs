...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Neon Serverless PostgreSQL
...
db = NeonDB()
agent = Agent(name="Neon Agent", memory={"db": db})
```
...
agent = Agent(name="Custom Neon Agent", memory={"db": db})
```
...
<Card title="Local PostgreSQL" icon="database" href="/features/persistence">
