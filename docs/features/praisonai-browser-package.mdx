...
`praisonai-browser run` inherits the same [engine-option matrix](/docs/cli/browser#run) as the wrapper — the standalone entry point mirrors `praisonai browser run` exactly, including which flags reach the wire on each engine and the "Ignored on the … engine" warnings.
...
### SessionManager custom `db_path`

`SessionManager` stores session and step history in SQLite at `~/.praisonai/browser_sessions.db` by default. Pass `db_path` to use a custom location — parent directories are auto-created.

```python
from praisonai_browser import SessionManager

sm = SessionManager(db_path="/tmp/scratch/nested/subdir/browser.sqlite")
```

<Note>
A custom `db_path` whose parent directory did not exist previously raised `sqlite3.OperationalError: unable to open database file`. `SessionManager` now calls `os.makedirs(parent, exist_ok=True)` for any non-`None` `db_path`, so no manual `os.makedirs()` is needed.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Browser Agent Deep Dive" icon="microscope" href="/features/browser-agent-deep-dive">
