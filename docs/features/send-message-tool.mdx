...
Need the answer back, not just delivery? See [Ask Conversation](/features/ask-conversation-tool).
...
The `MEDIA:` caveat applies only to the `send_message` tool path (via `BotOutboundMessenger`). Proactive / scheduled / gateway-driven deliveries via `DeliveryRouter.send_media` **do** deliver attachments and honour threaded targets — see [Outbound Media Delivery](/features/outbound-media-delivery).
</Note>
...
# React to the message being handled right now
send_message(action="react", target="origin", message="👍")

# Remove a reaction you added earlier
send_message(action="unreact", target="origin", message="👍")
```
...
| `status` | Meaning |
|----------|---------|
| `"ok"` | Reaction added/removed successfully |
| `"unsupported"` | The channel doesn't support reactions (e.g. WhatsApp) — degrade gracefully |
| `"failed"` | Blocked by send-policy, empty emoji, or downstream error |
| `"no_route"` | Target could not be resolved to a reachable channel |

```json
{"status": "ok", "target": "telegram:12345"}
{"status": "unsupported", "detail": "the active gateway does not support reactions"}
{"status": "failed", "detail": "no emoji provided"}
```

Channels that support reactions today: **Telegram, Slack, Discord**. Others (WhatsApp, Signal, email, agentmail, Linear) return `"unsupported"` — the tool never raises.

The same [Send Policy](/features/send-policy) that guards `action="send"` also guards `react`/`unreact` — a steered or prompt-injected agent cannot react on a channel the operator never intended.

```mermaid
graph LR
    A[🤖 Agent] --> B[📤 send_message action=react]
    B --> P{🛡️ Send Policy}
    P -->|deny| F[❌ failed]
    P -->|allow| C{🔍 Channel supports reactions?}
    C -->|no| U[⚠️ unsupported]
    C -->|yes| G[🛰️ Gateway]
    G --> D[👍 Emoji added]
...
sequenceDiagram
    ...
    User->>Bot: "@agent summarise last night's incident"
    Bot->>Gateway: send_message(action="react", target="origin", message="⏳")
    Gateway->>User: ⏳ appears on the user's message
    Bot->>Bot: Works on the summary...
    Bot->>Gateway: send_message(action="unreact", target="origin", message="⏳")
    Bot->>Gateway: send_message(action="react", target="origin", message="✅")
    Gateway->>User: ✅ swapped in — done, no extra reply
```
...
| `action` | `str` | `"send"` | `"send"` to deliver, `"list"` to enumerate reachable targets, `"react"` to add an emoji reaction, `"unreact"` to remove one |

When `action` is `"react"` or `"unreact"`, pass the emoji as the `message` argument.

`action` values at a glance:

| `action` | `message` carries | Returns |
|----------|-------------------|---------|
| `"send"` | text (`" MEDIA:<path>"` optional) | Short summary string, e.g. `"Delivered to slack:#ops"` |
| `"list"` | ignored | JSON array of `{target, platform, kind, label}` objects |
| `"react"` | the emoji, e.g. `"👍"` | JSON string of a `ReactionResult`, e.g. `{"status": "ok"}` |
| `"unreact"` | the emoji to remove | JSON string of a `ReactionResult`, e.g. `{"status": "ok"}` |
...
agent = Agent(
    name="Support",
    instructions="""
    In busy group chats, tap send_message(action="react", target="origin",
    message="⏳") to acknowledge a request, then react with "✅" once the
    task is done instead of posting a full reply.
    """,
    tools=[send_message],
)
```
...
<Card title="Bot Gateway" icon="robot" href="/features/bot-gateway">
