...
A third, lower-level [`RunTerminal`](./run-terminal) primitive exists for gateway/ledger scenarios where several observers watch the same run and their racing signals must fold into one sticky outcome.
</Note>
...
<Card title="Run Terminal" icon="shield-check" href="/features/run-terminal">
Fold racing terminal signals from multiple observers into one authoritative, sticky outcome.
</Card>
</CardGroup>
