...
<Note>
Pending approvals can be made restart-safe by attaching an `ApprovalStore` to your approval handler — a late Allow/Deny tap after a bot restart still resolves. See [Durable Approvals](/docs/features/durable-approvals).
</Note>
