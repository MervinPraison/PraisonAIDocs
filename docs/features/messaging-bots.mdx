...
# WhatsApp (starts a webhook server on port 8080)
...
| Token | Format | When to Use |
|-------|--------|-------------|
| **Access Token** | `EAAx...` | **Always required.** Get from Meta for Developers → WhatsApp → API Setup. Used for sending messages via Cloud API. |
| **Phone Number ID** | `123456789` | **Always required.** Found in API Setup page. Identifies which WhatsApp business number to send from. |
