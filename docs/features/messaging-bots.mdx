...
**Channel key vs platform** — the YAML key under `channels:` (e.g. `telegram`, `telegram_cfo`) is just an identifier you choose. When the key is not a platform name, set `platform: telegram` (or whichever) inside the block so the gateway can pick the right protocol. This is what enables running multiple bots on the same platform.
