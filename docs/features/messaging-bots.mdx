...
<Note>
On at-least-once channels with `mark_recovered=True`, users may see a `♻️ Recovered reply — the gateway restarted during delivery, so this may be a duplicate.` prefix after a gateway restart — that is the honest label, not a bug. See [Durable Delivery](/features/durable-delivery) to enable it.
</Note>
