...
<Note>
On at-least-once channels with `mark_recovered=True`, users may see a `♻️ Recovered reply — the gateway restarted during delivery, so this may be a duplicate.` prefix after a gateway restart — that is the honest label, not a bug. See [Durable Delivery](/docs/features/durable-delivery) to enable it.
</Note>
...
See [Interactive Bot Actions](/docs/features/interactive-bot-actions).
...
<Note>
Pending approvals can survive a bot restart when you configure an [`ApprovalStore`](/docs/features/durable-approvals) — late Allow/Deny taps still resolve after a deploy.

**Telegram-native durable approvals.** On Telegram, setting `Agent(approval="presentation")` is enough — the bot auto-wires the durable backend to the chat on start and rehydrates pending approvals across restarts. See [Telegram Durable Approval](/docs/features/telegram-durable-approval).
</Note>
...
| `allow_silence` | `bool` | `false` | When `true`, agent may return `NO_REPLY` to send nothing. See [Intentional Silence](/docs/features/bot-intentional-silence). |
| `silence_token` | `str` | `None` | Custom silence marker (default: `NO_REPLY`) |
| `bot_loop_protection` | low-level guard primitive | — | Break runaway bot-to-bot reply loops. Currently a `BotLoopGuard` primitive you wire yourself (not yet a `Bot(...)` kwarg). See [Bot Loop Protection](/docs/features/bot-loop-protection). |
...
<Note>
**Sender name sanitisation is on by default.** Display names in group / multi-party chats are neutralised (newlines collapsed, control chars stripped, length-bounded to 240) before they are interpolated into the `[{sender}] ` attribution prefix, so a hostile name can't masquerade as a fake system directive. See [Sender Attribution Sanitisation](/docs/features/prompt-attribution-neutralisation) for details.
</Note>
...
**Interactive UI.** Telegram, Slack, and Discord channels support inline buttons, dropdown menus, and structured message blocks via [`MessagePresentation`](/docs/features/bot-presentations). Agents return a `MessagePresentation` and the channel adapter renders it as a native widget. WhatsApp uses plain-text fallback.
...
<Note>
Telegram also ships `/automations` and `/blueprint` for consent-first scheduled automations — see [Automation Suggestions](/docs/features/automation-suggestions). Other adapters don't register these yet.
</Note>
...
<Info>
See the [Multi-Channel Bots guide](/docs/features/multi-channel-bots) for complete setup instructions including the onboard wizard flow and validation with `praisonai doctor`.
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Handle rate limits gracefully">
    Outbound sends now honour the platform's `Retry-After` / `retry_after` automatically — both on live retries (`deliver_with_retry`) and on durable drains (`OutboundQueue`). To make the limiter widen the lane for the rest of your bot's concurrent sends, pass a shared `RateLimiter` into `deliver_with_retry(rate_limiter=...)`. See [Bot Rate Limiting](/docs/features/bot-rate-limiting) and [Durable Delivery](/docs/features/durable-delivery).
  </Accordion>
  ...
  <Accordion title="New: gateway does this for you">
    If you set `channels.<platform>.allow_shell: true` with `auto_approve_shell: false`, the gateway now wires `SlackApproval` / `TelegramApproval` / `DiscordApproval` automatically — no Python glue required. See [Bot Shell Execution](/docs/features/bot-shell-execution).
  </Accordion>
</AccordionGroup>
