...
| `unknown_user_policy` | `Literal["deny","pair","allow"]` | `"deny"` | How to handle messages from users not in `allowed_users`. `deny` silently drops, `pair` runs the owner-approval flow, `allow` lets everyone through. With an empty `allowed_users`, every user is "unknown" and this policy applies to all of them. Readable from `gateway.yaml` per-channel — see [Bot Gateway → Channel Security](/docs/features/bot-gateway#channel-security). |
