...
| `EmailTool` | Email (Gmail/SMTP) | `EMAIL_USERNAME`, `EMAIL_PASSWORD` | 🔜 Coming Soon |
...
By default, WhatsApp Web mode responds **only to your own messages** (self-messages). This prevents the bot from replying to every conversation on your account.

```mermaid
graph TD
    MSG["📩 Incoming Message"] --> CHECK{"IsFromMe?"}
    CHECK -->|"Yes"| ALLOW["✅ Process"]
    CHECK -->|"No"| ALLOW_LIST{"In allowlist?"}
