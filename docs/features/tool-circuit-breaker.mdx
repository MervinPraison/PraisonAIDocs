...
<Note>
**Per-agent scoping:** breakers are now keyed per agent instance (`tool_{id(self)}_{function_name}`), so two agents that expose same-named tools (e.g. `search`) no longer share one breaker — one agent's failures can't degrade the other. `agent.close()` / `aclose()` also removes that agent's breakers so a reused instance id can't inherit a stale OPEN breaker.
</Note>
