...
<Note>
**Fixed in [PR #4969](https://github.com/MervinPraison/PraisonAI/pull/4969)** (async parity): earlier releases counted each async outcome twice, so an async breaker actually opened at half the configured `failure_threshold` and closed at half the configured `success_threshold`. If you tuned these values to compensate on async workloads, you can now restore them to the values you'd use on sync.

A further double-record in `agent/execution_mixin.py` — where `breaker.acall(...)` records the outcome and two sites below it re-recorded the same failure — was closed in [PR #4958](https://github.com/MervinPraison/PraisonAI/pull/4958). Users who still saw async breakers opening at 3 failures despite `failure_threshold=5` after #4969 will now see the configured threshold honoured.
</Note>
...
class HealthCheckProtocol(Protocol):
    def health_check(self) -> bool: ...          # sync — return True if healthy
    async def ahealth_check(self) -> bool: ...   # async — return True if healthy
```

The protocol is exported from `praisonaiagents.tools.circuit_breaker`. Attach an implementation when creating a breaker:

```python
from praisonaiagents.tools.circuit_breaker import CircuitBreaker

class DatabaseHealth:
    def health_check(self) -> bool:
        return ping_database()

breaker = CircuitBreaker("database", health_check=DatabaseHealth())
```
...
<Note>
As of [PR #5050](https://github.com/MervinPraison/PraisonAI/pull/5050), `HealthMonitor` dispatches on the canonical `health_check` / `ahealth_check` names. Classes written against the public protocol are no longer silently reported unhealthy.
</Note>
...
Calling `agent.close()` / `agent.aclose()` triggers `_cleanup_circuit_breakers()`, which removes every `tool_{id(agent)}_*` entry from the registry earlier and deterministically — keeping it bounded and preventing a reused id from inheriting a stale `OPEN` breaker. See [Agent Lifecycle Cleanup](/features/agent-lifecycle-cleanup) for the full teardown story.
...
@tool
async def slow_upstream(query: str) -> str:
    """Async tool that hangs past the tool timeout."""
...
agent = Agent(
    name="Researcher",
    tools=[slow_upstream],
    tool_config=ToolConfig(timeout=2),
)

async def main():
    # After 5 consecutive timeouts, the 6th call short-circuits.
    for _ in range(6):
        await agent.achat("Search quantum computing")
