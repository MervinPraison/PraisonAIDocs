...
<Note>
**Sync and Async Parity**: Circuit breaker protection applies uniformly to both sync and async tool execution paths as of [MervinPraison/PraisonAI#4533](https://github.com/MervinPraison/PraisonAI/pull/4533). The async path (`agent.achat()`, `agent.astart()`) short-circuits via `_circuit_breaker_precheck(...)` before invoking a tool and records the outcome via `_circuit_breaker_record(...)` after. Both a **raised exception** inside the tool and a **result dict carrying `error`** count as breaker failures — but approval / permission / policy / guardrail denials do not. Earlier releases skipped the breaker on `achat()`/`astart()`.
</Note>
