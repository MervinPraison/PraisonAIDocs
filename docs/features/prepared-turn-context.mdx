...
Run the same context through your own runtime by implementing `TurnRuntimeProtocol`:
...
class MyHarness:
    async def run_turn(self, context: PreparedTurnContext) -> str:
        return f"Handled turn {context.correlation.turn_id} with {context.get_message_count()} messages."
...
asyncio.run(MyHarness().run_turn(context))
```
...
<Tip>
A fuller reference harness (with plugin- and CLI-backend variants) lives in the SDK repo at [`examples/runtime/example_harness.py`](https://github.com/MervinPraison/PraisonAI/blob/main/src/praisonai-agents/examples/runtime/example_harness.py). It is an example, not part of the installed `praisonaiagents` package — copy it into your project rather than importing it.
</Tip>
