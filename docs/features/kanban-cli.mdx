...
Run history and retry controls are currently agent-tool-only — there is no `praisonai kanban runs` subcommand or `--max-retries` flag on `praisonai kanban create` in the current CLI. Use the agent tools instead:

- `kanban_runs(task_id)` — read attempt history (outcomes, summaries, errors)
- `kanban_create("title", max_retries=N, idempotency_key="key")` — set per-task retry limit and dedup key

See [Kanban — Run History & Retries](/docs/features/kanban#run-history-kanban_runs) for full examples.
