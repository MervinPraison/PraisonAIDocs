...
<Note>
`export ALLOWED_TOOLS=` and `ALLOWED_TOOLS="$UNSET_VAR"` in a shell both produce
an empty string, which is treated as an error at startup (not "unset"). Set the
variable to a non-empty list, or don't set it at all. Presence is tested with
`is not None`, so `ALLOWED_TOOLS=""` does **not** fall through to
`HERMES_ONLY_TOOLS`.
</Note>
