...
The offloaded coroutine did not finish inside `timeout + 1s`. Check for blocking I/O or a missing `await`, or raise `PRAISONAI_RUN_SYNC_TIMEOUT` if the work is legitimately long-running.
...
```mermaid
graph LR
    subgraph "run_sync_or_offload"
        A{🔍 Running loop?} -->|No| B[🚀 run_sync]
        B --> C[🌉 Shared bridge]
        A -->|Yes| D[🧵 Offload thread]
        D --> E[🌉 Same bridge]
        C --> F[✅ Result]
        E --> F
    end
...
value = run_sync_or_offload(fetch())
```
</Step>
...
@app.post("/run")
def run(prompt: str):
    return {"answer": run_sync_or_offload(agent.astart(prompt))}
```
</Step>
</Steps>
...
| `thread_name` | `str` | `"praisonai-sync-offload"` | Name of the offload worker thread (shows up in stack traces). |
...
| Helper | Plain sync caller | Inside a running loop |
|--------|-------------------|-----------------------|
| `run_sync` | ✅ works | ❌ `RuntimeError` |
| `run_sync_or_offload` | ✅ works | ✅ offloads onto a worker thread (blocks the caller) |
| `arun_sync_or_offload` | — (must be awaited) | ✅ **non-blocking** await; the loop stays responsive |
...
<AccordionGroup>
<Accordion title="Prefer run_sync_or_offload for any 3-way surface (CLI + YAML + Python)">
The caller's context isn't knowable at write time — the same code path can be reached from a plain script, a YAML run, or a Python call inside a running loop. `run_sync_or_offload` handles all three without a `RuntimeError`, while still reusing the shared bridge and its connection pools.
...
<Accordion title="Keep run_sync for sync-only paths">
`run_sync` fails loudly inside a loop, which is what you want for a code path that must never be called from one — the `RuntimeError` surfaces an architectural bug instead of silently offloading.

```python
from praisonai._async_bridge import run_sync

def cli_only(prompt: str) -> str:
    return run_sync(agent.astart(prompt))
```
</Accordion>
</AccordionGroup>

This helper landed in [PraisonAI #3492](https://github.com/MervinPraison/PraisonAI/pull/3492); its callers (`praisonai.auto`, `persistence.orchestrator`, `api.agent_invoke`) are migrated onto it.
...
Per-session bridge that `run_sync_or_offload` preserves across the offload hop.
</Card>
...
Inside a running loop, `run_sync_or_offload` offloads onto a worker thread and blocks the caller until the coroutine finishes. `arun_sync_or_offload` awaits instead, so the loop keeps serving other requests.
...
<CardGroup cols={2}>
<Card title="run_sync_or_offload" icon="arrows-left-right" href="/features/async-bridge#run-sync-or-offload-safe-from-any-calling-context">
