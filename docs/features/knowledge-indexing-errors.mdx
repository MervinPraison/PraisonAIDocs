...
`add()` always returns these keys — `errors` is present even on success (as an empty list).
...
| `failed_chunks` | `int` | Count of chunks that embedded elsewhere in the file but were swallowed by the backend — a partial loss (new key) |
...
<Accordion title="Partial-chunk loss inside one file">
When some of a file's chunks embed and others are swallowed by the backend, the loss is no longer silent. `add()` reports the count in the new `failed_chunks` integer, and `index()` folds it into `result.errors` as:

```
{filepath}: {n} chunk(s) failed to store (partial index)
```

A file with `failed_chunks > 0` (but not *every* chunk failing) is a partial index — the corpus is incomplete even though the file appears in `results`.
</Accordion>
...
`Knowledge.index()` returns an `IndexResult` whose `errors` list carries both whole-file failures and the partial-chunk losses described above.
...
<Warning>
**`result.success` now reflects `errors`.** `index()` now sets `result.success = not result.errors`. Previously `success` was never assigned, so it kept the dataclass default of `True` — a run where **every** file failed to embed still returned `success=True` with `files_indexed=0` and each failure sitting in `errors`.

If your CI relied on `if result.success` on a pre-PR version, it could report success on a fully failed index. Check `if not result.errors` for behaviour that is correct across versions.
</Warning>
...
<Card title="RAG CLI" icon="terminal" href="/rag/cli#index">
