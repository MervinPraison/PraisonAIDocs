...
| Path | Endpoint | Key |
|------|----------|-----|
| litellm (default when installed) | `api_base` ← config `base_url` (or `OPENAI_BASE_URL` / `OPENAI_API_BASE`) | `api_key` ← config `api_key` (or `OPENAI_API_KEY`) |
| openai (fallback) | `base_url` ← config `base_url` (or `OPENAI_BASE_URL`) | `api_key` ← config `api_key` (or `OPENAI_API_KEY`) |

Config keys are read from the top level or a nested `"config"` dict, e.g. `Memory(config={"base_url": ..., "api_key": ...})`.

<Note>
The litellm path now honours config `base_url` / `api_key` (passed as litellm's `api_base` kwarg). Before [PR #4958](https://github.com/MervinPraison/PraisonAI/pull/4958) it silently went to OpenAI's default endpoint — users pointing memory at Groq, OpenRouter, Together, or a private OpenAI-compatible endpoint will now see the quality-scoring call go to the configured host.
</Note>
