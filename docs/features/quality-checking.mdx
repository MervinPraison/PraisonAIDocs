...
```mermaid
graph LR
    subgraph "Quality Checking"
        Task[📋 Task Output] --> Assess[🔍 Quality Assessment]
        Assess --> Score[📊 Quality Score]
        Score -->|Score > 0.7| Store[💾 Store in Memory]
        Score -->|Score ≤ 0.7| Discard[🗑️ Discard]
        Store --> Future[🔮 Future Reference]
    end

    classDef task fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef process fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef score fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef success fill:#10B981,stroke:#7C90A0,color:#fff
    classDef skip fill:#6366F1,stroke:#7C90A0,color:#fff

    class Task task
    class Assess process
    class Score score
    class Store,Future success
    class Discard skip
```
