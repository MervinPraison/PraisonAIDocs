...
sidebarTitle: "Model Router System"
...
```mermaid
graph TB
    Start([What matters most?]) --> Q1{Priority?}
    Q1 -->|Lowest cost| A[cost-optimized<br/>cheapest model that fits]
    Q1 -->|Best quality| B[performance-optimized<br/>most capable model]
    Q1 -->|Let router decide| C[auto<br/>match to task complexity]
...
sequenceDiagram
    ...
    Router->>Router: Analyze complexity + cost
    Router->>Agent: Selected model
    Agent->>Model: Run task
