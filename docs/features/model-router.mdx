...
sequenceDiagram
    ...
    participant ModelA as Fast Model
    participant ModelB as Smart Model

    User->>Agent: Request
    Agent->>Router: Evaluate request complexity
    alt Simple task
        Router->>ModelA: Route to fast model
        ModelA-->>Agent: Quick response
    else Complex task
        Router->>ModelB: Route to smart model
        ModelB-->>Agent: Thorough response
    end
    Agent-->>User: Best-fit response
```
