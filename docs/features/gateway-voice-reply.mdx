...
| `stream` | `bool` | `False` | Opt-in sentence-level streaming. When `True`, the reply is split into sentences and each is synthesised & delivered as it becomes available — time-to-first-audio drops from full-response latency to first-sentence latency. Falls back to a single whole-clip synth when off (default), when the reply is a single sentence, or when the reply is over `max_chars`. |
...
Streaming TTS synthesises and delivers your reply sentence-by-sentence, so the user hears the first clause while the rest is still being spoken.

With `stream: false` (default) the whole reply is synthesised into one clip before anything plays — the user waits for full-response latency. With `stream: true` time-to-first-audio drops to first-sentence latency.
...
channels:
  telegram:
    ...
    voice:
      mode: match_inbound
      stream: true      # NEW: synthesise & deliver sentence-by-sentence
```
...
sequenceDiagram
    ...
    User->>Adapter: Voice / text message
    Adapter->>Agent: chat(text)
    Agent-->>Adapter: 💬 Plain-text reply
    Adapter-->>User: 💬 Text reply (always first)
    loop for each sentence
        Adapter->>TTS: synthesize(sentence)
        alt success
            TTS-->>Adapter: 🎧 clip
            Adapter-->>User: 🎧 Voice clip
        else fail, nothing played
            TTS-->>Adapter: None → fall back to whole-file
        else fail, audio already playing
            TTS-->>Adapter: None → skip this clause, continue
        end
    end
```
...
```mermaid
graph TB
    Q{Reply is likely long<br/>and voice matters?} -->|no| Off[stream: false<br/>whole-clip synth]
    Q -->|yes| Chatty{Want the user to hear<br/>the first sentence ASAP?}
    Chatty -->|yes| Stream[stream: true]
    Chatty -->|no| Off

    classDef question fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef opt fill:#6366F1,stroke:#7C90A0,color:#fff

    class Q,Chatty question
    class Off,Stream opt
```
...
channels:
  telegram:
    ...
    voice:
      ...
      voice: nova
      stream: true             # sentence-level delivery (opt-in)
```
...
|----------|--------------------|-------|
| Telegram | ✅ Wired | Reference consumer — delivers via `send_voice`. Optional sentence-level streaming (`stream: true`) delivers a clip per sentence. |
...
<Accordion title="Stream long replies for lower time-to-first-audio">
...
<Card title="Streaming Replies" icon="bolt" href="/features/bot-streaming-replies">
