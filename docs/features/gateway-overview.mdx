...
| `gateway.durable_runs` | `bool` | `false` | Opt every gateway agent into durable runs (`ExecutionConfig(durable=True)`) so a restart resumes without re-firing tools or re-billing LLM calls. Per-agent `durable` overrides. See [Gateway Durable Runs](/features/gateway-durable-runs). |
...
<Note>
Gateway durable runs are **default-off** — enable with `gateway.durable_runs: true` in `gateway.yaml`. Per-agent `durable: true` opts a single agent in even when the gateway default is off. See [Gateway Durable Runs](/features/gateway-durable-runs).
</Note>
