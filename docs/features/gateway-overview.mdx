...
The gateway diffs `gateway.yaml` against the running config and restarts only affected agents or channels — and for three timing/logging keys, applies the change in place with no restart at all. Invalid YAML saves keep the last-known-good config. The WebSocket server is never restarted. See [Gateway Hot-Reload](/docs/features/gateway-hot-reload).
...
| `session.store` | `str` | `"sqlite"` | Transcript backend when `persist` is on — `"sqlite"` ([SQLite Transcript Store](/docs/features/sqlite-transcript-store)) or `"file"` (legacy JSON) |
