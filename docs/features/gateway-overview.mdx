...
The gateway exits with structured codes so process supervisors (systemd, s6, Kubernetes) can act correctly:

- **Exit `0`** — clean shutdown; supervisor stops.
- **Exit `75`** — transient failure (network blip, API error); supervisor should restart.
- **Exit `78`** — fatal configuration error; **supervisor must stop restarting** until the config is fixed.

Add `RestartPreventExitStatus=78` to your systemd unit to prevent crash-loops on bad config.

<Card title="Gateway Exit Codes" icon="circle-stop" href="/docs/features/gateway-exit-codes">
  Supervisor-friendly exit codes for systemd, s6, and Kubernetes — so transient blips restart and misconfigs don't crash-loop.
</Card>
