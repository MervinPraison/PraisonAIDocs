...
<Note>
**Not the same as Tool Output Spill.** This store handles arbitrary large tool return values with time-based retention under `~/.praisonai/cache/` (`PRAISONAI_TOOL_OUTPUT_RETENTION_HOURS`). [Tool Output Spill](/docs/features/tool-output-spill) is specific to `execute_command` and uses a different dir (`PRAISONAI_TOOL_OUTPUT_DIR`) with process-exit cleanup.
</Note>
