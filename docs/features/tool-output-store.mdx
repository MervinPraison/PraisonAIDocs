...
| 2 | Full text is written to `~/.praisonai/cache/tool_outputs/{run_id}/{call_id}.txt` — `run_id` is auto-threaded from the current agent run |
...
When no `run_id` is supplied — i.e. `get_tool_output_store()` is called with no argument outside of an agent run — the store returns a **fresh, un-cached instance** scoped to a new auto-generated UUID. It does **not** adopt whichever store happens to be live process-wide, so outputs from unrelated runs never share a directory.
...
**Pair with per-tool budgets** — combine with [Context Per-Tool Budgets](/features/context-per-tool-budgets) for fine-grained limits.
...
<Note>
**Not the same as Tool Output Spill.** This store handles arbitrary large tool return values with time-based retention under `~/.praisonai/cache/` (`PRAISONAI_TOOL_OUTPUT_RETENTION_HOURS`). [Tool Output Spill](/features/tool-output-spill) is specific to `execute_command` and uses a different dir (`PRAISONAI_TOOL_OUTPUT_DIR`) with process-exit cleanup.
</Note>
...
<Card title="Context Per-Tool Budgets" icon="chart-pie" href="/features/context-per-tool-budgets">
  Per-tool truncation limits
</Card>
<Card title="Context Window Management" icon="window-maximize" href="/features/context-window-management">
