...
<Note>
`_call_store` and `_dispatch_async` are internal plumbing on `PraisonAIDB`, not public API. They are named here only to describe the shared dispatch mechanism.
</Note>
