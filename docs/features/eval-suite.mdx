...
Every `EvalSuiteResult` also carries `.fingerprint` — an id for the setup this run was scored under; see [Run Fingerprint](/features/eval-run-fingerprint).
...
<CardGroup cols={2}>
  <Card title="Context Evaluator" icon="shuffle" href="/features/context-evaluator">
  ...
  <Card title="Eval SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/eval">
...
<Note>
Unparseable judge verdicts fail closed — an unreadable LLM response returns `score=1.0, passed=False`, so a low threshold never lets a broken agent pass the gate. See [LLM as Judge — Fail-Closed Parsing](/features/llm-as-judge#fail-closed-parsing).
</Note>
...
Guard with `assert_comparable` before comparing two runs, so scores from a moved setup are refused instead of silently charted.
...
<CardGroup cols={2}>
  ...
  <Card title="Eval Datasets" icon="database" href="/features/eval-datasets">
    Load task sets from JSONL and export passing runs as SFT data
  </Card>
  ...
  <Card title="Run Fingerprint" icon="fingerprint" href="/features/eval-run-fingerprint">
    Refuse to compare scores when the setup moved
  </Card>
</CardGroup>
