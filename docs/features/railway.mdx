...
`praisonai deploy railway` runs `railway up --detach` alone — it does **not** apply any `CloudConfig` fields yet. Set them in the Railway dashboard until CLI wiring lands ([PraisonAI PR #4876](https://github.com/MervinPraison/PraisonAI/pull/4876)).

```mermaid
graph LR
    CC[CloudConfig fields] --> W[logger.warning + metadata.unapplied]
    CC --> DASH[Set in Railway dashboard]
...
|---|---|---|
| `env_vars` | Not applied (names logged only) | Add as service variables in the Railway dashboard |
| `image` | Not applied | Set the service source image in Railway settings |
| `cpu` | Not applied | Set resources in the Railway dashboard |
| `memory` | Not applied | Set resources in the Railway dashboard |
| `region` | Not applied | Choose the region in the Railway dashboard |
| `min_instances` / `max_instances` | Not applied | Configure scaling in the Railway dashboard |
