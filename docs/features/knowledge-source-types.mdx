...
description: "What Agent(knowledge=...) accepts today — file paths, plain text, and cloud storage URIs"
icon: "book"
---

`Agent(knowledge=...)` accepts local file paths (any UTF-8 text file), plain text, and cloud storage URIs (`s3://`, `gs://`, `az://`, Azure Blob HTTPS) — arbitrary web URLs are not ingested and are skipped with a warning.
...
graph TB
    subgraph "Knowledge routing (per source in Agent(knowledge=[...]))"
        In[📥 Input string] --> IsCloud{Cloud URI?<br/>s3:// gs:// az://<br/>or Azure blob URL}
        IsCloud -->|Yes| Cloud[☁️ Download & re-enter]
        IsCloud -->|No| IsURL{http:// or<br/>https://?}
    ...
    class In in
    class IsCloud,IsURL,KnownExt,Exists decision
    class Cloud,Handler,ReadText,RawText process
...
| Cloud URI | `"s3://bucket/handbook.pdf"`, `"gs://…"`, `"az://…"`, Azure blob URL | Downloaded to temp, indexed with the same reader as local files. Original URI preserved in metadata. See [Cloud Sources](/features/knowledge-cloud-sources). |
| Web URL | `"https://example.com"` | **Warns and skips** — not ingested |
...
<Note>
**Cloud storage URIs are ingested.** `s3://`, `gs://`, `az://`, and Azure Blob HTTPS URLs are downloaded and indexed automatically — see [Cloud Sources](/features/knowledge-cloud-sources). Only ordinary web page URLs are skipped.
</Note>

<Warning>
Ordinary `http(s)://` web URLs are still skipped. `Agent(knowledge=["https://example.com"])` does not fetch the page. It logs:
