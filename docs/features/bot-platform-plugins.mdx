...
Five registration paths reach the same adapter class: three that need Python (built-ins, entry point, `register_platform()`) and two zero-code paths (YAML `adapter:` ref and `.praisonai/channels/` drop-in files). See [Custom Gateway Channel (Zero-Code)](/features/gateway-custom-channel) for the full zero-code story.
...
See [Custom Gateway Channel (Zero-Code)](/features/gateway-custom-channel) for the trust model, error surface, and copy-paste recipes.
...
Adapters compute structured `MentionFacts` (`explicit`, `reply_to_bot`, `quoted_bot`, `thread_participant`) and hand them to `resolve_ingress_admission(mention=…)`. Under `mention_only` / `observe`, a reply to the bot is admitted as an implicit mention (see [Implicit Mentions](/features/bot-implicit-mentions)). The primitive is back-compatible with the old `is_mention: bool` argument, so existing adapters keep working unchanged.
...
graph TB
    ...
    Policy -->|mention_only<br/>+ reply to bot| OK3["✅ allowed (implicit)"]
    Policy -->|mention_only<br/>+ !mention/cmd/reply| Drop4["🛑 group_mention_only"]
...
<Note>
See [Plugins → Message-Lifecycle Plugins](/features/plugins#message-lifecycle-plugins) for the enriched payload keys and runnable per-user, per-channel, and dead-letter examples.
</Note>
