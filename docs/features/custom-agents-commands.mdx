...
<Note>
The same `.praisonai/commands/{name}.md` file now also **runs** in bot chats (Telegram / Slack / Discord), not just the code REPL/TUI. Enable it per channel with a `commands` block (opt-in via `bots.commands`) — one file, two surfaces, no duplication. Shell (`` !`cmd` ``) substitution stays off in bots unless both the deployment and the command's frontmatter opt in. See [File-based & Entry-point Commands in Bot Chats](/features/bot-commands#file-based-and-entry-point-commands-in-bot-chats).
</Note>
