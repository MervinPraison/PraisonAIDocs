...
- **[Deferred & Progress Tools](/docs/features/deferred-and-progress-tools)** — `ToolProgress` + `on_progress=` callback + `defer()` from `praisonaiagents.tools`. Use when you want the **executor** to record progress on the `ToolResult` and support long-running deferred jobs.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Deferred & Progress Tools" icon="hourglass" href="/docs/features/deferred-and-progress-tools">
