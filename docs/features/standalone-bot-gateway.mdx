...
<Note>
`praisonai-bot gateway` is the canonical CLI. When the `praisonai` wrapper is co-installed, **every `praisonai gateway <subcommand>` delegates to it** — including `start`, `status`, `stop`, `restart`, `doctor` (`--fix` too), `test`, `channels`, `pause` / `resume` / `reconnect`, `install` / `uninstall`, `mint-link`, `logs`, `send`, `hooks`, and `sessions`. `praisonai gateway start` is the canonical safe start with credential / tool preflight and `--openai-api` / `--mcp` flags.
</Note>

```bash
# Canonical (bot-tier only)
praisonai-bot gateway start --host 127.0.0.1 --port 8765
praisonai-bot gateway doctor --config gateway.yaml

# Wrapper alias (requires pip install praisonai) — every subcommand works
...
See [Standalone Bot YAML](/features/standalone-bot-yaml) for details.
</Accordion>
</AccordionGroup>
