...
<Note>
For **non-message inbound events** (reactions, edits, deletions, member changes, thread creation), see [Inbound Platform Events](/features/bot-inbound-events). This page covers gateway/session/schedule lifecycle only.
</Note>
...
<Note>
This page covers **in-process outbound lifecycle hooks** (GATEWAY_START, SESSION_START, SCHEDULE_TRIGGER) that fire inside the running gateway. For **HTTP inbound triggers** that let external services start agent runs via `POST /hooks/<path>`, see [Gateway Inbound Hooks](/features/gateway-inbound-hooks).
</Note>
...
| `SCHEDULE_ADD` | A schedule is persisted by the store (`add`) — from the `schedule_add` tool, the `praisonai schedule` CLI, or the `config.yaml` reconciler | `ScheduleAddInput` | `job_name`, `job_id`, `schedule`, `message`, `agent_id`, `principal`, `enabled` |
| `SCHEDULE_REMOVE` | A schedule is deleted by the store (`remove`/`remove_by_name`) or auto-cleaned after a one-shot `delete_after_run` fires | `ScheduleRemoveInput` | `job_name`, `job_id`, `schedule`, `principal` |
| `SCHEDULE_TRIGGER` | Scheduled job runs in `BotOS._execute_schedule_job`. Fires exactly once per job even when multiple `BotOS` processes share the same store (see [BotOS → Multi-Process / HA Deployments](/docs/features/botos#multi-process-ha-deployments)). | `ScheduleTriggerInput` | `job_name`, `job_id`, `message` |
...
| `MESSAGE_UNDELIVERED` | A reply failed **permanently** (target confirmed dead or retries exhausted) — opt-in via [`gateway.notify_on_undelivered`](/features/gateway-undelivered-notice) | `MessageUndeliveredInput` | `platform`, `content`, `channel_id`, `error`, `notice_delivered` |
...
<Note>
**Three schedule events, three moments.** `SCHEDULE_ADD` fires when a job is **persisted**, `SCHEDULE_REMOVE` when a job is **deleted**, and `SCHEDULE_TRIGGER` when a job **fires**. Add/remove are emitted by the schedule store (so they fire regardless of source); trigger is emitted by `BotOS` when the job actually runs. A one-shot `delete_after_run` job walks the full `SCHEDULE_ADD → SCHEDULE_TRIGGER → SCHEDULE_REMOVE` path.
</Note>
...
<Note>
**Exception — `MESSAGE_RECEIVED` is now a real gate.** Since PR #2589, returning `HookResult.deny(...)` from `MESSAGE_RECEIVED` drops the inbound message, and `HookResult(decision="allow", modified_input={"content": "…"})` rewrites it in place before the agent sees it. See [Inbound Message Gate](/features/inbound-message-gate).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Hook Events Reference" icon="list" href="/features/hook-events">
  ...
  <Card title="Session Management" icon="users" href="/features/session-persistence">
