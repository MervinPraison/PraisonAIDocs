...
| `MESSAGE_UNDELIVERED` | A reply failed **permanently** (target confirmed dead or retries exhausted) — opt-in via [`gateway.notify_on_undelivered`](/docs/features/gateway-undelivered-notice) | `MessageUndeliveredInput` | `platform`, `content`, `channel_id`, `error`, `notice_delivered` |
