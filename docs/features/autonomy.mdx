...
`AutonomyConfig.sandbox` is a forward-compat placeholder — no autonomy level auto-enables sandboxing and nothing reads the field. Use `Agent(sandbox=…)` to isolate model-generated code, or `AgentFlow(run_on="docker")` for a whole workflow.
</Accordion>
</AccordionGroup>
