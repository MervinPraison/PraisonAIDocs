...
`AutonomyConfig.sandbox` is a forward-compat placeholder — no autonomy level auto-enables sandboxing and nothing reads the field. Use `Agent(sandbox=…)` to enable the explicit `agent.execute_code()` API, or `AgentFlow(run_on="docker")` to isolate a whole workflow in a real container.
</Accordion>
</AccordionGroup>
