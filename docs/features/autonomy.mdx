...
<Note>
Replaces the deprecated `verification_hooks=[...]` kwarg — see [Legacy Agent Parameters](/features/agent-legacy-params).
</Note>

<Note>
The string form of `autonomy=` must name a valid preset (`full_auto`, …). A typo raises `ValueError` at construction time with a "Did you mean ...?" suggestion — see [Fail-Loud Defaults](/features/fail-loud-defaults). Matching is case-insensitive, whitespace-tolerant, and treats `-`/`_` interchangeably.
</Note>

```python
from praisonaiagents import Agent

# All four spellings enable the same 'full_auto' preset:
Agent(instructions="t", autonomy="full_auto")
Agent(instructions="t", autonomy="full-auto")
Agent(instructions="t", autonomy="FULL_AUTO")
Agent(instructions="t", autonomy=" full_auto ")

# Typo raises — no silent fallback to a disabled autonomy:
Agent(instructions="t", autonomy="fll_auto")
# ValueError: Unknown preset 'fll_auto' for parameter 'autonomy'.
#             Did you mean 'full_auto'? Valid presets: auto_edit, full_auto, suggest
```

| Call (Python) | Before PR #4186 | After PR #4186 |
|---|---|---|
| `Agent(autonomy="full_auto")` | ✅ autonomy enabled | ✅ autonomy enabled |
| `Agent(autonomy="full-auto")` | ⚠ validated → **silently disabled** | ✅ autonomy enabled |
| `Agent(autonomy=" full_auto ")` | ⚠ validated → **silently disabled** | ✅ autonomy enabled |
| `Agent(autonomy="FULL_AUTO")` | ⚠ validated → **silently disabled** | ✅ autonomy enabled |
| `Agent(autonomy="fll_auto")` (typo) | ❌ `ValueError` with suggestion | ❌ `ValueError` with suggestion |
...
roles:
  refactor_bot:
    ...
    backstory: "Meticulous senior engineer"
    autonomy: 5           # -> auto_edit preset
    tasks:
      refactor_module:
        description: "Refactor auth.py to use JWT"
        expected_output: "A cleaner auth.py"
```
...
<Tip>
When you have a real acceptance criterion for "done" (e.g. "a PR exists"), use [`agent.run_goal(...)`](/features/goal-loop) instead of the heuristics — it gates completion on an independent judge instead of guessing from keywords.
</Tip>
...
<Note>
**Doom-loop stops are now approval-driven.** A detected loop routes through the approval pipeline (default: stop). A legitimate repeat can continue with `doom_loop=allow` — see [Approval → Doom-loop safety gate](/features/approval#doom-loop-safety-gate).
</Note>

<Note>
**Safety controls belong together.** `max_iterations` and `doom_loop_threshold` cap *how long* an agent runs; `max_budget_usd` and `max_tokens` cap *how much it spends*. Set both when running unattended — see [Autonomy Budget](/features/autonomy-budget) for the spend/token kill-switch.
</Note>
...
<Accordion title="Don't rely on AutonomyConfig.sandbox for isolation">
`AutonomyConfig.sandbox` is a forward-compat placeholder — no autonomy level auto-enables sandboxing and nothing reads the field. Use `Agent(sandbox=…)` to enable the explicit `agent.execute_code()` API, or `AgentFlow(run_on="docker")` to isolate a whole workflow in a real container. See [Placement](/features/placement).
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card icon="robot" href="/features/autonomy-loop">
  Autonomy Loop — deep dive into iterative autonomy mode
</Card>
<Card icon="wallet" href="/features/autonomy-budget">
  Autonomy Budget — cap the money and tokens a run can burn
</Card>
<Card icon="bullseye-arrow" href="/features/goal-loop">
