...
```mermaid
graph LR
    subgraph "Reasoning Data Generation"
        G[📋 Generate Q&A] --> E[🔍 Evaluate]
        E --> C[🧠 CoT Steps]
        C --> U[✅ Upload Dataset]
    end
...
sequenceDiagram
    ...
    participant CoTGenerator
    participant HuggingFace

    User->>Generator: start("Generate reasoning data")
    Generator->>Generator: Create Q&A pairs
    Generator->>CoTGenerator: Q&A pairs
    CoTGenerator->>CoTGenerator: Expand into reasoning steps
    CoTGenerator->>HuggingFace: Upload dataset
    HuggingFace-->>User: Dataset published
```
...
<AccordionGroup>
  <Accordion title="Validate every generated step with Pydantic">
  ...
  <Accordion title="Evaluate before you accept">
  ...
  <Accordion title="Use decision tasks to control the loop">
...
<CardGroup cols={2}>
  <Card icon="brain" href="/features/reasoning">
    Enable step-by-step reasoning inside a single agent run.
  </Card>
  <Card icon="workflow" href="/features/workflows">
