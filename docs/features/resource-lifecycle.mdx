...
### HTTP invoke API — no manual cleanup needed

```python
# In an n8n / HTTP invoke deployment you do NOT need to wrap invokes
# in with / try/finally — the wrapper closes each per-request clone.
# Just register the template once and serve.

from praisonaiagents import Agent
from praisonai.api.agent_invoke import register_agent

register_agent("assistant", Agent(name="assistant", instructions="…"))
# praisonai serve agents.yaml — every POST /invoke closes its clone.
```

See [n8n API → Per-request cleanup](/features/n8n-api).
...
<CardGroup cols={2}>
<Card title="Advanced Memory System" icon="brain" href="/features/advanced-memory">
...
<Card title="Agent Lifecycle Cleanup" icon="broom" href="/features/agent-lifecycle-cleanup">
  Per-agent teardown of clients, MCP tools, and breakers
</Card>
