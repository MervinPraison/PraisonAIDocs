...
<Note>
**Multi-replica gateways:** the shared turn lock stamped onto channel bots is in-process only. If you scale the gateway to `replicas > 1`, enable a distributed backend — see [Gateway Turn Lock](/features/gateway-turn-lock).
</Note>
