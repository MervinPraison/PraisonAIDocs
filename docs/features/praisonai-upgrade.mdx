...
`--check` also warms the background update-check cache, so long-lived installs surface the [update hint](/features/praisonai-update-hint) on next start without their own network round-trip.
...
Leave the [background update hint](/features/praisonai-update-hint) on. `--check` warms its cache, so subsequent starts remind you to upgrade without any network call.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="praisonai uninstall" icon="trash" href="/features/praisonai-uninstall">
    Cleanly remove the managed install
  </Card>
  <Card title="Update Hint" icon="bell" href="/features/praisonai-update-hint">
