---
title: "Messaging Channels Strategy"
sidebarTitle: "Messaging Channels Strategy"
description: "PraisonAI messaging channel coverage, roadmap, and competitive positioning"
icon: "users-gear"
---
...
agent = Agent(name="Support", instructions="Help users on every connected channel.")
botos = BotOS(agent=agent, platforms=["telegram", "discord"])
agent.start("Draft a welcome message for new Telegram and Discord users.")
```
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Messaging Channels Strategy
...
- **BotOS Core**: 9 production-ready platforms (including Signal) with 24/7 orchestration
...
| **Signal** | ✅ Production | `SignalBot` | E2E encryption, linked-device bridge, DMs + groups, allowlist | ✅ |
| **Local** | ✅ Production | `LocalBot` | Terminal (stdin/stdout), token-free, owner-trusted, cross-channel continuity | ✅ |

### When to pick local

- **Zero-setup dev / smoke test** — talk to a gateway agent before wiring any remote channel.
- **Single-user personal agent** on your own box — no bot registration, no token.
- **Cross-channel continuity** from your terminal to a remote channel under one session.

See [Local (Terminal) Channel](/features/local-channel) for the full reference.

### Which channel do I add?

```mermaid
graph TB
    Q{Where will your<br/>operator sit?}
    Q -->|My own terminal| L[local — zero setup, token-free]
    Q -->|Phone / DM| T[telegram / signal / whatsapp]
    Q -->|Team chat| S[slack / discord]
    Q -->|Email inbox| E[email / agentmail — also token-free]
    Q -->|Ticketing| I[linear]
...
`local` and `email` / `agentmail` / `signal` are **token-free** — they need no bearer token to start. See [Degraded Channel Isolation](/features/gateway-degraded-channels) for the token-free allowlist.
...
See [Bot Platform Plugins](/features/bot-platform-plugins) for complete documentation on creating distributable bot platform plugins.
</Note>
...
<AccordionGroup>
  <Accordion title="Start with one channel, then scale via BotOS">
  ...
  <Accordion title="Write channel-agnostic instructions">
  ...
  <Accordion title="Match platform choice to your audience">
  ...
  <Accordion title="Plan for community plugins over forks">
...
<CardGroup cols={2}>
<Card title="Microsoft Teams RFC" icon="microsoft" href="/features/rfc-microsoft-teams-integration">
