...
agent = Agent(name="code-agent", instructions="Write and review Python code.")
agent.start("Write a function to check if a number is prime.")
```
...
graph LR
    subgraph "AI Code Editing"
        Agent[🤖 Agent] --> Read[📖 Read File]
        Agent --> Write[✍️ Write File]
        Agent --> Diff[🔄 Apply Diff]
        Read --> Content[📄 File Content]
        Write --> File[💾 Updated File]
        Diff --> Search[🔍 SEARCH Block]
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as AI Code Editing
...
When you launch `praisonai code` from the terminal, the CLI resolves the workspace from `--workspace` / `PRAISONAI_WORKSPACE` / the resumed session's directory and calls `set_workspace` for you (see [Workspace directory](/docs/cli/code#workspace-directory)). Call `set_workspace` yourself only when embedding the code tools in your own Python script.
...
<CardGroup cols={2}>
  <Card title="Code Interpreter" icon="terminal" href="/docs/features/codeagent">
