...
graph LR
    subgraph "Context Compression"
        Request[📋 User Request] --> Process[⚙️ Context Compression]
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Context Compression
...
agent = Agent(
    name="compress-agent",
    instructions="Compress conversation history to save context space.",
    compress_context=True,
)
agent.start("Summarise older messages to free context window space.")
```
...
graph LR
    subgraph "Context Compression"
        In[📝 Retrieved Chunks] --> Dedup[✂️ Deduplicate]
        Dedup --> Extract[🧠 Query Extract]
        Extract --> Agent[🤖 Agent]
        Agent --> Out[✅ Fitted Context]
    end
...
sequenceDiagram
    ...
    Agent->>Knowledge: Retrieve chunks
    Knowledge-->>Compressor: Large chunk set
    Compressor-->>Agent: Compressed, query-focused text
    Agent-->>User: Answer within budget
```
...
<Note>
Memory backends can implement the `on_pre_compress` hook to extract and persist important facts before compression discards messages. See [Memory Lifecycle Hooks](/features/memory-lifecycle-hooks) for details.
</Note>
...
<Card title="Token Budgeting" icon="coins" href="/features/token-budgeting">
