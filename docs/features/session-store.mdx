...
See [Gateway Session Portability](/features/gateway-session-portability) for the operator-facing backup / migrate / restore story.
...
Wrap any store — `DefaultSessionStore`, `SqliteSessionStore`, `HierarchicalSessionStore`, or a custom backend — with `EncryptedSessionStore` so message content and metadata are encrypted before they reach disk.
...
store = EncryptedSessionStore(
    SqliteSessionStore(db_path="~/.praisonai/sessions.db"),
    key=os.environ["PRAISONAI_SESSION_KEY"],
)
```

See [Encrypted Session Store](/features/encrypted-session-store) for keys, honest limits, and migration.
...
| `EncryptedSessionStore` | Wrap any session store to encrypt message content and metadata at rest — see [Encrypted Session Store](/features/encrypted-session-store) |
| `SessionEncryptionError` | Raised on missing/invalid keys, wrong-key decryption, or `search()` calls on an encrypted store |
| `generate_session_key()` | Generate a Fernet key suitable for `PRAISONAI_SESSION_KEY` |
...
| Indexed content | `_flatten` concatenates `archived_messages` (compacted turns) ahead of the active `messages`, so compacted history stays searchable (Issue #5031). |
| Content version | `INDEX_CONTENT_VERSION = 2`. Persisted in the `session_index_meta` table (`content_version` key). A stale version forces a one-time FTS rebuild on startup. |
...
### Upgrade note: archived-history reindex

<Note>
As of Issue #5031, `SqliteSessionStore` also indexes `archived_messages` (turns compacted out of the active window), bumping `INDEX_CONTENT_VERSION` to `2`. On the **first startup after upgrade**, the store detects the stale version and rebuilds every session's FTS row once so archived-only queries work immediately — rather than only after each session's next write. No user action is required: startup runs a bounded backfill, then persists the new version so later startups fall back to the cheap skip-already-indexed path. A new internal `session_index_meta` key/value table (`content_version` key) tracks the version, so operators upgrading an existing store should expect that table to appear in the index file.
</Note>

See [Cross-Session Recall → Recall Across Compacted History](/features/cross-session-recall#recall-across-compacted-history) for the caller-facing behaviour.
...
<CardGroup cols={2}>
  ...
  <Card title="Encrypted Session Store" icon="lock" href="/features/encrypted-session-store">
    Encrypt transcripts at rest by wrapping any store
  </Card>
</CardGroup>
