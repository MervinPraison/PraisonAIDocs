...
`SqliteSessionStore` is a drop-in subclass of `DefaultSessionStore` that keeps JSON transcripts as the durable record but maintains a stdlib `sqlite3` + FTS5 index of message content. `search()` becomes a bounded `MATCH ... ORDER BY bm25` lookup rather than an `O(all sessions)` directory scan.
...
| Aspect | Detail |
|--------|--------|
| Backend | Stdlib `sqlite3` (lazy-imported); FTS5 virtual table `session_fts(session_id UNINDEXED, content)` + `session_meta(session_id, updated_at)` |
| Query | `SELECT session_id FROM session_fts WHERE session_fts MATCH ? ORDER BY bm25(session_fts) LIMIT ?` |
| Fallback | If FTS5 is unavailable → plain `LIKE` table. If `sqlite3` fails entirely → parent `DefaultSessionStore.search` scan. |
| Backfill | Legacy JSON transcripts are indexed once, on first `search()` (guarded by a one-time flag; only session ids not already in the index are (re)indexed). |
| Write sync | `_save_session`, `add_message`, `_modify_session_locked` (covers `set_chat_history` and `append_compaction_checkpoint`), `clear_session`, and `delete_session` all keep the index in sync. |
...
|-------------|-----------|
| `sqlite3` + FTS5 present (default) | Bounded FTS5 `MATCH` lookup + bm25 ranking |
| `sqlite3` present, FTS5 unavailable | Bounded `LIKE` lookup against a plain table |
| `sqlite3` unavailable or index open fails | Transparent fallback to `DefaultSessionStore.search` full-directory scan |
