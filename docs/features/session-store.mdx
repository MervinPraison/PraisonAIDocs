...
2. Tagged legacy per-agent records (`parent_session_id` + `agent_key`) — migrated forward into the parent's `agent_histories` on the next save.
...
Legacy records are **never deleted**. Concurrent saves under the same parent are safe (bounded read-verify-write, up to 5 retries). `list_sessions()` still surfaces `agent_key` / `parent_session_id`, now used for legacy-record discovery.
</Note>
