...
<Note>
**Chat-history restore reads `SessionStore` first, with `Memory` as fallback.** When a session resumes, `Session._restore_agent_chat_history()` (and the bulk `_restore_agent_chat_histories()`) load history from `SessionStore` before falling back to `Memory` for backward compatibility. Sessions saved via `SessionStore` (the default writer) now restore correctly on resume — previously they could return empty history when save and restore targeted different stores. Bulk restore disambiguates composite session ids using the `agent_key` tag written by `_save_agent_chat_histories`; legacy tagless sessions still fall back to prefix parsing. `SessionStore.list_sessions()` surfaces `agent_key` alongside `parent_session_id` in each entry.
</Note>
