...
<Note>
`Agent(knowledge=[…])` accepts file paths, directory paths, and inline text — **not URLs**. Since PraisonAI PR [#4004](https://github.com/MervinPraison/PraisonAI/pull/4004) any `http(s)://` entry in the list is logged with a warning and skipped instead of silently dropped. To ingest a web page, fetch and pass its text, or download it to a local file first.
</Note>
...
|--------|------|---------|-------------|
| `sources` | `list[str]` | `[]` | Files, directories, or raw text to index (URLs not yet ingested) |
...
<Warning>
**Remote URLs are not yet ingested.** `Knowledge.add(url)` raises
`NotImplementedError("URL processing not yet implemented")` and
`Agent(knowledge=[url])` warns and skips. Download the file first, then add the
local path:
