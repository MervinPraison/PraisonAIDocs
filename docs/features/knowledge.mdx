...
description: "Add document knowledge to agents for grounded, accurate answers"
icon: "book"
---
...
<Note>
`Agent(knowledge=[…])` accepts file paths, directory paths, inline text, and cloud storage URIs (`s3://`, `gs://`, `az://`, Azure Blob HTTPS) — see [Cloud Sources](/features/knowledge-cloud-sources). Arbitrary web URLs are skipped: since PraisonAI PR [#4004](https://github.com/MervinPraison/PraisonAI/pull/4004) any `http(s)://` entry is logged with a warning and skipped instead of silently dropped. To ingest a web page, fetch and pass its text, or download it to a local file first.
</Note>
...
<Card title="KnowledgeConfig SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/feature_configs">
...
<Note>
`path` is optional. Omit it and data is stored under an absolute per-project directory: `<project>/.praisonai/knowledge/chroma`. See [Knowledge Storage](/features/knowledge-storage) for details.
</Note>
...
<Note>
When mem0's MongoDB vector store is configured as the knowledge backend, an upstream `TypeError` on `.search()` is caught and search returns empty results — the same fallback behaviour the memory-layer path uses. See [Memory Troubleshooting → `memory="mem0"` empty results](/features/memory-troubleshooting#why-is-memory-mem0-returning-empty-search-results-mongodb-vector-store).
</Note>
...
<Note>
`semantic`, `sdpm`, and `late` chunkers require the `[knowledge]` extra (it pulls in `chonkie` and its embedding backend). Without the extra, the first lookup raises `ImportError: chonkie package not found. Please install it using: pip install 'praisonaiagents[knowledge]'` (PraisonAI PR [#5105](https://github.com/MervinPraison/PraisonAI/pull/5105)). `sentence`, `paragraph`, `fixed`, and `token` chunkers do not need the embedding backend.
</Note>
...
See [Cloud Sources](/features/knowledge-cloud-sources) for providers, credentials, and error handling.
</Note>
...
| `ImportError: chonkie package not found. Please install it using: pip install 'praisonaiagents[knowledge]'` when calling `agent.start(...)` with `chunking_strategy="semantic"` (the default), `"sdpm"`, or `"late"`, or with any string `embedding_model` | The `chonkie` package is not installed. The default chunker is `semantic`, so this fires on the first knowledge lookup even if you never set `chunking_strategy` explicitly. Since PraisonAI PR [#5105](https://github.com/MervinPraison/PraisonAI/pull/5105) the error surfaces with an actionable hint instead of a raw `ModuleNotFoundError`. | `pip install 'praisonaiagents[knowledge]'` (quotes required in zsh). Or switch to a chunker that does not need chonkie embeddings by passing `KnowledgeConfig(chunking_strategy="sentence")` or `"paragraph"`. |
| `ImportError` mentioning `chonkie.embeddings` but **without** the `pip install` hint | `chonkie` **is** installed but its `embeddings` submodule failed to import (typically a broken or incompatible transitive dependency such as `sentence-transformers`, `torch`, or `transformers`). Since PraisonAI PR [#5105](https://github.com/MervinPraison/PraisonAI/pull/5105) the original error is preserved so the real cause is not masked behind misleading install advice. | Read the underlying error and reinstall or upgrade the named package. `pip install --upgrade sentence-transformers` fixes the most common case. |
...
See [Async Safety](/features/async-safety) for the full set of async guarantees.
...
<CardGroup cols={2}>
  ...
  <Card title="Cloud Sources" icon="cloud" href="/features/knowledge-cloud-sources">
