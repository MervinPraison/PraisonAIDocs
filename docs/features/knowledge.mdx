...
agent = Agent(knowledge=["docs/", "report.pdf"])

# Level 2: Dict (inline config)
agent = Agent(knowledge={"sources": ["docs/"], "embedder": "openai"})

# Level 3: KnowledgeConfig (full control)
...
| `embedder_config` | `dict \| None` | `None` | Provider-specific embedder settings |
...
| `chunker` | `dict \| None` | `None` | Dict-based chunker config (alternative to direct fields) |
| `retrieval_k` | `int` | `5` | Number of chunks to retrieve per query |
| `retrieval_threshold` | `float` | `0.0` | Minimum similarity score to include a chunk |
| `rerank` | `bool` | `False` | Apply reranking to improve result order |
| `rerank_model` | `str \| None` | `None` | Reranking model to use |
| `auto_retrieve` | `bool` | `True` | Auto-inject knowledge into context |
| `vector_store` | `dict \| None` | `None` | Vector database configuration |
| `config` | `dict \| None` | `None` | Full config dict for advanced use |
