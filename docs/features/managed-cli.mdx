...
description: "List and reclaim managed sandboxes, and manage Anthropic-hosted sessions, agents, and environments from the terminal"
icon: "terminal"
---
...
agent = Agent(name="managed-agent", instructions="Use managed CLI commands for automation.")
agent.start("Run the managed deploy command for production.")
```
...
graph LR
    subgraph "Managed CLI"
        ...
        B --> C[☁️ Anthropic API]
        B --> D[🐳 Local + cloud sandboxes<br/>docker / e2b / modal / …]
        C --> E[📋 Session / Agent / Env]
        D --> F[📦 Sandbox instance]
    end
...
| Command | Purpose | Example |
|---------|---------|---------|
| `managed ps` | List every running agent sandbox across all providers | `praisonai managed ps` |
| `managed ps --json` | Machine-readable output (`{"sandboxes": [...], "errors": [...]}`) | `praisonai managed ps --json` |
| `managed ps --provider docker` (or `-p`) | Filter to one provider | `praisonai managed ps -p docker` |
| `managed stop <instance-id>` | Reclaim one sandbox by ID | `praisonai managed stop docker_a1b2c3d4` |
| `managed stop --all` | Reclaim every running sandbox (across providers) | `praisonai managed stop --all` |
| `managed stop --all --provider docker` | Reclaim every sandbox from one provider | `praisonai managed stop --all -p docker` |

Sandboxes normally self-destruct when a run ends. A crashed or killed run leaves one behind, and `ps` / `stop` are the way to see and reclaim it.
...
graph TB
    ...
    Sandbox --> PsSB[ps - List running sandboxes]
    Sandbox --> StopSB[stop - Reclaim a sandbox]
    ...
    class Start,Sessions,Agents,Envs,Sandbox,IDs resource
    class ListS,GetS,ResS,DelS,ListA,GetA,UpdA,DelA,ListE,GetE,UpdE,DelE,PsSB,StopSB,Save,Show,Restore action
```
...
<Card title="HostedAgent + DB persistence" icon="cloud-database" href="/features/managed-agent-persistence">
...
<Card title="Shared Sandbox" icon="share-nodes" href="/features/shared-sandbox">
  Share one sandbox across a team or workflow with `tools_run_on=`
</Card>
</CardGroup>