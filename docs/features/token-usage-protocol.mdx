...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Token Usage Protocol
...
| `cached_tokens` | `int` | Cached tokens — a **subset** of `input_tokens`, not additive |
| `total_tokens` | `int` | Total tokens (`input_tokens + output_tokens`) |
| `cost` | `float` | Estimated USD cost |

`cached_tokens` is a subset of `input_tokens`, not a separate bucket — it counts prompt tokens the provider served from cache. The same holds for `reasoning_tokens` and audio tokens, which are subsets of the totals the provider already reports. See [How `total_tokens` is computed](/features/token-tracking#session-summary-shape) for the full explanation.
...
<Card title="SQLite Usage Sink" icon="database" href="/features/token-usage-sqlite-sink">
  Durable, per-identity implementation of this protocol
</Card>
</CardGroup>