...
<Note>
The **sync** path now runs `adapter.run()` inside a per-call `scoped_bridge()` (async unchanged), so a stuck coroutine in one run cannot park the shared loop for others. See [Async Bridge → Sync `generate_crew_and_kickoff` auto-scoping](/features/async-bridge#sync-generate_crew_and_kickoff-auto-scoping).
</Note>
...
- **Auto-call site:** `AgentsGenerator.generate_crew_and_kickoff` (sync) and `agenerate_crew_and_kickoff` (async) bracket the entire adapter lifecycle — `setup` + `run`/`arun` — in `observability_session(adapter.name)`. On context exit the session finalizes with `status="Failure"` when an exception is propagating and `status="Success"` otherwise. **Adapters must not finalize themselves.** This closes the AutoGen leak: the AutoGen adapter previously never called `finalize_observability`, so every AutoGen run left an `ObservabilityRun`, a `_swapped_runs` entry, and sink handles alive. For workflow YAMLs, the session is tagged with the config-resolved framework (registry default when omitted) rather than a hardcoded `"praisonai"` label — a fix landed in [PraisonAI PR #5039](https://github.com/MervinPraison/PraisonAI/pull/5039).
...
#### Per-task isolation under asyncio

Each asyncio task gets its own run stack, so the void-signature `finalize_observability(tag)` is safe even when N concurrent `arun()` tasks share one worker thread.

The run stack lives in a `contextvars.ContextVar`, not `threading.local()`. asyncio copies the context per task (`asyncio.create_task`, `asyncio.to_thread`, `copy_context`), so `_run_stack.get()` in each task returns that task's own stack — task A's `finalize_observability("crewai")` pops task A's run, never task B's. A thread-local slot would be shared by every task on the thread, letting task A close task B's sinks mid-run and restore another tenant's emitter.

The value is treated as **immutable (copy-on-write)**: `_push_run`, `_pop_run`, and `_remove_run` always `set` a brand-new list, and `_get_run_stack()` returns a copy. A `ContextVar` copied into a child context shares the same underlying list object, so mutating it in place would let a spawned task pop from its parent's stack.

```mermaid
sequenceDiagram
    participant T1 as Task 1
    participant T2 as Task 2
    participant CV as _run_stack ContextVar

    T1->>CV: _push_run(run1) → set [run1]
    T2->>CV: _push_run(run2) → set [run2]
    Note over T1,T2: Shared _run_stack ContextVar — each task sees a per-task copy
    T1->>CV: _get_run_stack() → [run1]
    T2->>CV: _get_run_stack() → [run2]
    T1->>CV: finalize_observability(tag) → pop run1
    T2->>CV: finalize_observability(tag) → pop run2
```

Prefer the explicit-handle form `finalize_observability(tag, run=my_run)` wherever you hold the handle; the void form is the compatibility shim for legacy call sites. Both are now concurrency-safe — the void form's `_pop_run()` reaches only this context's stack.

The regression suite `test_observability_run_stack_isolation.py` is the canonical guarantee: `_push` does not mutate a copied context, `_pop` in a copied context leaves the parent intact, `_get_run_stack` returns a copy, two concurrent asyncio tasks get isolated stacks, and `_remove_run` is copy-on-write. Fixed in PR [#4948](https://github.com/MervinPraison/PraisonAI/pull/4948) (fixes [#4945](https://github.com/MervinPraison/PraisonAI/issues/4945)).
...
<CardGroup cols={2}>
<Card title="AgentOps" icon="robot" href="/observability/agentops">
...
<Card title="Custom Tracing" icon="plug" href="/observability/custom-tracing">
