...
|--------|------|---------|-------------|
| `reflection=True` | `bool` | off | Enable with default limits (1–3 iterations). |
| `ReflectionConfig.min_iterations` | `int` | `1` | Minimum reflection passes. |
| `ReflectionConfig.max_iterations` | `int` | `3` | Maximum reflection passes. |
| `ReflectionConfig.llm` | `str \| None` | `None` | Model for reflection (defaults to agent model). |
| `ReflectionConfig.prompt` | `str \| None` | `None` | Custom evaluation prompt. |
...
<CardGroup cols={2}>
<Card title="ReflectionConfig" icon="gear" href="/docs/configuration/reflection-config">
  Full configuration reference
</Card>
<Card title="Self Improve" icon="arrows-spin" href="/docs/features/self-improve">
