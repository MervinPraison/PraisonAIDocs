...
| Table | `make_table(columns, rows)` | Tabular data |
| Chart | `make_chart(chart_kind, series)` | Chart/visualisation |
| Poll | `make_poll(question, options)` | Group vote — native poll or question + reply-buttons degrade |

Table and chart blocks default to `supports_tables=False` / `supports_charts=False` on every channel, so channel authors get an automatic markdown-table / text-summary degrade for free — see [Table & Chart Blocks](/features/table-chart-blocks).

Poll blocks default to `supports_native_poll=False` on every channel, so a group vote degrades to a question plus reply-buttons until a renderer adds a native-poll branch — see [Poll Blocks](/features/poll-blocks).
...
For adapters that want to surface downgrades to the user, use [`adapt_presentation_with_report()`](/features/degraded-delivery) — it returns the adapted presentation **plus** a typed `DegradedDelivery` report of what was downgraded (with machine-readable reason codes).
...
<Note>
See [Presentation Renderers](/features/presentation-renderers) for the full registry API, the built-in renderer table, and an "add a channel" recipe.
</Note>
...
When the underlying bot uses `MessagePresentation.approval(...)`, the button namespace is automatically actor-bound — see [Interactive Callback Authorization](/features/interactive-callback-authorization).
...
`TelegramBot` shares one `InMemoryCallbackPayloadStore` between its renderer and inbound registry, so long values round-trip by default. See [Interactive Callback Payload Store](/features/interactive-callback-store) for a dedicated walkthrough.
...
<CardGroup cols={2}>
  ...
  <Card icon="signal-slash" href="/features/degraded-delivery">
    Typed report of controls a channel could not render natively
  </Card>
</CardGroup>
