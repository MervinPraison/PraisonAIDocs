...
sequenceDiagram
    ...
    Workflow->>Agent: Execute initial task
    Agent-->>Workflow: Result
    Workflow->>Condition: Evaluate condition
    alt Condition true
        Condition-->>Workflow: Continue path A
        Workflow->>Agent: Execute Task A
    else Condition false
        Condition-->>Workflow: Continue path B
        Workflow->>Agent: Execute Task B
    end
    Agent-->>Workflow: Final result
    Workflow-->>User: Output
```
