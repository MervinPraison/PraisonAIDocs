...
At gateway boot, a `configured-but-unavailable` or `missing` credential now isolates just that channel instead of aborting the whole gateway — see [Degraded Channel Isolation](/docs/features/gateway-degraded-channels).
