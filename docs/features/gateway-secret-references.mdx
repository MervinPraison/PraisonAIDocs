...
At gateway boot, a `configured-but-unavailable` or `missing` credential now isolates just that channel instead of aborting the whole gateway — see [Degraded Channel Isolation](/features/gateway-degraded-channels).
...
<AccordionGroup>
  ...
  <Accordion title="Rotated secret files pick up only on a hot-reload">
    A rewritten `{ source: file }` reference is re-read on a config hot-reload, not on a bare `praisonai gateway reconnect`. If a channel parked in `CREDENTIAL_UNAVAILABLE` after a runtime token rejection, trigger a hot-reload so the new file value is resolved. See [Runtime credential rejection](/docs/features/gateway-channel-supervision#runtime-credential-rejection).
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Gateway Credential Rotation" icon="arrows-rotate" href="/features/gateway-credential-rotation">
    Rotating the gateway's own auth token.
  </Card>
  <Card title="Standalone Bot YAML" icon="file-code" href="/features/standalone-bot-yaml">
