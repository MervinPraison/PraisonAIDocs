...
<Note>
`CodeAgent(model="gpt-4o")` is now accepted (canonical). `llm=` still works as a deprecated alias — see [Model Parameter](/features/model-parameter).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Prefer safe execution mode">
    Keep `code_mode="safe"` unless you explicitly need direct host execution and accept the trade-offs. Safe mode runs code in a subprocess and enforces the `ExecutionConfig(timeout=...)` as a hard per-call limit; `"unsafe"` mode runs in-process and enforces **no** hard timeout, so a runaway loop can block the worker. Choose `"safe"` whenever the timeout must be a guarantee.
  </Accordion>
</AccordionGroup>
