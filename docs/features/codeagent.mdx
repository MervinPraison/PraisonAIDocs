...
<AccordionGroup>
  ...
  <Accordion title="Prefer safe execution mode">
    Keep `code_mode="safe"` unless you explicitly need direct host execution and accept the trade-offs. Safe mode runs code in a subprocess and enforces `ExecutionConfig(timeout=...)` as a hard per-call limit; `"unsafe"` mode runs in-process and has **no hard timeout** — a runaway loop blocks the worker. Choose `"safe"` whenever the timeout must be a guarantee.
  </Accordion>
</AccordionGroup>
