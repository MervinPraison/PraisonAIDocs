...
```python
# Removed — do not use in new projects (see Warning above)
...
graph LR
    subgraph "Scientific Paper Generation"
        A[Research Topic] --> B[Scientific Writer]
        B --> C[LaTeX Paper]
    end
...
```python
# Historical API — removed in PR #1641
from praisonaiagents import ScientificWriterAgent
...
<Warning>
All patterns below use the removed `ScientificWriterAgent` API. They are kept for historical reference only and will not run with current releases.
</Warning>
