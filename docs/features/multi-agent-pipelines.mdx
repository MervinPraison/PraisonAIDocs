...
sequenceDiagram
    ...
    participant Transcriber as AudioAgent
    participant Summariser

    User->>Pipeline: start(audio_file)
    Pipeline->>Transcriber: transcribe(input)
    Transcriber-->>Pipeline: transcript
    Pipeline->>Summariser: {{previous_output}}
    Summariser-->>Pipeline: summary
