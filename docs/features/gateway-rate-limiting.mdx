...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Gateway Rate Limiting
...
<CardGroup cols={2}>
  ...
  <Card title="Admission Control" icon="shield" href="/features/gateway-admission-control">
    Bound concurrent turns with a fair queue and overflow policy
  </Card>
  <Card title="Gateway Security" icon="lock" href="/features/gateway-operator-scopes">
