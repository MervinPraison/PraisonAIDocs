...
<Note>
Tool turns are treated specially: a mid-response network failure surfaces `provider_outcome_unknown` instead of silently re-running the tool. See [Replay-Safe Retries](/features/replay-safe-retries).
</Note>
...
<Note>
`async def` tools work identically on the sync tool loop (`chat()` / `start()`) and the async tool loop (`achat()` / `astart()`) as of [PraisonAI PR #4634](https://github.com/MervinPraison/PraisonAI/pull/4634). This applies to plain-callable tools, `BaseTool` subclasses (`.run` may be `async def`), LangChain-style `.run` methods, and CrewAI-style `._run` methods. The sync-side bridge that awaits the tool body was relocated to the stable `praisonaiagents.agent.async_safety.run_async_in_sync_context` helper in [PraisonAI PR #5005](https://github.com/MervinPraison/PraisonAI/pull/5005) so the guarantee survives the eventual removal of the deprecated `UnifiedExecutionMixin` ([Issue #2644](https://github.com/MervinPraison/PraisonAI/issues/2644)).
</Note>
</Step>
</Steps>
...
<Tip>
Drop a `@tool`-decorated function into `.praisonai/tools/*.py` for auto-load with no imports required — see [Project-local tools](/docs/features/custom-agents-commands#project-local-tools).
</Tip>
...
Declare approval right on the `@tool` decorator — the agent pauses and asks a human before the tool runs. See [Tool Approval](/features/tool-requires-approval).
...
|--------|------|-------|-------------|
| `approval` | `bool` | before the call | Pause and ask a human before the tool runs. See [Tool Approval](/features/tool-requires-approval). |
| `trust` | `str` | on the result | Wrap external tool output in a prompt-injection fence. |
| `input_guardrails` | `callable \| list` | before the call | Validate or rewrite the arguments before the tool runs. See [Per-Tool Guardrails](/features/per-tool-guardrails). |
| `output_guardrails` | `callable \| list` | on the raw result | Validate or substitute the result before it re-enters the LLM context. See [Per-Tool Guardrails](/features/per-tool-guardrails). |
...
<CardGroup cols={2}>
<Card icon="wrench" href="/features/tool-config">
  Tool Config — timeouts, retries, and artifact storage
</Card>
<Card icon="magnifying-glass" href="/features/tool-search">
