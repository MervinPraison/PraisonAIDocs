...
Load configuration from multiple sources with precedence — project config in `project_dir` overrides user, which overrides global.

```python
from praisonai.cli.features.config_hierarchy import HierarchicalConfig, load_config

# Precedence: project > user > global
# 1. .praison.json / praison.json (in project_dir, defaults to os.getcwd())
...
See [Hierarchical Configuration](/docs/features/hierarchical-config) for the full precedence diagram, config schema, and monorepo patterns.
