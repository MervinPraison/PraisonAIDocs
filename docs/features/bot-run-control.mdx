...
<Note>
Extended in [PR #4600](https://github.com/MervinPraison/PraisonAI/pull/4600) (merged 2026-09-02): `/stop` now also unblocks a turn parked on a blocking gateway approval wait — previously the turn hung silently even after `/stop` was acknowledged. A reviewer decision that arrives after the stop is dropped fail-closed, so no stale tool fires and no stale reply lands. See [Approval Turn Liveness](/features/gateway-approval-turn-liveness).
</Note>
...
<Note>
`SessionRunControl` now accepts an optional `on_supersede` callback (invoked as `(user_id, generation)` whenever a run is stopped or interrupted). `TelegramBot` wires this automatically when `busy_mode` is set. Custom integrators building `SessionRunControl` directly can pass `on_supersede=make_approval_supersede_callback()` to cancel pending gateway approvals bound to the abandoned turn. See [Approval Turn Liveness](/features/gateway-approval-turn-liveness).
</Note>
...
<Card title="BotConfig API Reference" icon="code" href="/sdk/reference/typescript/classes/BotConfig">
...
When the bot is configured with `busy_mode` and a gateway approval is pending, `/stop` also cancels the pending approval via the `on_supersede` callback wired automatically by `TelegramBot` (and any bot that opts into `SessionRunControl`). See [Approval Turn Liveness](/features/gateway-approval-turn-liveness).
</Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Message Steering" icon="compass" href="/features/message-steering">
  ...
  <Card title="Approval Turn Liveness" icon="hourglass-end" href="/features/gateway-approval-turn-liveness">
    `/stop` unblocks a turn parked on a pending approval; a late reviewer tap is dropped fail-closed
  </Card>
</CardGroup>