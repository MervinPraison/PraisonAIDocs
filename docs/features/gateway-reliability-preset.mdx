...
| Profile | Drain window | Admission ceiling | Overflow behaviour | Outbound ordering |
|---|---|---|---|---|
| `production` | 15 s | CPU-scaled `max_concurrent_runs` | Bounded fair-wait queue (`overflow=queue`) | `strict` |
| `default` / `None` (unset) | 5 s | none | pass-through | `best_effort` |
| `off` | 0 s (immediate teardown) | none | today's no-backpressure behaviour | `best_effort` |

The `production` preset enables strict per-conversation FIFO delivery on the outbox — see [Outbound Ordering](/features/outbound-ordering).
