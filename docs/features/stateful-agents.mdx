...
<Steps>
<Step title="Create a persistent session with memory">
```python
from praisonaiagents import Session

session = Session(session_id="research_001", memory={"user_id": "user_123"})
agent = session.Agent(
    name="Assistant",
...
<Step title="Save and restore state">
```python
session.save_state({"research_topic": "AI Safety", "progress": 50})

new_session = Session(session_id="research_001", memory={"user_id": "user_123"})
state = new_session.restore_state()
...
sequenceDiagram
    ...
    User->>Session: Session(session_id, memory)
    Session->>Memory: initialize STM + LTM
    User->>Session: session.Agent(name, memory=True)
    Session->>Agent: create with memory context
    User->>Agent: agent.chat(message)
    Agent->>Memory: search_long_term(query)
    Memory-->>Agent: relevant memories
    Agent-->>User: response
    Agent->>Memory: store_long_term(result)
```
...
<Tip>
For multi-turn chats on Anthropic or Google, use `build_context_for_task()` with `include_in_output=True` for cache-friendly prompt assembly. See [Prompt Caching](/features/prompt-caching).
</Tip>
...
Include a timestamp or task identifier (e.g., `research_2024-12`) so `restore_state()` and `replay list` are easy to navigate. Save state at key milestones, not just at the end.
</Accordion>

<Accordion title="Memory strategy: quality thresholds">
Use individual quality metrics (`completeness`, `relevance`, `clarity`, `accuracy`) with `store_long_term()` and set `min_quality=0.8` for `search_long_term()` to keep noise out of context.
</Accordion>

<Accordion title="State design: consistent naming">
Use descriptive snake_case keys. Use `has_state()` before accessing optional keys. Leverage `increment_state` and `append_to_state` instead of get-modify-set patterns.
</Accordion>

<Accordion title="Quality management: monitor distribution">
Periodically call `memory.search_with_quality(min_quality=0.0)` to audit stored memories. If average quality drifts below threshold, re-index or raise the storage threshold.
</Accordion>
</AccordionGroup>
...
<Card title="Graph Memory" icon="diagram-project" href="/features/graph-memory">
  Entity and relationship storage via Neo4j/Memgraph
</Card>
