...
<Tip>
For multi-turn chats on Anthropic or Google, prefer `build_cache_optimized_context()` to get a stable prefix and cache boundary. See [Prompt Caching](/features/prompt-caching).
</Tip>
