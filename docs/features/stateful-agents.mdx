...
sequenceDiagram
    ...
    User->>Agent: "Remember my name is Alice"
    Agent->>State: Store {name: Alice}
    Agent->>LLM: Acknowledge
    LLM-->>Agent: Confirmation
    Agent-->>User: "Got it, Alice!"

    User->>Agent: "What is my name?"
    Agent->>State: Retrieve stored state
    State-->>Agent: {name: Alice}
    Agent->>LLM: Question + state context
