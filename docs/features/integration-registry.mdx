...
#4176](https://github.com/MervinPraison/PraisonAI/pull/4176), this guard lives
in the base `PluginRegistry` — not per-registry — so `IntegrationRegistry` and
...
deliberately override a built-in. See [Plugin Precedence](/features/plugin-precedence)
for the full rule across every registry.
</Note>
...
|---|---|---|
| Third-party entry-point plugin | On first registry access | Skipped; built-in kept (`DEBUG` log) |
| Built-in integration | Registered on construction | Wins over plugin |
| `register_lazy(...)` / `register(...)` at runtime | Whenever the caller invokes it | Last write wins (can override a built-in) |

Built-ins can never be silently shadowed by a same-named entry-point plugin. Since PR #4176 the base `PluginRegistry` skips the colliding entry point during discovery and keeps the built-in — runtime `register(...)` remains the deliberate override path.
...
<Note>
`list_external_agents` and `external_agent_catalog` cover the `praisonai.external_agents` entry-point group (short-name `--external-agent` choices and UI toggles), **not** the `praisonai.integrations` group documented on this page. A listed name is always a usable name — see [Single source of truth](/features/external-cli-integrations#single-source-of-truth).
</Note>
...
Built-ins always win on name collisions. Since [PR #4176](https://github.com/MervinPraison/PraisonAI/pull/4176)
this rule is enforced in the base `PluginRegistry`, so it holds for every
...
`CLAUDECODEINTEGRATION` are the same key. See [Plugin Precedence](/features/plugin-precedence).
</Accordion>
</AccordionGroup>
