...
<Note>
`flyio` and `tenki` are compute providers (`run_on=` / `compute=`) only — they are not `Agent(sandbox=…)` backends. `sandlock`, `subprocess`, `ssh` and `novita` are sandbox backends only — they are not `run_on=` providers.
</Note>
