...
PR #2003 exposes all eight sandboxes through `SandboxRegistry` — selectable by string name from the CLI or Python.
...
| `novita` | `NovitaSandbox` | Novita cloud sandbox (requires the `novita` extra + `NOVITA_API_KEY`) |
...
The eight backends live in a dedicated package so you can use them without pulling in the full `praisonai` wrapper.
