...
sandbox_cls = registry.resolve("daytona")  # or docker, e2b, modal, novita, ssh, sandlock, subprocess
sandbox = sandbox_cls()
```
</Tab>
</Tabs>
...
<Note>
Novita requires `pip install "praisonai-sandbox[novita]"` (or `pip install "praisonai[novita]"`) and a `NOVITA_API_KEY` environment variable. The sandbox reads credentials from the environment at startup and fails loudly with `RuntimeError` if the key is missing.
</Note>
