...
### The 11 places `tools_run_on=` accepts

`tools_run_on=` (and per-call `run_in=`) now reach **11** places — up from 7. `subprocess`, `sandlock`, `ssh` and `novita` previously spoke only `SandboxProtocol` (one object *is* one environment) and could not be selected; an adapter now supplies the missing half, making `sandlock` (kernel-enforced Landlock + seccomp-bpf) reachable for the first time.

| Place | One-line description |
|------|----------------------|
...
<Warning>
`subprocess` is **not** a security boundary. Its blocked-command list is bypassable by ordinary shell syntax (`cat $(echo /etc/passwd)` reads the file). For untrusted code, use `docker`, a cloud place, or `sandlock`. See [Placement](/features/placement).
</Warning>
