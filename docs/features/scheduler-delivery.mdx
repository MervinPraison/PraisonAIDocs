...
A delivered brief is a resumable conversation by default — reply in the same chat and the agent picks up where the job left off, no IDs, no configuration.
...
agent = Agent(
    name="Scheduler",
    instructions="Schedule reminders on demand.",
    tools=[schedule_add],
)

# Continuable by default — reply to the delivered brief resumes the job.
agent.start("Send me a daily revenue brief at 9am to Slack")
# schedule_add(name=..., schedule="cron:0 9 * * *", message=..., deliver="slack")
# continuable=True is the default, no argument needed.
```
...
sequenceDiagram
    ...
    Scheduler->>Agent: run(job.message)
    Agent-->>Scheduler: brief text
    Scheduler->>Gateway: _deliver_scheduled_result(target, brief)
    Gateway->>Channel: send_message(brief)
    Gateway->>Store: _seed_continuable_session(reply_key, [job.message, brief])
    User->>Channel: "why did revenue dip on Tuesday?"
    Channel->>Gateway: inbound reply (same reply_key)
    Gateway->>Store: resume(reply_key) — brief in context
    Gateway->>Agent: continue(...)
```

### Opt out for fire-and-forget notices

Set `continuable=False` (Python/YAML) or `--no-continuable` (CLI) when a reply should start a fresh session — correct for one-shot notices like "backup finished" pings.
...
agent = Agent(name="Backup", instructions="Post the backup-complete notice")
target = DeliveryTarget(channel="slack", channel_id="C0123456", continuable=False)
AgentScheduler(agent, task="Notice", deliver=target).start("daily")
```
</Tab>
...
<Tab title="CLI">
```bash
praisonai schedule add "backup-notice" \
  -s daily \
...
### `continuable` at a glance

| `continuable` | User reply behaviour |
|---|---|
| `True` (default) | Reply in the same chat/thread resumes the job's session with the delivered brief in context. |
| `False` | Reply starts a new, contextless session (pre-#3449 behaviour). Correct for one-shot notices. |

### Which mode fits which scenario?

```mermaid
graph TB
    Start{What happens when the user replies?} -->|Ask follow-up questions on this brief| Keep[Keep the default<br/>continuable=True]
    Start -->|One-shot notice, reply should start fresh| Opt[continuable=False<br/>--no-continuable]
...
<Note>
Seeding is best-effort. A seed failure logs and never breaks the delivery itself — the brief still lands in the channel.
</Note>

<Tip>
`continuable=False` is only persisted when set — existing `DeliveryTarget` records on disk remain byte-identical to pre-#3449, so upgrading is a no-op for stored schedules.
</Tip>
