...
## Reading the outcome from stats {#delivery-outcomes}

Every scheduled run lands in exactly one of four delivery outcomes, and each outcome bumps its **own** counter — `delivered_deliveries` or `undelivered_deliveries` — read from `scheduler.get_stats()`.

```mermaid
graph LR
    Run[⏰ Scheduled run] --> Outcome{Delivery outcome}
    Outcome -->|Router accepted| Delivered[✅ DELIVERED<br/>delivered_deliveries +1]
    Outcome -->|Router said no / raised| Undelivered[🟥 UNDELIVERED<br/>undelivered_deliveries +1]
    Outcome -->|NO_REPLY / SILENT| Suppressed[🟡 SUPPRESSED<br/>0 / 0]
    Outcome -->|No deliver= target| NotConfigured[🟦 NOT_CONFIGURED<br/>0 / 0]
    ...
    classDef notconfig fill:#6366F1,stroke:#7C90A0,color:#fff

    class Run run
    class Outcome q
    class Delivered delivered
    class Undelivered undelivered
    class Suppressed suppressed
    class NotConfigured notconfig
```
...
The typed `DeliveryOutcome` (importable from `praisonai.scheduler._base_scheduler`) has a `.undelivered` convenience property that returns `True` only for `UNDELIVERED`.
...
<Note>
A scheduled `UNDELIVERED` run best-effort fires the existing `HookEvent.MESSAGE_UNDELIVERED` on the agent's attached hook runner. The event payload is built via `MessageUndeliveredInput` — it includes the agent name, the resolved platform + channel_id, the run's text, and `error="scheduled result could not be delivered"`. This reuses the existing seam — no new protocol or setup. It fires only when a hook runner is attached to the agent and never raises. See [Undelivered Messages](/features/undelivered-messages) and [Intentional Silence](/features/bot-intentional-silence).
</Note>

<Note>
**What did *not* change.** [PraisonAI PR #4464](https://github.com/MervinPraison/PraisonAI/pull/4464) adds **no** new stat, knob, or protocol — it only makes the two existing counters explicit (initialised to `0` on every scheduler instance, bumped only for their own outcome) and pins their per-outcome semantics with regression tests. The invariant above holds for both the sync and async scheduler and will continue to.
</Note>
---
