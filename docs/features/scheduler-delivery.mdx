...
The marker check calls `is_intentional_silence_response` from `praisonaiagents.bots.silence` — the same primitive the chat-bot path uses. Both `AgentScheduler._deliver_result` (lightweight path) and the full-gateway `ScheduledAgentExecutor` honour it.
