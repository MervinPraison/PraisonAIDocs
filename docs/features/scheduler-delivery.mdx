...
Run `praisonai schedule tick` as a cron entry / CI job / serverless function and each job's `deliver:` target is delivered via a stateless HTTP call — no gateway process required.
...
|---|---|---|
| `TELEGRAM_BOT_TOKEN` | Telegram | Bot token — required for Telegram delivery |
| `SLACK_BOT_TOKEN` | Slack | Bot token — required for Slack delivery |
| `DISCORD_BOT_TOKEN` | Discord | Bot token — required for Discord delivery |
| `TELEGRAM_HOME_CHANNEL` | Telegram | Default chat id when `deliver: telegram` has no `:channel_id` |
| `SLACK_HOME_CHANNEL` | Slack | Default channel id for bare `deliver: slack` |
| `DISCORD_HOME_CHANNEL` | Discord | Default channel id for bare `deliver: discord` |

### Chat-id resolution order

A bare `deliver: telegram` target (no `:channel_id`) resolves its destination in this order:

| Order | Source | Notes |
|---|---|---|
| 1 | `deliver: telegram:123456` explicit `channel_id` | Highest precedence |
| 2 | `TELEGRAM_HOME_CHANNEL` env var | Deployment override |
| 3 | `~/.praisonai/state/home_channels.json` | Populated by `/sethome` on a live gateway; reused out-of-process with no extra config |
...
| Scenario | Result |
|---|---|
| Token env unset | Run succeeds; `delivery_error` recorded (never silently dropped) |
...
| **Out-of-process delivery (new — #3726)** | When `praisonai schedule tick` runs with no live gateway, a per-platform standalone sender (Telegram / Slack / Discord) posts via a token-authenticated HTTPS call. Zero new dependencies. |
