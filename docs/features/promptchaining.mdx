...
<Steps>
    ...
    <Step title="Create a file">
        ...
        def process_data(ctx: WorkflowContext) -> StepResult:
            return StepResult(
                output=f"Processed: {ctx.input.upper()}",
        ...
        def analyze_results(ctx: WorkflowContext) -> StepResult:
            ...
            return StepResult(output=f"Analysis complete: {prev}")

        # Step 4: Generate final output
        def generate_output(ctx: WorkflowContext) -> StepResult:
            return StepResult(output=f"Final output: {ctx.previous_result}")
        ...
        # Run the workflow
        print("\nStarting Prompt Chain Workflow...")
        print("=" * 50)

        result = workflow.start("Hello World from PraisonAI")
