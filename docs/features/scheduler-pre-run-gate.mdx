...
<Note>
**Gate vs action:** `--pre-run` is a **gate** — a cheap check that decides whether the model turn happens. If you instead want to run a shell command *instead of* the model turn and deliver its stdout verbatim (no agent, no tokens), use the [Scheduler Command Action](/features/scheduler-command-action) (`--command`).
</Note>
...
<Note>
The gate fires **after** the [atomic claim](/docs/features/async-scheduler#multi-process-safety-at-most-once) and **before** the actual run, so only the worker that won the lease evaluates the gate.
</Note>
