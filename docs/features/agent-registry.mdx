...
A real gateway calls `registry.resolve(inbound_number)` on every message, then hands the chat to the chosen agent. See [WhatsApp Bot](/features/whatsapp-bot) for the full bot setup.
...
Pick by message direction: `AgentRegistry` routes **inbound** by phone number; [`ChannelDirectory`](/features/platform-aware-agents) routes **outbound** by target name.
