...
| Unknown top-level / agent field | Key not in schema | Warn + ignore field | Re-emitted as a `[strict mode]` error by `ConfigValidator` |

When both `roles:` and `agents:` are present, `roles:` wins for task→agent resolution.

<Note>
Seeing a run finish with `Task completed.` and **0 agents**? An empty `roles: {}` is treated the same as a missing one — a populated `agents:` block is still promoted into roles. See [Empty `roles: {}` is treated as missing](/features/yaml-configuration-reference#empty-roles-is-treated-as-missing).
</Note>
...
Any YAML key the schema doesn't declare is flagged, then ignored — so a typo like `instrucions:` doesn't silently drop your instructions.

```yaml
foo: bar                 # unknown top-level key
...
agents:
  myagent:
    ...
    goal: Find facts
    instructions: "Research the topic"
    bar: baz             # unknown agent key
```

```
Unknown top-level field 'foo'. This field will be ignored.
agents.myagent: Unknown field 'bar'. This field will be ignored.
```

The field lists are **derived from the Pydantic schema**, not hand-maintained:

- Top-level keys are checked against `YAMLConfig.model_fields`.
- Agent-block keys are checked against `AgentConfig.model_fields | {"name"}`.

Adding a field to `YAMLConfig` or `AgentConfig` automatically teaches the validator — there is no second list to edit. This is why `type`, `deploy`, and `dependencies` (added to `YAMLConfig` in [PraisonAI PR #4651](https://github.com/MervinPraison/PraisonAI/pull/4651)) stopped warning the moment they were declared on the schema.

<Warning>
**Task-only keys on an agent still warn.** `expected_output`, `output_file`, `async_execution`, `context`, and `condition` belong on a **task**, not an agent. The validator intentionally does *not* fold task fields into the agent set — the runtime ignores them on an agent, so a warning is the correct signal:

```
agents.myagent: Unknown field 'expected_output'. This field will be ignored.
```
</Warning>

<Note>
Unknown-field checks run inside `ConfigValidator._check_unknown_fields` and are always emitted as **warnings**. Under `PRAISONAI_VALIDATE_STRICT=true`, `ConfigValidator` promotes every warning to an error with a `[strict mode]` prefix, so an unknown field then fails validation. The duplicate-name / task-ref checks above raise `ValueError` directly under strict mode.
</Note>
...
<Card title="Tool Timeouts" icon="stopwatch" href="/features/tool-timeout">
  The other silent-collapse fix: per-agent `tool_timeout` is now honoured per agent (no cross-agent downgrade)
</Card>
<Card title="Workflow Validation Loop" icon="rotate" href="/features/workflow-validation">
