...
<Note>
The model context window used for the 80% trigger is resolved from litellm's `model_cost` registry first, so auto-compaction fires at the correct point for any litellm-supported model (Mistral, Bedrock, Azure, OpenRouter, versioned ids) — no override needed. See [How the context window is resolved](/features/context-budgeter#how-the-context-window-is-resolved).
</Note>
