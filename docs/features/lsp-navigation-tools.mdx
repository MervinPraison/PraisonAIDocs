...
description: "Zero-config, language-server-accurate go-to-definition, find-references, hover, symbol search, call hierarchy, and go-to-implementation for agents"
icon: "magnifying-glass-code"
---

Give your agent language-server-accurate go-to-definition, find-references, call hierarchy, implementation lookup, and symbol search that works out of the box — the right server is picked from the file extension, and a missing server degrades to an actionable install hint instead of a silent no-op.
...
<Note>
**New in the toolset:** call hierarchy (`lsp_incoming_calls`, `lsp_outgoing_calls`) and go-to-implementation (`lsp_implementations`) now join the navigation tools — the auto-detect flow above stays the same.
</Note>
...
agent.start("Where is `compute` defined, who calls it, and which classes implement `Storage`?")
```
...
Call-hierarchy tools first issue `callHierarchy/prepareCallHierarchy` to resolve a `CallHierarchyItem`, then fan out to `callHierarchy/incomingCalls` or `callHierarchy/outgoingCalls`. Multiple prepared items are queried and duplicates are collapsed automatically.
...
graph TB
    ...
    Q --> R["Where is X used?"]
    Q --> IC["Who calls X?"]
    Q --> OC["What does X call?"]
    Q --> IM["Which classes implement X?"]
    ...
    subgraph "Incoming Calls"
        IC --> lsp_inc[lsp_incoming_calls]
    end

    subgraph "Outgoing Calls"
        OC --> lsp_out[lsp_outgoing_calls]
    end

    subgraph "Implementations"
        IM --> lsp_imp[lsp_implementations]
    end
    ...
    class Q question
    class lsp_def,lsp_ref,lsp_inc,lsp_out,lsp_imp,lsp_hov,lsp_doc,lsp_wks tool
```
...
| `lsp_implementations` | `(file_path, line=None, character=None, symbol=None)` | Concrete implementations behind an interface / abstract method | `textDocument/implementation` |
| `lsp_incoming_calls` | `(file_path, line=None, character=None, symbol=None)` | Callers of a symbol (who calls this?) | `callHierarchy/incomingCalls` |
| `lsp_outgoing_calls` | `(file_path, line=None, character=None, symbol=None)` | Callees of a symbol (what does this call?) | `callHierarchy/outgoingCalls` |
...
Every `lsp_*` navigation tool embeds the hint in its error string:
...
Position-taking tools (`lsp_definition`, `lsp_references`, `lsp_hover`, `lsp_implementations`, `lsp_incoming_calls`, `lsp_outgoing_calls`) accept two addressing styles:
...
**Before renaming a function, see every real caller**

Call hierarchy resolves the symbol first, so it catches dynamic dispatch and skips same-named identifiers a `grep` would over- or under-match:

```python
from praisonaiagents.tools import lsp_incoming_calls, lsp_outgoing_calls

print(lsp_incoming_calls("src/utils.py", symbol="compute"))
print(lsp_outgoing_calls("src/utils.py", symbol="compute"))
```

**Find every concrete implementation of an interface**

```python
from praisonaiagents.tools import lsp_implementations

print(lsp_implementations("src/storage/base.py", symbol="Storage"))
```
