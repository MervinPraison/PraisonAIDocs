...
sequenceDiagram
    participant User
    participant AutoAgent as AutoAgents
    participant Planner as Task Planner
    participant Agents as Agent Pool
    participant LLM

    User->>AutoAgent: Complex task description
    AutoAgent->>Planner: Analyze and decompose
    Planner-->>AutoAgent: Sub-tasks
    loop For each sub-task
        AutoAgent->>Agents: Assign to agent
        Agents->>LLM: Execute
        LLM-->>Agents: Result
        Agents-->>AutoAgent: Sub-result
    end
    AutoAgent-->>User: Final combined result
```
