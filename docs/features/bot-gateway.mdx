...
Enable `per_chat` session scope to let all members of a Telegram group, Slack channel, or Discord server share one conversation transcript. Each message is prefixed with the sender's name so the agent knows who said what.
...
See [Group Chat Sessions](/docs/features/bot-group-sessions) for the full configuration reference, DM-vs-group routing rules, and best practices.
