...
<Note>
`unknown_user_policy` and `owner_user_id` are honoured **identically** by the single-bot CLI (`praisonai bot start`) since [PR #5111](https://github.com/MervinPraison/PraisonAI/pull/5111) — same semantics, same validation, same resolved-policy startup log. See [Standalone Bot YAML → Admission policy](/features/standalone-bot-yaml#admission-policy-unknown-user-policy-owner-user-id).
</Note>

<Note>
After [PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112), when a channel has no `allowed_users` the startup banner names the resolved `unknown_user_policy` explicitly (`deny` → "responds to NOBODY new", `allow` → "responds to EVERYONE", `pair` → "unknown users must complete pairing"). Every `deny` drop also logs a `WARNING` including the sender's `user_id`, so operators can distinguish a misconfigured allowlist from a broken bot. A typo in `unknown_user_policy` inside `gateway.yaml` now fails closed at load time — `Invalid unknown_user_policy 'alow'. Must be one of: allow, deny, pair`.
</Note>
...
| `implicit_mentions` | `list[str]` | `["reply"]` | Non-`@mention` signals that admit a message under `mention_only` / `observe`. Values: `"reply"`, `"quote"`, `"thread"`. Empty list = strict `@`-only. Telegram reports `reply` today. |
...
<Note>
Under `mention_only` / `observe`, a **reply to the bot** counts as an implicit mention by default — the natural follow-up (tap Reply on the bot's answer) is answered without re-typing `@bot`. See [Implicit Mentions](/features/bot-implicit-mentions) to enable quote/thread signals or set `implicit_mentions: []` for strict `@`-only.
</Note>
...
<Note>
See [Bot Shell Execution](/features/bot-shell-execution) for the full routing ladder, per-platform ID resolution, env-var reference, and copy-paste recipes.
</Note>
...
`GatewayMessage` carries an optional `presentation: MessagePresentation` field. When set, channel adapters that implement [`SupportsPresentation`](/features/bot-presentations) render it as a native widget (Telegram inline keyboard, Slack Block Kit, Discord components). Channels that do not implement the protocol fall back to the message's plain-text `content`. See [Interactive Bot Messages](/features/bot-presentations) for the full presentation model.

## Session Learning (default-on)

Every gateway/bot session enables the coordinated `learn=True` posture by default, so agents accrue persona and preferences from plain chat across sessions.

Opt a channel out with `learn: false` (or `session_learning: false`) in the channel YAML — the gateway forwards it through `config.metadata`:
...
See [Bot Session Learning](/features/bot-session-learning) for the full behaviour, coordinated-posture table, and opt-out surfaces.
...
See [Gateway Hot-Reload](/features/gateway-hot-reload) for the full restart-scope table.
...
<Note>
`gateway stop` won't release the PID lock if the running gateway is owned by another user — it reports the stop as not confirmed and leaves the lock in place so a duplicate gateway can't start on top of it. See [Gateway CLI — stop](/features/gateway-cli#commands).
</Note>
...
| `idle_policy` | `GatewayIdlePolicyProtocol` | `None` | When set, schedules an idle-dormancy loop that quiesces transports after inactivity. Default `None` = always-on. See [Scale-to-Zero Gateway](/features/gateway-scale-to-zero). |
...
<CardGroup cols={2}>
  ...
  <Card icon="route" href="/features/gateway-tracing-hook" title="Gateway Tracing Hook">
...
<AccordionGroup>
  ...
  <Accordion title="Monitor via /health and /metrics">
    Poll the health endpoint during deploys and wire [Gateway Metrics](/features/gateway-metrics) into Prometheus for message-flow visibility.
  </Accordion>
</AccordionGroup>

<Note>
`/undo` in a bot chat reverts files in that chat's workspace, not the gateway process cwd. This happens automatically — `apply_bot_smart_defaults()` calls `Agent.set_snapshot_root(workspace.root)` after attaching the `Workspace`. See [File Snapshot → Bot / Gateway Workspaces](/docs/features/file-snapshot#bot-gateway-workspaces).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card icon="arrows-rotate" href="/features/gateway-restart-continuation" title="Restart Continuation">
    Re-drive interrupted turns and notify the originating channel after a restart.
  </Card>
  <Card icon="user-graduate" href="/features/bot-session-learning" title="Bot Session Learning">
    Default-on cross-session learning for gateway/bot agents, with the channel opt-out key.
  </Card>
</CardGroup>
