...
Only CLIs that resolve via `shutil.which(...)` show a toggle. The CLI → command mapping below is a **view of the registry** (`EXTERNAL_AGENTS`, a `_RegistryAgents` Mapping), so a registered plugin gets a `"{name}_enabled"` toggle automatically:
