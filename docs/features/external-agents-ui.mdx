...
agent = Agent(name="dev-assistant", instructions="Help with coding tasks")
# Enable Claude Code or Codex in the UI sidebar — tools attach automatically
...
Only CLIs that resolve via `shutil.which(...)` show a toggle. The CLI → command mapping below is a **view of the registry** (`EXTERNAL_AGENTS`, a `_RegistryAgents` Mapping), so a registered plugin gets a `"{name}_enabled"` toggle automatically. Toggles are built from the same catalog as `--external-agent` choices, so a plugin that fails to load simply does not appear as a toggle — it never shows up as a dead switch:
...
These are the **built-in** toggles. A registered external agent gets a `"{name}_enabled"` toggle automatically — its label comes from `display_label` and its CLI from `cli_command`.
...
<Note>
`EXTERNAL_AGENTS` is a registry-backed `Mapping` (as of PraisonAI PR [#4156](https://github.com/MervinPraison/PraisonAI/pull/4156)), so `installed_external_agents()` and `aiui_settings_entries()` include plugins automatically. It still supports `in`, iteration, and `[]`; built-in order and labels are unchanged on a default install. See [Register a custom external agent](/features/external-cli-integrations#register-a-custom-external-agent).
</Note>
...
<Note>
The UI wires integrations with `as_tool()`, which is safe from both sync and async runtimes since PraisonAI PR #4022. Programmatic callers registering with a native async agent (via `agent.astart(...)`) should prefer `integration.as_async_tool()` to skip the worker-thread hop.
</Note>
...
<CardGroup cols={2}>
<Card title="External CLI Integrations (Python API)" icon="code" href="/features/external-cli-integrations">
  Use external agents programmatically in your code
</Card>
<Card title="External Agents (Code)" icon="terminal" href="/code/external-agents">
