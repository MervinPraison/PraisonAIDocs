...
<Note>
Available under `PraisonAIAgents` / `AgentTeam` as of PraisonAI [PR #4907](https://github.com/MervinPraison/PraisonAI/pull/4907). Earlier releases only exposed `output_variable` inside the standalone `Workflow` engine and its YAML syntax. PraisonAI [#4932](https://github.com/MervinPraison/PraisonAI/pull/4932) fixed the parallel-branch case, where an `output_variable` set inside `parallel([...])` was previously discarded.
</Note>
...
def score_a(ctx: WorkflowContext) -> StepResult:
    return StepResult(output="9")

def score_b(ctx: WorkflowContext) -> StepResult:
    return StepResult(output="7")

compare = Task(action="Compare {{branch_a_score}} and {{branch_b_score}}")

workflow = AgentFlow(steps=[
    parallel([
        Task(name="branch_a", handler=score_a, output_variable="branch_a_score"),
        Task(name="branch_b", handler=score_b, output_variable="branch_b_score"),
    ]),
    compare,
])
workflow.start("go")
...
If two branches use the same `output_variable`, the later-declared branch wins and a warning is logged — give them distinct names. See [Named outputs from branches](/features/workflow-parallel#named-outputs-from-branches) for the full merge rule.
