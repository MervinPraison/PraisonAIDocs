...
```mermaid
graph LR
    subgraph "Streaming Tool Events"
        Request[📋 User Request] --> Process[⚙️ Streaming Tool Events]
...
graph LR
    subgraph "Streaming Tool Events"
        In[📝 Tool-heavy Prompt] --> Agent[🤖 Agent]
        Agent --> Events[⚙️ Emit Tool Events]
        Events --> Out[✅ UI Progress]
    end
...
See [Tool Progress Streaming](/features/tool-progress-streaming) for the full guide.


### TODO_UPDATED Event

`TODO_UPDATED` fires on every `todo_add` / `todo_update` and carries the **full ordered todo list** in `metadata["todos"]` — subscribers render a live checklist without tracking deltas.

| Field | Type | Description |
|-------|------|-------------|
| `type` | `string` | Always `"todo_updated"` |
| `timestamp` | `float` | High-precision timestamp |
| `metadata.todos` | `list[dict]` | Full ordered list of todo dicts (`id`, `task`, `status`, `priority`, `category`) |
...
See [Live Todo Streaming](/features/todo-live-streaming) for the full guide.
...
Over the AG-UI wire, `TOOL_CALL_END` now maps to the standard `tool_call_end` (`ToolCallEndEvent`) frame, so UI consumers get an explicit completion marker. See the [AGUI API reference](/deploy/api/agui-api#event-types).

## Related

<CardGroup cols={2}>
<Card title="Agent Streaming" icon="stream" href="/features/streaming">
...
<Card title="Tool Development" icon="wrench" href="/tools/custom">
