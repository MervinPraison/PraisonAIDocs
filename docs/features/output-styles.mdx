...
| `text` | Response + tool trace | Simple CLI output |
| `status` | `[HH:MM:SS] ▸ tool → result ✓` | Progress monitoring |
| `verbose` | Task + Tools inline + Response panel | Interactive use |
| `debug` | verbose + metrics footer | Debugging |
...
**Output:**
```
[13:47:33] ▸ get_weather(city='Tokyo')
[13:47:33]   └─ get_weather → Sunny in Tokyo [0.2s] ✓
