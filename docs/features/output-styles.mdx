...
<Note>
`status` and `trace` route through a shared per-process singleton whose `_output_lock` is now actually acquired ([PR #5219](https://github.com/MervinPraison/PraisonAI/pull/5219), fixes [#5162](https://github.com/MervinPraison/PraisonAI/issues/5162)). Reaching for `output="status"` / `output="trace"` under `asyncio.gather` is now safe for the aggregate timing and token/cost metrics — with the caveat that `_tool_start_times` / `_agent_start_times` are still keyed by name only. Give each agent a distinct `name=`. Full detail in [Thread Safety → Output singleton lock](/features/thread-safety#output-singleton-tracestatus-lock-pr-5219).
</Note>
...
<Card title="OutputConfig SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/feature_configs">
...
a = Agent(name="A", instructions="You are helpful.", output="editor")   # editor mode wins
b = Agent(name="B", instructions="You are helpful.", output="trace")    # keeps editor mode
c = Agent(name="C", instructions="You are helpful.", output="status")   # keeps editor mode
```

Precedence *within a single Agent* when multiple mode flags are set:

```
editor_output > status_trace > actions_trace
```

```mermaid
graph TB
    A1[🤖 Agent A<br/>output='editor'] -->|first mode wins| REG[📋 Process registry]
    A2[🤖 Agent B<br/>output='trace'] -.->|skipped| REG
    A3[🤖 Agent C<br/>output='status'] -.->|skipped| REG
    REG --> OUT[📤 Editor formatting for A, B, C]
...
<AccordionGroup>
  ...
  <Accordion title="Set the console mode once at the entry point">
...
<CardGroup cols={2}>
  ...
  <Card title="Callbacks" icon="webhook" href="/features/callbacks">
