...
`DrainMarkerPolicy` only decides *when* to drain; the actual bounded wait still goes through `gateway.stop(drain_timeout=...)` documented on the [Session Continuity](/features/gateway-session-continuity) page. Without a `drain_timeout`, shutdown cancels in-flight turns immediately.
</Accordion>
