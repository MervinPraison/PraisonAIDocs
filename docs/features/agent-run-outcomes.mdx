...
The `praisonai run` CLI wraps the SDK outcome into a single **success / failure** exit signal for pipelines: any falsy or empty result (`None`, `""`, all-whitespace) exits **`1`** with a machine-readable `run_failed` payload; a non-empty result exits **`0`** with the existing text/JSON output.

| SDK outcome | CLI exit code | `--output json` `status` |
|---|---|---|
| `success` (non-empty output) | `0` | absent (success payload) |
| `failure` / `timeout` / `cancelled` / `invalid_output` (falsy or empty result surfaced to `Agent.start`/`run`) | `1` | `"failed"` (with `code: "run_failed"`) |

See [`praisonai run` → Exit Codes](/docs/cli/run#exit-codes) for the full CLI contract, the failure payload schema, and CI usage examples.

<Note>
The SDK-level statuses stay available programmatically for Python/YAML users who want fine-grained matching — the CLI just collapses everything that isn't a real answer into a single non-zero exit so shell pipelines don't need to parse status strings.
</Note>
