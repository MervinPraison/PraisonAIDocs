...
<Note>
`AgentRunOutcome` (this page) and `RunOutcome` (see [Run Outcome](./run-outcome)) are **two different types** with two different jobs:

| | `AgentRunOutcome` | `RunOutcome` |
|---|---|---|
| **Import** | `from praisonaiagents import AgentRunOutcome` | `from praisonaiagents import RunOutcome` |
| **Module** | `praisonaiagents.run_outcome` | `praisonaiagents.agent.run_outcome` |
| **Field** | `status` | `reason` |
| **Values** | `success \| failure \| timeout \| cancelled \| invalid_output` | `completed \| hard_timeout \| cancelled \| aborted \| failed \| content_filtered \| refused \| length_truncated` |
| **Where it comes from** | Task validation, handoff results | `agent.run(..., return_outcome=True)` / `agent.astart(..., return_outcome=True)` |
| **Purpose** | Typed validation/handoff status (retry, invalid-output detection) | Host-facing terminal outcome for a whole run (bot/gateway dispatch) |
</Note>
...
```typescript
import { AgentRunOutcome } from 'praisonai';

const outcome = AgentRunOutcome.success('Summarised agent run outcomes');
console.log(outcome.status);      // 'success'
console.log(outcome.isSuccess()); // true

const failed = AgentRunOutcome.failure('validation failed');
console.log(failed.status);       // 'failure'
```
</Tab>
</Tabs>
...
`Process` normalises its routing key with `.lower().strip()` — the same normalisation `validate_decision_string` applies at `praisonaiagents/run_outcome.py:303`. Before the fix in [MervinPraison/PraisonAI#4260](https://github.com/MervinPraison/PraisonAI/pull/4260), the two disagreed on a whitespace-padded verdict like `"invalid\n"`; now they agree, so the typed outcome is reliably stored even when the model pads its verdict.
...
| SDK outcome | CLI exit code | `--output json` `status` |
|---|---|---|
| `success` (non-empty output, `agent.last_stop_reason != "max_steps"`) | `0` | absent (success payload) |
| `success` (non-empty output, `agent.last_stop_reason == "max_steps"`) — a step-limit-truncated finalisation summary | `2` | `"truncated"` (result preserved as the summary text) |
| `content_filtered` (`agent.last_stop_reason == "content_filtered"`) | `2` | `"content_filtered"` (result preserves any partial text) |
| `refused` (`agent.last_stop_reason == "refused"`) | `2` | `"refused"` (result preserves any partial text) |
| `length_truncated` (`agent.last_stop_reason == "length_truncated"`) | `2` | `"length_truncated"` (result preserves any partial text) |
| `failure` / `timeout` / `cancelled` / `invalid_output` (falsy or empty result surfaced to `Agent.start` / `run`) | `1` | `"failed"` (with `code: "run_failed"`) |

See [`praisonai run` → Exit Codes](/docs/cli/run#exit-codes) for the full CLI contract, the failure and truncated payload schemas, and CI usage examples.

<Note>
The SDK-level statuses stay available programmatically for Python/YAML users who want fine-grained matching. On the CLI, step-limit truncation is now surfaced distinctly (exit `2`, `status: "truncated"`) rather than masked as success — see [Exit Codes](/docs/cli/run#exit-codes).
</Note>
