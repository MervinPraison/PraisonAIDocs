...
<Tip>
**Model-aware ordering.** The order in which `apply_patch` and `edit_file` are advertised is automatically tuned for Claude vs. GPT models via the [Model Harness](/docs/features/model-harness). Unknown models keep the historic ordering unchanged.
</Tip>
