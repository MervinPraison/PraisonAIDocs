...
validation_pipeline = Workflow(
    steps=[
        ...
        WorkflowStep(name="tech", handler=tech_review_handler, guardrail=tech_validator),
        WorkflowStep(name="style", handler=style_check_handler, guardrail=style_validator)
    ]
)
