...
validation_pipeline = Workflow(
    steps=[
        WorkflowStep(name="write", handler=write_docs_handler),
        WorkflowStep(name="tech", handler=tech_review_handler, guardrails=tech_validator),
        WorkflowStep(name="style", handler=style_check_handler, guardrails=style_validator)
    ]
)
