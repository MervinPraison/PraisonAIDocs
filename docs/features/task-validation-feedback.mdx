...
To enforce a typed contract instead of custom checks, use [Structured Outputs](/features/structured); parse failures now surface via `TaskResult.error`.
...
<Note>
Decision routing is **case- and whitespace-insensitive** as of the `praisonaiagents`
build merging [MervinPraison/PraisonAI#4260](https://github.com/MervinPraison/PraisonAI/pull/4260)
...
judge task's `condition` keys are not matching what the model returns.
</Note>
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Task Validation & Feedback
...
writer = Agent(name="Writer", instructions="Write a short paragraph.")
judge = Agent(name="Judge", instructions="Reply with exactly one word: valid or invalid.")

write = Task(
    name="write_article",
    description="Write a paragraph about renewable energy.",
    agent=writer,
    is_start=True,
    next_tasks=["judge_article"],
)

judge_article = Task(
    name="judge_article",
    description="Judge the paragraph. Reply with exactly one word: valid or invalid.",
    agent=judge,
    task_type="decision",
    condition={
        "valid": [],                    # lowercase, no whitespace, no punctuation
        "invalid": ["write_article"],   # route back to retry
    },
)

PraisonAIAgents(
    agents=[writer, judge],
    tasks=[write, judge_article],
).start()
```

These model outputs all route to the `invalid` branch:

| Model output | Routes as |
|---|---|
| `"invalid"` | `invalid` |
| `"Invalid"` | `invalid` |
| `"INVALID"` | `invalid` |
...
VALIDATION_FAILURE_DECISIONS = [
    "invalid", "retry", "failed", "error", "unsuccessful",
    "fail", "errors", "reject", "rejected", "incomplete",
]
```
...
<Note>
**Since PraisonAI [#4929](https://github.com/MervinPraison/PraisonAI/pull/4929)**, `AgentTeam` re-runs fill `Task.validation_feedback` from the guardrail reason. Earlier releases logged the reason and dropped it before the re-run, so the second attempt saw the same prompt as the first with no clue about what to fix.
</Note>
