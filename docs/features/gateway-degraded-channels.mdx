...
<Note>
This page covers the **channel-specific** boot-time isolation contract. Since PraisonAI 2026-07-30 (PR [#3520](https://github.com/MervinPraison/PraisonAI/pull/3520)), the same fact appears in a broader unified surface alongside provider / capability / route degradation — see [Unified Degraded-Capability Registry](/docs/features/gateway-degraded-capabilities).
</Note>
...
The same degraded channel also appears in the unified `degraded_owners` list, alongside any degraded provider, capability, or route — see [Unified Degraded-Capability Registry](/docs/features/gateway-degraded-capabilities).
...
<Card title="Gateway Channel Supervision" icon="heart-pulse" href="/docs/features/gateway-channel-supervision#runtime-credential-rejection">
