...
<Note>
Running `praisonai schedule tick` from OS cron with no gateway now delivers stdout out of process using only `{PLATFORM}_BOT_TOKEN` — see [Deliver from cron / CI](/features/scheduler-delivery#deliver-from-cron-ci-no-gateway-required).
</Note>
...
| `provider` | `Optional[str]` | `None` | Model provider snapshot for a pinned job — not applicable to `--command` jobs (no model turn). See [Scheduler Model Pin](/features/scheduler-model-pin). |
| `model` | `Optional[str]` | `None` | Model snapshot for a pinned job — not applicable to `--command` jobs (no model turn). See [Scheduler Model Pin](/features/scheduler-model-pin). |
| `pin_model` | `bool` | `True` | Enforce the model snapshot on drift — not applicable to `--command` jobs (no model turn). See [Scheduler Model Pin](/features/scheduler-model-pin). |

`command` is persisted only when set; `command_timeout` is persisted only when a `command` is configured **and** the timeout differs from the default `60.0`. Agent-only jobs are unchanged on disk. A `--command` job takes no model turn, so `provider` / `model` / `pin_model` have no effect on it — pinning is skipped for command jobs.
...
<Card title="Context Chaining" icon="link" href="/features/scheduler-context-chaining">
  Feed one job's output into the next as bounded context
</Card>
...
<Card title="Scheduler Model Pin" icon="lock" href="/features/scheduler-model-pin">
  Pin agent jobs to a model — skipped for `--command` jobs
</Card>
</CardGroup>
