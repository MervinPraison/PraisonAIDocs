...
<Note>
**After a wall-clock timeout:** the orphaned worker thread keeps running, the single-worker executor is retired, and the agent-level retry loop is **skipped unless the tool is idempotent**. Re-driving a mutating tool while its body is still executing would duplicate side effects, so only tools marked idempotent are retried — unknown tools default to non-idempotent (safe). See [Tool Timeouts → Retry-on-timeout is opt-in](/features/tool-timeout#retry-on-timeout-is-opt-in-for-mutating-tools).
</Note>
