...
| `DEGRADE_POLL_AS_BUTTONS` | `"poll_rendered_as_buttons"` | Channel has `supports_native_poll=False` — POLL became a question TEXT block + reply-buttons grid, e.g. `"poll (3 options) rendered as buttons"`. See [Poll Blocks](/features/poll-blocks). |
