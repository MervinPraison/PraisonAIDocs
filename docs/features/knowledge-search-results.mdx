...
<Note>
When you retrieve `source` via the internal `_extract_value(item, key, default)` helper (used by higher-level context assembly), a present-but-`None` `source` now falls through to your `default`. Changed in [PR #4958](https://github.com/MervinPraison/PraisonAI/pull/4958) — previously the `None` default on the dataclass masked any caller-supplied default. Direct access via `item.source` still returns `None`; only the fallback helper's behaviour changed.
</Note>
...
<Note>
**Related fix.** [PR #4887](https://github.com/MervinPraison/PraisonAI/pull/4887) fixed three SDK call sites that hit exactly this pitfall — async chat and both streaming branches tried to `"\n".join(...)` a `SearchResult` (truthy but not iterable). See [Async Knowledge Retrieval](/features/async-knowledge-retrieval) for the concrete SDK-side fix this guidance protects against.
</Note>
</Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Knowledge CLI" icon="terminal" href="/cli/knowledge-cli">
