...
Point the release at the pre-created Secret. The same chart is also installable via `praisonai deploy helm --chart gateway` (see the [Helm CLI Wrapper](/docs/features/helm-cli)).
...
<Warning>
Scaling `replicas > 1` requires a distributed turn lock — the default in-process lock does not serialise turns across pods, so two pods will run concurrent turns on one session and corrupt the shared transcript. Set `gateway.turn_lock.backend: redis` (see [Gateway Turn Lock](/features/gateway-turn-lock)) before increasing `replicaCount`.
</Warning>
...
Every key below is from `src/praisonai-bot/infra/helm/praisonai-gateway/values.yaml`.
...
<CardGroup cols={2}>
<Card title="Gateway & Control Plane" icon="tower-broadcast" href="/gateway">
