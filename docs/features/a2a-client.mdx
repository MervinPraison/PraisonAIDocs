...
agent = Agent(name="a2a-agent", instructions="Communicate with remote A2A agents.")
agent.start("Connect to the remote A2A server and fetch available tasks.")
```


A2AClient enables PraisonAI applications to discover and interact with remote A2A agents through an async HTTP interface.

The user asks the agent to reach a remote A2A server; the client discovers capabilities and exchanges messages over HTTP.
...
<Note>
`A2AClient` is exported lazily from `praisonaiagents.ui.a2a`. The first `A2AClient` reference triggers `import aiohttp`; server-only usage that never touches the client keeps startup light.
</Note>
...
```python
import asyncio
from praisonaiagents import Agent, A2A, A2AClient
...
<CardGroup cols={2}>
<Card title="A2A Protocol" icon="server" href="/features/a2a">
