...
The boundary check is dialect-agnostic: it fires on `Remove-Item C:\outside\x` the same way it fires on `rm /outside/x`. See [Command-Aware Permissions → Windows / PowerShell dialect](/features/command-aware-permissions#windows-powershell-dialect).
...
<Card title="Interactive Approval" icon="check" href="/features/interactive-approval">
  User-facing approval prompts and backends
</Card>
<Card title="Secret Read Gate" icon="shield-lock" href="/features/secret-read-gate">
  Sibling core-PermissionManager default — reads of .env and private keys ask by default
</Card>
</CardGroup>
