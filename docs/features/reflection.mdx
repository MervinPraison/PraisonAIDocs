...
When the reflection agent's underlying model triggers a content filter / refusal / length cutoff, the outer run's `agent.last_stop_reason` is set to `content_filtered` / `refused` / `length_truncated` — see [Run Outcome](/features/run-outcome#provider-block-outcomes).

With `output="verbose"` (or `output=OutputConfig(verbose=True)`), look for this log line on the terminating pass:
