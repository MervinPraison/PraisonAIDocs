...
<Note>
`--record-video` writes `recording.webm` and is reachable only on `--engine cdp`. It implies vision, so the model is upgraded to `gpt-4o` for that run. See [Video recording](/docs/cli/browser#video-recording-cdp-only).
</Note>
...
See the full [Engine × Option matrix](/docs/cli/browser#engine--option-matrix) in the CLI reference for the authoritative per-flag breakdown.
...
When `_execute_action` reports `success=false, error="selector matched no element: …"`, `CDPBrowserAgent.run()` retries with three alternative selectors before propagating the failure to the LLM. See [Browser CLI → Retry & selector fallback](/cli/browser#retry--selector-fallback-cdp--hybrid-engines) for the full flow diagram.
...
Run `playwright install chromium` (add `--with-deps` in Docker / CI) to provision it. See [Offline & CI Install](/features/offline-and-ci-install).
</Warning>
