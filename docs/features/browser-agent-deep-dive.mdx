...
<AccordionGroup>
  <Accordion title="Prefer CDP or Playwright for Automation Pipelines">
    Extension mode requires a human-visible Chrome window and supports only one session at a time. Use CDP mode for headless server automation and Playwright mode for cross-browser testing.
  </Accordion>
  <Accordion title="Always Clean Up Sessions">
    Call `praisonai browser reload` or restart Chrome if a previous session was interrupted. Stale debugger attachments block new sessions in extension mode.
  </Accordion>
  <Accordion title="Use Session IDs for Memory Isolation">
    Provide a unique `session_id` per task so `chat_history` stays isolated. Call `agent.reset(new_session_id=...)` between tasks to avoid context bleed.
  </Accordion>
  <Accordion title="Enable Debug Mode During Development">
    Run with `--debug` to log every CDP command and LLM decision. Disable it in production to reduce noise and latency.
  </Accordion>
</AccordionGroup>
