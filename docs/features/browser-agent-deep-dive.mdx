...
Chrome 137+ (notably on Windows) blocks automated `--load-extension`, so `praisonai browser launch` can fail to auto-load the extension. Load it manually into your daily "Work" Chrome instead.
...
Click **Load unpacked** and select the extension path printed by `launch` (the extension `dist/` folder).
</Step>
<Step title="Confirm the connection">
Open the PraisonAI side panel and confirm it connects to `ws://127.0.0.1:8765/ws`.

If you launched with `--no-server`, start the bridge first — no bridge is listening otherwise:
```bash
...
```
Or check `/health` directly (expect `extension_connections >= 1`):
```bash
