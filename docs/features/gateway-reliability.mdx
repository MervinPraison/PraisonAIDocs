...
`reliability="off"` restores the pre-reliability behaviour (immediate cancel on SIGTERM, unbounded concurrency). It also forces [`gateway.durable_runs`](/features/gateway-durable-runs) **off** — the explicit escape hatch to the zero-overhead path. Use it only in tests — never deploy `off` to production.
</Accordion>
...
<Card title="Memory-Pressure Eviction" icon="memory" href="/features/gateway-memory-pressure-eviction">
  Reclaim memory from idle warm caches — the eviction-side companion to `MemoryPressurePolicy`
</Card>
