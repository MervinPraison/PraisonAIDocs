...
<Note>
**Upgrade note (PraisonAI PR #5002).** The durable payload now also carries per-task status, output, retry count, and variables — protected by a task-set fingerprint. A team that crashed on task 9 of 12 now resumes at task 9 instead of restarting from task 1. See [Team Checkpoint & Resume](/features/team-checkpoint-resume) for the Python-API-level details.
</Note>

<Note>
**The recorded model is restored here too.** `praisonai run agents.yaml --continue` follows the same `run_main` path, so with no explicit `--model` the team resumes on the model it was recorded with. See [Session Model Restore](/features/session-model-restore).
</Note>
