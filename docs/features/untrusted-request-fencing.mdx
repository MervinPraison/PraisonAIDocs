...
<Note>
Fencing is the prompt-plane defence — it wraps untrusted payload *content* as data. The matching control-plane defence lives on `BotMessage.allow_control` — see [Control-Trust Provenance](/features/bot-control-trust). Producers of untrusted content set both: fence the payload for the agent, and `allow_control=False` so the bot never treats it as a slash-command.
</Note>
...
<Card title="Control-Trust Provenance" icon="shield-keyhole" href="/features/bot-control-trust">
  The control-plane counterpart — gates commands by content provenance
</Card>
</CardGroup>
