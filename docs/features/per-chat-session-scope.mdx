...
<Tip>
**Temporal grounding:** add `timestamps: true` under `session:` to prefix each turn with its arrival time so the agent can reason about "now" and relative-time requests. In `per_chat` the time sits outermost, after the sender name. See [Bot Temporal Grounding](/features/bot-temporal-grounding).
</Tip>
...
## per_chat + observe = shared group memory

Pair `session_scope: per_chat` with `group_policy: observe` for the classic group-assistant experience: every message lands on the shared per-chat transcript, the bot only replies when @mentioned, and the reply sees the full recent conversation with `[sender]` attribution.

On its own, `observe` records passive turns under the sender's own `per_user` key — still useful for solo group DMs, but not shared across participants. Adding `per_chat` routes those passive turns to the shared per-chat key that a later addressed turn reads from.
...
channels:
  telegram:
    platform: telegram
    token: "${TELEGRAM_BOT_TOKEN}"
    group_policy: observe        # record unmentioned messages as context
    session:
      session_scope: per_chat    # share that context across the group
```

<Note>
`observe` is Telegram-only today. See [Observe Mode](/features/bot-observe-mode).
</Note>
...
<CardGroup cols={2}>
<Card title="Bot Gateway" icon="plug" href="/features/gateway">
