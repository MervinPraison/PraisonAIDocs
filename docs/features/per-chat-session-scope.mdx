...
In group chats, route all participants to one shared session so your agent follows a single multi-party conversation thread, with each message prefixed by the sender's name.
