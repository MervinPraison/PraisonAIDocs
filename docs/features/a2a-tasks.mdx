...
agent = Agent(name="task-manager", instructions="Manage A2A tasks via JSON-RPC.")
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as A2A Task Management
...
<Note>
Use the built-in [`A2AClient`](/features/a2a-client) for task management from Python instead of hand-rolling `aiohttp`/`requests` calls — it wraps the JSON-RPC surface and handles auth, streaming, and task lifecycle for you.
</Note>
...
<Card title="Handoffs" icon="users" href="/features/handoffs">
