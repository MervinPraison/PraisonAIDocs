...
| `"cron:0 7 * * *"` | best-effort | Best-effort integer collapse for the polling loop |
...
<Note>
A scheduled run whose whole output is exactly `NO_REPLY` (or `[SILENT]` / `SILENT`) skips delivery — the tick still succeeds and is recorded in stats, but nothing is pushed. `AsyncAgentScheduler` inherits the same `AgentScheduler._deliver_result` code path, so this holds for async schedules too. See [Scheduler Delivery → Intentional Silence](/docs/features/scheduler-delivery#intentional-silence).
</Note>
