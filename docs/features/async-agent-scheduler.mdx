...
| `undelivered_deliveries` | `int` | Runs whose configured `deliver=` target failed (router returned `False` or the send raised). Bumped **only** by an `UNDELIVERED` outcome — never inferred. |
