...
sequenceDiagram
    ...
    User->>Scheduler: stop()
    Scheduler->>Scheduler: Cancel gracefully (10s timeout)
```
...
The `stop()` method waits up to 10 seconds for the current execution to complete before canceling. This prevents data corruption and ensures clean shutdown.
