...
| `"cron:0 9 * * *"` | wall-clock | Fires at the real time-of-day. Requires the optional `croniter` package. |
...
**Natural-language expressions.** A bare clock time (`at 9am`) is treated as a one-shot at the next matching local time in the schedule's timezone. Any recurring form — `every …`, `daily …`, or one that names a day of the week — is translated to a cron expression and follows the same wall-clock semantics as `cron:` (requires optional `croniter` at run time for wall-clock accuracy).
...
- **Optional dependency** — install `croniter` to enable wall-clock cron:

  ```bash
  pip install croniter
  ```

  Without `croniter`, a `cron:` schedule falls back to the pre-#3526 process-relative interval and the wrapper logs a one-time warning at startup.
...
<Accordion title="Install croniter for wall-clock cron schedules">
`cron:` schedules honour wall-clock time only when the optional `croniter` package is installed (`pip install croniter`). Without it, the scheduler logs a one-time warning and falls back to a coarse interval anchored to process start — the same behaviour as before PR #3527.

```python
from praisonai.scheduler import AsyncAgentScheduler

# Requires croniter — fires at 09:00 wall-clock every day
