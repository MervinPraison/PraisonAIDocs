...
<Note>
The same jobs can be seeded from templates and consent-first suggestions — see [Automation Suggestions](/features/automation-suggestions).
</Note>

<Note>
`praisonai schedule tick` fired from OS cron now delivers without a running gateway, using only `{PLATFORM}_BOT_TOKEN` — see [Deliver from cron / CI](/features/scheduler-delivery#deliver-from-cron-ci-no-gateway-required).
</Note>
...
<Note>
The pre-tick policy runs **before** the [atomic claim](/docs/features/async-scheduler#multi-process-safety-at-most-once), so a policy rejection avoids taking a lease entirely.
</Note>
...
<Note>
**Model drift fails closed too.** A [pinned job](/features/scheduler-model-pin) whose resolved model has drifted from its snapshot is recorded `failed` with no model turn taken — and `deliver_on_failure=True` surfaces that drift summary to the delivery target, exactly like a blocked prompt scan.
</Note>

<Note>
**Silent runs.** Return exactly `NO_REPLY`, `[SILENT]`, or `SILENT` from your agent to suppress delivery on a scheduled run — the job still records as `succeeded`. Useful for unattended monitors that only speak when there's something to say. See [Intentional Silence → Scheduled agents](/docs/features/bot-intentional-silence#scheduled-agents).
</Note>
...
## Failure Alerting — Once Per Incident

Failure summaries are de-duplicated per incident by default: a broken job alerts **once**, stays silent on identical repeats, and sends a single recovery note on the next success.

```mermaid
graph LR
    F[❌ Failure] --> Sig[🔐 error_signature]
    Sig --> Same{Same signature<br/>already alerted?}
    Same -->|Yes| Silent[🤫 Silent]
    Same -->|No / changed| Alert[📣 ⚠️ Failure summary]
    S[✅ Success] --> Open{Incident open?}
    Open -->|Yes| Recover[🟢 ✅ recovered.]
    Open -->|No| NoOp[No-op]
    ...
    class F,S input
    class Sig process
    class Same,Open gate
    class Silent,NoOp silent
    class Alert,Recover output
```

Tolerate a single flaky tick — alert only when the same failure repeats twice.
...
# Tolerate a single flaky tick — alert only when the same failure repeats twice.
# The wrapper folds each failure through the built-in IncidentTracker for you.
policy = RunPolicy(alert_after_failures=2)

agent.start("Every day at 02:00, brief last-night activity to slack:#eng.")
```

| Run outcome | Prior state | What operators see |
|---|---|---|
| Failure, first with this signature | no open incident | `⚠️ Scheduled job '<name>' failed: <error>` (one alert) |
| Failure, same signature repeats | already alerted | Silent — no re-alert |
| Failure, different signature | already alerted on signature A | Fresh `⚠️` alert for signature B |
| Failure, below `alert_after_failures` threshold | still counting | Silent — counter advances |
| Success after an alerted incident | incident alerted | `✅ Scheduled job '<name>' recovered.` (one note) |
| Success with no open incident | clean | Silent |
| Failure with `deliver_on_failure=False` | any | Silent (no-op) |
| Failure with no state store attached | any | Falls back to per-tick delivery (prior behaviour preserved) |

<Note>
A failed send does **not** consume the alert or the recovery note — the incident state is committed only after delivery lands. If the channel is down, the next tick re-observes and retries the alert; the next success retries the recovery note.
</Note>
...
| `alert_after_failures` | `int` | `1` | When `deliver_on_failure` is on, alert only after this many *consecutive* failures of the **same error signature**. Latches failure summaries through the core `IncidentTracker` so a broken job alerts **once** per distinct error and sends a single `✅ recovered.` note on the next success. Values below `1` are treated as `1`. Falls back to per-tick delivery when no state store is available. |
...
<Note>
`no_change` is a distinct status for [change-detection monitors](/features/scheduler-monitor): a watched source was unchanged, so the model turn was suppressed silently — different from a generic `skipped` gate decision.
</Note>
...
A failed send does not consume the alert. When a failure summary or recovery note does not reach the channel, the incident state is left un-committed, so the next tick re-observes and retries the alert and the next success retries the recovery note (see `test_incident_wiring.py::TestDeliveryFailureRetry`).
</Accordion>
</AccordionGroup>
...
<Card title="Schedule CLI" icon="terminal" href="/cli/scheduler">
...
<Card title="Scheduler Model Pin" icon="lock" href="/features/scheduler-model-pin">
