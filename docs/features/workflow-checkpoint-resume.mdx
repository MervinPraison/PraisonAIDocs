...
For a Python-instantiated `AgentTeam`, use `team.save_session_state()` /
`team.restore_session_state()` instead — see
[Team Checkpoint & Resume](/features/team-checkpoint-resume).
</Warning>
...
## Passing Variables on Resume (Human-in-the-Loop)

A workflow that paused for human review resumes with the reviewer's decision handed in as a variable.

```bash
praisonai workflow run release-notes --resume --var reviewer_decision=approved
```
...
result = manager.execute(
    ...
    default_agent=agent,
    resume="run-1",
    variables={"reviewer_decision": "approved"},
)
```

Variables merge in three layers — the freshest statement of intent wins.

| Layer | Wins over | Source |
|---|---|---|
| Workflow defaults | — | The workflow definition |
| Checkpoint snapshot | defaults | State at the moment the run paused |
| Resume-time `variables=` / `--var` | both above | The caller's fresh input on this resume |

```mermaid
sequenceDiagram
    participant Defaults as Workflow defaults
    participant Checkpoint as Checkpoint snapshot
    participant Resume as Resume-time --var
    participant Step as Next step

    Defaults->>Checkpoint: merge (checkpoint wins)
    Checkpoint->>Resume: merge (resume wins)
    Resume->>Step: effective variables
```

Before this fix, resume-time variables were silently discarded — the checkpoint's snapshot overwrote them. The `variables=` / `--var` argument on resume now always wins.
...
| `variables` | dict | The snapshot layer — not the final value the resumed step sees. See _Passing Variables on Resume_ |
...
<AccordionGroup>
  ...
  <Accordion title="Hand a reviewer's decision back with --var">
    After a workflow pauses for human review, resume with the decision as a
    variable: `--var reviewer_decision=approved`. It wins over the checkpoint
    snapshot, so the resumed step sees the fresh value.
  </Accordion>
