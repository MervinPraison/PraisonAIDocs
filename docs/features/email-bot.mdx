...
AgentMail webhook mode (`mode="webhook"`) requires a signing secret to authenticate inbound events.

<Warning>
`AGENTMAIL_WEBHOOK_SECRET` is **required** in production. Without it, all inbound webhooks are rejected with **HTTP 401** (fail-closed). The AgentMail API token (`AGENTMAIL_API_KEY`) is **not** used as a fallback signing secret — a dedicated webhook secret is mandatory.
</Warning>
...
AgentMail webhooks are verified against these signature headers (checked in order):
- `X-AgentMail-Signature`
- `X-Webhook-Signature`
- `X-Signature`

For local development without a real AgentMail platform:

```bash
PRAISONAI_INSECURE_WEBHOOKS=true praisonai bot agentmail --mode webhook
```

<Note>
See [Webhook Verification](/docs/features/webhook-verification) for the full fail-closed behaviour, decision diagram, and the `PRAISONAI_INSECURE_WEBHOOKS` dev escape hatch.
</Note>
...
| `AGENTMAIL_WEBHOOK_SECRET` | AgentMail | — | HMAC signing secret for webhook mode (required in production) |
