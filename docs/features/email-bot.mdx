...
bot = AgentMailBot(token=os.getenv("AGENTMAIL_API_KEY"), agent=agent)
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Email Bot
...
<Tip>
`Bot("email", agent=agent).run()` auto-reconnects on a dropped connection by default and stays supervised until the IMAP poll loop stops — see [Auto-reconnect on `long-running-bots`](/docs/features/long-running-bots#auto-reconnect-default).
</Tip>

<Note>
`praisonai bot email` and all four `agentmail` modes now stay running correctly. Previously these non-blocking adapters exited `0` in about a second, so `systemd Restart=on-failure` never restarted them. The CLI now holds the loop open while the adapter is running, and startup failures exit **1**.
</Note>
...
bot = AgentMailBot(token=os.getenv("AGENTMAIL_API_KEY"), agent=agent, mode="ws")
await bot.start()
```
<!-- slide -->
```python
# Hybrid — recommended
bot = AgentMailBot(token=os.getenv("AGENTMAIL_API_KEY"), agent=agent, mode="hybrid")
await bot.start()
```
<!-- slide -->
```python
# Webhook — production
bot = AgentMailBot(
    token=os.getenv("AGENTMAIL_API_KEY"), agent=agent,
...
For local development without a real AgentMail webhook, set `PRAISONAI_INSECURE_WEBHOOKS=true` — never in production. See [Webhook Verification](/features/webhook-verification).
...
# Default: a UUID is auto-generated per logical send
...
```python
# Advanced: reuse a stable key so a re-send is dedup'd by AgentMail
await bot.send_message(
    channel_id="user@example.com",
    content={"subject": "Order confirmation", "body": "..."},
    idempotency_key=f"order-{order_id}",   # 1–256 chars, honoured by AgentMail for 24h
)
```
...
sequenceDiagram
    ...
    participant Bot as 📧 AgentMailBot
    participant AM as ☁️ AgentMail

    Agent->>Bot: send_message(...)
    Note over Bot: derive ONE Idempotency-Key
    Bot->>AM: messages.send (Idempotency-Key: K)
    AM--xBot: timeout (email already accepted)
    Bot->>AM: retry (same Idempotency-Key: K)
    AM-->>Bot: 200 (deduplicated — no second email)
    Bot-->>Agent: BotMessage
```
...
from praisonaiagents.tools import (
    ...
    search_emails, archive_email, draft_email, resolve_profiles,
)
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `channel_id` | `str` | — | Recipient email address |
| `content` | `str \| dict` | — | Body string, or `{"subject": ..., "body": ...}` |
| `reply_to` | `str \| None` | `None` | Message-ID to reply to |
| `thread_id` | `str \| None` | `None` | Thread ID for conversation tracking |
| `idempotency_key` | `str \| None` | auto-generated UUID | Stable key forwarded as AgentMail's `Idempotency-Key` header. The same value is reused across retries of one logical send, so a timeout-after-send never duplicates the email. 1–256 chars; 24h key lifetime per AgentMail. |
