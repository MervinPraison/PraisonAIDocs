...
## Fast rollouts (`fast_inference`)

GRPO always ran the slow HF-generate path for its rollouts. Flip `fast_inference: true` to swap it for vLLM.
...
|-----|------|---------|-------------|
| `fast_inference` | `bool` | `false` | Swap HF-generate for vLLM rollouts. |
| `gpu_memory_utilization` | `float` | — | vLLM GPU memory fraction. Only forwarded when `fast_inference` is on. |
| `max_lora_rank` | `int` | — | Max LoRA rank vLLM allocates for. Only forwarded when `fast_inference` is on. |

The two companion knobs are ignored unless `fast_inference` is on. See [Unsloth Surface](/features/praisonai-train-unsloth-surface#vllm-fast-rollouts-grpo) for the full walkthrough.
...
<Card title="Unsloth Surface" icon="sliders" href="/features/praisonai-train-unsloth-surface">
  vLLM fast rollouts, model access, PEFT selectors, and offline merge.
</Card>
</CardGroup>
