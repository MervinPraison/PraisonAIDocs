...
Change agent instructions or a single channel token in `gateway.yaml` and save. The watcher applies a selective reload within ~5 seconds (1s debounce). Credential fields can also be [secret references](/features/gateway-secret-references) (`{source, id}`) instead of plaintext.
...
| `scheduler.*`, `routes.*`, `routing.*` | **FULL** — full channel restart; the candidate is [validated first](#full-restart-safety-validated-cutover--rollback) and the previous config is restored if channels fail to start |
...
| Entire `channels` section (bare) | **FULL** — full channel restart; the candidate is [validated first](#full-restart-safety-validated-cutover--rollback) and the previous config is restored if channels fail to start |
...
<Note>
Token-free channels survive a reload cleanly. The `local` channel is on the gateway's token-free allowlist alongside `email`, `agentmail`, and `signal`, so a `gateway reload` keeps it healthy instead of dropping it into a degraded state. Before this fix the hot-reload path (`_start_single_channel`) required a token and marked `local` degraded on every reload. See [Local (Terminal) Channel](/features/local-channel) and [Degraded Channel Isolation](/features/gateway-degraded-channels).
</Note>

<Note>
Changing `unknown_user_policy` on a channel with an empty `allowed_users` re-fires the startup warning with the new policy's text ([PR #2856](https://github.com/MervinPraison/PraisonAI/pull/2856)). Existing sessions are unaffected — the change applies to inbound DMs after the reload commits. Since [PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112), the re-fired banner text also matches the resolved policy (`deny` → "responds to NOBODY new", `allow` → "responds to EVERYONE", `pair` → "unknown users must complete pairing") instead of always claiming the bot responds to everyone.
</Note>

---

## Full-restart safety: validated cutover & rollback

A structural edit to `gateway.yaml` is now an atomic, validated cutover — a schema-valid but runtime-invalid edit is a rejected reload with the old gateway still up, not an outage.
...
agent = Agent(
    name="ops-bot",
    instructions=(
        "Answer ops questions about safe gateway edits. Stay connected — "
        "even a bad structural edit to gateway.yaml no longer takes me down."
    ),
)

agent.start(
    "Under the hood: on a full-restart edit (e.g. gateway.port or channels "
    "added/removed), the gateway validates the new config first. If it fails, "
    "the previous config keeps serving. If channels fail to start after the "
    "cutover, the previous config is automatically restored."
)
```

The candidate config is built and proven before any live channel is drained; only a passing candidate is swapped in.

```mermaid
graph LR
    Save[✏️ Save gateway.yaml] --> Diff[🔍 Diff detects FULL]
    Diff --> Validate{🧪 Validate candidate}
    Validate -->|ok=false| Reject[❌ Reject reload]
    Reject --> KeepOld[✅ Previous config keeps serving]
    Validate -->|ok=true| Drain[⏳ Drain live channels]
    Drain --> Cutover[⚡ Activate candidate agents]
    Cutover --> Start[🚀 start_channels]
    Start -->|success| Live[✅ New config live]
    Start -->|raise or all-degraded| Restore[🛟 Restore previous config]
    Restore --> KeepOld
    ...
    class Save edit
    class Validate,Diff check
    class Drain,Cutover,Start,Restore process
    class Live,KeepOld good
    class Reject bad
```

### How it works

The candidate agents stay off the live registry until after the drain, so in-flight turns keep resolving the previous agents right up to the cutover.

```mermaid
sequenceDiagram
    participant Operator
    participant Gateway
    participant LiveChannels as Live channels
    participant NewChannels as New channels

    Operator->>Gateway: Save gateway.yaml (structural)
    Gateway->>Gateway: validate_candidate(new_cfg)
    alt Candidate build fails
        Gateway-->>Operator: reload=failed, live untouched
    else Candidate ok
        Gateway->>LiveChannels: stop_channels (drain)
        Gateway->>Gateway: activate candidate agents
        Gateway->>NewChannels: start_channels
        alt Start ok and at least one channel live
            Gateway-->>Operator: reload=ok
        else Start raises OR every channel degraded
            Gateway->>Gateway: restore previous config
            Gateway->>LiveChannels: rebuild previous agents + start
            Gateway-->>Operator: reload=failed, previous config restored
        end
    end
```
...
Every outcome below is visible through the existing [`health().reload`](#reload-status-in-health) payload — grep for your scenario.

| Scenario | Live channels drained? | Live agents mutated? | Final state | `health().reload.last_result` | `health().reload.error` |
|---|---|---|---|---|---|
| Agent build fails (unreachable model, adapter throws on load, bad wiring) | ❌ No | ❌ No | Previous config still serving | `"failed"` | `agent build failed: <exception>` |
| Agent build succeeds, all channels start OK | ✅ Yes (proven candidate first) | ✅ Yes (atomically, after drain) | New config live | `"ok"` | `None` |
| Agent build succeeds, `start_channels` **raises** | ✅ Yes | Restored to previous | Previous config brought back up | `"failed"` | `<start_channels exception>` |
| Agent build succeeds, every new channel is **degraded** (no live bot) | ✅ Yes | Restored to previous | Previous config brought back up | `"failed"` | `all channels failed to start (<name>: <reason>; …)` |
| Selective (non-`FULL`) reload | Unchanged | Unchanged | Unchanged | Unchanged | Unchanged |

The selective-reload paths (`AGENTS`, `CHANNEL`, `HOT`) are untouched — the safety guarantees add to `FULL` only.

### Observability of a rejected or rolled-back reload

A rejected candidate and a rolled-back cutover both surface as `last_result="failed"` with a specific `error`.

```python
# After a rejected candidate:
print(gateway.health()["reload"])
# {'watcher': 'active', 'last_result': 'failed', 'last_at': 1731801234.5,
#  'changed_paths': ['gateway.port', 'channels.discord.token'],
#  'error': 'agent build failed: unreachable model'}

# After a rolled-back cutover:
print(gateway.health()["reload"])
# {'watcher': 'active', 'last_result': 'failed', 'last_at': 1731801240.1,
#  'changed_paths': ['gateway.port', 'channels.discord.token'],
#  'error': 'all channels failed to start (discord: credential unavailable)'}
```

See [ReloadStatus fields](#reloadstatus-fields) for the full schema.

### Programmatic pre-flight

Any orchestrator can await validation before writing a file — `validate_candidate` does not touch the live runtime, so it is safe to call from a canary or deploy script before saving `gateway.yaml`.

```python
from praisonaiagents.gateway import ReloadValidationProtocol, CandidateReport

assert isinstance(gateway, ReloadValidationProtocol)

report: CandidateReport = await gateway.validate_candidate(candidate_cfg)
if not report.ok:
    raise RuntimeError(f"Candidate rejected: {'; '.join(report.failures)}")
# Only reached when the candidate is safe to apply.
```

`CandidateReport` carries the three fields the wrapper returns:

| Field | Type | Default | Meaning |
|---|---|---|---|
| `ok` | `bool` | *(required)* | Whether the candidate built and passed pre-flight — only then may the live runtime be drained and the candidate swapped in. |
| `failures` | `list[str]` | `[]` | Human-readable reasons the candidate was rejected (empty when `ok`); joined into `health().reload.error`. |
| `candidate` | `Optional[Any]` | `None` | Opaque handle to the already-built candidate runtime activated on success; `None` when the runtime validates in place. |

`ReloadValidationProtocol` is `runtime_checkable`, so `isinstance(gateway, ReloadValidationProtocol)` holds for every gateway build — the "no cutover to an unvalidated candidate" guarantee is canonical, not per-entry-point.

<Tip>
**Safety guarantees**
- **Never cuts over to an unvalidated candidate.** Candidate agents build in an isolated registry and the live registry is snapshotted; a build failure restores it byte-for-byte before returning.
- **In-flight turns during the drain window see the previous agents.** The candidate is installed into the live registry only after `stop_channels` has quiesced ingress.
- **A wholly-broken replacement channel set never commits as `ok`.** `start_channels` is per-channel fault-tolerant (marks failures degraded and returns), so the reload checks that at least one channel came up live and rolls back otherwise.
</Tip>

<Note>
**Backward compatibility:** Additive only — every non-`FULL` reload path is unchanged. `validate_candidate({})` returns `CandidateReport(ok=True)` — a config with no `agents` section is a trivial pass, matching the pre-PR behaviour. `CandidateReport` and `ReloadValidationProtocol` are new exports from `praisonaiagents.gateway`; no old imports were renamed. `health()` keys and `ReloadStatus` fields are unchanged — new failure paths just populate `last_result="failed"` + `error=<reason>` using the existing schema.
</Note>
...
The [`pressure`](/features/gateway-pressure-telemetry) block is a sibling additive `health()` key that follows this exact `to_dict()` pattern — present only when the back-pressure machinery is wired.
...
<Card title="Gateway Channel Supervision" icon="shield" href="/features/gateway-channel-supervision">
  Self-healing channels
</Card>
<Card title="Code-Skew Guard" icon="shield-halved" href="/features/gateway-code-skew-guard">
  Detect in-place code updates and refuse hot operations until the process restarts.
</Card>
<Card title="Gateway Reliability Preset" icon="shield-check" href="/features/gateway-reliability-preset">
