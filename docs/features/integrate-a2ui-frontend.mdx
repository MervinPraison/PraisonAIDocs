...
```mermaid
graph LR
    subgraph "A2UI Integration"
        A[🤖 Agent] --> B[📤 send_a2ui_messages]
        B --> C[📶 mime: application/json+a2ui]
        C --> D[🎨 React Renderer]
        C --> E[🔗 Custom Mapper]
    end
    ...
    classDef payload fill:#6366F1,stroke:#7C90A0,color:#fff
...
agent = Agent(
    name="assistant",
    instructions="When asked for UI, call send_a2ui_messages with A2UI v0.9 JSON.",
    tools=[send_a2ui_messages],
)
```
</Step>

<Step title="Detect the payload in your frontend">
```python
...
sequenceDiagram
    ...
    Agent->>Tool: send_a2ui_messages(messages)
    Tool-->>Agent: {mime_type, messages, a2ui_part}
    Agent-->>UI: tool result
    UI->>UI: detect mime_type == application/json+a2ui
    UI->>UI: render with A2UI renderer
```
...
<AccordionGroup>
<Accordion title="Always check mime_type before rendering">
Never assume every tool result is A2UI. Guard with `result.get("mime_type") == "application/json+a2ui"` to avoid rendering errors.
</Accordion>

<Accordion title="Use the simplest tier that meets your needs">
Start at Tier 0 (markdown streaming) and only move to Tier 3 (A2UI) if you need cross-platform surface rendering.
</Accordion>

<Accordion title="Wire user actions back to the agent">
A2UI is bidirectional. POST button clicks and form submissions back to your agent as a new turn for the interaction to be useful.
</Accordion>

<Accordion title="Use PraisonAIUI as the reference implementation">
For complex rendering logic (surface IDs, version fields), copy or vendor `a2ui_utils.py` from PraisonAIUI rather than reimplementing from scratch.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="A2UI Protocol" icon="code" href="/features/a2ui">
  A2UI tool result protocol details
</Card>
<Card title="Generative UI" icon="wand-magic-sparkles" href="/features/generative-ui">
  Full tier list for generative UI patterns
</Card>
</CardGroup>
