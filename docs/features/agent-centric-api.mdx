...
```python
# Consolidated params take precedence over legacy flat params
# Use only one output setting — duplicate keys are not valid Python
agent = Agent(
    instructions="...",
    output="verbose",  # Use this; do not pass output twice
)
```
