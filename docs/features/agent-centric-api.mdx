...
sequenceDiagram
    ...
    User->>Agent: Configure once, then prompt
    Agent->>Features: Apply memory / execution / output
    Features-->>Agent: Configured behaviour
    Agent-->>User: Response
```
...
`max_steps` is the unified outer-loop step budget honoured identically by both execution loops. Detect truncation with `agent.last_stop_reason == "max_steps"`. See [Step Budget](/features/max-steps).
...
# Or a non-OpenAI embedder
agent = Agent(instructions="...", knowledge=KnowledgeConfig(sources=["docs/"], embedder="gemini"))
```
...
# NOTE: policy-string guardrails (e.g. guardrails=["policy:strict", "pii:redact"])
# were removed in PraisonAI PR #3790 and now raise ValueError. For tool allow/deny
# enforcement, use Agent(policy=PolicyEngine(...)) — see /features/policy-engine.
...
### Keyword-Only Legacy Params

Seven previously-inline params are now **keyword-only** and consolidated into config objects. Passing them by keyword still works and still emits a `DeprecationWarning`; the config-object form below is preferred.

| Old param (keyword-only) | New config form |
|--------------------------|-----------------|
...
| `auto_save` | `memory=MemoryConfig(auto_save="name")` |
| `rate_limiter` | `execution=ExecutionConfig(rate_limiter=obj)` |
| `verification_hooks` | `autonomy=AutonomyConfig(verification_hooks=[...])` |
| `cli_backend` | `runtime=...` |

<Warning>
Since 2026-08 these seven params are **keyword-only**. Passing a **positional** argument in the slot where `allow_delegation` used to sit now raises `TypeError`. Old keyword usage still works with a `DeprecationWarning`.

Unknown kwargs (typos like `Agent(..., some_typo=1)`) now raise `TypeError` instead of being silently ignored.
</Warning>
...
<CardGroup cols={2}>
  <Card icon="book" href="/configuration/agent-config" title="Agent Config">
  ...
  <Card icon="arrow-right-arrow-left" href="/features/agent-legacy-params" title="Legacy Agent Parameters">
    Migrate the 7 deprecated `Agent()` params to config objects.
  </Card>
</CardGroup>
