...
| `code_sandbox_mode` | `str` | `"sandbox"` | `"sandbox"` / `"direct"` — **NOT IMPLEMENTED, no execution path reads this.** Kept for backward compat. Use `Agent(sandbox=…)` for local isolation or `AgentFlow(run_on=…)` for a remote workflow sandbox. |
