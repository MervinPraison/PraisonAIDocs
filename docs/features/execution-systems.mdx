...
| `code_sandbox_mode` | `str` | `"sandbox"` | `"sandbox"` / `"direct"` — **NOT IMPLEMENTED, no execution path reads this.** Kept for backward compat. `Agent(sandbox=…)` only isolates explicit `agent.execute_code(...)` calls (not model-driven execution); for isolating code the model runs, use `AgentFlow(run_on=…)` or `LocalAgent(compute=…)`. |
