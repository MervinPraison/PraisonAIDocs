...
`max_steps` is the single tool-use budget honoured identically by both execution loops. Detect truncation with `agent.last_stop_reason == "max_steps"`. See [Step Budget](/docs/features/max-steps).
