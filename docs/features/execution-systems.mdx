...
<Note>
`code_sandbox_mode` was removed — no execution path ever read it. To isolate code the model runs, use `AgentFlow(tools_run_on=…)` or `Agent(tools_run_on=…)`. See [Placement](/features/placement).
</Note>
