...
sequenceDiagram
    participant User
    participant Agent
    participant ExecutionExtensionSystems

    User->>Agent: Request
    Agent->>ExecutionExtensionSystems: Process
    ExecutionExtensionSystems-->>Agent: Result
    Agent-->>User: Response
```
...
<Card title="ExecutionConfig SDK Reference" icon="code" href="/docs/sdk/reference/python/classes/ExecutionConfig">
...
`max_steps` is the unified outer-loop step budget honoured identically by both tool-execution loops. Detect truncation with `agent.last_stop_reason == "max_steps"`. See [Step Budget](/docs/features/max-steps).
