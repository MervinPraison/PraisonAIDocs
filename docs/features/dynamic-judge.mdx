...
<Note>
Unparseable verdicts fail closed — an unreadable judge response returns `score=1.0, passed=False`, so a low threshold never produces a false pass. See [LLM as Judge — Fail-Closed Parsing](/features/llm-as-judge#fail-closed-parsing).
</Note>
...
<Card title="Evaluator Optimiser" icon="rotate" href="/features/evaluator-optimiser">
