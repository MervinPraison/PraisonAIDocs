...
<Warning>
**Breaking change:** `AgentTeam(verbose=…)`, `AgentTeam(max_iter=…)`, and `AgentTeam(session=…)` now raise `TypeError`. Use `output=`, `execution=`, and `variables=` instead. See [Code Migration → API Migration](/features/migrate#api-migration-verbose-to-output).
</Warning>
...
<Card title="MultiAgentOutputConfig API Reference" icon="code" href="/sdk/reference/praisonaiagents/classes/MultiAgentOutputConfig">
  Full API reference
</Card>
...
<CardGroup cols={2}>
  <Card icon="bolt" href="/features/streaming">
    Single-agent streaming (auto-detect + fallback)
  </Card>
  <Card icon="display" href="/configuration/output-config">
    Single-agent output configuration
  </Card>
  <Card icon="users" href="/features/multi-agent-patterns">
    Sequential, parallel, hierarchical patterns
  </Card>
  <Card icon="code" href="/sdk/reference/praisonaiagents/classes/MultiAgentOutputConfig">
