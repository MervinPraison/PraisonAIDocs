...
Multi-agent output controls how an `AgentTeam` displays its progress and results. Streaming is off by default for stability across providers — enable it explicitly when you want token-by-token output.
...
sequenceDiagram
    ...
    participant Config as Output Config
    participant Agents
    participant Display
    
    User->>AgentTeam: start("task")
    AgentTeam->>Config: Resolve output settings
    Config-->>AgentTeam: verbose, stream settings
    
    loop Each Agent
        AgentTeam->>Agents: Execute task
        Agents->>Display: Emit progress (if verbose > 0)
        Display-->>User: Show formatted output
    end
    
    AgentTeam-->>User: Return final result
```
...
|--------|-----------|----------|----------|
| `"silent"` | `0` | `False` | Programmatic / API / batch (default) |
| `"minimal"` | `1` | `False` | Short status lines |
| `"verbose"` | `2` | `False` | Rich panels, full trace |

<Note>
**Streaming is OFF in every preset.** This changed in PR #1911 to make multi-agent teams work on every provider, including sync-only adapters. Opt in explicitly with `output=["verbose", {"stream": True}]` or `output=MultiAgentOutputConfig(verbose=2, stream=True)`.
</Note>
...
| `stream` | `bool` | `False` | Stream tokens as the team's agents respond |

---

## Enabling streaming for a multi-agent team
...
<Warning>
Streaming a multi-agent team requires every agent's LLM provider to support sync streaming. Sync OpenAI adapter and Deepseek (sync) do not — use `stream=False` (the default) for those.
</Warning>
