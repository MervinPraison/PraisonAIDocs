...
|-------|------|---------|-------------|-------------|
| `identity_resolver` | `Optional[Any]` | `None` | Unifies the same human across platforms onto one session key. | [Cross-Platform Mirror](/docs/features/cross-platform-mirror) |
| `delivery_router` | `Optional[Any]` | `None` | Backing router for the `send_message` tool (proactive mid-turn delivery). | [Proactive Delivery](/docs/features/proactive-delivery) |
| `admission_gate` | `Optional[Any]` | `None` | Gateway-wide concurrency ceiling / fair queue / backpressure on inbound runs. | [Gateway Admission Control](/docs/features/gateway-admission-control) |
| `turn_lock_map` | `Optional[Any]` | `None` | Shared per-turn lock map for cross-platform turn serialisation on the resolved session id. | [Cross-Platform Mirror](/docs/features/cross-platform-mirror) |
...
<Card title="Cross-Platform Mirror" icon="users" href="/docs/features/cross-platform-mirror">
