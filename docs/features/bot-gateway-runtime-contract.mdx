...
description: "Let your custom channel adapter receive the gateway's five reliability seams — admission control, delivery routing, identity resolution, identity canonicalization, per-turn locking"
icon: "plug"
---

Custom channel adapters implement `SupportsGatewayRuntime` to receive the gateway's five reliability seams; miss the contract and the gateway fails loudly instead of silently dropping admission control, delivery routing, and cross-platform turn locking.
...
class MyBot:
    def __init__(self, agent):
        ...
        # and wires all five seams automatically.
...
class MyBot:
    def attach_gateway_runtime(self, runtime: GatewayRuntimeSeams) -> None:
        ...
        if runtime.identity_canonicalizer is not None:
            self._identity_canonicalizer = runtime.identity_canonicalizer
...
sequenceDiagram
    ...
    Note over Bot: all five seams None? → no-op return
...
|------|-----------|---------|
| 1 | All five seams are `None` | Return immediately — pure no-op. |
...
| `identity_canonicalizer` | `Optional[Any]` | `None` | Reconciles a volatile raw platform id (WhatsApp JID→LID, number↔UUID alias flip) to a stable canonical id **before** the `identity_resolver` runs, so the same human keeps one session instead of silently forking. Any object satisfying `IdentityCanonicalizerProtocol` — an active `BasePlatformAdapter` implements it. | [Identity Canonicalization](/features/gateway-identity-canonicalization) |
...
| `turn_lock_map` | `Optional[Any]` | `None` | Shared per-turn lock map for cross-platform turn serialisation on the resolved session id. See [Gateway Turn Lock](/features/gateway-turn-lock) for the pure protocol contract (`TurnLockProtocol` / `LocalTurnLock`) that this seam now conforms to. | [Cross-Platform Mirror](/features/cross-platform-mirror) |
...
If your adapter already builds a `BotSessionManager`, expose it as `self._session` and do nothing else. The gateway finds `attach_gateway_runtime` on the session and wires all five seams for free.
</Accordion>
