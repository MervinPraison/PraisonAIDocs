...
graph LR
    subgraph "Deferred Resolution"
        ...
        Resolve --> Inject["✅ Re-injected into chat_history"]
    end
...
The run loop registers the handle automatically, so tool authors only call `resolve_deferred(...)` from the completion callback — no `LLM` wiring needed.
...
<CardGroup cols={2}>
  ...
  <Card title="Async Tool Safety" icon="rotate" href="/features/async-tool-safety">
