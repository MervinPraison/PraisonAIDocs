...
agent = Agent(name="multi-channel-bot", instructions="Respond across Telegram, Slack, and Discord.")
...
<Note>
`group_policy` accepts any of `mention_only`, `command_only`, `respond_all`, or `observe` per channel. `observe` keeps unmentioned Telegram group messages as passive context. See [`observe`: passive group context](/docs/features/bot-gateway#observe-passive-group-context).
</Note>
...
<Tab title="Local + remote continuity">
Add the built-in `local` channel next to a remote one — start a conversation in your terminal and keep the same session going from your phone.
...
Both surfaces share one resolved session id via the identity resolver and delivery router. See [Local (Terminal) Channel](/features/local-channel).
</Tab>
</Tabs>
...
<CardGroup cols={2}>
<Card title="Bot Onboarding" icon="user-plus" href="/features/onboard">
