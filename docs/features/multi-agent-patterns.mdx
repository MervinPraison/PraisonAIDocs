...
triage = Agent(name="Triage", instructions="Route the request to the right specialist")
triage.start("Help me debug a failing CI pipeline")
```
...
<Card title="Handoff Configuration" icon="cog" href="/configuration/handoff-config">
...
<Note>
All context knobs (`context_policy`, `max_context_messages`, `preserve_system`, and the `input_filter` chain) took effect starting with PR #4213. On earlier releases the target received only the literal handoff prompt regardless of these settings.
</Note>
