...
<Note>
Compaction runs on **all** entry points — including `start(stream=True)` and `iter_stream()`. Before PraisonAI PR [#4729](https://github.com/MervinPraison/PraisonAI/pull/4729) the streaming path skipped compaction; since #4729 it is applied identically. See [Streaming → long histories](/features/streaming#streaming-with-long-histories).
</Note>
