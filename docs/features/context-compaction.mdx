...
<Note>This runtime `ContextCompactor` (the `/context compact` slash command and `ExecutionConfig(context_compaction=True)`) is **unrelated** to the `praisonai context compact` CLI. The CLI operates on the process-local [`get_global_store()`](/features/context-api#scope-process-local-in-memory-opt-in) and never touches an agent's live `chat_history`.</Note>
...
<Note>
This runtime `ContextCompactor` (the `/context compact` slash command and `ExecutionConfig(context_compaction=True)`) is unrelated to the `praisonai context compact` CLI, which operates on the CLI-process-local `get_global_store()`. See [CLI → context](/cli/context#why-is-my-store-empty).
</Note>

<Note>
As of PraisonAI [#4203](https://github.com/MervinPraison/PraisonAI/pull/4203), the [pre-compaction memory flush](/features/pre-compaction-memory-flush) is no longer a silent no-op on non-`FileMemory` backends — it durably commits facts on `sqlite`, `chroma`, `mongodb`, `mem0`, and `learn` too.
</Note>

<Note>
Compaction runs on **all** entry points — including `start(stream=True)` and `iter_stream()` — and on **both** streaming branches (OpenAI-client and custom-LLM), so `llm="openai/gpt-4o-mini"` and other routed models get the same treatment as native OpenAI. PraisonAI PR [#4729](https://github.com/MervinPraison/PraisonAI/pull/4729) fixed the OpenAI-client branch; PR [#4753](https://github.com/MervinPraison/PraisonAI/pull/4753) fixed the custom-LLM branch. See [Streaming → long histories](/features/streaming#streaming-with-long-histories).
</Note>
...
Prevents endless compaction cycles in long-running agents by tracking savings effectiveness and giving up when returns diminish — while still re-testing the verdict once the conversation grows materially past where the guard latched, so a long session's compactor never disables itself permanently.

<Note>
As of PraisonAI [#4679](https://github.com/MervinPraison/PraisonAI/pull/4679), the anti-thrashing guard is periodically re-tested after conversations grow. In earlier versions a compactor that hit the streak cap stopped compacting permanently for the rest of the session.
</Note>
...
sequenceDiagram
    ...
    AntiThresh->>AntiThresh: streak < max_consecutive_low_savings?

    alt Streak below cap
        AntiThresh-->>Compactor: Continue
        Compactor->>Compactor: Perform compaction
        Compactor-->>Agent: Return compacted messages
    else Streak at cap
        AntiThresh->>AntiThresh: tokens ≥ 1.25 × tokens_when_latched?
        alt Grown past retry threshold
            AntiThresh->>AntiThresh: Reset streak + latched-size
            AntiThresh-->>Compactor: Release guard, continue
            Compactor->>Compactor: Perform compaction
            Compactor-->>Agent: Return compacted messages
        else Not enough growth
            AntiThresh-->>Compactor: Skip (thrashing)
            Compactor-->>Agent: was_skipped_due_to_low_savings = True
        end
    end
```
...
4. **Reset on successful compaction**: When a compaction pass achieves ≥ `min_savings_pct` savings, the streak clears immediately.
5. **Growth-based release**: If the streak reaches the cap and stays there, the guard remembers the token count it gave up at. Once the conversation grows to ≥ **125%** of that size, the guard releases and the compactor retries with a fresh budget.

<Note>
The retry-growth factor is fixed at **25%** and is not exposed as a config option. This is deliberate: it is the trade-off between silent context growth (no release) and re-running the summariser every turn (thrashing). If you need a different trade-off, tune `min_savings_pct` and `max_consecutive_low_savings` — the release fires relative to the token count at the moment those thresholds were hit.
</Note>
...
The built-in `DefaultSummaryBuilder` renders `SUMMARY_TEMPLATE` deterministically. Provide a custom `SummaryBuilderProtocol` implementation when you want a domain-specific summary (e.g. LLM-distilled, phase-specific vocabulary).

```python
from praisonaiagents.compaction import DefaultSummaryBuilder, SummaryBuilderProtocol
...
Save and reload your `compaction_strategy` across restarts — see [ExecutionConfig Persistence](/docs/configuration/execution-config#persistence) for the `to_dict()` → `from_dict()` round-trip.
...
<Note>
When `structured_template=True` (the default), `ContextCompactor` now uses the built-in `DefaultSummaryBuilder` automatically — no injected builder needed. The default builder renders `SUMMARY_TEMPLATE` **deterministically** (no LLM call), so it is offline-safe and never blocks the hot compaction path. It falls back to `"Summary of N messages"` only if rendering raises.
</Note>
...
# In-loop two-tier compaction is ON by default — nothing to configure
agent = Agent(
    name="LongRunner",
    instructions="You do multi-step research with lots of tool calls.",
)

agent.start("Research 20 vendors, compare pricing, and summarise trade-offs.")
```
...
```mermaid
graph TB
    Iter[🔁 Tool-loop iteration] --> Count[🧮 Count tokens accurately]
    Count --> Q1{Above 50% of window?}
    Q1 -->|No| Pass[⚡ No-op — pass through]
    Q1 -->|Yes| Clear[🧹 Tier 1: Clear old tool results<br/>keep last 6 verbatim]
    Clear --> Q2{Still above 80%?}
    Q2 -->|No| Continue[✅ Continue loop]
    Q2 -->|Yes| Summarise[📝 Tier 2: LLM summarise dialogue<br/>anti-injection framing]
    Summarise --> Continue
    ...
    classDef output fill:#10B981,stroke:#7C90A0,color:#fff
    class Iter,Count input
    class Q1,Q2 decision
    class Clear,Summarise process
    class Pass,Continue output
```

### The Two Tiers

| Tier | Trigger | Action | What it preserves |
|------|---------|--------|-------------------|
| **Tier 1 — Clear** | Tokens ≥ `clear_threshold_pct` × window (default **50%**) | Replace old `role: "tool"` message contents with a short placeholder | Assistant `tool_calls` stay intact, so the model still sees which tool was called with which args; the most recent `keep_recent_tool_results` (default `6`) tool results stay verbatim |
| **Tier 2 — Summarise** | After tier 1, tokens still ≥ `compact_threshold_pct` × window (default **80%**) | Run the structured LLM summariser (`CompactionStrategy.SUMMARIZE`) with anti-injection framing | System messages + last `preserve_recent` messages stay verbatim |

Cleared tool results are replaced with this exact placeholder:

```
[tool result cleared to save context; re-fetch if needed]
```
...
agent = Agent(
    ...
    llm={
        "model": "gpt-4o",
        # Two-tier in-loop compaction knobs
        "in_loop_compaction": True,      # default True
        "clear_threshold_pct": 0.5,      # default 0.5 — clear at 50% of window
        "compact_threshold_pct": 0.8,    # default 0.8 — summarise at 80% of window
        "keep_recent_tool_results": 6,   # default 6 — preserve most-recent N tool results
    },
)
```
...
```python
agent = Agent(name="…", instructions="…", llm={"in_loop_compaction": False})
```
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `model` | `Optional[str]` | `None` | Model name used to pick the correct tiktoken encoding for accurate token counts. When unset, falls back to `gpt-4` encoding when tiktoken is locally available, otherwise the `len//4` heuristic. |
| `in_loop_compaction` | `bool` | `True` | Enable the two-tier clear-then-compact pass between tool iterations. `False` restores pre-#2995 behaviour. |
| `clear_threshold_pct` | `float` | `0.5` | Fraction of the model context window at which tier 1 (clear old tool results) fires. |
| `compact_threshold_pct` | `float` | `0.8` | Fraction of the window at which tier 2 (LLM summarise) fires, after tier 1 has run. |
| `keep_recent_tool_results` | `int` | `6` | Number of most-recent `role: "tool"` messages preserved verbatim during a tier-1 clear pass. |
...
```mermaid
graph TB
    Start[When should I change thresholds?]
    Start --> Q1{Tool loops hit context limit?}
    Q1 -->|Yes, often| Lower[Lower clear_threshold_pct to 0.4<br/>or raise keep_recent_tool_results]
    Q1 -->|No, but summariser is chatty| Raise[Raise compact_threshold_pct to 0.9<br/>defer summarisation]
    Q1 -->|Never hits| Default[Leave defaults — no-op below threshold]
    ...
    class Start input
    class Q1 decision
    class Lower,Raise,Default output
```
...
| `max_context_tokens` | `Optional[int]` | `None` (auto-detect from model) | Token limit before compaction triggers (via litellm's `model_cost` registry when available, otherwise the built-in `MODEL_LIMITS` table). |
...
| `structured_template` | `bool` | `True` | Render `SUMMARY_TEMPLATE` phase-aware sections via the built-in `DefaultSummaryBuilder`. Set `False` for the flat one-line summary. |
...
# Export full metrics — to_dict() now includes "summary"
metrics = result.to_dict()
print(f"Summary: {metrics['summary'][:120]}")
...
| `summary` | `str` | The summary text produced by the summarising strategy this pass (`""` for `TRUNCATE`/`SLIDING`/`PRUNE`). Persisted via the session store's `append_compaction_checkpoint` when `session_id` is bound. Populated in `1.6.152+` — previously always `""`. |

Only summarising strategies fill `result.summary`:

```mermaid
graph TB
    A[Which strategy did you pick?] --> B[LLM iterative /<br/>naive summarise]
    A --> C[TRUNCATE / SLIDING /<br/>PRUNE]
    B --> D[result.summary populated<br/>→ persisted to session store]
    C --> E["result.summary == ''<br/>→ nothing persisted (by design)"]
...
Inspect the populated summary directly:
...
agent = Agent(
    name="Inspector",
    context=CompactionConfig(
        strategy=CompactionStrategy.LLM_SUMMARIZE,  # summarising strategy required
        enable_iterative_summary=True,
    ),
)

compacted, result = agent.compactor.compact(messages)
print(bool(result.summary))         # True — populated by _extract_summary_text
print(result.summary[:120])         # peek at the distilled context
print(result.to_dict()["summary"])  # same value, via to_dict()
```
...
# Observe the summary a compaction pass produced
compacted, result = agent.compactor.compact(agent.conversation_history)
print(f"Has summary: {bool(result.summary)}")   # True for summarising strategies
print(f"Summary peek: {result.summary[:120]}")
```
...
Every compaction strategy keeps each assistant `tool_calls` message paired with its matching `tool` result, so strict providers never reject the next turn.

```python
from praisonaiagents import Agent

# Long-lived assistant with tool-heavy history — compaction now
# guarantees no orphaned tool results, so no 400s from strict providers.
agent = Agent(
    name="Assistant",
    instructions="Answer questions using the available tools.",
    tools=[search_tool, fetch_tool],
    compact_threshold=0.7,   # trigger compaction at 70% of context window
)

agent.start("Keep researching this topic across many turns...")
```

Strict providers (OpenAI, Anthropic, Azure OpenAI) reject a transcript that starts with a `tool` message whose `tool_call_id` has no matching preceding `tool_calls` message — that used to wedge long-lived sessions on every turn after compaction split a pair.

```mermaid
graph LR
    Boundary[📍 Chosen boundary] --> Detect{Orphaned tool<br/>result at head?}
    Detect -->|Yes| Snap[🔧 Snap boundary outward]
    Snap --> Kept[🛡️ tool_calls + tool result<br/>both on kept side]
    Kept --> Accept[✅ Provider accepts request]
    Detect -->|Pre-fix path| Orphan[⚠️ Orphan sent alone]
    Orphan --> Reject[❌ 400 from strict provider]
    ...
    class Snap,Kept snap
    class Accept output
    class Orphan,Reject bad
```

The boundary snap runs at every recent/older split — inside `TRUNCATE`, `SLIDING`, `SUMMARIZE`, `PRUNE`, and `LLM_SUMMARIZE` (sync + async). In `TRUNCATE`, the fallback "drop more" loop also removes a leading assistant `tool_calls` message together with all its immediately-following `tool` results, so it never strands an orphan at the head either.

<Note>
`preserve_recent` still controls how many recent messages are kept. To keep a tool pair intact, the compactor may keep **one or two more** than `preserve_recent` — never fewer. The boundary only ever snaps outward (to a lower index), growing the kept window.
</Note>

### Parallel tool calls

A single assistant turn can request several tools at once — the sliding strategy reconciles all their results to a fixed point.

A single assistant turn can request several tools at once (parallel tool calls) — the assistant message carries `tool_calls: [A, B, ...]` and is followed by one `role: "tool"` result per id. When a sliding window keeps only some of these results, the sliding strategy now iterates its pair reconciliation to a fixed point: re-inserting the shared `tool_calls=[A, B]` message to satisfy B also detects that A is now missing its response and pulls A's result back in. The final message list never contains a `tool_call` id without its matching response, so strict providers (OpenAI, Anthropic, Azure OpenAI) accept the next request. Fixed in PraisonAI [PR #4034](https://github.com/MervinPraison/PraisonAI/pull/4034).

```mermaid
graph LR
    Keep[🛡️ keep B_result] --> Reinsert[🔧 reinsert msg tool_calls=A,B]
    Reinsert --> Missing{A missing its response?}
    Missing -->|Yes| PullA[🔧 reinsert A_result]
    PullA --> Check[🔁 check again]
    ...
    class Keep input
    class Reinsert,PullA,Check process
    class Missing decision
    class Balanced output
```

Manually stitching tool pairs after compaction is unnecessary — the fixed-point loop guarantees a balanced transcript.

<CardGroup cols={2}>
<Card title="Optimizer" icon="gauge" href="/features/optimizer">
  Also preserves tool_call/tool_result pairs to maintain API validity
</Card>
</CardGroup>
...
> `BEFORE_COMPACTION` and `AFTER_COMPACTION` hook events now fire consistently around every compaction (both sync and async). See [Hook Events](/docs/features/hook-events#system-events).

<Note>
`AFTER_COMPACTION` now receives a populated `CompactionResult.summary` for summarising strategies (previously always `""`), so plugin persisters that subscribe to `AFTER_COMPACTION` can finally read the summary text. See [Reading the summary from a hook](/docs/features/hook-events#system-events).
</Note>
...
As of `1.6.152+`, this applies to **both** `agent.start(...)` and `agent.astart(...)` / `agent.achat(...)` / `agent.arun(...)`. Prior versions silently skipped persistence for async proactive compaction; [PR #3081](https://github.com/MervinPraison/PraisonAI/pull/3081) fixed it and routes the checkpoint write through `asyncio.to_thread` so the event loop is not blocked.
...
<Note>
This is fully backward compatible — sessions without a checkpoint resume from raw messages exactly as before. See [Compacted Session Resume](/features/session-compaction-checkpoint) for the persistence details.
</Note>
...
See [Intelligent Conversation Compaction](/features/intelligent-conversation-compaction) for detailed usage.
...
<CardGroup cols={2}>
<Card title="Memory Management" icon="brain" href="/features/advanced-memory">
  Long-term memory storage and retrieval
</Card>
<Card title="Agent Configuration" icon="settings" href="/features/agent-profiles">
  Complete agent configuration options
</Card>
<Card title="Context Budgeter" icon="gauge" href="/features/context-budgeter">
...
<Card title="Session Handoff" icon="clock-rotate-left" href="/features/session-handoff">
  Turn the durable recap into a continuation prompt for a fresh context
</Card>
</CardGroup>
