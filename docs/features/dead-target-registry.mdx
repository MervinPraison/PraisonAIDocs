...
<Note>
The same permanent-failure branch that marks a target dead is where the `MESSAGE_UNDELIVERED` hook fires and the opt-in fallback notice is sent. See [Undelivered Message Notice](/features/undelivered-messages) to close the loop with the user and your operators.
</Note>
...
The registry prevents re-sending to dead targets, but in-flight messages that fail permanently should also go to the outbound DLQ. See [Durable Delivery](/features/durable-delivery) and [Delivery Config](/features/delivery-config).
</Accordion>
...
<Card title="Proactive Delivery" icon="send" href="/features/proactive-delivery">
