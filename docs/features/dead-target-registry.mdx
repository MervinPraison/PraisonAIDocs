...
<Note>
The same permanent-failure branch that marks a target dead is where the `MESSAGE_UNDELIVERED` hook fires and the opt-in fallback notice is sent. See [Undelivered Message Notice](/docs/features/undelivered-messages) to close the loop with the user and your operators.
</Note>
