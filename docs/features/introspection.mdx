...
The warning fires for `subprocess` / `local` **only** — never for `sandlock` / `native`, `docker`, or any hosted provider. Since [PR #4107](https://github.com/MervinPraison/PraisonAI/pull/4107), `native` (an alias for `sandlock`: Landlock + seccomp + scrubbed env + deny-all networking) is correctly excluded from the warning, so `tools_run_on="native"` and `tools_run_on="sandlock"` no longer give opposite advice for the same backend.
</Accordion>
