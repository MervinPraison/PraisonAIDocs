...
Set `PRAISONAI_STATE_URL` or pass `state_url=` when creating agents. See [Database Persistence](/features/database-persistence) for configuration options.
...
- Uses `state_store.scan_prefix(prefix)` when available (non-blocking cursor `SCAN` on Redis and Upstash; prefix-scoped `keys()` fallback on other backends). This replaces the previous `KEYS praison:*` pattern, so a "recent runs" read never blocks other clients on the same Redis instance.
...
<CardGroup cols={2}>
<Card title="Database Persistence" icon="database" href="/features/database-persistence">
  Configure state stores for persistence
</Card>
<Card title="Session Management" icon="users" href="/features/persistence">
