...
<Warning>
The suggestion store's isolation is **opt-in**: pass the resolved user identity as `principal=` on every read/write call. If the caller passes `principal=None` (the default), suggestions live in a global pool that every caller can see and mutate — see the [Multi-tenant isolation](#multi-tenant-isolation) section above.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Gate automations behind admins until identity wiring lands">
