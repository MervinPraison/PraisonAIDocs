...
Pass the resolved identity on every read or mutation. One user's pending queue is then invisible and non-mutable to another's:
...
The `principal` string is a canonical identity (e.g. `telegram:12345`, `slack:U01ABC`) chosen by the caller — core does not invent a schema.
...
<AccordionGroup>
  <Accordion title="Isolate suggestions per user on multi-user gateways">
    For multi-user gateways, pass the resolved end-user identity as `principal=` when reading or mutating suggestions — one user's pending queue is then invisible and non-mutable to another's:
