...
`decode_client_frame` covers the **inbound** contract only. The gateway also relays **outbound** stream events — `token_stream`, `tool_call_stream`, `stream_end`, `reasoning_stream`, `tool_progress_stream`, and `stream_error` — plus `accepted` vs `final` response frames after a client negotiates `streaming`. Those six event types and the response-frame states are documented in [Gateway Stream Events](/docs/features/gateway-stream-events).
...
<Card title="Gateway Stream Events" icon="waveform-lines" href="/docs/features/gateway-stream-events">
