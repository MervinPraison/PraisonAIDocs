...
`HelloParams` is documented on the [Gateway Handshake Protocol](/features/gateway-handshake-protocol) page (the `hello` half of the same union). The codec adds two conveniences to the hello path: it tolerates both the flat `protocol_min` / `protocol_max` shape **and** the legacy nested `protocol: {min, max}` shape, and it accepts a `capabilities` / `caps` alias.
...
| `request_id` | `Optional[str]` | `None` | Optional request-idempotency key. A resend of the same id runs the turn exactly once — see [Gateway Client → Exactly-Once Send](/features/gateway-client#exactly-once-send-queue-and-flush) |
| `attachments` | `List[AttachmentRef]` | `[]` | Optional first-class attachments (files) carried on the message turn — see [Gateway Attachments](/features/gateway-attachments). Each entry is either inline (`data` base64) or by reference (`ref_id` in the gateway attachment store), validated against the advertised policy |
...
<Note>
A frame with no `attachments` decodes exactly as before — the field defaults to `[]`, so existing clients are unaffected.
</Note>
...
`decode_client_frame` covers the **inbound** contract only. The gateway also relays **outbound** stream events — `token_stream`, `tool_call_stream`, `stream_end`, `reasoning_stream`, `tool_progress_stream`, and `stream_error` — plus `accepted` vs `final` response frames after a client negotiates `streaming`. Those six event types and the response-frame states are documented in [Gateway Stream Events](/features/gateway-stream-events).
...
The `HelloError` fields and full recovery-step vocabulary are documented on the [Gateway Handshake Protocol](/features/gateway-handshake-protocol) and [Gateway Error Handling](/features/gateway-error-handling) pages — `error.to_dict()` produces the same wire shape used there.
...
<Card title="Session Protocol" icon="arrows-rotate" href="/features/session-protocol">
...
The outbound streaming events relayed after a client negotiates `streaming`.
</Card>

<Card title="Gateway Attachments" icon="file-arrow-up" href="/features/gateway-attachments">
The first-class `attachments` a `message` frame carries — inline or by reference.
</Card>
