...
<Note>
**Works inside `praisonai code`.** The coding session reuses this same collector, so servers declared under `mcp.servers` appear as an `mcp` tool group. Opt out with `PRAISON_TOOLS_DISABLE=mcp`; a server can never shadow a built-in tool name. See [MCP tools in coding sessions](/cli/code#mcp-tools-in-coding-sessions).
</Note>
