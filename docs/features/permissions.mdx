...
<Info>
For the CLI, see [`praisonai permissions`](/cli/permissions) and [Interactive Tool Approval](/features/interactive-approval).
</Info>

<Note>
To also block reads, writes, and shell commands **outside** a project root, see [Workspace Boundary](/features/workspace-boundary).
</Note>
...
<Note>
Shell commands containing `$IFS`, `${VAR}`, or bare `$VAR` are escalated to `ASK` rather than silently allowed — see [Command-Aware Permissions → Shell expansion escalates to ASK](/docs/features/command-aware-permissions#shell-expansion-escalates-to-ask).
</Note>
...
For YAML and CLI surfaces, see [Declarative Permissions](/features/declarative-permissions).

</Step>
</Steps>

<Note>
Permission rules resolve identically whether you run a direct prompt or a YAML workflow — `praisonai run workflow.yaml --deny "bash:rm *"` uses the same CLI-flag + project-config merge as `praisonai run "<prompt>"`. A permission rule with no `--approval` implicitly selects the `console` backend so the rule is enforced. See [Approval › YAML workflow approval gating](/features/approval#yaml-workflow-approval-gating).
</Note>
...
Global modes for subagent delegation — see [Permission Modes](/features/permission-modes) for full details.
...
| `workspace_root` | `str` | `None` | Optional project/workspace root. When set, shell and file targets that resolve outside this root emit a distinct `external_dir:<parent>/*` sub-target defaulting to `ask`. When `None`, no boundary check runs (backward compatible). See [Workspace Boundary](/features/workspace-boundary). |
...
Every `Agent` gets a globally-unique approval scope id at construction. Skill `allowed-tools` pre-approvals and "this session" human decisions are keyed by that id in a process-global registry, so PraisonAI knows exactly which grants belong to which agent.

Closing an agent reclaims those grants:

- Calling `agent.close()` / `await agent.aclose()` evicts **only that agent's** entries — its skill auto-approvals, its "this session" decisions, and its per-context approval-cache keys. Every other agent's grants stay intact.
- If you never call `close()`, `__del__` releases the same scope when the agent is garbage-collected, so a server that creates one short-lived `Agent` per request/session no longer leaks approval entries. `close()` is still recommended for deterministic teardown, since finalizer timing is not guaranteed.

```python
from praisonaiagents import Agent

# One short-lived agent per request in a long-running server
def handle_request(prompt: str) -> str:
    agent = Agent(name="Worker", instructions="Handle one request.")
    try:
        return agent.start(prompt)
    finally:
        agent.close()  # drops only this agent's approval grants
```

<Note>
Advanced: `close()` delegates to the registry's `ApprovalRegistry.release_scope(scope_id)`, which mutates the shared grant collections in place under an internal lock so a concurrent writer's grant for a different agent can never be lost to a race. The user-facing surface is `close()` — prefer it over calling `release_scope` directly. See [Agent Lifecycle Cleanup](/features/agent-lifecycle-cleanup).
</Note>
...
Compound shell commands (`&&`, `||`, `|`, subshells) are decomposed and evaluated independently — deny beats ask beats allow. See [Command-Aware Permissions](/features/command-aware-permissions).

When `workspace_root` is set, any path that escapes the root emits an `external_dir:<parent>/*` sub-target — a first-class permission target alongside `bash:`, `write:`, and `read:`. Set it once to stop broad approvals from granting whole-machine access. See [Workspace Boundary](/features/workspace-boundary).
...
<CardGroup cols={2}>
<Card title="Declarative Permissions" icon="file-code" href="/features/declarative-permissions">
  YAML, CLI, and Python permission policies
</Card>
<Card title="Command-Aware Permissions" icon="shield-halved" href="/features/command-aware-permissions">
...
<Card title="Reusable Approval Scopes" icon="list-check" href="/features/reusable-approval-scopes">
...
<Card title="Agent Lifecycle Cleanup" icon="broom" href="/features/agent-lifecycle-cleanup">
  How close() reclaims approval grants and other resources
</Card>
</CardGroup>
