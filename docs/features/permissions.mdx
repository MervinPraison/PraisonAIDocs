...
description: "Control what tools agents can use with pattern-based allow/deny rules and doom-loop detection"
icon: "shield-halved"
---

Set rules once — agents stay safe automatically, even across sessions.

```mermaid
graph LR
    subgraph "Permission Check"
        A[🤖 Agent calls tool] --> B[🔍 PermissionManager]
        B --> |pattern match| C{Rule?}
        C -->|ALLOW| D[✅ Execute]
        C -->|DENY| E[❌ Block]
        C -->|ASK| F[💬 User approves]
        F --> D
    end
...
<Step title="Create a permission manager">
```python
...
<AccordionGroup>
  <Accordion title="Start with deny-all, then allow specifics">
    Add a low-priority `DENY` rule for `*` and then `ALLOW` rules for exactly what your agent needs. This prevents any unexpected tool from slipping through.
  </Accordion>
  <Accordion title="Use session scope for interactive approvals">
    When a user approves a tool call interactively, record it with `scope="session"` so they aren't asked again in the same run — but the approval expires when the process exits.
  </Accordion>
  <Accordion title="Enable doom-loop detection for autonomous agents">
    Wrap long-running agents with `DoomLoopDetector`. Set `loop_threshold=3` to catch repeated identical tool calls before they burn tokens or damage state.
  </Accordion>
  <Accordion title="Use agent_name for multi-agent isolation">
    Give each agent only the permissions it needs. A reviewer agent should never have `write:*` even if the coder agent does.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Interactive Approval" icon="circle-check" href="/docs/features/interactive-approval">
    Runtime user-approval flow for tool calls
  </Card>
  <Card title="Declarative Permissions" icon="file-code" href="/docs/features/declarative-permissions">
    Define permission rules in YAML or config files
  </Card>
</CardGroup>
