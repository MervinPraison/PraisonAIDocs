...
## Built-in secret file gate (new default)

Reading `.env`, private keys, and keystores defaults to **ask** — even under a broad `read:*` allow — so credentials never leak silently to the model provider.

The gate lives in the core `PermissionManager`, so it protects every frontend by default. A broad `read:*` allow does **not** opt in; use a secret-specific rule to skip the prompt for a trusted workflow:

```bash
praisonai run "Compare prod.env and staging.env" --allow 'read:*.env'
```

See [Secret File Read Gate](/docs/features/secret-read-gate) for the full pattern list and override precedence.
