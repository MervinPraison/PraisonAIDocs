...
<Accordion title="Start with explicit DENY rules for dangerous operations">
Add DENY rules for `bash:rm *`, `bash:sudo *`, and similar destructive commands before deploying agents in sensitive environments.
</Accordion>

<Accordion title="Use regex for complex patterns">
Set `is_regex=True` for patterns that need character classes or groups — e.g. `bash:(sudo|su)\s+.*` to block privilege escalation.
</Accordion>

<Accordion title="Set agent_name to isolate permissions">
Scope rules to specific agents so a reviewer agent cannot write files even if a coder agent is allowed to.
</Accordion>

<Accordion title="Do not commit approvals.json">
`approvals.json` holds per-session decisions and is developer-local. Only commit `rules.json` to share team-wide policies.
</Accordion>
