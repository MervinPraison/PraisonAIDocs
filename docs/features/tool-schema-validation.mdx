...
`*args` and `**kwargs` are excluded from the generated tool schema — they never appear as properties and are never marked required.
...
The schema for `search` requires only `query`. Previously a decorated `def f(query: str, **kwargs)` mis-emitted `kwargs` as a required property; the builder now skips `VAR_POSITIONAL` / `VAR_KEYWORD` params, matching the tool-definition builder.
...
<CardGroup cols={2}>
<Card title="Tool Parameter Types" icon="shapes" href="/tools/parameter-types">
...
<Card title="Tool Reliability" icon="shield" href="/tools/reliability">
  Runtime error handling and tool safety patterns
</Card>
<Card title="Dynamic Tool Schemas" icon="refresh" href="/features/dynamic-tool-schemas">
