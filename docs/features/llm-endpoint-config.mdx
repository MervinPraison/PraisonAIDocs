...
graph LR
    subgraph "Which env var should I set?"
        ...
        B -->|Ollama| D[MODEL_NAME=ollama/llama3.2]
...
| `PRAISONAI_AUXILIARY_MODEL` | Model used for internal helper calls (session titles, context compaction, memory quality scoring, workflow routing). Overrides `OPENAI_MODEL_NAME` for these calls only; the agent's own primary model is unaffected. | — |
...
Name the model with the `ollama/` prefix. It resolves the base URL automatically and routes through `OllamaAdapter` (tool-call repair) instead of the plain `OpenAIClient`.

<Tabs>
<Tab title="bash">
```bash
export MODEL_NAME="ollama/llama3.2"
# OPENAI_BASE_URL is not needed — ollama/ resolves it automatically.
...
<Warning>
Setting only `OPENAI_BASE_URL="http://localhost:11434/v1"` with a bare model name (`llama3`) still works, but routes via `OpenAIClient` — no provider adapter, no tool-call repair. Worse, with **no** model named, the OpenAI default `gpt-4o-mini` is sent and Ollama returns `404 model 'gpt-4o-mini' not found`. Always use the `ollama/` prefix. See [Ollama → Common pitfall](/models/ollama#common-pitfall-naming-the-model-matters).
</Warning>
...
# OPENAI_BASE_URL not needed — ollama/ resolves it automatically.
...
<Card title="LLM Configuration" icon="cog" href="/configuration/llm-config">
...
<Card title="Groq" icon="zap" href="/models/groq">
  High-speed inference setup
</Card>
<Card title="Google" icon="brain" href="/models/google">
  Gemini models and API access
</Card>
<Card title="Cohere" icon="text" href="/models/cohere">
  Language model configuration
</Card>
<Card title="OpenRouter" icon="route" href="/models/openrouter">
...
*C7 note: As of PR #2550, the tool-discovery pipeline (`tool_resolver`), safe loader (`_safe_loader`), framework probes (`_framework_availability`), and plugin registry (`tool_registry`) also moved to `praisonai_code` — completing the same C7 arc as LLM config. See [Tool Discovery Order](/features/tool-discovery-order) and [Local Tools Loading](/features/local-tools-loading).*