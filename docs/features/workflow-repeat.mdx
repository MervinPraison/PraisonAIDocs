...
Prevent infinite loops by setting `max_iterations`. The workflow stops automatically when the limit is reached even if the condition is never met.
</Accordion>
<Accordion title="Clear stopping conditions">
Make `until` conditions unambiguous. Test both positive and negative cases before deploying.
</Accordion>
<Accordion title="Track progress with variables">
Use `StepResult(variables={"count": n})` to persist state across iterations so the condition function can inspect it via `ctx.variables`.
</Accordion>
<Accordion title="Handle the max-iteration edge case">
Decide upfront what should happen when `max_iterations` is reached without the condition being satisfied — log, escalate, or return partial results.
</Accordion>
</AccordionGroup>
