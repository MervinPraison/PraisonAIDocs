...
```
The model learns which arguments to send next.
</Step>
</Steps>
...
When the callable itself raises (e.g. a missing argument), the runtime introspects the signature and folds the parameters into the message:
...
No new public API, no new `Agent` parameters, and no new environment variables. The prior behavior on unknown tools was a bare `Tool 'X' is not callable` — not a stable contract, so no valid callers depended on it.
</Accordion>
</AccordionGroup>
