...
Small local models sometimes emit a tool call as JSON text in the `content` field instead of using the `tool_calls[]` field. The adapters for Ollama and local OpenAI-compatible servers (LM Studio, vLLM, llama.cpp) parse that JSON and dispatch the call anyway.

<Note>
**Async parity fixed in PraisonAI PR [#4940](https://github.com/MervinPraison/PraisonAI/pull/4940)** (merged 2026-09-08). Before #4940, text-based recovery fired only on the sync path (`agent.start()` / `agent.chat()`); the async path (`agent.astart()` / `agent.achat()`) silently ignored a tool call the model emitted as JSON text, so the same agent, same model, same prompt stopped calling tools the moment the caller awaited instead of called. Both paths now behave identically. Streaming does not yet have this recovery — a small local model that emits a tool call as JSON text inside a streamed response still surfaces as prose.
</Note>
...
# The model may reply with the tool call as JSON text — it dispatches anyway.
```

The recovered shape is a JSON object (or a list of them) with `name` and `arguments`:

```json
{"name": "get_weather", "arguments": {"city": "Paris"}}
```

Both `{...}` and `[{...}, {...}]` shapes work. Hosted OpenAI-compatible endpoints, Anthropic, and Gemini deliberately do **not** get this behaviour — a hosted model returning JSON prose must never have it silently interpreted as a tool call.

| Adapter | Text-based recovery | Applies on |
|---|---|---|
| `OllamaAdapter` (`ollama/…`, `ollama_chat/…`) | ✅ recovers | sync, async |
| `LocalOpenAIAdapter` (LM Studio, vLLM, `hosted_vllm/`, llama.cpp) | ✅ recovers | sync, async |
| `DefaultAdapter` (hosted OpenAI-compatible via `base_url`) | ❌ | — |
| `AnthropicAdapter` | ❌ | — |
| `GeminiAdapter` | ❌ | — |

Streaming is deliberately omitted from the "Applies on" column because no adapter recovers on the streaming path today.

If the response also carries a native `tool_calls[]` entry, that is dispatched — text-based recovery only runs when `tool_calls[]` is empty. So a well-formed native call is never overridden by JSON prose the model happened to also include.
...
```mermaid
graph TB
    Text[🤖 Tool call as JSON text] --> Which{🔌 Which adapter?}
    Which -->|Ollama · LM Studio · vLLM · llama.cpp| Parse[🧩 Parse + dispatch]
    Which -->|Hosted · Anthropic · Gemini| Skip[🚫 Left as prose]
