...
| `/whoami`, `/learn` | Hidden from a user's menu when the policy denies it |
| `/automations`, `/blueprint` (Telegram) | See [Automation Suggestions](/features/automation-suggestions) |

Admins see everything; regular users only see the commands their `user_allowed_commands` allows.

<Note>
File-based (`.praisonai/commands/*.md`) and entry-point (`praisonai.bot_commands`) commands are merged into this native menu too, through the same shared `ChatCommandMixin` — no per-adapter wiring. Enable them per channel with a `commands` block. See [File-based & Entry-point Commands in Bot Chats](/features/bot-commands#file-based-and-entry-point-commands-in-bot-chats).
</Note>
...
<Note>
**Additive — and never a dead end.** The native slash menu never runs the handler directly on Discord; the text-command path is the execution route. With PraisonAI PR [#4320](https://github.com/MervinPraison/PraisonAI/pull/4320), that text-command path no longer silently drops a command it doesn't recognise — unknown commands fall through to the agent on both Telegram and Discord, matching Slack. See [Unknown or Misspelled Commands](/features/bot-commands#unknown-or-misspelled-commands).
</Note>
...
Filtered menus keep the UX safe — a user never sees a command they cannot run. See [Command Access Control](/features/bot-command-access-control).
</Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Learn a Skill" icon="graduation-cap" href="/features/learn-skill">
