...
One `.praisonai/environment.yaml` file declares your project's execution environment; every `tools_run_on=` backend reads it automatically.
...
graph LR
    subgraph "Environment File"
        ...
        B --> C[⚙️ ComputeConfig]
        C --> D[✅ Docker / E2B / Modal / Daytona / Novita]
    end
...
Add `tools_run_on="docker"`. No `image` or `packages` kwargs; the file supplies them.
...
| **Consumer** | Every `tools_run_on=` backend (Docker, E2B, Modal, Daytona, Novita, Flyio, local) |
...
backend: docker               # docker | e2b | modal | daytona | novita | flyio | local → stored as ComputeConfig.metadata["backend"]
