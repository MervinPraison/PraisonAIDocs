...
<Note>
**See also:** to skip the `ollama/<model>` prefix and let PraisonAI find and configure a running local server for you, set `llm="local"` — the explicit form. Keyless auto-detection remains the implicit form. See the [Local Model Resolver](/features/local-model-resolver).
</Note>
