...
agent = Agent(name="researcher", instructions="Answer research questions via A2A.")
agent.start("Summarise the latest on agent protocols.")
```
...
<Note>
**Import cost:** importing just the server side of A2A does not pull in the HTTP client stack.

```python
from praisonaiagents.ui.a2a import A2A         # no aiohttp loaded
from praisonaiagents.ui.a2a import A2AClient   # aiohttp loaded on demand
```

`A2AClient` is lazy-loaded via the package's `__getattr__` — the first time you reference it, `aiohttp` is imported. Server-only deployments that never touch `A2AClient` never pay that cost. `aiohttp` remains a core dependency; this is an import-time optimisation, not a dependency change.
</Note>

```mermaid
graph LR
    subgraph "Import paths"
        Import[📦 Import A2A package] --> Choice{Which symbol?}
        Choice -->|A2A · server| Server[🖥️ Server ready<br/>no aiohttp]
        Choice -->|A2AClient · client| Client[🔌 aiohttp loaded<br/>on demand]
    end
...
<Note>
A2A `message/stream` currently emits text-only artifacts. For live A2UI during streaming, prefer [AG-UI](/deploy/servers/agui) or [PraisonAIUI](/features/integrate-a2ui-frontend).
</Note>

See [A2UI Protocol](/features/a2ui) for the tool contract and renderer options.
...
<CardGroup cols={2}>
<Card title="A2UI Protocol" icon="window" href="/features/a2ui">
  Declarative UI via send_a2ui_messages and DataPart artifacts
</Card>
<Card title="A2A Client" icon="plug" href="/features/a2a-client">
  A2A client for connecting to other agents
</Card>
<Card title="MCP Lifecycle" icon="puzzle-piece" href="/features/mcp-lifecycle">
