...
flowchart LR
    subgraph Run
        R[recipe run] --> H[History auto]
        R -->|--save| T[Replay Trace]
    end
...
<Note>
Run **history** (Run IDs, statuses, sessions, timings) is captured automatically for every run. The `--save` flag additionally captures a full **replay trace** for deep debugging.
</Note>
...
| Option | Description |
|--------|-------------|
| `--save`, `-s` | Save replay trace for debugging |
| `--name`, `-n NAME` | Custom trace name |
...
<Steps>
  ...
  <Step title="Judge the Execution">
    Find the run ID with `praisonai recipe runs`, then pass it to `recipe judge`.
    ```bash
...
<CardGroup cols={2}>
  <Card title="History is Automatic" icon="clock-rotate-left">
    Every run is recorded to history; add `--save` only for full replay traces
  </Card>
...
- [Run History](/cli/recipe-runs) - Auto-recorded runs and the `recipe runs` command
