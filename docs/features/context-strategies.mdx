...
sidebarTitle: "Context Strategies"
...
Context Strategies control how PraisonAI agents compress conversation history when approaching the model's token limit.
...
graph LR
    subgraph "Context Strategies"
        A[🤖 Agent] --> B{80% Full?}
        B -->|smart| C[🧠 Smart]
        B -->|summarize| D[📝 Summarize]
        B -->|sliding_window| E[🪟 Sliding Window]
        B -->|truncate| F[✂️ Truncate]
        C --> G[✅ Optimized]
        D --> G
        E --> G
        F --> G
...
<Step title="Fine-tune with ManagerConfig">
```python
...
<AccordionGroup>
<Accordion title="Use smart strategy as your default">
The `smart` strategy intelligently combines multiple techniques and is the safest choice for most production workloads.

```python
agent = Agent(instructions="...", context=ManagerConfig(strategy="smart"))
```
</Accordion>

<Accordion title="Lower threshold for safety-critical agents">
Set `compact_threshold=0.7` to trigger optimization earlier, leaving more headroom for unexpected context bursts.
</Accordion>

<Accordion title="Use conversation strategy for long sessions">
The `conversation` strategy preserves narrative continuity across multi-hour sessions better than simple truncation.
</Accordion>

<Accordion title="Enable monitoring during development">
Set `monitor_enabled=True` to write snapshots after each turn, so you can see exactly how context grows.

```python
agent = Agent(instructions="...", context=ManagerConfig(monitor_enabled=True, monitor_path="./ctx.txt"))
```
</Accordion>
</AccordionGroup>
...
<Card title="Context Budgeter" icon="coins" href="/features/context-budgeter">
  Token budget allocation per segment
</Card>
