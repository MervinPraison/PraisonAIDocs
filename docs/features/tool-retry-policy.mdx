...
| 2 | On error — a returned error string **or a raised exception** — `_classify_error_type` tags it: `timeout`, `rate_limit`, `connection_error`, or `unknown`. On the async path the flattened error dict also carries an explicit `retryable` verdict |
...
On the async path a raised exception is flattened into an error dict carrying a `retryable` verdict, then the retry loop honours it:

```mermaid
graph LR
    Raise[Tool raises] --> Flatten[Flatten to error dict + retryable verdict]
