...
<Note>
`AgentTeam(caching=…)` is not wired yet (PraisonAI [#4004](https://github.com/MervinPraison/PraisonAI/pull/4004)) — set `caching=…` on each `Agent(...)` in the team instead.
</Note>
...
<Step title="Level 2 — String (preset)">
...
<CardGroup cols={2}>
  <Card icon="database" href="/features/prompt-caching">
    Prompt Caching — provider-level caching for stable context
  </Card>
  <Card icon="database" href="/features/prompt-cache-optimization">
