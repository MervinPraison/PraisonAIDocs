...
<Note>
See also: [Env Config Injection](/features/env-config-injection) — supply the whole config layer from `PRAISONAI_CONFIG_CONTENT` / `PRAISONAI_CONFIG` with nothing on disk.
</Note>
...
| `<home>/skills/` | User-scope skills (see [Skill Manage](/features/skill-manage)) |
...
## Project data directory (`.praisonai/`)

Project-local CLI state lives in `./.praisonai/` — the same directory the runtime (`praisonaiagents.paths`) reads from. Everything below goes through a single pair of helpers in `cli/configuration/paths.py`, derived from the one constant `PROJECT_DATA_DIRNAME` (itself `praisonaiagents.paths.DEFAULT_DIR_NAME`), so a new site cannot drift:

- `get_project_data_path(...)` — the canonical **write** path (`<cwd>/.praisonai/<file>`).
- `resolve_project_data_path(...)` — the **read** path: returns the canonical file if it exists, else an existing legacy `<cwd>/.praison/<file>`, else the canonical path.

<Warning>
**Why this matters (PraisonAI PR #4946).** `.praison` is the *first* project marker the config loader checks. Before this fix, any feature that created `./.praison/` in a project whose config lived in `./.praisonai/` made the loader silently flip and stop reading the project's own `.praisonai/config.toml` — the same silent-drift class of bug as the hooks path in [#4927](https://github.com/MervinPraison/PraisonAI/pull/4927). The migration is **one-way**: writes always land on the canonical `.praisonai/`, and the very first write (or a `praisonai doctor --deep`) moves you off the legacy path.
</Warning>

### Files written under `./.praisonai/`

These nine (plus one) project-data sites migrated from `./.praison/` (or `~/.praison/`) to the canonical `./.praisonai/` in PR #4946. A read-only fallback to `.praison/` is preserved so nothing saved earlier is orphaned:

| File / directory | Feature |
|------------------|---------|
| `thinking.json` | Thinking level (`praisonai thinking`) |
| `compaction.json` | Conversation compaction state |
| `output.json` | Output-style (`praisonai output-style`) |
| file-history | Interactive input history |
| `queue.db` | Background queue database |
| `knowledge/<collection>` | Knowledge vector-store path (config loader, unified schema, and three `retrieval.py` sites) |
| skills check | `praisonai doctor` skills check |
| memory check | `praisonai doctor` memory check |

<Note>
**Left on `.praison` deliberately** (documented read-only fallbacks or home-level sites, **not** changed by PR #4946): the `config.toml` helpers in `cli/configuration/paths.py` and `credentials.py`; the `~/.praison` todo workspace (`features/todo.py`); `features/logs.py`; `features/package.py`; `llm/catalogue.py`; `resolver.py`; interactive history files; `doctor`'s `mcp_checks` (already canonical-first), `permissions_checks`, and `config_checks`; and `job_workflow.py` (a read-only path that never creates the directory). Where docs mention these, only the project-data sites above have migrated — home-level sites and read-only reads still use `.praison`.
</Note>
...
<Note>
`api_key` is never serialised to YAML — use environment variables or [`praisonai auth`](/cli/auth).
</Note>
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `instructions` | `list[str]` | `[]` | File paths, globs (`docs/standards/*.md`), `~`/env-var paths, or `http(s)://` URLs. See [Instruction Sources](/features/instruction-sources) for source-type behaviour, the SSRF guard, and merge semantics. |
...
See [Single-Source Config](/features/single-source-config) for a full guide to using all three sections together. Other top-level sections (`output`, `traces`, `session`) are also valid — see [Config CLI reference](/cli/config).
...
| `PRAISONAI_BACKGROUND_JOB_STORE` | background job durability | Default `1` (on). Set to `0` / `false` to force `get_job_manager()` to pure in-memory. See [Durable Background Jobs](/features/durable-background-jobs) |
...
<CardGroup cols={2}>
<Card title="Config CLI Reference" icon="terminal" href="/cli/config">
...
<Card title="LLM Endpoint Config" icon="link" href="/features/llm-endpoint-config">
  Custom base URLs and provider routing
</Card>
<Card title="Single-Source Config" icon="gear" href="/features/single-source-config">
  Model + MCP + permissions in one file
</Card>
<Card title="Editor Support" icon="file-code" href="/features/editor-support">
