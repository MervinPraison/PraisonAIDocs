...
```python
from praisonaiagents.scheduler import ScheduleLoop, get_default_store

engine = ScheduleLoop(
    store=get_default_store(),         # share the canonical default
    on_trigger=lambda job: handle(job),
)
```
...
```python
import os
from praisonaiagents.scheduler import set_default_store
from myproject.stores import PostgresScheduleStore

set_default_store(PostgresScheduleStore(dsn=os.environ["DB_URL"]))
```
...
```mermaid
graph LR
    T[🛠️ Agent Tools] --> D[🗄️ get_default_store]
    G[⏰ Gateway Tick] --> D
    H[🔗 Host Bridge] --> D
    D --> J[🔔 Fired Job]
...
<CardGroup cols={2}>
  <Card title="Scheduler Protocols" icon="code" href="/sdk/praisonai/scheduler">
...
### Long-running poller under any supervisor

`praisonai schedule run` is a zero-dependency way to host the default in-process poller under any process supervisor (systemd, `pm2`, tmux, foreman) — no need to run the whole BotOS gateway. It calls [`ScheduleLoop.run_forever(poll_seconds=…)`](/cli/schedule#run-the-store-poller-standalone-no-gateway) in the foreground.

```ini
# /etc/systemd/system/praisonai-schedules.service
[Unit]
Description=PraisonAI standalone schedule poller
After=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/praisonai schedule run --poll 15
Restart=on-failure
User=praisonai
...
<AccordionGroup>
  ...
  <Accordion title="Share get_default_store() across every runtime">
...
<CardGroup cols={2}>
  <Card title="Background Tasks — ScheduleLoop" icon="clock" href="/features/background-tasks#scheduleloop">
  ...
  <Card title="Scheduled Run Policy" icon="shield" href="/features/scheduled-run-policy">
