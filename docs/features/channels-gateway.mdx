...
<Note>
`ChannelDirectory` covers **outbound** delivery (which channel an agent sends to). For **inbound** routing — dispatching an incoming WhatsApp/SMS number to a specific agent — use the [Agent Registry](/docs/features/agent-registry).
</Note>
