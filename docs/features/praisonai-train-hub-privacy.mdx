...
<Note>
Prefer config-level `hf_token` inside CI configs where you don't want the token to leak into environment inspection. `hf_token` also reaches Unsloth as the download credential for **gated base models** (Llama, Gemma) — see [Unsloth Surface](/features/praisonai-train-unsloth-surface#model-access).
</Note>
