...
Five commands work with recipes over MCP — one to discover, four to serve. Each works as `praisonai mcp <command>` inside the full stack, or `praisonai-mcp <command>` standalone.
...
praisonai-mcp list-recipes
praisonai mcp list-recipes --source package
praisonai mcp list-recipes --tags video,social
praisonai mcp list-recipes --json
```

| Flag | Default | Description |
|------|---------|-------------|
| `--source` | `None` (all sources) | Filter by source. Only `custom`, `project`, `package` are accepted — the exact labels `TemplateDiscovery` assigns. |
| `--tags` | `None` | Comma-separated tag filter (`--tags video,social`). |
| `--json` | off | JSON output — safe for pipes and CI. |

<Note>
`--source` used to advertise `local` and `all` — neither ever worked, and the command itself raised `TypeError` on every invocation before PraisonAI PR [#4962](https://github.com/MervinPraison/PraisonAI/pull/4962). `custom`, `project`, and `package` are the only accepted values.
</Note>

**Source labels — what each one lists:**

| Source | Where discovery reads from | Typical use |
|--------|----------------------------|-------------|
| `custom` | `~/.praison`, `~/.config/praison` | User-installed recipes on this machine |
| `project` | `./.praison` (current working directory) | Recipes checked into a repo |
| `package` | Bundled `agent_recipes` inside the wheel | The recipes that ship with PraisonAI |
...
Available Recipes (2):
  ...
  • support-reply (v1.0.0)
    Draft a friendly reply to a customer support ticket
```
  ...
  "recipes": [
    {
      "name": "data-analyst",
      "version": "1.0.0",
      "description": "Data analysis template using pandas tools",
      "source": "package",
      "tags": ["data", "pandas"]
    }
  ]
...
Pair `list-recipes` with `serve-recipe` — discover first, then serve.

```mermaid
graph LR
    Q{Know the recipe name?} -->|No| L[praisonai mcp list-recipes]
    L --> Pick[Pick a recipe]
    Q -->|Yes| Pick
    Pick --> S[praisonai mcp serve-recipe]
    S --> Client[✅ Claude Desktop / Cursor]

    classDef q fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef list fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef pick fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef serve fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef ok fill:#10B981,stroke:#7C90A0,color:#fff
...
<Note>
Since **v4.6.154**, recipe configs auto-pick the right entry point — `praisonai-mcp serve-recipe` when the standalone binary is on your PATH, and `praisonai mcp serve-recipe` otherwise.
</Note>
...
```json
{
  "mcpServers": {
    "support-reply": {
      ...
      "args": ["serve-recipe", "support-reply", "--transport", "stdio"]
    }
  }
}
```
...
<CardGroup cols={2}>
  <Card title="PraisonAI MCP Server" icon="server" href="/mcp/praisonai-mcp-server">
  ...
  <Card title="The Three MCP Layers" icon="layer-group" href="/features/mcp-three-layers">
