...
`AGUI` wraps any agent and exposes a `POST /agui` endpoint that emits text delta and tool call events per the AG-UI protocol. Recoverable failures and progress (retry, model fallback, tool progress, todo updates) arrive as `custom` events named `praisonai.*`, and mid-stream provider errors surface as `run_error`. See the [AGUI API reference](/deploy/api/agui-api#praisonai-custom-events).
...
<Card title="AGUI TypeScript Reference" icon="code" href="/sdk/reference/typescript/classes/AGUI">
...
<CardGroup cols={2}>
<Card title="A2UI Protocol" icon="sparkles" href="/features/a2ui">
