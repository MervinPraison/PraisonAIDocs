...
agent = Agent(
    name="data-assistant",
    instructions="Query and store records with Xata.",
    memory=MemoryConfig(
        db={"database_url": os.environ["XATA_DATABASE_URL"], "api_key": os.environ["XATA_API_KEY"]},
    ))
agent.start("Save this note about our launch plan")
```
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Xata Serverless Database
...
dev_agent = Agent(name="Dev Agent", memory={"db": dev_db})
prod_agent = Agent(name="Prod Agent", memory={"db": prod_db})
```
...
<Card title="Search & Analytics" icon="chart-line" href="/features/retrieval">
