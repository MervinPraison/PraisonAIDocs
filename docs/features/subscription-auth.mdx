...
<AccordionGroup>
  ...
  <Accordion title="Read-Only Claude Code Auth">
    With `auth="claude-code"`, PraisonAI **reads** the Keychain / `~/.claude/.credentials.json` OAuth token but never calls Anthropic's OAuth refresh endpoint. Refresh is the Claude CLI's job, so PraisonAI never rotates the shared refresh token that `claude` and other subscription tools depend on. When the token expires, run `claude /login`.
  </Accordion>
...
<Note>
The `context-1m-2025-08-07` beta header is no longer negotiated for `claude-code` — it is unsupported on many subscription plans and caused API failures. If you previously relied on it, use a standard API key instead.
</Note>
