...
agent = Agent(
    name="assistant",
    instructions="You are helpful with persistent memory.",
    db={
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Supabase Database
...
agent = Agent(name="Manual Agent", memory={"db": store})
```
...
rest_agent = Agent(memory={"db": {"database_url": "https://xxx.supabase.co", "supabase_key": "..."}})

# Direct: Better for production, lower latency  
direct_agent = Agent(memory={"db": {"database_url": "postgresql://postgres:pass@db.xxx.supabase.com:6543/postgres"}})
```
</Accordion>
...
public_agent = Agent(memory={"db": {"supabase_key": os.environ["SUPABASE_KEY"]}})

# Service role (full access) — use carefully
admin_agent = Agent(memory={"db": {"supabase_key": os.environ["SUPABASE_SERVICE_KEY"]}})
```
</Accordion>
...
| `SUPABASE_KEY` | REST | JWT token | From Supabase dashboard → Settings → API |
| `SUPABASE_DATABASE_URL` | Direct | `postgresql://...` | `postgresql://postgres:pass@db.xxx.supabase.com:5432/postgres` |
| `OPENAI_API_KEY` | Both | API key | Set in environment for LLM calls |
...
<Card title="Real-time Features" icon="bolt" href="/features/streaming">
