...
sequenceDiagram
    ...
    participant Coder as Coder Tool

    User->>Writer: "Write article on async Python"
    Writer->>Researcher: invoke_researcher("async Python patterns")
    Researcher-->>Writer: Research findings
    Writer->>Coder: invoke_coder("Show async example")
    Coder-->>Writer: Code snippet
    Writer-->>User: Complete article with research and code
```
