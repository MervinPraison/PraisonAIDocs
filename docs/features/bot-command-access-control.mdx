...
description: "Restrict who can run privileged bot commands on Telegram, Slack, and Discord with admin users and per-user allow-lists."
icon: "shield-keyhole"
---

Restrict who can run privileged bot commands on Telegram, Slack, and Discord — admins run anything, regular users only run commands you explicitly permit.
...
| `/learn` | Author a skill from sources you describe | No | **Yes** — admin-only by default once a policy is configured |

`PRIVILEGED_COMMANDS = {"learn"}` — these are admin-only whenever `admin_users` or `user_allowed_commands` is set, unless you explicitly include them in `user_allowed_commands`.
