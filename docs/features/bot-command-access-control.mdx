...
<Info>
**The native `/` menu is filtered per user.** When a `CommandAccessPolicy` is configured, Telegram and Discord only surface the commands a user may run in their typed-`/` autocomplete — restricted commands never appear for users who cannot run them. See [Native `/` Autocomplete](/docs/features/bot-commands).
</Info>
