...
<Note>
`CommandAccessPolicy` gates by **identity** (`admin_users`, `user_allowed_commands`). This is one of two gates — the other is **content-trust** on `BotMessage.allow_control`, which fires *before* identity is checked. See [Control-Trust Provenance](/features/bot-control-trust).
</Note>
...
<Info>
**The native `/` menu is filtered per user.** When a `CommandAccessPolicy` is configured, Telegram and Discord only surface the commands a user may run in their typed-`/` autocomplete — restricted commands never appear for users who cannot run them. See [Native `/` Autocomplete](/features/bot-commands).
</Info>
...
<Accordion title="File & entry-point commands are gated identically">
`CommandAccessPolicy` gates apply equally to file-based (`.praisonai/commands/*.md`) and entry-point (`praisonai.bot_commands`) commands — not just Python-registered ones. Add their names to `user_allowed_commands` for non-admins, and denied commands stay hidden from the `/` menu too. See [File-based & Entry-point Commands in Bot Chats](/features/bot-commands#file-based-and-entry-point-commands-in-bot-chats).
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Learn Skill" icon="graduation-cap" href="/features/learn-skill">
    The privileged /learn command — authors a grounded SKILL.md from sources
  </Card>
  <Card title="Control-Trust Provenance" icon="shield-keyhole" href="/features/bot-control-trust">
    Content-trust gate that fires before identity ACL
  </Card>
</CardGroup>
