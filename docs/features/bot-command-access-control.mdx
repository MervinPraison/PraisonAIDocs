...
The five new conversation-control commands — `/undo`, `/sessions`, `/resume`, `/retry`, and `/reasoning` — are **non-privileged**. They behave like `/status` and `/help` under the policy: open by default, and included in `user_allowed_commands` when an allow-list is configured. Only `/learn` remains in `PRIVILEGED_COMMANDS`. When a user is denied any command, every adapter replies:

```
⛔ You are not permitted to run /<cmd>
```
...
| `/undo` | Undo the last turn's file changes | No | No |
| `/sessions` | List the caller's saved sessions | No | No |
| `/resume` | Switch to a saved session | No | No |
| `/retry` | Re-run the last user message | No | No |
| `/reasoning` | Toggle extended-thinking visibility | No | No |
