...
<Step title="Concurrent handoffs to a shared target">
Overlapping handoffs to the **same** target agent — a natural pattern when `parallel_handoffs` fans several prompts to one specialist — run one at a time on that agent. Its seeded conversation context stays isolated per handoff and cannot interleave:
...
router = Agent(name="Router", instructions="Route work to specialists.")
...
Each chunk sees only its own seeded context — no leak from the other. See [Agent Handoffs](/features/handoffs#concurrent-handoffs-to-the-same-target) and [Thread Safety](/features/thread-safety).
</Step>
...
- **Handoff chain** (`_handoff_chain_var`) rebound per task, so `parallel_handoffs` siblings keep isolated cycle/depth tracking — see [Handoff chain isolation](/docs/features/thread-safety#handoff-chain-isolation-under-asyncio-gather)

<Note>
**Memory:** two agents in the same process no longer share one on-disk store by default — as of PraisonAI [#4203](https://github.com/MervinPraison/PraisonAI/pull/4203) each `Memory` backend (`sqlite`, `chroma`, `mongodb`, `mem0`, `learn`) inherits the same per-agent isolation as `FileMemory`. See [Per-agent isolation across backends](/features/multi-agent-memory#per-agent-isolation-across-backends).
</Note>
