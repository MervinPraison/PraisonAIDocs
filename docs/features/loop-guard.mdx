...
<Note>
Looking for **bot-to-bot** reply-loop protection? See [Bot-to-Bot Loop Protection](/features/bot-loop-protection) — a separate gateway-layer primitive that caps how many exchanges a pair of bots can trade.
</Note>

<Note>
**Sync and async parity (as of 2026-08-17).** Loop Guard runs on both the sync tool-execution path (`agent.chat()`, `agent.start()`) and the async one (`agent.achat()`, `agent.astart()`, async workflows). The async path gained the same pre/post `check()` and `record()` wiring in [PR #4005](https://github.com/MervinPraison/PraisonAI/pull/4005) — before that, async runs had no loop-guard protection. No config change is required to pick this up; every `Agent` already gets it.
</Note>
...
| `HALT` | `ToolExecutionError` raised, `is_retryable=False` | N/A | Exception propagates out of `agent.chat()`, `agent.start()`, `agent.achat()`, `agent.astart()`, or the async workflow that scheduled the tool |

---

## Sync and Async coverage

Loop Guard applies identically to both execution paths — sync (`agent.start()` / `agent.chat()`) and async (`agent.achat()` / async workflows).
...
sequenceDiagram
    ...
    User->>Agent: start / achat with a task
    Agent->>LoopGuard: check(tool, args, pre=True)
    LoopGuard-->>Agent: ALLOW / WARN / BLOCK / HALT
    Agent->>Tool: run (unless BLOCK/HALT)
    Tool-->>Agent: result or exception
    Agent->>LoopGuard: record(tool, args, success, result=…)
    Agent->>LoopGuard: check(tool, args, pre=False)
    LoopGuard-->>Agent: post-execution decision
    Agent-->>User: result (with [loop-guard] annotation on WARN)
```

**What triggers Loop Guard on the async path:**
...
agent = Agent(instructions="Poll a flaky endpoint.")

# Async path: identical failing calls escalate WARN → HALT and the run stops.
result = await agent.achat("Fetch /status until it returns COMPLETE.")
```
...
<Accordion title="Drop manual retry caps around async polling">
...
<CardGroup cols={2}>
<Card icon="triangle-exclamation" href="/features/doom-loop-detection">
  Loop Detection
</Card>
<Card icon="robot" href="/features/escalation-pipeline">
  Agent Autonomy
</Card>
</CardGroup>