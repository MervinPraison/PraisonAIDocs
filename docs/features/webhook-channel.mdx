...
<Note>
Webhook-produced messages should carry `allow_control=False` so a webhook body of `/stop` or `/new` is treated as plain text, never a privileged command. See [Control-Trust Provenance](/features/bot-control-trust).
</Note>
...
<Steps>
<Step title="One command (CLI)">

The quickest way to start a generic webhook receiver — no Python needed:

```bash
praisonai bot run --platform webhook --port 8080
```

`bot run` starts any registered platform by name. See [`praisonai bot run`](/features/messaging-bots#registered-but-verb-less-platforms-praisonai-bot-run) for the full flag list.
...
<Warning>
The `verify` extra declared in a `bot.yaml` is now forwarded correctly through `praisonai bot start` (and `praisonai bot run --platform webhook`). Previously the `start` path silently dropped it, replacing your configured verifier with the adapter's unverified default — accepting unsigned traffic. Re-check your exit code and logs after upgrading to confirm verification is active.
</Warning>
</Accordion>
