...
<Warning>
The top-level `Agent(max_budget=...)` shortcut was **removed** (PraisonAI PR #1642). Use `execution=ExecutionConfig(max_budget=...)` — see [CLI budget handling](/features/cli-budget-handling).
</Warning>
...
Schedulers use the same brake — see [Async Scheduler → Budget-aware Scheduling](/docs/features/async-agent-scheduler#budget-aware-scheduling). Setting `max_cost=0` on `AsyncAgentScheduler` is a real (zero) budget and short-circuits `start()` (returns `False`, no background task) when combined with `run_immediately=True`.
...
<CardGroup cols={2}>
  <Card title="ExecutionConfig" icon="code" href="/sdk/reference/praisonaiagents/classes/ExecutionConfig">
    Full execution configuration reference
  </Card>
  <Card title="CLI Budget Handling" icon="terminal" href="/features/cli-budget-handling">
...
For autonomous runs, prefer the `AutonomyConfig` cap — it pauses recoverably instead of raising. See [Autonomy Budget](/features/autonomy-budget).

## Related

<CardGroup cols={2}>
  <Card title="Autonomy Budget" icon="wallet" href="/features/autonomy-budget">
    Spend/token kill-switch for autonomous runs
  </Card>
  <Card title="CLI Budget Handling" icon="terminal" href="/features/cli-budget-handling">
    Set budgets from the command line
  </Card>
  <Card title="Execution Config" icon="settings" href="/features/agent-centric-api">
    All execution options in one place
  </Card>
  <Card title="Gateway Spend Budget" icon="dollar-sign" href="/features/gateway-spend-budget">
    Cap cumulative per-identity spend across runs on a shared-Agent gateway
  </Card>
</CardGroup>
