...
Embeddings are configured via the top-level `embedder` key. The adapter recombines it into a litellm string as `"<provider>/<model>"` — except `openai`, which stays bare. It falls back to `text-embedding-3-small` when nothing is set:
...
<Note>
Atlas vector search indexes are **built for a specific dimension**. Switching the embedder model — e.g. from `text-embedding-3-small` (1536) to `nomic-embed-text` (768) — means you must re-create the vector index on the collection at the new dimension, or searches will error or return nothing.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Local Memory & Knowledge" icon="server" href="/features/local-memory-and-knowledge">
