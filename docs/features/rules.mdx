...
The user drops an instruction file in their repo; the agent discovers it on the first run and injects the rules into the system prompt.

```mermaid
sequenceDiagram
    participant Dev as 👨‍💻 Developer
    participant Agent as 🤖 Agent.__init__
    participant Init as 🔍 _init_rules_manager
    ...
    Agent->>Init: lazy first-run discovery
    Init->>FS: Scan workspace (discovery gate)
    FS-->>Init: AGENTS.md, CLAUDE.md, ...
    Init-->>Prompt: rules context
    Prompt-->>Dev: def sort_list(items: list[int]) -> list[int]: ...
```
...
Project instruction files auto-inject into `praisonai run`, `chat`, and `code` — no Python required.
...
|----------|--------------|
| `rules=True` (or omit) | Auto-discover per-project instruction files and apply if any exist |
| `rules=RulesConfig(...)` | Custom control — character budget, extra `files`, or a different `workspace_path` |
...
agent.start("Review this pull request against our conventions.")
```
...
<Note>
**Discovery gate**: on the first run the workspace is scanned; if no instruction file or directory is found, the RulesManager is dropped and every later run pays zero cost. Extra `files` are registered **before** the gate, so a workspace whose only rules are explicit `files=[...]` is never dropped.
</Note>
