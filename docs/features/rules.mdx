...
PraisonAI discovers instruction files (`AGENTS.md`, `CLAUDE.md`, `.praison/rules/*.md`) and auto-injects them into every `praisonai run`, `chat`, and `code` invocation. From Python, opt in explicitly with `RulesManager` — a plain `Agent(...)` constructor does **not** apply project rules on its own today.

```bash
# Create AGENTS.md or CLAUDE.md in your project root, then:
praisonai run "Help me with Python"
# → discovered files are injected into the system prompt automatically
```

The user works in a repo with instruction files; the CLI discovers them and injects the rules into the agent system prompt.
...
sequenceDiagram
    ...
    Dev->>Agent: Agent(instructions="...")
    Agent->>FS: Auto-discover instruction files
    FS-->>Agent: AGENTS.md, CLAUDE.md, ...
...
Create an instruction file in your project root — the CLI loads it automatically:
...
<Step title="Python (explicit)">

From Python, wire `RulesManager` yourself and thread the context into `instructions`:

```python
from praisonaiagents import Agent
from praisonaiagents.memory import RulesManager

rules = RulesManager(workspace_path=".")
extra = rules.build_rules_context()  # AGENTS.md / CLAUDE.md / .praison/rules/*.md

agent = Agent(
    name="Assistant",
    instructions=f"You are helpful.\n\n## Project rules\n{extra}" if extra else "You are helpful.",
)
...
`rules=True` is the same as the default — auto-discover and apply:
...
Turn rules off for sandboxed or reproducible runs where project files should not leak in:
...
Pass a `RulesConfig` for a custom budget, extra files, or a different workspace:
...
agent = Agent(
    ...
    instructions="You are helpful.",
    rules=RulesConfig(
        char_budget=6000,                        # cap injected chars
        files=["docs/CONVENTIONS.md"],           # extra files beyond auto-discovery
        workspace_path="/path/to/other/project", # scan a different folder
    ),
)
```
...
|--------|------|---------|-------------|
| `char_budget` | `int` | `None` | Maximum total characters injected into the system prompt. `None` uses the RulesManager default. |
| `files` | `List[str]` | `None` | Extra instruction file paths or globs to include alongside auto-discovered files. |
| `workspace_path` | `str` | `None` | Directory to scan for instruction files. Defaults to `os.getcwd()`. Set this when your agent runs from a different folder than the project you want to load rules from. |

<Note>
**Discovery gate**: if no instruction file exists in the workspace, the RulesManager is dropped after the first scan — zero-config runs incur no ongoing cost.
</Note>
