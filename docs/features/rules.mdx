...
<Note>
The [Model Harness](/features/model-harness) fragment sits *before* the rules block in the assembled system prompt, so family-tuned guidance comes first, then your project rules.
</Note>
...
<Note>
As of PraisonAI PR #3552, `--no-rules` also gates the on-demand `build_subtree_context_hook` under `praisonai run` (previously it only affected the up-front load under `chat`). The env-var equivalent `PRAISON_NO_RULES=true` (`1` / `true` / `yes`, case-insensitive) skips both. See [Context Files → On-demand Subtree Attachment](/docs/features/context-files#on-demand-subtree-attachment).
</Note>
...
<CardGroup cols={2}>
  <Card title="Context Files" icon="file-lines" href="/features/context-files">
