...
<Note>
**Duplicate Page Notice:** This page documents the same `AsyncAgentScheduler` class as `/docs/features/async-agent-scheduler`. The canonical reference is [Async Agent Scheduler](/docs/features/async-agent-scheduler).
</Note>
...
See [Async Agent Scheduler](/docs/features/async-agent-scheduler#factory-methods) for complete parameter documentation.

---

## Reading Stats

⚠️ **Behaviour change from previous releases:** `get_stats()` is now an async method that delegates to `get_stats_async()`. Callers that did `scheduler.get_stats()` (no `await`) now receive a coroutine. Either `await` it, or call `get_stats_sync()` for a drop-in sync replacement.

| Method | Sync/Async | Atomicity | When to Use |
|--------|-----------|-----------|-------------|
| `await scheduler.get_stats()` | async | **Atomic snapshot** under `_stats_lock` | Anywhere — this is the canonical reader |
| `await scheduler.get_stats_async()` | async | **Atomic snapshot** under `_stats_lock` | Equivalent to `get_stats()` |
| `scheduler.get_stats_sync()` | sync | **Best-effort, no lock** | Sync contexts only (tests, scripts, REPL) |
