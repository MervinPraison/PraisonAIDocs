...
Statistics can be read in both sync and async contexts. See [Async Agent Scheduler - Reading Stats](/docs/features/async-agent-scheduler#reading-stats) for detailed information about `get_stats()`, `get_stats_async()`, and `get_stats_sync()` methods.

| Method | Sync/Async | Atomicity | When to Use |
|--------|-----------|-----------|-------------|
| `scheduler.get_stats()` | sync | **Best-effort, no lock** | Backward compatibility. Quick stats access |
| `await scheduler.get_stats_async()` | async | **Atomic snapshot** under `_stats_lock` | Async contexts where consistent stats needed |
| `scheduler.get_stats_sync()` | sync | **Best-effort, no lock** | Explicit sync contexts (tests, scripts) |
