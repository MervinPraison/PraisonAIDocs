...
An `auth` error with **no failover manager configured** now surfaces immediately (`surface_error`) instead of falling through the generic retry ladder. Before [PR #4958](https://github.com/MervinPraison/PraisonAI/pull/4958), such an error burned the whole retry budget on a credential that would never recover; with a failover manager registered, `auth` still triggers `rotate_profile` as before.

<Note>
`AgentErrorKind` (returned by `classify_error_kind()`) is the classifier the runtime consults for retry and failover decisions. A second, lower-level `classify_error()` in `praisonaiagents/llm/error_classifier.py` returns an older `ErrorCategory` enum used by some helpers and tests. The two can label the same phrase differently — for example, `quota exceeded` is `billing` here (non-retryable) but `RATE_LIMIT` in the lower-level classifier — because the higher-level one applies billing precedence. Read `AgentErrorKind` when deciding how a call will actually behave.
</Note>
...
<Warning>
The private `_classify_error_and_should_retry` method (and its `_legacy` alias) was removed in `praisonaiagents 1.6.161` ([PR #3487](https://github.com/MervinPraison/PraisonAI/pull/3487)). Overriding it in a subclass now has **no effect** — the framework only calls `resolve_failover_decision`.
</Warning>

If you previously subclassed `LLM` to customise retry classification via `_classify_error_and_should_retry`, override `resolve_failover_decision` instead. It returns a typed `FailoverDecision` in place of the old `(category, retry, delay)` tuple.

```python
# OLD — no longer works after praisonaiagents 1.6.161
from praisonaiagents import LLM

class MyLLM(LLM):
    def _classify_error_and_should_retry(self, error, attempt=1):
        # Silently ignored — nothing calls this method any more
        return "rate_limit", True, 5.0

# NEW — override the live decision point
from praisonaiagents import LLM
...
If you called `llm._classify_error_and_should_retry(...)` directly (rare — the underscore prefix marked it as private), the equivalent public path is `llm.resolve_failover_decision(error, {"attempt": n, "max_retries": llm._max_retries})`.
