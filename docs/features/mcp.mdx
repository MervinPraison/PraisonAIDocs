...
An MCP command has an executable, its own flags, and any runtime arguments — paths, tokens, or database URLs. Supply them in any of three equivalent ways.

<Tabs>
<Tab title="Single string">
```python
from praisonaiagents import Agent, MCP
...
agent.start("List all files in /tmp.")
```
</Tab>
<Tab title="String + args">
```python
from praisonaiagents import Agent, MCP

agent = Agent(
    instructions="You can read and write files.",
    tools=MCP("npx -y @modelcontextprotocol/server-filesystem", args=["/tmp"]),
)
agent.start("List all files in /tmp.")
```
</Tab>
...
agent = Agent(
    instructions="You can read and write files.",
    tools=MCP(command="npx", args=["-y", "@modelcontextprotocol/server-filesystem", "/tmp"]),
)
agent.start("List all files in /tmp.")
```
</Tab>
...
All three produce the same subprocess: `npx -y @modelcontextprotocol/server-filesystem /tmp`. The command string is shell-split; any `args=` you pass are appended after the tokens from the string. Keep the server package name in the string and pass user-supplied paths through `args=` when that reads best.

```mermaid
graph TB
    Q{How do you want to pass arguments?}
    Q -->|Everything in one line| A["MCP(&quot;npx -y ...server-filesystem /tmp&quot;)"]
    Q -->|Package in string,<br/>runtime arg separate| B["MCP(&quot;npx -y ...server-filesystem&quot;, args=[&quot;/tmp&quot;])"]
    Q -->|Executable and every<br/>token separate| C["MCP(command=&quot;npx&quot;, args=[&quot;-y&quot;, ...])"]

    A --> Same["Same subprocess:<br/>npx -y ...server-filesystem /tmp"]
    B --> Same
    C --> Same
