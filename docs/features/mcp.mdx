...
Agents can use tools from multiple MCP servers simultaneously. Unpack each server's tools with `*MCP(...)` and combine them into a single `tools` list for maximum capability. When you load several servers with [`load_mcp_tools`](/docs/features/load-mcp-tools), each server's tools are auto-namespaced (`filesystem_search`, `github_search`) so overlapping names never collide.
</Accordion>
</AccordionGroup>
