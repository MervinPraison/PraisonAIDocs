...
<Tip>
**First run downloads the server.** Cold-start launchers (`npx`, `uvx`, `bunx`, `pnpm`, `yarn`) fetch the package before the handshake starts, so their init timeout auto-extends to **180s** when you don't pass `timeout=` (PraisonAI [#5110](https://github.com/MervinPraison/PraisonAI/pull/5110)). On Windows over VPN or a corporate proxy the first download can still exceed 3 minutes — pre-warm once (`npx -y @modelcontextprotocol/server-filesystem %TEMP%`, wait for `listening on stdio`) or pass a larger `timeout=`.
</Tip>

<Note>
**Works inside `praisonai code`.** Declaring `mcp.servers` in your project config surfaces those tools in the coding session as a standard `mcp` tool group, using the same collector as `praisonai run`. Opt out with `PRAISON_TOOLS_DISABLE=mcp`; an MCP server can never shadow a built-in tool name. See [MCP tools in coding sessions](/cli/code#mcp-tools-in-coding-sessions).
</Note>

The user asks to inspect the filesystem; the agent calls MCP tools on the connected server.

<Note>
MCP tool turns are treated specially: a mid-response network failure surfaces `provider_outcome_unknown` instead of silently re-running the tool. See [Replay-Safe Retries](/features/replay-safe-retries).
</Note>
...
This page covers the **client** layer. To serve your own agents, see the [praisonai-mcp Package](/features/praisonai-mcp-package) and [The Three MCP Layers](/features/mcp-three-layers).
...
<Note>
**Cold-start launchers already wait 3 minutes.** When you don't pass `timeout=`, the default init timeout is **60s** — except for cold-start launchers (`npx`, `uvx`, `bunx`, `pnpm`, `yarn`), which auto-extend to **180s** to absorb the first-run package download (PraisonAI [#5110](https://github.com/MervinPraison/PraisonAI/pull/5110)). A `TimeoutError` on a fresh `npx …` install therefore means the download took longer than 3 minutes, not 1. An explicit `MCP(..., timeout=N)` always wins.
</Note>

| Exception | When | What to do |
|-----------|------|-----------|
| `TimeoutError` | The MCP server did not finish the handshake within the effective `timeout=` (60s default, 180s for cold-start launchers). | Increase `timeout=`, or diagnose why the subprocess never responded (Node not installed, wrong `npx` shim on Windows, offline `-y` install). |
...
try:
    tools = MCP("uvx mcp-server-time", timeout=30)
...
The effective init timeout depends on the command and whether you passed `timeout=` explicitly.

```python
from praisonaiagents import Agent, MCP

# Cold-start launcher — no explicit timeout, effective init timeout is 180s
agent = Agent(
    instructions="You manage files.",
    tools=MCP("npx -y @modelcontextprotocol/server-filesystem /tmp"),
)

# Explicit timeout always wins, even for a launcher
agent = Agent(
    instructions="You manage files.",
    tools=MCP("npx -y @modelcontextprotocol/server-filesystem /tmp", timeout=300),
)

# Non-launcher stdio command — plain 60s default
agent = Agent(
    instructions="You manage files.",
...
Forcing a tight timeout on a launcher raises with a copy-pasteable pre-warm hint:

```python
from praisonaiagents import MCP

try:
    tools = MCP("uvx mcp-server-time", timeout=5)
except TimeoutError as e:
    print(e)
    # MCP initialization timed out after 5 seconds
    # (command='uvx' args=['mcp-server-time']). This looks like a first-run
    # package download; pre-warm it once with `uvx mcp-server-time` (wait for
    # the server to start), or pass a larger timeout, e.g. MCP(..., timeout=300).
```
...
# tools = MCP("npx -y @modelcontextprotocol/server-filesystem /tmp")   # now raises

# Windows fix — pass the .cmd shim explicitly:
tools = MCP(command="npx.cmd", args=["-y", "@modelcontextprotocol/server-filesystem", "/tmp"])

# Or use a Python MCP server (works on every platform):
tools = MCP(command=sys.executable, args=["path/to/server.py"])

agent = Agent(instructions="Use the filesystem tools.", tools=tools)
agent.start("List the files in /tmp.")
```
...
On Windows, see [Windows SDK Quickstart → Troubleshooting](/features/windows-sdk-quickstart#troubleshooting). For transport details, see [MCP Transports](/mcp/transports#fail-closed-on-empty-or-timed-out-servers).
...
Agents can use tools from multiple MCP servers simultaneously. Unpack each server's tools with `*MCP(...)` and combine them into a single `tools` list for maximum capability. Loading via [`load_mcp_tools`](/features/load-mcp-tools) auto-namespaces each server's tools (e.g. `filesystem_search`, `github_search`) so overlapping names never collide.
</Accordion>
...
<Card icon="plug" href="/features/mcp-tool-filtering">
