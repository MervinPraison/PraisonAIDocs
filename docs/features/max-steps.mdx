...
| `"content_filtered"` / `"refused"` / `"length_truncated"` | The provider blocked, refused, or truncated the response — see [Run Outcome → Provider block outcomes](/features/run-outcome#provider-block-outcomes) |
...
See [`praisonai run` → Exit Codes](/docs/cli/run#exit-codes) for the full CLI contract.
...
See [`praisonai code` → Step budget](/cli/code#step-budget-for-coding-sessions) for the full contract and the "why 200" rationale.
...
<Card title="Error Handling" icon="shield-alert" href="/features/error-handling">
...
<Card title="praisonai run → Exit Codes" icon="terminal" href="/docs/cli/run#exit-codes">
  How a truncated run maps to exit `2` and `status: "truncated"` on the CLI
</Card>
<Card title="praisonai code → Step budget" icon="code" href="/cli/code#step-budget-for-coding-sessions">
