...
`max_steps` bounds outer-loop iterations (one LLM round-trip each). `max_tool_calls_per_turn` caps how many tool calls may fire within a single LLM response. They govern different things — a per-turn cap of 5 does **not** halve your step budget.
