...
## Unsupported-field warnings (PraisonAI #3771)

When a YAML config declares a field a specific framework adapter doesn't wire — for example `approval:` on the CrewAI or AutoGen adapter — the adapter now emits a `logging.warning` naming the dropped field(s). Previously the field was silently ignored, so users who wrote `approval:` and wondered why nothing happened had no signal in the logs.

- **PraisonAI-native (`framework: praisonai`)** — no warning; the native runner supports every declared field.
- **CrewAI / AutoGen** — warning logged for each unsupported field per agent/task.

### The `SUPPORTED_YAML_FIELDS` contract (PraisonAI #4651)

Each adapter declares the extended YAML fields it honours as a class attribute — the single source of truth is the adapter that reads a field. [PraisonAI PR #4651](https://github.com/MervinPraison/PraisonAI/pull/4651) replaced the old module-level `_ADAPTER_SUPPORTED_FIELDS` dict with this per-class set.
...
class MyFrameworkAdapter(BaseFrameworkAdapter):
    name = "myframework"
    # Extended fields this backend actually consumes, beyond the structural
    # keys (tools, tasks, role, goal, backstory). Empty set = "supports
    # everything" (the praisonai backend).
    SUPPORTED_YAML_FIELDS: ClassVar[FrozenSet[str]] = frozenset({"llm"})
```

`warn_unsupported_fields(adapter, extras)` reads `adapter.SUPPORTED_YAML_FIELDS` directly. Any per-agent YAML key not in this set — after the structural keys `tools`, `tasks`, `role`, `goal`, `backstory` are removed — is logged once per agent:

```
framework='crewai' ignores YAML field(s) ['approval'] for agent 'Researcher'; these are only honoured by framework=praisonai.
```

<Warning>
**API change:** `warn_unsupported_fields(adapter, extras)` now takes the **adapter instance** as its first argument, not the adapter's name string. If you call it directly, pass `self`, not `self.name`.
</Warning>

The two built-in overrides are working templates for third-party authors:
...
# autogen_adapter.py — shared by AutoGenAdapter, AutoGenV4Adapter,
# AG2Adapter, and AutoGenFamilyAdapter
SUPPORTED_YAML_FIELDS = frozenset({"llm", "function_calling_llm"})
```

### AutoGen callback parity

- **`agent_callback` now fires per agent (PraisonAI #3771).** The AutoGen adapter previously declared but never invoked `agent_callback`. It now runs once per agent, matching the CrewAI adapter's behaviour. `task_callback` is still **not** wired for AutoGen — `initiate_chats` doesn't accept a `message_callback` kwarg, so passing `task_callback` on an `autogen:` config has no effect (declare only what you actually need).
...
<Card title="Framework Adapter Registry API" icon="code" href="/sdk/reference/praisonai/modules/adapters">
...
| `SUPPORTS_WORKFLOW` | ❌ | `bool`, default `False`. Set `True` (class-level) to accept native workflow YAML dispatch. See [Capability Flags](#capability-flags). |
| `SUPPORTS_RUNTIME_FEATURES` | ❌ | `bool`, default `False`. Set `True` (class-level) to accept `cli_backend` / `runtime` config keys. See [Capability Flags](#capability-flags). |
| `SUPPORTED_YAML_FIELDS` | ❌ | `frozenset[str]`, default `frozenset()`. Extended YAML fields this backend honours beyond the structural keys. Empty set = "supports everything". See [Capability Flags](#capability-flags). |
...
| `run(config, llm_config, topic, *, tools_dict, agent_callback, task_callback, cli_config)` | ✅ | Execute framework logic (all four trailing kwargs are validated at registration time — see Troubleshooting). Relying on `**kwargs` instead of naming these validates but logs a warning — see [`**kwargs`-only adapters are accepted but warned](#kwargs-only-adapters-are-accepted-but-warned). |
| `arun(config, llm_config, topic, *, tools_dict, agent_callback, task_callback, cli_config)` | ❌ | Async execution path. Validated at registration time whenever it exists — same rules as `run()`, including the [`**kwargs`-only warning](#kwargs-only-adapters-are-accepted-but-warned). `arun` remains optional; adapters without it keep the sync-only offload behaviour. When `SUPPORTS_ASYNC` is `False` (default), it offloads `run()` to this adapter's bounded thread pool and logs a one-line warning. Override with a native `async def` for a fully async path. |
...
Since PR #2083, `FrameworkAdapterRegistry.create()` validates every adapter at construction time. If the `run()` method does not accept the four parameters `tools_dict`, `agent_callback`, `task_callback`, and `cli_config`, instantiation fails with a per-method `TypeError`:

```
TypeError: FrameworkAdapter '<name>'.run does not implement the protocol:
missing keyword parameters ['agent_callback', 'cli_config', 'task_callback', 'tools_dict']
```

The error names the failing method (`.run` or `.arun`), so `arun()` is now validated with the same rules whenever it exists. A `run()` (or `arun()`) that declares a `**kwargs` catch-all satisfies the check without listing each protocol keyword explicitly.
...
#### `**kwargs`-only adapters are accepted but warned

A signature that relies on `**kwargs` still validates for backward compatibility — an adapter can legitimately forward `**kwargs` to a delegate. But if `run()` / `arun()` does **not** name every one of `tools_dict`, `agent_callback`, `task_callback`, and `cli_config` explicitly and instead leans on `**kwargs`, registration now logs a warning naming the un-declared kwargs. If those kwargs aren't forwarded, their safety guarantees — `tool_timeout` wraps, `agent_callback` / `task_callback` observability hooks, and `cli_config` approval / guardrails / allowed-tools knobs — are silently dropped.

```
FrameworkAdapter 'my_adapter'.run names none of the adapter kwargs
['agent_callback', 'cli_config', 'task_callback', 'tools_dict']
explicitly and relies on **kwargs; if these are not forwarded, their
safety guarantees (tool_timeout, callbacks, approval/guardrails via
cli_config) will be silently dropped.
```

The warning fires whenever the signature has a `**kwargs` catch-all **and** is missing one or more of the four named kwargs. The log line lists exactly the missing ones. Built-in adapters (`crewai`, `autogen`, `autogen-v4`, `ag2`, `praisonai`) name all four explicitly, so no warning fires for them.

| `run` / `arun` signature | Registration behaviour |
|--------------------------|-------------------------|
| Names all four kwargs explicitly (with or without `**kwargs`) | Silent — no warning |
| Names some / none of the four kwargs, **no** `**kwargs` | `TypeError` at registration |
| Missing one or more of the four kwargs, **has** `**kwargs` | Accepted **but** `logger.warning` names the un-declared kwargs |

<Tip>
Always name `tools_dict`, `agent_callback`, `task_callback`, and `cli_config` explicitly in `run()` / `arun()` (even if you also accept `**kwargs`) so the safety guarantees are preserved and the registry stays silent.
</Tip>
...
```mermaid
graph LR
    ARun[⚡ arun called] --> Flag{SUPPORTS_ASYNC?}
    Flag -->|True| Native[🚀 native async run]
    Flag -->|False| Warn[⚠️ log one-line warning]
    Warn --> Pool[🧵 bounded ThreadPoolExecutor<br/>max_workers = _THREAD_OFFLOAD_MAX_WORKERS]
    Pool --> Sync[🔁 offload sync run]
...
Capability flags are class-level booleans that tell PraisonAI which features your adapter can execute.

Three flags gate distinct wrapper features. All default to `False`, so an adapter opts in only to what it can actually run.
...
| `SUPPORTED_YAML_FIELDS` | `frozenset[str]` | `frozenset()` | Extended YAML fields this backend honours beyond the structural keys (`tools`, `tasks`, `role`, `goal`, `backstory`). Empty set = "supports everything" (the `praisonai` adapter). Otherwise any per-agent YAML key not in this set — after structural keys are removed — is logged as `framework=%r ignores YAML field(s) %s for agent %r; these are only honoured by framework=praisonai.` Read by `warn_unsupported_fields(adapter, extras)`. |

The built-in `PraisonAIAdapter` sets both `SUPPORTS_WORKFLOW` and `SUPPORTS_RUNTIME_FEATURES` to `True`. Third-party adapters registered via the `praisonai.framework_adapters` entry-point group opt in by setting the flags on their subclass.
...
graph LR
    subgraph "Capability Flags"
        WF[🚩 SUPPORTS_WORKFLOW]
        RF[🚩 SUPPORTS_RUNTIME_FEATURES]
    end
    WF --> V[🛠️ validate_workflow_framework]
    WF --> E[🛠️ JobExecutor._framework_supports_native]
    RF --> C[🛠️ AgentsGenerator._validate_cli_backend_compatibility]
    V --> R1[✅ Workflow YAML runs]
    E --> R2[✅ Native dispatch]
    C --> R3[✅ cli_backend / runtime accepted]
...
The router consults the *owning* registry — the registry that constructed this adapter — with `get_default_registry()` as the fallback when the adapter was constructed outside a registry (standalone use). `FrameworkAdapterRegistry.create()` post-attaches itself as `adapter._owning_registry`, so router adapters honour the same registry that instantiated them without any explicit plumbing. A module-level `_owning_registry(adapter)` helper returns `adapter._owning_registry` when present, else the process-default registry; `is_available()`, `resolve_alias()`, and `resolve()` all route through it.
...
class AutoGenFamilyAdapter(BaseFrameworkAdapter):
    ...
    def resolve(self, *, config: Optional[Dict[str, Any]] = None) -> "BaseFrameworkAdapter":
        ...
        # _owning_registry(self) returns self._owning_registry when the registry
        # that built this adapter post-attached it, else the process default.
        return _owning_registry(self).create(adapter_name)
...
- `resolve()` passes `config` to `resolve_alias()`, then fetches the concrete adapter from the *owning* registry (`get_default_registry()` fallback for standalone use)
...
<Note>
**Owning-registry resolution (PR [#4861](https://github.com/MervinPraison/PraisonAI/pull/4861)).** A tenant-scoped `FrameworkAdapterRegistry` passed as `adapter_registry=` is now honoured when a workflow uses the `framework: autogen` family alias — not just direct aliases like `autogen_v2`. `AutoGenAdapter.resolve()` keeps its v0.2 → v0.4 fallback: an *unregistered* `autogen_v4` name still falls back to the built-in `AutoGenV4Adapter()` stub, but if `autogen_v4` **is** registered and its loader/constructor/validation raises, that error now surfaces instead of being silently swallowed as "v0.4 absent" — the actionable-error path for anyone registering a real v0.4 replacement via entry points.
</Note>

<Note>
As of PR #2654, AutoGen v0.2 now wires YAML `tools:` into the chat via `register_for_llm` + `register_for_execution`, and translates a per-call `ToolTimeoutError` into an LLM-readable string in `_wrap_tool_for_execution`. Custom adapters that run tools inside their own loop should follow the same `_wrap_tool_for_execution` pattern for timeout translation. See [AutoGen v0.2 YAML Tools](/features/autogen-yaml-tools).
</Note>
...
As of PR #3152, `setup()` is invoked **inside** `observability_session`, so any events emitted by `setup()` — and any exception it raises — are captured and finalized. Custom adapters should therefore **not** open their own observability session inside `setup()`.

### Orchestrator Pipeline (Post-#3152)

As of PR #3152, `AgentsGenerator` owns the observability lifecycle. Both the sync (`generate_crew_and_kickoff`) and async (`agenerate_crew_and_kickoff`) run paths bracket `adapter.setup()` **and** `adapter.run()`/`adapter.arun()` in a single `observability_session`, so setup and run share one session and finalize is impossible to forget. The orchestrator now follows this sequence:

<Note>
The **sync** path additionally runs `adapter.run()` inside a per-call `scoped_bridge()`, isolating its `run_sync`-driven work onto its own loop+thread. The async path (`adapter.arun()`) is unchanged. See [Async Bridge → Sync `generate_crew_and_kickoff` auto-scoping](/features/async-bridge#sync-generate_crew_and_kickoff-auto-scoping).
</Note>
...
sequenceDiagram
    ...
    participant Obs as observability_session
    ...
    Gen->>Gen: assert_framework_available(adapter.name, registry=self._adapter_registry)
    Gen->>Obs: enter observability_session(adapter.name)
    Gen->>Adapter: setup(framework_tag=adapter.name)
    Note over Gen,Adapter: setup now runs inside the session
    Gen->>Adapter: run(config, llm_config, topic)
    Adapter-->>Gen: result
    Gen->>Obs: exit observability_session (auto-finalize)
```
...
<Accordion title="Name the four adapter kwargs explicitly">

Always name `tools_dict`, `agent_callback`, `task_callback`, and `cli_config` in `run()` / `arun()` — even if you also accept `**kwargs`. This keeps the safety guarantees (`tool_timeout` wraps, callbacks, and `cli_config` approval / guardrails) intact and stops the registry logging a warning at registration.
...
An adapter that leans on `**kwargs` alone still registers, but logs a warning naming the un-declared kwargs — see [`**kwargs`-only adapters are accepted but warned](#kwargs-only-adapters-are-accepted-but-warned).
...
| `get_install_hint(name, *, registry=None)` | `str` | Adapter-aware install hint string (e.g. `pip install 'praisonai-frameworks[autogen]'`). Pass `registry=` to derive the hint from an injected registry (DI-friendly); defaults to the process-default. |
...
The static fallback list used by the CLI only when the adapter layer itself cannot be imported is `["ag2", "autogen", "crewai", "praisonai"]` (alphabetical) — `ag2` still appears in this install-hint list for discoverability, but is **not** in the runtime `DEFAULT_PRIORITY` tuple. This prevents `pick_default()` from routing to `ag2` and hitting a `NotImplementedError`.
</Note>

### `resolve_or_default()` — for optional framework inputs

When a caller may or may not pass a framework name, use `resolve_or_default()` — it returns the caller's choice if given, or the registry's `pick_default()` otherwise.

```python
FrameworkAdapterRegistry.resolve_or_default(name: Optional[str]) -> str
```

| Input | Returns |
|---|---|
| `"crewai"` (any truthy string, whitespace stripped) | `"crewai"` |
| `"  autogen  "` | `"autogen"` |
| `None` / `""` / `"   "` | Result of `self.pick_default()` |
| `None` and `pick_default()` raises `RuntimeError` | `DEFAULT_PRIORITY[0]` (safe fallback) |

`resolve_or_default()` now:
- **strips whitespace** from truthy strings (`"  autogen  "` → `"autogen"`), matching how `pick_default()` normalises its own output.
- **falls back to `DEFAULT_PRIORITY[0]`** when `pick_default()` raises `RuntimeError` (no adapter installed), instead of propagating — so downstream availability validation gets a chance to raise its own actionable error rather than this helper masking it.
...
registry.resolve_or_default("crewai")   # → "crewai"
registry.resolve_or_default(None)       # → whatever pick_default() picks
```

```mermaid
graph LR
    In[name arg] --> Q{name is None or empty?}
    Q -->|no| Explicit[✅ return name stripped]
    Q -->|yes| PD[pick_default]
    PD --> Ret[✅ return default]
    PD -->|RuntimeError| Fallback[✅ DEFAULT_PRIORITY 0]
    classDef q fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef ok fill:#10B981,stroke:#7C90A0,color:#fff
    classDef in fill:#6366F1,stroke:#7C90A0,color:#fff
    class In in
    class Q q
    class Explicit,Ret,PD,Fallback ok
```

<Note>
Wrapper entry points used to fall back to a hardcoded `"praisonai"` when no framework was passed, which bypassed the registry's default-selection policy — a problem when a custom / tenant-scoped registry was injected. `resolve_or_default()` centralises the "None → pick_default()" step so every caller stays consistent.
</Note>

As of PR [#5039](https://github.com/MervinPraison/PraisonAI/pull/5039) (Gap 1 of [#5016](https://github.com/MervinPraison/PraisonAI/issues/5016)), every wrapper entry point that used to fall back to a hardcoded `"praisonai"` now goes through the registry:

| Call site | Purpose |
|---|---|
| `AgentsGenerator._prepare_for_run` | Selects the adapter for `agents.yaml` runs |
| `AgentsGenerator.generate_crew_and_kickoff` / `agenerate_crew_and_kickoff` | Tags workflow-YAML observability sessions with the config-resolved framework (was a hardcoded `"praisonai"` label — wrong for non-praisonai workflow adapters) |
| `AgentsGenerator._build_yaml_workflow` | Resolves the framework for `cli_backend` compatibility validation on workflow YAMLs |
| `praisonai.jobs.router.submit_job` | Resolves the framework passed on the Jobs API's `POST /api/v1/runs` request |
| `praisonai.jobs.executor._run_agent` / `_run_praisonai_agents` | Resolves the framework for job execution — first invocation is dispatched via `asyncio.to_thread` so first-time registry construction (which probes entry-point imports) does not stall the event loop |
| `praisonai.scheduler.yaml_loader.load_agent_yaml_with_schedule` | Resolves the framework recorded in scheduler-YAML configurations |
| `praisonai.framework_adapters.workflow_framework.framework_from_config` | The core workflow-YAML helper — now accepts an optional `registry=` kwarg |
...
For workflow YAML, use `WorkflowAutoGenerator` — its `framework=` kwarg is honoured end-to-end (see [AutoAgents](/features/autoagents)).

### Framework validation honours the injected registry

As of PR #3152, framework-availability validation is DI-aware — a scoped/per-tenant adapter registered on the generator's own registry is no longer rejected just because it is absent from the process-default registry.

- `assert_framework_available(name, *, registry=None)` and `get_install_hint(name, *, registry=None)` now accept an optional `registry` kwarg (default: process-default).
- `AgentsGenerator._prepare_for_run` threads its injected `_adapter_registry` through into `assert_framework_available`, so validation consults the tenant registry, not the process-default.
- The install hint is derived from the same registry that failed validation (DI end to end).
- **Fallback for resolved built-in names:** if the injected registry does not know a name but the process-default does (typical when a family router resolves `autogen → autogen_v2`), validation falls back to the default and the framework is accepted. This preserves the DI seam without rejecting resolved built-ins.
...
tenant_registry = FrameworkAdapterRegistry()
tenant_registry.register("tenant_scoped_fw", TenantScopedAdapter)

gen = AgentsGenerator(
    agent_file="agents.yaml",
    framework="tenant_scoped_fw",
    config_list=[...],
    adapter_registry=tenant_registry,   # DI seam
)
# Validation now consults tenant_registry, not the process-default.
```
...
<AccordionGroup>
<Accordion title="TypeError: <name>.run does not implement the protocol: missing keyword parameters [...]">

Your adapter's `run()` (or `arun()`) method is missing the four parameters required by the `FrameworkAdapter` protocol. As of [PR #2083](https://github.com/MervinPraison/PraisonAI/pull/2083), `FrameworkAdapterRegistry.create()` validates these at adapter construction time. The error names the failing method:
...
A `**kwargs` catch-all also passes — handy when your adapter forwards to a delegate:

```python
def run(self, config, llm_config, topic, **kwargs):  # ✅ now accepted
    tools_dict = kwargs.get("tools_dict")
    agent_callback = kwargs.get("agent_callback")
    ...
```
...
A `**kwargs` adapter is no longer a cause of this — if your adapter still disappears, the culprit is a named-parameter signature that omits one of the four protocol kwargs, or an `ImportError` in `__init__`.
...
<CardGroup cols={2}>
<Card title="Registry Dependency Injection" icon="puzzle-piece" href="/features/registry-dependency-injection">
  Learn about injecting custom registries for multi-tenant isolation
</Card>
<Card title="CrewAI Integration" icon="users" href="/framework/crewai">
  See how built-in framework adapters are implemented
</Card>
<Card title="Workflow YAML Framework Field" icon="diagram-project" href="/features/workflow-yaml-framework-field">
