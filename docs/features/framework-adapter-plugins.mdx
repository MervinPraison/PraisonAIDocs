...
| `arun(config, llm_config, topic)` | ⚠️ Optional (default provided) | Async-native execution. `BaseFrameworkAdapter` provides a default that offloads `run()` to a worker thread via `asyncio.to_thread`, so existing adapters need no changes. |
...
class MyAsyncAdapter(BaseFrameworkAdapter):
    ...
    def run(self, config, llm_config, topic, **kwargs) -> str:
        # Sync implementation for backward compatibility
        import myframework
        return myframework.execute_sync(config, topic)

    async def arun(self, config, llm_config, topic, **kwargs) -> str:
        # Native async path - called when invoked from a running event loop
        import myframework
        return await myframework.aexecute(config, topic)
```

Implement `arun` only if your framework has a native async API; otherwise the default thread offload is correct.
...
```python
from praisonai.framework_adapters._framework_availability import is_available as _is_available

def is_available(self) -> bool:
    return _is_available("myframework")  # cached, uses importlib.util.find_spec
```
