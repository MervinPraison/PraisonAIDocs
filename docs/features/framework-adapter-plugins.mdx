...
| Input | Returns |
|---|---|
| `"crewai"` (any truthy string) | `"crewai"` (unchanged) |
...
`AgentsGenerator._prepare_for_run` now uses `self._adapter_registry.resolve_or_default(self.framework or config.get('framework'))` internally, so an omitted framework resolves through the injected registry's `pick_default()` instead of a hardcoded string.
