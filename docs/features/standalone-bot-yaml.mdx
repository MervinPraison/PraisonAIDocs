---
title: "Standalone Bot YAML"
sidebarTitle: "Standalone YAML"
description: "Describe a standalone gateway + agent + channels declaratively"
icon: "file-code"
---

Describe your agent, gateway, and channel platforms in a single YAML file — no Python required.

```python
from praisonaiagents import Agent

agent = Agent(name="bot-agent", instructions="Reply to users across chat channels.")
agent.start("Answer incoming messages from Telegram and Discord.")
```

The user defines a `bot.yaml`; the gateway loads it and connects the agent to every configured channel.

```mermaid
graph LR
    subgraph "Standalone Bot YAML"
        In[📄 bot.yaml] --> GW[🗼 Gateway]
        GW --> Agent[🤖 Agent]
        Agent --> Out[✅ Channel Reply]
    end

    classDef input fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef process fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef agent fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef output fill:#10B981,stroke:#7C90A0,color:#fff

    class In input
    class GW process
    class Agent agent
    class Out output
```

## How It Works

```mermaid
sequenceDiagram
    participant User
    participant Channel
    participant Gateway
    participant Agent

    User->>Channel: Message on Telegram / Discord
    Channel->>Gateway: Forward inbound event
    Gateway->>Agent: Run configured agent
    Agent-->>User: Reply on the same channel
```

### Config Structure

```mermaid
graph TB
    subgraph "YAML Config"
        Y[📄 bot.yaml] --> A[agent]
        Y --> G[gateway]
        Y --> P[platforms]
    end
    A --> Agent[🤖 Agent]
    G --> GW[🗼 WebSocket\nGateway]
    P --> C1[💬 Telegram]
    P --> C2[🎮 Discord]
    Agent --> GW
    GW --> C1
    GW --> C2

    classDef yaml fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef agent fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef gateway fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef channel fill:#10B981,stroke:#7C90A0,color:#fff

    class Y,A,G,P yaml
    class Agent agent
    class GW gateway
    class C1,C2 channel
```

## Quick Start

<Steps>
<Step title="Install">
```bash
pip install praisonaiagents "praisonai-bot[gateway,bot]"
```
</Step>

<Step title="Set credentials">
```bash
export OPENAI_API_KEY=your_openai_api_key
export TELEGRAM_BOT_TOKEN=your_telegram_token
```
</Step>

<Step title="Create bot.yaml">
```yaml
agent:
  name: assistant
  instructions: You are a helpful assistant.
  llm: gpt-4o-mini

gateway:
  host: 127.0.0.1
  port: 8765

platforms:
  telegram:
    token: ${TELEGRAM_BOT_TOKEN}
```
</Step>

<Step title="Start the gateway with your YAML">
```bash
praisonai-bot gateway start --config bot.yaml
```
The gateway registers the agent and starts all configured channel bots automatically.
</Step>
</Steps>

---

## Config Options Reference

### `agent` section

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `agent.name` | `str` | required | Agent display name, used as the `agent_id` for gateway routing |
| `agent.instructions` | `str` | required | System prompt / instructions for the agent |
| `agent.llm` | `str` | `"gpt-4o-mini"` | LLM model identifier (e.g. `gpt-4o`, `gpt-4o-mini`) |

### `gateway` section

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `gateway.host` | `str` | `"127.0.0.1"` | Host to bind the WebSocket server |
| `gateway.port` | `int` | `8765` | Port to listen on |

### `platforms` section

Each key under `platforms` is a platform name. Use `${VAR_NAME}` for environment variable interpolation.

| Key pattern | Type | Description |
|-------------|------|-------------|
| `platforms.<name>.token` | `str` | Bot token for the platform. Supports `${ENV_VAR}` substitution |
| `platforms.slack.app_token` | `str` | Slack App-Level Token (`xapp-...`) for Socket Mode. Required for Slack when Socket Mode is used. Supports `${ENV_VAR}` and secret references. |

**Supported platforms:** `telegram`, `discord`, `slack`, `whatsapp`, and any platform registered via entry points.

### `events` — inbound platform events (opt-in)

Add an `events:` list under a channel to receive reactions, edits, deletions, member changes, and thread creations:

```yaml
channels:
  my-discord-bot:
    platform: discord
    token: "${DISCORD_BOT_TOKEN}"
    events: [reactions, edits, members, threads]
```

See [Inbound Platform Events](/features/bot-inbound-events) for the full list of accepted classes and what each one enables.

### Single-platform shorthand

For one platform, declare a top-level `platform:` (and optionally `token:`) instead of a `platforms:` map:

```yaml
platform: telegram
token: "${TELEGRAM_BOT_TOKEN}"
```

`token:` is optional — omit it and the declared `platform:` resolves the platform's own documented env var (`TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, etc.):

```yaml
platform: telegram
```

The declared platform always wins. If other platform credentials are exported in the environment, `bot start` refuses to auto-enable them and logs a warning:

```
WARNING Config declares 'platform: telegram'; NOT auto-enabling discord, slack
despite their credentials being present in the environment. The declared
platform wins. To run them too, list them under 'channels:' explicitly.
```

<Warning>
Earlier versions could silently start the wrong platform if unrelated credentials were set. Now the declared platform always wins — list multiple platforms under `channels:` explicitly to run them together.
</Warning>

#### Admission policy (`unknown_user_policy` / `owner_user_id`)

Control who may DM a single-bot instance directly from `bot.yaml`. Since [PR #5111](https://github.com/MervinPraison/PraisonAI/pull/5111), `praisonai bot start` wires `unknown_user_policy` and `owner_user_id` into the running bot for **telegram / discord / slack** — matching the gateway path exactly.

```yaml
# bot.yaml — single-bot admission policy
platform: telegram
token: ${TELEGRAM_BOT_TOKEN}
unknown_user_policy: allow        # deny (default) | pair | allow
owner_user_id: 987654321          # required when unknown_user_policy: pair
agent:
  name: Assistant
  instructions: Reply to incoming messages.
```

Both fields also work at the channel level for the multi-channel shape:

```yaml
# bot.yaml — channel-level admission policy
channels:
  telegram:
    token: ${TELEGRAM_BOT_TOKEN}
    unknown_user_policy: pair
    owner_user_id: 987654321
agent:
  name: Assistant
  instructions: Reply to incoming messages.
```

| Value | Meaning |
|-------|---------|
| `deny` | Default. Drop inbound DMs from users not on `allowed_users`. Since [PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112) every drop logs a `WARNING` with the sender's `user_id`, `platform`, and `conversation` so operators can distinguish a misconfigured allowlist from a broken bot. |
| `pair` | Route unknown users through the owner-approval / pairing flow. Requires `owner_user_id`. |
| `allow` | Open DMs to everyone. Trusted networks / open bots only. |

Values are case-insensitive on load (`ALLOW` normalises to `allow`) and whitespace is trimmed. An unquoted numeric `owner_user_id` (e.g. `987654321`) is coerced to a string automatically.

<Note>
On startup, `praisonai bot start` prints the resolved policy so you can confirm it at a glance:

```text
unknown_user_policy: allow
```

The line reflects the YAML value, or `deny` when omitted — the secure default is now **visible** rather than silent. It only prints on platforms that actually wire the policy (telegram / discord / slack).

When `allowed_users` is empty, the startup banner also names the resolved policy explicitly:
`... unknown_user_policy=deny. DMs from users not in allowed_users … are dropped and the bot responds to NOBODY new.` for `deny`,
`... unknown_user_policy=allow. Bot will respond to EVERYONE. …` for `allow`, and
`... unknown_user_policy=pair. Unknown users must complete pairing …` for `pair` ([PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112)).
</Note>

<Warning>
A misspelled policy (`alow`, `deney`, …) fails closed at load time rather than silently reverting to `deny`:

```text
Invalid unknown_user_policy 'alow'. Must be one of: allow, deny, pair
```

Treat this error as intended — fix the typo and restart. The same fail-closed validation now applies to the multi-channel `gateway.yaml` schema too ([PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112)).
</Warning>

<Card title="Unknown-User Pairing" icon="user-check" href="/features/bot-unknown-user-pairing">
  Full owner-approval flow, inline buttons, and CLI fallback.
</Card>

#### Slack single-bot shorthand

Slack Socket Mode also needs an app-level token, so add a top-level `app_token:` alongside `token:`:

```yaml
platform: slack
token: "${SLACK_BOT_TOKEN}"
app_token: "${SLACK_APP_TOKEN}"
```

`app_token:` accepts the same three forms as `token:` — plaintext, `${ENV}` interpolation, and the `{ source, id }` [secret reference](/features/gateway-secret-references).

<Note>
`app_token` is Slack-specific. When `platform:` is anything other than `slack`, a top-level `app_token:` is ignored — this keeps a strict non-Slack adapter from rejecting an unexpected keyword. For the multi-channel form, set `app_token` under the Slack entry in `channels:` (see [Messaging Bots](/features/messaging-bots)).
</Note>

Run a Slack bot in three lines plus the agent block:

```yaml
# bot.yaml — Slack Socket Mode single-bot shorthand
platform: slack
token: "${SLACK_BOT_TOKEN}"
app_token: "${SLACK_APP_TOKEN}"

agent:
  name: assistant
  instructions: Reply to Slack mentions.
  llm: gpt-4o-mini
```

```bash
export SLACK_BOT_TOKEN=xoxb-...
export SLACK_APP_TOKEN=xapp-...
praisonai bot start --config bot.yaml
```

```mermaid
graph LR
    Y[📄 bot.yaml] --> S{platform:}
    S -->|slack| CS[✅ token + app_token]
    S -->|telegram / discord| CT[✅ token only]
    Y -.app_token.-> S
    S -.dropped for non-Slack.-> W[⚠️ ignored]
    CS --> Adapter[🔌 Slack Socket Mode]

    classDef yaml fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef process fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef warn fill:#F59E0B,stroke:#7C90A0,color:#fff

    class Y,S yaml
    class CS,CT,Adapter process
    class W warn
```

<Note>
**Bot startup exit codes:** `praisonai bot start` now exits **1** on any startup failure (bad config path, missing token, unknown platform). This makes `systemd Restart=on-failure` and `if praisonai bot start ... ; then` scripting work as expected.
</Note>

### Shell execution keys

Add these per-channel keys to let a bot run shell commands with approval routing.

| Key pattern | Type | Default | Description |
|-------------|------|---------|-------------|
| `<name>.allow_shell` | `bool` | `False` | Opt in shell execution. Injects `execute_command`, lifts the deny-list entry, and appends a permission-grant line to instructions. |
| `<name>.auto_approve_shell` | `bool` | `True` | Runs every shell call immediately **only on a loopback-bound gateway + non-group channel**. On externally-bound gateways or any channel with a `group_policy`, the gateway automatically downgrades this to an approval backend (see `<name>.auto_approve_shell_acknowledge_exposed`). |
| `<name>.auto_approve_shell_acknowledge_exposed` | `bool` | `False` | Explicit acknowledgement that blanket auto-approval is safe on this externally-exposed or multi-user channel. Set to `true` (with `auto_approve_shell: true`) to keep the blanket behaviour on `0.0.0.0` binds or group channels. Off by default — the gateway fails closed to approval. |
| `<name>.approval_channel` | `str` | `None` | Where the approval prompt is delivered (platform-specific ID). |
| `<name>.approval_users` | `str` \| `list[str]` | `None` | Approver allowlist. Comma-separated string or list. |
| `<name>.approval_mode` | `str` | `None` (= `channel`) | Backend selector: `channel`, `gateway`, `http`, `webhook`. Typos raise `ValueError`. |
| `<name>.approval_webhook_url` | `str` | `None` | HTTPS URL for `approval_mode: webhook`. |
| `<name>.approval_http_host` | `str` | `"127.0.0.1"` | HTTP dashboard host for `approval_mode: http`. |
| `<name>.approval_http_port` | `int` | `8899` | HTTP dashboard port for `approval_mode: http`. |

<Card title="Bot Shell Execution" icon="terminal" href="/features/bot-shell-execution">
  Full routing ladder, per-platform ID resolution, and recipes.
</Card>

---

## Environment Variable Interpolation

Values in the form `${VAR_NAME}` are resolved from the current environment at startup:

```yaml
platforms:
  telegram:
    token: ${TELEGRAM_BOT_TOKEN}   # reads os.environ["TELEGRAM_BOT_TOKEN"]
  discord:
    token: ${DISCORD_BOT_TOKEN}    # reads os.environ["DISCORD_BOT_TOKEN"]
```

Tokens stored in `~/.praisonai/.env` (written by `praisonai-bot onboard`) are loaded automatically before interpolation.

---

## When to Pick YAML vs Python

```mermaid
graph TB
    Q{Who configures it?} -->|Non-developer / ops| YAML[📄 YAML config]
    Q -->|Developer / code-first| Python[🐍 Python script]

    YAML --> Y1[✅ No code to maintain]
    YAML --> Y2[✅ Easy credential rotation]
    YAML --> Y3[✅ Works with praisonai-bot gateway start]

    Python --> P1[✅ Programmatic agent construction]
    Python --> P2[✅ Dynamic agent_id or custom registration]
    Python --> P3[✅ Custom middleware / hooks]

    classDef yaml fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef python fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef yes fill:#10B981,stroke:#7C90A0,color:#fff

    class Q yaml
    class YAML,Python python
    class Y1,Y2,Y3,P1,P2,P3 yes
```

| Scenario | Use |
|----------|-----|
| Operations team manages tokens | **YAML** — credentials stay out of code |
| Static single-agent setup | **YAML** — fewer moving parts |
| Dynamic agents (created at runtime) | **Python** — full control over `register_agent()` |
| Custom gateway middleware or hooks | **Python** — subclass or extend `WebSocketGateway` |
| Multiple environments (dev / staging / prod) | **YAML** — per-environment files with env vars |

---

## Common Patterns

### Multi-platform setup

```yaml
agent:
  name: assistant
  instructions: You are a helpful assistant.
  llm: gpt-4o-mini

gateway:
  host: 0.0.0.0
  port: 8765

platforms:
  telegram:
    token: ${TELEGRAM_BOT_TOKEN}
  discord:
    token: ${DISCORD_BOT_TOKEN}
  slack:
    token: ${SLACK_BOT_TOKEN}
```

### Custom port from environment

```yaml
gateway:
  host: 0.0.0.0
  port: 9000
```

Or set the `GATEWAY_PORT` environment variable — the CLI reads it automatically when `--port` is not passed.

---

## Best Practices

<AccordionGroup>
<Accordion title="Always use ${...} for tokens">
Never hardcode tokens in YAML. Use environment variables and store them in `~/.praisonai/.env` (written by `praisonai-bot onboard`) or your secrets manager.

```yaml
# Good
platforms:
  telegram:
    token: ${TELEGRAM_BOT_TOKEN}

# Never do this
platforms:
  telegram:
    token: 1234567890:ABCdef...
```
</Accordion>

<Accordion title="Run praisonai-bot gateway doctor before going live">
```bash
praisonai-bot gateway doctor --config bot.yaml
```
This validates every channel token before starting, so a bad token fails fast instead of silently reconnecting. It also prints a credential availability table for `token`, `app_token`, and `verify_token` — including the [secret-reference form](/features/gateway-secret-references) — **without revealing any value**.
</Accordion>

<Accordion title="Use separate YAML files per environment">
Keep `dev.yaml`, `staging.yaml`, and `prod.yaml` — each referencing different environment variables. Pass the right one with `--config`.
</Accordion>
</AccordionGroup>

---

## Related

<CardGroup cols={2}>
<Card title="Standalone Bot Gateway (Python)" icon="server" href="/features/standalone-bot-gateway">
  Python API equivalent — register agents programmatically
</Card>
<Card title="Gateway CLI" icon="tower-broadcast" href="/features/gateway-cli">
  Full list of gateway CLI commands
</Card>
<Card title="BotOS" icon="robot" href="/features/botos">
  Multi-platform bot orchestration
</Card>
<Card title="praisonai-bot SDK" icon="comments" href="/sdk/praisonai-bot/index">
  Full bot-tier SDK reference
</Card>
</CardGroup>
