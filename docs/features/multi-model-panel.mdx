...
| `base_url` / `api_key` / `api_version` / `auth` | — | — | Connection kwargs forwarded to the underlying `LLM` **and** propagated to reference LLMs so they hit the same backend. |

### Module functions

| Function | Purpose |
|---|---|
| `register_panel_preset(name: str, config: dict)` | Register a `panel:<name>` preset. `config` accepts `references`, `aggregator`, `enabled`, plus any extra LLM kwargs. |
| `resolve_panel_config(descriptor)` | Returns `{"references", "aggregator", "enabled", "options"}`. Raises `ValueError` on unknown preset, missing aggregator, non-string refs, non-bool `enabled`, or recursion. |
| `is_panel_descriptor(llm)` | True if `llm` is `"panel:..."` or `{"provider": "panel", ...}`. |
| `create_panel_llm(descriptor, **llm_kwargs)` | Build a `PanelLLM` from a descriptor; explicit kwargs beat descriptor `options`. |
| `PANEL_PRESETS` | Module-level dict — the preset registry. No bundled presets in core. |
...
- **Partial-failure tolerance** — if one reference fails (network, credentials), it becomes a labelled `(unavailable: reference call failed)` note and the turn continues. The whole turn never fails just because a reference did.
...
Deterministic reference outputs maximise cache hits and keep the aggregator's view stable across tool-loop iterations.
</Accordion>
