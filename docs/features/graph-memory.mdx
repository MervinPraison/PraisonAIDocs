...
sequenceDiagram
    ...
    User->>Agent: Share information
    Agent->>Graph: Extract entities and relationships
    Graph-->>Agent: Stored as nodes/edges
    User->>Agent: Ask question
    Agent->>Graph: Query related nodes
    Graph-->>Agent: Relevant subgraph
    Agent->>LLM: Question + graph context
    LLM-->>Agent: Answer
