...
- Exit `75` is also emitted by [`LoopWatchdog`](/features/gateway-loop-watchdog) when a wedged event loop is detected.
- **Shutdown-phase deadline (PR [#5084](https://github.com/MervinPraison/PraisonAI/pull/5084) / issue [#5079](https://github.com/MervinPraison/PraisonAI/issues/5079)):** the `LoopWatchdog` shutdown-phase deadline fires `os._exit(75)` if graceful drain does not complete within `drain_timeout + gateway.watchdog.shutdown_grace`. The stack-dump header reads *"graceful shutdown did not complete within ~Ns"* instead of the loop-wedge header. Configured via `gateway.watchdog.shutdown_grace` (YAML) or `stop(grace=…)` (Python). See [Loop Watchdog → Shutdown-phase deadline mode](/features/gateway-loop-watchdog#shutdown-phase-deadline-mode).
...
- **Unresolved tool in the config (strict tool pre-flight)** — a tool named in `gateway.yaml` that cannot be resolved (typo, uninstalled optional package, or a gated local `tools.py`). Strict is the default; pass `--no-strict-tools` or set `strict_tools: false` to warn-and-continue instead. See [Pre-flight tool check](/docs/features/gateway-cli#pre-flight-tool-check).
- `FatalConfigError` raised by a plugin or adapter during startup

<Note>
A channel credential that is merely **unavailable** — an unset env var, an empty secret file, a rotation/expiry blip, or a token the platform rejects at probe time — is **not** an exit-78 condition by default. That channel is isolated as degraded and the gateway keeps serving every healthy channel; see [Degraded Channel Isolation](/features/gateway-degraded-channels). Only a missing `token:` *key* or a structurally invalid config fails closed. Operators who explicitly opt into strict preflight (`--strict-preflight` / `gateway.preflight.strict: true`, [PR #4863](https://github.com/MervinPraison/PraisonAI/pull/4863)) get the old fail-fast behaviour back — any bad token aborts start with `typer.Exit(1)`. When no channel is left serviceable, preflight aborts even without strict mode (nothing to serve).
</Note>
...
graph TB
    ...
    WEDGE[Loop wedge] --> F75
    DEADLINE[Shutdown-phase deadline] --> F75

    classDef fatal fill:#EF4444,stroke:#7C90A0,color:#fff
    classDef temp fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef watchdog fill:#8B0000,stroke:#7C90A0,color:#fff

    class F78 fatal
    class F75 temp
    class WEDGE,DEADLINE watchdog
```
...
- Unresolved tool named in the config under the strict tool pre-flight (default `--strict-tools`) — `raise typer.Exit(78)`; opt out with `--no-strict-tools` or `strict_tools: false`
...
### Installed daemons honour these exit codes automatically

`praisonai-bot gateway install` (and `praisonai onboard`) generate a unit that already stops on exit `78`, so a broken `gateway.yaml` never crash-loops. Add `RestartPreventExitStatus=78` yourself only when you hand-write the unit.

<Tabs>
<Tab title="systemd (generated)">
`~/.config/systemd/user/praisonai-bot.service`:
...
`StartLimitIntervalSec` / `StartLimitBurst` also bound restart storms on transient (`75`) failures.
</Tab>

<Tab title="launchd (generated)">
...
launchd has no `RestartPreventExitStatus` equivalent — the exit-`78` no-crash-loop guarantee comes from `ThrottleInterval=30` plus the runtime's `RestartLoopGuard`, not from launchd stopping the service outright.
</Tab>

<Tab title="Windows (generated)">
The `PraisonAIGateway` scheduled task runs a generated `.cmd` wrapper that translates `78` to a clean exit:
...
Windows Task Scheduler has no exit-code-specific stop either — the wrapper maps `78` to `0` so the task's re-run policy sees a clean exit.
</Tab>
</Tabs>

<Note>
The `python -m praisonai_bot gateway start` command now propagates the runtime's exit code via `typer.Exit`, so the installed units above see the real `78` instead of `0` on a fatal config error.
</Note>
...
<AccordionGroup>
<Accordion title="Pin RestartPreventExitStatus=78 in systemd (hand-written units only)">
This applies only to units you write yourself — `praisonai-bot gateway install` and `praisonai onboard` already set it. Without it, a typo in `gateway.yaml` restarts a hand-written unit indefinitely, burning through CPU, log storage, and pager budget.
...
<Accordion title="Always set RestartPreventExitStatus=78 in hand-written systemd units">
The installed unit (`praisonai-bot gateway install` / `praisonai onboard`) already sets this. In a hand-written unit without it, systemd restarts a misconfigured gateway in a tight crash loop, filling logs and burning CPU. Exit 78 is the standard way to say "don't restart me".
</Accordion>
