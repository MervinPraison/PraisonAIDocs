...
Call `reset_default_resolver()` to clear the process-wide resolver cache.
...
for resetting cached state and is safe to call concurrently — the
ContextVar-based implementation handles isolation.
