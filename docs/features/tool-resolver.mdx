...
|--------|---------|---------|
| `resolve_all_from_yaml(yaml_config)` | `Dict[str, Callable]` | One call returns every tool referenced in a parsed YAML config. Additively merges (a) cwd `tools.py` **or** cwd `tools/` classes (mutual-XOR, unchanged) and (b) `.praisonai/tools/*.py` `@tool`-decorated functions keyed by their tool name — gated by `PRAISONAI_ALLOW_LOCAL_TOOLS=true`. See [#3107](https://github.com/MervinPraison/PraisonAI/issues/3107). |
