...
CLI, YAML, recipes, templates, and Python all share this resolution chain. See [CLI Reference](/cli/cli-reference) for `--tools` usage.
...
Tools appear under one of four source labels: `builtin` (`praisonaiagents.tools`), `local` (your `tools.py`), `external` (`praisonai-tools` package), or `registered` (wrapper `ToolRegistry` or core SDK entry-point plugin). See [CLI tools reference](/cli/tools) for the full label table and `--source` filter usage.
...
| Explicit resolver injection into `AgentsGenerator(tool_resolver=...)` | ❌ Not needed — you already own the instance |
...
resolver = _get_default_resolver()   # cached in a ContextVar
tools_dict = resolver.list_available()
```
...
| Caller | Behaviour |
|---|---|
| `BaseAutoGenerator._get_tool_resolver()` — used by `AutoGenerator`, `WorkflowAutoGenerator`, `JobWorkflowAutoGenerator` for the `--auto` planner prompt | Reuses the context-local resolver. |
| `AgentsGenerator.__init__(...)` (no `tool_resolver=` passed) | Reuses the context-local resolver. |
| `AgentsGenerator.__init__(..., tool_resolver=my_resolver)` | Uses your explicit resolver — for tests or multi-tenant runtimes. |
| Module-level helpers `resolve()`, `resolve_many()`, `list_available()`, `has_tool()`, `validate_yaml_tools()` | Already used the context-local resolver — unchanged. |
...
gen = AgentsGenerator(
    ...
    tool_resolver=my_resolver,   # ← new in PR #3067
)
```
...
The `resolve_tools()` helper used by recipes and templates has its own small memoisation layer on top of this — a bounded LRU keyed on the registry object so per-agent calls in a workflow build share one resolver. See [Tools Resolve → Resolver memoisation](/docs/cli/tools-resolve#resolver-memoisation).
...
```python
# Simple: accept every function or callable, including private names
funcs = resolver.load_functions_from_module("./helpers/my_tools.py")
# {"calculate_sum": <function>, "get_weather": <function>}

# CLI-style: only public plain functions (what --tools file.py uses)
funcs = resolver.load_functions_from_module(
    "./helpers/my_tools.py",
    functions_only=True,
    skip_private=True,
)
...
sequenceDiagram
    ...
    ToolResolver->>SafeLoader: cwd tools.py XOR tools/ classes
    SafeLoader-->>ToolResolver: classes (or empty)
    ToolResolver->>ToolResolver: gate: PRAISONAI_ALLOW_LOCAL_TOOLS?
    alt gate on
        ToolResolver->>Discovery: discover_project_tools() (.praisonai/tools/*.py)
        Discovery-->>ToolResolver: @tool callables keyed by tool name
    else gate off (default)
        Note over ToolResolver: short-circuit — no directory walk, no git subprocess
    end
    ToolResolver-->>User: {name: callable} dict
```

<Note>
**The `.praisonai/tools/` merge is additive and gated.** With `PRAISONAI_ALLOW_LOCAL_TOOLS=true`, `@tool` functions in `.praisonai/tools/*.py` are merged into the resolved dict keyed by tool name — so a `@tool(name="weather_lookup")` is referenceable from a YAML `tools:` list as `weather_lookup`. When the gate is off (default), the resolver skips the directory walk-up and `git rev-parse` subprocess entirely (perf guard from commit `12df267`). See [#3107](https://github.com/MervinPraison/PraisonAI/issues/3107).
</Note>

| Method | Returns | Purpose |
|--------|---------|---------|
| `resolve_all_from_yaml(yaml_config)` | `Dict[str, Callable]` | One call returns every tool referenced in a parsed YAML config. Additively merges (a) cwd `tools.py` **or** cwd `tools/` classes (mutual-XOR, unchanged) and (b) `.praisonai/tools/*.py` `@tool`-decorated functions keyed by their tool name — gated by `PRAISONAI_ALLOW_LOCAL_TOOLS=true`. See [#3107](https://github.com/MervinPraison/PraisonAI/issues/3107). Also invoked by `praisonai.scheduler.yaml_loader.create_agent_from_config`, so a scheduled `agents.yaml` resolves tools identically to `praisonai run` (fixed in [#3420](https://github.com/MervinPraison/PraisonAI/issues/3420)). |
| `load_functions_from_module(path, *, functions_only=False, skip_private=False, module_name="tools_module")` | `Dict[str, Callable]` | Load callables from a single `.py` file. Pass `functions_only=True` to keep only `inspect.isfunction` members (drops callable class instances). Pass `skip_private=True` to drop members whose name starts with `_`. `module_name` names the module inside `sys.modules` so multiple simultaneous loads (e.g. `--tools` + `--rewrite-tools`) don't shadow each other. |
...
### Already have a loaded module? Use the module-level helper

`load_functions_from_module(path, ...)` takes a **file path** and loads the module itself. If you have **already loaded** a module object, call the sibling module-level `extract_functions_from_loaded_module(module, ...)` instead — it runs the same extraction walk without re-loading (re-loading produces a distinct object and breaks `inspect.getmodule(obj) is my_module` origin filters).
...
tools = extract_functions_from_loaded_module(module, functions_only=True, skip_private=True)
```

Already loaded a module? Use the module-level helper. Only have a path? Use `ToolResolver.load_functions_from_module`. See [Module Tool Extraction](/features/module-tool-extraction) for the full guide.
...
The key used to reference a `.praisonai/tools/` callable differs between the two surfaces — this is the single most common source of "why doesn't my tool show up?" confusion.
...
The YAML resolver keys everything by tool name so YAML lists stay flat, so a `@tool(name="weather_lookup")` is referenceable as `weather_lookup`, **not** `weather.weather_lookup`. Fixed in [#3107](https://github.com/MervinPraison/PraisonAI/issues/3107).
...
Third-party packages can register `ToolSource` implementations via the `praisonai.tool_sources` entry-point group — `ToolResolver` appends them after the default 5-step chain. See [Tool Source Registry](/features/tool-source-registry) for the full pattern, plugin authoring, and opt-out.

<Note>
Publishing an **individual tool** (not a whole `ToolSource`) now uses the canonical `praisonai.tools` entry-point group — see [Tool Discovery Order](/features/tool-discovery-order#publishing-a-tool-package). The `praisonai.tool_sources` group described here remains the correct place to publish `ToolSource` implementations.
</Note>
...
<Card title="Tools Resolve (CLI)" icon="terminal" href="/cli/tools-resolve">
  Recipe and template tool resolution via resolve_tools()
</Card>
<Card title="Create Custom Tools" icon="wrench" href="/guides/tools/create-custom-tools">
