...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as CockroachDB Serverless
...
db = CockroachDB()
agent = Agent(name="CRDB Agent", memory={"db": db})
```
...
agent = Agent(name="High-Availability Agent", memory={"db": db})
```
...
agent = Agent(name="High-Contention Agent", memory={"db": db})
```
...
agent1 = Agent(name="Agent 1", memory={"db": db})
agent2 = Agent(name="Agent 2", memory={"db": db})
```
</Accordion>
...
<Card title="Multi-Region Deployment" icon="globe" href="/features/gateway">
