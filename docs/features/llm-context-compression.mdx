...
When `auxiliary_model` is omitted, the optimizer resolves it via `PRAISONAI_AUXILIARY_MODEL` → `OPENAI_MODEL_NAME` → `"gpt-4o-mini"` (see [Auxiliary model resolution](/docs/features/config-file#env-var-fallback-new-in-pr-4812)). This is what lets a fully-local setup point compaction at a small model without editing a config file.
