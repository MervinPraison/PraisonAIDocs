...
description: "How praisonaiagents, praisonai-code, praisonai-bot, praisonai-train, praisonai-browser, praisonai-mcp, and praisonai layer on top of each other"
icon: "layer-group"
---
...
<Step title="MCP host — serve agents to Claude Desktop / Cursor">
```bash
...
graph TB
    ...
    Q1 -->|MCP server for Claude<br/>Desktop / Cursor| P7["pip install praisonai-mcp[all]"]
...
| 2e | `praisonai-mcp` | Heavy MCP host, adapters, recipe bridge, HTTP-stream auth | `praisonai` PyPI (lazy `_wrapper_bridge`) |
| 2f | `praisonai-deploy` | API/Docker/cloud deploy, `deploy` CLI group, `Deploy` API (C14, publish slot #8) | `praisonai` PyPI (lazy `_wrapper_bridge`) |
| 3 | `praisonai` | Framework adapters (CrewAI/AutoGen), serve, dashboard, async jobs API | — |

**Publish order:** `praisonaiagents` → `praisonai-code` + `praisonai-bot` + `praisonai-train` + `praisonai-browser` + `praisonai-mcp` → `praisonai`
...
<Card title="praisonai-deploy Package" icon="rocket" href="/features/deploy/overview">
  Deploy-tier package guide
</Card>
