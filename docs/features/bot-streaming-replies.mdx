...
<Info>
Session-level `streaming: true` is a shortcut. For unified per-platform defaults and overrides, use [Display Policy](/features/display-policy).
</Info>
...
See [Channel Capabilities](/features/channel-capabilities) for the full matrix including reactions and typing.
...
graph TB
    subgraph "Mode Selection"
        ...
        A --> H[Mixed channels<br/>Not sure which edit]
...
| `auto` | Resolves to `draft` where the channel can edit, else `off`; other config fields are preserved | Multi-channel bots where you don't want to hard-code per-channel modes |

<Note>
Live streaming (`draft` mode) works on **Telegram, Slack, and Discord** — each adapter honours its own `edit_rate_limit` and `text_limit` from [Channel Capabilities](/features/channel-capabilities). **WhatsApp** and **Email** can't edit (`live_edit=False`): an explicit `draft`/`progress` mode degrades to `off` there, now logged at **WARNING** so the fallback is visible. Use [`auto`](#auto-mode) to opt into "stream where supported, otherwise single-message" without raising a warning. The simple `streaming: true` / `--stream` form enables `draft` mode automatically.
</Note>
...
from praisonai.bots import TelegramBot, WhatsAppBot, StreamingConfig, StreamingMode

agent = Agent(name="assistant", instructions="Be helpful and concise.")

# Same StreamingConfig works on both channels — Telegram streams live,
# WhatsApp delivers a single final message. No per-channel branching.
cfg = StreamingConfig(mode=StreamingMode.AUTO, min_interval=1.0)

telegram = TelegramBot(token="...", agent=agent, config=BotConfig(token="..."))
telegram.configure_streaming(cfg)

whatsapp = WhatsAppBot(token="...", agent=agent, config=BotConfig(token="..."))
whatsapp.configure_streaming(cfg)
```
...
```yaml
channels:
  telegram:                 # live_edit=True → resolves to draft
    token: ${TELEGRAM_BOT_TOKEN}
    streaming:
      mode: auto
      min_interval: 1.0
  whatsapp:                 # live_edit=False → resolves to off
...
On Telegram the user sees a placeholder that edits in place; on WhatsApp the same agent replies with one final message. Tail the bot log to confirm: `INFO  ... 'auto' resolved to 'draft'` for Telegram, `WARNING ... 'auto' resolved to 'off'` for WhatsApp.

```mermaid
graph TB
    A{Channel supports<br/>live edits?} --> B[Yes<br/>e.g. Telegram/Slack/Discord]
    A --> C[No<br/>e.g. WhatsApp/Email]
    A --> D[Mixed / not sure]

    B --> E[draft or progress]
    C --> F[off]
    D --> G[auto<br/>resolves per channel]
...
```mermaid
graph TB
    A["mode: auto"] --> B{can_edit?}
    B -->|True| C["→ draft<br/>INFO: resolved to 'draft'"]
    B -->|False| D["→ off<br/>WARNING: resolved to 'off'"]
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `mode` | `StreamingMode` | `StreamingMode.OFF` | Streaming mode: `OFF`, `DRAFT`, `PROGRESS`, or `AUTO` |
...
channels:
  telegram:
    token: ${TELEGRAM_BOT_TOKEN}
    streaming:
      mode: progress                # off | draft | progress | auto (see Auto mode)
...
<Accordion title="Why is my channel not streaming even though I set `streaming: true`?">
...
<Accordion title="How do I tell whether `auto` resolved to `draft` or `off`?">
...
<Card title="Streaming Tool Events" icon="wrench" href="/features/streaming-tool-events">
  Understand tool-event details used in progress mode
</Card>
<Card title="Progress Compositor" icon="layer-group" href="/features/streaming-progress-compositor">
  Fold typed StreamEvents into your own multi-line status view
</Card>
<Card title="Bot Gateway" icon="gateway" href="/features/bot-gateway">
