...
<Note>
Live streaming (`draft` mode) works on **Telegram, Slack, and Discord** — each adapter honours its own `edit_rate_limit` and `text_limit` from [Channel Capabilities](/docs/features/channel-capabilities). **WhatsApp** and **Email** can't edit (`live_edit=False`): an explicit `draft`/`progress` mode degrades to `off` there, now logged at **WARNING** so the fallback is visible. Use [`auto`](#auto-mode) to opt into "stream where supported, otherwise single-message" without raising a warning. The simple `streaming: true` / `--stream` form enables `draft` mode automatically.
</Note>
