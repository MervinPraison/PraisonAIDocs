...
Channel bots can post a quick placeholder message and progressively edit it in place as the agent's answer streams in — replacing the old "stare at a typing indicator for 45s, then a wall of text lands in one burst" experience. `streaming: true` is a shortcut into this lower-level API; for the unified per-platform form that sets streaming, tool progress, and footer together, see [Bot Display Policy](/docs/features/bot-display-policy).
