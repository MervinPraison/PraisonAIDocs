...
<Note>
As of [PR #2115](https://github.com/MervinPraison/PraisonAI/pull/2115), `AgentsGenerator()` no longer calls `register_builtin_autogen_adapters()` internally on construction. The method remains available — call it explicitly if you need builtin AutoGen adapters pre-populated in the registry.
</Note>
