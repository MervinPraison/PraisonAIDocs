...
<Note>
**Now supported (PR #2654):** YAML `tools:` under `framework: autogen` (v0.2) is fully wired via `register_for_llm` + `register_for_execution`. See [AutoGen v0.2 YAML Tools](/features/autogen-yaml-tools).
</Note>
...
| `create_autogen_tool_function(tool_class)` and the 17 `autogen_<Tool>` wrappers on `praisonai.inbuilt_tools.autogen_tools` | **Removed (PR #3583)** | No replacement needed — the sole consumer (`ToolRegistry.register_builtin_autogen_adapters()`) was removed in PR #1590. Use `AutoGenAdapter` from `praisonai.framework_adapters`. The module itself remains as a `praisonai_tools` presence probe. |

<Warning>
**Wrapper-side cleanup (PR #3583).** The wrapper functions on `praisonai.inbuilt_tools.autogen_tools` — `create_autogen_tool_function()` and the 17 `autogen_<Tool>` symbols (`autogen_CSVSearchTool`, `autogen_PDFSearchTool`, `autogen_RagTool`, `autogen_ScrapeWebsiteTool`, … — see the [PR diff](https://github.com/MervinPraison/PraisonAI/pull/3583/files) for the full list) — have been **removed**. They were unreachable dead code since PR #1590 removed their only consumer. Code that does `from praisonai.inbuilt_tools.autogen_tools import autogen_CSVSearchTool` now raises `ImportError`. Migrate the same way — use `AutoGenAdapter` from `praisonai.framework_adapters`.

The module `praisonai.inbuilt_tools.autogen_tools` **still exists** — it now only holds the `praisonai_tools` presence probe that feeds `PRAISONAI_TOOLS_AVAILABLE` and the release `import *` smoke test, so `import praisonai.inbuilt_tools.autogen_tools` continues to succeed.
</Warning>
...
PR #3067 also adds an optional `tool_resolver=` kwarg to `AgentsGenerator.__init__` (defaults to the shared context-local `ToolResolver`). See [Tool Resolver → Sharing one resolver across the run](/docs/features/tool-resolver#sharing-one-resolver-across-the-run).
</Warning>
...
<CardGroup cols={2}>
  <Card title="Framework Availability" icon="check-circle" href="/features/framework-availability">
