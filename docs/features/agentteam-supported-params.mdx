...
| `llm` | Team | Default LLM for members that never named one; an agent constructed with its own `llm=` / `model=` / `auth=` keeps it. Not related to `manager_llm`. See [Container Default LLM](/features/container-default-llm). |
| `name` | Team | Team name |
