...
<Note>
The TypeScript `AgentTeam` mirrors this — `knowledge`, `guardrails`, `web`, `reflection`, `caching`, and `learn` all emit the same "does not yet apply at the team level" notice and belong on individual `Agent(...)` instances. See [TypeScript AgentTeam](/docs/js/agent-team) for the JS equivalents.
</Note>
...
| `manager_llm` | Team | Hierarchical manager's model. Never propagated to member agents. |
| `model` | Team | Canonical name for the default model for members that never named one. `AgentTeam(model="gpt-4o-mini")` is now accepted. `llm=` is a deprecated alias — see [Model Parameter](/features/model-parameter). Not related to `manager_llm`. |
| `llm` | Team | Deprecated alias for `model=`. Default LLM for members that never named one; an agent constructed with its own `model=` / `llm=` / `auth=` keeps it. Passing both `llm=` and `model=` raises `TypeError`. Not related to `manager_llm`. See [Container Default LLM](/features/container-default-llm). |
| `name` | Team | Team name |
...
<Note>
`execution` is now louder than "silently does nothing". If you pass the single-agent `ExecutionConfig` to `AgentTeam(execution=...)`, the adapter carries over `max_iter` (and `max_retry_limit` when you changed it) and emits a `UserWarning` listing every field with no team-level counterpart — so you see exactly what was dropped rather than losing it silently. Pass `MultiAgentExecutionConfig` to stay silent. See [`ExecutionConfig` vs `MultiAgentExecutionConfig`](/features/multi-agent-execution#executionconfig-vs-multiagentexecutionconfig).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card icon="layer-group" href="/features/container-default-llm" title="Container Default LLM">
    How `llm=` fills in members that didn't pick a model.
  </Card>
</CardGroup>
