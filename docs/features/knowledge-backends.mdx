...
| *anything registered via `register_knowledge_adapter()`* | Custom adapter | See [Custom Knowledge Adapters](/features/knowledge-custom-adapters). |
...
Register before the first `Agent(...)` or `Knowledge(...)` call reads `.memory`. See [Custom Knowledge Adapters](/features/knowledge-custom-adapters) for the full protocol contract and registration API.
