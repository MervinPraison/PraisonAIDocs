...
<Warning>
`SqliteTranscriptStore` (this page) and `SqliteSessionStore` (see [Session Store](/features/session-store)) are **two different types** with two different jobs.
...
With `session.persist: true`, the gateway now builds `SqliteTranscriptStore` unless you opt out with `session.store: file`. And `session.persist` itself now defaults to `true` — see [Gateway Session Persistence](/features/gateway-session-persistence).
...
| Archived-history recall | `search()` scans the shared archived + active projection, so compacted turns stay searchable and hits carry the `archived: bool` flag — matching the other stores (Issue #5031). See [Cross-Session Recall](/features/cross-session-recall#recall-across-compacted-history). |
...
<Accordion title="Encrypt transcripts at rest">
This store writes message content to disk in plaintext. Wrap it in `EncryptedSessionStore` to encrypt content and metadata before they land on the filesystem — see [Encrypted Session Store](/features/encrypted-session-store).
</Accordion>
