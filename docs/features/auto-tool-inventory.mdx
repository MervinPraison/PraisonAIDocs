...
The `--auto` planner sees whatever the [`ToolResolver`](/features/tool-resolver) sees — install more tools and they show up automatically.
...
The planner prompt's `Available Tools:` line is filled from the canonical [`ToolResolver`](/features/tool-resolver), so it always matches what your agents can call at run time.
