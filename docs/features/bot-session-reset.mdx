...
<Note>
Session reset — idle-triggered, scheduled, or `/new` — also clears the per-user `/model` override. Once the session is reopened, the user's next turn runs on the agent's configured model, so a bad or unavailable model set via `/model` never survives a reset (PR [#4320](https://github.com/MervinPraison/PraisonAI/pull/4320)). See [`/model`](/features/bot-commands#model).
</Note>
...
Users can always send `/new` for an immediate reset. Manual reset works alongside automatic policies — see [Bot Commands](/features/bot-commands).
</Accordion>
