...
In `per_chat` (group session) mode, `/new` clears the **shared** group transcript — not just the sender's history. See [Group Chat Sessions](/docs/features/bot-group-sessions).
</Accordion>
