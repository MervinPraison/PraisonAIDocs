...
<Note>
The string form of `output=` must name a valid preset (`verbose`, `silent`, `stream`, … or a file path like `report.md`). A typo raises `ValueError` at construction time with a "Did you mean ...?" suggestion — see [Fail-Loud Defaults](/features/fail-loud-defaults). Matching is case-insensitive, whitespace-tolerant, and treats `-`/`_` interchangeably.
</Note>

```python
from praisonaiagents import Agent

# All spellings resolve to the 'verbose' preset (verbose=True, markdown=True):
for value in ("verbose", " verbose ", "VERBOSE"):
    agent = Agent(instructions="t", output=value)
    assert agent.verbose is True and agent.markdown is True
```

| Call (Python) | Before PR #4186 | After PR #4186 |
|---|---|---|
| `Agent(output="verbose")` | ✅ `verbose=True, markdown=True` | ✅ `verbose=True, markdown=True` |
| `Agent(output=" verbose ")` | ⚠ validated → `verbose=False, markdown=False` | ✅ `verbose=True, markdown=True` |
| `Agent(output="VERBOSE")` | ⚠ validated → `verbose=False, markdown=False` | ✅ `verbose=True, markdown=True` |

<Note>
Before PraisonAI PR #4186, hyphen / uppercase / padded variants were accepted by the validator but silently fell back to the silent defaults. Upgrade to a build that includes PR #4186 to get the behaviour shown above.
</Note>
...
## Concurrent execution (PR #5219)

`output="status"` and `output="trace"` route through a shared per-process singleton. As of [PR #5219](https://github.com/MervinPraison/PraisonAI/pull/5219) (fixes [#5162](https://github.com/MervinPraison/PraisonAI/issues/5162)), that singleton's `_output_lock` is actually acquired around every timing and token/cost update, so an `asyncio.gather` over several agents no longer corrupts the aggregate metrics.

<Note>
Safe under concurrency now, with one caveat: the fix removes the data races on the shared singleton, but `_tool_start_times` / `_agent_start_times` are still keyed by name only. Give each agent a distinct `name=` for accurate per-agent attribution. Full detail in [Thread Safety → Output singleton lock](/features/thread-safety#output-singleton-tracestatus-lock-pr-5219).
</Note>
