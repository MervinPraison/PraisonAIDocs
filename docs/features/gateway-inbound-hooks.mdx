...
hooks:
  idempotency:
    store_backend: sqlite      # "memory" (default) | "sqlite"
  hooks:
    - path: gmail
      agent: assistant
      # ...
...
|---------|-------------------|--------------------------|--------------|-------------|
| `memory` (default) | ❌ | ❌ | none | Single-replica dev / low-stakes deployments |
| `sqlite` | ✅ | ✅ (when the DB file is shared) | stdlib `sqlite3` only | Production, any provider with retries, multi-replica |
