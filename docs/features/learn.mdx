...
<Note>
The string form of `learn=` must name a valid preset (`disabled`, `agentic`, `propose`). A typo raises `ValueError` at construction time with a "Did you mean ...?" suggestion — see [Fail-Loud Defaults](/features/fail-loud-defaults). Matching is case-insensitive, whitespace-tolerant, and treats `-`/`_` interchangeably.
</Note>
...
const result = await agent.start('I prefer concise bullet points over long paragraphs');
console.log(result);
```
</Tab>
</Tabs>
...
<Warning>
**Fixed in PR [#4755](https://github.com/MervinPraison/PraisonAI/pull/4755):** the TypeScript learn manager never stripped the `sqlite:///` prefix from `db_url` (Python does, `manager.py:189-191`). A configured SQLite database was silently ignored and every write fell back to JSON files. After the fix a `db_url` such as `sqlite:///./data/learn.db` resolves to the real file path `./data/learn.db`.
</Warning>

<Note>
`AgentTeam(learn=…)` is not wired yet (PraisonAI [#4004](https://github.com/MervinPraison/PraisonAI/pull/4004)) — set `learn=…` on each `Agent(...)` in the team instead.
</Note>
...
<Card title="LearnConfig SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/classes/LearnConfig">
...
The `file` backend is built on `BaseJSONStore`, whose file lock now tolerates long critical sections. Concurrent `agent.remember(...)` and autosave paths across multiple agents or processes will not clobber each other's learnings even if a save legitimately takes longer than the lock timeout, thanks to the mtime + heartbeat staleness rule. See [Thread Safety](/features/thread-safety) for details ([PR #5136](https://github.com/MervinPraison/PraisonAI/pull/5136)).
