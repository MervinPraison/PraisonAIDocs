...
### Advertise honestly — the contract is verified at build time

A `supports_*` flag with no backing method is caught when the adapter is built, not mid-turn.

```python
# BROKEN — the flag is a lie; caught at Bot(...) build time, not mid-turn.
...
# FIXED — declaration is backed by a real implementation.
class MyBot(BasePlatformAdapter):
    capabilities = PlatformCapabilities(supports_edit=True)
...
`delete_message` is now **capability-gated for parity with `edit_message`**. When `supports_delete=False` the default returns `False` (unsupported). When an adapter declares `supports_delete=True`, the default raises `NotImplementedError` — but the capability contract catches this earlier, at build time. Set the flag `True` **only** when you provide a real `delete_message` implementation. See [Adapter Capability Contract](/features/bot-capability-contract).

---

## Control-trust posture

Set `allow_control=False` on any `BotMessage` your adapter emits that is not an interactive human turn — webhook bodies, relayed or mirrored bot-authored content, and proactive/wake/scheduler injects. The default (`True`) is only correct for a real user typing into a live chat. When `False`, `is_command` returns `False`, so an untrusted `/stop` or `/new` is handled as plain text instead of a privileged command. See [Control-Trust Provenance](/features/bot-control-trust).
...
Because a vanilla subclass already satisfies the protocol, hand the adapter itself to `identity_canonicalizer=` wherever the protocol is accepted. See [Identity Canonicalization](/features/gateway-identity-canonicalization) for the full wiring story.
...
| `status` | `SendStatus` (property) | _derived_ | Closed, branchable delivery outcome. Precedence: `queued` beats `duplicate` beats (`"sent"` if `ok` else `"failed"`). |
...
<Card title="BasePlatformAdapter SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/main">
...
`format_for_dialect(text, dialect)` returns `(rendered_text, parse_mode)`. See [How `markdown_dialect` is consumed](/docs/features/bot-platform-capabilities#how-markdown-dialect-is-consumed) for the full dialect table.
...
<Accordion title="Declare a capability only when you back it">
Setting `supports_edit=True` (or `supports_delete=True`) without overriding the corresponding method now raises `CapabilityContractError` when the adapter is built — not later, mid-turn. Declare only what your adapter actually implements; see [Adapter Capability Contract](/features/bot-capability-contract).
</Accordion>
</AccordionGroup>
...
<Card title="Adapter Capability Contract" icon="shield-check" href="/features/bot-capability-contract">
  Declared capabilities are verified at build time so unbacked flags fail loudly.
</Card>
