...
2. **Reply-to only on the first chunk.** In a multi-chunk reply, chunk #1 carries `reply_to`; chunks 2..N are ordinary messages.
...
| `message_ids` | `List[str]` | `[]` | All platform message ids produced (one per chunk). |
...
| `metadata` | `Dict[str, Any]` | `{}` | Additional platform-specific result details. |
