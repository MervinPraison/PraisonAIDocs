...
graph LR
    subgraph "Native Async Flow"
        ...
        C --> S[🌉 async_scoped_bridge]
        S --> D[🔧 FrameworkAdapter.arun]
    ...
    classDef bridge fill:#6366F1,color:#fff
...
Each `agenerate_crew_and_kickoff()` run opens an [`async_scoped_bridge()`](/features/async-bridge#async-scoped-bridge-async-safe-context-manager) around the adapter call, so concurrent tenants each get their own per-run bridge and a stuck coroutine in one cannot park the shared loop for the others.
...
sequenceDiagram
    ...
    Note over AgentsGenerator,FrameworkAdapter: async with async_scoped_bridge()<br/>(per-run isolated bridge)
...
As of [PR #3963](https://github.com/MervinPraison/PraisonAI/pull/3963), an **explicit** `tool_timeout` on `process: workflow` now **raises** on both paths rather than logging a warning — pick `sequential` or `hierarchical` if you need per-tool timeouts. See [tool_timeout on workflow YAML](/docs/features/yaml-workflows#tool_timeout-on-workflow-yaml).
</Info>
...
See [Advanced CLI options from Python](/developers/wrapper#advanced-cli-options-from-python) for the full alias table and precedence rules.
...
<CardGroup cols={2}>
<Card title="YAML Template Variables" icon="code" href="/features/yaml-template-variables">
