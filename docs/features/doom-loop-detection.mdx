...
Loop Detection is opt-in (`loop_detection=True`) and catches identical-argument / no-progress patterns. **Loop Guard** ([docs](/docs/features/loop-guard)) is always-on and counts per-turn tool calls with idempotent-vs-mutating thresholds. The two are complementary — Loop Guard is the always-on safety net, Loop Detection is the deeper pattern matcher you opt into for autonomous agents.
