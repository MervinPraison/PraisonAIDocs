...
<Note>
There are **two** loop-detection subsystems. This page covers the always-on, per-agent **result-aware tool-loop detector** (tool-call fingerprints). For the session-level **escalation** detector that records actions and progress markers and recommends a `RecoveryAction` — including the `NO_PROGRESS` recency-window fix — see [Escalation Doom-Loop Detection](/features/escalation-doom-loop).
</Note>
...
<Note>
Async parity added in PraisonAI PR [#3877](https://github.com/MervinPraison/PraisonAI/pull/3877): the async tool dispatch path used by `achat()` / `astart()` and middleware-driven async tool calls now records tool calls, checks all three detectors, and back-fills result hashes just like the sync path. Async poll-loops with identical output are now caught.
</Note>

<Note>
Async parity for the human-override approval on a `critical` verdict added in PraisonAI PR [#4858](https://github.com/MervinPraison/PraisonAI/pull/4858): the async tool dispatch path (`achat()` / `astart()`) now routes a critical verdict through the same `doom_loop` approval gate as sync. A `PermissionManager` rule (`doom_loop: allow`), YAML `permissions: { doom_loop: allow }`, `PRAISONAI_AUTO_APPROVE`, or an interactive-backend continue can now recover a false-positive stop on async runs — see [Approval → Doom-loop safety gate](/features/approval#doom-loop-safety-gate). Fail-closed: deny / timeout / no backend / any error still stops with `loop_blocked: True`.
</Note>
