...
Backends decide *how* to ask a human. A **store** decides *whether the pending request survives* a process restart. The two are orthogonal — wire any backend with or without a store.

```mermaid
graph LR
    subgraph "Backend vs Store"
        B["🔌 Backend\n(how to ask)"] --> P["📱 Slack / Telegram / Webhook"]
        S["💾 Store\n(whether it survives)"] --> DB["SQLite on disk"]
    end
    P --> D["👤 Human decision"]
    DB --> R["⚡ Rehydrate on restart"]
...
The `ApprovalStoreProtocol` (from `praisonaiagents.approval`) is the contract any backend must satisfy:
...
The shipped SQLite implementation is `ApprovalStore` from `praisonai.bots`:
...
print(f"Re-hydrated {rehydrated} pending approvals from last run")
```

See [Durable Approvals](/docs/features/durable-approvals) for the full how-to, configuration table, audit trail, and best practices.
