...
<Note>
The registry is the **declared** classification. Today the WebSocket dispatcher still enforces scopes per handler; `resolve_required_scope()` will be wired into a single dispatch wrapper in a follow-up. Classifying `hello` / `join` / `leave` / `abort` ([PraisonAI PR #4345](https://github.com/MervinPraison/PraisonAI/pull/4345)) is the prerequisite that makes that wiring safe — without it, wiring the resolver would demand `admin` for the connection handshake and lock out every client.
</Note>
...
Connection-lifecycle frames (`hello`, `join`, `leave`) run before a client has negotiated any scope, so they classify as `read` — the lowest meaningful level. `abort` mutates the turn just like sending one, so it carries the same `write` requirement as `message`.
...
|---|---|
| `hello`, `join`, `leave` | `read` |
| `agent.message`, `message`, `abort` | `write` |
