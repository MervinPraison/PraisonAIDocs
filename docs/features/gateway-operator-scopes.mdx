...
WebSocket frames go through the central `authorize_method` gate before any handler runs.

```mermaid
sequenceDiagram
    participant Client
    participant Dispatcher
    participant Registry as resolve_required_scope
    participant Handler

    Client->>Dispatcher: WS frame {type: "message", ...}
    Dispatcher->>Registry: authorize_method("message", scopes, params)
    alt scope satisfied
        Registry-->>Dispatcher: OK
        Dispatcher->>Handler: dispatch
        Handler-->>Client: result
    else GatewayUnauthorized
        Registry-->>Dispatcher: raise (method, required)
        Dispatcher-->>Client: {type: error, code: insufficient_scope, required_scope}
    end
```
...
<Note>
The `APPROVALS` scope is **coarse** — any holder may attempt to resolve any pending request. To bind a specific request to a specific operator inside that scope, use [per-request reviewer custody](/features/gateway-scoped-approvals#per-request-reviewer-custody).
</Note>

### Per-user attribution under a shared token

Operators sharing one gateway token collapse to a single audit identity by default. A proxy or SSO layer can stamp the `X-Operator-Id` header per user so each decision is still attributed to an individual on the audit trail.
...
The gateway stamps this as `operator:alice` on the audit trail. See [Approval Attribution](/features/gateway-approval-attribution#how-resolver-identity-is-captured) for the full identity derivation order.
...
| WebSocket `hello` / `join` / `leave` | — | `read` |
| WebSocket `message` | — | `write` |
| WebSocket `abort` | — | `write` |
...
<Note>
As of [PraisonAI PR #5217](https://github.com/MervinPraison/PraisonAI/pull/5217), the WebSocket dispatcher **calls the registry directly** — every inbound method goes through a central `authorize_method(method, client_scopes, params)` gate before any handler runs. `resolve_required_scope()` is the single source of truth. A plugin-registered or newly-added method is gated automatically; a method nobody classified requires `admin` by omission rather than being reachable ungated. See [Central authorization gate](#central-authorization-gate) below.
</Note>
...
|---|---|
| `hello`, `join`, `leave` | `read` |
| `agent.message` | `write` |
| `message` | `write` |
| `abort`, `message_abort` | `write` |
| `session.status` | `read` |
...
Connection-lifecycle frames (`hello`, `join`, `leave`) run before a client has negotiated any scope, so they classify as `read` — the lowest meaningful level. `abort` (and its wire alias `message_abort`) mutates the turn just like sending one, so both carry the same `write` requirement as `message`.
...
Every gateway method now flows through one default-deny gate before any handler runs.

Nothing changes for end users — the gateway gates dispatch for you.

```python
from praisonaiagents import Agent

agent = Agent(name="assistant", instructions="You are a helpful assistant.")
agent.start("hello")  # dispatch is gated automatically; nothing to configure
```

Plugin authors extending the dispatcher can call the gate directly.
...
try:
    ...
    print(exc.method, exc.required.value)
    # -> message write
```

- Called once, centrally, at the top of `_handle_client_message` — no per-handler wiring needed.
- Default-deny: an unclassified method requires `admin`.
- `admin` implies every scope.
- `read` is the baseline lifecycle scope — any operator holding at least one actionable scope (`write` / `approvals` / `pairing` / `admin`) satisfies `read`, so a `write`-only operator can still complete `hello` / `join` / `leave` / `session.status`. An empty scope set cannot satisfy even `read`.
- A malformed `method` (non-string / unhashable) fails closed to `admin` rather than raising `TypeError`.
- Transport `PING` / `PONG` frames bypass the gate (handled earlier in the wire loop).

<Note>
`GatewayUnauthorized` is a `PermissionError` subclass carrying `.method: str` and `.required: OperatorScope`. `str(exc)` renders as `method 'X' requires scope 'Y'`, and the dispatcher maps it to the `insufficient_scope` wire envelope with `required_scope` set.
</Note>
...
|---|---|
| `hello`, `join`, `leave` | `read` |
| `agent.message`, `message`, `abort`, `message_abort` | `write` |
...
| `authorize_method(method, client_scopes, params=None)` | Central default-deny gate. Resolves the required scope from `resolve_required_scope()` and raises `GatewayUnauthorized` when `client_scopes` do not satisfy it. Called by the WebSocket dispatcher for every non-ping frame |
| `GatewayUnauthorized` | `PermissionError` subclass raised by `authorize_method`. Carries `.method: str` and `.required: OperatorScope` so the dispatcher can render `insufficient_scope` with `required_scope` set |
...
For the approval-flow context these scopes gate, see [Scoped Approvals](/features/gateway-scoped-approvals).
...
<CardGroup cols={2}>
<Card title="Bind-Aware Auth" icon="shield" href="/features/gateway-bind-aware-auth">
...
<Card title="Scoped Approvals" icon="circle-check" href="/features/gateway-scoped-approvals">
