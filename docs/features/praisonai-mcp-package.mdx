...
See [The Three MCP Layers](/features/mcp-three-layers) for a decision diagram.
...
praisonai-mcp list-recipes                  # list discoverable recipes
...
| `list-recipes` | List discoverable recipes — `--source` accepts `custom`, `project`, `package` (see [Recipe → MCP Bridge](/features/mcp-recipe-server#list-recipes)) |
...
| `serve-recipe` | Serve a recipe as a scoped MCP server — see [Recipe → MCP Bridge](/features/mcp-recipe-server) |

The host also registers four memory tools — see [MCP Memory Tools](/features/mcp-memory-tools) — and four todo tools — see [MCP Todo Tools](/features/mcp-todo-tools).
...
<CardGroup cols={2}>
  ...
  <Card title="MCP Todo Tools" icon="list-check" href="/features/mcp-todo-tools">
