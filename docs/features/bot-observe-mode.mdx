...
| `mention_only` (default) | Unmentioned messages dropped, **no memory**. A **reply to the bot's own message** is admitted just like an `@mention`. | Safe default. Bot forgets everything until addressed. |
| `command_only` | Only `/commands` reach the agent. | Command-driven bots. |
| `observe` | Unmentioned messages stored as passive context. Agent only runs when @mentioned; the reply sees the full recent conversation. A **reply to the bot** is admitted like an `@mention`. | Group assistants (summarise, refer back, catch up). |
...
<Tip>
Reply-to-bot works on **both** branches: under `mention_only` and `observe`, tapping Reply on the bot's message admits the follow-up without re-typing `@bot`. See [Implicit Mentions](/features/bot-implicit-mentions) to enable quote/thread signals or turn this off.
</Tip>
