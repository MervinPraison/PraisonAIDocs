...
The pairing system intercepts messages from users not in `allowed_users` and routes them through an approval workflow controlled by the `unknown_user_policy`. On every successful approval (CLI, inline button, or web UI), a `pairing_approved` event is published on the default EventBus.
