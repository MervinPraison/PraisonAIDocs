...
<Step title="Multi-host deployments (optional)">
On a single host, callback signing works out of the box — the signing key is
the persisted gateway secret at `~/.praisonai/gateway/.gateway_secret`
(mode `0600`, auto-created on first use), so buttons survive restarts with no
operator action.

Set `PRAISONAI_CALLBACK_SECRET` to a shared value **only** when running the
gateway on multiple hosts/replicas so each verifies the same signatures:
...
<Note>
**Single-bot CLI (`praisonai bot start --config bot.yaml`)**: Since [PR #5111](https://github.com/MervinPraison/PraisonAI/pull/5111), the single-bot CLI honours `unknown_user_policy` and `owner_user_id` too — at **both** the top-level single-bot position and the channel-level position — for telegram / discord / slack. You no longer need to switch to `praisonai gateway start` just to enable pairing from YAML.

```yaml
# bot.yaml — pairing driven from the single-bot CLI
platform: telegram
token: ${TELEGRAM_BOT_TOKEN}
unknown_user_policy: pair
...
See [Standalone Bot YAML → Admission policy](/features/standalone-bot-yaml#admission-policy-unknown-user-policy-owner-user-id) for the resolved-policy startup banner and fail-closed validation on typos.
</Note>
...
<Note>
Since [PR #5112](https://github.com/MervinPraison/PraisonAI/pull/5112), when `unknown_user_policy=deny` drops an inbound DM the drop is no longer silent — a `WARNING` is logged with the sender's `user_id`, `platform`, and `conversation`. If you're using `pair` you won't see this WARNING (pairing runs before the deny path), but if you toggle back to `deny` you'll now be able to trace exactly which DMs are being rejected.
</Note>
...
<Info>
**Internals (PR #2108):** Pairing approve/deny now dispatches through the generic `InteractiveRegistry` under the reserved `pair` namespace. The behavior, callback format, and HMAC verification are identical — but you can now register your own handlers under different namespaces (e.g. `approval`, `menu`) on the same adapter without conflicting. See [Interactive Bot Actions](/features/interactive-bot-actions).
</Info>
...
- **Signature**: First 8 hex chars of `HMAC-SHA256(callback_secret, payload)`,
  where `callback_secret` is `PRAISONAI_CALLBACK_SECRET` when set, otherwise
  the persisted gateway secret from `~/.praisonai/gateway/.gateway_secret`.
- **Verification**: Tampered `callback_data` fails verification and is silently ignored + logged

<Note>
Callback and pairing-code signing share the same HMAC secret at
`~/.praisonai/gateway/.gateway_secret` (mode `0600`), auto-created on first
use. Inline buttons survive bot restarts by default — no operator action
required on a single host.

- **Single host, default install:** nothing to do — buttons and codes just work across restarts.
...
<AccordionGroup>
<Accordion title="Share the callback secret only across multiple hosts">
On a single host, inline buttons survive restarts by default — the signing key
is the persisted gateway secret at `~/.praisonai/gateway/.gateway_secret`
(mode `0600`, auto-created on first use). No environment variable is needed.

Set `PRAISONAI_CALLBACK_SECRET` to a shared value only when running the gateway
on multiple hosts/replicas so each verifies the same signatures:

```bash
# Generate and export the same value on every host
export PRAISONAI_CALLBACK_SECRET="$(openssl rand -hex 32)"
...
<Card title="Web-UI HTTP API Approval" icon="globe" href="/features/bot-pairing#web-ui-http-api-approval">
