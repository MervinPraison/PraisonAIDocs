...
```json
{
  "type": "hello_ok",
  "protocol": 1,
  "features": {"methods": ["message", "leave"], "events": ["message", "error", "token_stream", "tool_call_stream", "stream_end", "reasoning_stream", "tool_progress_stream", "stream_error"]},
...
|------------|-----------------|---------------------|
| (none) | `message`, `error` | — |
| `streaming` | `token_stream`, `tool_call_stream`, `stream_end`, `reasoning_stream`, `tool_progress_stream`, `stream_error` | — |
...
Events are advertised only if the client requested the matching capability. The six `streaming` events — including their payloads and the `accepted` vs `final` response frame — are detailed in [Gateway Stream Events](/features/gateway-stream-events).
