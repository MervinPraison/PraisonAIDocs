...
Events are advertised only if the client requested the matching capability. The six `streaming` events — including their payloads and the `accepted` vs `final` response frame — are detailed in [Gateway Stream Events](/docs/features/gateway-stream-events).
