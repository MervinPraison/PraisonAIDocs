...
Handshake frames (`hello`, `join`, `leave`) classify as **`read`** in the [Method → Scope Registry](/features/gateway-operator-scopes#method-scope-registry). They must run *before* a client has negotiated any scope, so requiring anything higher would make the handshake unreachable. Any operator token (viewer, ops, admin) can complete the handshake; scope only starts to matter for the frames that follow.
...
| `policy` | `Dict[str, Any]` | `max_payload`, `max_buffered_bytes`, `max_queued_frames`, `heartbeat_ms`. When attachments are enabled it also carries `max_attachment_bytes`, `max_attachments`, `chunk_bytes`, and `allowed_attachment_types` (added **only** when attachments are enabled) — see [Gateway Attachments](/features/gateway-attachments) |
...
```json
{
  "type": "hello_ok",
  "protocol": 1,
  "features": {"methods": ["message", "leave"], "events": ["message", "error", "token_stream", "tool_call_stream", "stream_end", "reasoning_stream", "tool_progress_stream", "model_fallback_stream", "retry_stream", "todo_stream", "tool_result_stream", "stream_error"]},
...
With attachments enabled, the `policy` gains the attachment ceilings a `/ws` client self-limits against:
...
When attachments are disabled, none of those keys are added and the `policy` is byte-for-byte the shape above — see [Gateway Attachments](/features/gateway-attachments) for the full story.
...
| `rate_limited` | Too many connection attempts — driven by an injectable [`RateLimitPolicyProtocol`](/features/gateway-rate-limit-policy) | `wait_then_retry` (+ `retry_after_seconds`) |
...
<Note>
The `rate_limited` code is driven by an injectable `RateLimitPolicyProtocol` — see [Gateway Rate Limit Policy](/features/gateway-rate-limit-policy) for `SlidingWindowRateLimitPolicy`, YAML `gateway.rate_limit`, and custom policies.
</Note>
...
|------------|-----------------|---------------------|
| (none) | `message`, `error` | — |
| `streaming` | `token_stream`, `tool_call_stream`, `stream_end`, `reasoning_stream`, `tool_progress_stream`, `model_fallback_stream`, `retry_stream`, `todo_stream`, `tool_result_stream`, `stream_error` | — |
...
Events are advertised only if the client requested the matching capability. The ten `streaming` events — including their payloads and the `accepted` vs `final` response frame — are detailed in [Gateway Stream Events](/features/gateway-stream-events).
...
client.supports_event("reasoning_stream")        # new streaming event
client.supports_event("tool_progress_stream")    # new streaming event
...
Clients should self-configure from the `policy` object in `hello_ok`. See [Gateway Flow Control](/features/gateway-flow-control) for tuning `max_buffered_bytes` and `max_queued_frames`.
...
<CardGroup cols={2}>
  <Card title="Gateway & Control Plane" icon="gateway" href="/gateway">
  ...
  <Card title="Gateway Attachments" icon="file-arrow-up" href="/features/gateway-attachments">
    The attachment ceilings advertised in the `hello_ok` policy
  </Card>
</CardGroup>
