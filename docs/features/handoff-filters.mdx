...
The user asks the coordinator; on handoff the input filters run in order and the filtered messages are seeded onto the target agent's `chat_history` for the duration of the handoff, then restored on exit.

```mermaid
sequenceDiagram
    participant Src as Source Agent
    participant Cfg as HandoffConfig
    participant F as input_filter chain
    participant Tgt as Target Agent

    Src->>Cfg: chat_history + kwargs
    Cfg->>Cfg: apply context_policy<br/>(FULL / LAST_N / SUMMARY / NONE)
    Cfg->>F: filtered messages
    F-->>Cfg: transformed messages
    Cfg->>Tgt: seed chat_history<br/>(scoped to this handoff)
    Tgt-->>Src: response
    Note over Tgt: chat_history restored on exit
```

<Note>
On releases **before PR #4213**, `input_filter` (and `context_policy`) had **no runtime effect** — the target only ever received the literal `[Handoff from …] <prompt>` string, so "what the target sees" examples above were aspirational. Filtered history now genuinely reaches the target's context.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Agent as Tool" icon="wrench" href="/features/agent-as-tool">
    Use agents as callable tools
  </Card>
  <Card title="Context Policies" icon="shield" href="/features/context-files">
    Control context sharing across agents
  </Card>
  <Card title="Handoff Tool Policy" icon="filter-circle-dollar" href="/features/handoff-tool-policy">
