...
<AccordionGroup>
  ...
  <Accordion title="Handle Errors Gracefully">
    ...
    When an iteration inside `loop(parallel=True)` raises, that iteration is replaced with `{"step": "loop_{idx}", "output": "Error: {e}"}` and the loop continues — the other iterations are not aborted. The error is logged as `Parallel loop iteration {idx} failed: {e}`. Add try/except inside your step function if you need per-item recovery beyond this default (e.g. to convert an error into a valid `StepResult` that downstream steps can consume normally).
  </Accordion>
</AccordionGroup>
