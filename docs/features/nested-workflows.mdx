...
As of PraisonAI [#4932](https://github.com/MervinPraison/PraisonAI/pull/4932), a nested `Loop(..., output_variable="x")` or `parallel(...)` inside a parallel branch now propagates its aggregate variable back to the workflow scope — previously that write was lost. As of [#4947](https://github.com/MervinPraison/PraisonAI/pull/4947), the loop **body's** `output_variable` writes now also propagate out of the loop (and, when the loop sits inside a `parallel()` branch, out of that branch too). See [Named outputs from branches](/features/workflow-parallel#named-outputs-from-branches) and [Named outputs from a loop body](/features/workflow-loop#named-outputs-from-a-loop-body).
...
As of PraisonAI [#3086](https://github.com/MervinPraison/PraisonAI/pull/3086), steps inside a nested pattern (`parallel`, `loop`, `route`, `if_`, `repeat`, `Discussion`) — and steps inside hierarchical mode — inherit the same policy machinery the top-level workflow uses:
...
As of PraisonAI [#4194](https://github.com/MervinPraison/PraisonAI/pull/4194), a failing step inside any nested pattern (`loop`, `parallel`, `when`/`if_`, `route`, `repeat`) propagates the same way it does at the top level: the run is marked `status="failed"` and the exception text is never handed forward as the next step's input.

```mermaid
flowchart TD
    Step[⚙️ Nested step raises] --> Check{on_error?}
    Check -->|stop default| Abort[🛑 Propagate stop → workflow aborts]
    Check -->|continue| Next[▶️ Next step runs]
    Abort --> Failed[❌ status = failed]
    Next --> Failed
...
As of PraisonAI [#4203](https://github.com/MervinPraison/PraisonAI/pull/4203), a nested `on_error` inside `parallel()` now behaves the same as inside `route()` / `loop()` / `repeat()` / `if_()` — a stopping branch halts the enclosing workflow even under `on_failure="partial_ok"`. See [Stop propagation from nested steps](/features/workflow-parallel#stop-propagation-from-nested-steps).

<Warning>
Runs that previously reported `completed` while containing a failed step now correctly report `failed`. See PR [#4194](https://github.com/MervinPraison/PraisonAI/pull/4194) — the old behaviour silently swallowed nested failures.
</Warning>
...
print(any(s["step"] == "after" for s in result["steps"]))   # False — stop propagated out
```
