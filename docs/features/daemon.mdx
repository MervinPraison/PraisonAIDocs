...
```bash
# First run — no daemon, normal cold-start (same as always)
praisonai run "summarize this"

# Boot once in the background
praisonai daemon start --background

# All subsequent runs skip cold-start automatically — same command, no flags
praisonai run "summarize this again"
praisonai run "now translate to French"

# Check what's running
praisonai daemon status

# Tear down when done (or let --idle-timeout do it)
