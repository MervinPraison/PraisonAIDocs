...
sequenceDiagram
    ...
    daemon-->>run: {ok: true}
    run->>daemon: POST /run {prompt, session_id?, event_id?}
    alt session_id present
        daemon->>daemon: _get_session_agent(session_id, model)<br/>rehydrate once, retain history
    else anonymous
        daemon->>daemon: _get_agent(model)<br/>per-model stateless, cleared post-call
    end
...
| **WarmRuntime** | Holds two agent caches: (1) one **stateless** warm `Agent` per model for anonymous (no-session) calls — history cleared after each call; (2) one **stateful** warm `Agent` per session id for `--continue`/`--session` runs — history rehydrated once and **retained** across turns, persisted per-turn through the project session store. Both caches evict on turn failure to prevent state bleed. |
...
## When `--continue` / `--session` Attach Warm

<Note>
As of PraisonAI 4.x ([PR #3465](https://github.com/MervinPraison/PraisonAI/pull/3465)), `praisonai run --continue` and `praisonai run --session <id>` **now attach to a warm, per-session, stateful agent** on the daemon — when one is running and no per-invocation tool/memory/approval overrides, no `--thinking`, and no `--worktree` are set. The agent is built once, its history is rehydrated once from the project session store, and it is **retained across turns** — so an iterative loop no longer pays cold-start each turn. Per-turn deltas are still persisted through the same session store the in-process path uses, so a crash or idle eviction resumes deterministically.

After upgrading, `--continue` just gets faster. Nothing else changes.
</Note>
...
| `--fork` | The forked session id is minted in-process; subsequent turns against the forked id attach warm. |
| `--thinking`, `--worktree` | Reasoning-effort and isolated git-worktree runs need in-process state. |
...
<Card title="MCP Integration" icon="plug" href="/mcp/mcp-server">
