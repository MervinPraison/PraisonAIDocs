...
<Note>
`PraisonAIAgents` here is a silent alias for `AgentTeam`, the canonical multi-agent class. `AgentManager` and `Agents` are aliases too — see [Back-compat class aliases](/features/migrate#back-compat-class-aliases).
</Note>
...
- maps `max_retry_limit` → `max_retries` **only when you changed it** (otherwise the team keeps its own default of `5`, since `ExecutionConfig.max_retry_limit` defaults to `2`),
...
<Tabs>
<Tab title="Right — team-level config">
```python
from praisonaiagents import Agent, Task, PraisonAIAgents, ExecutionConfig, MultiAgentExecutionConfig

# Per-agent knobs go on each Agent
agent = Agent(
    name="Researcher",
    instructions="Research topics thoroughly.",
    execution=ExecutionConfig(code_execution=True, max_rpm=60),
)
task = Task(description="Research AI history", agent=agent)

# Orchestration knobs go on MultiAgentExecutionConfig — no warning
workflow = PraisonAIAgents(
    agents=[agent],
    tasks=[task],
    execution=MultiAgentExecutionConfig(max_iter=20, max_retries=3),
...
<Tab title="Wrong — single-agent config">
```python
...
# UserWarning: max_iter carried over; code_execution, max_rpm, ... ignored.
# Use MultiAgentExecutionConfig for team-level settings.
workflow = PraisonAIAgents(
    agents=[agent],
    tasks=[task],
    execution=ExecutionConfig(max_iter=20, code_execution=True, max_rpm=60),
)
workflow.start()
```
</Tab>
...
<Tabs>
<Tab title="Wrong shape (single-agent config)">
```python
from praisonaiagents import Agent, Task, AgentTeam, ExecutionConfig

agent = Agent(name="Researcher", instructions="Research thoroughly.")
task = Task(description="Research AI history", agent=agent)

# UserWarning: pass MultiAgentExecutionConfig; code_execution / max_rpm ignored
team = AgentTeam(
    agents=[agent],
    tasks=[task],
    execution=ExecutionConfig(max_iter=20, max_rpm=100, code_execution=True),
)
```
</Tab>
<Tab title="Right shape (team-level + per-agent)">
```python
from praisonaiagents import Agent, Task, AgentTeam, MultiAgentExecutionConfig, ExecutionConfig

# Per-agent knobs go on the Agent
agent = Agent(
    name="Researcher",
    instructions="Research thoroughly.",
    execution=ExecutionConfig(code_execution=True, max_rpm=100),
)
task = Task(description="Research AI history", agent=agent)

# Team-level orchestration knobs go on MultiAgentExecutionConfig
team = AgentTeam(
    agents=[agent],
    tasks=[task],
...
<Card title="Execution Systems" icon="cpu" href="/features/execution-systems">
  Single-agent execution configuration
</Card>
<Card title="Agent Retry" icon="rotate-cw" href="/features/agent-retry">
  Single-agent retry behavior
</Card>
<Card title="Spawn & Announce" icon="rocket" href="/features/spawn-announce">
  Reliable async sub-agent spawn — no hangs, safe sync/async mixing ([#4887](https://github.com/MervinPraison/PraisonAI/pull/4887))
</Card>
</CardGroup>
