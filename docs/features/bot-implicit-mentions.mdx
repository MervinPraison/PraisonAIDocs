...
<Note>
**Reply also carries context.** In addition to admitting the message under `mention_only`, the referenced message's text is folded into the agent turn as a compact quoted block — so "do the second one" resolves to the right option. See [Quoted Reply Context](/features/bot-quoted-reply-context).
</Note>
...
<Card title="Quoted Reply Context" icon="reply-all" href="/features/bot-quoted-reply-context">
  The same reply gesture also surfaces the referenced content to the agent
</Card>
</CardGroup>
