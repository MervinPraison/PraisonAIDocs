...
Once all of a child's parents reach `done` or `archived`, the dispatcher auto-promotes the child to `ready` on its next tick — no manual `kanban_move` needed. The DAG becomes self-driving.
...
sequenceDiagram
    ...
    Worker->>Store: kanban_complete("design")
    Dispatcher->>Store: recompute_ready() (next tick)
    Store-->>Dispatcher: ["backend", "frontend"]
    Dispatcher->>Store: status = ready (atomic)
    Dispatcher-->>ChildAgent: KANBAN_TASK_MOVED (to_status="ready")
    ChildAgent->>Store: claim "backend"
```

| Behavior | Rule |
|----------|------|
| Single parent done | Child promoted on next tick |
| Multi-parent fan-in | Child waits until **all** parents are terminal |
| Child linked after parent already done | Still promoted on next tick |
| Child currently `blocked` | Re-evaluated and promoted when parents complete |
| Parentless `todo` task | Left untouched — `todo` stays manual (backward compatible) |

<Note>
Auto-promotion fires `KANBAN_TASK_MOVED` per promoted task with `to_status='ready'`. Subscribe to that hook to react to newly-ready work.
</Note>
