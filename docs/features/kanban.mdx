...
sequenceDiagram
    ...
    alt Clean merge
        ...
        Dispatcher-->>Human: KANBAN_TASK_BLOCKED (files on comment thread)
...
- The `KANBAN_TASK_BLOCKED` hook fires (from the store's `blocked` transition); the conflicted files are recorded on the **task comment thread**, not on the hook payload. Read them via `kanban_show(task_id)`.
