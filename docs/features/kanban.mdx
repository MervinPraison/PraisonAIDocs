...
<Note>
One exception to auto-upgrade: if the `git rev-parse --is-inside-work-tree` probe times out (default 15 s), the task is created in the **shared** workspace with a `Timed out probing` warning in the gateway log, rather than blocking `create_task` indefinitely. See [Subprocess Guard](/features/gateway-subprocess-guard#user-visible-behaviour-change) for details.
</Note>
...
sequenceDiagram
    ...
    alt Clean merge
        ...
        Dispatcher->>Store: task → blocked + conflicted files comment
        Dispatcher-->>Human: KANBAN_TASK_BLOCKED
        Note over Human: read comment via kanban_show, resolve, re-ready
    end
```
...
| `PRAISONAI_KANBAN_GIT_TIMEOUT` | Total seconds allowed for any single git command the dispatcher runs (worktree add, merge, status, rev-parse). Timing out returns `returncode=124` and a diagnosis in stderr — the dispatcher treats that as failure exactly like any other non-zero exit. See [Subprocess Guard](/features/gateway-subprocess-guard). | `export PRAISONAI_KANBAN_GIT_TIMEOUT=600` |
...
- The task moves to `blocked` with a comment listing the conflicted files — read it via `kanban_show(task_id)`.
- The `KANBAN_TASK_BLOCKED` hook fires; the conflicted files live on the task comment thread, **not** the hook payload.
...
<CardGroup cols={2}>
<Card title="Kanban CLI" icon="terminal" href="/features/kanban-cli">
  CLI commands for viewing boards, tasks, and run history
</Card>

<Card title="Async Jobs" icon="clock" href="/features/async-jobs">
