---
title: "Kanban"
sidebarTitle: "Kanban"
description: "Persistent task board for multi-agent coordination"
icon: "kanban"
---

Kanban enables agents to coordinate through persistent tasks, creating a shared workspace where work is tracked and distributed across multiple agents.

```mermaid
graph LR
    subgraph "Kanban Task Flow"
        User[👤 User] --> Coordinator[🧠 Coordinator Agent]
        Coordinator --> Store[📋 Kanban Store]
        Store --> Board[📋 Board]
        Board --> Worker[⚡ Worker Agent]
    end
    
    classDef user fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef agent fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef store fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef result fill:#10B981,stroke:#7C90A0,color:#fff
    
    class User user
    class Coordinator,Worker agent
    class Store store
    class Results result

    classDef agent fill:#8B0000,color:#fff
    classDef tool fill:#189AB4,color:#fff
```

## Quick Start

<Steps>
<Step title="Create Agent with Kanban Tools">

```python
from praisonaiagents import Agent
from praisonaiagents.kanban import KanbanStoreProtocol

# Agent with kanban protocols (implementation needed from wrapper)
agent = Agent(name="Coordinator", instructions="Break tasks down")
result = agent.start("Create user auth system")
```

</Step>

<Step title="Add Worker Agent">

```python
from praisonaiagents import Agent
from praisonaiagents.kanban import VALID_KANBAN_STATUSES

worker = Agent(name="Worker", instructions="Claim and complete tasks")
result = worker.start("Find ready tasks")
```

</Step>
</Steps>

---

## Task dependencies & auto-promotion

Link tasks with parent edges using `kanban_link` — when all parents complete, the child task automatically becomes `ready` on the next dispatcher tick. No flag or manual call needed.

```mermaid
graph LR
    Parent1[✅ Parent 1] --> Child[⏳ Child task]
    Parent2[✅ Parent 2] --> Child
    Child -.auto-promote.-> Ready[🎯 Ready]

    classDef done fill:#10B981,stroke:#7C90A0,color:#fff
    classDef pending fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef ready fill:#189AB4,stroke:#7C90A0,color:#fff

    class Parent1,Parent2 done
    class Child pending
    class Ready ready
```

```bash
praisonai kanban create "Design auth flow" --status done   # parent 1
praisonai kanban create "Set up DB schema" --status done   # parent 2
praisonai kanban create "Implement auth API" --status todo  # child

praisonai kanban link design_task auth_task
praisonai kanban link db_task auth_task

# On the next dispatcher tick, auth_task moves to ready automatically.
```

<Note>
**Parentless `todo` tasks are intentionally left untouched.** Only tasks with at least one parent link participate in auto-promotion. `todo` remains a valid manual staging column for tasks you want to promote yourself.
</Note>

Each auto-promoted task fires a `KANBAN_TASK_MOVED` event. See `kanban_link` in the [Agent Tools](#agent-tools) section for how to create parent edges.

---

## How It Works

```mermaid
sequenceDiagram
    participant User
    participant Coordinator
    participant Store
    participant Dispatcher
    participant Worker
    participant UI
    
    User->>Coordinator: "Build login system"
    Coordinator->>Store: Create tasks (design, implement, test)
    Store->>Dispatcher: Dispatch ready tasks
    Dispatcher->>Worker: Claim task
    Worker->>Store: Add comments/heartbeat
    Worker->>Store: Mark completed
    Store->>UI: Update status
    UI->>User: Show progress
```

Task coordination happens through a SQLite-backed persistent store that all agents and the UI share.

| Component | Purpose |
|-----------|---------|
| **Kanban Store** | SQLite database storing tasks, comments, links |
| **Agent Tools** | 8 functions for task CRUD operations |
| **CLI Commands** | Human interface for task management |
| **Background Dispatcher** | Auto-claims `ready` tasks **and auto-promotes dependent tasks whose parents are all complete** |

---

## Task Status Flow

```mermaid
graph TB
    subgraph "Task Lifecycle"
        Triage[🔍 triage] --> Todo[📝 todo]
        Todo -->|"parents done<br/>(auto)"| Ready[🟢 ready]
        Todo -->|manual| Ready
        Ready --> Running[⚡ running]
        Running --> Review[👁️ review]
        Running --> Blocked[🚫 blocked]
        Blocked -->|"parents done<br/>(auto)"| Ready
        Review --> Done[✅ done]
        Done --> Archived[📦 archived]
    end
    
    classDef status fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef ready fill:#10B981,stroke:#7C90A0,color:#fff
    classDef done fill:#10B981,stroke:#7C90A0,color:#fff
    
    class Triage,Todo,Running,Review,Blocked status
    class Ready ready
    class Done,Archived done
```

## Reliability: claim leases & automatic reclamation

If a worker crashes mid-task, PraisonAI automatically retries by detecting the dead worker and putting the task back on the board. No manual intervention required.

```mermaid
sequenceDiagram
    participant D as Dispatcher
    participant W as Worker
    participant S as Store

    D->>S: claim_task(ttl=900s, pid=W.pid)
    S-->>D: task claimed (running)
    D->>W: spawn worker
    W->>S: heartbeat every ~60s
    Note over W: Worker crashes
    D->>S: reclaim_stale_claims() (next tick)
    Note over S: lease expired + PID dead
    S-->>D: task → ready
    D->>W: re-dispatch to new worker

    classDef dispatcher fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef worker fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef store fill:#189AB4,stroke:#7C90A0,color:#fff

    class D dispatcher
    class W worker
    class S store
```

**Env vars for tuning:**

| Variable | Default | What it controls |
|----------|---------|-----------------|
| `PRAISONAI_KANBAN_CLAIM_TTL` | `900` (15 min) | Claim lease duration in seconds. After this, the claim is reclaimable if the worker is dead or stale. |
| `PRAISONAI_KANBAN_STALE_TIMEOUT` | `1800` (30 min) | Heartbeat staleness threshold. A live worker whose heartbeat is older than this is treated as hung. |

For long-running tasks, increase `PRAISONAI_KANBAN_CLAIM_TTL` to match the expected task duration. A live worker can extend its own lease by heartbeating — the reclaim sweep leaves it untouched.

The `KANBAN_TASK_RECLAIMED` event fires when a stranded task returns to `ready`, so monitoring hooks can react. The SQLite store auto-migrates pre-existing databases — no manual action needed on upgrade.

### Advanced: implementing a custom store

Custom store implementations can add lease + heartbeat support by conforming to `KanbanReclaimProtocol` in `praisonaiagents.kanban`. This is an additive extension protocol — existing `KanbanStoreProtocol` implementations are not broken.

---

## Concepts

### Task Statuses

Tasks flow through 8 defined states from creation to completion:

```mermaid
graph TB
    subgraph "Status Flow"
        Triage[🔍 triage] --> Todo[📝 todo]
        Todo -->|"parents done<br/>(auto)"| Ready[🟢 ready]
        Todo -->|manual| Ready
        Ready --> Running[⚡ running]
        Running --> Review[👁️ review]
        Running --> Blocked[🚫 blocked]
        Blocked -->|"parents done<br/>(auto)"| Ready
        Review --> Done[✅ done]
        Done --> Archived[📦 archived]
    end
    
    classDef status fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef ready fill:#10B981,stroke:#7C90A0,color:#fff
    classDef done fill:#10B981,stroke:#7C90A0,color:#fff
    
    class Triage,Todo,Running,Review,Blocked status
    class Ready ready  
    class Done,Archived done
```

### Boards

Boards provide workspace isolation for different projects or contexts:

```mermaid
graph TB
    subgraph "Board Architecture"
        Default[🏠 Default Board] --> DB1[kanban.db]
        ProjectA[📁 Project A] --> DB2[project-a/kanban.db] 
        ProjectB[📁 Project B] --> DB3[project-b/kanban.db]
    end
    
    classDef board fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef storage fill:#6366F1,stroke:#7C90A0,color:#fff
    
    class Default,ProjectA,ProjectB board
    class DB1,DB2,DB3 storage
```

### DAG Links

Tasks form directed acyclic graphs through parent-child dependencies:

```mermaid
graph TB
    subgraph "Task Dependencies"
        Design[🎨 Design Task] --> Backend[⚙️ Backend Task]
        Design --> Frontend[🖥️ Frontend Task]
        Backend --> Integration[🔗 Integration]
        Frontend --> Integration
        Integration --> Testing[🧪 Testing]
    end
    
    classDef task fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef ready fill:#10B981,stroke:#7C90A0,color:#fff
    
    class Design,Backend,Frontend task
    class Integration,Testing ready
```

See [Dependency Auto-Promotion](#dependency-auto-promotion) — the dispatcher promotes children automatically once parents finish.

### Claim/Release

Workers coordinate through atomic claim and release operations:

```mermaid
sequenceDiagram
    participant Worker1
    participant Store
    participant Worker2
    
    Worker1->>Store: Claim ready task
    Store-->>Worker1: Task claimed (status: running)
    Worker2->>Store: Try claim same task
    Store-->>Worker2: Already claimed
    Worker1->>Store: Heartbeat signal
    Worker1->>Store: Release (complete/block)
    Store-->>Worker1: Task updated
```

---

## Dependency Auto-Promotion

When you link a child under one or more parent tasks, the dispatcher promotes the child from `todo`/`blocked` to `ready` as soon as every parent reaches `done` or `archived`.

### Quick Start

```python
from praisonaiagents import Agent

coordinator = Agent(
    name="Coordinator",
    instructions=(
        "Break the user request into linked kanban tasks. "
        "Use kanban_link(parent_id, child_id) for each dependency. "
        "Children stay in 'todo' until the dispatcher promotes them."
    ),
)
coordinator.start("Build a login system: design → backend → frontend → tests")
```

### How Auto-Promotion Works

```mermaid
sequenceDiagram
    participant Agent as 🧠 Agent
    participant Store as 📋 Store
    participant Dispatcher as ⚙️ Dispatcher
    participant Worker as ⚡ Worker

    Agent->>Store: Create child task (status: todo)
    Agent->>Store: kanban_link(parent_id, child_id)
    Note over Store: Child waits while parent is running
    Agent->>Store: Complete parent task (status: done)
    Dispatcher->>Store: Tick — calls recompute_ready()
    Store-->>Dispatcher: Child promoted to ready
    Dispatcher->>Worker: Fire KANBAN_TASK_MOVED event
    Worker->>Store: Claim child task

    classDef agent fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef store fill:#189AB4,stroke:#7C90A0,color:#fff
    classDef dispatcher fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef worker fill:#10B981,stroke:#7C90A0,color:#fff
```

### Behaviour by Scenario

| Scenario | Result |
|---|---|
| Child linked, parent still `running` | Child stays in `todo`/`blocked` |
| Last parent moves to `done` | Child auto-promoted to `ready` on next tick |
| Child has multiple parents, only some done | Child waits for **all** parents |
| Child linked *after* parent already `done` | Promoted on the next tick |
| Parent moves to `archived` | Counts as terminal — promotes child |
| `todo` task with no parents | **Not** auto-promoted (backward compatible) |
| `blocked` child whose parents finish | Re-evaluated and promoted to `ready` |

### Does My Task Auto-Promote?

```mermaid
graph TB
    Start([Task created]) --> HasParents{Has parent links?}
    HasParents -- No --> Manual[Stays in todo\nmanual promotion]
    HasParents -- Yes --> AllDone{All parents done\nor archived?}
    AllDone -- No --> Wait[Stays in todo/blocked\nwaiting for parents]
    AllDone -- Yes --> Promote[✅ Auto-promoted to ready\non next dispatcher tick]

    classDef start fill:#6366F1,stroke:#7C90A0,color:#fff
    classDef decision fill:#F59E0B,stroke:#7C90A0,color:#fff
    classDef wait fill:#8B0000,stroke:#7C90A0,color:#fff
    classDef ready fill:#10B981,stroke:#7C90A0,color:#fff
    classDef manual fill:#189AB4,stroke:#7C90A0,color:#fff

    class Start start
    class HasParents,AllDone decision
    class Wait wait
    class Promote ready
    class Manual manual
```

### Observability — Subscribe to Promotions

```python
from praisonaiagents.hooks import HookRegistry, HookEvent, HookResult

registry = HookRegistry()

@registry.on(HookEvent.KANBAN_TASK_MOVED)
def on_promotion(event_data):
    if event_data["to_status"] == "ready":
        print(f"Task {event_data['task_id']} auto-promoted to ready")
    return HookResult.allow()
```

<Note>
Auto-promotion only runs while the dispatcher is active (`praisonai kanban dispatch` or the gateway dispatcher loop). Without a running dispatcher, children remain waiting until the next tick.
</Note>

---

## Agent Tools

Kanban tools are available through the SDK protocols. The wrapper implementation provides these agent tools:

### Task Management

| Tool | Purpose | Example |
|------|---------|---------|
| `kanban_create` | Create new task | `kanban_create("Implement auth", assignee="dev")` |
| `kanban_list` | Filter tasks | `kanban_list(status="ready", assignee="dev")` |
| `kanban_show` | Get task details | `kanban_show("task_abc123")` |

### Status Changes

| Tool | Purpose | Example |
|------|---------|---------|
| `kanban_complete` | Mark task done | `kanban_complete("task_abc123", "Auth working")` |
| `kanban_block` | Block with reason | `kanban_block("task_abc123", "Need API keys")` |

### Coordination

| Tool | Purpose | Example |
|------|---------|---------|
| `kanban_comment` | Add progress note | `kanban_comment("task_abc123", "50% complete")` |
| `kanban_link` | Create dependency | `kanban_link("design_task", "implement_task")` |
| `kanban_heartbeat` | Signal liveness | `kanban_heartbeat("task_abc123", "testing")` |

Once all parents of a linked child are `done` or `archived`, the child auto-promotes to `ready` on the next dispatcher tick — see [Auto-Promotion](#auto-promotion).

---

## Boards & Storage

### Single Board (Default)
```
~/.praisonai/kanban.db
```

### Multi-Board Layout
```
~/.praisonai/kanban/boards/
├── project-a/kanban.db
├── project-b/kanban.db
└── team-x/kanban.db
```

### Environment Configuration

| Variable | Effect | Example |
|----------|--------|---------|
| `PRAISONAI_KANBAN_BOARD` | Select active board | `export PRAISONAI_KANBAN_BOARD=project-a` |
| `PRAISONAI_KANBAN_DB` | Override DB path | `export PRAISONAI_KANBAN_DB=/custom/path.db` |

---

## Common Patterns

### Coordinator-Worker Pattern

```python
# Coordinator breaks down requests
from praisonaiagents import Agent
from praisonaiagents.kanban import VALID_KANBAN_STATUSES

coordinator = Agent(
    name="Coordinator", 
    instructions="Break user requests into kanban tasks"
)

# Worker with heartbeat reporting
worker = Agent(
    name="Worker",
    instructions="Claim ready tasks, report progress via heartbeat"
)
```

### Self-Driving Pipeline

Link tasks once and let the dispatcher handle promotion automatically:

```python
from praisonaiagents import Agent

coordinator = Agent(
    name="Coordinator",
    instructions="""
    Break the user request into linked tasks:
      1. kanban_create(...) for each step
      2. kanban_link(parent_id, child_id) to wire dependencies
    Then exit. The dispatcher will auto-promote children as parents finish.
    """,
)

coordinator.start("Ship a login feature: design → backend → frontend → test")
```

### Background Processing

```python
# Background processing requires wrapper implementation
# The praisonaiagents.kanban protocols support:
# - Task claiming and status updates
# - Heartbeat reporting for long-running tasks
# - Multi-board coordination
```

### Worker with Heartbeat

```python
from praisonaiagents import Agent
from praisonaiagents.kanban import VALID_KANBAN_STATUSES

worker = Agent(
    name="Worker",
    instructions="""Claim ready tasks and report liveness via heartbeat. 
    Use kanban_heartbeat every 30 seconds during long-running work."""
)

# Worker claims task and reports progress
result = worker.start("Find ready tasks, claim one, and report progress")
```

### Human-Agent Collaboration

```bash
# Human-agent collaboration pattern
# Requires wrapper implementation of:
# - CLI commands for task management
# - UI board visualization
# - Agent-to-store protocol bindings
```

---

## Best Practices

<AccordionGroup>
<Accordion title="Task Granularity">
Create tasks that can be completed in 15-30 minutes. Break larger work into linked subtasks using `kanban_link` for proper dependency tracking.
</Accordion>

<Accordion title="Status Management">
Move tasks through statuses systematically: `todo` → `ready` → `running` → `done`. Use `blocked` for dependencies and `review` for human approval.
</Accordion>

<Accordion title="Agent Coordination">
Use `kanban_heartbeat` during long-running tasks to signal liveness. Add detailed comments with `kanban_comment` to track progress and decisions.
</Accordion>

<Accordion title="Board Organization">
Use separate boards for different projects or contexts. Default board works well for single-project setups.
</Accordion>
</AccordionGroup>

---

## Related

<CardGroup cols={2}>
<Card title="Async Jobs" icon="clock" href="/docs/features/async-jobs">
  Asynchronous job processing and queuing
</Card>

<Card title="Background Tasks" icon="clock" href="/docs/features/background-tasks">
  Async job processing and scheduling
</Card>

<Card title="CLI Dispatcher" icon="terminal" href="/docs/features/cli-dispatcher">
  Command-line task orchestration
</Card>
</CardGroup>