...
<Warning>
**Behaviour change (PR #4477):** heterogeneous per-agent `tool_timeout` values are now honoured **per agent** instead of collapsing to the tightest value. If you set differing values across agents deliberately (e.g. 5s on a fast search agent and 60s on a slower research agent), each agent now keeps its own budget. A slow agent that previously inherited a fast agent's tighter budget (masking a real timeout) may now run longer. Uniform declarations are unaffected. (Earlier, PR #3176 had reversed the collapse from `max` to `min`.)
</Warning>
