...
| `cli_backend` | string / dict | **Deprecated — use `runtime` instead.** CLI backend (e.g. `claude-code`) or `{id: ..., overrides: {...}}`. Requires `framework: praisonai`. See [CLI Backend Protocol](/docs/features/cli-backend-protocol). |
