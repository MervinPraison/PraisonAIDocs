...
<Info>
New `agents.yaml` files scaffolded by `praisonai --init` include a `# yaml-language-server: $schema=…` header on line 1, so any editor with a YAML language server (VS Code + Red Hat YAML, JetBrains, Neovim + `yaml-language-server`) gives you autocomplete and inline validation immediately. See [Editor Autocomplete](/features/editor-autocomplete) to wire it into an existing file.
</Info>

<Tip>
Want a minimal, runnable sequential team? Copy [`examples/yaml/teams/research-writer/`](https://github.com/MervinPraison/PraisonAI/tree/main/examples/yaml/teams/research-writer) — a two-agent (`researcher` → `writer`) YAML you run with `praisonai agents.yaml`.
</Tip>
...
Duplicate agent/task names and unknown task→agent references are also caught during this normalisation on every `praisonai start` — both are warned about (and preserved with a `__dup_i` suffix) by default, or raise under `PRAISONAI_VALIDATE_STRICT=true`. See [Runtime YAML normalisation](/docs/cli/validate#runtime-yaml-normalisation-normalize-yaml-config) for the full rules.
</Note>

---

## Empty `roles: {}` Is Treated as Missing

A YAML that carries an empty `roles: {}` alongside a populated `agents:` block (dict or list) and a list-form `tasks:` promotes the agents into roles and grafts the tasks — it does **not** run with zero agents.

This YAML now yields one role per agent with its tasks attached:
...
agents:
  researcher:
    ...
    goal: Find the latest solar breakthroughs
    backstory: A materials-science PhD.
tasks:
  - name: summarise
    agent: researcher
    description: Summarise the top 3 breakthroughs of 2026.
    expected_output: A 3-bullet summary.
```

<Note>
`roles: {}`, `roles: null`, and a missing `roles:` all behave the same way: a populated `agents:` block is promoted into `roles:` and any list-form `tasks:` are grafted onto their agents. An empty `roles: {}` no longer shortcuts the promotion, so the run never silently ends with `Task completed.` and zero agents.

Debugging a "0 agents" run? See [YAML Validation](/features/yaml-validation).
</Note>
...
Tasks in a roles-file YAML run in the order they are declared under each role and, across roles, in the order the roles appear.
...
The loader walks `roles:` top-to-bottom, and each role's `tasks:` top-to-bottom, appending every task to a single run list in that order.

<Warning>
The optional top-level `dependencies:` block on a roles-file YAML is **documentary only** — the roles-file loader (`praisonai_adapter._build_agents_and_tasks`) does **not** consume it.

To reorder execution on the roles-file path, **reorder the tasks in the YAML**. Do not add `dependencies:` and expect it to change execution order.

`dependencies:` **is** honoured on the [workflow YAML path](/docs/features/yaml-workflows) (`process: workflow`); the divergence is intentional and long-standing.
</Warning>
...
roles:
  researcher:
    role: Research Analyst
    goal: Gather findings on the topic
    backstory: Expert researcher.
    tasks:
      research_task:
        description: "Research {{topic}}"
  ...
  writer:
    ...
    goal: Summarise the findings
    backstory: Clear, concise writer.
    tasks:
      summary_task:
        description: "Summarise the research findings"
        expected_output: "A short summary"

# documentary only — the roles-file loader does NOT read this block
dependencies:
  summary_task:
    - research_task
```
...
Copy-paste starter: [`examples/yaml/teams/research-writer/`](https://github.com/MervinPraison/PraisonAI/tree/main/examples/yaml/teams/research-writer).
...
See [CLI Reference](/cli/cli-reference) for the full flag surface of the positional form, and [`praisonai run`](/cli/run) for the per-prompt entry point.
...
The **editor** schema (used for [autocomplete](/features/editor-autocomplete)) does *not* require these three fields — the runtime auto-fills `role`/`goal` from the agent key and maps `instructions` → `backstory`. The **strict validator** behind `praisonai validate` still requires them, so run `validate` before shipping even if your editor shows no errors.
...
| `max_rpm` | int | Max requests per minute — auto-builds a `RateLimiter` when set; `max_rpm <= 0` raises `ValueError` |
...
| `tool_timeout` | int | Per-call tool execution timeout in seconds. Enforced at **two layers** when running `framework: praisonai`: (1) the wrapper wraps every tool callable in a timeout-enforcing shim that **raises `ToolTimeoutError`** (a `TimeoutError` subclass) on timeout — framework adapters translate it per framework — and (2) the SDK Agent's executor pool also honours the same value. Sync tools run in an instance-owned `ThreadPoolExecutor` on the wrapper layer; async tools use `asyncio.wait_for`. When agents declare different `tool_timeout` values, each agent's tools carry its own budget (per-agent wrap; shared tool objects). The shared-wrap fast path is used only when **every** agent declares `tool_timeout` **and** every declared value is identical. If any agent omits the field, the shared wrap is skipped and undeclared agents run with **no imposed timeout** (they do not inherit another agent's declared value). CLI `--tool-timeout` overrides all agents. See [Tool Timeouts](/features/tool-timeout) for the precedence table. YAML `bool` values (`tool_timeout: yes`) are ignored. See [Concurrency → Effective Timeout Precedence](/docs/features/concurrency#effective-timeout-precedence) and [Tool Configuration](/configuration/tool-config) for the full contract. In Python this maps to `tool_config=ToolConfig(timeout=…)` — the standalone `tool_timeout=` kwarg on `Agent(...)` was removed. On `process: workflow` YAMLs an **explicit** `tool_timeout` (per-agent/role, or `--tool-timeout` different from the default 60) now **raises `ValueError`** — [PR #3963](https://github.com/MervinPraison/PraisonAI/pull/3963). Use `process: sequential`/`hierarchical` if you need it enforced. When a tool call exceeds the timeout the executor pool is recycled — the next call gets a fresh worker so a hung tool cannot progressively degrade throughput. See [PR #3960](https://github.com/MervinPraison/PraisonAI/pull/3960). |
...
| `autonomy` | int (0-10) | Autonomy level. Mapped to core preset: 0=off, 1-3=suggest, 4-7=auto_edit, 8-10=full_auto. Also accepts `bool` / `str` preset / `dict` / `AutonomyConfig` (passed through unchanged). |
...
| `runtime` | string / dict | Per-agent runtime override. Requires `framework: praisonai`. See [Runtime Selection](/features/runtime-selection). |
| `cli_backend` | string / dict | **Deprecated** — use `runtime` or `models.<name>.runtime` instead. Still works through 2.0.0 with `DeprecationWarning`. Run `praisonai doctor fix --execute` (or `praisonai doctor runtime --fix --execute`) to migrate. See [Runtime Selection](/features/runtime-selection). |
| `reflection` | bool/object | Reflection configuration |

<Note>
**Framework note:** the agent-level fields `planning`, `reflection`, `guardrails`, `web`, `skills`, and `autonomy` are honored end-to-end on `framework: praisonai` as of the 2026-07-30 release ([PR #3517](https://github.com/MervinPraison/PraisonAI/pull/3517)). On older releases the wrapper accepted these keys in YAML but the native adapter silently dropped them — upgrade to the latest release for full parity.
</Note>

### Agent Behaviour Flags

Six per-agent keys map directly onto the matching `Agent(...)` kwargs. See [Agent Behaviour Flags in YAML](/features/yaml-agent-flags) for the full mapping and [Autonomy](/features/autonomy) for the level presets.

```yaml
roles:
  writer:
    role: "Writer"
    goal: "Draft and refine content"
    backstory: "Senior technical writer"
    planning: true          # enable planning
    reflection: true        # enable self-reflection
    guardrails: ["safe_output"]
    web: true               # enable web tools
    skills: ["research", "editing"]
    autonomy: 3             # -> suggest preset
```

The `autonomy` integer maps onto a core preset before it reaches `Agent(autonomy=…)`:

| YAML `autonomy:` value | Core `Agent(autonomy=…)` receives |
|---|---|
| `0` / omitted | off — nothing forwarded |
| `1` – `3` | `"suggest"` |
| `4` – `7` | `"auto_edit"` |
| `8` – `10` | `"full_auto"` |
| `bool` / `str` / `dict` / `AutonomyConfig` | passed through unchanged |
...
Deep dives: [Planning](/features/planning), [Reflection](/features/reflection), [Guardrails](/features/guardrails), [Web](/features/web), [Skills](/features/skills), [Autonomy](/features/autonomy).

<Warning>
**Behaviour change (PR #4477):** heterogeneous per-agent `tool_timeout` values are now honoured **per agent** instead of collapsing to the tightest value. If you set differing values across agents deliberately (e.g. 5s on a fast search agent and 60s on a slower research agent), each agent now keeps its own budget. A slow agent that previously inherited a fast agent's tighter budget (masking a real timeout) may now run longer. Uniform declarations are unaffected. (Earlier, PR #3176 had reversed the collapse from `max` to `min`.) PR #4468 additionally corrects a P1 edge case: a single declared `tool_timeout` on one agent no longer forces the other agents to inherit it via the shared-dict wrap.
</Warning>

<Warning>
**`tool_timeout` is not enforced on `process: workflow` — and an explicit value now fails fast.** Per-tool timeouts apply only to `process: sequential` / `process: hierarchical`. On `process: workflow`, an **explicit** per-agent/role `tool_timeout` (or a **changed** `--tool-timeout` CLI value) now **raises** rather than silently doing nothing — pick `sequential` or `hierarchical` if you need per-tool timeouts. Leaving `tool_timeout` off, or the bare `--tool-timeout` default in place, is fine.

Historical: PR [#3252](https://github.com/MervinPraison/PraisonAI/pull/3252) first added warning diagnostics; PR [#3963](https://github.com/MervinPraison/PraisonAI/pull/3963) upgraded them to fail-fast. See [tool_timeout on workflow YAML](/docs/features/yaml-workflows#tool_timeout-on-workflow-yaml).
</Warning>
...
Strict mode also **raises** on duplicate agent/task `name` keys and unknown task→agent references. Without strict mode both are surfaced as loud warnings, and the second entry is preserved with a `__dup_i` suffix instead of being silently collapsed (as of [PR #3176](https://github.com/MervinPraison/PraisonAI/pull/3176)).
...
<AccordionGroup>
  <Accordion title="Workflow Metadata" icon="info-circle">
    ...
    | `run_on` | string | `None` | A shared remote sandbox that every step's shell/file tools bind to — maps directly to the Python `AgentFlow(run_on=…)` kwarg. One of: `docker`, `e2b`, `modal`, `daytona`, `flyio`, `tenki`, `local`. Omit for default local execution. Validated at load time — a typo fails immediately with the list of valid names. See [Shared Sandbox](/features/shared-sandbox) and [Placement](/features/placement). |
    | `type` | string | `null` | Configuration type discriminator. Accepted at the top level and ignored by the runner; declared on the schema so it never warns as an unknown field. |
    | `deploy` | object | `null` | Deployment configuration block. See [Deploy](/features/deploy/overview). |
    | `dependencies` | object / array | `null` | Task dependency declarations. Honoured on the workflow YAML path (`process: workflow`); documentary-only on the roles-file loader (see [Task Ordering in Roles-File YAML](#task-ordering-in-roles-file-yaml)). |

    <Note>
    `type`, `deploy`, and `dependencies` are first-class schema fields as of [PraisonAI PR #4651](https://github.com/MervinPraison/PraisonAI/pull/4651). The validator derives its known-field list straight from the Pydantic schema, so these three no longer warn as "Unknown top-level field" — and any field added to the schema later teaches the validator with no second edit. See [YAML Validation](/features/yaml-validation#unknown-field-warnings).
    </Note>
  </Accordion>
  ...
  <Accordion title="Tool Retry Policy" icon="rotate">
    ...
    For detailed configuration options, see [Tool Retry Policy](/features/tool-retry-policy).
  </Accordion>
  ...
  <Accordion title="Runtime Selection" icon="play">
    Model-scoped runtime configuration. Requires `framework: praisonai`. See [Runtime Selection](/features/runtime-selection).
...
If `llm` is omitted, PraisonAI falls back to the `MODEL_NAME` environment variable, then to `openai/gpt-4o-mini`. This fallback is shared by every framework adapter.
  ...
  <Accordion title="Rate Limiting & Execution" icon="clock">
    ...
    |-------|---------|-------------|
    | `max_rpm` | Unlimited | Max requests per minute — auto-builds a `RateLimiter` when set; `max_rpm <= 0` raises `ValueError` |
...
See the [Validate CLI page](/cli/validate) for all flags, JSON output format, and CI integration examples.
