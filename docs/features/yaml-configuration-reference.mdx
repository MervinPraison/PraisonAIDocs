...
agent.start("Run the workflow defined in agents.yaml.")
```
...
| `tool_timeout` | int | YAML/CLI shorthand for `tool_config.timeout`. Maps to `ToolConfig(timeout=...)` in the direct-prompt path. In Python use `Agent(tool_config=ToolConfig(timeout=...))` instead — `Agent(tool_timeout=...)` is **rejected**. See [Tool Configuration](/docs/configuration/tool-config). |
| `tool_retry_policy` | object | YAML shorthand for `tool_config.retry_policy`. Maps to `ToolConfig(retry_policy=RetryPolicy(...))` in the direct-prompt path. In Python use `Agent(tool_config=ToolConfig(retry_policy=...))` instead — `Agent(tool_retry_policy=...)` is **rejected**. |
...
<AccordionGroup>
  <Accordion title="Workflow Metadata" icon="info-circle">
    ...
    | `framework` | string | "praisonai" | Framework: `praisonai`, `crewai`, `autogen`, `langgraph` (or any name registered via the `praisonai.framework_adapters` entry-point group) |
...
Both `llm` and `function_calling_llm` accept a model name as a **string**, or a **dict** when you also need to override `base_url` / `api_key`. Both shapes work identically across every supported framework (`praisonai`, `crewai`, `autogen`, `autogen_v4`, `langgraph`).
...
<Note>
**Framework LLM Configuration Parity:** The `llm` and `function_calling_llm` configuration shapes (string and dict forms) work identically across all supported frameworks (`praisonai`, `crewai`, `autogen`, `autogen_v4`, `langgraph`). You can switch between frameworks without changing your LLM configuration syntax.
</Note>
