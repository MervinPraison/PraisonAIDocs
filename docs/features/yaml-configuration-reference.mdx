...
<AccordionGroup>
  <Accordion title="Workflow Metadata" icon="info-circle">
    ...
    | `framework` | string | "praisonai" | Framework: `praisonai`, `crewai`, `autogen`, `langgraph`, `openai_agents` (other names work if a third-party adapter is registered via the `praisonai.framework_adapters` entry-point group) |
