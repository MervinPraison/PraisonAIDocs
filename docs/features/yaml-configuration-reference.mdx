...
| `tool_timeout` | int | Per-call tool execution timeout in seconds. Enforced at **two layers** when running `framework: praisonai`: (1) the wrapper wraps every tool callable in a timeout-enforcing shim that **raises `ToolTimeoutError`** (a `TimeoutError` subclass) on timeout — framework adapters translate it per framework — and (2) the SDK Agent's executor pool also honours the same value. Sync tools run in an instance-owned `ThreadPoolExecutor` on the wrapper layer; async tools use `asyncio.wait_for`. When multiple values are declared across roles/agents, the wrapper uses the **tightest (smallest)** value for the shared tool dict — a fast agent is never forced to wait for another agent's larger budget. Every overridden agent is warned in the logs. CLI `--tool-timeout` still overrides all. YAML `bool` values (`tool_timeout: yes`) are ignored. See [Concurrency → Effective Timeout Precedence](/docs/features/concurrency#effective-timeout-precedence) and [Tool Configuration](/configuration/tool-config) for the full contract. In Python this maps to `tool_config=ToolConfig(timeout=…)` — the standalone `tool_timeout=` kwarg on `Agent(...)` was removed. On `process: workflow` YAMLs an **explicit** `tool_timeout` (per-agent/role, or `--tool-timeout` different from the default 60) now **raises `ValueError`** — [PR #3963](https://github.com/MervinPraison/PraisonAI/pull/3963). Use `process: sequential`/`hierarchical` if you need it enforced. |
...
<Warning>
**`tool_timeout` is not enforced on `process: workflow` — the run now aborts with `ValueError` if you set it explicitly.** It applies only to `process: sequential` / `process: hierarchical`. On `process: workflow`, an explicit per-agent/role `tool_timeout:` or a non-default `--tool-timeout` value raises:
...
The bare CLI default (`--tool-timeout 60`) is ignored on this path so ordinary workflow runs work unchanged. Behaviour changed in [PR #3963](https://github.com/MervinPraison/PraisonAI/pull/3963); previously ([PR #3252](https://github.com/MervinPraison/PraisonAI/pull/3252)) it was warn-only. See [tool_timeout on workflow YAML](/docs/features/yaml-workflows#tool_timeout-on-workflow-yaml).
</Warning>
...
<AccordionGroup>
  <Accordion title="Workflow Metadata" icon="info-circle">
    ...
    | `run_on` | string | `None` | Provider name for a shared remote sandbox that every step's shell/file tools bind to. One of: `docker`, `e2b`, `modal`, `daytona`, `flyio`, `tenki`, `local`. Omit for default local execution. Validated at load time — a typo fails immediately with the list of valid names. See [Shared Sandbox](/features/shared-sandbox). |
  </Accordion>
