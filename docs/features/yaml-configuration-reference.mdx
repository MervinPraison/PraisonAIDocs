...
<Warning>
**`tool_timeout` is not enforced on `process: workflow`.** It applies only to `process: sequential` / `process: hierarchical`. On `process: workflow`, the value is validated but the warning is logged (verbatim) and the value has no effect:

```
tool_timeout=<value> is not enforced on the workflow (process: workflow) path;
per-tool timeouts apply to sequential/hierarchical runs only.
```

Fixed in [PR #3252](https://github.com/MervinPraison/PraisonAI/pull/3252). See [tool_timeout on workflow YAML](/docs/features/yaml-workflows#tool_timeout-on-workflow-yaml).
</Warning>
