...
Memory results are now deterministically ordered for prompt caching (no API change needed). For users assembling prompts manually, use `build_cache_optimized_context()` to get a stable prefix and cache boundary.
...
# ctx contains:
# - stable_prefix: deterministically ordered context string
# - cache_boundary: marker for separating cached from dynamic content
system_prompt = ctx["stable_prefix"] + ctx["cache_boundary"] + "Current user message..."
```
