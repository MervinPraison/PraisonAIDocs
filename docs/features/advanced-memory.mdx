...
| Any registered adapter (e.g. `dakera`) | Adapter itself | `memory_adapter.*` (all methods) |

The search, reset, and delete gates are now symmetric with store — no more sqlite-only special case. Any registered custom adapter routes `store_*`, `search_*`, `reset_*`, and `delete_*` through `memory_adapter.*`. See [Custom Memory Adapters](/docs/features/custom-memory-adapters#every-operation-routes-through-the-adapter).
