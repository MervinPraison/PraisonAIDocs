...
Use [Redis persistence](/features/persistence-redis) (`db(url=...)`) instead.
</Warning>
...
| `redis` / `postgres` / `valkey` | raises `ValueError` | use `db(url=...)` |
...
Pass a live store via `db(url=...)` — this path always worked and is unchanged:
...
agent = Agent(
    ...
    memory=MemoryConfig(db=db(url="redis://localhost:6379/0"), user_id="user_123"),
)
