...
Accessing anything exported through `praisonai` (for example `from praisonai import Agent`) points straight at a broken dependency instead of hiding it.

When the `praisonaiagents` package is installed but fails to import — typically a transitive dependency version conflict — the error now reads:

```
AttributeError: module 'praisonai' has no attribute 'Agent': praisonaiagents is installed but failed to import (<real ImportError message>)
```
...
<Card title="Doctor CLI Reference" icon="terminal" href="/cli/doctor-cli">
  CLI command reference for all doctor subcommands
</Card>
<Card title="Runtime Config Migration" icon="arrows-rotate" href="/features/doctor-runtime-migration">
