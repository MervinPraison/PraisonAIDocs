...
<Note>
See also: the [Model Harness](/features/model-harness) decides whether `edit_file` or `apply_patch` is advertised first, based on the agent's model family.
</Note>
...
| **apply_patch** | High | Recommended | Atomic multi-file Add/Update/Delete/Move with rollback; **per-path locks across the whole patch (both endpoints of a Move)**; staleness guard on Update, Delete **and Move**; `force=True` to override |
...
<Tip>
**Which primitive comes first?** For Claude vs. GPT models, the order in which `edit_file` and `apply_patch` are advertised is tuned automatically by the [Model Harness](/features/model-harness). Both stay available; unknown models keep the historic ordering.
</Tip>
...
# 2. Force an entire patch (overrides Update, Delete & Move staleness; per-file lock still applies)
...
`apply_patch` lets an agent Add, Update, Delete, and Move/rename multiple files in a single atomic call — all changes succeed together or none are committed.
...
graph TB
    ...
    A1 -->|Rename with or without edits| MV[apply_patch with *** Move File]
...
| `*** Move File: <old> -> <new>` | Optional `@@` hunks (`<old>\n===\n<new>`) | Rename a file (and optionally edit it in the same atomic op); errors if source missing or destination exists |

Optional sentinels `*** Begin Patch` / `*** End Patch` are accepted and stripped.

The `Move File` split is on the literal `" -> "` (space on both sides), so paths containing `->` without surrounding spaces are unaffected. Both source and destination must be non-empty and resolve to different paths.
...
Rename only — the header stands on its own line:

```
*** Move File: src/user.py -> src/account.py
```

Rename and edit in one atomic op — add one or more `@@` hunks after the header:
...
Move hunks run through the same fuzzy ladder as `Update`, and BOM/CRLF are preserved from the source.

<Steps>
<Step title="Agent rename-and-edit in one op">
...
agent = Agent(
    name="Refactor Agent",
    instructions="Rename files with apply_patch's Move File; combine with @@ hunks to edit in the same op.",
    tools=["read_file", "apply_patch", "edit_file"],
)

agent.start("Rename src/user.py to src/account.py and swap `class User` for `class Account`.")
```
...
result = apply_patch("""*** Move File: src/user.py -> src/account.py
@@
class User:
===
class Account:
""")
print(result)  # "Renamed src/user.py -> src/account.py\n<diff>"
```
...
| Move is atomic | The destination is written first, then the source renamed aside — either both land or neither does, and the source is restored on any later failure |
| Move preserves BOM / line endings | Same as Update — UTF-8 BOM and CRLF/LF are carried over from the source |
| Cross-op destination conflicts fail up front | Two operations producing the same destination are rejected in validate; nothing is committed |
| Staging never clobbers a sibling | A pre-existing file at the default `.praison_patch_tmp` staging name is stepped around instead of truncated or deleted |
...
sequenceDiagram
    ...
    AP->>FS: Write destination (staged temp → os.replace)
    AP->>FS: Rename source aside (backup)
    alt Both steps succeed
        FS-->>AP: ✅ Renamed src -> dst
    else Later failure
        AP->>FS: Drop destination
        AP->>FS: Restore source + re-arm staleness guard
        FS-->>AP: Rolled back
    end
```
...
| Malformed `Move File` header (no `-> `) | `Error: Invalid patch: Malformed Move File header (expected '<old> -> <new>'): '<payload>'` |
| Empty source or destination in `Move File` header | `Error: Invalid patch: Malformed Move File header (empty source or destination): '<payload>'` |
| Move source and destination are the same | `Error: Cannot move '<src>' to '<dst>': source and destination are the same path` |
| Move source missing | `Error: Cannot move '<src>': file not found` |
| Move destination already exists | `Error: Cannot move '<src>' to '<dst>': destination already exists` |
| Empty hunk old-block in a Move | `Error: Empty hunk in move for '<src>'` |
| Hunk not found in a Move | `Error: Hunk not found in '<src>': '<preview>'` |
| Ambiguous hunk in a Move | `Error: Ambiguous hunk in '<src>': '<preview>' matches N locations` |
| Cross-op conflict on same destination | `Error: Conflicting operations target the same path '<dst>' (also produced by '<earlier op display>')` |
| Automatic staleness — Move | `Error: Cannot move '<src>': file changed since it was read - re-read before editing (or pass force=True to override).` |
| Undecodable source on Move | `Error: Cannot move '<src>': <decode error>` |
| Move success line | `Renamed <src> -> <dst>\n<diff><diagnostics>` |
...
| `apply_patch` | `patch: str`, `force=False` | `str` | High-risk; atomic multi-file Add/Update/Delete/Move with rollback; **per-path locks (both endpoints of a Move) + staleness guard on Update, Delete and Move** |
...
| `root` | `Optional[str]` | `None` (current working directory at construction time) | Base directory that confines all path resolution when no `workspace=` is supplied. Any path resolving outside this base is rejected with `Path traversal detected: <input> escapes workspace <root>`. Pinned at construction so a later `os.chdir()` never moves the boundary. Ignored when `workspace=` is set. |
...
# Pin the fallback base to a specific project directory.
edit_tools = create_edit_tools(root="/srv/projects/demo")

# Reads inside the root work; anything outside is rejected.
content, meta = edit_tools.read_file("/srv/projects/demo/README.md")   # ok
content, meta = edit_tools.read_file("/etc/passwd")                    # Path traversal detected
```
...
Move the file and fix its importer in one lossless, atomic op — no need to restate the file body:

```python
from praisonaiagents.tools.edit_tools import apply_patch

result = apply_patch("""*** Begin Patch
*** Move File: src/auth.py -> src/account.py
@@
class UserService:
===
class AccountService:
...
Without `Move File`, the same rename needs a `Delete + Add` pair that restates the whole file body — heavier and not lossless.
...
<Accordion title="Prefer Move File over Delete+Add for renames">
`*** Move File: <old> -> <new>` is atomic, race-safe on both endpoints, and preserves BOM/CRLF — without re-sending the file body. Combine it with `@@` hunks to rename and edit in one operation, instead of a heavier `Delete + Add` pair that restates the whole file.
</Accordion>
...
<CardGroup cols={2}>
<Card title="Workspace" icon="folder-lock" href="/features/workspace">
  How workspace containment secures file operations — the per-file lock complements workspace boundaries
</Card>
<Card title="Bot Default Tools" icon="toolbox" href="/features/bot-default-tools">
