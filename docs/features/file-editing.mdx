...
<Note>
See also: the [Model Harness](/docs/features/model-harness) decides whether `edit_file` or `apply_patch` is advertised first, based on the agent's model family.
</Note>
...
<Tip>
**Which primitive comes first?** For Claude vs. GPT models, the order in which `edit_file` and `apply_patch` are advertised is tuned automatically by the [Model Harness](/docs/features/model-harness). Both stay available; unknown models keep the historic ordering.
</Tip>
