...
The env-only client also **rebuilds** when `OPENAI_API_KEY` or `OPENAI_BASE_URL` changes between calls. A rotated key is picked up without a restart. See [JS Credential Rotation](/features/js-credential-rotation) for the full contract and `resetOpenAIClient()`.
</Accordion>
</AccordionGroup>
