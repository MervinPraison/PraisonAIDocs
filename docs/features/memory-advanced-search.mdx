...
sidebarTitle: "Memory Advanced Search"
...
The user asks a question; advanced search reranks and filters memory hits before the agent answers.

```mermaid
graph LR
    subgraph "Memory Advanced Search"
        Query[🔍 Search Query] --> Search[📡 Vector Search]
        Search --> Rerank[🔄 Reranking]
        Search --> Filter[🎯 Relevance Filter]
        Rerank --> Score[📊 Quality Score]
        Filter --> Cutoff[✂️ Cutoff Threshold]
        Score --> Results[📤 Top Results]
        Cutoff --> Results
    end
    ...
    classDef filter fill:#6366F1,stroke:#7C90A0,color:#fff
...
sequenceDiagram
    participant User
    participant Agent
    participant MemoryAdvancedSearch

    User->>Agent: Request
    Agent->>MemoryAdvancedSearch: Process
    MemoryAdvancedSearch-->>Agent: Result
    Agent-->>User: Response
```
...
<CardGroup cols={2}>
  <Card icon="brain" href="/features/memory">
    Store and recall agent memory across turns and sessions.
  </Card>
  <Card icon="database" href="/features/rag">
