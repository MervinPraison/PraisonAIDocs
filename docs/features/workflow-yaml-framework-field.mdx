...
If `framework:` is omitted, agents.yaml resolves via `FrameworkAdapterRegistry.resolve_or_default(None)` — which delegates to [`pick_default()`](/features/framework-adapter-plugins#default-framework-selection) — while workflow YAML defaults to `praisonai`.
