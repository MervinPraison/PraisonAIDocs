...
<Note>
**See also:** the top-level `run_on:` key (a silent alias for `tools_run_on:`) shares one remote sandbox across every step — [Remote execution (`run_on:`)](/features/yaml-workflows#remote-execution-run_on) and [Placement](/features/placement).
</Note>
