...
<Note>
**See also:** the top-level `run_on:` key (the Python `AgentFlow(run_on=…)` kwarg) shares one remote sandbox across every step — [Remote execution (`run_on:`)](/features/yaml-workflows#remote-execution-run_on) and [Placement](/features/placement).
</Note>
...
If `framework:` is omitted, **both** agents.yaml and workflow YAML resolve via `FrameworkAdapterRegistry.resolve_or_default(None)` — which delegates to [`pick_default()`](/features/framework-adapter-plugins#default-framework-selection). The observability session tagged around a workflow-YAML run now reflects the config-resolved framework, not a hardcoded `"praisonai"` label.
...
fw = framework_from_config(cfg)                 # "praisonai" (uses process-default registry)
validate_workflow_framework(fw)                 # no-op when valid; raises ValueError otherwise
```

`framework_from_config({})` returns the process-default registry's `pick_default()` (`"praisonai"` on a standard install), not a hardcoded literal. `framework_from_config({"framework": "CrewAI"})` returns `"crewai"` (lowercased before validation). Pass `registry=` (new in [PR #5039](https://github.com/MervinPraison/PraisonAI/pull/5039)) to route the default through an injected registry:

```python
from praisonai.framework_adapters.registry import FrameworkAdapterRegistry

reg = FrameworkAdapterRegistry()
framework_from_config({}, registry=reg)         # → reg.pick_default().lower()
framework_from_config({"framework": "CrewAI"}, registry=reg)  # → "crewai"
```
...
On a standard install the registry-selected default is `praisonai`, so you can omit the `framework:` key entirely in workflow YAML. In a custom or tenant-scoped registry, ensure the default your registry picks has `SUPPORTS_WORKFLOW = True` before omitting the key, or the workflow validator will refuse the run.
</Accordion>
...
<CardGroup cols={2}>
<Card icon="puzzle-piece" href="/features/framework-adapter-plugins">
  Add new execution backends via Python entry points
</Card>
<Card icon="file-code" href="/features/yaml-workflows">
