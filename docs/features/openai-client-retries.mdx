...
<Note>
`praisonai --auto` uses LiteLLM first and only falls back to OpenAI on error. `OPENAI_MAX_RETRIES` therefore only takes effect on that fallback path — see [Auto Mode Providers](/features/auto-generator-providers) for the fallback flow.
</Note>
...
Retries help transient blips; for provider outages use `LLMConfig(fallbacks=[...])` — see [Model Fallback](/features/model-fallback).
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="Agent Retry" icon="clock-rotate-left" href="/features/agent-retry">
