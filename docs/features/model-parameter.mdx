...
Pass any object with a `get_response` or `get_response_async` method to `llm=` and the agent uses it as its backend instead of the OpenAI default. The tool loop calls `llm_instance.get_response(**kwargs)` (or its async twin), so those are the two surfaces the agent detects on.
...
An async-only backend works too — expose `get_response_async` and the async tool loop drives it. Either method alone is enough; you do not need both.

```python
from praisonaiagents import Agent

class MyAsyncBackend:
    model = "my-async-backend"
    async def get_response_async(self, prompt, **kwargs):
        return "answer from my async backend"

agent = Agent(instructions="Use my async backend.", llm=MyAsyncBackend())
agent.start("hello")
```

The agent detects on `get_response` and `get_response_async`, so any backend exposing either works — including the `ScriptedModel` test double from [Offline Testing](/features/offline-testing).

<Note>
**What is not adopted.** `praisonaiagents/llm/protocols.py` also defines `LLMProviderProtocol` (`chat` / `achat`) and `UnifiedLLMProtocol` (`chat_completion` / `achat_completion`). Those are provider-side protocols for other subsystems — the `Agent` tool loop drives `get_response` / `get_response_async` only. A backend exposing only `chat` / `chat_completion` is not picked up by `Agent(llm=<obj>)`; wrap it in a small class that exposes `get_response`, or use it as a plain provider.
</Note>

```mermaid
graph TB
    Start["🧩 Agent(llm=&lt;object&gt;)"] --> Q1{Has get_response<br/>or get_response_async?}
    Q1 -->|Yes| Adopt["✅ Adopted as backend<br/>_using_custom_llm = True"]
    Q1 -->|No| Q2{Only chat / achat /<br/>chat_completion / achat_completion?}
    Q2 -->|Yes| Skip["⚠️ Not adopted<br/>wrap it to expose get_response"]
    Q2 -->|No| Default["❌ Falls through to string/dict path"]
...
<Warning>
**Before PraisonAI [#4929](https://github.com/MervinPraison/PraisonAI/pull/4929)**, passing your own model object fell through every branch to the plain OpenAI path: `self.llm` became the object itself and `_using_custom_llm` stayed `False`, so every turn went to OpenAI under a model name that was really a `repr()`. [#4929](https://github.com/MervinPraison/PraisonAI/pull/4929) adopted `get_response`-shaped backends; [#4959](https://github.com/MervinPraison/PraisonAI/pull/4959) refined the detection to `get_response` and `get_response_async` and pinned it to the surface the tool loop can actually drive.
</Warning>
