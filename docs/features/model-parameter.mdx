...
The agent duck-types on `get_response`, so any conforming backend works — including the `ScriptedModel` test double from [Offline Testing](/features/offline-testing).
