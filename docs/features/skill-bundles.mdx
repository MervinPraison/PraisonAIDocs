...
<Note>
Skill Bundles are a planned feature currently in design. The APIs described below (`@bundle` selectors, `BundleManifest`, `discover_bundles`) are not yet available in the SDK. This page documents the intended behaviour to help contributors and early adopters prepare.
</Note>
...
### Programmatic discovery (planned API)
...
<Note>
`discover_bundles()`, `bundle_names`, and `resolve()` are planned additions to `SkillManager`. Use `skill_names` to list currently loaded skills.
</Note>
