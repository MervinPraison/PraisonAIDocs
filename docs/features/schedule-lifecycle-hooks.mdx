...
| `claim_due()` retires a spent job (one-shot `delete_after_run`, `max_runs` budget met, or past `until`) | — | ✅ (once) |
| `claim_due()` runs a recurring job that is not yet spent | — | ❌ |
| `_save()` fails and rolls back the delete | — | ❌ |

Retirement via any `should_retire()` path (one-shot, `max_runs`, or `until`) emits the same `SCHEDULE_REMOVE` hook as a manual `schedule_remove` — the store is the single chokepoint every deletion funnels through.
