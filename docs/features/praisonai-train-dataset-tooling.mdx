...
graph LR
    ...
    class Recipe,Trials input
    class Teacher,Merge,QC,FromTrials,Train process
...
See [praisonai-train Package](/features/praisonai-train-package) for the full `llm` reference.
...
- **Atomic write** — `generate` writes to a sibling temp file (via `tempfile.mkstemp` next to the destination) and `os.replace`s the destination only when **at least one row is produced**. A run that fails on credentials, recipe, or every teacher request (no JSON-mode support, all timeouts, etc.) leaves any pre-existing file byte-for-byte untouched — the temp file is unlinked and the CLI still exits `1` with `error: no rows generated`. A self-referential `dedup_from` is also read before it would be emptied. Fixed in [PraisonAI PR #4239](https://github.com/MervinPraison/PraisonAI/pull/4239).
...
Export a scored trials report into a trainer-ready SFT dataset — keep the attempts that passed verification, drop the rest, and write a ShareGPT (or Alpaca) JSONL the `praisonai-train llm` trainer consumes unchanged.

```mermaid
graph LR
    Trials[🧪 trials.json] --> Export[♻️ data from-trials]
    Export --> Data[✅ train.jsonl]
    Data --> LLM[⚙️ praisonai-train llm]
    LLM --> Reverify[🔁 re-run trials]
    ...
    class Trials input
    class Export,LLM,Reverify process
    class Data result
```
...
<Steps>
<Step title="Export the passing attempts">
...
<Step title="Fine-tune on the exported dataset">

Feed the JSONL straight into the trainer — no reshaping needed.

```bash
praisonai-train llm data/train.jsonl --model unsloth/gemma-2-2b-it-bnb-4bit
```

</Step>

<Step title="Re-verify to measure the gain">

Re-run the same trials on the fine-tuned model and compare pass rates.

```bash
praisonai-train agents --scenarios trials_cases.json
```
...
| `--qc` | — | flag | `false` | Run emitted rows through the same QC filter as `validate` (see [Export trials with QC](#export-trials-with-qc-from-trials-qc)) |
...
```text
─── from-trials: trials.json ───
  written=42 skipped_failed=7 skipped_tool_runs=3 skipped_no_text=0 skipped_unscored=2 skipped_saturated=15
  wrote 42 rows -> data/train.jsonl
  provenance -> data/train.jsonl.meta.json
```
...
```python
from praisonai_train.data import export_trials, ExportSummary

summary = export_trials(report, "data/train.jsonl")
print(summary)   # written=42 skipped_failed=7 ...
```
...
```python
def export_trials(
    report,                       # dict, object, or path to serialised report JSON
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `report` | `dict \| object \| str` | required | A serialised report, an in-memory report object, or a path — a path is loaded with `json.loads(Path(...).read_text())`; anything else is duck-typed as-is |
| `path` | `str` | required | Destination `.jsonl`; a missing parent directory is created automatically |
| `only_passed` | `bool` | `True` | Rejection sampling — drop verifier-failed attempts. `False` keeps failed / unscored (the `--all` flag) |
| `frontier_only` | `bool` | `True` | Drop all-pass / zero-pass cases. `False` keeps them (the `--include-saturated` flag) |
| `format` | `str` | `"messages"` | `"messages"` (ShareGPT) or `"alpaca"`. Any other value raises `ValueError` |
| `qc` | `bool` | `False` | Run emitted rows through the `validate` QC filter (see [Export trials with QC](#export-trials-with-qc-from-trials-qc)) |
| `qc_cfg` | `dict \| None` | `None` | QC overrides — e.g. re-enable a target-script check with `{"script_range": [...]}` |
...
| Field | Type | Counts |
|-------|------|--------|
| `written` | `int` | Rows emitted to the dataset |
| `skipped_unscored` | `int` | Attempts with `score is None` (kept under `--all`) |
| `skipped_failed` | `int` | Verifier-failed attempts dropped by `only_passed` |
| `skipped_saturated` | `int` | Attempts in all-pass / zero-pass cases dropped by `frontier_only` |
| `skipped_tool_runs` | `int` | Tool-using runs excluded by default |
| `skipped_no_text` | `int` | Attempts with no assistant text to export |
...
{
  "package": "demo",
  "source_report": "trials.json",
  "options": {"only_passed": true, "frontier_only": true, "format": "messages", "qc": false},
  "summary": {"written": 42, "skipped_failed": 7, "skipped_tool_runs": 3, "...": "..."},
  "lines": [
    {"case_id": "c1", "attempt_index": 0, "score": 0.92},
    {"case_id": "c1", "attempt_index": 2, "score": 0.87}
  ]
}
```
...
sequenceDiagram
    ...
    participant QC as filter_rows_for_export

    User->>Trials: run_trials(agent, package, k=8)
    Trials-->>User: TrialReport
    User->>Export: export_trials(report, "train.jsonl", qc=True)
    Export->>QC: rows + qc_cfg
    Note over QC: script-purity check auto-disabled<br/>(English agent trajectories)
    QC-->>Export: kept rows + drops dict
    Export-->>User: ExportSummary(written, skipped_qc, qc_drops, ...)
```
...
package = EvalPackage(name="suite", cases=[
    EvalCase(name="short", input="PraisonAI is an agent framework.",
             verify=lambda output, expected: "agent" in output.lower()),
])

report = run_trials(agent, package, k=8)

# English agent trajectories — QC keeps them (script check auto-disabled).
summary = export_trials(report, "train.jsonl", qc=True)
print(summary)
# written=42 skipped_failed=6 ... skipped_qc=3 qc_drops={'exact_dup': 3}

# Spanish trajectories: opt back into the Latin script check.
summary = export_trials(
    report, "train.jsonl", qc=True,
    qc_cfg={"script_range": (0x0041, 0x024F)},   # Latin block
)
```
...
```bash
praisonai-train from-trials trials.json --qc -o data/train.jsonl
# error: no rows exported — QC dropped all rows: {'low_script_purity': 8}
```
...
|-------|------|-------------|
| `skipped_qc` | `int` | Rows removed by the QC filter (previously untracked). |
| `qc_drops` | `dict[str, int]` | Per-reason drop counts (e.g. `{"low_script_purity": 42, "exact_dup": 3}`). |
...
- **File mode and symlinks handled the same as `generate`** — the atomic swap preserves the destination's existing mode (or `0o666 & ~umask` for a fresh file), and a symlink destination is followed so the swap lands on the target, not the link itself. See [PraisonAI PR #4239](https://github.com/MervinPraison/PraisonAI/pull/4239).
...
Preference tuning reads columns by name — give each method the exact shape it needs. See [Preference Tuning](/features/praisonai-train-preference-tuning).
...
`from-trials --qc` auto-disables the target-script check for English agent trajectories, so exports aren't dropped as `low_script_purity` out of the box. For other languages, pass `qc_cfg={"script_range": [...]}` to `export_trials()` to re-enable the check with the QC defaults (`0.50` / `0.70`).
</Accordion>
...
<Accordion title="Prefer from-trials over generate when you have real agent trajectories">
...
<Card title="Trials Engine" icon="flask" href="/features/trials-engine">
