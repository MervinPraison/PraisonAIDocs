...
| 3 | Registry default | `FrameworkAdapterRegistry.resolve_or_default(None)` → `pick_default()` → `praisonai` on a standard install |
...
<CardGroup cols={2}>
  <Card title="CrewAI Framework" icon="users" href="/framework/crewai">
    CrewAI framework integration guide
  </Card>
  <Card title="PraisonAI Agents" icon="users-gear" href="/framework/praisonaiagents">
