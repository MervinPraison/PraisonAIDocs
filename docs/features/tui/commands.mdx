...
Inside a running TUI session, `/diff` and `/undo` inspect and roll back workspace file changes.

| Command | Description |
|---------|-------------|
| `/diff` | Show file changes this session (`--turn` for the last turn, `<file>` for one file). Requires `checkpoints.auto`. |
| `/undo` | Roll back the last turn's file changes (and conversation history when a session store is wired). Requires `checkpoints.auto`. |

These commands are also available in the legacy `praisonai code` REPL and are backed by the same engine, so behaviour is identical.

<Note>
See [Checkpoints](/features/checkpoints) for the full API and config precedence, and [Slash Commands](/cli/slash-commands) for the slash-command overview.
</Note>
...
<CardGroup cols={2}>
<Card title="TUI Overview" icon="terminal" href="/features/tui/overview">
  Interactive terminal interface
</Card>
<Card title="TUI Simulation" icon="flask-vial" href="/features/tui/simulation">
