...
reviewer = Agent(
    ...
    instructions="Only approve file reads. Deny everything else. Reply APPROVE, DENY, or ESCALATE if uncertain.",
)
...
<Note>
Supplying your own `approver_agent` opts you out of the hardened defaults — you are unaffected unless you adopt the new instructions and tri-state verdict yourself. The default reviewer (auto-created when none is passed) gets the hardened instructions. See [AgentApproval Hardening](/features/approval-agent-hardening).
</Note>
