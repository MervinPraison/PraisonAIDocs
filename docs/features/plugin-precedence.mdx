...
**Standalone installs of `praisonai-sandbox` and `praisonai-deploy`** carry a vendored fallback copy of `PluginRegistry` that runs when `praisonai-code` isn't importable (`pip install praisonai-sandbox` on its own). Since [PraisonAI PR #4184](https://github.com/MervinPraison/PraisonAI/pull/4184) both vendored copies enforce the **same** built-in check — an entry point whose name (case-insensitive) matches a built-in is skipped on that path too, so the guarantee holds in every install shape. Prior to PR #4184 a standalone `praisonai-sandbox` install still let a pip plugin replace `docker`, and `praisonai-deploy` let one replace `aws`.
...
sequenceDiagram
    participant Pkg as pip package
    participant Reg as vendored PluginRegistry<br/>(praisonai-sandbox / praisonai-deploy<br/>standalone)
    participant Built as _builtin_names

    Pkg->>Reg: entry point "docker" (sandbox) / "aws" (deploy)
    Reg->>Built: lowercase(name) in built-ins?
    alt collision
        Built-->>Reg: yes
        Reg-->>Reg: skip entry point (DEBUG log), keep built-in
    else novel name
...
| Entry point discovered by a **vendored fallback** copy (standalone `praisonai-sandbox` / `praisonai-deploy`), name collides with a built-in | **Skipped**; built-in kept (`DEBUG` log) — same rule, applied on the fallback path since [PR #4184](https://github.com/MervinPraison/PraisonAI/pull/4184). |
...
| Plugin-provided tool (`Plugin.get_tools()`) collides with an agent tool (`Agent(tools=[...])`) | **Agent tool wins**; the plugin's same-named tool is skipped with a `WARNING` log — never silently shadowed. See [Tool Plugins](/features/plugins#tool-plugins-subclass-plugin-with-get-tools). |
...
The two groups that matter most decide **where user code executes**: `praisonai.sandbox` and `praisonai.managed_backends`. A pip-installed package cannot silently take over `docker` — whether via the canonical base class ([PR #4176](https://github.com/MervinPraison/PraisonAI/pull/4176)), the legacy group spelling ([PR #4185](https://github.com/MervinPraison/PraisonAI/pull/4185)), or the vendored fallback copy that runs when `praisonai-sandbox` / `praisonai-deploy` is installed standalone ([PR #4184](https://github.com/MervinPraison/PraisonAI/pull/4184)).

| Entry-point group | Registry class | Runtime accessor | Example guarded built-ins |
|---|---|---|---|
| `praisonai.llm_providers` | `LLMProviderRegistry` | `get_default_llm_registry()` | `openai`, `anthropic`, `google`, `ollama`, `ollama_chat`, `lm_studio`, `vllm`, `hosted_vllm` |
...
<Note>
Since [PR #4800](https://github.com/MervinPraison/PraisonAI/pull/4800), the local routes `ollama`, `ollama_chat`, `lm_studio`, `vllm`, `hosted_vllm` are guarded built-ins under `praisonai.llm_providers`. A pip package publishing an entry point named `ollama` (or any of the five) is now **silently skipped** (`DEBUG` log) instead of taking over — matching the existing behaviour for `openai`. Plugin authors must use runtime `register(..., override=True)` to replace any of these.
</Note>
...
<Note>
Standalone installs of `praisonai-sandbox` and `praisonai-deploy` use a vendored fallback copy of `PluginRegistry` (they don't import `praisonai-code`). Since [PR #4184](https://github.com/MervinPraison/PraisonAI/pull/4184) those fallback copies enforce the same built-in check — a pip package publishing `docker` (sandbox) or `aws` (deploy) on its own can no longer replace the built-in on those slim installs. Runtime `register(...)` on the standalone package's registry remains the only supported override.
</Note>
...
Additive plugins (novel names) are unchanged. Switching to a **deprecated/legacy group spelling is not a workaround** — since PR #4185 the guard applies there too. A previously‑silent shadow via a standalone `praisonai-sandbox` (`docker`) or `praisonai-deploy` (`aws`) install — the last remaining path — is closed by [PR #4184](https://github.com/MervinPraison/PraisonAI/pull/4184). Runtime `register(...)` on the standalone package's registry remains the supported override.
</Warning>
