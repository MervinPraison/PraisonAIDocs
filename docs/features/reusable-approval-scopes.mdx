...
## How this differs from skill grants

Reusable prefix scopes (`bash:git *`) live on the `PermissionManager` and cover any matching tool call within that agent's scope. Skill `allowed-tools` auto-approval is separate: it is keyed to the **specific agent instance** that activated the skill, not to a command prefix.

| Mechanism | Where it lives | Scope |
|---|---|---|
| Reusable prefix scope | `PermissionManager` | Agent-wide — any matching `bash:git …` call |
| Skill `allowed-tools` grant | Per-agent approval-scope id | Only the agent instance that activated the skill |

Two unnamed `Agent()` instances (both defaulting to `name="Agent"`) never share a skill grant, because each carries a unique per-instance approval-scope id. See [Tool Approval → Scope of a skill grant](/features/tool-requires-approval#scope-of-a-skill-grant).

<Note>
Closing an agent (`agent.close()` / `await agent.aclose()`, or `__del__`) releases **only that agent's own** approval-scope id — its skill `allowed-tools` grants and "this session" decisions. Reusable prefix scopes on the `PermissionManager` and every other live agent's grants are untouched. See [Permissions → Approval-scope cleanup](/features/permissions#approval-scope-cleanup).
</Note>
...
A `deny` rule on `bash:rm *` still wins. Reusable scopes only extend `allow` approvals — deny is enforced at the rule level. See [Command-Aware Permissions](/features/command-aware-permissions).
</Accordion>
...
<Card title="PermissionManager API Reference" icon="code" href="/features/permissions">
...
<CardGroup cols={2}>
  ...
  <Card title="Workspace Boundary" icon="folder-lock" href="/features/workspace">
