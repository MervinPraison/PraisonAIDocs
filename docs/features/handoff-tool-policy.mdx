...
```mermaid
graph LR
    subgraph "Handoff Tool Policy"
        Request[📋 User Request] --> Process[⚙️ Handoff Tool Policy]
...
`HandoffConfig` also carries `max_concurrent`, enforced independently per handoff — see [Per-Instance Concurrency](/docs/configuration/handoff-config#per-instance-concurrency-max_concurrent).
</Step>
</Steps>
