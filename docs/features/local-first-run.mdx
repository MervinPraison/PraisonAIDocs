...
<Step title="Same path from the SDK">
The SDK `Agent()` constructor resolves the same default — no `llm=`, only `OLLAMA_HOST` set. This works from the SDK as of [PR #4795](https://github.com/MervinPraison/PraisonAI/pull/4795).

```python
# no OPENAI_API_KEY, no llm=; only OLLAMA_HOST is set
from praisonaiagents import Agent

Agent(instructions="local-first").start("Explain black holes to a 5-year-old")
```
</Step>
</Steps>
