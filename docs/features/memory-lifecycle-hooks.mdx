...
<Note>
This page is about lifecycle callbacks on a `Memory` subclass. For the standalone `.praisonai/hooks.json`-driven utility see [Memory HooksManager](/features/memory-hooks-manager). For hooks that fire around agent tool/LLM calls see [Hooks](/features/hooks).
</Note>
...
<Card title="Handoffs" icon="handshake" href="/features/handoffs">
