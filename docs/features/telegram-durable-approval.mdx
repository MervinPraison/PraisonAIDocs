...
<Note>
**Superseded turns.** If the originating turn was `/stop`ped or superseded in the meantime (for example in `busy_mode="interrupt"`), the button tap is rejected fail-closed and the card updates to a "no longer applicable" outcome (recorded with `reason="superseded"`). No stale tool fires. See [Approval Turn Liveness](/features/gateway-approval-turn-liveness).
</Note>
