...
<Note>
If the originating turn was `/stop`ped or superseded while the Allow / Deny card was still open, the tap is dropped **fail-closed** by the core registry. The card updates to a "no longer applicable" state and the decision recorded is `ApprovalDecision(approved=False, reason="Turn no longer live (stopped or superseded)")` — no `session`/`always` grant is persisted for the abandoned turn.
</Note>

### Superseded turns

A reviewer can tap Allow minutes or hours after the card first appears. If the user cancelled the run (or a new turn took over) in the meantime, that late tap must not fire the tool. The core SDK enforces exactly this since [PraisonAI #4950](https://github.com/MervinPraison/PraisonAI/pull/4950), so it applies to *every* backend on this page — not just the durable Telegram one:

- The tool does not run.
- No `session`/`always` grant is persisted for the abandoned turn.
- The returned decision carries `reason="Turn no longer live (stopped or superseded)"`.

See [Approval Turn Liveness](/features/gateway-approval-turn-liveness) and [Approval Protocol → Live-Authority Binding](/features/approval-protocol#live-authority-binding).
...
<CardGroup cols={2}>
<Card title="Secure Approval Backend" icon="code" href="/features/approval-secure-backend">
...
<Card title="Gateway Approval Durability" icon="plug" href="/features/gateway-approval-durability">
...
<Card title="Approval Turn Liveness" icon="hourglass-end" href="/features/gateway-approval-turn-liveness">
  A tap on a `/stop`ped or superseded turn is dropped fail-closed
</Card>
<Card title="Live-Authority Binding" icon="shield-check" href="/features/approval-protocol#live-authority-binding">
