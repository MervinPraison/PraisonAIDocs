...
```typescript
import { Agent, GoalEngineer } from 'praisonai';

const agent = new Agent({ name: 'Summariser', instructions: 'Summarise the input' });
const engineer = new GoalEngineer();
const goal = await engineer.engineer('Summarise the report in under 100 words');

const output = await agent.start(goal.toPrompt());
const result = await engineer.verify(goal, output);
...
<Card title="Goal Module Reference" icon="code" href="/sdk/praisonaiagents/index">
...
|-------|------|---------|-------------|
| `description` | `str` | — | What must be true for this criterion to be met |
| `id` | `str` | 8-char UUID | Unique identifier |
| `weight` | `float` | `1.0` | Relative importance when scoring the goal |
| `status` | `Literal["pending", "met", "unmet"]` | `"pending"` | Current status |
| `notes` | `str` | `""` | Notes from verification |
...
|----------|------|-------------|
| `progress` | `float` | Weighted fraction of criteria whose `status == "met"`, clamped to `[0.0, 1.0]`. Non-positive weights count as `0`; when total weight is `0`, criteria count equally. |
| `is_achieved` | `bool` | `True` when there is at least one criterion and **all** criteria are `met` |
...
|-------|------|-------------|
| `goal_id` | `str` | The verified goal's id |
| `score` | `float` | Overall score in `0.0`–`10.0` |
| `achieved` | `bool` | Whether the goal is considered achieved (`score >= config.threshold`) |
| `criteria` | `List[SuccessCriterion]` | Independent snapshot of the goal's criteria with updated statuses; mutating this list does not affect the original `Goal` |
| `reasoning` | `str` | Judge explanation, or `"Verification unavailable: …"` when the judge fell through |
