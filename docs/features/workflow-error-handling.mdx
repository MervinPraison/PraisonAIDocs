...
<AccordionGroup>
  ...
  <Accordion title="In Discussion">
    A speaker with `on_error="stop"` halts the entire discussion, and the stop propagates to the enclosing workflow — later steps do not run.
  </Accordion>
</AccordionGroup>
...
| Mode | When any branch fails | Variables merged from failed branch |
|------|-----------------------|-------------------------------------|
| `fail_fast` | Cancel remaining branches, raise `WorkflowStepError` immediately. | None — nothing from any branch is merged |
| `fail_all` | Let all branches finish, then raise with collected errors. | None — nothing from any branch is merged |
| `partial_ok` | Record the failure, keep the other branches, return combined output. | Any variables the failed branch wrote before it failed (its own `output_variable` is never set, since the failure returns before that write) |

For the full merge rule (declaration order, collision warning, delta-only merge), see [Named outputs from branches](/features/workflow-parallel#named-outputs-from-branches).
