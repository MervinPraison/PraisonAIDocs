...
| `busy` | **No** | `active_runs > 0` with any progress (inbound transport OR in-run: tool calls, streamed tokens, emitter events) within `stuck_after` — protects long runs from mid-run kills |
| `stuck` | Yes | `active_runs > 0` and no progress at all (neither inbound transport nor in-run) for `> stuck_after` — likely wedged |
