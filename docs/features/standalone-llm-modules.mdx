...
```mermaid
graph LR
    subgraph "Standalone LLM Modules"
        Ask[📋 Ask about models] --> Cat[📚 Catalogue + Credentials]
        Cat --> Agent[🤖 Agent]
        Agent --> Ans[✅ Answer]
    end
...
sequenceDiagram
    participant User
    participant Agent
    participant Credentials as llm.credentials
    participant Catalogue as llm.catalogue

    User->>Agent: Ask about available models
    Agent->>Credentials: is_configured()
    Credentials-->>Agent: Ready
    Agent->>Catalogue: list_models(provider="openai")
    Catalogue-->>Agent: Model list
    Agent-->>User: Answer with model details
```
...
sequenceDiagram
    ...
    User->>Agent: "Which model supports vision?"
    Agent->>Credentials: is_configured()
    Credentials-->>Agent: True
    Agent->>Catalogue: list_models(provider="openai")
    Catalogue-->>Agent: ModelInfo list
    Agent-->>User: Answer with matching model
```
...
sequenceDiagram
    ...
    User->>Agent: "Which model supports vision?"
    Agent->>Catalogue: list_models(provider="openai")
    Catalogue-->>Agent: Available models
    Agent->>Credentials: is_configured()
    Credentials-->>Agent: Endpoint + key
    Agent-->>User: Answer with model details
```
...
catalogue = ModelCatalogue(
    cache_dir=None,   # defaults to ~/.praisonai/cache
...
Cache file: `~/.praisonai/cache/models.json` (TTL 3600s, honours `PRAISONAI_HOME`). Falls back to 14 curated models (OpenAI 5, Anthropic 3, Google 3, Groq 2, Ollama 1) when litellm is unavailable.
...
<Card title="ModelCatalogue Python Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/main">
  Python configuration options for ModelCatalogue
</Card>
<Card title="LLMEndpoint Python Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/main">
...
<CardGroup cols={2}>
  <Card title="Model Catalogue" icon="microchip" href="/features/models-cli">
  ...
  <Card title="PraisonAI Code CLI" icon="terminal" href="/features/praisonai-code-cli">
    The standalone CLI runtime these modules power
  </Card>
  <Card title="Package Boundaries (C7.1)" icon="layer-group" href="/sdk/praisonai-code#package-boundaries-c71">
