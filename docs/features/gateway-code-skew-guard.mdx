...
<Step title="Read fingerprints programmatically">

```python
from praisonaiagents.gateway import read_code_fingerprint, detect_code_skew

boot_fp = read_code_fingerprint()   # captured at startup
disk_fp = read_code_fingerprint()   # read from disk now

if detect_code_skew(boot_fp, disk_fp):
    print("Code changed — restart before hot operations")
else:
    print("Code is consistent")
```
