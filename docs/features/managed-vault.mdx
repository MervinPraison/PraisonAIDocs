...
managed = AnthropicManagedAgent(config=ManagedConfig(model="claude-sonnet-4-6"))
agent = Agent(name="Integrations", instructions="Call external APIs using vault-stored OAuth tokens.", llm=managed)
agent.start("Summarise my latest calendar events.")
```
