...
<Steps>
<Step title="Set required env vars (CLI only)">
```bash
...
- **Custody survives restart**: `authorized_reviewers` on a pending request is persisted and rehydrated by the same SQLite store, so per-request reviewer bindings survive a gateway restart. See [Per-request reviewer custody](/features/gateway-scoped-approvals#per-request-reviewer-custody).
- **Fail-open startup**: a corrupted store logs the error and returns 0 — it never blocks gateway startup.

---

## What Happens on `/stop` or Supersede

Pending approvals bound to a superseded turn are cancelled fail-closed via `cancel_for_generation()` — the awaiting tool call unwinds promptly with `reason="cancelled"`. A resolution that arrives after the supersede is dropped and recorded with `reason="superseded"` in the audit trail, so a late reviewer decision never fires a stale tool or delivers a stale reply.

This binding is a no-op for approvals that carry no `session_id`/`run_generation`. See [Approval Turn Liveness](/features/gateway-approval-turn-liveness).
...
<CardGroup cols={2}>
<Card title="Secure Approval Backend" icon="shield-check" href="/features/approval-secure-backend">
  Full reference for the actor-authorised, durable approval backend
</Card>
<Card title="Gateway" icon="network" href="/features/gateway">
  Gateway deployment and configuration
</Card>
<Card title="Durable Approvals (Bots)" icon="database" href="/features/durable-approvals">
  SQLite-backed approval storage for messaging bots
</Card>
<Card title="Durable Chat-Native Backends" icon="comments" href="/features/durable-approvals#durable-chat-native-backends">
  Add `store=` to Slack, Telegram, Discord, Webhook, and HTTP approvals
</Card>
...
<Card title="Approval Turn Liveness" icon="hourglass-end" href="/features/gateway-approval-turn-liveness">
  Cancel pending approvals when a turn is `/stop`ped or superseded
</Card>
</CardGroup>
