...
Sub-agent transcripts (from `session.Agent(...)`) live inside that same `state.json` under `metadata["agent_histories"]`, not in separate per-agent files.
...
Each sub-agent's chat history persists **into the parent session's record** under `metadata["agent_histories"][agent_key]` (PraisonAI PR #4126), so:

* Multiple sub-agents under one `Session` share one file.
* `session.close()` / `save_state()` cannot overwrite an unrelated user's session even when the sanitised sub-agent names would collide.
* Deleting, forking, or exporting the parent takes the sub-agent transcripts with it.
...
researcher = session.Agent(name="Researcher", role="research")
writer     = session.Agent(name="Writer",     role="draft")

researcher.start("Find recent quantum-computing papers.")
writer.start("Draft a two-paragraph summary.")

session.close()   # Both transcripts saved under metadata["agent_histories"]
                  # on the "chat" record. Any unrelated user session named
                  # "chat_researcher_research" or "chat_writer_draft" is untouched.
```

Sub-agent transcripts written by pre-#4126 versions **that carry the tags** (`parent_session_id` + `agent_key`) are migrated automatically on the next save — no flag needed, no recency limit (PraisonAI PR [#4162](https://github.com/MervinPraison/PraisonAI/pull/4162)).

To recover **untagged** pre-tag transcripts (older records that predate the tagging change), opt in with `PRAISONAI_SESSION_LEGACY_AGENT_KEYS=1` (accepts `1` / `true` / `yes` / `on`). Set it only when you know which on-disk records are legacy sub-agent transcripts — an untagged id like `"chat_support_agent"` may be a real user session.
...
For database-backed persistence without manual session management, see the [HostedAgent persistence guide](/features/managed-agent-persistence).

Persistence backends:

- **[Session Mirror](/features/session-mirror)** — local-first sink for replicating a session's append-only records to a remote backend (postgres/supabase/turso adapters land in the wrapper alongside #3645).
...
<CardGroup cols={2}>
  <Card icon="rotate" href="/features/bot-session-reset">
