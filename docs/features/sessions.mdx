...
To recover sub-agent transcripts written by pre-#4126 versions that lack the `parent_session_id` / `agent_key` tags, opt in with `PRAISONAI_SESSION_LEGACY_AGENT_KEYS=1` (accepts `1` / `true` / `yes` / `on`). Set it only when you know which on-disk records are legacy sub-agent transcripts — an untagged id like `"chat_support_agent"` may be a real user session.
