...
graph TD
    ...
    classDef item fill:#F59E0B,stroke:#7C90A0,color:#fff
...
<Accordion title="Handle errors per item">
  Wrap each item's processing in a try/except so one failure doesn't abort the entire loop. Return a `StepResult` with an error message instead of raising.
</Accordion>
<Accordion title="Log progress">
  Use verbose mode or emit a log line in each step function. For large datasets, knowing which item is being processed helps diagnose slow or stuck runs.
</Accordion>
<Accordion title="Batch large datasets">
  For very large lists or files, consider splitting them into smaller chunks and chaining multiple loops. This avoids memory issues and makes partial failures easier to recover from.
</Accordion>
<Accordion title="Clean up resources">
  Close file handles, database connections, and API sessions in a `finally` block inside the step function, not after the workflow completes.
</Accordion>
</AccordionGroup>
