...
| `Agent(sandbox=SandboxConfig.docker(...))` | No — no tools auto-added | No | Yes (container) | Depends on backend |
...
<Note>
With the docker backend, cross-command filesystem state **is** provided: `write_file()`, `run_command()`, and `execute()` share a `/sandbox` bind mount, so a file written by one is visible to the next. That mount does **not** expose the host filesystem (`ls /Users` fails), and `list_files()` never returns host paths — even on platforms where `TMPDIR` sits behind a symlink (macOS `/var → /private/var`).
</Note>
