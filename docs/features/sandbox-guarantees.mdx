...
|---|---|---|---|---|
| `Agent(sandbox=True)` (default subprocess backend) | No — no tools auto-added | No | No (subprocess only) | Partial — `blocked_imports` now enforced on both `run_command()` and `execute()` ([PR #4875](https://github.com/MervinPraison/PraisonAI/pull/4875), Python code, AST-based); other clauses still `run_command()`-only |
...
| `Agent(sandbox=SandboxConfig.native())` (sandlock) | Isolates only explicit `execute_code()` | No | Yes — Landlock + seccomp-bpf, the only real **local** boundary on **Linux** | Kernel-enforced |
...
<Note>
No row above provides **macOS** local containment: `SandboxConfig.native()` is Linux-only (`sandlock`) and raises `ImportError` on macOS. macOS Seatbelt (`sandbox-exec`) and Linux `bwrap` live in the `praisonai code` CLI, reached with [`PRAISON_SHELL=sandboxed`](/features/os-sandbox) — a separate path from `SandboxConfig.native()`.
</Note>
...
### File persistence per backend

A `True` return from `write_file()` means the bytes are on disk; Modal is honest about writing nothing and returns `False`.

| Backend | File persistence | `write_file()` return |
|---|---|---|
| `subprocess` / `sandlock` | Yes | `True` when the bytes land on disk |
| `docker` | Yes — shared `/sandbox` mount | `True` when the bytes land on disk |
| `ssh` | Yes — on the remote host | `True` when the bytes land on disk |
| `e2b` / `daytona` / `novita` | Yes — on the cloud sandbox | `True` when the bytes land on disk |
| `modal` | **No** — stateless, no file storage | `False` — nothing is written; use `execute(code=...)` |

<Warning>
**Modal has no file persistence.** `write_file()` returns `False` and writes nothing, `read_file()` returns `None`, and `execute_file()` fails with a "no file storage" error. Pass code inline via `execute(code=...)`, or use `docker` / `subprocess` / `ssh` when the workflow needs files. See [Modal caveats](/docs/features/sandbox-backends#modal-caveats).
</Warning>
...
<Note>
As of [PR #4875](https://github.com/MervinPraison/PraisonAI/pull/4875), the subprocess backend's `execute()` path enforces **`blocked_imports`** on Python code. The check parses the source with `ast` (no substring false positives from strings, comments, or longer identifiers) and resolves aliases back to their real dotted names. See the [`sandbox` warning](/features/sandbox#what-securitypolicy-actually-controls-in-subprocess) for the end-to-end example.
</Note>

<Warning>
`blocked_imports` is now enforced on the subprocess **`execute()`** path. The rest of `SecurityPolicy` is still **not** enforced there (this warning is scoped to that backend and that path):
