...
Covered reasons mirror `Agent.last_stop_reason`: `completed | max_steps | cancelled | error`, plus the three provider-block reasons `content_filtered | refused | length_truncated`. A deep-research agent hits its step limit; instead of a truncated answer, the reply ends with `⏳ I stopped after reaching the step limit — reply "continue" to carry on.`
...
| `content_filtered` | `⏳ This turn ended early — please try again.` (generic fallback — see note) |
| `refused` | `⏳ This turn ended early — please try again.` (generic fallback — see note) |
| `length_truncated` | `⏳ This turn ended early — please try again.` (generic fallback — see note) |
| *(unknown/new)* | `⏳ This turn ended early — please try again.` |

<Note>
The three provider-block reasons (`content_filtered` / `refused` / `length_truncated`) are **non-clean** stop reasons, so `TurnCompletion.truncated` is `True` and a note is surfaced — but they currently fall through to the generic fallback rather than a per-reason message. Pass a `detail` override for a tailored, user-safe note (never leak refusal text):
...
reply = AgentReply(
    ...
    completion=TurnCompletion(
        ...
        detail="This response was blocked by the provider's content filter.",
    ),
)
```
