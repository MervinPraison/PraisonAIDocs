...
<Note>
**Ephemeral / CI environments.** On ephemeral runners you can skip both `setup` and `auth login` entirely — set `PRAISONAI_AUTH_CONTENT` to a JSON credential store and PraisonAI will load it in memory with no disk writes. See [Zero-disk mode](/cli/auth#zero-disk-mode-praisonai_auth_content).
</Note>
