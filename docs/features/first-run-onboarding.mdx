...
| 3.5. Local endpoint probe | `detect_local_model()` probes `OPENAI_BASE_URL` → `OLLAMA_HOST` → `http://127.0.0.1:11434`. Reachable → uses that model with a stderr notice; timeout-bounded (150 ms per probe) | `ollama serve` running locally |
