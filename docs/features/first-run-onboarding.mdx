...
|------------------|------|---------|-------------|
| `OPENAI_API_KEY` | `str` | — | OpenAI API key; detected automatically on startup |
| `ANTHROPIC_API_KEY` | `str` | — | Anthropic Claude key |
| `GOOGLE_API_KEY` / `GEMINI_API_KEY` | `str` | — | Google / Gemini key |
| `GROQ_API_KEY` | `str` | — | Groq key |
| `COHERE_API_KEY` | `str` | — | Cohere key |
| `~/.praison/credentials.json` | file | — | Persistent credential store written by `praisonai setup` |
