...
All four LLM-bound entrypoints — `praisonai` (bare), `praisonai run`, `praisonai code`, and `praisonai chat` — route their first-run check through the same shared helper, so a keyless newcomer is offered the wizard (or a clean hint) from *any* first command.
...
```mermaid
graph TB
    A[🚀 praisonai / run / code / chat] --> B{Cloud key<br/>configured?}
...
Answer `n` to fall through to the full provider menu. The pre-selection surface is no longer four keys — the wizard auto-detects **any** catalogued credential env var (the full [14-row table](/cli/setup#credential--default-model-map)), including Together (`TOGETHER_API_KEY` / `TOGETHERAI_API_KEY`) and Fireworks (`FIREWORKS_API_KEY` / `FIREWORKS_AI_API_KEY`) aliases. The provider menu only appears when no key is detected, and is **catalogue-driven** — it lists every provider `ModelCatalogue` knows about (Groq, OpenRouter, Mistral, DeepSeek, xAI, …), curated ones first and `custom` last.
...
<Tab title="Mistral">
```python
# Only MISTRAL_API_KEY set in the environment — no wizard, no --model needed.
from praisonaiagents import Agent

agent = Agent(name="assistant", instructions="Answer briefly.")
agent.start("Say hello in one line.")
# Uses mistral/mistral-large-latest automatically.
```
</Tab>
<Tab title="DeepSeek">
```python
# Only DEEPSEEK_API_KEY set in the environment.
from praisonaiagents import Agent

agent = Agent(name="assistant", instructions="Answer briefly.")
agent.start("Say hello in one line.")
# Uses deepseek/deepseek-chat automatically.
```
</Tab>
<Tab title="Fireworks">
```python
# Only FIREWORKS_AI_API_KEY set — the alias is honoured.
from praisonaiagents import Agent

agent = Agent(name="assistant", instructions="Answer briefly.")
agent.start("Say hello in one line.")
# Uses fireworks_ai/accounts/fireworks/models/llama-v3p3-70b-instruct automatically.
```
</Tab>
</Tabs>
</Step>
...
|------|----------------|---------|
| 1. Env vars (fast path) | **Any** catalogued provider key from `PROVIDER_ENV_CATALOGUE` — OpenAI, Anthropic, Google/Gemini, Groq, Cohere, OpenRouter, Mistral, DeepSeek, xAI, Together, Perplexity, Fireworks, Ollama | `export MISTRAL_API_KEY=...` |
| 2. Zero-disk env blob | `PRAISONAI_AUTH_CONTENT` — a full JSON credential store loaded in memory (CI / containers), preferred over disk | `export PRAISONAI_AUTH_CONTENT='{...}'` |
| 3. Credential store | `~/.praisonai/credentials.json` written by `praisonai setup` (also mirrored to `~/.praisonai/.env`) | `praisonai setup` |
| 4. LLM resolution | `resolve_llm_endpoint_with_credentials()` — final fallback | Stored OpenAI base URL |
| 5. Keyless local-first | `praisonai_code.llm.local_detect.detect_local_model()` — probes `OPENAI_BASE_URL` / `OLLAMA_HOST` / `127.0.0.1:11434` when no cloud key is set | Any running Ollama server auto-adopted |
| 6. Setup wizard auto-detection | Any catalogued provider key, offered as pre-selected default when the user is inside the wizard | Detected on wizard entry, not first `Agent.start()` |

<Note>
Provider auto-detection is **catalogue-driven** — any provider with its API-key env var set is recognised, not just OpenAI/Anthropic/Google/Ollama. See [Provider Auto-Detection](/docs/models#provider-auto-detection-no-config-first-run) for the full env-var → default-model table.
</Note>

When no cloud key is present, PraisonAI adopts a reachable local endpoint before offering the wizard — the first `praisonai run "..."` works against a running Ollama on `localhost:11434` with zero setup. See [Keyless local-first fallback](/docs/models#keyless-local-first-fallback-no-env-vars-set) and [Local Models](/features/local-models).

<Note>
`PRAISONAI_HOME` relocates the entire CLI home (`config.yaml`, `.env`, `credentials.json`, sessions, traces, logs, cache) in one place; it defaults to `~/.praisonai/`. See [Delivery Config → base directory override](/features/delivery-config).
</Note>
...
| No cloud key, local endpoint reachable, **headless** | Adopts the detected local model + base URL for this run; the run proceeds. |
...
| No cloud key, **no** local endpoint, **headless** | Exit `1` with: `No API key configured. Run: praisonai setup` … `(a running local endpoint such as Ollama would be used automatically)`. |

### Headless triggers per entrypoint

"Headless" (`interactive=False`) is decided per command — this is what "non-TTY" means for each:

| Entrypoint | Headless when… |
|------------|----------------|
| `praisonai run` | `not sys.stdin.isatty()` OR `--output json` |
| `praisonai code` | `-p` / `--print` OR piped stdin (non-TTY) |
| `praisonai chat` | `--json` OR piped stdin (non-TTY) |
| Bare `praisonai` | `not sys.stdin.isatty()` OR `--quiet` |
...
```
$ praisonai code "add a test for foo"
No API key configured.
...
Verifying your setup...
✅ Verified — your agent is working!
[code assistant starts]
```
</Tab>
<Tab title="Headless (-p)">
```
$ praisonai code -p "add a test for foo" --output json
...
[code assistant starts against local Ollama]
```
</Tab>
</Tabs>

The equivalent trio for `praisonai chat` uses `--json` for the headless line. See [Chat → First-run credential gate](/cli/chat#first-run-credential-gate) and [Code → First-run credential gate](/cli/code#first-run-credential-gate).
...
See also [Keyless Local-First Run](/features/keyless-local-first-run) for the full deep-dive.
...
The second line is a reminder that a reachable local endpoint (Ollama or any OpenAI-compatible server) is adopted automatically — see [Keyless Local-First Run](/features/keyless-local-first-run).
...
<CardGroup cols={2}>
  <Card title="Setup Wizard" icon="key" href="/cli/setup">
  ...
  <Card title="Code Command" icon="code" href="/cli/code">
    Code assistant — first-run credential gate
  </Card>
  <Card title="Chat Command" icon="comments" href="/cli/chat">
