...
|------|----------------|---------|
| 1. Env vars (fast path) | Any credential env var declared in `PROVIDER_ENV_CATALOGUE` — `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `GEMINI_API_KEY`, `GROQ_API_KEY`, `COHERE_API_KEY`, `OPENROUTER_API_KEY`, `OLLAMA_HOST`, `MISTRAL_API_KEY`, `DEEPSEEK_API_KEY`, `XAI_API_KEY`, `TOGETHER_API_KEY` / `TOGETHERAI_API_KEY`, `PERPLEXITYAI_API_KEY`, `FIREWORKS_API_KEY` / `FIREWORKS_AI_API_KEY` (full [14-row table](/cli/setup#credential--default-model-map)) | `export MISTRAL_API_KEY=...` |
