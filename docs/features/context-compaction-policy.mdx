...
<Note>Looking for compaction **inside** a running tool loop? See [In-Loop Compaction](/features/context-compaction#in-loop-compaction).</Note>
...
Under the hood, the underlying `ContextCompactor` now periodically re-tests its anti-thrashing guard as conversations grow — see [Anti-Thrashing Protection](/features/context-compaction#anti-thrashing-protection).
...
<Note>
The `policy strategy value → compactor strategy` mapping in the "Maps to Legacy" column above is exposed as a single classmethod — `CompactionStrategy.from_policy_value(value)` — so custom callers don't need to hand-sync a dict.

```python
from praisonaiagents.compaction.strategy import CompactionStrategy

CompactionStrategy.from_policy_value("summarise")         # CompactionStrategy.SUMMARIZE
CompactionStrategy.from_policy_value("drop_oldest_tools") # CompactionStrategy.PRUNE
CompactionStrategy.from_policy_value("does_not_exist")    # CompactionStrategy.PRUNE (fallback)
```

Available since PraisonAI [#5197](https://github.com/MervinPraison/PraisonAI/pull/5197).
</Note>
...
<Note>
When the agent is bound to a `session_id`, the summaries produced by the policy-driven compactor are persisted automatically — so resume replays the compacted working history (summary + tail). See [Compacted Session Resume](/features/session-compaction-checkpoint).
</Note>
...
<Card title="Memory Flush on Compaction" icon="database-backup" href="/features/pre-compaction-memory-flush">
  Save durable facts at the older-slice boundary before the policy's strategy runs
</Card>
</CardGroup>