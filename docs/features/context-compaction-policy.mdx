...
<Note>
When the agent is bound to a `session_id`, the summaries produced by the policy-driven compactor are persisted automatically — so resume replays the compacted working history (summary + tail). See [Compacted Session Resume](/docs/features/session-compaction-checkpoint).
</Note>
...
<Card title="Compacted Session Resume" icon="bookmark" href="/docs/features/session-compaction-checkpoint">
