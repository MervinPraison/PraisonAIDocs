...
agent = Agent(name="monitor", instructions="Track and log all agent activities.")
agent.start("Show me the activity log for the last hour.")
```
...
The user asks for a recent audit trail; the agent queries the activity log and summarises workspace events.
...
<CardGroup cols={2}>
<Card title="Workspace Management" icon="building" href="/features/sessions">
  Learn about workspace organization and management
</Card>
<Card title="API Authentication" icon="key" href="/features/security-environment-variables">
