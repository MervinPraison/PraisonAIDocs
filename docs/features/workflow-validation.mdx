...
sidebarTitle: "Workflow Validation Loop"
...
validator = Agent(name="Validator", instructions="Check quality and request fixes")
workflow = AgentFlow(agents=[validator], steps=["validate_input", "run", "validate_output"])
workflow.start("Run with schema checks")
```

The user runs a workflow; validation steps guard inputs and outputs.
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Workflow Validation Loop
...
<Steps>
<Step title="Define guardrail and handlers">
```python
...
<Step title="Run workflow with guardrail retry">
```python
workflow = AgentFlow(
    steps=[
        Task(
...
For YAML structural checks (duplicate names, unknown `agent:` on a task), see [YAML Validation](/features/yaml-validation).

## Related

<CardGroup cols={2}>
  <Card title="YAML Validation" icon="shield-check" href="/features/yaml-validation">
    Duplicate names and unknown task→agent reference checks
  </Card>
  ...
  <Card title="Autonomous Workflows" icon="robot" href="/features/autonomous-workflow">
    Environment-based feedback loops
  </Card>
