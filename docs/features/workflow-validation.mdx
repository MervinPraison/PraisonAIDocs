...
Validation loops automatically retry steps with feedback until output meets quality standards.
...
<AccordionGroup>
<Accordion title="Define specific, measurable validation criteria">
Vague criteria lead to inconsistent validation. Define explicit, checkable rules:
...
<Accordion title="Ensure validators provide actionable feedback">
Feedback that loops back into the generator step should be specific enough for the generator to act on:
...
<Accordion title="Always set retry limits">
Prevent infinite validation loops by using `max_retries` on `Task` or implementing escalation logic when the retry count is exceeded.
</Accordion>
</AccordionGroup>
