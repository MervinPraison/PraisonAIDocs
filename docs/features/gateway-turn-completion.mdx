...
description: "Surface why a bot turn ended (max_steps, cancelled, error, provider blocks) as a user-safe note"
icon: "message-check"
---
...
graph LR
    subgraph "Turn Completion Notes"
        ...
        R -->|max_steps / cancelled / error<br/>content_filtered / refused / length_truncated| Wrap[📝 append user-safe note]
        Wrap --> Out[💬 Reply + note]
    end
...
sequenceDiagram
    ...
    else max_steps / cancelled / error / content_filtered / refused / length_truncated
...
| `content_filtered` | `⏳ This turn ended early — please try again.` (generic fallback) | Provider's content filter blocked the response |
| `refused` | `⏳ This turn ended early — please try again.` (generic fallback) | Model declined to answer (safety refusal) |
| `length_truncated` | `⏳ This turn ended early — please try again.` (generic fallback) | Response hit the model's output length limit |
| *(any other string)* | `⏳ This turn ended early — please try again.` | Forward-compat fallback for future reasons |

`.truncated` is `True` for anything not in `{"completed", ""}` — so the three provider-block reasons always surface a note.

<Note>
The three provider-block reasons (`content_filtered` / `refused` / `length_truncated`) are recognised as non-clean stops and surface a note, but they currently degrade to the generic fallback rather than a per-reason message. Set a `detail` for a tailored, user-safe note — never leak refusal text:

| `last_stop_reason` | Suggested `detail` |
|---|---|
| `content_filtered` | "This response was blocked by the provider's content filter." |
| `refused` | "The model declined to answer this request." |
| `length_truncated` | "The response was cut off before finishing." |

See [Run Outcome → Provider block outcomes](/features/run-outcome#provider-block-outcomes) for where each reason comes from.
</Note>
...
<Card title="Error Handling" icon="triangle-alert" href="/features/error-handling">
