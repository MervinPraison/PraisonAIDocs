...
When the tool-calling loop reaches its budget — `ExecutionConfig.max_steps` (or `max_iter` when `max_steps` is unset) — the agent injects a wrap-up instruction on the final step. The model returns a real summary answer describing what it accomplished and what remains, not a placeholder.
...
<Card icon="gauge-max" href="/docs/features/max-steps">
  Step Budget — cap tool-use steps and detect graceful truncation
</Card>
