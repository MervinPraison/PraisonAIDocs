...
A tool failure now raises `ToolExecutionError` on the sync path, and returns an error dict to the model on the async path.

```mermaid
graph TB
    subgraph "Tool failure — sync path"
        SReq[👤 chat request] --> SLLM[🤖 Model calls tool]
        SLLM --> STool[🔧 Tool raises]
        STool --> SRaise[🛑 ToolExecutionError propagates]
    end
    subgraph "Tool failure — async path"
        AReq[👤 achat request] --> ALLM[🤖 Model calls tool]
        ALLM --> ATool[🔧 Tool raises]
        ATool --> ADict[📄 error dict returned to model]
    end
    ...
    class SReq,AReq req
    class SLLM,ALLM,STool,ATool proc
    class SRaise err
    class ADict ok
```

Wrap sync `chat()` / `start()` calls in `try / except ToolExecutionError`:
