...
<Card title="RuntimeConfig SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/feature_configs">
...
<Note>
`preferred_runtime` values are canonicalised (case-insensitive, whitespace-tolerant, `-`/`_` interchangeable). Canonical names are `native` and `plugin-harness`; aliases like `harness`, `plugin`, and `reduced` map to `plugin-harness`. A close typo of a known runtime (e.g. `nativ`) raises `ValueError`. See [Runtime name normalisation](/features/runtime#runtime-name-normalisation).
</Note>
...
Check the [Runtime Capabilities](/features/runtime-capabilities) page for the list of valid capability names supported by each runtime.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="Runtime Capabilities" icon="bolt" href="/features/runtime-capabilities">
  List of all available runtime capabilities
</Card>
<Card title="Runtime Preflight" icon="clipboard-check" href="/features/runtime-preflight">
  Pre-execution runtime validation
</Card>
<Card title="Runtime MCP" icon="plug" href="/features/runtime-mcp">
  MCP tool runtime configuration
</Card>
<Card title="Agent Runtime Protocol" icon="cpu" href="/features/agent-runtime-protocol">
