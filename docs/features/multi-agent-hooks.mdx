...
<Note>
`process` accepts `"sequential"`, `"hierarchical"`, or `"workflow"`. Passing any other value (including `"parallel"`) raises `ValueError` at `PraisonAIAgents(...)` construction. To fan out tasks in parallel, keep `process="workflow"` and set `async_execution=True` on each `Task`.
</Note>
...
<Card title="MultiAgentHooksConfig SDK Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/feature_configs">
...
| `on_task_complete` | `Callable \| None` | `None` | Called after a task finishes. Signature: `(task, task_output) -> None`. May be sync or `async def`. Note: a **per-`Task`** `on_task_complete` uses a *different* signature — `(task_output)` or `(task_output, metadata)`, **not** `(task, task_output)`. See [Human-in-the-Loop Review](/features/human-in-the-loop-review). |
| `completion_checker` | `Callable \| None` | `None` | Custom validator. Signature: `(task, task_output) -> bool` |

<Note>
**Team-level vs per-`Task` `on_task_complete` — different signatures.** Per-`Task` callbacks (set on the `Task(...)` constructor) take **either**:

- `on_task_complete(task_output)` — 1 param, or
- `on_task_complete(task_output, metadata)` — 2 params, where `metadata` is a dict with `task_id` and `task_name`.

The 2-param form is `(task_output, metadata)` — **NOT** `(task, task_output)` like the `AgentTeam`-level hook above. Copy-pasting a team-style callback onto a `Task` silently binds the wrong objects (guarded by a regression test in PraisonAI PR #3668). Full comparison: [Human-in-the-Loop Review](/features/human-in-the-loop-review).
</Note>
...
<Card title="Callbacks" icon="rotate-cw" href="/features/callbacks">
...
<Card title="Human-in-the-Loop Review" icon="user-check" href="/features/human-in-the-loop-review">
  Gate a sequential team on human approval between tasks
</Card>
</CardGroup>
