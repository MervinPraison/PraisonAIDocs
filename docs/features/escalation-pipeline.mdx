...
- **Loop guard** — idempotency-aware tool-call guardrails on every `Agent`. See [Loop Guard](/features/loop-guard).
...
<Accordion title="Override a false-positive doom-loop stop">
...
agent = Agent(
    instructions="Poll the build until it finishes.",
    permission_manager=PermissionManager(rules={"doom_loop": "allow"}),
)
```

The gate is **fail-closed**: deny, timeout, no backend, or any error falls back to the historical hard-stop. See [Approval](/features/approval#doom-loop-safety-gate).
</Accordion>
...
<CardGroup cols={2}>
<Card title="Doom Loop Detection" icon="rotate" href="/features/doom-loop-detection">
