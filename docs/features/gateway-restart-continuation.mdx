...
Alongside interrupted-turn resumption, boot also reconciles the durable [Run Ledger](/features/run-ledger). For **background runs** (long-running agent jobs kicked off from a channel), the gateway calls `recover_orphans()` right after the session scan, marking each still-active run `LOST` and posting a **durable, exactly-once** restart notice back to that thread's origin.
...
sequenceDiagram
    ...
    Process->>Process: resume interrupted turns
...
The two mechanisms are complementary: the **session scan** re-drives turns that were mid-flight; the **ledger recovery** terminalises background runs that a crash left `running` and tells their origin.
...
<Info>
**Default-safe.** If your gateway never used the run ledger, boot recovery is a no-op — nothing scans, nothing sends. `SQLiteRunLedger` at `~/.praisonai/runs/ledger.db` is created the first time a run is upserted; boot recovery activates from that point on. See [Run Ledger → Behaviour on gateway restart](/features/run-ledger#behaviour-on-gateway-restart).
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Run Ledger" icon="database" href="/features/run-ledger">
    Durable run state — `recover_orphans()` runs on boot alongside turn resumption
  </Card>
</CardGroup>
