...
<Note>
`workdir` is expanded by the **remote** shell — `~/.praisonai-train` lands in the remote user's home, not yours. It is created with `mkdir -p -m 700`, so on a shared GPU box the umask does not decide who can read the shipped config and dataset. Only letters, digits, `.`, `_`, `-`, `/`, and an optional leading `~` are allowed; the runner refuses anything else before any SSH. See PraisonAI [PR #4550](https://github.com/MervinPraison/PraisonAI/pull/4550) for the tilde-expansion fix and the mode-`700` hardening.
</Note>
...
<Step title="A local dataset is copied; a Hub dataset is not">

The dispatcher walks the resolved `dataset` — a string or a list-of-mappings — and copies the first local file it finds, whether it was named on the CLI or only inside `config.yaml`. A HuggingFace id or a path that lives only on the remote host is not a local file, so it is left alone. Name a local file explicitly with `data_files` when you want `name` to stay a label.

| You wrote | Ships? |
|-----------|--------|
| `dataset: "d.jsonl"` (local file) | Yes — copied to the run dir. |
| `dataset: [{name: "d.jsonl"}]` (local file) | Yes — `name` is the path, exactly as the trainer treats it. |
| `dataset: [{name: "my-set", data_files: "d.jsonl"}]` | Yes — `data_files` is what ships; `name` stays the label. |
| `dataset: [{name: "org/alpaca-cleaned"}]` (Hub id) | No — the far side loads it from the Hub. |
| `dataset: "/remote/only/path.jsonl"` (only on the host) | No — nothing to copy. |

A `dataset: "path.jsonl"` string is normalised to `[{name: "path.jsonl"}]` in the shipped config for the remote trainer.
...
The `remote:` block appears in the preview with the host, interpreter, workdir, and GPU count the run would actually use:
...
A bad `remote:` block fails here too — `--dry-run` resolves and validates it, so a malformed block exits `1` with `Bad remote settings` before dispatch rather than after an hour of rented GPU.
...
<Note>
A remote run that ends `failed (exit N)` now exits `1` locally too, so `$?` and CI gating see the failure. Earlier the CLI printed the status and then exited `0`, reporting success for a run that had died.
</Note>
...
| `remote: []` *(an empty list)* | Refused — `remote: must be a mapping of key: value`. An omitted key (or `remote: null`) means "train here". |
| `remote: ""` *(an empty string)* | Same refusal — a present-but-falsey value is a malformed block, not an absence. |
| `remote: false` | Same refusal. |
