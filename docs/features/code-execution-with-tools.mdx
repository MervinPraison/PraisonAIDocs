...
## Tools must also be granted to the agent
...
# fetch is on the allow-list but NOT in tools=[] → unreachable, run fails
agent = Agent(
    name="broken",
    instructions="Fetch and summarise.",
    tools=[],                                   # <- fetch missing here
    execution=ExecutionConfig(
        code_execution=True,
        code_mode="unsafe",
        code_tools=True,
        code_tools_allow=["fetch"],             # allow-list alone is not enough
    ),
)
```

Grant the tool on the agent as well as the allow-list:

```python
agent = Agent(
    name="working",
    instructions="Fetch and summarise.",
    tools=[fetch],                              # <- granted here
    execution=ExecutionConfig(
        code_execution=True,
        code_mode="unsafe",
        code_tools=True,
        code_tools_allow=["fetch"],
    ),
)
```

<Warning>
`code_tools` is only reachable in `code_mode="unsafe"`. In `"safe"` mode the code runs in a subprocess where your tools do not exist, so `code_tools_allow` is ignored and `code_tools=True` emits a `UserWarning`.
</Warning>

<Warning>
**Unsafe mode does not enforce `timeout`.** In `code_mode="unsafe"` the code runs in the current process, so a runaway loop blocks the worker with no hard cut-off. Use `code_mode="safe"` whenever the timeout must be a hard guarantee.
</Warning>

<Note>
Every code-mode tool proxy passes the approval gate on **each call**, not once per run. Approving `fetch` once inside a script does not unlock later `fetch(...)` calls with different arguments — each is gated independently (YAML / env auto-approve still apply as configured).
</Note>
...
agent = Agent(
    ...
    execution=ExecutionConfig(
        code_execution=True,
        code_mode="unsafe",  # code reaches tools only in unsafe mode
        code_tools=True,
...
<Warning>
`code_tools_allow` is a **filter, not a grant**. Every named tool must **also** be present in `Agent(tools=[...])`. Code mode is scoped to a private registry built from the agent's own tools, so a globally-registered plugin the agent was never given cannot be reached.

```python
# ❌ tool is allow-listed but never given to the agent → unreachable, run fails
Agent(
    instructions="…",
    tools=[],  # fetch not granted
    execution=ExecutionConfig(code_execution=True, code_mode="unsafe",
                              code_tools=True, code_tools_allow=["fetch"]),
)
```
</Warning>
...
<Warning>
`"unsafe"` code mode does **not** enforce `timeout`. The code runs in the current process, so a runaway loop blocks the worker with no hard cut-off. Use `code_mode="safe"` (subprocess) when the timeout must be a hard guarantee — there `ExecutionConfig(timeout=...)` is a real per-call limit on `execute_code`.
</Warning>
...
- **Every tool-proxy call passes the approval gate** — the gate fires on **each** invocation, not once per run. `require_approval`, allow-lists, and `PRAISON_AUTO_APPROVE` env var all apply on every call the model's code makes.
