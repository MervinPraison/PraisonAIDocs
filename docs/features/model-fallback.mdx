...
Many users share a single `Agent` across `asyncio.gather` (see [Thread-Safe Agent State → Chat History](/features/thread-safety#chat-history)). Before this fix, a mid-fallback shared-state mutation could silently flip a concurrent turn to the fallback model — wrong cost, wrong logs, wrong hook payloads. Both paths now keep each turn on its own model.
...
<Card title="Thread-Safe Agent State" icon="lock" href="/features/thread-safety">
