...
<Accordion title="Bound a monitor that watches for a one-off event">
Recurring monitors can be bounded — see [`max_runs` and `until`](/tools/schedule-tools#schedule_add) — so a change-detection job that watches for a one-off event can retire itself instead of ticking forever.
</Accordion>
