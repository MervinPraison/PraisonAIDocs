...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `no_change` | `bool` | `False` | `True` when the watched source was unchanged. Suppressed silently and recorded as `no_change`. **Implies `run=False`** (enforced in `__post_init__`) |
| `state_updates` | `Optional[Dict[str, Any]]` | `None` | Bounded key/value to persist for the next tick (last-seen hash, watermark). `None` means "write nothing" |
