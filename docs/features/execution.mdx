...
`code_mode="safe"` is the right default for `code_execution=True`. Do **not** rely on `code_sandbox_mode` for isolation — the field is currently NOT IMPLEMENTED and setting it to `"sandbox"` provides no actual sandboxing. For real isolation of model-generated code, use `Agent(sandbox=…)`; for a whole workflow, use `AgentFlow(run_on="docker")` / `run_on="e2b"`.
</Accordion>
