...
`code_mode="safe"` is the right default for `code_execution=True`. Do **not** rely on `code_sandbox_mode` for isolation — the field is currently NOT IMPLEMENTED and setting it to `"sandbox"` provides no actual sandboxing. `Agent(sandbox=…)` only isolates explicit `agent.execute_code(...)` calls — it does **not** give the model a tool. For isolation of code the model runs, use `AgentFlow(run_on="docker")` / `run_on="e2b"` (whole workflow) or `LocalAgent(compute="docker")` (per-agent).
</Accordion>
