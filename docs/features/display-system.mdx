...
<Note>
This page covers **terminal** output only. For per-channel presentation (streaming, tool progress, footers), see [Display Policy](/features/display-policy).
</Note>
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Display System
...
Event names, autonomy callbacks, and async patterns are documented on the [Display Callbacks](/features/display-callbacks) page.
</Accordion>
</AccordionGroup>
...
<Card title="Output Config" icon="sliders" href="/configuration/output-config">
