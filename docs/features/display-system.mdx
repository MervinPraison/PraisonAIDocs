...
| `agent` | `str` | Name of the agent that ran the task |
| `json_dict` | `dict` | Parsed JSON when structured output is enabled |
| `pydantic` | `BaseModel` | Parsed model when a schema is set |
| `output_format` | `str` | `"RAW"`, `"JSON"`, or `"Pydantic"` |
| `summary` | `str` | Optional summary of the result |
| `token_metrics` | `TokenMetrics` | Token usage metrics |
...
| `satisfactory` | `Literal["yes", "no"]` | Whether reflection passed |
