...
<Note>
On the channel `send_message` contract, the typed [`SendResult.status`](/features/bot-platform-adapter#branching-on-delivery-outcome) (`"sent" | "failed" | "queued" | "duplicate"`) is the machine-checkable expression of this guarantee — every send resolves to one of four named outcomes, so a silent drop is unrepresentable rather than merely discouraged.
</Note>
