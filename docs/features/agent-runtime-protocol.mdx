...
agent = Agent(name="runtime-agent", instructions="Execute tasks via the runtime protocol.")
agent.start("Process this request using the standard runtime protocol.")
```


One protocol, many runtimes — pick the engine that runs a single agent turn without changing your Agent code.

The user swaps runtimes without changing agent code; the registry resolves the engine and streams `RuntimeResult` deltas.
...
<CardGroup cols={2}>
<Card icon="cloud" href="/features/managed-runtime-protocol">Run the whole agent loop on remote provider infrastructure</Card>
<Card icon="bolt" href="/features/runtime-selection">Select runtimes by model or provider at the YAML level</Card>
<Card icon="route" href="/features/runtime-resolution">Resolve and cache runtimes at turn time</Card>
<Card icon="plug" href="/features/load-mcp-tools">Plug in tools from any Model Context Protocol server</Card>
</CardGroup>
