...
| `JOB_COMPLETED` | Background job reaches terminal state | `JobCompletedInput` | `job_info` (`job_id`, `status`, `result`/`error`, `duration`, `origin`) | Observability, custom delivery routing, external side effects |
