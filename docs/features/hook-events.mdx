...
| `SESSION_PERSIST_FAILED` | A durable session write failed (disk-full / SQLite corruption / permission / `OSError`) | `SessionPersistFailedInput` | **Observe-only** — alert/metric on silent data-loss; the turn is salvaged to a spill file and re-ingested on next load |
...
def on_persist_failed(event_data):
    # Mirror to metrics/alerting so a silent data-loss failure is observable.
    print(
...
agent = Agent(name="MyAgent", instructions="Be helpful.")
agent.start("Do something that would append a turn.")
```
...
| `SESSION_PERSIST_FAILED` | Session | A durable session write failed — observe-only; turn is salvaged to a spill file and re-ingested on next load (see [Write-failure salvage](/features/session-persistence#write-failure-salvage)) |
