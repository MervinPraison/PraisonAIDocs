...
| `KANBAN_TASK_MOVED` | `KanbanHookInput` | Task status changed (includes auto-promotion: dispatcher fires this with `to_status='ready'` for each dependent task whose parents are all terminal) |
