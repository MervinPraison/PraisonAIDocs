...
| `MESSAGE_RECEIVED` | Message | Inbound gate — drop (deny) or redact/rewrite before agent dispatch (see [Inbound Message Gate](/features/inbound-message-gate)) |
| `MESSAGE_SENDING` | Message | Outbound gate — cancel or rewrite before sending |
| `MESSAGE_SENT` | Message | After message sent |
| `MESSAGE_UNDELIVERED` | Message | Reply permanently undeliverable — operator routing (see [Undelivered Message Notice](/features/undelivered-messages)) |
