...
<Note>
**`AFTER_TOOL` result may be an error payload.** If the tool raised, `event_data.result` is `{"error": "<message>"}`; if it returned a non-JSON-serializable value (`datetime`, `set`, `bytes`, a custom class), it is `{"result": "<str(value)>"}`. The run continues either way (PR #3855), so `AFTER_TOOL` handlers should tolerate both shapes.
</Note>
