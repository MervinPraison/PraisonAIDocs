...
<Note>
Plugin persisters subscribe to `AFTER_COMPACTION` to read `event_data.summary`. See [Compacted Session Resume](/docs/features/session-compaction-checkpoint) for the built-in checkpoint that ships with PraisonAI, and [Context Compaction](/docs/features/context-compaction#new-compactionresult-fields) for the full `CompactionResult` fields.
</Note>
...
| `MESSAGE_UNDELIVERED` | Message | Reply permanently undeliverable — operator routing (see [Undelivered Message Notice](/docs/features/undelivered-messages)) |
