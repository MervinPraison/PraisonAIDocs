...
<Note>
This page covers **in-process lifecycle hooks** (SETUP, SESSION_START, BEFORE_AGENT, etc.) that fire inside a running agent. For **HTTP inbound triggers** that start agent runs from external services via `POST /hooks/<path>`, see [Gateway Inbound Hooks](/features/gateway-inbound-hooks).
</Note>
