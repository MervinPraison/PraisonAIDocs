...
`TOOL_RESULT_PERSIST` resolves to the same enum member as `AFTER_TOOL` — there is no separate persistence chokepoint. Subscribe to `AFTER_TOOL`, which already sees the tool result before it is written into the conversation.
