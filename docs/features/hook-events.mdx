...
| Event | Trigger | Input Type | Use Case |
|-------|---------|------------|----------|
| `REACTION_RECEIVED` | User adds or removes a reaction; `kind` disambiguates | `PlatformEventInput` | Reaction-as-approval, quick polls, moderation |
| `MESSAGE_EDITED` | User edits a previously-sent message | `PlatformEventInput` | Refresh cached content, re-summarise, invalidate downstream artefacts |
| `MESSAGE_DELETED` | User deletes a message | `PlatformEventInput` | Retract stored plans, cancel scheduled follow-ups |
| `MEMBER_JOINED` | User joins a channel/guild | `PlatformEventInput` | Welcome message, onboarding, seat provisioning |
| `MEMBER_LEFT` | User leaves a channel/guild | `PlatformEventInput` | Offboarding, cleanup |
| `THREAD_CREATED` | New thread created | `PlatformEventInput` | Thread-scoped session, per-thread agent config |

<Note>
These events are **opt-in per channel** via `events:` in the gateway config and currently wired on the Discord adapter. See [Inbound Platform Events](/features/bot-inbound-events) for the full walk-through.
</Note>
...
Schedule hooks fire when scheduled jobs are managed and triggered by `BotOS`. `JOB_COMPLETED` fires from `BackgroundJobManager.start_job` for **every** managed background job when it reaches a terminal state (`COMPLETED` or `FAILED`) — best-effort (a raising handler cannot crash the worker).
...
| `JOB_COMPLETED` | Any managed background job reaches a terminal state (`COMPLETED` or `FAILED`) | `JobCompletedInput` | `job_id`, `status`, `result`, `error`, `agent_name`, `session_id`, `deliver`, `platform`, `chat_id`, `thread_id` | Observability, custom delivery routing, chat-back delivery (see [Background Subagents](/features/background-subagents)) |
...
`JOB_COMPLETED` fires from `BackgroundJobManager.start_job` for **every** managed background job when it reaches a terminal state (`COMPLETED` or `FAILED`) — not only jobs launched via `spawn_subagent(background=True)`. It does **not** fire for `CANCELLED` or `LOST`. Best-effort: a raising handler cannot crash the worker.
...
add_hook("job_completed", log_job_result)
```

#### `JobCompletedInput` Fields

The payload carries flat, typed fields — origin fields are always strings (empty string when unset).
...
| `agent_name` | `str` | Always `"background"` |
| `session_id` | `str` | Origin session (empty when unset) |
| `deliver` | `str` | Delivery target (empty when the job was not routed) |
| `platform` | `str` | Origin platform |
| `chat_id` | `str` | Origin chat/channel ID |
| `thread_id` | `str` | Origin thread ID |
...
<Note>
**Where do these events come from?** Every writer — the CLI (`praisonai kanban ...`), agent tools, the gateway dispatcher, the HTTP API and the UI — goes through the SQLite store. The store is the single emission chokepoint, so one `fire_hook` per transition covers every writer. You don't call `fire_hook` yourself for these; subscribe with `add_hook` and the store emits.
</Note>

| Event | Emitted From | Use Case |
|-------|--------------|----------|
| `KANBAN_TASK_CREATED` | `create_task`, after commit | Task added to board |
| `KANBAN_TASK_CLAIMED` | `claim_task`, only when the CAS won | Task assigned |
| `KANBAN_TASK_MOVED` | `move_task` (every status change), `reclaim_stale_claims`, dispatcher promotion | Task status changed |
| `KANBAN_TASK_DONE` | `move_task` when destination is `done` (rides alongside `MOVED`) | Task completed |
| `KANBAN_TASK_BLOCKED` | `move_task` when destination is `blocked`, and `record_failure` on a circuit-breaker trip | Task blocked |
| `KANBAN_TASK_FAILED` | `record_failure` — one per recorded attempt failure | Task failed |

Every kanban event delivers a `KanbanHookInput`. These keys are always present: `task_id`, `board`, `status`, plus `to_status` on every transition and `from_status` on everything except `CREATED`.

| Key | On | Notes |
|-----|-----|-------|
| `assignee` | most | `worker_id` for `CLAIMED`/reclaim, current assignee for `MOVED`/`DONE`/`BLOCKED`. |
| `task` | `CREATED`/`MOVED`/`DONE`/`BLOCKED` | Full `task.to_dict()`. Absent on `FAILED` and reclaim moves. |
| `reclaimed` | `MOVED` (reclaim only) | `True` when a stale claim was reclaimed. |
| `reason` | `MOVED` (reclaim only) | `"dead"` or `"stale_heartbeat"`. |
| `worker_pid` | `CLAIMED` | The claiming worker's PID. |
| `claim_expires` | `CLAIMED` | ISO-8601 UTC timestamp. |
| `consecutive_failures` | `FAILED`, `BLOCKED` (on break) | Attempt counter. |
| `max_retries` | `FAILED`, `BLOCKED` (on break) | Retry limit for the task. |
| `circuit_broken` | `FAILED`, `BLOCKED` (on break) | `True` when this failure tripped the breaker. |
| `error` | `FAILED`, `BLOCKED` (on break) | First 500 chars of the failure text. |

```python
from praisonaiagents.hooks import add_hook, HookResult

def track_progress(data):
    print(f"Task {data.task_id}: {data.from_status} → {data.to_status}")
    return HookResult.allow()

add_hook("kanban_task_moved", track_progress)
```

A subscriber runs the moment a user changes the board.
...
sequenceDiagram
    ...
    Plugin-->>User: Logs the new task
```

#### Circuit-breaker double-emit

A `record_failure` that trips the breaker fires **both** `KANBAN_TASK_FAILED` **and** `KANBAN_TASK_BLOCKED`, in that order. A subscriber on both events sees one of each for the same transition — use `circuit_broken` to detect it.

```python
from praisonaiagents.hooks import add_hook, HookResult

def on_failed(data):
    if data.extra.get("circuit_broken"):
        print(f"Task {data.task_id} tripped the breaker; BLOCKED follows")
    return HookResult.allow()

add_hook("kanban_task_failed", on_failed)
```

#### Reclaim

Reclaiming a stale claim (`running → ready`) fires `KANBAN_TASK_MOVED` with `reclaimed=True` and `reason` in `{"dead", "stale_heartbeat"}`. There is no separate reclaim event — reclaim *is* a move.

```python
from praisonaiagents.hooks import add_hook, HookResult

def on_reclaim(data):
    if data.extra.get("reclaimed"):
        print(f"Task {data.task_id} reclaimed ({data.extra.get('reason')})")
    return HookResult.allow()

add_hook("kanban_task_moved", on_reclaim)
```

<Note>
**Merge conflicts are recorded on the task comment thread, not the hook payload.** When a worktree task fails to merge cleanly, the conflicted files are written to the task's comments (read via `kanban_show(task_id)`) — the `KANBAN_TASK_BLOCKED` payload does not carry `conflicted_files`. See [Kanban → Per-Task Worktree Isolation](/docs/features/kanban#per-task-worktree-isolation).
</Note>
...
<Note>
Dispatcher-emitted auto-promotion events do **not** include `from_status`, while store-emitted `MOVED` events always do. Check for its absence to distinguish an auto-promotion from a manual move.
</Note>

```python
from praisonaiagents.hooks import add_hook, HookResult

def on_auto_promotion(data):
    if data.to_status == "ready" and not data.from_status:
        print(f"Task {data.task_id} auto-promoted to ready")
    return HookResult.allow()

add_hook("kanban_task_moved", on_auto_promotion)
```
...
| `REACTION_RECEIVED` | Platform | User adds or removes a reaction (`kind` disambiguates); opt-in via `events: [reactions]` — see [Inbound Platform Events](/features/bot-inbound-events) |
| `MESSAGE_EDITED` | Platform | User edits a message; `new_text` carries the new content; opt-in via `events: [edits]` |
| `MESSAGE_DELETED` | Platform | User deletes a message; opt-in via `events: [edits]` |
| `MEMBER_JOINED` | Platform | User joins a channel/guild; opt-in via `events: [members]` (privileged intent) |
| `MEMBER_LEFT` | Platform | User leaves a channel/guild; opt-in via `events: [members]` |
| `THREAD_CREATED` | Platform | New thread created; `thread_id` populated; opt-in via `events: [threads]` |
