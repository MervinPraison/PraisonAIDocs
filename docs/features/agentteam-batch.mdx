...
| `total` | `int` | `len(items)`. |
| `token_usage_total` | `dict` | **This team's** cumulative token usage from `PraisonAIAgents.get_token_usage_summary()` across the whole run — **not** batch-item-scoped. Since PraisonAI PR [#4462](https://github.com/MervinPraison/PraisonAI/pull/4462) it is scoped to this instance's own named agents, so concurrent teams do not leak into each other. |
...
`token_usage_total` comes from `PraisonAIAgents.get_token_usage_summary()` and reflects **this team's** cumulative usage across the whole run (not just the batch item). Since PraisonAI PR [#4462](https://github.com/MervinPraison/PraisonAI/pull/4462) it is scoped to this instance's own named agents, so concurrent teams do not leak into each other. For per-item accounting, still reset the team between batches or inspect per-item outputs.
</Accordion>
