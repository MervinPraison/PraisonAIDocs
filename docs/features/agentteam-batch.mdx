...
<Warning>
`AgentTeam` / `PraisonAIAgents` accepts `guardrails`, `web`, `reflection`, `caching`, `learn`, and `knowledge` kwargs for API symmetry with `Agent`, but does **not yet apply** them at the team level. Since PraisonAI PR [#4004](https://github.com/MervinPraison/PraisonAI/pull/4004), passing a truthy value for any of the six now logs a warning telling you to set them on each individual `Agent(...)` instance instead. `autonomy=` on `AgentTeam` remains fully wired — this caveat does not apply to it.
</Warning>
...
| `total` | `int` | `len(items)`. |
| `token_usage_total` | `dict` | **This team's** cumulative token usage from `PraisonAIAgents.get_token_usage_summary()` across the whole run — **not** batch-item-scoped. Since PraisonAI PR [#4462](https://github.com/MervinPraison/PraisonAI/pull/4462), finalized in [#4470](https://github.com/MervinPraison/PraisonAI/pull/4470) (fixes [#4446](https://github.com/MervinPraison/PraisonAI/issues/4446)), it is scoped to this instance's own named agents, so concurrent teams do not leak into each other. |
...
writer = Agent(name="Writer", instructions="Write a short, factual bio")
task = Task(description="Write a short bio for {{name}}",
            expected_output="A 2-3 sentence bio", agent=writer)
team = AgentTeam(agents=[writer], tasks=[task])

async def main():
    # ❌ Same instance twice at once → RuntimeError
    await asyncio.gather(team.astart(), team.astart())

asyncio.run(main())
```

Construct one instance per concurrent run instead — they are cheap to build:
...
def make_team():
    ...
    task = Task(description="Write a short bio for {{name}}",
                expected_output="A 2-3 sentence bio", agent=writer)
    return AgentTeam(agents=[writer], tasks=[task])

async def main():
    # ✅ One instance per concurrent run
    await asyncio.gather(make_team().astart(), make_team().astart())

asyncio.run(main())
```

`start_for_each` and `astart_for_each` are unaffected: they hold the guard for the whole batch and re-enter it per item on the same thread and task, so batched runs keep working exactly as before.

<Note>
**Fix reference:** [PraisonAI PR #5121](https://github.com/MervinPraison/PraisonAI/pull/5121). The re-entrancy guard now tracks ownership via a `contextvars.ContextVar` (`_run_owner`) with a compound identity `(id(self), threading.get_ident(), id(asyncio.current_task()))`. Before #5121, the guard read `threading.get_ident()` alone — every asyncio Task on one event loop shares a single OS thread id, so two coroutines calling `astart()` on the same team under `asyncio.gather` both took the re-entrant branch and bypassed the lock. Nested `astart()` on the same task still re-enters (batches keep working); a child task copying the parent context can no longer falsely re-enter (the task-id pin catches it).
</Note>
...
See [Concurrency](/features/concurrency) for the full list of concurrency guarantees.
</Note>
...
<Accordion title="token_usage_total is team-cumulative, not batch-item-scoped">
`token_usage_total` comes from `PraisonAIAgents.get_token_usage_summary()` and reflects **this team's** cumulative usage across the whole run (not just the batch item). Since PraisonAI PR [#4462](https://github.com/MervinPraison/PraisonAI/pull/4462), finalized in [#4470](https://github.com/MervinPraison/PraisonAI/pull/4470) (fixes [#4446](https://github.com/MervinPraison/PraisonAI/issues/4446)), it is scoped to this instance's own named agents, so concurrent teams do not leak into each other. For per-item accounting, still reset the team between batches or inspect per-item outputs.
</Accordion>
