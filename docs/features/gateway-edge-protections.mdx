...
<Note>
**Loopback is always exempt.** Connections from `127.0.0.1`, `::1`, `localhost`, or a gateway bound to loopback bypass the pre-auth budget entirely. Your local dev workflow is never affected.
</Note>
...
async def connect_with_backoff(uri, token):
    backoff = 1.0
    while True:
        try:
            async with websockets.connect(uri) as ws:
                backoff = 1.0  # reset on success
                await ws.send(f'{{"type":"hello","token":"{token}"}}')
                async for message in ws:
                    handle(message)
        except websockets.exceptions.ConnectionClosedError as e:
            if e.code == CLOSE_CODE_UNAUTHORIZED_FLOOD:
                # Token/scope problem — fix first, then back off
                ...
                # Too many pre-auth connections — back off and retry
