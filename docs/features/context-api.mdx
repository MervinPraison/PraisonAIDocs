...
sidebarTitle: "Context Management API"
...
mutator = store.get_mutator("agent-1")
mutator.append({"role": "user", "content": "hi"})
mutator.commit()   # nothing is stored until you commit
```
...
# Process A — dump the store to bytes
data = get_global_store().snapshot()
open("store.bin", "wb").write(data)

# Process B — rebuild the store from those bytes
store = get_global_store()
store.restore(open("store.bin", "rb").read())
```
...
mutator = store.get_mutator("agent-1")
mutator.append({"role": "user", "content": "hi"})
mutator.commit()  # nothing is stored until commit()
```

**Cross-process sharing** with `snapshot()` / `restore()`:

```python
from praisonaiagents.context import get_global_store, ContextStoreImpl

# Process A — serialise the store
blob = get_global_store().snapshot()  # bytes: agent history + shared context

# Process B — rebuild it
store_b = ContextStoreImpl()
store_b.restore(blob)
```
