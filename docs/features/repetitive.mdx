...
<Steps>
    ...
    <Step title="Create a file">
        ...
        # Create workflow with loop over list
        workflow = Workflow(
        ...
        def process_row(ctx: WorkflowContext) -> StepResult:
            ...
            return StepResult(output=f"Processed {name}: {issue}")

        workflow = Workflow(steps=[
            loop(process_row, from_csv="tasks.csv")
        ])

        result = workflow.start("Process CSV")
        ```
    </Step>
