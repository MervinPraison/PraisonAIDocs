...
<Warning>
`/learn` is **admin-only by default** when a `CommandAccessPolicy` is configured. See [Bot Command Access Control](/features/bot-command-access-control).
</Warning>
</Step>
...
See [Bot Command Access Control](/features/bot-command-access-control) to configure `admin_users` and grant selective access.
...
<CardGroup cols={2}>
  <Card title="Skill Management" icon="puzzle-piece" href="/features/skill-manage">
  ...
  <Card title="Self Improve" icon="chart-line" href="/features/self-improve">
