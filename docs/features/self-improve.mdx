...
<Step title="Background Mode">
For long-lived chatbots or gateway agents, defer the review so the user reply is not gated on an extra LLM turn.
...
# Reply returns immediately; the skill-review runs after, in the background.
```
</Step>
</Steps>
...
Unknown strings (e.g. typos like `"backround"`) disable self-improvement and log a warning.
...
agent = Agent(name="Bot", self_improve="background")
```
</Accordion>
</AccordionGroup>
