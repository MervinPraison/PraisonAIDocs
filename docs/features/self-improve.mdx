...
```python
from praisonaiagents import Agent
from praisonaiagents.skills import DefaultSkillReviewPolicy

agent = Agent(
    name="Engineer",
    instructions="Help debug and fix issues.",
    self_improve=DefaultSkillReviewPolicy(min_tool_calls=2),
)
```
</Step>
</Steps>
...
agent = Agent(
    ...
    self_improve=DefaultSkillReviewPolicy(min_tool_calls=3),
)
```
...
```python
from praisonaiagents import Agent
from praisonaiagents.skills import SkillReviewProtocol
...
assert isinstance(TimedReviewPolicy(), SkillReviewProtocol)
...
<Card title="Reflection" icon="rotate" href="/docs/concepts/reflection">
  The other self-improvement loop — improves answer quality within a task
</Card>
</CardGroup>
