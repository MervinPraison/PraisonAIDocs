...
<Note>
**This page covers executor-level cancellation** — stopping the *next* tool call via `timeout_ms` / `cancel_token`. To abort a tool body that is *already running* (e.g. kill a running `shell` subprocess on `/stop`), see [Cooperative Tool Cancellation](/features/cooperative-tool-cancellation).
</Note>
...
<Accordion title="Streaming path now forwards the token too">
Prior to [PraisonAI PR #5081](https://github.com/MervinPraison/PraisonAI/pull/5081), the streaming tool loop (`get_response_stream`) accepted `cancel_token` on public entry points but never forwarded it to `execute_batch`, so a Stop during a streamed tool batch let shell / MCP / file tools run to completion. Since #5081, `start(stream=True)` and `iter_stream()` thread the same token end-to-end (from `Agent.interrupt_controller` when no explicit token is passed), the batch short-circuits with typed `cancelled` results, and the follow-up completion request is skipped. See [Cooperative Tool Cancellation → Streaming honours the same Stop](/features/cooperative-tool-cancellation#streaming-honours-the-same-stop).
</Accordion>
...
<Accordion title="Tool middleware async path now honours tool_timeout">
Prior to [PR #5080](https://github.com/MervinPraison/PraisonAI/pull/5080), an async tool invoked through the tool middleware chain (`@wrap_tool_call` / `@before_tool` / `@after_tool`, or any plugin-registered tool middleware such as audit/guardrail wrappers) scheduled its coroutine on the running loop and blocked a thread-pool worker on `future.result()` with no timeout. A hung async tool blocked the middleware chain indefinitely and `ToolConfig.timeout` was silently ignored on this path. The worker wait is now bounded by the same `ToolConfig.timeout` the direct async path uses (with a small grace so the inner `asyncio.wait_for` surfaces its typed timeout first when configured). On timeout the future is cancelled and the same structured error dict the direct async path returns is surfaced (`{"error": "Tool timed out after Ns", "timeout": True, "_praison_retryable": False}`). No new API — set `tool_config=ToolConfig(timeout=...)` on the agent as before.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card icon="circle-stop" href="/features/cooperative-tool-cancellation">
