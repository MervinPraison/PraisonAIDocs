...
<Note>
Bot platform adapters ship in the `praisonai-bot` package. `praisonai bot serve` still works exactly as documented here; for a standalone install see [praisonai-bot Migration](/guides/praisonai-bot-migration).
</Note>
...
<Note>
For the **outbound** counterpart — replying **in** voice — see [Voice Reply (Text-to-Speech)](/features/gateway-voice-reply). It is off by default; set `voice.mode: match_inbound` for the intuitive "speak in / speak out" symmetry.
</Note>
...
<CardGroup cols={2}>
<Card title="Voice Reply (TTS)" icon="waveform-lines" href="/features/gateway-voice-reply">
  The outbound counterpart — speak the agent's reply back as a native voice note.
</Card>
