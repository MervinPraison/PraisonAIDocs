...
agent = Agent(name="hybrid-agent", instructions="Run hybrid sequential and parallel workflows.")
agent.start("Process steps 1-3 sequentially, then run steps 4 and 5 in parallel.")
```
...
sequenceDiagram
    ...
    participant H as HybridExecutor
    participant J as Shell / Script
    participant A as AI Agent

    U->>CLI: praisonai workflow run pipeline.yaml
    CLI->>H: load hybrid pipeline
    H->>J: run deterministic steps
    J-->>H: stdout / exit code
    H->>A: run agent step
    A-->>H: AI output
    H-->>CLI: combined result
    CLI-->>U: output
```
