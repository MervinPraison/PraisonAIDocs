...
2. **Uniform declared values take the shared-wrap fast path.** When every declared `tool_timeout` under `roles:` and `agents:` is identical (or only one agent declares one), a single guard wraps the shared tool dict.
3. **Heterogeneous per-agent values are honoured per agent.** When agents declare different values, each agent's tools carry its own budget (tool objects stay shared; only the guard closure differs). The tightest value no longer collapses onto every agent.
...
<Warning>
As of [PR #4477](https://github.com/MervinPraison/PraisonAI/pull/4477), heterogeneous per-agent `tool_timeout` values are honoured per agent instead of collapsing to the tightest value. A slow agent that previously inherited a fast agent's tighter budget (masking a real timeout) may now run longer. Uniform declarations are unaffected. (Earlier, PR #3176 had reversed the collapse from `max()` to `min()`.)
</Warning>
