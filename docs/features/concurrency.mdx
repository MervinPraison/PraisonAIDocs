...
2. **Otherwise, the tightest (smallest) per-role/per-agent value.** The wrapper picks `min(tool_timeout)` across every entry under `roles:` and `agents:` in the YAML (changed from `max` in PR #3176). This is the safe-by-default choice for a shared tool dict — an agent that asked to bail out quickly is never forced to wait for another agent's larger budget. Every overridden agent is warned in the logs.
