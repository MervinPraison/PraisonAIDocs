...
`RedisStateStore` is a thin wrapper around `RedisStorageAdapter` — one connection path, one shared key prefix for both state and storage operations.

```mermaid
graph LR
    A[Agent / StateStore API] --> B[RedisStateStore]
    B --> C[RedisStorageAdapter]
    C --> D[(Redis)]
...
<Info>
The public `RedisStateStore` API is unchanged — it now delegates to `RedisStorageAdapter` internally. The `prefix` you set applies consistently whether you read via `StateStore` or `StorageAdapter`.
</Info>

```mermaid
sequenceDiagram
    participant Agent
    participant StateStore as RedisStateStore
    participant Adapter as RedisStorageAdapter
    participant Redis as Redis Server
    
    Agent->>StateStore: set(key, value)
    StateStore->>Adapter: save(key, value)
    Adapter->>Redis: SET praison:key
    Agent->>StateStore: get(key)
    StateStore->>Adapter: load(key)
    Adapter->>Redis: GET praison:key
    Redis-->>Agent: value
```

### RedisStateStore Parameters

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `url` | `str \| None` | `None` | Full Redis URL; overrides host/port/db/password |
| `host` | `str` | `"localhost"` | Redis host when `url` is not set |
| `port` | `int` | `6379` | Redis port |
| `db` | `int` | `0` | Redis database number |
| `password` | `str \| None` | `None` | Redis password |
| `prefix` | `str` | `"praison:"` | Shared key prefix (StateStore + StorageAdapter) |
| `decode_responses` | `bool` | `True` | Kept for backward compatibility; **ignored** |
| `socket_timeout` | `int` | `5` | Socket timeout in seconds |
| `max_connections` | `int` | `10` | Kept for backward compatibility; **ignored** |

Redis stores different types of state data optimised for performance:
