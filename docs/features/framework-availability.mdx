...
agent = Agent(name="framework-agent", instructions="Check available framework integrations.")
agent.start("Which frameworks are available in this PraisonAI installation?")
```
...
frameworks = ["autogen", "autogen_v4", "crewai", "ag2", "langgraph"]
...
| `langgraph` | `importlib.util.find_spec("langgraph.prebuilt")` | Guarded against import-time errors; wrapper adapter ships separately in `praisonai-frameworks[langgraph]` |
...
<CardGroup cols={2}>
  ...
  <Card title="LangGraph Framework" icon="diagram-project" href="/docs/framework/langgraph">
