...
<Note>
When **none** of the supported framework adapters are installed (e.g. a container that ships only `praisonai` without the `crewai` / `autogen` / `praisonaiagents` extras), the auto-detect chain raises `RuntimeError: No supported framework installed. Available adapters: []`. This is the intended, actionable failure — install one of the adapters (`pip install praisonai[crewai]`, `pip install praisonai[autogen]`, or `pip install praisonaiagents`) and re-run.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Google ADK Framework" icon="robot" href="/framework/google-adk">
