...
frameworks = ["autogen", "autogen_v4", "crewai", "ag2", "openai_agents"]
...
| `openai_agents` | `distribution("openai-agents")` + `find_spec("agents")` + `from agents import Runner` | Three-step probe: dist must be installed, `agents` namespace must be discoverable, and `Runner` must import without error. Install: `pip install 'praisonai-frameworks[openai-agents]'` |
