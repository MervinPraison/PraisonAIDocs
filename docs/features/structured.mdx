...
Structured AI Agents return type-safe, predictable outputs using Pydantic models and JSON schemas — perfect for integrations that need reliable data.

```mermaid
graph LR
    subgraph "Structured Outputs"
        Agent[🤖 Agent] --> Task[📋 Task]
        Task --> Schema[📐 Pydantic Schema]
    ...
    class Agent agent
    class Task task
    class Schema,Validate schema
    class JSON,Model validate
    class App result
```
