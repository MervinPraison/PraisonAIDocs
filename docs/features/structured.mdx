...
description: "Get type-safe, validated JSON from any agent using Pydantic models"
icon: "cube"
---

Get perfectly shaped data back from your agent — no parsing, no surprises.
...
graph LR
    ...
    classDef validate fill:#F59E0B,stroke:#7C90A0,color:#fff
...
<AccordionGroup>
  <Accordion title="Keep Pydantic models flat">
    Avoid deeply nested models for your first pass — agents produce more reliable JSON when the schema is flat. Add nesting once basic output works.
  </Accordion>
  <Accordion title="Use native structured output for GPT-4o and Claude">
    Pass `native_structured_output=True` (or rely on auto-detection) for models that support `response_format`. This gives cleaner JSON without schema echoing.
  </Accordion>
  <Accordion title="Set expected_output to describe structure, not content">
    Write `expected_output="Structured analysis report with title, findings, and summary fields"` — this primes the agent to match your Pydantic model.
  </Accordion>
  <Accordion title="Test with a single-agent task first">
    Validate your Pydantic model on a simple single-agent task before wiring it into a multi-agent pipeline where failures are harder to diagnose.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="AutoAgents" icon="robot" href="/docs/features/autoagents">
    Automatically create and manage agent teams
  </Card>
  <Card title="Workflows" icon="diagram-project" href="/docs/features/workflows">
    Chain structured outputs across multi-step pipelines
  </Card>
</CardGroup>
