...
sequenceDiagram
    ...
    Agent->>Schema: Validate against fields
    alt Valid
        Schema-->>Agent: Typed object
        Agent-->>User: model.field values
    else Invalid
        Schema-->>Agent: Validation error
        Agent->>Agent: Regenerate to match schema
    end
```
