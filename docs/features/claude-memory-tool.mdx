...
sequenceDiagram
    participant User
    participant Agent
    participant ClaudeMemoryTool

    User->>Agent: Request
    Agent->>ClaudeMemoryTool: Process
    ClaudeMemoryTool-->>Agent: Result
    Agent-->>User: Response
```
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Claude Memory Tool
...
<CardGroup cols={2}>
  <Card title="Agent Memory" icon="memory" href="/features/advanced-memory">
