...
<Note>
`BotLoopGuard` is a gateway-layer primitive: it decides whether to admit one bot's reply to another bot. It differs from [Loop Guard](/features/loop-guard), which caps tool calls inside a single agent turn.
</Note>
