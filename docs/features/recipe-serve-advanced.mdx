...
sequenceDiagram
    participant User
    participant Agent
    participant RecipeServeAdvanced

    User->>Agent: Request
    Agent->>RecipeServeAdvanced: Process
    RecipeServeAdvanced-->>Agent: Result
    Agent-->>User: Response
```
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Recipe Serve Advanced 
...
Advanced features layer onto the base recipe server from [Recipe Serve](/features/recipe-serve-code). Rate limiting uses a sliding window per client; metrics expose request counts and latency; admin endpoints hot-reload recipes without restart.
