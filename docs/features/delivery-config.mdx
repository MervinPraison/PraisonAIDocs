...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Delivery Config
...
A growing DLQ (`praisonai bot dlq list`) signals chronic LLM failures. Wire `dlq.size()` into your alerting stack. Since [PR #3521](https://github.com/MervinPraison/PraisonAI/pull/3521), a growing DLQ is a stronger signal — transient outages no longer inflate the count. A trend rising over multi-hour windows indicates genuinely poisoned traffic worth paging on.
</Accordion>
</AccordionGroup>
...
<Card title="Dead Target Registry" icon="x-circle" href="/features/dead-target-registry">
