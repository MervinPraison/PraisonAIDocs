...
## Pairs with `outbound_resilience:`

The `delivery:` block handles **inbound** durability. Its sibling `outbound_resilience:` handles **outbound** retry and dead-letter queuing for replies. Both live under the same channel block:
...
channels:
  slack:
    ...
    delivery:
      durable: true                     # inbound durability (this page)
    outbound_resilience:
      dlq_path: ~/.praisonai/state/slack/outbound_dlq.sqlite  # outbound DLQ
```

See [Outbound Resilience](/docs/features/outbound-resilience) for the full `outbound_resilience:` schema and configuration options.
...
<Card title="Outbound Resilience" icon="shield-check" href="/docs/features/outbound-resilience">
  Retry transient send failures and park permanent ones in an outbound DLQ
</Card>
</CardGroup>
