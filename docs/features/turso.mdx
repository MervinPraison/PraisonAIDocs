...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Turso Edge Database
...
<Warning>
**Deprecated import.** `from praisonai.db.adapter import TursoDB` and the top-level `from praisonai import DB / PraisonAIDB / PraisonDB` now emit `DeprecationWarning` — as of PR [#4595](https://github.com/MervinPraison/PraisonAI/pull/4595) the top-level aliases route through `praisonai.db` so the warning fires consistently. Use the `db(...)` factory instead:

```python
from praisonaiagents import db

store = db(database_url="libsql://mydb-user.turso.io", auth_token="eyJ...")
```
</Warning>
...
<Tip>
`praisonai persistence doctor` auto-detects `libsql://…` URLs as the `turso` backend — no `--*-backend` flag needed. See [Backend Auto-Detection](/cli/persistence#backend-auto-detection).
</Tip>
...
store = db(database_url="libsql://mydb-user.turso.io", auth_token="eyJ...")
agent = Agent(name="Edge Agent", memory=MemoryConfig(db=store))
```

<Note>
**Legacy import.** `from praisonai.db.adapter import TursoDB` still works but now emits `DeprecationWarning` (PR [#4595](https://github.com/MervinPraison/PraisonAI/pull/4595)). Prefer the `db(...)` factory above.
</Note>
...
agent = Agent(name="Global Agent", memory={"db": primary_db})
```
...
agent = Agent(name="Chat Agent", memory={"db": conversation_db})
```
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="Cloud Databases Overview" icon="cloud" href="/features/cloud-databases">
  Compare all cloud database providers
</Card>
<Card title="Local SQLite" icon="database" href="/features/persistence">
  Pure local SQLite for development
</Card>
</CardGroup>