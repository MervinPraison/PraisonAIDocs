...
<Note>
As a one-release compatibility fallback, the CLI still resolves a legacy `deploy/` layout **in your own checkout** — it walks up from the working directory looking for `deploy/helm`, `deploy/compose`, or `deploy/starters`. A fresh PraisonAI clone ships **no** root `deploy/` folder, so this path stays inactive unless you created that layout yourself. Prefer the new `praisonai deploy compose` commands.
</Note>
