...
onto the `Agent(...)` instances it was meant for. Passing the single-agent
`ExecutionConfig` to the team warns and adapts what it can — use
`MultiAgentExecutionConfig` instead. See
[`ExecutionConfig` vs `MultiAgentExecutionConfig`](/features/multi-agent-execution#executionconfig-vs-multiagentexecutionconfig).
</Accordion>
</AccordionGroup>
