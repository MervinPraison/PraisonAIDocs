...
Postgres and Redis are **not** memory backends — pass a live store with `db(url=...)`. Set durable defaults (like `learn`) in the config file, then attach the store in code.
...
# Redis/Postgres go through db(url=...), not backend="postgres"
agent = Agent(
    name="Assistant",
    instructions="Help users",
    memory=MemoryConfig(db=db(url="postgresql://localhost:5432/praisonai")),
)
```
</Accordion>
