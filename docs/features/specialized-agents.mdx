...
artist = ImageAgent(model="openai/dall-e-3")
...
sequenceDiagram
    participant User
    participant Agent
    participant Feature as Specialised Agents
...
audio = AudioAgent(model="openai/tts-1")
audio.speech("Hello world!", output="hello.mp3")

image = ImageAgent(model="openai/dall-e-3")
image.generate("A mountain landscape", output="sunset.png")

video = VideoAgent(model="openai/sora-2")
video.generate("A sunset timelapse", output="timelapse.mp4")

ocr = OCRAgent(model="mistral/mistral-ocr-latest")
...
<CardGroup cols={2}>
<Card title="Multi-Agent Pipelines" icon="diagram-project" href="/features/multi-agent-pipelines">
  Chain specialised agents together
</Card>
<Card title="OCR" icon="file-lines" href="/features/ocr">
