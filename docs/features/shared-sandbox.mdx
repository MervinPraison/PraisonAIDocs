...
| `run_on:` (YAML top-level key) | Workflow | Same effect, no code — one shared sandbox for every step in `agents.yaml` / `workflow.yaml`. |
...
Team- and workflow-level `compute=` was renamed to `run_on=`. Per-agent `compute=` is unchanged.
...
team = AgentTeam(agents=[writer, reader], compute="docker")
```
</Tab>
<Tab title="New">
```python
...
There is no `compute=` alias on `AgentFlow` / `AgentTeam`. Passing `compute=` raises:

```
TypeError: __init__() got an unexpected keyword argument 'compute'
```

`LocalAgent(compute=…)` and `ManagedAgent(compute=…)` on a single agent are unchanged — they still take `compute=`.
