...
Set `tools_run_on=` (Python) or `run_on:` (YAML) once on a flow, team, or workflow and every agent shares one remote sandbox, so a file written by one step is visible to the next.
...
```mermaid
graph LR
    User[👤 User] --> Flow[AgentFlow / AgentTeam<br/>tools_run_on=docker]
    Flow --> SANDBOX
...
Without a shared sandbox, each agent provisions its own isolated instance, so agents cannot hand files to each other. `tools_run_on=` gives them all the same `/workspace`.

## Quick Start

<Steps>
<Step title="Share one sandbox across a workflow">
Set `tools_run_on` once — every step runs its shell and file tools in one sandbox. Python uses `tools_run_on=`; no-code YAML uses the `run_on:` alias.
...
The same `tools_run_on=` kwarg works on a team.
...
<Tab title="e2b">
```python
flow = AgentFlow(tools_run_on="e2b", steps=[writer, reader])
```
</Tab>
<Tab title="modal">
```python
flow = AgentFlow(tools_run_on="modal", steps=[writer, reader])
```
</Tab>
<Tab title="daytona">
```python
flow = AgentFlow(tools_run_on="daytona", steps=[writer, reader])
```
</Tab>
<Tab title="flyio">
```python
flow = AgentFlow(tools_run_on="flyio", steps=[writer, reader])
```
</Tab>
<Tab title="tenki">
```python
flow = AgentFlow(tools_run_on="tenki", steps=[writer, reader])
```
</Tab>
<Tab title="local">
```python
flow = AgentFlow(tools_run_on="local", steps=[writer, reader])
```
</Tab>
</Tabs>
</Tab>
...
|:--|:--|
| One sandbox per run | `tools_run_on=` provisions a single sandbox for the whole run and binds every agent's shell and file tools to it. |
...
| Opt-in and lazy | No `tools_run_on=` means nothing changes, and nothing is provisioned until a tool actually runs. |
...
`tools_run_on=` accepts a provider name or a pre-configured provider instance. The accepted values are identical for Python (`tools_run_on="docker"`) and YAML (`run_on: docker`, the silent alias).
...
Moving tools is now one knob under one name: `tools_run_on=`. It works on the flow, the team, and each individual agent.

| Parameter | Level | Effect |
|:--|:--|:--|
| `AgentFlow(tools_run_on=...)` / `AgentTeam(tools_run_on=...)` | Team / workflow | ONE shared sandbox for every agent in the run. |
| `run_on:` (YAML top-level key) | Workflow | Silent alias for `tools_run_on:` — one shared sandbox for every step in `agents.yaml` / `workflow.yaml`. |
| `LocalAgent(tools_run_on=...)` / `Agent(tools_run_on=...)` | Per-agent | That single agent gets its own isolated sandbox. |

<Note>
`tools_run_on=` is the same knob at every level — flow, team, or single agent. See [Placement](/features/placement) for how it relates to `run_on=` (whole agent) and `run_in=` (one code call).
</Note>

### Renamed from `compute=` and Python `run_on=`

Two old names both meant "run only the tools somewhere else". They are now one word: `tools_run_on=`.

<Tabs>
<Tab title="Old">
```python
from praisonaiagents import Agent, AgentFlow, AgentTeam

# per-agent compute=
agent = Agent(name="builder", instructions="Build", compute="docker")
# flow/team run_on=
flow = AgentFlow(run_on="docker", steps=[writer, reader])
team = AgentTeam(agents=[writer, reader], run_on="docker")
```
</Tab>
<Tab title="New">
```python
...
flow = AgentFlow(tools_run_on="docker", steps=[writer, reader])
team = AgentTeam(agents=[writer, reader], tools_run_on="docker")
```
</Tab>
</Tabs>

<Warning>
Python `AgentFlow(run_on=…)` / `AgentTeam(run_on=…)` now **raises** — no deprecation window, because `run_on=` on `Agent` means the opposite (the whole agent, not just its tools). `compute=` on `Agent` / `LocalAgent` still works but emits a `FutureWarning`. YAML `run_on:` is unaffected — it stays a silent alias.
</Warning>
...
<AccordionGroup>
<Accordion title="Use tools_run_on= when agents share files">
Per-agent `tools_run_on=` gives each agent its own isolated instance, so files written by one agent are invisible to the next. Set `tools_run_on=` on the flow or team when agents need the same `/workspace`.
</Accordion>

<Accordion title="Leave tools_run_on unset unless you need shared execution">
The default is zero-overhead — nothing is provisioned. Only set `tools_run_on=` when a run actually shares files or shell state across agents.
</Accordion>
