...
<Note>
As of [PR #4071](https://github.com/MervinPraison/PraisonAI/pull/4071), `run_on=` accepts the full compute set — `anthropic`, `docker`, `e2b`, `modal`, `daytona`, `flyio`, `tenki`, `novita` — the same places as `tools_run_on=` for compute-backed backends. See [Placement](/features/placement#where-run-on-and-compute-can-point).
</Note>
