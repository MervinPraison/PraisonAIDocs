...
sequenceDiagram
    ...
    User->>Agent: start(tasks)
    Agent->>Planner: create_plan(tasks)
    Planner-->>Agent: plan steps
    Agent->>Agent: execute each step
    Agent->>Agent: restore original tasks
    Agent-->>User: final result
```
...
t1 = Task(description="Research AI trends", agent=researcher)
t2 = Task(description="Write summary", agent=writer)
...
# Your original tasks are still there:
assert team.tasks[0].description == "Research AI trends"
assert len(team.tasks) == 2

# The auto-generated plan steps are here if you want to see what the planner did:
for step in team._plan_tasks.values():
    print(step.description)

# Follow-up work gets a fresh id, no collision with t1/t2:
team.add_task(Task(description="Follow-up", agent=researcher))
```

<Note>
Fixed in PraisonAI [PR #4034](https://github.com/MervinPraison/PraisonAI/pull/4034) — previously `team.tasks` was overwritten with the plan-derived tasks and the originals were lost. If you have code that reached into `team.tasks[0]` after a planned run expecting to see the auto-plan, that code needs to read `team._plan_tasks` now.
</Note>
