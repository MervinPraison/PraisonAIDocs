...
Over the AG-UI wire, a `TODO_UPDATED` event reaches the client as a `custom` event named `praisonai.todo_updated` with `value` `{ content, tool_call, metadata }`. See the [AGUI API reference](/deploy/api/agui-api#praisonai-custom-events).
