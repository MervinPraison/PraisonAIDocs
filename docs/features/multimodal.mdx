...
sequenceDiagram
    ...
    User->>Agent: Text + Image/Audio/Video
    Agent->>Processor: Extract content from media
    Processor-->>Agent: Structured content
    Agent->>LLM: Combined text + media context
    LLM-->>Agent: Multimodal response
    Agent-->>User: Answer about your media
```
