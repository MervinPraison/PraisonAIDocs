...
<Warning>
On PraisonAI releases before [PR #4367](https://github.com/MervinPraison/PraisonAI/pull/4367), a CPT run silently dropped `embedding_learning_rate`, `max_seq_length`, and `packing` — the embeddings ended up trained at the adapter's learning rate, the exact recipe CPT exists to avoid. `UnslothTrainingArguments` is a plain `__init__` subclass and `__dataclass_fields__` is inherited, so the drop-unknown filter looked at the parent's fields and deleted these CPT-only knobs after they were set. Nothing raised. If you ran CPT on an earlier release and the vocab shift looked weak, re-run on 4.6.148+ (or whichever release carries #4367) and compare.
</Warning>
...
| CPT needs | Why |
|---|---|
| A lower `embedding_learning_rate` | Embeddings train at a fraction of the adapter rate. That field lives on `UnslothTrainer` / `UnslothTrainingArguments`, not `SFTTrainer`. It **is now** passed through — on releases before [#4367](https://github.com/MervinPraison/PraisonAI/pull/4367) it was silently filtered out; the fix walks the class's `__init__` signature in addition to `__dataclass_fields__`. |
...
<Accordion title="Verify embedding_learning_rate was applied">
After the trainer starts, the effective LR is echoed in the launch banner. If it matches your top-level `learning_rate` exactly instead of your `embedding_learning_rate`, you are on a release without [PR #4367](https://github.com/MervinPraison/PraisonAI/pull/4367) — upgrade and re-run.
</Accordion>
</AccordionGroup>
