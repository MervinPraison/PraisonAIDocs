...
Run checkpoint commands from any terminal without writing Python.

```mermaid
graph LR
    subgraph "CLI Checkpoint Workflow"
        R[praisonai run] -->|auto: true| A[auto-checkpoint run:]
        A --> E[Agent executes]
        E -->|something wrong?| U[praisonai run --restore last]
        U --> W[Workspace rewound]
    end
...
# Show diff from last checkpoint to working directory
praisonai checkpoint diff

# Show diff between specific checkpoints
praisonai checkpoint diff abc123 def456

# Restore to a checkpoint (accepts 'last', 'latest', full id, or short id)
...
### Auto-checkpointing on `praisonai run`

Every `praisonai run` saves a `run:` checkpoint automatically before the agent starts. To disable this for a single run:

```bash
praisonai run --no-checkpoint "Quick read-only task"
```

To restore after a run:

```bash
praisonai run --restore last
```

To disable auto-checkpointing for all runs in a project:
