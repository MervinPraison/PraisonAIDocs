...
**Now active in the async TUI too.** The same wiring runs in `praisonai tui`. Previously the TUI's `/undo` was a stub that printed "use git diff" — it now shares the coding REPL's engine, so `Undo: Workspace restored to `<short_id>` (<label>).` is reported on success.
