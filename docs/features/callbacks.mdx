...
sequenceDiagram
    ...
    Agent->>Callback: on_start event
    Callback->>Log: Record start
    Agent->>Callback: on_tool_call event
    Callback->>Log: Record tool usage
    Agent->>Callback: on_complete event
    Callback->>Log: Record completion
    Agent-->>User: Result
```
