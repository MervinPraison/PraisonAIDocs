...
graph LR
    subgraph "Callback Flow"
        In[📥 User Message] --> Agent[🤖 AI Agent]
        Agent --> Out[📤 Response]
...
<CardGroup cols={2}>
  ...
  <Card title="Tool Call Callback" icon="wrench">
    Activated when tools are used — fires uniformly on non-streaming and streaming paths since PR [#4735](https://github.com/MervinPraison/PraisonAI/pull/4735). See [Display Callbacks → Streaming coverage](/features/display-callbacks#streaming-coverage).
  </Card>
...
<Tabs>
  <Tab title="Code">
    ...
    # A per-Task on_task_complete accepts either (task_output) or
    # (task_output, metadata) — NOT the team-level (task, task_output) shape.
    # See /features/human-in-the-loop-review for the full comparison.
  ...
  <Tab title="Code">
    ...
    async def main():
        ...
        task = Task(
            ...
            on_task_complete=async_task_callback
        )
...
Verbose UI is now driven by composing `on_task_start` and `on_task_complete` callbacks, ensuring workflow processes work correctly with `output="verbose"`:
...
agents = PraisonAIAgents(
    ...
    output="verbose",  # composes user callbacks with verbose UI display
)
agents.start()
```
...
Gate a team on human approval between tasks with `on_task_complete`. Use the **team-level** hook (`(task, task_output)`) for a soft approve/edit gate, or a **per-task** hook (`(task_output)` with `fail_on_callback_error=True`) to hard-stop on reject. Full comparison: [Human-in-the-Loop Review](/features/human-in-the-loop-review).

### Callback Composition

The composition happens at the `PraisonAIAgents` / `AgentTeam` level when user hooks are supplied via `MultiAgentHooksConfig` with `output="verbose"`:
...
agents = PraisonAIAgents(
    ...
    output="verbose",  # Composes user hooks with verbose display
)
```
...
for config in configs:
    agents = PraisonAIAgents(
        ...
        output="verbose",  # Works with all process types
    )
...
If you see this when calling `PraisonAIAgents.start()` with `output="verbose"` and a custom `on_task_start` / `on_task_complete` hook, your hook signature does not match the multi-agent contract. Use:

- `on_task_start(task, task_id)`
- `on_task_complete(task, task_output)`

Per-`Task` callbacks (set on the `Task(...)` constructor) take **either** `on_task_complete(task_output)` (1 param) **or** `on_task_complete(task_output, metadata)` (2 params, where `metadata` is a dict with `task_id` and `task_name`) — **not** the team-level `(task, task_output)` shape. See [Human-in-the-Loop Review](/features/human-in-the-loop-review) for the full comparison.
</Note>
...
<Note>
In multi-agent setups where several `LLM` instances register token-tracking events, each instance now merges its callbacks into the shared `litellm.callbacks` list rather than overwriting it. You do not need to reorder agent construction to avoid losing sibling callbacks.
</Note>
...
<CardGroup cols={2}>
  <Card icon="webhook" href="/features/hooks">
    Run shell or Python hooks on agent lifecycle events.
  </Card>
  <Card icon="activity" href="/features/observability-hooks">
