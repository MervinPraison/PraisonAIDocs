...
Always wrap callback logic in try-except blocks and log errors appropriately. Unhandled exceptions inside callbacks can suppress the error silently.
</Accordion>
<Accordion title="Keep callbacks lightweight">
Avoid blocking operations inside callbacks. Use async callbacks for I/O-bound work and keep sync callbacks fast to avoid slowing down the agent turn.
</Accordion>
<Accordion title="Use async for concurrent agents">
When running multiple agents concurrently, prefer async callbacks (`is_async=True`) to avoid blocking the event loop.
</Accordion>
<Accordion title="Monitor resource usage">
Callbacks that write to files or databases can accumulate quickly. Set log rotation and monitor disk usage in production.
</Accordion>
</AccordionGroup>
