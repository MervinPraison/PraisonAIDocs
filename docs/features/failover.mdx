...
```typescript
import { FailoverManager } from 'praisonai';
...
let provider = manager.getNextProvider(); // 'openai' under the priority strategy

// Rotate on a retryable failure (e.g. a 401 that a fresh key may fix):
manager.markFailed(provider!);
provider = manager.getNextProvider(); // now falls through to 'anthropic'

manager.markHealthy('openai', 120); // back online, 120ms latency
console.log(manager.getStatuses());
```
</Tab>
</Tabs>
...
<Note>
The LLM tool-loop failover rotates profiles on a retryable auth failure (a `401` that a fresh key may fix) and fires an event hook carrying `{ attempt, maxAttempts, profile, retryInSeconds }` so you can log or alert on each rotation.
</Note>
