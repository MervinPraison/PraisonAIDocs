...
<Note>
Failover integrates with [LLM Error Classification](/docs/features/llm-error-classification) via `FailoverDecision`, which coordinates profile rotation with typed error handling.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Integrate with error classification">
    Pair failover with [LLM Error Classification](/docs/features/llm-error-classification) so `FailoverDecision` coordinates profile rotation with typed errors.
  </Accordion>
