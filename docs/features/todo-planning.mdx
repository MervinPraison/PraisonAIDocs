...
todo_update(2, status="in_progress")   # Actively working on it (exactly one at a time)
todo_update(3, status="completed")     # Task finished
todo_update(4, status="cancelled")     # Task abandoned
```

Setting a todo to `in_progress` demotes any other `in_progress` item back to `pending`, so exactly one item is active at a time.

<Note>
Subscribe to the `TODO_UPDATED` stream event to render a live checklist that ticks off in real time. See [Live Todo Streaming](/features/todo-live-streaming).
</Note>
...
<Card title="Live Todo Streaming" icon="bars-progress" href="/features/todo-live-streaming">
  Render a live checklist that ticks off in real time via `TODO_UPDATED`
</Card>
<Card title="Self-Improving Skills" icon="wand-magic-sparkles" href="/features/skill-manage">
