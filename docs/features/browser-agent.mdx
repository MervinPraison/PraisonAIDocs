...
- **Session entry points**: A run can start from either the **CLI** (`praisonai browser run "…"`) or the **side panel** (Ctrl+Shift+P). Both send `start_session` to the bridge; only one may hold the extension at a time — check `/health` `extension_busy` before starting a second run.
...
<CardGroup cols={2}>
  <Card icon="box" href="/docs/features/praisonai-browser-package" title="praisonai-browser Package">
