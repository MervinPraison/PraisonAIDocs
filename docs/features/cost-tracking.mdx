...
<Step title="Read cost from an async agent">
An async run reports the same telemetry as sync — every tool-loop iteration emits `llm_end` with tokens, cost, and latency.
...
async def main():
    await agent.astart("Summarise the theory of relativity in one paragraph")

asyncio.run(main())

summary = get_token_collector().get_session_summary()
print(summary["total_metrics"]["input_tokens"], summary["total_metrics"]["output_tokens"])
# Same non-zero counts you would see from a sync agent.start(...)
```
</Step>
</Steps>

<Warning>
**Fixed in PraisonAI [PR #4887](https://github.com/MervinPraison/PraisonAI/pull/4887).** Prior to this release, async agent runs (`agent.achat()`, `agent.astart()`, any async tool-calling workflow) never emitted `llm_start` / `llm_end` callbacks, so cost dashboards and `--trace` / status output reported **zero LLM spans and zero cost for every async workload** — the identical run synchronously reported full per-call cost and latency. After this release, async cost tracking is at full parity with sync: per-iteration `llm_end` events fire with `tokens_in`, `tokens_out`, `cost`, and `latency_ms`. Latency is measured **per iteration** (each tool-loop step captures its own `iteration_start_time`), so every billed call reports its own latency rather than a cumulative total.
</Warning>
...
<Card title="Display Callbacks" icon="display" href="/features/display-callbacks">
`llm_start` / `llm_end` fire on the async tool-loop too.
</Card>
<Card title="SQLite Usage Sink" icon="database" href="/features/token-usage-sqlite-sink">
Durable, per-identity cost ledger for gateways.
</Card>
</CardGroup>
