...
As of PR #3515, `store`, `search`, `reset`, and `delete` all delegate to the Dakera adapter whenever it is configured — no `sqlite` fallback is used for any of these operations. Reads route through Dakera's own `working` / `episodic` stores, so data written via Dakera is always findable, resettable, and deletable through Dakera.
...
<CardGroup cols={2}>
<Card title="Memory Configuration" icon="brain" href="/features/memory">
  Provider strings, MemoryConfig, and backend overview.
</Card>
<Card title="Custom Memory Adapters" icon="puzzle-piece" href="/features/custom-memory-adapters">
...
<Card title="Memory Overview" icon="book" href="/memory/overview">
  Memory system overview and quickstart.
</Card>
<Card title="Memory Storage" icon="database" href="/memory/storage">
