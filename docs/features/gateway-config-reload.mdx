...
| Config change | Action | Scope |
|---------------|--------|-------|
| Agent `instructions` / `model` / `tools` | Recreate affected agents in-place | `ReloadScope.AGENTS` |
| Channel `token` or platform args | Drain + restart that single channel | `ReloadScope.CHANNEL` |
| Gateway `host` / `port` | Full restart (WebSocket server stays up) | `ReloadScope.FULL` |
| Scheduler / routes / guardrails | Full channel restart | `ReloadScope.FULL` |
| `identity:` block | Rebuild + re-stamp the identity resolver, restart affected channels | `ReloadScope.FULL` |
...
To pre-check what a path will do, call `classify_reload(path)` — see [Programmatic reload-scope classification](/docs/features/gateway-hot-reload#programmatic-reload-scope-classification).

<Note>
A reload that would introduce an **unknown route/binding agent** is rejected — the previous (valid) config keeps serving. Check `praisonai gateway doctor` output for the exact error, or see [Fail-Fast Validation](/docs/features/gateway-route-bindings#fail-fast-validation).
</Note>

<Note>
Three timing/logging keys — `gateway.logging.level`, `gateway.drain_timeout`, and `gateway.reload_drain_timeout` — are **hot-applied in place**: the gateway mutates the live subsystem without restarting any channel or agent. See [Hot-Apply (no restart)](/docs/features/gateway-hot-reload#hot-apply-no-restart).
</Note>
...
<Card title="Gateway Credential Rotation" icon="key" href="/features/gateway-credential-rotation">
...
<Card title="Credential Rotation" icon="key" href="/features/gateway-credential-rotation">
