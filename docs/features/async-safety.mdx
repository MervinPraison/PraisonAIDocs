...
agents = PraisonAIAgents(
    agents=[worker],
    tasks=[
        Task(description="Task A", agent=worker, async_execution=True),
        Task(description="Task B", agent=worker, async_execution=True),
        Task(description="Task C", agent=worker, async_execution=True),
    ],
    # process defaults to "sequential"; async_execution on tasks fans them out.
)
...
| `_goal_gate_async` on the async autonomous loop — see [Goal Loop](/features/goal-loop) | `asyncio.to_thread` (cancellation-safe via `asyncio.shield`) |
...
<Note>
**See also (PR #5219):** two more concurrency guarantees landed in [PR #5219](https://github.com/MervinPraison/PraisonAI/pull/5219) (fixes [#5162](https://github.com/MervinPraison/PraisonAI/issues/5162)):

- [Image Generation → Non-blocking async](/features/image-generation#non-blocking-async-pr-5219) — `ImageAgent.achat` / `agenerate_image` now offload via `asyncio.to_thread`, so a `gather` of an image agent and a text agent no longer starves the text agent.
- [Thread Safety → Output singleton lock](/features/thread-safety#output-singleton-tracestatus-lock-pr-5219) — `output="status"` / `output="trace"` metrics no longer corrupt under `asyncio.gather`.
</Note>
