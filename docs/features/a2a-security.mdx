...
agent = Agent(name="secure-agent", instructions="Serve A2A requests only to authenticated clients.")

agent.start("Process this authenticated A2A message.")
```
...
<CardGroup cols={2}>
  <Card title="A2A Protocol" icon="handshake" href="/docs/features/a2a">
    Learn the A2A protocol basics and setup
  </Card>
  <Card title="Agent API" icon="api" href="/docs/features/agent-api-launch">
