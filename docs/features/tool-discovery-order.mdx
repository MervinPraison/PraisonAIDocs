...
The same pipeline runs for `Agent(tools=[...])`, `LocalManagedAgent(config=LocalManagedConfig(tools=[...]))`, YAML `tools:`, and CLI `--tools`.
...
|------|--------|-------------|
| 1 | **Local `tools.py`** (backward compat, custom tools, custom variables) | Your own function with that name exists in `tools.py`, `tools/`, or `.praisonai/tools/*.py` (project-local or user-global) |
...
| 4 | **Tool registry (plugins via `entry_points`)** | A third-party plugin registered under the canonical `praisonai.tools` entry-point group (or the deprecated `praisonaiagents.tools` / `praisonai.tool_sources` aliases, which still resolve but emit a one-cycle `DeprecationWarning`) |

Tier 1 now covers three concrete locations: (a) `./tools.py` at your CWD, (b) a `./tools/` folder, and (c) `.praisonai/tools/*.py` — walked from the project up to the git root, plus `~/.praisonai/tools/*.py` user-global. The `.praisonai/tools/` layer is auto-loaded on `praisonai run` with no `--tools` flag; see [Project-Local Tools](/features/project-local-tools).

<Note>
**Within Tier 4, built-in names win.** The plugin registry subclasses the base `PluginRegistry`, so since [PraisonAI PR #4176](https://github.com/MervinPraison/PraisonAI/pull/4176) an `entry_points` plugin whose name (case-insensitive) collides with a built-in is skipped — built-ins are not reachable via entry-point overrides (a `DEBUG` line notes the collision). Runtime `register(...)` is the deliberate override path. See [Plugin Precedence](/features/plugin-precedence).
</Note>
...
## Publishing a tool package

Publish a `pip`-installable tool under the **canonical** `praisonai.tools` entry-point group and agents resolve it by name — no manual import.
...
Once installed, the tool auto-resolves in `Agent(tools=["my_tool"])`, `praisonai run --tools my_tool`, and YAML `tools: [my_tool]`.
...
agent.start("Do the thing with my_tool")
```

The legacy groups `praisonaiagents.tools` and `praisonai.tool_sources` still resolve tools registered under them but emit a one-cycle `DeprecationWarning` on first discovery. On a name collision, the canonical group wins — an alias entry with the same name is skipped, never overwritten.

```mermaid
graph LR
    subgraph "Entry-point discovery"
        A["📦 praisonai.tools<br/>(canonical)"] --> R["🔌 ToolRegistry"]
        B["⚠️ praisonaiagents.tools<br/>(deprecated alias)"] -.->|"skipped on name<br/>collision"| R
        C["⚠️ praisonai.tool_sources<br/>(deprecated alias)"] -.->|"skipped on name<br/>collision"| R
        B -->|"first hit"| W["DeprecationWarning"]
        C -->|"first hit"| W
    end

    classDef canon fill:#10B981,stroke:#7C90A0,color:#fff
    classDef alias fill:#F59E0B,stroke:#7C90A0,color:#fff
...
Precedence ladder: `praisonai.tools` (canonical, wins) > `praisonaiagents.tools` (deprecated alias) > `praisonai.tool_sources` (deprecated alias).
...
### Referencing `.praisonai/tools/` @tool functions

A `@tool` function dropped into `.praisonai/tools/*.py` is now resolvable by its tool name from any YAML `tools:` list — no explicit `--tools` flag, no `tools.py` in cwd required. The same `PRAISONAI_ALLOW_LOCAL_TOOLS=true` opt-in that gates `praisonai run` also gates the YAML resolver.
...
```yaml
# agents.yaml — weather_lookup is resolved from .praisonai/tools/
roles:
  researcher:
    tools:
      - weather_lookup
...
The tool is keyed by the `@tool(name="…")` value if given, otherwise by the function `__name__` — not by the module-namespaced form `weather.weather_lookup`. Fixed in [#3107](https://github.com/MervinPraison/PraisonAI/issues/3107).
...
Plugins register under the **canonical** `praisonai.tools` entry-point group in `pyproject.toml`:
...
Once installed, the tool auto-resolves by name in `Agent(tools=["my_tool"])`, `praisonai run --tools my_tool`, and YAML `tools: [my_tool]` — no manual import required.
...
|--------|------|---------|-------------|
| `PRAISONAI_ALLOW_LOCAL_TOOLS` | env var (`bool`-like string) | `false` | Opt-in flag that enables loading local `tools.py` / `tools/` (Tier 1) **and** `.praisonai/tools/*.py` `@tool` functions on the YAML resolver path (fixed in [#3107](https://github.com/MervinPraison/PraisonAI/issues/3107)). |
| `LOGLEVEL` | env var (`string`) | unset | Set to `INFO` to log which tier resolved each tool name. |

<Card title="Tool Resolver Python Reference" icon="code" href="/sdk/reference/praisonaiagents/modules/main">
...
The single resolver behind YAML `tools:` lists, `--tools`, and `Agent(tools=[...])`.
</Card>
...
<Card title="Approval Backends" icon="shield-check" href="/features/approval-backends">
