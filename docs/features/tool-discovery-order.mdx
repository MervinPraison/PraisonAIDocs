...
agent = Agent(tools=[lambda q: search(q)])
```
</Accordion>

<Accordion title="Gate local tools with PRAISONAI_ALLOW_LOCAL_TOOLS">
The `PRAISONAI_ALLOW_LOCAL_TOOLS` environment variable must be set to `true` before Tier 1 (local `tools.py`) is consulted. This is an opt-in security gate — without it, local files are silently skipped and only SDK built-ins and plugins are considered.
