...
| `retry_hint` | `str` | `""` | The exact next action, e.g. `praisonai gateway doctor --fix`. |
...
`praisonai gateway doctor --fix` is the sensible default. Prefer a specific one that names the exact env var, secret file, or rotation command — e.g. `re-set OPENAI_API_KEY` — so the operator's next step is unambiguous.
</Accordion>
