...
| `gateway` | `durability:idempotency` | Durable webhook-idempotency store unavailable; running in-memory (dedup does not survive restart). |
| `gateway` | `durability:idempotency` | `store_backend: redis` set but no Redis backend is implemented; running per-replica on the durable SQLite fallback. Cross-replica dedup only holds if replicas share the SQLite state file. See [Gateway Hooks → Multi-replica gateways](/features/gateway-hooks#multi-replica-gateways-and-store_backend-redis). |
| `gateway` | `durability:session` | Durable session store unavailable; running in-memory (session history does not survive restart). |
...
When the [fleet-level crash-loop breaker](/features/gateway-crash-loop-guard#fleet-level-breaker) trips, the health monitor records exactly **one** `gateway` owner — `owner_id = "fleet"` — so a fleet-wide reconnect storm shows up as a single, actionable fact instead of one entry per channel.
...
### The `gateway` / `durability:*` owners

When a *durable* store — the SQLite webhook-idempotency store or the configured session store — genuinely cannot initialise, the boundary that owns it falls back to in-memory to keep serving and records a `gateway` owner with `owner_id = "durability:idempotency"` or `owner_id = "durability:session"`.
...
- **Emitted by** the `praisonai-bot` wrapper (`praisonai_bot.bots._session`) at the boundary that owns the state.
- **Recorded** only when a durable store genuinely cannot initialise and the wrapper has to run in-memory to keep serving inbound deliveries.
- `retry_hint` is `praisonai gateway doctor --fix`.
- **Auto-cleared** on the next successful build of the durable store — a config hot-reload that fixes the store path clears the entry with no operator step.
- **Reasons are redacted** — the raw store path stays in the log, never in `health()` / `gateway status`.

See [Gateway State Durability](/features/gateway-durability) for the full contract.
...
<Card title="Gateway Channel Supervision" icon="heart-pulse" href="/features/gateway-channel-supervision">
...
<Card title="Weak / Placeholder Secret Guard" icon="shield-exclamation" href="/features/gateway-bind-aware-auth#weak-placeholder-secret-guard">
