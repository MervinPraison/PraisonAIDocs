...
<Step title="Name each branch's output">
...
def research_a(ctx: WorkflowContext) -> StepResult:
    return StepResult(output="Finding A")

def research_b(ctx: WorkflowContext) -> StepResult:
    return StepResult(output="Finding B")

combine = Task(action="Combine {{finding_a}} and {{finding_b}}")

workflow = AgentFlow(steps=[
    parallel([
        Task(name="research_a", handler=research_a, output_variable="finding_a"),
        Task(name="research_b", handler=research_b, output_variable="finding_b"),
    ]),
    combine,
])
result = workflow.start("Research the topic")
```
...
| `output_variable` (on each branch) | `str` | `None` | Name the branch's result so later steps can read it by name |
...
<Accordion title="Prefer named outputs over positional aggregation when order matters">
Name each branch's result with `output_variable` and read it by name downstream, rather than relying on the position of `parallel_outputs`. See [Named outputs from branches](/features/workflow-parallel#named-outputs-from-branches).
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="Orchestrator Worker" icon="sitemap" href="/features/orchestrator-worker">
