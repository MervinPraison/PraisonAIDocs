...
|--------|------|---------|-------------|
| `steps` | `List[Agent \| Callable]` | — | Agents or functions run concurrently |
| `max_workers` | `int \| None` | `None` | Cap on concurrent workers; defaults to `min(3, len(steps))` |
| `on_failure` | `str` | `"partial_ok"` | `"partial_ok"` collects partial results, `"fail_fast"` stops on first error, `"fail_all"` fails if any worker fails |
| Context key | `str` | `"parallel_outputs"` | Where aggregated results are stored (`ctx.variables["parallel_outputs"]`) |
