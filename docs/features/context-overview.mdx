...
<Note>
The low-level `get_global_store()` singleton is **not** auto-populated by the runtime. It is an opt-in, in-memory, process-local structure a caller fills explicitly; `Agent.chat_history` and the memory subsystems keep their own state. See [Scope: process-local, in-memory, opt-in](/features/context-api#scope-process-local-in-memory-opt-in).
</Note>
