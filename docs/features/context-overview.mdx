...
<Note>
The `get_global_store()` context store is **opt-in and process-local** — the agent runtime never auto-populates it. `Agent.chat_history`, sessions, and memory keep their own state. Populate the store yourself and share it across processes with `snapshot()` / `restore()`. See [Scope: process-local, in-memory, opt-in](/features/context-api#scope-process-local-in-memory-opt-in).
</Note>
...
<Note>
**Uniform across entry points.** Context management runs on every entry point — `chat()`, `start()`, `start(stream=True)`, `iter_stream()`, and their async variants. There is no "streaming bypass": if a ContextManager is configured, the outbound messages are optimized before the request is sent, regardless of whether the response is streamed. See [Streaming → long histories](/features/streaming#streaming-with-long-histories).

This statement is **only fully true after PR #4753**. Before #4753 the OpenAI-client streaming branch ran compaction (since #4729), but the custom-LLM branch — used by `llm="openai/gpt-4o-mini"`, `llm="ollama/…"`, and every routed provider — still bypassed it.

**PraisonAI 2026-09-03 ([PR #4729](https://github.com/MervinPraison/PraisonAI/pull/4729))**: the OpenAI-client streaming branch now runs the same `_apply_context_management` step as the non-streaming path.

**PraisonAI 2026-09-03 ([PR #4753](https://github.com/MervinPraison/PraisonAI/pull/4753))**: the custom-LLM streaming branch also compacts. The trigger is the routing flag `_using_custom_llm`, not the vendor — `llm="openai/gpt-4o-mini"`, `llm="ollama/…"`, and every routed provider take this branch. Together with #4729, long-running streamed chats no longer blow past the context window on **any** provider. Fixes [#4714](https://github.com/MervinPraison/PraisonAI/issues/4714).
</Note>
