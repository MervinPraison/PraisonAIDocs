...
<CardGroup cols={2}>
  <Card title="Completion Promise" icon="flag-checkered">
    Agent signals "done" with `<promise>TEXT</promise>` tag
  </Card>
...
<AccordionGroup>
  <Accordion title="promise">
    Agent output contained `<promise>TEXT</promise>` matching the configured promise.
  </Accordion>
...
<Steps>
  ...
  <Step title="Include Promise in Instructions">
    ...
    ```python
    instructions="When finished, output <promise>DONE</promise>"
    ```
  </Step>
</Steps>
