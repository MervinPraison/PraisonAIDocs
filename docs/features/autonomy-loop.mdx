...
<Tip>
**Unified API**: When `autonomy=True`, `agent.start()` automatically runs the autonomous loop with **16 default tools** (file ops, shell, web search, ast-grep) via the `AUTONOMY_PROFILE`. No need to call `run_autonomous()` separately!
</Tip>
