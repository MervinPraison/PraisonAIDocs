...
<Tip>
See [Autonomy Budget](/docs/features/autonomy-budget) for the full spend/token kill-switch guide, including pause-and-resume.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="budget_exhausted">
    `max_budget_usd` or `max_tokens` was reached. `result.metadata` carries `spend_usd`, `tokens`, `max_budget_usd`, `max_tokens`, and `status` (`"paused"` or `"stopped"`). With `budget_action="pause"` the run is resumable — raise the cap and re-invoke `run_autonomous`. See [Autonomy Budget](/docs/features/autonomy-budget).
  </Accordion>
