...
```mermaid
graph LR
    subgraph "Autonomous Loops"
        Request[📋 User Request] --> Process[⚙️ Autonomous Loops]
...
Use an explicit autonomy dict for iterative mode, completion_promise, and optional clear_context — see the additional tabs in the code group above.
</Step>
</Steps>
...
<Tip>
See [Autonomy Budget](/features/autonomy-budget) for the full spend/token kill-switch guide, including pause-and-resume.
</Tip>

<Note>
When `doom_loop_threshold` trips, the stop is now an approval decision rather than a hard block (default: stop). Set `doom_loop=allow` to let a legitimate repeat continue — see [Approval → Doom-loop safety gate](/features/approval#doom-loop-safety-gate).
</Note>

<Note>
The session-level escalation detector's `NO_PROGRESS` check now uses a recency window — an early success no longer suppresses stall detection for the rest of the run. See [Escalation Doom-Loop Detection](/features/escalation-doom-loop).
</Note>

The loop checks cumulative spend after every iteration. When a cap is hit, `budget_action` decides whether the run pauses (recoverable) or stops (terminal).

```mermaid
graph TB
    Iter[⚙️ Iteration] --> Check{Spend or tokens\nover cap?}
    Check -->|Under cap| Iter
    Check -->|Over cap| Action{budget_action}
    Action -->|pause| Paused["⏸️ budget_exhausted\nstatus=paused\nraise cap + resume"]
    Action -->|stop| Stopped["🛑 budget_exhausted\nstatus=stopped\nterminal"]
...
result.metadata         # dict | None - Extra data, e.g. spend on budget_exhausted
```

On a `budget_exhausted` run, `result.metadata` carries the spend snapshot:
...
result.metadata = {
    ...
    "status": "paused",       # "paused" (budget_action="pause") or "stopped"
}
```
...
<AccordionGroup>
  ...
  <Accordion title="doom_loop">
    Detected repeated identical actions (agent stuck in a loop). The session-level [Escalation Doom-Loop Detection](/features/escalation-doom-loop) subsystem also catches `NO_PROGRESS` stalls — now using a recency window so an early success no longer suppresses it.
  </Accordion>
  <Accordion title="budget_exhausted">
    `max_budget_usd` or `max_tokens` was reached. `result.metadata` carries `spend_usd`, `tokens`, `max_budget_usd`, `max_tokens`, and `status` (`"paused"` or `"stopped"`). With `budget_action="pause"` the run is resumable — raise the cap and re-invoke `run_autonomous`. See [Autonomy Budget](/features/autonomy-budget).
  </Accordion>
...
agent = Agent(
    ...
    memory=MemoryConfig(auto_save="project_session"),
)
...
<AccordionGroup>
  <Accordion title="Choose the mode that matches the task">
    Loops only run in `mode="iterative"`. `autonomy=True` uses `mode="caller"` (a single chat), so set `level="full_auto"` or pass `mode="iterative"` explicitly when you want the agent to keep working across turns. Use single-turn mode for quick answers to avoid unnecessary iterations.
  </Accordion>

  <Accordion title="Give the agent a clear completion signal">
    The loop stops on tool completion, a promise match, or a safety limit. Write instructions that produce a definite end state ("stop when all tests pass") so the agent hits tool completion instead of drifting until it reaches the iteration cap.
  </Accordion>

  <Accordion title="Keep doom-loop detection on">
    Doom-loop detection halts an agent that repeats the same unproductive action. Leave it enabled for long-running tasks; it's the safety net that turns a runaway loop into a clean `completion_reason` you can inspect.
  </Accordion>

  <Accordion title="Check the RunOutcome, don't assume success">
    `start()` returns a result with `success`, `iterations`, and `completion_reason`. Branch on `result.success` and log `completion_reason` so a limit-reached run is handled distinctly from genuine completion.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Autonomous Workflows" icon="robot" href="/features/autonomous-workflow">
