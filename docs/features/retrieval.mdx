...
<Note>
`embedder="openai"` is the default and means "no override". For Gemini, Cohere, or local Ollama embeddings, see [Knowledge → Use a non-OpenAI embedder](/features/knowledge#use-a-non-openai-embedder).
</Note>
...
| `embedder_config` | `Dict[str, Any]` | `None` | Full embedder dict `{"provider": ..., "config": {...}}` |
...
<Card title="CLI Retrieval" icon="terminal" href="/cli/retrieval">
