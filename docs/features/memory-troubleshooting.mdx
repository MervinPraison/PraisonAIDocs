...
Both the memory-layer and the knowledge-layer mem0 search paths now delegate to the same `safe_mem0_search` helper in `praisonaiagents.memory.adapters.factories`, so this fallback behaviour is identical whether you configured mem0 via `memory=` or via `knowledge=`. Fix reference: [PraisonAI PR #5204](https://github.com/MervinPraison/PraisonAI/pull/5204).
...
If you need MongoDB itself, use PraisonAI's first-party [MongoDB Memory](/features/mongodb-memory) adapter — it is a separate implementation, unrelated to mem0, and unaffected by this bug.
...
See also the [Mem0 integration guide](/tools/mem0).

---

## My `mem0` / `mongodb` config isn't being used — retrieval feels like keyword search

On releases before 2026-07-14, `Knowledge(config=...)` could silently degrade a configured `mem0` or `mongodb` backend to SQLite because the adapter constructor rejected an internal `verbose` keyword ([PR #2982](https://github.com/MervinPraison/PraisonAI/pull/2982)). Check which adapter you actually got:
...
If it isn't `Mem0Adapter` / `MongoDBKnowledgeAdapter`, upgrade to a release built after commit `69d7ecf`. On the fixed release, an explicit provider that fails to construct emits a `WARNING` naming the provider and the reason:

```
Knowledge provider '<name>' failed to initialize (<reason>); falling back to a different backend. Retrieval quality may be reduced.
```

See [Knowledge Backends → Troubleshooting: silent SQLite fallback](/features/knowledge-backends#troubleshooting-silent-sqlite-fallback) for the full walkthrough.

---

## Memory silently returned empty context

On releases **before PR #4213**, `Agent(memory=True)` retrieved memory hits but only included them in the task prompt when the root logger was at `DEBUG`. Under normal `WARNING`/`INFO` logging the retrieved context was logged and then discarded, so nothing reached the LLM.

**Old workaround (no longer needed):** enabling `verbose=5` / DEBUG logging, or calling `agent.memory.build_context_for_task(..., include_in_output=True)` by hand.

**Now:** retrieval reaches the prompt by default — `build_context_for_task()` defaults `include_in_output=True`, and `agent.memory.get_context(query=...)` returns the prompt-ready string for the non-prompt-caching path.

```python
from praisonaiagents import Agent

agent = Agent(name="Assistant", instructions="Help the user.", memory=True)
agent.memory.remember("User prefers metric units.")

# Reaches the prompt under normal logging — no DEBUG required
print(agent.memory.get_context(query="What units do I prefer?"))
```

<Note>
An empty or whitespace-only `query` to `get_context()` returns `""` on purpose — this avoids a `LIKE "%%"` scan that would leak unrelated (and possibly other users') records into the prompt.
</Note>

---

## Why did my metadata dict come back as a string? (or: my `None` values disappeared)

The `chroma` / `rag` (default) memory backend only accepts primitive metadata values, so non-primitives are coerced to `str(...)` and `None` values are dropped.

**Cause:** ChromaDB rejects non-primitive metadata (`str`, `int`, `float`, `bool` only). PraisonAI coerces `dict` / `list` / other types to `str(...)`, and drops `None` values, so the ChromaDB write never fails.

**Fix (if you need round-trippable structured metadata):** serialise it yourself with `json.dumps(...)` into a `str` field, or switch to a metadata-native backend like `mongodb`, `mem0`, or `dakera`.
...
# Instead of a raw dict/list (which gets stringified), store JSON in a str field
memory.store_long_term(
    text="Project atlas kicked off",
    metadata={"tags_json": json.dumps(["backend", "urgent"])},  # round-trips cleanly
)
```

See [Metadata types (ChromaDB / RAG backend)](/features/advanced-memory#metadata-types-chromadb-rag-backend) for the full coercion table.
...
## My other agent's memory disappeared when I called `reset_long_term()`

Older builds reset long-term memory by wiping **every** collection in the shared Chroma persistent directory, so calling `reset_long_term()` on one agent also erased sibling agents that shared the same path.

`reset_long_term()` now deletes only this instance's own collection and re-initializes it — other agents sharing the same `rag_db_path` keep their data. A failed reset also raises now instead of silently reporting success.

Give each agent a distinct `collection_name` when they share a persistent path so resets stay isolated:
...
agent = Agent(
    ...
    memory={
        ...
        "collection_name": "researcher_ltm",   # default is "memory_store"
    },
)
```

See [Memory Configuration → Chroma Collection Name](/docs/configuration/memory-config#chroma-collection-name).
...
<CardGroup cols={2}>
  <Card title="Memory Concepts" icon="brain" href="/features/advanced-memory">
  ...
  <Card title="Dakera Memory" icon="database" href="/features/dakera-memory">
    Self-hosted, decay-weighted vector recall via the Dakera server.
  </Card>
  <Card title="Memory Consolidation" icon="broom" href="/features/memory-consolidation">
    Merge, promote, and prune memories under a loss-bounded guard.
  </Card>
</CardGroup>