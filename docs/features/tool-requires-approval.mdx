...
### Scope of a skill grant

When a skill's `allowed-tools` frontmatter grants approval for a tool, the grant is scoped to the agent instance that activated the skill. It does **not** leak onto other agents in the same process — even other unnamed agents that share the default display name `"Agent"`. Each `Agent` gets a unique per-instance approval-scope id at construction, and skill grants are keyed by that id rather than the display name.

```python
from praisonaiagents import Agent, tool

@tool(approval=True)
def destructive_delete(path: str) -> str:
    """Delete a file."""
    return f"Deleted {path}"

reviewer = Agent(skills=["./skills/file-reviewer"])   # grants read_file via allowed-tools
deleter  = Agent(tools=[destructive_delete])          # separate agent; still gated

# `deleter` cannot inherit `reviewer`'s skill grant, even though both default to name="Agent".
```

<Note>
This is distinct from `PermissionManager` prefix scopes, which are agent-wide. Skill `allowed-tools` auto-approval is per-agent-instance. See [Reusable Approval Scopes](/features/reusable-approval-scopes).
</Note>
