...
| **Local** | `supports_edit=False`, `supports_typing=False`, `needs_rate_limit=False`, `accepts_webhooks=False`, `supports_media=False` — a TTY-honest declaration (text-only, pull-based, no rate limit). See [Local (Terminal) Channel](/features/local-channel). |
