...
<Info>
Capabilities describe what a channel **can** render; [Display Policy](/features/display-policy) controls what you **want** shown.
</Info>
...
| `supports_delete` | `bool` | `False` | Whether the platform supports deleting a sent message. When `True`, the adapter MUST override `delete_message`; verified at build time by the capability contract. See [Adapter Capability Contract](/features/bot-capability-contract). |
...
Methods: `to_dict()` and `from_dict(data)` — both round-trip `supports_delete` alongside the other flags. `supports_delete` is appended last in the dataclass on purpose, preserving the positional constructor order for existing callers.

<Note>
**Capability contract.** Every flag in the [Contract Backing Map](/features/bot-capability-contract#how-it-works) (currently `supports_edit` and `supports_delete`) is verified at adapter build time: if you set the flag `True` without overriding the backing method, `CapabilityContractError` is raised on construction. Purely informational descriptors (e.g. `supports_idempotency_token`) carry no method obligation and are not checked.
</Note>
...
`mark_recovered` defaults to `False` — behaviour is unchanged unless you explicitly set it. See [Durable Delivery](/features/durable-delivery) for the full outbox reference.
...
Platforms that set `accepts_webhooks=True` must also expose a `webhook_verifier` so `enforce_webhook_verification` can enforce signatures fail-closed. See [Webhook Verification](/features/webhook-verification).

## Built-in platform defaults

| Platform | Notes |
|----------|-------|
| **Telegram** | `max_message_length=4096`, `length_unit="utf16"`, `supports_edit=True`, `supports_delete=False`, `markdown_dialect="telegram_markdown_v2"`, `needs_rate_limit=True`, `edit_interval_ms=1000`, `max_file_size_mb=50` |
| **Discord** | `max_message_length=2000`, `length_unit="codepoints"`, `supports_edit=True`, `supports_delete=False`, `needs_rate_limit=False`, `edit_interval_ms=500`, `max_files_per_message=10`, `max_file_size_mb=8` |
| **Local** | `supports_edit=False`, `supports_delete=False`, `supports_typing=False`, `needs_rate_limit=False`, `accepts_webhooks=False`, `supports_media=False` — a TTY-honest declaration (text-only, pull-based, no rate limit). See [Local (Terminal) Channel](/features/local-channel). |
| slack, whatsapp, linear, email, agentmail | Uses `PlatformCapabilities()` defaults (`supports_delete=False`) until the adapter declares its own |

No built-in adapter declares `supports_delete=True`, so none trips the capability contract.
...
See [Native `/` Autocomplete](/features/bot-commands) for user-facing behaviour and the Discord shim caveat.
...
<CardGroup cols={2}>
  <Card title="Durable Outbound Delivery" icon="shield-check" href="/features/durable-delivery#effectively-once-delivery">
    Effectively-once delivery via crash reconciliation
  </Card>
  <Card title="Display Policy" icon="monitor" href="/features/display-policy">
  ...
  <Card title="Adapter Capability Contract" icon="shield-check" href="/features/bot-capability-contract">
    Declared `supports_*` flags are verified at build time so unbacked flags fail loudly
  </Card>
  <Card title="Bot Platform Plugins" icon="puzzle-piece" href="/features/bot-platform-plugins">
    Register custom adapters
  </Card>
  <Card title="Bot Streaming Replies" icon="stream" href="/features/bot-streaming-replies">
  ...
  <Card title="Reply Delivery" icon="paper-plane" href="/features/bot-reply-delivery-reliability">
    How length_unit drives chunking so replies reach the user intact
  </Card>
</CardGroup>
