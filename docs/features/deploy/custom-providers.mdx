...
<Note>
**Built-in provider names are protected.** Publishing an entry point whose name (case-insensitive) matches a built-in deploy provider — `aws`, `azure`, `gcp`, `fly`, `railway`, `render` — is skipped, and the built-in is kept (a `DEBUG` line notes the collision). The rule applies whether the deploy registry comes from `praisonai-code` (guard from [PR #4176](https://github.com/MervinPraison/PraisonAI/pull/4176)) or from a standalone `praisonai-deploy` install (guard from [PR #4184](https://github.com/MervinPraison/PraisonAI/pull/4184)). Use runtime `CloudProviderRegistry.default().register("aws", …, override=True)` for a deliberate override. See [Plugin Precedence](/features/plugin-precedence).
</Note>
...
<Accordion title="Prefer a vendor-prefixed name over a built-in">
A built-in name (`aws`, `azure`, `gcp`, `fly`, `railway`, `render`) is reserved — an entry point matching one is skipped, not honoured. Ship `acme-aws` instead, or use runtime `register(..., override=True)` when overriding a built-in is deliberate.
</Accordion>
</AccordionGroup>
