...
PraisonAI ships six providers out of the box (`aws`, `azure`, `gcp`, `fly`, `railway`, `render`). You can add your own by publishing a package that registers under the `praisonai.deploy.providers` entry-point group — see [Custom cloud providers](/docs/features/deploy/custom-providers).

---

## Provider Coverage of CloudConfig

Not every provider applies every `CloudConfig` field. Fields a provider cannot apply are logged and echoed in `DeployResult.metadata["unapplied"]` ([PraisonAI PR #4876](https://github.com/MervinPraison/PraisonAI/pull/4876)).

| CloudConfig field | AWS | Azure | GCP | Fly | Railway | Render |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| `env_vars` | ✅ | ✅ | ✅ | ✅ (`fly secrets set --stage`) | ⚠️ | ⚠️ |
| `image` | ✅ | ✅ | ✅ | ✅ | ⚠️ | ⚠️ |
| `cpu` / `memory` | ✅ | ✅ | ✅ | ✅ (non-default only) | ⚠️ | ⚠️ |
| `region` | ✅ | ✅ | ✅ | ⚠️ (`fly.toml`) | ⚠️ | ⚠️ |
| `min_instances` / `max_instances` | ✅ | ✅ | ✅ | ⚠️ (`flyctl scale count`) | ⚠️ | ⚠️ |

✅ applied · ⚠️ not applied (logged + `metadata["unapplied"]`)

See [Fly](/docs/features/fly), [Railway](/docs/features/railway), and [Render](/docs/features/render) for the per-provider details and workarounds.
...
```bash
praisonai deploy validate --file agents.yaml   # strict schema check — catches typos and missing sections
