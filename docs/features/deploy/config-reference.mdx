...
<Warning>
**Strict validation.** `APIConfig`, `DockerConfig`, and `CloudConfig` reject unknown fields (`extra="forbid"`), and the `deploy:` section rejects unknown top-level keys. Allowed `deploy:` keys are `type`, `api`, `docker`, `cloud`, and `agents`. A misspelled field or section used to deploy silently with all-defaults — it now fails fast.
...
deploy:
  ...
  cloud:
    ...
    clustor_name: prod-shared-cluster   # typo — should be cluster_name
```

```text
❌ Validation failed: 1 validation error for CloudConfig
clustor_name
  Extra inputs are not permitted
```

Fix the spelling to `cluster_name`. Landed in [PraisonAI PR #4308](https://github.com/MervinPraison/PraisonAI/pull/4308) ([issue #4306](https://github.com/MervinPraison/PraisonAI/issues/4306)).
</Warning>
...
<Note>
Setting `push: true` without `registry` no longer silently skips the push — it now fails with `Registry URL required for push operation`. Set `registry` whenever `push: true`. Landed in [PraisonAI PR #4308](https://github.com/MervinPraison/PraisonAI/pull/4308).
</Note>
...
| `resource_group` | `Optional[str]` | `None` | Resource group (Azure). Type is optional at the dataclass level, but the Azure provider **requires** it at deploy/status/destroy time — the call returns a failure result if unset. |
| `subscription_id` | `Optional[str]` | `None` | Subscription ID (Azure). Type is optional at the dataclass level, but the Azure provider **requires** it at deploy/status/destroy time — the call returns a failure result if unset. Threaded onto every `az` command via `--subscription`. |
| `project_id` | `Optional[str]` | `None` | Project ID (GCP). Type is optional at the dataclass level, but the GCP provider **requires** it at deploy/status/destroy time — the call returns a failure result if unset. |
...
<Note>
**Runtime-required fields per provider.** These are `Optional` in the dataclass but the provider returns a failure result when they are missing:

- **AWS** — `region`, `service_name`
- **Azure** — `region`, `service_name`, `resource_group`, `subscription_id`
- **GCP** — `region`, `service_name`, `project_id`
</Note>
