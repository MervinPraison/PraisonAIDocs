...
Run `praisonai deploy validate --file agents.yaml` to catch schema errors early. It fails fast on:

- A `type: cloud` block without a `cloud:` section.
- Misspelled field keys (e.g. `clustor_name` vs `cluster_name`) — no more silent defaults.
- Misspelled section names (e.g. `dockerr:` vs `docker:`), with the allowed-keys list in the error.
</Accordion>
</AccordionGroup>
