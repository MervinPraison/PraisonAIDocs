...
<Note>
`validate` is a strict schema check. It rejects misspelled field keys and misspelled section names with a clear `ValueError`, and for `type: cloud` it prints `Cluster:` alongside `Provider`, `Region`, and `Service` — previously the cluster name only appeared under `--json`, so a wrong `cluster_name` was invisible in the default output. See [PraisonAI PR #4308](https://github.com/MervinPraison/PraisonAI/pull/4308).
</Note>
...
See [Custom cloud providers](/docs/features/deploy/custom-providers) to add your own target.
...
<Accordion title="Unknown deploy config key(s): dockerr">
A misspelled section like `dockerr:` (instead of `docker:`) now fails with `Unknown deploy config key(s): dockerr. Allowed keys: agents, api, cloud, docker, type`. Fix the spelling — the allowed keys are listed in the error message.
</Accordion>

<Accordion title="Extra inputs are not permitted (misspelled field)">
A misspelled field like `clustor_name` inside `cloud:` now fails with a pydantic `Extra inputs are not permitted` error naming the bad field. Fix the spelling — see the [Config Reference](/docs/features/deploy/config-reference) for the full list of allowed fields per block.
</Accordion>
</AccordionGroup>
