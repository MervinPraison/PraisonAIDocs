...
`allowed=False` maps to `ConnectErrorCode.RATE_LIMITED` on the wire and sets `HelloError.retry_after_seconds` so clients know when to retry. Wire details are in [Gateway Handshake Protocol](/features/gateway-handshake-protocol).
...
<Card title="Bot Rate Limiting" icon="traffic-cone" href="/features/bot-rate-limiting">
...
<Card title="Rate Limiter" icon="gauge-high" href="/features/rate-limiter">
...
<Card title="Spend Budget" icon="dollar-sign" href="/features/gateway-spend-budget">
  Gate on cumulative per-identity USD spend instead of request count
</Card>
</CardGroup>
