...
An enabled scheduled job keeps the gateway resident. If you instead run `praisonai schedule tick` from OS cron with the gateway scaled to zero, delivery still lands out of process — the tick falls back to a per-platform standalone HTTPS sender using only `{PLATFORM}_BOT_TOKEN`. See [Deliver from cron / CI (no gateway required)](/features/scheduler-delivery#deliver-from-cron-ci-no-gateway-required).
