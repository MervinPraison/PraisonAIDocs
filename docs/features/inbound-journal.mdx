...
<Accordion title="Only call journal.replay() yourself outside the gateway">
Gateway-managed bots — every bot started via `praisonai bot start`, `onboard`, or `bot.yaml` — get startup and hot-reload replay automatically (see [Automatic crash-replay](#automatic-crash-replay)). Call `replay()` yourself only if you drive `InboundJournal` directly.
