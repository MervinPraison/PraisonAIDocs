...
<Note>
**Different scope from the [Run-State Journal](/features/run-state-journal).** The **inbound journal** deduplicates and recovers *webhook messages*. The **run-state journal** persists the *execution cursor* of a single run. A webhook may spawn a run; the two journals track the two halves of the pipeline.
</Note>
...
- Quarantines genuine poison entries to the inbound DLQ under the shared age-gated policy (see [Inbound DLQ](/features/inbound-dlq)).
...
The outbound reply path is now symmetric — see [Outbound Resilience → Automatic crash-recovery on start](/features/outbound-resilience#automatic-crash-recovery-on-start).
...
<CardGroup cols={2}>
<Card title="Gateway Restart Continuation" icon="rotate-right" href="/features/gateway-restart-continuation">
  How in-flight turns resume after a gateway restart
</Card>
<Card title="Gateway Session Continuity" icon="link" href="/features/gateway-session-continuity">
