...
<Note>
The hook-layer scanner protects agent prompts and tool outputs. The **agent↔agent** boundary is protected separately by the [Inter-Agent Provenance](/features/inter-agent-provenance) envelope — enabled by default on every `create_subagent_tool` call.
</Note>
...
<Note>
**`add_external_tool` is now safe to call at any time.** It mutates the module-level `EXTERNAL_TOOL_NAMES` set in place, so a module that imported the name earlier still sees the addition — the previous frozenset+rebind behaviour could leave importers holding a stale, pre-registration copy. Fixed in [PR #4969](https://github.com/MervinPraison/PraisonAI/pull/4969).
</Note>
...
Outbound tool results are wrapped in `<external_tool_result>`; **inbound** webhook and hook payloads are wrapped in `<external_request_payload>` — same idea, opposite direction. See [Untrusted Request Fencing](/features/untrusted-request-fencing) for the ingress boundary.
...
### What happens if the trust decision fails

When the trust registry exists but raises while answering a trust lookup, PraisonAI now fails **closed**: the tool is treated as external and its result is fenced. Under-fencing an external tool is the exact bug the fence exists to prevent, so a broken registry never quietly downgrades a tool to "trusted".

The one exception is "no registry at all" (e.g. a slim build): the hardcoded external list is the whole answer there, so unknown tools stay trusted — otherwise every ordinary function tool would be fenced.

| Situation | Result |
|-----------|--------|
| Tool is in the hardcoded external list | External — result is fenced |
| Registry exists and says `EXTERNAL` | External — result is fenced |
| Registry exists and says `TRUSTED` (or `None`) | Trusted — result passes through |
| Registry exists and **raises** while answering | External — result is fenced (fail closed) |
| No registry available at all | Trusted (fallback to hardcoded list only) |

Fixed in [PR #4969](https://github.com/MervinPraison/PraisonAI/pull/4969).
...
| Registry lookup error | — | — | Treated as external (fail closed) |
...
Combine with [Tool Circuit Breaker](/features/tool-circuit-breaker) and input validation for defence in depth.
</Accordion>
...
<Note>
**Related defence.** Tool outputs get wrapped in `<external_tool_result>` markers (this page). **Inbound webhook and hook payloads** get the ingress counterpart — wrapped in `<external_request_payload>`, described in [Untrusted Request Fencing](/features/untrusted-request-fencing). **Platform display names and group titles** interpolated into the `[{sender}] ` attribution prefix get a different treatment — collapse-and-strip — described in [Sender Attribution Sanitisation](/features/prompt-attribution-neutralisation).
</Note>
...
<CardGroup cols={2}>
<Card title="Inter-Agent Provenance" icon="shield-check" href="/features/inter-agent-provenance">
...
<Card title="Attribution Sanitisation" icon="user-shield" href="/features/prompt-attribution-neutralisation">
  Neutralise untrusted platform display names in the prompt prefix
</Card>
<Card title="Tool Circuit Breaker" icon="shield-exclamation" href="/features/tool-circuit-breaker">
