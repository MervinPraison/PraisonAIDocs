...
<Note>
**Related defence.** Tool outputs get wrapped in `<external_tool_result>` markers (this page). **Platform display names and group titles** interpolated into the `[{sender}] ` attribution prefix get a different treatment — collapse-and-strip — described in [Sender Attribution Sanitisation](/docs/features/prompt-attribution-neutralisation).
</Note>
