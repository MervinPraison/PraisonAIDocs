...
HostedAgent executes on Anthropic's managed infrastructure while seamlessly persisting conversation history and session state to any of the registry-supported conversation and state backends. See the [persistence overview](/features/persistence) for the full list.
...
<Note>
When context compaction is enabled, managed persistence surfaces the compaction checkpoint automatically — resume replays the compacted working history (summary + tail). See [Compacted Session Resume](/features/session-compaction-checkpoint).
</Note>
...
agent2 = Agent(
    ...
    memory=MemoryConfig(
        db=db(database_url="conversation.db"),
        session_id="session-1",
    ),  # Same ID → resumes with full metadata
)
...
<Card title="PraisonDB Reference" icon="database" href="/sdk/reference/typescript/classes/Memory">
...
<Note>
This page uses `MemoryConfig(db=PraisonAIDB(...))` throughout — the live path. As of PR [#4394](https://github.com/MervinPraison/PraisonAI/pull/4394), the orchestrator-based entry points (`wrap_agent_with_persistence`, `PersistentAgent`) share the same store-write logic under the hood, so you can pick whichever style suits your app.
</Note>
