...
The simplest way to add hooks to your agent:
...
<Step title="Enable Built-in Plugins">
```python
from praisonaiagents import Agent, plugins

# Enable logging and metrics globally
...
<Note>
**Hooks are automatically applied** to all agents when you use `@add_hook`. No need to pass anything to the Agent constructor.
</Note>
...
The easiest way to extend your agent - just define functions:
...
agent = Agent(
    name="Assistant",
    instructions="Help users with search and math",
    tools=[search_web, calculate]  # Just pass functions!
)

agent.start("What is 25 * 4 and search for Python tutorials")
```

<Tip>
**This is the recommended pattern.** Define functions in any `.py` file and pass them to your agent.
</Tip>

### Hooks (Intercept Events)

For logging, validation, or modifying behavior:
