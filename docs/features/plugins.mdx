...
## Selective Enable and Late Registration

After `plugins.enable([names])`, any plugin registered later stays disabled unless its name is in that list.
...
plugins.enable(["logging"])

# Anything registered after this stays disabled unless it is in the allow-list.
get_plugin_manager().register(MyMetricsPlugin())     # registered but disabled
get_plugin_manager().register(MyLoggingPlugin())     # allow-listed → enabled

agent = Agent(name="Assistant", instructions="Help users")
agent.start("Hello!")     # only the logging plugin's tools/hooks are active
```

The allow-list keeps working even for plugins you add later, so nothing slips in after you declare the selected set.

```mermaid
graph TB
    Q{Is plugins.enable&#40;names&#41; active?}
    Q -->|no| E[✅ Late registration → enabled]
    Q -->|yes| A{Plugin name in allow-list?}
    A -->|yes| E
    A -->|no| D[🛑 Late registration → registered but disabled]
...
Entry-point discovery, auto-discovery, and manual `PluginManager.register(...)` all inherit the same rule, because each path flows through `register()`.

```mermaid
sequenceDiagram
    participant User
    participant Facade as plugins.enable([...])
    participant Manager as PluginManager (singleton)
    participant Plugin

    User->>Facade: plugins.enable(["only_this"])
    Facade->>Manager: set_registration_allow_list({"only_this"})
    User->>Manager: register(late_plugin)
    Manager->>Manager: _default_enabled_for_registration("late") → False
    Manager-->>User: registered, is_enabled("late") == False
    User->>Manager: register(only_this)
    Manager->>Manager: allow-listed → True
    Manager-->>User: registered, is_enabled("only_this") == True
```

### Widen or Reset the Allow-List

| You want to… | Do this |
|---|---|
| Turn on more plugins later | `plugins.enable(["only_this", "also_that"])` — re-call with the new list |
| Return to default (every registration enabled) | `plugins.disable()` — clears the allow-list |
| Enable everything at once | `plugins.enable()` — no argument |
| Keep a standalone manager unrestricted | Construct via `PluginManager()` directly — the facade allow-list does not apply |

<Warning>
Re-calling `plugins.enable([...])` **resets** the allow-list to exactly the new list — it does not merge with the previous one. Pass every plugin you still want in the same call.
</Warning>

<Note>
A manager created directly via `PluginManager()` is not bound by the facade's allow-list — every registration on it starts enabled. Only the singleton used by `plugins.enable(...)` carries the restriction.
</Note>

<Note>
`plugins.enable()` with no argument (unrestricted) enables everything currently registered and every future registration — the late-registration rule only applies to a selective `enable([list])`.
</Note>
...
|-------------|--------------|
| **Tool** | Functions registered via `@tool` in a single-file plugin, **or** returned from `Plugin.get_tools()` (see [Tool Plugins](#tool-plugins-subclass-plugin-with-get-tools)) |
...
graph TB
    ...
    Q --> D[Ship a class-based Plugin that returns callables]
    A --> R1[discover_and_load_plugins]
    B --> R2[plugins.enable list]
    C --> R3[add_hook decorator]
    D --> R4[register a Plugin with get_tools]
    ...
    class R1,R2,R3,R4 option
```

<Note>
Enabling built-ins by name with `plugins.enable([...])` starts a selective allow-list — see [Selective Enable and Late Registration](#selective-enable-and-late-registration) for what happens to plugins you register afterwards.
</Note>
...
<Note>
This gate decides **whether a plugin loads at all**. A separate boundary decides whether a loaded plugin may read or rewrite conversation content — see [Security: Conversation-Content Capability Gate](#security-conversation-content-capability-gate).
</Note>

---

## Security: Conversation-Content Capability Gate

A plugin that isn't bundled must be explicitly granted `allow_conversation` before it can read or rewrite prompts, responses, and outbound messages — its tools still run either way.

```mermaid
graph LR
    Hook[📩 Hook fires] --> Kind{🔎 Conversation<br/>hook?}
    Kind -->|No — tool/channel/utility| Fire[✅ Dispatch]
    Kind -->|Yes| Origin{🏛️ First-party?}
    Origin -->|Yes — bundled| Fire
    Origin -->|No| Grant{🔐 allow_conversation<br/>= true?}
    Grant -->|Yes| Fire
    Grant -->|No| Skip[⏭️ Silently skipped]
    ...
    class Hook event
    class Kind,Origin,Grant check
    class Fire allow
    class Skip deny
```

A third-party plugin's `tools` still register and fire; only the eight conversation-content hooks are withheld until you grant access.

### Which hooks are gated

These eight hooks carry — and can rewrite — the user's prompt, the model's response, or an outbound reply, so they are default-deny for third-party plugins:

| Plugin method | Hook | Direction |
|---------------|------|-----------|
| `before_agent` | `BEFORE_AGENT` | Inbound prompt |
| `before_llm` | `BEFORE_LLM` | Inbound messages |
| `after_llm` | `AFTER_LLM` | Model response |
| `after_agent` | `AFTER_AGENT` | Agent response |
| `before_message` | `MESSAGE_RECEIVED` | Inbound message |
| `after_message` | `MESSAGE_SENDING` | Outbound reply |
| `message_sent` | `MESSAGE_SENT` | Outbound reply |
| `message_undelivered` | `MESSAGE_UNDELIVERED` | Outbound reply |

<Warning>
The outbound `message_sending` / `message_sent` / `message_undelivered` events are gated **on purpose**. `message_sending` can rewrite the assistant reply, and `message_sent` / `message_undelivered` expose it — so "conversation content" means both directions, not just inbound prompts.
</Warning>

### Grant a third-party plugin access

Add `allow_conversation: true` to the plugin's own option block in `.praisonai/config.yaml`:

```yaml
plugins:
  my_pii_redactor:
    allow_conversation: true      # opt in to before_llm / after_llm / message_*
    # ...any other plugin options are still delivered to on_config as before
```
...
manager.set_plugin_options({"my_pii_redactor": {"allow_conversation": True}})
```

<Note>
`allow_conversation` is **framework-reserved** — the manager interprets it and does **not** pass it through to the plugin's `on_config`. Every other key in the block still reaches `on_config` as before.
</Note>

### What "first-party" means

First-party plugins — those bundled inside the installed `praisonaiagents/` package on disk — are granted conversation access automatically. This is a different notion from the location trust levels (project vs user-global vs entry-point) of the [Project-Plugin Trust Gate](#security-project-plugin-trust-gate).

A plugin is first-party only when **both** hold:

1. its class's module name is `praisonaiagents` or starts with `praisonaiagents.`, **and**
2. that module's real on-disk file resolves to a path inside the installed `praisonaiagents/` package directory.

<Warning>
A malicious pip package that sets `__module__ = "praisonaiagents.evil"` is **rejected** — its file lives in the attacker's `site-packages`, not inside the framework's package directory, so the second check fails. Claiming first-party trust is not spoofable by a module string alone.
</Warning>

### Interaction with the project-plugin trust gate

The two gates are complementary. A project-local single-file plugin authorised to *load* via `PRAISONAI_ALLOW_PROJECT_PLUGINS=true` is still **not** first-party — it must also carry `allow_conversation: true` in its config block before it sees any conversation-content hook.

| Plugin origin | `before_tool`, tools, channels | `before_llm` / `after_llm` / `message_*` |
|---------------|:------------------------------:|:----------------------------------------:|
| First-party (bundled) | ✅ Fires | ✅ Fires |
| Third-party, no grant | ✅ Fires | ❌ Silently skipped |
| Third-party with `allow_conversation: true` | ✅ Fires | ✅ Fires |

### Tools and channels are never gated

An ungranted plugin still registers `get_tools()`, `before_tool`, `after_tool`, `before_tool_definitions`, `on_permission_ask`, channel adapters, `on_init` / `on_shutdown`, `session_start` / `session_end`, `on_error`, `on_config`, and `on_auth`. Only the eight conversation-content hooks above require the grant.

### Which plugins see prompt content
...
sequenceDiagram
    ...
    participant Bundled as First-party plugin
    participant Third as Third-party plugin<br/>(ungranted)
    participant Granted as Third-party plugin<br/>(allow_conversation=true)

    User->>Agent: prompt
    Agent->>Manager: before_llm(messages)
    Manager->>Bundled: before_llm(...) ✅
    Manager-->>Third: skipped (default-deny)
    Manager->>Granted: before_llm(...) ✅
    Manager-->>Agent: (rewritten) messages
```

<Warning>
**Upgrading?** Any third-party plugin whose value depended on `before_llm` / `after_llm` / `message_*` — guardrails, PII redactors, translators, message-lifecycle telemetry — silently stops rewriting until you grant it. Enumerate the affected plugins:

```bash
praisonai plugins list --json | jq '.[] | select(.requests_conversation == true and .conversation_granted == false)'
```
</Warning>
...
Each entry is a dict: `{name, version, description, source, enabled, hooks, requests_conversation, conversation_granted}` (single-file entries also include `path`). The `source` field records provenance:
...
Two fields surface the [conversation-content capability gate](#security-conversation-content-capability-gate):

| Field | Type | Meaning |
|-------|------|---------|
| `requests_conversation` | `bool` or `null` | `true` if the plugin declares any of the eight conversation-content hooks. `null` for an entry-point plugin not yet loaded — its hooks are unknown without executing it, which the runtime avoids. |
| `conversation_granted` | `bool` | `true` if the plugin is first-party or has `allow_conversation: true`. |

---

## Reload Plugins at Runtime

Edited a plugin, or dropped a new one in, and want it live in the current process? Run `praisonai plugins reload` — it forces rediscovery and rewires hooks so newly-added or edited plugins take effect without a restart. See the [Plugins CLI](/cli/plugins#reload-plugins).
...
## Tool Plugins (subclass `Plugin` with `get_tools()`)

Subclass `Plugin`, return callables from `get_tools()`, register once — every agent in the process can call them.

```mermaid
sequenceDiagram
    participant User
    participant Init as Agent.__init__
    participant Merge as _merge_plugin_tools
    participant Mgr as PluginManager
    participant Plugin as Registered Tool Plugin
    participant Agent

    User->>Init: Agent(instructions=...)
    Init->>Merge: after tools=[...] resolved
    Merge->>Mgr: get_all_tools()
    Mgr->>Plugin: plugin.get_tools()
    Plugin-->>Mgr: [random_number]
    Mgr-->>Merge: [random_number]
    Merge-->>Init: append to self.tools (skip collisions)
    Init-->>User: Agent ready — plugin tool callable
```

A class-based `PluginType.TOOL` plugin's `get_tools()` output is merged into `self.tools` during `Agent.__init__` — you do **not** call `plugins.enable()`, touch `PluginManager.get_all_tools()`, or pass anything special to `Agent(...)`. Registering the plugin marks it enabled in the manager, and that is enough for its tools to reach every subsequent agent.

### Quick Start

<Steps>
<Step title="Register a tool plugin in code">

Return callables from `get_tools()`, register once, and the tool reaches the agent — no `plugins.enable()`, no `tools=[...]`.

```python
from praisonaiagents import Agent, Plugin, PluginInfo
...
# Register once for the process.
get_plugin_manager().register(BasicToolPlugin())

# No plugins.enable(), no tools=[...] — the plugin's tool reaches the agent.
agent = Agent(instructions="Give me a random number using your tools.")
agent.start()
```

</Step>

<Step title="Load the bundled plugin by name">

The reference `basic_tools` plugin ships in the separate `praisonai-plugins` package. Install it, enable it once, and every subsequent agent gains its tool.

```bash
pip install praisonai-plugins
praisonai plugins enable basic_tools
```
...
agent = Agent(instructions="Give me a random number using your tools.")
agent.start()   # random_number is callable — no tools=[...] needed
```
...
Before this wiring, an agent asked for a random number would fabricate one. Now it actually calls the plugin's tool:

```
$ python app.py
User: give me a random number using your tools
Agent → tool call: random_number()
Tool  → 42
Agent → 42
```
...
| Rule | Behaviour |
|------|-----------|
| **Agent tools win on name collision** | If `Agent(tools=[foo])` and a plugin also returns a `foo`, the agent's `foo` is kept and the plugin's `foo` is **skipped with a `WARNING` log** — never silently shadowed. |
| **Metadata-only descriptors are skipped** | A bare `{"name": "phantom_tool"}` dict with no callable is **not** appended (logged as a `WARNING`) — it advertises a tool the agent could never actually invoke. |
| **Registration alone enables tool plugins** | Tool plugins do not require `plugins.enable()`. The `PluginManager` marks a plugin enabled on `register(...)`; that is enough for `get_all_tools()` to include it. Only hook / metric / lifecycle plugins need the package-wide `plugins.enable()` flag. |
| **Zero cost when plugin system unused** | If the plugin system is not enabled, the merge short-circuits — no scan, no cost. |
| **Never breaks agent init** | Any failure in plugin discovery / merge is caught; the agent is constructed regardless. |
| **Thread-safe iteration** | `get_all_tools()` snapshots the enabled `(name, plugin)` pairs under the manager's lock, so a concurrent `register` / `unregister` on another thread cannot raise `RuntimeError: dictionary changed size during iteration` and drop every plugin tool. |

The collision rule in code — the agent's own tool always wins:

```python
from praisonaiagents import Agent
from praisonaiagents.plugins import get_plugin_manager
# BasicToolPlugin from the Quick Start already registered — it exposes random_number

def random_number() -> str:
    return "agent's own version"

agent = Agent(instructions="test", tools=[random_number])
# agent.tools contains ONE random_number: the user's.
# The plugin's random_number is skipped with:
#   WARNING  Plugin tool 'random_number' skipped: an agent tool with the same name
#            already exists (existing tool wins).
```

### `@tool` vs `Plugin.get_tools()` vs `FunctionPlugin`

Two ways to add a tool from a plugin — pick the one that matches how the tool is shipped.

```mermaid
graph TB
    Q[How is the tool shipped?] --> A[Standalone .py file in ~/.praisonai/plugins/]
    Q --> B[Class-based plugin — bundled with hooks, config, session lifecycle]
    Q --> C[One-off callable at runtime]

    A --> R1["@tool decorator in a single-file plugin"]
    B --> R2["Subclass Plugin, return callables from get_tools()"]
    C --> R3["FunctionPlugin(hooks={...})"]
...
<Note>
The bundled `basic_tools` plugin in the separate `praisonai-plugins` package is the canonical `PluginType.TOOL` example — its `get_tools()` returns `[random_number]`, and that tool now reaches every agent end-to-end.
</Note>
...
<Warning>
`PIIRedactor` uses `after_llm`, a conversation-content hook. If you install it from pip (third-party), it won't fire until you grant `plugins.pii_redactor.allow_conversation: true` — see [Conversation-Content Capability Gate](#security-conversation-content-capability-gate).
</Warning>

---

## Plugin Lifecycle Observability

`plugin.on_init` and `plugin.on_shutdown` are the plugin's **own** callbacks — they run on the plugin being registered or unregistered.

To have a *different* plugin (a metrics collector, an audit log, a gateway health monitor) observe the plugin set changing at runtime, subscribe to the `ON_INIT` / `ON_SHUTDOWN` hook events. They are emitted from `PluginManager.register()` / `PluginManager.unregister()` immediately after the plugin's own callback runs.
...
def audit_plugin_registered(event_data: PluginLifecycleInput):
    print(f"plugin_registered name={event_data.plugin_name} version={event_data.plugin_version}")
    return HookResult.allow()

@registry.on(HookEvent.ON_SHUTDOWN)
def audit_plugin_unregistered(event_data: PluginLifecycleInput):
    print(f"plugin_unregistered name={event_data.plugin_name}")
    return HookResult.allow()
```

**Observe-only.** The emit is fully guarded — a failing hook never breaks plugin loading. Zero cost when nothing is listening (the runtime early-outs on `registry.has_hooks(...)`).

See [Hook Events → Plugin Lifecycle Events](/features/hook-events#plugin-lifecycle-events) for the payload reference.
...
<Warning>
All four `message_*` methods are conversation-content hooks. A third-party plugin overriding them needs `allow_conversation: true` before they fire — see [Conversation-Content Capability Gate](#security-conversation-content-capability-gate). Bundled and first-party plugins are granted automatically.
</Warning>
...
```python ~/.praisonai/plugins/per_user_rate_limit.py
...
```python ~/.praisonai/plugins/delivery_counter.py
from praisonaiagents.plugins import Plugin, PluginInfo, PluginHook
...
```python ~/.praisonai/plugins/dead_letter.py
from praisonaiagents.plugins import Plugin, PluginInfo, PluginHook
...
<Note>
`message_undelivered` is the counterpart to the plain `MESSAGE_UNDELIVERED` hook already covered in [Undelivered Messages](/features/undelivered-messages) and [Gateway → Undelivered Notice](/features/gateway-undelivered-notice) — this hook exposes the same signal at the plugin surface, so `Plugin` subclass authors don't need to reach for the raw `HookRegistry`.
</Note>
...
```python log every successful delivery
...
```python capture permanent delivery failures
from praisonaiagents import FunctionPlugin, PluginHook
from praisonaiagents.plugins import get_plugin_manager
...
<Tip>
A list like `enabled = ["logging", "metrics"]` flows through `plugins.enable(...)`, so it triggers the same late-registration allow-list rule — plugins registered afterwards stay disabled unless they are in the list. See [Selective Enable and Late Registration](#selective-enable-and-late-registration).
</Tip>
...
Reserved keys (`enabled`, `auto_discover`, `directories`, `allow_project_plugins`) configure the plugin *system*; **any other key whose value is a mapping is treated as a per-plugin option map** delivered to that plugin's `on_config` hook. Inside a per-plugin block, `allow_conversation` is also framework-reserved — the manager reads it to gate conversation-content hooks and does **not** forward it to `on_config`.
...
<Note>
The five `before_*` methods (`before_agent`, `before_llm`, `before_tool`, `before_tool_definitions`, `before_message`) also accept a deny/block decision instead of a rewrite — see [Blocking Plugins](#blocking-plugins-guardrails--policies). `after_tool` additionally accepts a rewrite **or** block — see [Redact / Block Tool Output](/features/redact-tool-output).
</Note>

| Plugin method | Hook event | Requires grant? | In-place mutation |
|---------------|-----------|:---------------:|-------------------|
...
<Note>
🔐 marks the eight [conversation-content hooks](#security-conversation-content-capability-gate). For a third-party plugin they fire only after `allow_conversation: true`; first-party plugins are granted automatically.
</Note>
...
| Plugin | Type | Enable |
|--------|------|--------|
| `cli_backend_tracer` | `cli_backend_execute` hook | `PRAISONAI_CLI_BACKEND_DEBUG=1 praisonai plugins enable cli_backend_tracer` |
| `basic_tools` | `PluginType.TOOL` | `praisonai plugins enable basic_tools` — every subsequent agent gains `random_number` |
...
See [Hook Events → CLI Backend Events](/docs/features/hook-events#cli-backend-events) for the full `CliBackendExecuteInput` payload reference.
...
<Warning>
A third-party guardrail that hooks `before_llm` or `before_message` — both conversation-content hooks — is silently skipped until granted `allow_conversation: true`. Blocking on `before_tool` / `after_tool` needs no grant. See [Conversation-Content Capability Gate](#security-conversation-content-capability-gate).
</Warning>
...
`PluginDecision.deny(reason)` and `PluginDecision.block(reason)` both stop the action (`is_denied()` is `True` for each); `allow(reason=None)` is an explicit no-op. `GuardrailBlocked(reason: str = "Blocked by guardrail plugin")` is caught by the bridge and converted to a block. It may be raised from any `before_*` method **and from `after_tool`** — the tool has already run there, but the output is suppressed before it reaches the model (see [Redact / Block Tool Output](/features/redact-tool-output)).
...
| `after_tool` | ✅ | ✅ | Tool output rewritten/redacted, or suppressed — the model sees the block reason instead ([details](/features/redact-tool-output)) |
...
Once installed, verify the plugin registered with `praisonai plugins add <package> --dry-run` — see [plugins add](/features/plugins-add).

<Note>
**A shipped plugin cannot replace a built-in name.** PraisonAI's registries subclass the base `PluginRegistry`, so since [PR #4176](https://github.com/MervinPraison/PraisonAI/pull/4176) any entry point whose name (case-insensitive) matches a shipped built-in is skipped and the built-in is kept (a `DEBUG` line notes the collision). Pick a name unique to your package. Runtime `register(...)` is the deliberate override path. See [Plugin Precedence](/features/plugin-precedence).
</Note>
...
`praisonai plugins list` shows every plugin with its `source` (`entry_point:<dist>`, `registered`, or `single_file`). `enable`/`disable` persist to the config the runtime reads, `reload` picks up edits without a restart, and `doctor` diagnoses issues. See the full reference on the [Plugins CLI](/cli/plugins) page.
...
The process-wide plugin manager returned by `get_plugin_manager()` is safe to share across concurrently running agents.

All lifecycle methods — `register`, `unregister`, `enable`, `disable`, `execute_hook` — synchronise on a single `RLock`, so concurrent registration and hook firing cannot collide. See [Thread-Safe Agent State → PluginManager (PR #5204)](/features/thread-safety#pluginmanager-pr-5204) for the mechanism.
...
<Card title="Toolsets" icon="wrench" href="/features/toolsets">
...
<Card title="Redact Tool Output" icon="shield-halved" href="/features/redact-tool-output">
  Scrub secrets or block tool results before the model sees them
</Card>
<Card title="Conversation-Content Gate" icon="lock" href="/features/plugins#security-conversation-content-capability-gate">
