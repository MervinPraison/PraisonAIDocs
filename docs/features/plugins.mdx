...
| `PluginHook.AFTER_TOOL` | `fn(tool_name: str, result: Any) -> Any` | Rewrites tool result |
...
<Note>
The five `before_*` methods (`before_agent`, `before_llm`, `before_tool`, `before_tool_definitions`, `before_message`) also accept a deny/block decision instead of a rewrite — see [Blocking Plugins](#blocking-plugins-guardrails--policies).
</Note>
...
`PluginDecision.deny(reason)` and `PluginDecision.block(reason)` both stop the action (`is_denied()` is `True` for each); `allow(reason=None)` is an explicit no-op. `GuardrailBlocked(reason: str = "Blocked by guardrail plugin")` is caught by the bridge and converted to a block.

### Where Blocks Fire

Five `before_*` methods can block; `after_*` methods are observational and cannot.
...
| `after_*` | ✅ | ❌ | *(observational — no block path)* |
