...
If you need a *different* plugin (a metrics collector, an audit log, a gateway health monitor) to observe the plugin set changing at runtime, subscribe to the `ON_INIT` / `ON_SHUTDOWN` hook events. They are emitted from `PluginManager.register()` / `PluginManager.unregister()` immediately after the plugin's own callback runs.
