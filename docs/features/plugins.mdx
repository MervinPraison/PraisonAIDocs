...
Plugins let you add logging, metrics, and custom behavior to your agents.

## Quick Start

<Steps>
<Step title="Use Built-in Logging">
```python
...
agent.chat("Hello!")  # Now logs all activity
```
</Step>
...
agent.chat("What are today's trends?")

# Check how many LLM calls were made
...
Plugins hook into lifecycle events (before/after agent runs, tool calls, LLM calls) to add functionality without changing your agent code.
...
# Create agent - plugins hook into lifecycle automatically
agent = Agent(
    name="Assistant",
...
manager = PluginManager()
manager.register(LoggingPlugin(log_tools=True, log_llm=True))
