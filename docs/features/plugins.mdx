...
# Plugin tools are auto-discovered!
...
<Note>
**Plugins are auto-discovered** from `~/.praisonai/plugins/` (user-wide) and `./.praisonai/plugins/` (project-specific). No registration needed!
</Note>
...
agent = Agent(name="Assistant", instructions="Help with weather")
agent.start("What's the weather in Tokyo?")
# Agent automatically has access to get_weather and get_forecast tools
```
