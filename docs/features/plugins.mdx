...
Three ways to create your own plugins:

<Tabs>
<Tab title="Class Plugin">
Full control with the Plugin base class:
...
WordPress-style - just a Python file with a header:
...
Register your plugin so agents can use it:
...
# All agents now use registered plugins
agent = Agent(name="Worker", instructions="Do tasks")
agent.start("Search for news")
```

<Tip>
**Plugins are global.** Once registered with `PluginManager`, all agents automatically use them. No need to pass plugins to individual agents.
</Tip>

### Alternative: Use Hooks Directly

For simple cases, use `@add_hook` without creating a plugin class:
...
def validate_results(data):
    if "error" in str(data.result).lower():
...
|------|---------|---------|
| **Hook Plugin** | Intercept lifecycle events | Logging, validation, metrics |
| **Tool Plugin** | Provide additional tools | Weather API, calculator |
| **Agent Plugin** | Modify agent behavior | Prompt filtering, response formatting |
| **LLM Plugin** | Intercept LLM calls | Token counting, caching |

```mermaid
graph LR
    subgraph "What can plugins do?"
        A["🪝 Hook"] --> A1["Log events"]
        A --> A2["Validate data"]
        A --> A3["Modify args/results"]
        
        B["🔧 Tool"] --> B1["Add new tools"]
        B --> B2["Wrap existing tools"]
        
        C["🤖 Agent"] --> C1["Filter prompts"]
        C --> C2["Format responses"]
        
        D["🧠 LLM"] --> D1["Cache responses"]
        D --> D2["Count tokens"]
    end
