...
<Tip>
`praisonai persistence doctor` now uses this same detection — not just the live path — so passing a Neon, Supabase, or Turso URL to a `--*-url` option picks the right backend automatically. See [Backend Auto-Detection](/cli/persistence#backend-auto-detection).
</Tip>
...
<Warning>
**Deprecated import.** `from praisonai.db.adapter import PraisonAIDB / NeonDB / TursoDB / ...` and the top-level `from praisonai import DB / PraisonAIDB / PraisonDB` now emit `DeprecationWarning` — as of PR [#4595](https://github.com/MervinPraison/PraisonAI/pull/4595) the top-level aliases route through `praisonai.db` so the warning fires consistently. Use the `db(...)` factory instead:

```python
from praisonaiagents import db

store = db(database_url="postgresql://...")
```
</Warning>
...
<CardGroup cols={2}>
<Card title="Local Databases" icon="hard-drive" href="/features/persistence">
  SQLite, PostgreSQL, MySQL for local development
</Card>
<Card title="Vector Databases" icon="vector-square" href="/features/rag">
