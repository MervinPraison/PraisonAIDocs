...
- With a `BackgroundJobStore` configured, `on_complete` may be re-fired once on the next boot for any job that completed-but-was-not-delivered before a crash — `reconcile_on_start()` replays the interrupted deliver-back. Keep the handler idempotent. See [Durable Background Jobs](/features/durable-background-jobs).
