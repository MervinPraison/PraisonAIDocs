...
A parent agent can fire off one or more subagents and continue working. Each background subagent returns a `job_id` immediately; poll or await results whenever you need them — or let the job push its result back to the originating chat when it finishes.
