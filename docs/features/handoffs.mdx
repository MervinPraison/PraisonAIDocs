...
<Info>
YAML handoffs through external frameworks require a handoff-capable runtime: `praisonai`, `autogen_v4`, or `openai_agents` (see `praisonai doctor`). Example YAML for OpenAI Agents: [OpenAI Agents framework](/docs/framework/openai_agents).
</Info>

## Which Handoff Setup Should I Use?

The routing agent always decides via a tool call — pick how much you configure that tool.
...
graph TB
    ...
    Q1 -->|Yes, steer the choice| B[Handoff&#40;&#41; with<br/>tool_description_override]
    Q1 -->|Need structured input| C[Handoff&#40;&#41; with<br/>input_type / input_filter]
...
sequenceDiagram
    ...
    User->>Triage: "I need a refund"
    Triage->>Triage: Match request to a handoff tool
    Triage->>Specialist: transfer_to_refunds(context)
    Specialist->>Specialist: Run full chat() pipeline
    Specialist-->>Triage: HandoffResult
    Triage-->>User: Specialist response
```

When you specify handoffs for an agent, PraisonAI automatically converts each into a tool, adds routing instructions to the prompt, and passes conversation history on transfer.

| Step | What happens |
|---|---|
| 1. Tool creation | Each handoff becomes a `transfer_to_*` tool the agent can call |
| 2. Prompt update | Routing instructions are added so the agent knows when to delegate |
| 3. Transfer | The full conversation history moves to the target agent |
