...
Handoffs in YAML workflows require one of these three runtimes: `praisonai`, `autogen_v4`, or `openai_agents`. Other framework adapters (CrewAI, AutoGen v0.2) do not implement the handoff dispatch path.

For the `openai_agents` YAML handoff shape (including spaced role names like `role: Triage Agent`), see [OpenAI Agents with PraisonAI](/docs/framework/openai_agents).
