...
description: "Delegate tasks between specialised agents — each agent does what it does best"
icon: "hand-holding-hand"
---

Handoffs let one agent pass control to another, so each specialist handles exactly the work it knows best.

```mermaid
graph LR
    subgraph "Handoff Flow"
        U[👤 User] --> T[🤖 Triage Agent]
        T -->|billing issue| B[💳 Billing Agent]
        T -->|tech issue| S[🔧 Support Agent]
        T -->|escalate| M[👔 Manager Agent]
    end
...
<AccordionGroup>
  <Accordion title="Give each agent a clear, narrow role">
    Handoffs work best when each agent has a single area of expertise. A "billing" agent shouldn't know about refunds — that's what the handoff is for.
  </Accordion>
  <Accordion title="Filter context to keep handoffs lean">
    Use `filter={"keep_last_n_messages": 5}` to pass only what the target agent needs. Full conversation history often includes noise that degrades the target agent's response.
  </Accordion>
  <Accordion title="Use TypedHandoff for structured data contracts">
    When the receiving agent needs specific fields (customer_id, order_id), use `TypedHandoff` with a Pydantic schema. This catches missing data at handoff time, not after.
  </Accordion>
  <Accordion title="Always handle the default case">
    Your triage agent should have a fallback when no specialist matches. Add a `general_agent` at the end of `handoffs=[]` or implement an `else` path so no request goes unanswered.
  </Accordion>
</AccordionGroup>
