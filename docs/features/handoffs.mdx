...
<Note>
Replaces the deprecated `allow_delegation=True` — see [Legacy Agent Parameters](/features/agent-legacy-params).
</Note>
...
<Note>
Handoffs are secure by default — the target agent only inherits tools shared with the source agent. See [Handoff Tool Policy](/features/handoff-tool-policy).
</Note>
...
<Card icon="code" href="/features/handoff-tool-policy">
...
### Concurrent handoffs to the same target

When two entries in `targets` point at the **same** specialist agent, both handoffs now run one-at-a-time on that agent. Each seeded turn is atomic, so one caller's context can never leak into another's — no user code change is needed.

```python
from praisonaiagents import Agent, parallel_handoffs
import asyncio

specialist = Agent(name="Summariser", instructions="Summarise the chunk.")
main = Agent(name="Router", instructions="Route work to specialists.")

async def main_run():
    results = await parallel_handoffs(
        main,
        targets=[
            (specialist, "Summarise chunk 1"),
            (specialist, "Summarise chunk 2"),
        ],
        max_concurrent=2,
    )
    for r in results:
...
<Note>
Because a target agent's `chat_history` is shared state, PraisonAI serialises the seed-and-chat step per target agent (a private lock attached lazily as `agent._handoff_seed_lock`). Sending both chunks above to one `specialist` no longer interleaves their seeded context. See [Multi-Agent Context Safety](/features/multi-agent-context-safety) and [Thread Safety](/features/thread-safety).
</Note>
...
<Note>
`HandoffTimeoutError` is always retryable in TypeScript too — `error.isRetryable === true` — because a timeout may succeed on a second attempt. Cycle and depth guards (`HandoffCycleError`, `HandoffDepthError`) carry `context.cycle_path` / `context.max_depth`. The handoff context accepts the back-compat aliases `chain`, `depth`, and `max_depth`. As of PR #4836, TypeScript honours `timeoutSeconds`, `maxDepth`, and `detectCycles` per handoff — the same knobs Python exposes. See [TypeScript Handoffs](/docs/js/handoffs).
</Note>
...
<CardGroup cols={2}>
<Card title="Handoff Tool Policy" icon="shield-check" href="/features/handoff-tool-policy">
  Secure tool boundaries during handoff
</Card>
<Card title="Typed Handoffs" icon="filter" href="/features/typed-handoffs">
