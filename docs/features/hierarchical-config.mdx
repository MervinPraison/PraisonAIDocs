...
<Note>
This is the JSON-based `.praison.json` system used by the `praisonai` CLI (nearest-wins, git-boundary-scoped, no merge). For YAML/TOML `.praisonai/config.*` with **deep-merge + ancestor discovery**, see [Configuration File](/features/config-file).
</Note>
...
graph LR
    subgraph "Hierarchical Configuration"
        Request[📋 User Request] --> Process[⚙️ Hierarchical Configuration]
...
graph LR
    subgraph "Hierarchical Configuration"
        In[📝 CLI in Subdir] --> Discover[🔍 Walk-up Discovery]
        Discover --> Merge[⚙️ Merge Configs]
        Merge --> Agent[🤖 Agent]
        Agent --> Out[✅ Merged Settings]
    end
...
<Note>
This JSON-based `HierarchicalConfig` system stops only at `.git` (or the 10-level depth cap). The YAML-based `praisonai config` system (see [CLI Configuration](/docs/features/cli-configuration#walk-up-boundaries)) additionally stops the walk-up at `$HOME` — the two systems have different boundaries.
</Note>
...
| `temperature` | `number` | Sampling temperature (0–2). Forwarded to every LLM call (PraisonAI PR #4245) |
...
<CardGroup cols={2}>
  <Card title="YAML CLI Configuration" icon="file-code" href="/features/cli-configuration">
    `.praisonai/config.yaml` — a separate YAML-based config system, not affected by walk-up discovery.
  </Card>
  <Card title="Advanced Features" icon="rocket" href="/features/advanced-features">
    Other advanced CLI features including safe shell, file history, and output modes.
  </Card>
  <Card title="praisonai config subcommand" icon="terminal" href="/cli/config">
    Reference for the `praisonai config` command (YAML system).
  </Card>
  <Card title="Context Files" icon="folder-open" href="/features/context-files">
