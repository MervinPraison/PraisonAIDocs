...
<Note>
`ToolSearchConfig` is lazy-loaded. Importing it (or leaving `tool_search=False`) does not initialise the tools subsystem, so it adds no startup cost.
</Note>
</Step>
...
<Card title="Allowed Tools" icon="filter" href="/features/allowed-tools">
