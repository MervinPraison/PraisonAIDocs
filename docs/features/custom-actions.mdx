...
agent = Agent(name="workflow-agent", instructions="Run custom build actions.")
agent.start("Run the greet action then bump the version.")
```
...
<Steps>
<Step title="Run actions from an agent">
```python
from praisonaiagents import Agent
...
<Step title="Reference an action in a workflow step">
```yaml
steps:
  - name: Say hello
    action: greet
...
Custom actions turn repeating logic into reusable building blocks for [job workflows](/features/job-workflows). Define them inline in YAML, as standalone Python files, or use built-in actions — all referenced with `action: my-action`.
...
sequenceDiagram
    ...
    User->>Workflow: run workflow.yaml
    Workflow->>Resolver: resolve "action: my-action"
    Resolver->>Action: YAML → file → built-in
    Action-->>Resolver: {"ok": True, "output": ...}
    Resolver-->>Workflow: step result
    Workflow-->>User: workflow output
```
