...
<Steps>
    ...
    <Step title="Create a file">
        ...
        def handle_high_priority(ctx: WorkflowContext) -> StepResult:
            return StepResult(output=f"🚨 HIGH PRIORITY: Escalating immediately!")

        def handle_support(ctx: WorkflowContext) -> StepResult:
            return StepResult(output=f"💬 SUPPORT: Routing to help desk")

        def handle_normal(ctx: WorkflowContext) -> StepResult:
            return StepResult(output=f"📋 NORMAL: Processing standard request")
