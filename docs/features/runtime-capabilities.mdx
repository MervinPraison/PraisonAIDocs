...
<Note>
`preferred_runtime` is canonicalised (case-insensitive, whitespace-tolerant, `-`/`_` interchangeable). Aliases `harness`, `plugin`, and `reduced` map to `plugin-harness`; a close typo of a known runtime raises `ValueError`. See [Runtime name normalisation](/features/runtime#runtime-name-normalisation).
</Note>
