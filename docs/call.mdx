...
The media-stream WebSocket used to receive Twilio audio no longer takes the shared `CALL_SERVER_TOKEN` in its URL. When a phone call comes in, the call server mints a **one-shot session token** with a **60-second TTL**, embeds that in the media-stream URL, and consumes it on the WebSocket handshake. Direct callers (custom integrations that open `/media-stream` themselves) should send the shared token in an `x-call-token` header instead of the URL query string.

<Note>
**Effect for operators:** nothing to configure — the change is transparent for `praisonai call` / `praisonai call --public`. Custom integrations that hard-coded the token in the media-stream URL must move it to the `x-call-token` header.
</Note>

Token comparisons are constant-time (`hmac.compare_digest`), so a wrong token no longer leaks its correctness through response timing. Connection-count and per-IP rate-limit counters are guarded by `asyncio.Lock`s, so `MAX_CONCURRENT_CONNECTIONS` and `MAX_REQUESTS_PER_WINDOW` cannot be over-committed by concurrent WebSocket opens.
