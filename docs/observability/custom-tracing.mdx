...
# 1. Create your sink (implement 3 methods)
class MySink:
    def emit(self, event):
...
```mermaid
flowchart LR
    A[Agent] -->|emit| B[ContextTraceEmitter]
    B -->|emit| C[Your Custom Sink]
    C -->|store| D[Any Destination]
...
Your sink must implement 3 methods:
...
flowchart TB
    subgraph Session
        SS[session_start] --> SE[session_end]
    end
    
    subgraph Agent
        AS[agent_start] --> AE[agent_end]
        AS --> AH[agent_handoff]
    end
    
    subgraph Operations
        MA[message_added]
        TS[tool_call_start] --> TE[tool_call_end]
        LR[llm_request] --> LP[llm_response]
    end
...
flowchart TB
    subgraph "Core SDK (praisonaiagents)"
        P[ContextTraceSink Protocol]
        E[ContextTraceEmitter]
        N[ContextNoOpSink]
        L[ContextListSink]
    end
    
    subgraph "Wrapper (praisonai)"
        W[ContextTraceWriter]
    end
...
- [Context Replay](/features/replay) - Debug agent execution
- [Observability Overview](/observability/overview) - 20+ provider integrations
- [Telemetry](/features/telemetry) - Performance monitoring
