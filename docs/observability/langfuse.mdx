...
<Note>
As of PR #1461, `atexit` auto-closes the sink — no manual flush required for CLI runs. See [Custom Tracing](/observability/custom-tracing) for the underlying `ContextTraceSinkProtocol`.
</Note>
...
`LangfuseClient` holds a persistent `requests.Session` with the auth handler attached, so every trace, session, and observation lookup during an interactive `praisonai langfuse` browse shares keep-alive connections. Use it as a context manager for deterministic cleanup:

```python
from praisonai.cli.langfuse_client import LangfuseClient

with LangfuseClient.from_config_file() as client:
    traces = client.get_traces(session_id="session-abc123")
    for trace in traces:
        detail = client.get_trace(trace["id"])
        print(detail["name"])
# session (and its keep-alive pool) is closed on scope exit
```

Or call `.close()` explicitly if you construct the client outside a `with` block.
...
```python
from praisonaiagents import Agent, Task, PraisonAIAgents, obs
...
<Card title="Agent Configuration" icon="user" href="/concepts/agents">
