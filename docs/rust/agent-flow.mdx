...
description: "Build complex workflow patterns with routing, loops, and parallelism"
...
graph TB
    subgraph "AgentFlow Patterns"
        Input[📝 Input] --> Route{🔀 Route}
        Route -->|Pattern A| Agent1[🤖 Agent 1]
        Route -->|Pattern B| Agent2[🤖 Agent 2]
        Agent1 --> Parallel[⚡ Parallel]
        Agent2 --> Parallel
        Parallel --> Loop[🔄 Loop]
        Loop --> Output[✅ Output]
    end
...
```mermaid
graph LR
    Input[📝 Input] --> Check{Check Condition}
    Check -->|True| AgentA[🤖 Agent A]
    Check -->|False| AgentB[🤖 Agent B]
