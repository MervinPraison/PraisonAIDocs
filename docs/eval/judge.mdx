...
| Rule | Behaviour |
|------|-----------|
| Label match | `SCORE:` and `REASONING:` are matched at the start of a line. Markdown-bolded (`**SCORE:** 1`) and bullet-prefixed labels are tolerated. |
| Score clamping | Values below `1` become `1.0`; values above `10` become `10.0`. |
| Unparseable score | If no numeric score can be read, the verdict fails closed — `score=1.0`, `passed=False`, `reasoning="Unable to parse response"`. |
| Shared parser | `AccuracyEvaluator`, `CriteriaEvaluator`, and `ComparisonEvaluator` all parse responses with `parse_score_reasoning` from `praisonaiagents.eval`. |

<Warning>
**Unparseable verdicts fail closed.** When nothing numeric is parsed, `Judge.run(...)` returns `score=1.0, passed=False` — not a fabricated middle score. A `5.0` returned for empty or prose-only text would silently pass any threshold ≤ 5; `1.0` fails every threshold. A `logger.warning` records the first 200 characters of the raw response. See [LLM as Judge — Fail-Closed Parsing](/features/llm-as-judge#fail-closed-parsing).
</Warning>

<Note>
`MediaEvaluator` accepts the same `SCORE:` / `REASONING:` shape for its image and text branches. Its parser matches the labels case-insensitively and keeps multi-line reasoning. See [Media Evaluator](/docs/features/media-evaluator#response-format).
</Note>
...
```bash
praisonai eval list          # short form — preferred
praisonai eval list-judges   # long form — still supported
```
...
<ResponseField name="passed" type="bool">
  `True` only when a numeric score was parsed AND `score >= threshold`. An unparseable verdict is always `passed=False`.
</ResponseField>
...
```bash List Judges (short)
praisonai eval list
```

```bash List Judges (long)
...
<CardGroup cols={2}>
  <Card title="Safety Eval" icon="shield-halved" href="/docs/features/safety-eval">
    Detect harmful, biased, or inappropriate outputs with a fail-closed guarantee
  </Card>
</CardGroup>
