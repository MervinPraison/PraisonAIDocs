...
| Rule | Behaviour |
|------|-----------|
| Label match | `SCORE:` and `REASONING:` are matched at the start of a line. |
| Score clamping | Values below `1` become `1.0`; values above `10` become `10.0`. |
| Malformed score | If `SCORE:` is missing or the value isn't numeric, the score defaults to `5.0`. |
| Malformed reasoning | If `REASONING:` is missing, reasoning defaults to `"Unable to parse response"`. |
