...
<Tabs>
  ...
  <Tab title="Goal-gated variant">
    Passing `goal=` delegates to the tool-using [goal loop](/docs/features/goal-loop) — an independent judge gates completion against acceptance criteria.

    ```python
    from praisonaiagents import Agent
    from praisonaiagents.goal import GoalCriteria

    agent = Agent(name="ops", instructions="You are an ops assistant", autonomy=True)

    result = agent.run_until(
        "Do the task",
        goal="A PR is open with green CI",
        goal_criteria=GoalCriteria(outcome="PR merged", verification="PR URL exists"),
        max_iterations=15,          # forwarded as max_turns
        judge_model="gpt-4o-mini",  # optional independent judge
    )
    # NOTE: this returns AutonomyResult, not EvaluationLoopResult
    print(result.completion_reason)  # "goal_met" | "budget_paused"
    ```

    <Warning>
    When `goal` is passed, `run_until()` delegates to `run_goal()` and returns an `AutonomyResult` (`success` / `output` / `completion_reason`) instead of the usual `EvaluationLoopResult`. Adjust downstream code that accesses `result.final_score` / `result.iterations`.
    </Warning>

    If a bare string `criteria` is passed alongside `goal`, it is promoted to `GoalCriteria(outcome=criteria)` so it is not silently dropped.
  </Tab>
</Tabs>
...
<ParamField path="metric" type="Callable[[str], float]">
...
```python
result = agent.run_until("Draft a launch email", criteria="clear and persuasive")

print(result.best_score)    # highest score across all iterations
print(result.best_output)   # output of the best iteration
print(result.final_output)  # output of the last iteration (may differ)
```
...
<ResponseField name="best" type="IterationResult">
  The highest-scoring iteration
</ResponseField>

<ResponseField name="best_iteration" type="IterationResult">
...
<ResponseField name="best_output" type="string">
...
agent = Agent(name="answerer", instructions="Answer the question.")
...
loop = EvaluationLoop(
    ...
    max_iterations=3,
    mode="review",
    metric=lambda output: word_overlap(output, "gold answer"),
)

result = loop.run("Answer the question.")
print(result.best_score, result.best_output)
```
...
<CardGroup cols={2}>
  ...
  <Card title="Goal Loop" icon="bullseye-arrow" href="/docs/features/goal-loop">
    Gate a tool-using loop on an independent acceptance-criteria judge
  </Card>
  <Card title="Prompt Optimizer" icon="wand-magic-sparkles" href="/docs/features/prompt-optimizer">
    Optimise an agent's instructions against an eval set
  </Card>
  <Card title="Trials Engine" icon="dice" href="/docs/features/trials-engine">
    Run K isolated attempts per eval case for a pass rate and frontier
  </Card>
</CardGroup>
