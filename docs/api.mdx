...
<Note>
As of [PR #3634](https://github.com/MervinPraison/PraisonAI/pull/3634), a missing `agent_file` now raises `FileNotFoundError` from `PraisonAI(...).run()` / `.arun()` and the `serve` HTTP handler (previously a silent `None`). Wrap the call in `try` / `except FileNotFoundError` if the file may be absent.
</Note>
