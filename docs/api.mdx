...
- OpenAI Agents support (requires `praisonai[openai-agents]`)
...
pip install "praisonai[openai-agents]"    # Install with OpenAI Agents support
pip install "praisonai[crewai,autogen]"   # Install multiple frameworks
...
When installing with `pip install "praisonai[openai-agents]"`, you get:
- OpenAI Agents SDK framework support via `framework: openai_agents`
- Agent handoff support (one of three runtimes that support YAML-level handoffs)
- Task chaining via `context: [task_name]`
- PraisonAI tools integration