...
Same as `execute()`, including `checkpoint` and `resume`.

<Note>
`aexecute()` has full parity with `execute()`. `loop_over` iteration, `branch_condition` / `next_steps` routing, and `checkpoint` / `resume` all work identically on the async path. Async executors, `agent.achat`, and async handlers are awaited in-loop (true async, not sync-in-a-thread).
</Note>
...
<CardGroup cols={2}>
  <Card title="Workflows Guide" icon="diagram-project" href="/docs/features/workflows">
