...
When a step exhausts its retries and its `on_error` is `"stop"` (the `Task` default), the workflow aborts instead of feeding the error forward:

```python
from praisonaiagents import Agent, Task, AgentFlow

bad = Agent(name="bad", llm="openai/does-not-exist-model")
good = Agent(name="good")

wf = AgentFlow(steps=[
    Task(name="fetch", description="fetch data", agent=bad, on_error="stop"),
    Task(name="use_result", description="use {{fetch_output}}", agent=good),
])
result = wf.start("run")

print(result["status"])              # "failed"
print(result["steps"][0]["status"])  # "failed"
print("use_result" in [s["step"] for s in result["steps"]])  # False — short-circuited
```

- **`on_error="stop"`** (the `Task` default) → step failure aborts the workflow, the error is surfaced instead of being fed forward as the next step's input, and `result["status"] == "failed"`.
- **`on_error="continue"`** → the failing step keeps its `failed` state, the workflow continues, and the final `result["status"]` is `"failed"` if any step ended in `failed`.

#### Inside nested patterns

As of PR [#4194](https://github.com/MervinPraison/PraisonAI/pull/4194), the same behaviour applies to steps nested inside `loop`, `parallel`, `when`/`if_`, `route`, and `repeat`. A stopping step inside a pattern halts the whole workflow — not just the pattern iteration — and downstream steps do not run.
...
print(result["status"])                                     # "failed"
print(any(s["step"] == "after" for s in result["steps"]))   # False — short-circuited
```

<Note>
Runs that previously reported `completed` while containing a failed nested step now correctly report `failed`. See [Error Handling](/features/workflow-error-handling) for the full guide.
</Note>
...
| `rebase_checkpoint` | `bool` | `False` | Force resume at the same step index even when the workflow definition changed since the checkpoint (skips the fingerprint check with a warning) |
...
```python
{
    ...
    "variables": Dict[str, Any],
    "resumed_from_step": int   # present only when the run resumed (0-indexed)
}
```

<Note>
On `resume`, the checkpoint's stored `definition_fingerprint` is compared
against the current workflow. A missing checkpoint or a fingerprint mismatch
(without `rebase_checkpoint=True`) returns `{"success": False, "error": ...}`
and nothing runs. The `resumed_from_step` field is added only when a run
actually resumed. See
[Workflow Checkpoint & Resume](/features/workflow-checkpoint-resume) for the
full behaviour.
</Note>
...
Same as `execute()`, including `checkpoint`, `resume`, and `rebase_checkpoint`. The result also carries the same `resumed_from_step` field when a run resumed.

<Note>
`aexecute()` has full parity with `execute()`. `loop_over` iteration, `branch_condition` / `next_steps` routing, and `checkpoint` / `resume` / `rebase_checkpoint` all work identically on the async path. Async executors, `agent.achat`, and async handlers are awaited in-loop (true async, not sync-in-a-thread).
</Note>
...
result = asyncio.run(
    manager.aexecute("deploy", default_agent=agent, checkpoint="deploy-v1")
)

# Resume from that checkpoint
result = asyncio.run(
    manager.aexecute("deploy", default_agent=agent, resume="deploy-v1")
)
```
...
List of checkpoint info dicts with keys: `name`, `workflow`, `completed_steps`, `definition_fingerprint`, `saved_at`. `definition_fingerprint` is `None` for checkpoints written before fingerprinting existed.
