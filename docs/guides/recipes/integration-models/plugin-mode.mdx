...
```mermaid
graph LR
    subgraph "Plugin Mode"
        U[📋 Host Action] --> A[🔌 Plugin Adapter]
        A --> O[✅ In-app Result]
    end
...
sequenceDiagram
    ...
    User->>Plugin: Trigger command in host app
    Plugin->>Runner: POST recipe + host context
    Runner-->>Plugin: Output
    Plugin-->>User: Render result in the UI
```
...
<CardGroup cols={2}>
  <Card title="Local HTTP Sidecar" icon="server" href="/guides/recipes/integration-models/local-http-sidecar">
    Run the sidecar plugins connect to
  </Card>
  <Card title="Remote Managed Runner" icon="cloud" href="/guides/recipes/integration-models/remote-managed-runner">
