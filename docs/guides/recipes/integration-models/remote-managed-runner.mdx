...
graph LR
    subgraph "Remote Runner"
        U[📋 HTTPS Request] --> A[☁️ Runner Cluster]
        A --> O[✅ Result]
    end
...
sequenceDiagram
    ...
    User->>LB: HTTPS request + API key
    LB->>Runner: Route to a healthy node
    Runner-->>LB: Output + trace ID
...
<AccordionGroup>
<Accordion title="Terminate TLS and auth at the load balancer">
Front the runner cluster with nginx or a managed LB so certificates and API keys are handled once, not per node.
</Accordion>

<Accordion title="Retry idempotent calls with backoff">
...
<Accordion title="Propagate a trace ID on every request">
Send `X-Trace-ID` so a single call can be followed across the LB, runner, and LLM in your traces and logs.
</Accordion>

<Accordion title="Scale horizontally, keep nodes stateless">
