...
- **Scope split** — `praisonai-code` covers only the agentic terminal product; `bots/`, `gateway/`, and `daemon/` moved to `praisonai-bot` in C9 (see [praisonai-bot Migration](/docs/guides/praisonai-bot-migration)). Although `praisonai-code` hosts the `chat`, `code`, and `run` entry points, only `chat` and `code` are **wrapper-required** on a standalone install and emit an install hint (`pip install praisonai`). Default `run "…"` and every `--output` text mode run in-process (PR #2853).
