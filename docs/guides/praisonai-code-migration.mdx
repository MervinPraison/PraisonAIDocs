...
| Terminal CLI | `praisonai-code` | `run --output actions|json|stream|stream-json`, warm runtime, CLI backends. Default `run "…"` is fully standalone; `chat` / `code` open their resident interactive TUI standalone (PR #3818) and gain the richer legacy dispatch when the wrapper is installed — see the [standalone-limits table](/docs/features/praisonai-code-cli#standalone-limits). |
...
- **Scope split** — `praisonai-code` covers the agentic terminal product; `bots/`, `gateway/`, and `daemon/` moved to `praisonai-bot` in C9 (see [praisonai-bot Migration](/guides/praisonai-bot-migration)). `praisonai-code` hosts the `chat`, `code`, and `run` entry points: default `run "…"` and every `--output` text mode run in-process (PR #2853), and `chat` / `code` open their resident interactive TUI standalone (PR #3818). The wrapper adds the richer legacy dispatch for `code`, gateway, and channel bots.
...
As of C9, the following CLI commands and features live in `praisonai-bot`, not `praisonai-code`: `gateway`, `bot`, `onboard`, `pairing`, `identity`, `kanban`, `mint_link`, `claw`. The `praisonai.bots`, `praisonai.gateway`, and `praisonai.daemon` paths remain as `alias_package` shims. See [praisonai-bot Migration](/guides/praisonai-bot-migration).
...
<Card title="Development setup" icon="wrench" href="/developers/development-setup">
  Clone the monorepo and install editable packages with uv
</Card>
<Card title="praisonai-bot Migration" icon="package" href="/guides/praisonai-bot-migration">
