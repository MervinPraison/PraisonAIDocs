...
- **Scope split** — `praisonai-code` covers only the agentic terminal product; `bots/`, `gateway/`, and `daemon/` moved to `praisonai-bot` in C9 (see [praisonai-bot Migration](/guides/praisonai-bot-migration)). `praisonai-code` hosts the `chat`, `code`, and `run` entry points: default `run "…"` and every `--output` text mode run in-process (PR #2853), and `chat` / `code` open their resident interactive TUI standalone (PR #3818). The wrapper adds only the richer legacy dispatch for `code`.
