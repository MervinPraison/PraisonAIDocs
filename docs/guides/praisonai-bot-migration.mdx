...
<Note>
**Hyphens and underscores both work at the `praisonai-bot` root.** As of PR [#2847](https://github.com/MervinPraison/PraisonAI/pull/2847), the top-level dispatcher normalizes between the two forms — `praisonai-bot mint-link` and `praisonai-bot mint_link` resolve to the same command. `--help` and shell tab-completion display the hyphenated form (`mint-link`) to match Click's convention. Existing scripts that call the underscore form keep working unchanged.
</Note>
