...
- Shim registration is safe under concurrent module imports — if another thread mutates `sys.modules` while shims are being installed (e.g. an async client loading modules during a parallel test run), the shim retries the snapshot and falls back to key-by-key reads instead of raising `RuntimeError: dictionary changed size during iteration`.
...
└── mint-link   Mint magic links
```

<Note>
Both hyphenated and underscored forms resolve at the root — `praisonai-bot mint-link` and `praisonai-bot mint_link` are equivalent. As of PR [#2847](https://github.com/MervinPraison/PraisonAI/pull/2847), the top-level dispatcher normalizes between the two forms. `--help` and shell tab-completion display the hyphenated form (`mint-link`) to match Click's convention. Use the hyphenated form in new scripts; existing scripts that call the underscore form keep working unchanged.
</Note>
...
praisonai-bot mint-link                 # hyphen form (matches --help output)
praisonai-bot mint-link --ttl 600       # with custom TTL
praisonai-bot mint_link                 # underscore form (backward-compatible)
```
...
<Note>
**Contributor rule:** `praisonai_bot` production code no longer hard-imports `praisonai_code.*` — it goes through the lazy `praisonai_bot._code_bridge.import_code_module()` boundary. The CI gate `scripts/check_bot_code_imports.sh` enforces this.
</Note>
...
Recent fixes to the `praisonai bot ...` CLI that operators upgrading should know about:

| Change | What to expect |
|--------|----------------|
| **Declared platform wins** | `bot start` respects the config's `platform:` even when the token comes from the environment. Other platforms are no longer auto-enabled from unrelated credentials — a warning names each one skipped. |
| **`token:` is optional** | Omit `token:` and the declared `platform:` resolves its own documented env var (`TELEGRAM_BOT_TOKEN`, etc.). |
| **Slack app token required** | `bot slack` now errors early if `--app-token` / `SLACK_APP_TOKEN` is missing (previously it silently subscribed to nothing). |
| **Non-blocking bots stay up** | `linear`, `email`, `agentmail`, `webhook`, and WhatsApp cloud no longer exit `0` in ~1s — the loop is held open while the adapter runs. |
| **Non-zero exit codes** | Any startup failure now exits **1**, so `systemd Restart=on-failure` and `if praisonai bot ... ; then` scripting work. |
| **New `bot run` verb** | `praisonai bot run --platform <name>` starts any registered platform — including `local`, `signal`, `webhook`, plugin channels, and `adapter:` drop-ins. See [`praisonai bot run`](/features/messaging-bots#registered-but-verb-less-platforms-praisonai-bot-run). |
...
<CardGroup cols={2}>
<Card title="praisonai-code Migration" icon="package" href="/guides/praisonai-code-migration">
  The C0–C6 sibling extraction that moved the agentic terminal CLI into its own package
</Card>
<Card title="Python Wrapper" icon="box" href="/developers/wrapper">
