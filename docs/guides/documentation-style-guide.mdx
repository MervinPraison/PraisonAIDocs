...
agent = Agent(
    name="Research Agent",
    instructions="Research and summarise topics clearly.",
)
...
| **Rust** | `praisonai-package/src/praisonai-rust/praisonai/src/` | `docs/rust/` | `docs/rust/DOCS_PARITY.md` |

<Note>
The Rust crate is at `praisonai-rust/praisonai/src/`, not `praisonai-rust/src/`. The `src/praisonai-rust/src/**` path does not exist and was previously wired into the parity workflows — see upstream PR #4689.
</Note>

<Note>
**Two parity trackers, two questions.**

PraisonAI uses two distinct parity systems — do not conflate them.
...
```
praisonai-rust/praisonai/src/
├── agents/           # Agent implementations and builders
├── llm/              # LLM providers
├── tools/            # Tool system
├── memory/           # Memory stores
├── mcp/              # MCP client
├── parity/           # Parity-shim modules (config, telemetry, plugins, param_resolver)
├── display/          # Display and callbacks
├── planning/         # Planner and PlanStep
├── session/          # Session
├── streaming/        # Streaming metrics
├── sandbox/          # Sandbox config
├── embedding/        # Embedding config
├── error.rs          # Result/error helpers
└── lib.rs            # Main exports
```
</Tab>
</Tabs>
...
<Card title="[Feature] API Reference" icon="code" href="/docs/sdk/typescript">
...
```mdx
<CardGroup cols={2}>
<Card title="Card Title" icon="icon-name" href="/docs/sdk">
Card description.
</Card>
<Card title="Card Title" icon="icon-name" href="/docs/sdk">
Card description.
