...
```
Turn pre-flight failed — the agent did not produce a reply: <detail>
```

**Cause:** The channel credentials are valid, but the resolved agent's model provider credential is missing / invalid / rate-limited / out of quota. The startup real-turn check (`--verify-turn`, on by default) sends one `"ping"` prompt to the agent and the model round-trip failed.

**Fix (any of):**

<Steps>
<Step title="Add the model/provider API key">
```bash
# Add the key (e.g. OPENAI_API_KEY) to ~/.praisonai/.env, then:
...
```bash
praisonai gateway test --turn ping
```
</Step>

<Step title="Temporarily skip the turn check">
```bash
praisonai gateway start --no-verify-turn
```
...
<Note>
The turn pre-flight is independent of `--preflight` — it catches most `REASON_MISSING_KEY` / `REASON_AUTH_PERMANENT` credential problems at startup instead of on the first inbound message. See [Gateway Readiness → Turn pre-flight](/features/gateway-readiness#turn-pre-flight).
</Note>
...
<Note>
Since [PR #4197](https://github.com/MervinPraison/PraisonAI/pull/4197), the **cross-uid** variant of this symptom is fixed on `main`: a gateway owned by another user is now treated as running, so its lock is preserved and a second gateway is refused rather than started on the same token. Before the fix, a foreign-uid gateway was reported dead, its lock was deleted, and a duplicate poller could start.
</Note>
...
```
ERROR: Error checking gateway server status:  returned a result with an exception set
```
...
<Step title="Read the line as advisory">
...
<Step title="Confirm with the health endpoint">
```bash
curl http://127.0.0.1:8765/health
```
...
<Step title="Inspect the lock file if needed">
```powershell
# PowerShell
Get-Content $env:USERPROFILE\.praisonai\gateway-127_0_0_1-8765.pid
```
The lines are `pid`, `host`, `port`, `timestamp`, and (since [PR #4197](https://github.com/MervinPraison/PraisonAI/pull/4197)) a `create_time` start-time fingerprint. The 5th line may be empty on older 4-line locks or when psutil is unavailable — the lock then degrades to the previous PID-only check. A stale PID is cleaned up automatically on the next start/stop/status call.
</Step>
</Steps>

<Note>
Related: [PraisonAI #2844](https://github.com/MervinPraison/PraisonAI/issues/2844), fixed by [PR #2849](https://github.com/MervinPraison/PraisonAI/pull/2849).
</Note>
...
<Steps>
<Step title="Check the agent_id you pass">
```python
GatewayClient(url="ws://localhost:8765", agent_id="assistant")
#                                                  ^^^^^^^^^ must exist on the server
```
</Step>

<Step title="List agents registered on the gateway">
```bash
...
<Step title="Fix the id or register the agent">
...
<Step title="Surface the failure without log scraping">
Setting `client.on_reconnect_paused` before `connect()` gives you an immediate callback with `(code, next_step)` so the failure surfaces without waiting for a log scrape. See [Gateway Client → Terminal vs Transient Connect Errors](/docs/features/gateway-client#terminal-vs-transient-connect-errors).
</Step>
</Steps>
...
<Steps>
<Step title="Confirm the degraded owner and its retry_hint">
```bash
curl http://127.0.0.1:8765/health | jq '.push, .degraded_owners'
```
Look for `push.redis_degraded: true` and a `route:redis-pubsub` entry whose `retry_hint` points you at `praisonai gateway doctor`.
</Step>

<Step title="Run gateway doctor">
```bash
...
<Step title="Verify Redis reachability from the gateway host">
```bash
...
See [Real-Time Push Notifications → HA & cross-instance delivery](/features/push-notifications#ha--cross-instance-delivery) for the full contract.
...
See [Gateway Handshake Protocol](/features/gateway-handshake-protocol) for the full field reference and `ConnectRecoveryStep` values.
...
<Steps>
<Step title="Preview (writes nothing, exits 1)">
```bash
...
<Step title="Apply (mints, persists, re-validates)">
```bash
...
An explicit weak `auth_token` in `gateway.yaml` is rewritten in place. `${ENV}` references are left alone — they resolve from `~/.praisonai/.env`, which `--fix` already updated. See [Gateway CLI › Auto-repair](/docs/features/gateway-cli#auto-repair-with-fix).
...
See [Gateway Config Reload](/features/gateway-config-reload) for file-watch vs SIGHUP paths and YAML keys.
...
### Bot answers group @mentions with no context of the preceding conversation

You're on `group_policy: mention_only` — unmentioned messages are dropped and leave no session record, so an @mention only sees its own text. Switch to `group_policy: observe` (Telegram) so the transcript retains unmentioned messages as passive context. Combine with `session_scope: per_chat` for shared group memory. Note that a **reply to the bot** is still admitted as an implicit mention, so the natural follow-up (tap Reply on the bot's answer) works without re-typing `@bot` — see [Implicit Mentions](/features/bot-implicit-mentions) to disable this or enable quote/thread signals.
...
channels:
  telegram:
    ...
    group_policy: observe        # keep unmentioned messages as context
    session:
      session_scope: per_chat    # share that context across the group
```

<Note>
`observe` is Telegram-only today — Slack, Discord, and WhatsApp ignore it. See [Observe Mode](/features/bot-observe-mode).
</Note>
...
**Cause:** A **systemic** upstream fault — a bad shared provider, a network partition, or an org-wide expired token — is restarting every channel at once. The [fleet-level breaker](/features/gateway-crash-loop-guard#fleet-level-breaker) tripped one aggregate breaker instead of letting each channel silently burn its own `max_restarts_per_hour` budget.

<Steps>
<Step title="Diagnose the shared cause">
```bash
...
<Step title="Fix the underlying fault">
...
<Step title="Wait one cooldown, then confirm recovery">
...
```bash
curl http://127.0.0.1:8765/health | jq '.channels'   # fleet owner should be gone
```
</Step>
</Steps>
...
See [Gateway Config Migration](/features/gateway-config-migration) for before/after YAML.
