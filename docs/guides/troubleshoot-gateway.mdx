...
You're on `group_policy: mention_only` — unmentioned messages are dropped and leave no session record, so an @mention only sees its own text. Switch to `group_policy: observe` (Telegram) so the transcript retains unmentioned messages as passive context. Combine with `session_scope: per_chat` for shared group memory.
