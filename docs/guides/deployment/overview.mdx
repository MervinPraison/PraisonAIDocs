---
title: "Deployment Overview"
sidebarTitle: "Deployment Overview"
description: "Options for deploying agents to production"
icon: "book"
---
Deploy your PraisonAI agents to production environments.

```python
from praisonaiagents import Agent

agent = Agent(
    name="Production Agent",
    instructions="Handle production workloads.",
)

agent.start("Ready for production deployment")
```

The user chooses a hosting pattern, configures secrets and scaling, then points clients at the running agent.

```mermaid
graph LR
    U[Input] --> A[Agent]
    A --> O[Output]

    classDef agent fill:#8B0000,color:#fff
    classDef tool fill:#189AB4,color:#fff

    class A agent
    class U,O tool
```


## Quick Start

<Steps>
<Step title="Simple Usage">
  Start with an agent-centric approach:
  
  ```python
  from praisonaiagents import Agent
  
  # Create production-ready agent
  agent = Agent(
      name="Production Agent",
      instructions="Handle production workloads",
  )
  
  # Deploy to production
  agent.start("Ready for production deployment")
  ```
</Step>

<Step title="With Configuration">
  Choose between different deployment options based on your needs.
</Step>
</Steps>

## Deployment Methods

<CardGroup cols={2}>
  <Card title="24/7 Production Setup" icon="rocket-launch" href="/guides/deployment/production-minimal">
    **Start here** for always-on deployment with process supervision
  </Card>
  <Card title="Advanced Production" icon="gear" href="/tutorials/production-deployment">
    Comprehensive production guide with monitoring and scaling
  </Card>
</CardGroup>

## Deployment Options

| Option | Best For | Complexity | Guide |
|--------|----------|------------|-------|
| **Production Minimal** | Always-on 24/7 operation | **Low** | [📖 Guide](/docs/guides/deployment/production-minimal) |
| Compose Stack | Local API + Postgres, one command | Low | [📖 Guide](/docs/features/compose-stack) |
| Docker Compose | Containerized deployments | Low | [📖 Guide](/docs/guides/deployment/docker) |
| Fly.io | One-command PaaS deploy | Low | [📖 Guide](/docs/features/fly) |
| Railway | One-command PaaS deploy | Low | [📖 Guide](/docs/features/railway) |
| Render | One-command PaaS deploy | Low | [📖 Guide](/docs/features/render) |
| Kubernetes (Agents API chart) | Scalable production + Postgres | Medium | [📖 Guide](/docs/features/helm-chart-agents-api) |
| Cloud Functions | Serverless | Low | [📖 Guide](/docs/sdk/praisonai/deploy) |
| API Server | REST/WebSocket APIs | Medium | [📖 Guide](/docs/sdk/praisonai/deploy) |

## Quick Deploy with CLI

```bash
# Deploy as API server
praisonai deploy --type api

# Deploy as Docker container
praisonai deploy --type docker

# Deploy to cloud
praisonai deploy --type cloud --provider aws

# Run the local API + Postgres compose stack
praisonai deploy compose up

# Scaffold and deploy to Fly.io in one line
praisonai deploy create --template fly && praisonai deploy fly
```

## API Server

```python
from praisonaiagents import Agent
from praisonaiagents.api import serve

agent = Agent(name="API Agent")

# Start API server
serve(agent, host="0.0.0.0", port=8000)
```

## Environment Variables

```bash
# Required
export OPENAI_API_KEY="sk-..."

# Optional
export PRAISONAI_LOG_LEVEL="INFO"
export PRAISONAI_DB_URL="postgresql://..."
```

## Related

<CardGroup cols={2}>
  <Card title="Production Minimal" icon="rocket-launch" href="/guides/deployment/production-minimal">
    Always-on deployment
  </Card>
  <Card title="Docker Deployment" icon="docker" href="/guides/deployment/docker">
    Containerised agents
  </Card>
</CardGroup>
