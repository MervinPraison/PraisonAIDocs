...
sequenceDiagram
    ...
    Docker->>Agent: agent.launch() on port 8000
...
if __name__ == "__main__":
    # 0.0.0.0 is required so the agent is reachable outside the container.
    # Binding non-loopback enforces a token: set PRAISONAI_LAUNCH_AUTH_TOKEN
    # or the SDK auto-generates and prints a one-time token.
    # See /features/agent-api-launch#authentication
...
`agent.launch(host="0.0.0.0", port=8000, path="/agent")` makes the agent reachable from outside the container. Map it with `-p 8000:8000`. Because this is a non-loopback bind, set `PRAISONAI_LAUNCH_AUTH_TOKEN` (or use the one-time token the SDK prints) — see [Authentication](/features/agent-api-launch#authentication).
</Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Deploy Module" icon="rocket" href="/sdk/praisonai/deploy">
