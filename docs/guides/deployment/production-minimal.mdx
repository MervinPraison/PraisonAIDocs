...
# 24/7 Production Minimal Guide

This guide provides a **prescriptive** path to deploy PraisonAI agents for **always-on operation** similar to OpenClaw's onboarding experience. Choose your preferred deployment method below.
...
<Accordion title="Reverse Proxy with SSL">
  ```nginx
  # /etc/nginx/sites-available/praisonai
  server {
...
<Accordion title="Auto-updates">
  ...
  cat > update-praisonai.sh << 'EOF'
  #!/bin/bash
  cd /path/to/praisonai-deploy
  docker-compose pull
  docker-compose up -d
  docker system prune -f
  EOF
  
  chmod +x update-praisonai.sh
  
  # Add to crontab for weekly updates
  (crontab -l 2>/dev/null; echo "0 2 * * 0 /path/to/update-praisonai.sh") | crontab -
  ```
</Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Security Setup" icon="shield" href="/docs/best-practices/security">
    Production security best practices
  </Card>
  <Card title="Monitoring" icon="chart-line" href="/docs/features/telemetry">
