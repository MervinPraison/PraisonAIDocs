...
<Note>
Model-name lookup in `get_dimensions()` is case-insensitive as of [PraisonAI PR #3027](https://github.com/MervinPraison/PraisonAI/pull/3027) — `all-MiniLM-L6-v2` and `sentence-transformers/all-MiniLM-L6-v2` both resolve to `384`. Earlier releases returned `1536` for these names, producing dimension-mismatch errors on memory / RAG upsert.
</Note>
