...
Run agents at the same time by setting `async_execution=True` on each Task inside a `workflow` (or `sequential`) process:
...
team = AgentTeam(
    ...
    tasks=[
        Task(description="First analysis", agent=agent1, async_execution=True),
        Task(description="Second analysis", agent=agent2, async_execution=True),
        Task(description="Third analysis", agent=agent3, async_execution=True),
    ],
...
<Card title="Next: Agent Process" icon="arrow-right" href="/docs/course/agents/08-agent-process">
