...
All agents work at the same time when each Task sets `async_execution=True` inside a `workflow` process:
...
team = AgentTeam(
    ...
    tasks=[
        Task(description="Analysis A", agent=analyst1, async_execution=True),
        Task(description="Analysis B", agent=analyst2, async_execution=True),
        Task(description="Analysis C", agent=analyst3, async_execution=True),
    ],
...
<Card title="Next: Building Conversational Agents" icon="arrow-right" href="/docs/course/agents/14-conversational-agents">
