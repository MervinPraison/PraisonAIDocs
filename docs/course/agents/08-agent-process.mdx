...
<Tip>
Best for: Independent tasks that can run simultaneously. Set `async_execution=True` on each `Task` to fan them out in parallel — `process="parallel"` is not valid and `PraisonAIAgents(...)` raises a `ValueError` for any value outside `sequential`, `hierarchical`, or `workflow`.
</Tip>
