...
for event in run_recipe("ai-code-refactorer", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-code-refactorer", input={"input": "data"})
```
...
- [Test Generator](/docs/examples/agent-recipes-new/developer/ai-test-generator)
