...
for event in run_recipe("ai-seo-optimizer", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-seo-optimizer", input={"input": "data"})
```
...
- [Social Media Generator](/docs/examples/agent-recipes-new/web-content/ai-social-media-generator)
