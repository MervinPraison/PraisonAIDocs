...
description: "Extract key moments and highlights from videos"
icon: "film"
---
...
for event in run_recipe("ai-video-highlight-extractor", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-video-highlight-extractor", input={"input": "data"})
```
...
- [Video Compressor](/docs/examples/agent-recipes-new/video-audio/ai-video-compressor)
