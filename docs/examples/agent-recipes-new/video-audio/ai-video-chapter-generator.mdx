...
for event in run_recipe("ai-video-chapter-generator", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-video-chapter-generator", input={"input": "data"})
```
