...
for event in run_recipe("ai-video-compressor", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-video-compressor", input={"input": "data"})
```
...
- [Video Highlight Extractor](/docs/examples/agent-recipes-new/video-audio/ai-video-highlight-extractor)
- [Video Chapter Generator](/docs/examples/agent-recipes-new/video-audio/ai-video-chapter-generator)
- [Subtitle Generator](/docs/examples/agent-recipes-new/video-audio/ai-subtitle-generator)
