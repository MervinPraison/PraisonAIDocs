...
description: "Noise removal, EQ, loudness normalization"
icon: "volume-high"
---
...
for event in run_recipe("ai-audio-enhancer", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-audio-enhancer", input={"input": "data"})
```
