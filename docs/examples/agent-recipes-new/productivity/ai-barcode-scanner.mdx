...
description: "Extract data from barcodes and QR codes"
icon: "barcode-read"
---
...
for event in run_recipe("ai-barcode-scanner", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-barcode-scanner", input={"input": "data"})
```
...
- [Email Parser](/docs/examples/agent-recipes-new/productivity/ai-email-parser)
- [Calendar Scheduler](/docs/examples/agent-recipes-new/productivity/ai-calendar-scheduler)
- [File Organizer](/docs/examples/agent-recipes-new/productivity/ai-file-organizer)
