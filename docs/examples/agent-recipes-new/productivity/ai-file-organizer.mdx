...
description: "Auto-organize files into folders"
icon: "folder-tree"
---
...
for event in run_recipe("ai-file-organizer", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-file-organizer", input={"input": "data"})
```
