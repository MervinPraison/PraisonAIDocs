...
description: "Parse and schedule events from text"
icon: "calendar"
---
...
for event in run_recipe("ai-calendar-scheduler", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-calendar-scheduler", input={"input": "data"})
```
...
- [Note Summarizer](/docs/examples/agent-recipes-new/productivity/ai-note-summarizer)
