...
description: "AI upscale images 2x-8x with quality preservation"
...
for event in run_recipe("ai-image-upscaler", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-image-upscaler", input={"input": "data"})
```
...
- [Background Remover](/docs/examples/agent-recipes-new/images/ai-background-remover)
- [Watermark Remover](/docs/examples/agent-recipes-new/images/ai-watermark-remover)
- [Watermark Adder](/docs/examples/agent-recipes-new/images/ai-watermark-adder)
