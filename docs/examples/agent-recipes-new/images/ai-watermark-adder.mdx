...
description: "Batch add watermarks and logos to images"
icon: "droplet"
---
...
for event in run_recipe("ai-watermark-adder", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-watermark-adder", input={"input": "data"})
```
