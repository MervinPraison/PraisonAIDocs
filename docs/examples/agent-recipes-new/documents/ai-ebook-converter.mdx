...
description: "Convert documents to EPUB/MOBI formats"
icon: "book"
---
...
for event in run_recipe("ai-ebook-converter", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-ebook-converter", input={"input": "data"})
```
