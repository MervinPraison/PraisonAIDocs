...
description: "Extract key terms and obligations from contracts"
icon: "file-contract"
---
...
for event in run_recipe("ai-contract-analyzer", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-contract-analyzer", input={"input": "data"})
```
...
- [Invoice Processor](/docs/examples/agent-recipes-new/documents/ai-invoice-processor)
- [Resume Parser](/docs/examples/agent-recipes-new/documents/ai-resume-parser)
- [Meeting Summarizer](/docs/examples/agent-recipes-new/documents/ai-meeting-summarizer)
