...
description: "Summarize meeting transcripts with action items"
icon: "users"
---
...
for event in run_recipe("ai-meeting-summarizer", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-meeting-summarizer", input={"input": "data"})
```
...
- [Contract Analyzer](/docs/examples/agent-recipes-new/documents/ai-contract-analyzer)
