...
for event in run_recipe("ai-sentiment-analyzer", input={}, stream=True):
    print(event)
```
...
```python
from praisonai.recipes import run_recipe
result = run_recipe("ai-sentiment-analyzer", input={"input": "data"})
```
...
- [Data Anonymizer](/docs/examples/agent-recipes-new/data/ai-data-anonymizer)
