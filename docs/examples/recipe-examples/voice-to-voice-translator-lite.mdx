...
<Steps>
  ...
  <Step title="Create README.md">
    ...
    Translate spoken audio with optional TTS output.

    ## Quick Start

    ```bash
    praison recipes run voice-to-voice-translator-lite \
      --input '{"audio_path": "speech.mp3", "target_language": "es"}'
    ```

    ## TTS Configuration (Optional)

    For audio output, set:
    ```bash
    export ELEVENLABS_API_KEY=your_key
    ```

    Without TTS, the recipe outputs a translated transcript only.
