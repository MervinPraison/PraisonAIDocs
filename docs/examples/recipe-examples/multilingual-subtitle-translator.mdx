...
<Steps>
    ...
    def run(input_data: dict, config: dict = None) -> dict:
        ...
        try:
            ...
            for i in range(0, len(subtitles), batch_size):
                ...
                agents = PraisonAIAgents(agents=[translator], tasks=[task])
                result = agents.start()
  ...
  <Step title="Create README.md">
    ...
    praison recipes run multilingual-subtitle-translator \
      --input '{"subtitles_file": "video.srt", "target_language": "es"}'
    ```

    ## Supported Languages

    Any language code: en, es, fr, de, it, pt, ja, zh, ko, ar, ru, etc.
