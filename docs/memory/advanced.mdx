...
See [Dakera Memory](/features/dakera-memory) for the full guide.
...
Memory quality scoring uses `litellm` when available and falls back to the OpenAI client. Point it at any OpenAI-compatible local server by either:

**Via memory config (recommended when the rest of your memory layer is config-driven):**
...
<Note>
Before [PR #4812](https://github.com/MervinPraison/PraisonAI/pull/4812), the OpenAI-client fallback in `calculate_quality_metrics` silently ignored `base_url` supplied through config — it worked only if you *also* exported `OPENAI_BASE_URL`. If you were seeing memory reads/writes go to your local server but quality scoring still hit `api.openai.com`, that was the bug; it is now fixed.
</Note>
