...
| Need | Solution | Link |
|------|----------|------|
| Persist conversations | Database adapters | [Storage →](/memory/storage) |
| Resume sessions | Session ID | [Session Resume →](/persistence/session-resume) |
| Store user preferences | Long-term memory | [Advanced Memory →](/features/advanced-memory) |
| Track relationships | Graph memory | [Graph Memory →](/features/graph-memory) |
| Decay-weighted, self-hosted recall | Dakera memory | [Dakera Memory →](/features/dakera-memory) |

## Key Concepts

<CardGroup cols={2}>
  <Card title="Session Persistence" icon="clock-rotate-left" href="/persistence/overview">
    Continue conversations across restarts
  </Card>
  <Card title="Database Support" icon="database" href="/memory/storage">
...
<CardGroup cols={2}>
  <Card title="Storage Options" icon="hard-drive" href="/memory/storage">
    Configure database backends
  </Card>
  <Card title="Quick Start" icon="rocket" href="/persistence/quickstart">
