...
| `"chroma"` | `MemoryConfig(backend="chroma")` |
...
<Note>
You can add provider-specific settings with `config=` — the outer `backend=` still takes effect: `MemoryConfig(backend="sqlite", config={"short_db": "./mem/short.db"})` builds a **sqlite** store. Only a `provider`/`backend` key *inside* `config=` overrides it. See [Memory → Combining `backend` and `config`](/features/memory#combining-backend-and-config).
</Note>
...
graph TD
    ...
    B -->|"backend=chroma"| V["ChromaDB (vector)"]
    B -->|"db=db(url=...)"| DB["Redis / Postgres store"]
...
`FileMemory` persists to `~/.praisonai/memory/`. *Writes are atomic — see [Storage → Durability](/memory/storage#durability).*
...
<CardGroup cols={2}>
  ...
  <Card title="Storage Backends" icon="database" href="/memory/storage">
    File, SQLite, ChromaDB, mem0, MongoDB, Dakera, in-memory
  </Card>
