...
output="text"      # Response only with tool trace
output="status"    # Clean inline timestamps: [HH:MM:SS] ▸ tool → result ✓
output="verbose"   # Task + Tools inline + Response panel
output="debug"     # verbose + metrics footer
...
| IDE autocomplete | Config | `output=OutputConfig(verbose=True)` |
