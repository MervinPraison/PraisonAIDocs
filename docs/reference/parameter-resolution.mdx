...
<Note>
`postgresql://` and `redis://` are **not** accepted as `MemoryConfig` string URLs — they raise a `ValueError`. Use `memory=MemoryConfig(db=db(url="redis://..."))` instead. See [Redis persistence](/features/persistence-redis).
</Note>
...
See [Memory → Combining `backend` and `config`](/features/memory#combining-backend-and-config).
...
| `execution` | ✅ | ✅ | ✅ | ✅ | Execution settings (max_iter, timeout) |
...
- [Agent Reference](/sdk/praisonaiagents/agent/agent) - Core Agent class
- [Agents Reference](/sdk/praisonaiagents/agents/agents) - Multi-agent orchestration
- [Memory](/concepts/memory) - Memory configuration details
- [Knowledge](/rag/quickstart) - RAG and knowledge base setup
