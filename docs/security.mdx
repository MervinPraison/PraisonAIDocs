...
If you're using the auto-generated API server (created by `praisonai deploy run --type api`):

<Steps>
  <Step title="Capture the Bearer Token">
    When you start your API server, look for this output in stderr:
    ```
    [praisonai-api] generated API token: abc123def456...
    ```
    Copy this token.
  </Step>
  ...
  <Step title="Alternative: Disable Auth (Not Recommended)">
...
For detailed API authentication setup, see [API Server Authentication](/features/api-server-auth).
