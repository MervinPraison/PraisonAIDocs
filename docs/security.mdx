...
<Note>
`enable_injection_defense()` registers both a `before_tool` and `before_agent` hook. It returns the `before_tool` hook ID as a string. Use `remove_hook("before_tool", hook_id)` to remove it.
</Note>
...
# hook_id is the before_tool hook ID (str)
# before_agent hook is also registered automatically
```

Also in PR #2062:
- `execute_command` blocks `DANGEROUS_COMMANDS` by default (`allow_dangerous=False`)
- `write_file` and `apply_diff` enforce protected paths
- Audit log uses append-only per-call writes (no shared file handle)
