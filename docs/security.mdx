...
<Note>
For bot and gateway deployments, two provenance defences complement injection defense: [Untrusted Request Fencing](/features/untrusted-request-fencing) (prompt-plane — wraps untrusted payload content as data) and [Control-Trust Provenance](/features/bot-control-trust) (control-plane — untrusted producers cannot drive `/stop`, `/new`, `/compress` even from an owner identity).
</Note>
...
### Recent Security Hardening (PraisonAI #3315)

[PR #3315](https://github.com/MervinPraison/PraisonAI/pull/3315) (fixes [#3313](https://github.com/MervinPraison/PraisonAI/issues/3313)) neutralises untrusted platform metadata before it is concatenated into the per-chat attribution prefix.

```mermaid
graph LR
    A[👤 raw display name<br/>Hostile newline SYSTEM directive] --> B[🧼 neutralize_untrusted_text]
    B --> C[✅ safe single line]
    C --> D[🧠 sender prefix<br/>into per-turn prompt]
...
**What changed:**
In `per_chat` scope, the bot prepends a `[{sender}] ` prefix to every turn in group / multi-party channels. That prefix now automatically neutralises the raw platform display-name / group-title before formatting: newlines are collapsed to spaces, control characters stripped, whitespace collapsed, and the value length-bounded to 240 characters. Ordinary names (`"Bob"`, `"Alice 🙂"`) render byte-identically — no visible change.

**Threat closed:**
An embedded newline in a hostile group display name previously let the attacker's name masquerade as a fake markdown heading / system directive that the model re-read every turn — a prompt-injection vector. Group / multi-party channels are the framework's largest untrusted-input surface; this closes the vector at the one call site that concatenates the name into the prompt.

<Note>
**On-by-default, backward compatible.** No YAML flag, no opt-in step, no visible change for well-behaved names. The default *is* the protection.
</Note>

**New public helper:** `neutralize_untrusted_text(value, *, max_chars=240)` from `praisonaiagents.session.context`. Apply it anywhere you concatenate a user-controlled string into a prompt — a display name, a chat title, a webhook-provided label, an inbound tool argument.

```python
from praisonaiagents.session.context import neutralize_untrusted_text

# Sanitise any untrusted string before interpolating it into a prompt.
safe_label = neutralize_untrusted_text(untrusted_display_name)
prompt = f"[{safe_label}] {user_message}"

# Tighter bound for a value that must stay short (e.g. a tab title):
safe_title = neutralize_untrusted_text(untrusted_title, max_chars=64)
```

Semantics:
- Collapses `\r` / `\n` (and other line separators) to single spaces so a hostile value cannot fake a new line, heading, or system directive.
- Strips ASCII / Unicode control characters.
- Collapses runs of whitespace to a single space.
- Hard length bound to `max_chars` (default `240`) — even `max_chars <= 3` returns `len(out) <= max_chars` (the `"..."` marker is dropped in that range).
- Non-string inputs are stringified first, so `None`, integers, and arbitrary objects are all safe to pass.

<Tip>
**Fallback guarantee.** Even if the wrapper runs against an older core that satisfies `praisonaiagents >= 1.6.152` but lacks the helper, the bot uses an inline sanitiser with byte-identical semantics — you still get the protection.
</Tip>

### Recent Security Hardening (PR #3616)

[PR #3616](https://github.com/MervinPraison/PraisonAI/pull/3616) (fixes [#3615](https://github.com/MervinPraison/PraisonAI/issues/3615)) closes a symlink-based bypass in the protected-path guard.

**What changed:**
- `is_protected(path)` and `get_protection_reason(path)` in `praisonai.security.protected` now `os.path.realpath()` the input before matching.
- A same-directory symlink to a protected file (for example, `harmless.txt → .env`) is now treated as the real target and blocked, instead of being treated as `harmless.txt` and allowed.
- Centralised in the guard, so `write_file` / `apply_diff` / `search_replace` all inherit the fix.

**Who is affected:**
Anyone whose agent can write files inside a workspace that also contains protected files (`.env`, `*.pem`, `id_rsa`, etc.) and whose LLM can be talked into creating a symlink first.

**Action required:**
None. On by default. Verify by attempting a `write_file("harmless.txt", ...)` where `harmless.txt` is a symlink to `.env` — the write should now be rejected with the standard protected-path reason (previously it silently overwrote `.env`).

See [Protected Paths](/features/protected-paths).

### Recent Security Hardening (PR #3812)

[PR #3812](https://github.com/MervinPraison/PraisonAI/pull/3812) (fixes [#3811](https://github.com/MervinPraison/PraisonAI/issues/3811)) splits the injection defense's Check 1 instruction patterns into **strict** and **soft** tiers so benign role-play / coaching prompts stop triggering a block.

**What changed:**
- Instruction-override patterns now live in two sets. **Strict** phrases (`ignore all previous instructions`, `disregard your previous prompt`, `DAN:`, `jailbreak`, `unrestricted mode`, …) are unambiguous jailbreaks — a single hit is auto-`HIGH` and still blocks on one signal.
- **Soft** phrases (`you are now …`, `pretend to be …`, `roleplay as …`, `new instructions:`) are common in ordinary prompts. On their own they are only `MEDIUM` (log + warn, not blocked); they escalate to `HIGH` only when combined with another signal (for example an authority claim).
- Two new public helpers ship in `praisonai.security.injection`: `detect_instruction_strict()` and `detect_instruction_soft()`. `detect_instruction_patterns()` stays a back-compat union of both sets.

**Who is affected:**
Anyone running `enable_injection_defense()` / `scan_text()`. Benign prompts like `"You are now analysing the market data"` or `"Pretend to be a writing coach"` are no longer blocked, while real jailbreaks (`ignore all previous instructions`, `DAN`) still block on a single hit.

<Note>
**On-by-default, no config change.** The tiered behaviour is the default — no YAML flag, no opt-in step. Real attacks are still caught on one signal; only ambiguous soft phrases were downgraded.
</Note>

See [Security Best Practices → The 6-Check Injection Pipeline](/best-practices/security#the-6-check-injection-pipeline).

### Recent Security Hardening (PR #3837)

[PR #3837](https://github.com/MervinPraison/PraisonAI/pull/3837) (fixes [#3836](https://github.com/MervinPraison/PraisonAI/issues/3836)) closes a nesting-based bypass in the injection scanner and hardens the before-tool gate to fail closed.

**What changed:**
- `InjectionDefense._extract_strings` was recursive and depth-capped at 4 — a payload nested in 5+ dicts was silently unscanned and the tool executed unmediated.
- Replaced with an iterative walker (`_extract_strings_bounded`) bounded by **1 MiB total scanned bytes** (`_EXTRACT_MAX_TOTAL_BYTES`) and **10 000 strings** (`_EXTRACT_MAX_STRINGS`), not by depth. A legitimately nested tool argument (filter DSL, JSON-schema payload, handoff routing config) is fully scanned; pathological input cannot OOM the process.
- Scans **dict keys** too (attacker-controlled keys reach the prompt in many frameworks), is **cycle-safe** via a seen-set on container `id()`, and logs a `WARNING` when the budget is exhausted.
- The before-tool hook now **blocks** any call whose input exceeded the scan budget (`HookResult(decision="block", reason="Injection defense: tool input exceeded the scan budget and could not be fully vetted [CRITICAL]")`) — a fail-closed gate instead of allow-on-no-match. Allow-on-no-match would let an attacker pad benign strings ahead of an injection payload so the payload is never reached.

<Warning>
A 5-deep nested payload — `{"a": {"b": {"c": {"d": {"e": "ignore all previous instructions"}}}}}` — previously bypassed the scanner because the walk returned `[]` past depth 4. It now **blocks**.
</Warning>
...
sequenceDiagram
    ...
    User->>Defense: tool input (nested / large)
    Defense->>Defense: walk bounded (bytes + cardinality)
    alt truncated (budget exhausted)
        Defense-->>User: block [CRITICAL]
    else fully scanned, no match
        Defense-->>Tool: forward
    end
```

**Who is affected:**
Anyone running `enable_injection_defense()` or `scan_text()`.

**Action required:**
None. On by default. The new fail-closed behaviour may block a tool call that ships an unusually large tool input (>1 MiB of strings, or >10 000 strings). Operators can grep for the log line `"[praisonai.security] extraction bounded: ..."` to spot it.

The back-compat shim `_extract_strings(obj)` still returns just the string list (dropping the `truncated` flag), so external callers of the old private helper keep working; the security hook itself uses the bounded variant so it can observe truncation.

See [Security Best Practices → The 6-Check Injection Pipeline](/best-practices/security#the-6-check-injection-pipeline).

### Recent Security Hardening (PR #3855)

**Hook exceptions and timeouts fail closed ([PR #3855](https://github.com/MervinPraison/PraisonAI/pull/3855)).** Every hook lifecycle path — function hooks, command hooks, and the top-level executor — now returns `HookResult(decision="deny", reason=...)` on raise or timeout. A crashing `BEFORE_TOOL` gate no longer allows the tool call through by accident.
...
This matches the `GuardrailChain` fail-closed posture and the injection-scanner fail-closed gate from PR #3837 above. See [Hooks → Fail-closed behaviour](/features/hooks#how-it-works).

### Recent Security Hardening (PR #4244)

[PR #4244](https://github.com/MervinPraison/PraisonAI/pull/4244) (fixes [#4243](https://github.com/MervinPraison/PraisonAI/issues/4243)) tightens session-share redaction and closes secret-leak gaps in the redactor.

**What changed:**
- `praisonai session share` now defaults `--redact-level` to **`strict`** (was `standard`) because it publishes an HTML transcript for wider distribution. `session export` still defaults to `standard`.
- Broader secret shapes are recognised at **both** levels: Stripe `sk_live_…` / `sk_test_…` keys, the `access_key` secret word, and `key=value` pairs whose key *contains* a secret word (so `AWS_SECRET_ACCESS_KEY=…` and `STRIPE_SECRET_KEY=…` are caught, not only keys that end in one). Secret values are masked as a whole — no half-masking when a value contains `/`.

**Who is affected:**
Anyone using `praisonai session share` or `session export --sanitise`. Shared transcripts are now redacted more aggressively by default.

**Action required:**
None. On by default. Pass `praisonai session share <id> --redact-level standard` to opt down when the recipient is trusted. See [Session → Share a Session](/docs/cli/session#share-a-session).

### Recent Security Hardening (PR #4248)

[PR #4248](https://github.com/MervinPraison/PraisonAI/pull/4248) enforces sandbox `ResourceLimits` on POSIX and mirrors the async throttle/cancel guards onto the sync `chat()` backend path.

**What changed:**
- The subprocess sandbox for `execute_code` now applies the full `ResourceLimits` bundle via `resource.setrlimit` in a `preexec_fn`. Memory bombs and CPU-bound loops are stopped by the kernel (`RLIMIT_AS`, `RLIMIT_CPU`) instead of only by the wall-clock timeout.
- Sync `Agent.chat(...)` with a managed backend now runs `_rate_limiter.acquire()` and the cancel-token check before delegating, matching `achat()`.

```mermaid
graph LR
    Code[Untrusted code] --> Sub[Subprocess]
    Sub --> Caps[setrlimit caps]
    Caps --> Kernel{Over a cap?}
    Kernel -->|Yes| Killed[🚫 killed]
    Kernel -->|No| Result[✅ result]
...
**Who is affected:**
Anyone using `Agent(execution=ExecutionConfig(code_execution=True, code_mode="safe"))` — i.e. any subprocess-sandboxed `execute_code` call.

**Action required:**
None — on by default on POSIX. On Windows only the timeout applies; a `logging.warning` is emitted at launch to make the gap visible. See [Python Agent → Sandbox Mode](/tools/python_tools#sandbox-mode).

Since PraisonAI PR #5220, PowerShell cmdlets and `cmd` built-ins are gated with the same precision as POSIX commands — including `-EncodedCommand` payloads and `& { … }` scriptblocks. See [Command-Aware Permissions → Windows / PowerShell dialect](/features/command-aware-permissions#windows-powershell-dialect).

### Recent Security Hardening (PR #4966)

[PR #4966](https://github.com/MervinPraison/PraisonAI/pull/4966) (closes [GHSA-5c6w-wwfq-7qqm](https://github.com/advisories/GHSA-5c6w-wwfq-7qqm)) fixes an octal‑encoded loopback bypass in the spider tools SSRF guard.

**What changed:**
- `SpiderTools._host_is_blocked` now consults both `socket.inet_aton` and `socket.getaddrinfo` and blocks if **either** reads the host as internal.
- Previously, `ipaddress.ip_address` refused ambiguous IPv4 forms (`0177.0.0.1`, `127.1`) which then fell through to only the resolver — which reads `0177.0.0.1` as the public `177.0.0.1` and let it through, even though libc-based HTTP clients dial the loopback `127.0.0.1`.

```mermaid
graph LR
    Host[Host from URL] --> A[inet_aton reading]
    Host --> G[getaddrinfo reading]
    A --> Q{Either internal?}
    G --> Q
    Q -->|Yes| Block[🚫 refused]
    Q -->|No| Allow[✅ fetched]
...
**Who is affected:**
Anyone using `scrape_page`, `extract_links`, `crawl`, `extract_text`, or `@url:` mentions. The bypass allowed an LLM-supplied URL to reach loopback services (e.g. `http://0177.0.0.1:<port>/`) that the guard was supposed to block.

**Action required:**
None — on by default, no flag, no opt-in. Verify by calling `scrape_page("http://0177.0.0.1:8765/")` — the tool now returns `{'error': 'Invalid or potentially dangerous URL: ...'}` instead of a fetch result. See [Spider Agent → Built-in URL Safety](/tools/spider_tools#built-in-url-safety).
...
## Recent Security Hardening (PraisonAI #4088)

`Agent.launch()` and `Agents.launch()` now enforce `PRAISONAI_LAUNCH_AUTH_TOKEN`: every request must send `Authorization: Bearer <token>` (or `X-Auth-Token: <token>`) when the token is set, or gets `401`. The launched server binds to `127.0.0.1` by default, and if you bind to a non-loopback host without a token the SDK auto-generates a one-time bearer token and prints it to stdout. See [Agent API Launch → Authentication](/features/agent-api-launch#authentication).
...
## Inter-Agent Provenance Envelope (PR #4915)

[PR #4915](https://github.com/MervinPraison/PraisonAI/pull/4915) hardens the agent↔agent boundary so a sub-agent's output cannot drive the parent as a first-person instruction.

### What changed
`praisonaiagents.session` now exports `wrap_inter_agent()` and `MessageOrigin`. `create_subagent_tool(...)` gained a `trusted_sources` parameter and now envelopes every sub-agent's returned text as inter-agent data by default. The envelope is idempotent, bounded (`max_chars=8000`), and honours a per-source trust override.

### Who is affected
Anyone who reads sub-agent output as free-form text. Programs that pattern-match on the raw first line should either strip the envelope, opt the source into `trusted_sources`, or read the wrapped body.

### Migration
None required for the default behaviour. To opt out for a trusted upstream:
...
See [Inter-Agent Provenance](/features/inter-agent-provenance) for the full story.
...
<Tip>
Never paste a raw `praisonai session export` into a public bug report, gist, or ticket — the transcript is verbatim. Use `praisonai session export <id> --sanitise` (or `--redact-level strict` for wider sharing) to mask secrets, absolute paths, and the working directory before you share. `praisonai session share <id>` is redaction-mandatory and now defaults to `--redact-level strict` — the tightest net is on for anything you publish through `share`. Pass `--redact-level standard` only when the recipient is trusted. See [Sanitise before sharing](/docs/cli/session#sanitise-before-sharing).
</Tip>

### Webhook Verification

Bot adapters that accept inbound webhooks verify HMAC signatures **fail-closed** by default — missing secrets or invalid signatures return HTTP 401. Set `PRAISONAI_INSECURE_WEBHOOKS=true` only for local development. See [Webhook Verification](/features/webhook-verification).
...
<Tabs>
  ...
  <Tab title="Staging">
    ...
    Sensitive keys (api keys, tokens, passwords) in tool inputs are redacted by default; see [Audit Logging](/features/audit-logging#secret-redaction) to customise.
  </Tab>
...
<CardGroup cols={2}>
  ...
  <Card title="Protected Paths" icon="lock" color="#f59e0b" href="/features/protected-paths">
    Blocks modification of sensitive system files and directories
  </Card>
  <Card title="Audit Logging" icon="clipboard-list" color="#22c55e" href="/features/audit-logging">
    Thread-safe JSONL audit log for tool calls
  </Card>
  <Card title="Untrusted Text Neutralisation" icon="broom" color="#189AB4" href="/security#recent-security-hardening-praisonai-3315">
    `neutralize_untrusted_text` sanitises platform metadata before it reaches a prompt
  </Card>
  <Card title="Untrusted Request Fencing" icon="shield-check" color="#189AB4" href="/features/untrusted-request-fencing">
    Inbound webhook / hook payloads wrapped in `<external_request_payload>` — the ingress counterpart to `<external_tool_result>`
  </Card>
</CardGroup>
...
### Secret-file read gate

The core `PermissionManager` gates reads of common credential-file globs (`.env`, `*.env`, `.env.*`, `*.pem`, `*.key`, `id_rsa`, `id_dsa`, `id_ecdsa`, `id_ed25519`, `*.pfx`, `*.p12`) to `ask` by default. Safe example/template siblings (`*.env.example`, `*.env.sample`, `*.env.template`, `*.example`, `*.sample`, `*.template`) are **not** gated. A broad `"read:*": "allow"` rule **does not** authorise these — you must opt in with a secret-specific rule (`"read:*.env": "allow"`), a persistent approval, or an explicit deny to harden. See [Permissions → Built-in Secret-File Read Gate](/docs/features/permissions#built-in-secret-file-read-gate).
...
**Reference:** See [GCP Deploy Documentation](/deploy/cli/gcp) for implementation details.
...
- **Availability probe never prints values.** `praisonai gateway doctor` reports each field as `available` / `configured-but-unavailable` / `configured` / `missing` without revealing the value, so operators can validate wiring before start-up. `gateway doctor --fix` auto-mints a strong token when the current one is weak/missing and re-validates, without printing the value — see [Gateway CLI › Auto-repair](/docs/features/gateway-cli#auto-repair-with-fix).
- **Weak / placeholder secrets are rejected.** Publicly-known placeholder secrets (`change-me`, `your-token-here`, the literal `$(openssl rand -hex 16)`) are rejected on external binds — see [Weak Secret Guard](/docs/features/gateway-bind-aware-auth#weak-placeholder-secret-guard).
...
Pairing codes **and** the inline Approve/Deny callback buttons sign with this **same** persisted secret at `~/.praisonai/gateway/.gateway_secret` (mode `0600`), so both survive gateway restarts by default — no operator action required. `PRAISONAI_CALLBACK_SECRET` is only needed to share one secret across multiple hosts/replicas (or to override for rotation). See [Bot Pairing → Secret Management](/docs/features/bot-pairing#secret-management) for pairing codes and [Interactive Bot Actions](/features/interactive-bot-actions) for the callback-button flow.

[PR #3974](https://github.com/MervinPraison/PraisonAI/pull/3974) further hardens the pairing store by hashing codes at rest and adding a store-level brute-force lockout. Legacy plaintext stores still load, so there is nothing to run — see [Bot Security → Gateway Pairing](/best-practices/bot-security#gateway-pairing).
...
<CardGroup cols={2}>
  ...
  <Card title="Audit Logging" icon="clipboard-list" href="/features/audit-logging">
    Thread-safe JSONL audit log for tool calls.
  </Card>
</CardGroup>
