...
Bot adapters that accept inbound webhooks (Linear, AgentMail, WhatsApp Cloud) are **fail-closed** by default — every request must carry a valid HMAC signature or it is rejected with HTTP 401.

- Set the per-platform secret env var (`LINEAR_WEBHOOK_SECRET`, `AGENTMAIL_WEBHOOK_SECRET`, `WHATSAPP_APP_SECRET`) before starting the bot.
- Never set `PRAISONAI_INSECURE_WEBHOOKS=true` outside local development — it disables all signature checks globally.
